...
RSpec.describe Voice::AgentDispatcher do
  fab!(:user)
  fab!(:room, :voice_room)
  fab!(:integration) { Fabricate(:voice_agent_integration, rooms: [room], role: 1) }
  ...
  before do
    ...
    stub_request(
      :post,
      "https://livekit.example.com/twirp/livekit.AgentDispatchService/ListDispatch",
    ).to_return { { status: 200, body: { agentDispatches: [@dispatch].compact }.to_json } }
    @delete =
      stub_request(
        :post,
        "https://livekit.example.com/twirp/livekit.AgentDispatchService/DeleteDispatch",
  ...
  describe ".dispatch!" do
    it "maps the provider-attested job identity to the authorized bot and applies its permissions" do
      expect(described_class.dispatch!(integration:, room:, agent_name: "assistant")).to eq(
        "AD_test",
      )
    ...
    it "rejects invitations to empty, mesh, unscoped, and excluded rooms" do
      other_room = Fabricate(:voice_room)
      expect {
        described_class.dispatch!(integration:, room: other_room, agent_name: "assistant")
      }.to raise_error(Voice::AgentManager::AuthorizationError)
      Voice::ParticipantTracker.remove(room.id, user.id)
      expect {
        described_class.dispatch!(integration:, room:, agent_name: "assistant")
      }.to raise_error(Voice::AgentManager::AuthorizationError)
      Voice::ParticipantTracker.add(room.id, user.id)
      ...
      Voice::ParticipantTracker.clear_transport_pin(room.id)
      Voice::ParticipantTracker.pin_transport!(room.id, "livekit")
      Voice::AgentManager.exclude!(room:, user_id: integration.bot_user_id)
      expect {
        described_class.dispatch!(integration:, room:, agent_name: "assistant")
      }.to raise_error(Voice::AgentManager::AuthorizationError)
    ...
    it "applies listener permissions and targets the mapped identity during moderation" do
      integration.update!(role: 0)
      described_class.dispatch!(integration:, room:, agent_name: "assistant")
      Voice::AgentManager.reconcile(room)
      ...
      expect(
        a_request(
          ...
          body:
            hash_including(
              "identity" => "agent-test",
              "permission" => hash_including("canPublish" => false, "canSubscribe" => true),
            ),
        ),
      ).to have_been_made.once
      integration.update!(role: 1)
      Voice::Livekit::RoomServiceClient.update_participant(room, integration.bot_user)
      expect(
        a_request(
          :post,
          "https://livekit.example.com/twirp/livekit.RoomService/UpdateParticipant",
          ...
          body:
            hash_including(
              "identity" => "agent-test",
              "permission" => hash_including("canPublish" => true),
            ),
        ),
      ).to have_been_made.once
    end
    ...
    it "recovers and deletes a remotely created dispatch after its response times out" do
      ...
      expect {
        described_class.dispatch!(integration:, room:, agent_name: "assistant")
      }.to raise_error(described_class::DispatchError)
  ...
  describe ".cancel" do
    it "removes local admission immediately and retries failed dispatch deletion after exclusion" do
      described_class.dispatch!(integration:, room:, agent_name: "assistant")
      Voice::AgentManager.reconcile(room)
      @delete =
        stub_request(
