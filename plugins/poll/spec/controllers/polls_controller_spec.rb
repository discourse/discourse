...
describe ::DiscoursePoll::PollsController do
  ...
  describe "#vote" do
    ...
    it "works" do
      ...
      xhr :put, :vote, { post_id: poll.id, poll_id: 1, options: ["5c24fc1df56d764b550ceae1b9319125"] }

      expect(response).to be_success
      json = ::JSON.parse(response.body)
      expect(json["poll"]["id"]).to eq("1")
    ...
    it "requires at least 1 valid option" do
      xhr :put, :vote, { post_id: poll.id, poll_id: 1, options: ["A", "B"] }
    ...
    it "supports vote changes" do
      ...
      xhr :put, :vote, { post_id: poll.id, poll_id: 1, options: ["e89dec30bbd9bf50fabf6a05b4324edf"] }
      expect(response).to be_success
      json = ::JSON.parse(response.body)
      expect(json["poll"]["voters"]).to eq(1)
    ...
    it "ensures topic is not archived" do
      ...
      xhr :put, :vote, { post_id: poll.id, poll_id: 1, options: ["A"] }
      expect(response).not_to be_success
      json = ::JSON.parse(response.body)
    ...
    it "ensures polls are associated with the post" do
      xhr :put, :vote, { post_id: Fabricate(:post).id, poll_id: "foobar", options: ["A"] }
      expect(response).not_to be_success
      json = ::JSON.parse(response.body)
      expect(json["errors"][0]).to eq(I18n.t("poll.no_polls_associated_with_this_post"))
    end

    it "checks the id of the poll" do
      xhr :put, :vote, { post_id: poll.id, poll_id: "foobar", options: ["A"] }
      expect(response).not_to be_success
      json = ::JSON.parse(response.body)
      expect(json["errors"][0]).to eq(I18n.t("poll.no_poll_with_this_id", id: "foobar"))
    end

    it "ensures poll is open" do
      closed_poll = Fabricate(:post, raw: "[poll status=closed]\n- A\n- B\n[/poll]")
      xhr :put, :vote, { post_id: closed_poll.id, poll_id: 1, options: ["A"] }
      expect(response).not_to be_success
      json = ::JSON.parse(response.body)
    ...
    it "doesn't discard anonymous votes when someone votes" do
      default_poll = poll.custom_fields["polls"]["1"]
    ...
    it "tracks the users ids for public polls" do
      ...
      body = { post_id: public_poll.id, poll_id: 1 }
  ...
  describe "#toggle_status" do
    ...
    it "works for OP" do
      ...
      xhr :put, :toggle_status, { post_id: poll.id, poll_id: 1, status: "closed" }
      expect(response).to be_success
      json = ::JSON.parse(response.body)
