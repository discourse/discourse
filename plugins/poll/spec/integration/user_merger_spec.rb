...
RSpec.describe UserMerger do
  ...
  it "leaves no votes from the source user" do
  ...
  it "keeps the target user's vote when both users voted in the same poll" do
  ...
  it "uses the source user's vote for a ranked-choice poll" do
    Fabricate(
      :poll_vote,
