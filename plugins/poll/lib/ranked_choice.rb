...
class DiscoursePoll::RankedChoice
  ...
  class << self
    def outcome(poll_id)
      options =
        PollOption.where(poll_id: poll_id).map { |hash| { id: hash.digest, html: hash.html } }
    ...
    def run(starting_votes, options)
