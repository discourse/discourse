...
class DiscoursePoll::Poll
  ...
  class << self
    ...
    def remove_vote(user, post_id, poll_name)
      DiscoursePoll::Poll.change_vote(user, post_id, poll_name) do |poll|
        PollVote.where(poll: poll, user: user).delete_all
      end
    end

    def toggle_status(user, post_id, poll_name, status, raise_errors = true)
      Poll.transaction do
    ...
    def preload_serialized_voters!(polls, opts = {})
    ...
    def grouped_poll_results(user, post_id, poll_name, user_field_name)
      post = Post.find_by(id: post_id)
