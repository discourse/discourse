...
module DiscoursePoll
  class PollsUpdater
    ...
    class << self
      ...
      def delete_polls(post, names)
        ::Poll.where(post: post, name: names).destroy_all
      end

      def create_polls(post, polls_hash)
        polls_hash.each { |name, poll| Poll.create!(post.id, poll) }
      end

      def update_existing_polls(post, polls, edit_window)
        has_changed = false
      ...
      def existing_polls_for(post)
        ::Poll.includes(:poll_votes, :poll_options).where(post: post)
      end

      def process_poll_update(post, old_poll, new_poll_hash, edit_window)
      ...
      def votes_present_and_restricted?(old_poll, dynamic_flag)
        old_poll.poll_votes.size > 0 && !dynamic_flag
      end

      def edit_window_expired?(old_poll, edit_window)
        edit_window > 0 && old_poll.created_at < edit_window.minutes.ago
      end

      def build_edit_window_error(poll_name, edit_window)
      ...
      def apply_poll_attribute_updates(old_poll, candidate)
        POLL_ATTRIBUTES.each do |attr|
          old_poll.public_send("#{attr}=", candidate.public_send(attr))
        end
        old_poll.save!
      end

      def update_poll_options(old_poll, new_options, dynamic_flag)
        if dynamic_flag
      ...
      def extract_poll_attributes(new_poll, old_poll)
      ...
      def compute_dynamic_flag(new_poll, old_poll)
      ...
      def dynamic_update_options(old_poll, new_poll_options)
      ...
      def nondynamic_replace_options(old_poll, new_poll_options)
      ...
      def update_post_custom_fields(post)
      ...
      def is_different?(old_poll, new_poll, new_options)
        POLL_ATTRIBUTES.each do |attr|
