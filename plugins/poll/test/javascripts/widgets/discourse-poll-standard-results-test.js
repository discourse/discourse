...
module(
  ...
  function (hooks) {
    ...
    test("options showing absolute number", async function (assert) {
      this.set("showPercentage", false);
      this.set(
        "poll",
        EmberObject.create({
          options: [{ votes: 5 }, { votes: 4 }],
    ...
    test("multiple options showing absolute number", async function (assert) {
      this.set("showPercentage", false);
      this.set("isMultiple", true);
      this.set(
        "poll",
      ...
      assert.strictEqual(percentages[3].innerText.split(" ")[0], "1");
