...
module DiscourseDataExplorer
  class ReportGenerator
    class << self
      def generate(query_id, query_params, recipients, opts = {})
        ...
        table =
          ResultToMarkdown.convert(
            result[:pg_result],
            render_url_columns: opts[:render_url_columns],
          )

        build_report_pms(query, table, recipients, attach_csv: opts[:attach_csv], result:)
      end

      def generate_post(query_id, query_params, opts = {})
        query = DiscourseDataExplorer::Query.find(query_id)
        return {} if !query
      ...
      def build_report_pms(query, table = "", targets = [], attach_csv: false, result: nil)
        pms = []
      ...
      def build_report_post(query, table = "", attach_csv: false, result: nil)
        upload = create_csv_upload(query, result) if attach_csv
      ...
      def create_csv_upload(query, result)
        tmp_filename =
      ...
      def filter_recipients_by_query_access(recipients, query, users_from_group: false)
