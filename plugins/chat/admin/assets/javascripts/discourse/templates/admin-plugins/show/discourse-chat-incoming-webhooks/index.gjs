...
<div class="discourse-chat-incoming-webhooks admin-detail">
  <DPageSubheader @titleLabel={{i18n "chat.incoming_webhooks.title"}} @descriptionLabel={{i18n "chat.incoming_webhooks.instructions"}}>
    <:actions as |actions|>
      <actions.Primary @label="chat.incoming_webhooks.new" @title="chat.incoming_webhooks.new" @route="adminPlugins.show.discourse-chat-incoming-webhooks.new" @routeModels="chat" @icon="plus" class="admin-incoming-webhooks-new" />
    </:actions>
  </DPageSubheader>

  <div class="incoming-chat-webhooks">
    {{#if @controller.model.incoming_chat_webhooks}}
      <AdminChatIncomingWebhooksList @webhooks={{@controller.model.incoming_chat_webhooks}} />
    {{else}}
