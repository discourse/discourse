...
RSpec.describe Chat::GuardianExtensions do
  ...
  it "only allows staff to create channels" do
  ...
  describe "#can_post_in_chatable?" do
    ...
    context "when channel is a category channel" do
      context "when allowed category ids are supplied without a chatable" do
        it "returns false" do
      ...
      context "when allowed category ids are supplied by an anonymous user" do
        it "returns false" do
      ...
      context "when allowed category ids are supplied by an admin" do
        it "returns true" do
      ...
      context "when user is a shadow account" do
        ...
        it "allows a shadow account to chat when its master account can chat" do
        ...
        it "doesn't allow a shadow account to chat when anonymous chat is disabled" do
          SiteSetting.allow_chat_in_anonymous_mode = false
          anonymous = AnonymousShadowCreator.get(user)
          expect(anonymous.id).to be_present
          expect(Guardian.new(anonymous).can_chat?).to eq(false)
          expect(Guardian.new(user).can_chat?).to eq(true)
        ...
        it "doesn't allow a shadow account whose master cannot chat when anonymous chat is disabled" do
          SiteSetting.allow_chat_in_anonymous_mode = false
          anonymous = AnonymousShadowCreator.get(non_chatter)
          expect(anonymous.id).to be_present
          expect(Guardian.new(anonymous).can_chat?).to eq(false)
      ...
      context "when no chatable or allowed category ids are supplied" do
        it "returns false" do
          expect(guardian).not_to be_able_to_post_in_chatable(nil)
        end
      end

      context "when an anonymous user omits allowed category ids" do
        it "returns false" do
          expect(Guardian.new).not_to be_able_to_post_in_chatable(channel.chatable)
        end
      end

      context "when an admin omits allowed category ids" do
        it "returns true" do
          guardian = Fabricate(:admin).guardian
      ...
      context "when the user can post without explicit allowed category ids" do
        it "returns true" do
          expect(guardian).to be_able_to_post_in_chatable(channel.chatable)
        end
      end
  ...
  describe "#can_restore_chat?" do
    ...
    context "when user is not owner of the message" do
      ...
      context "when category is restricted" do
        ...
        it "allows a group moderator to restore when group moderation is enabled" do
