...
RSpec.describe Chat::Action::SearchMessage::ProcessSearchQuery do
  subject(:result) { described_class.call(query:, messages:, guardian:) }
  ...
  context "with no filters" do
    let(:query) { "hello world" }
    let(:ts_query) do
      Search.ts_query(term: Search.prepare_data(query), ts_config: Search.ts_config)
    end
    ...
    it "returns the proper messages relation" do
  ...
  context "with @username filter" do
    ...
    context "with username containing special characters" do
      ...
      let(:query) { "@user_underscore message" }
  ...
  context "with #channel filter" do
    ...
    let(:query) { "##{channel.slug} message" }
    ...
    it "removes #channel from processed_query" do
      expect(result.processed_query).to eq("message")
    end
    ...
    context "when channel slug is case insensitive" do
      let(:query) { "##{channel.slug.upcase} message" }
    ...
    context "with channel slug containing special characters" do
      ...
      let(:messages) { Chat::Message.joins(:chat_channel) }
      let(:query) { "#my-special-channel message" }
  ...
  context "with batch filtering" do
    ...
    fab!(:alice_msg) do
      Fabricate(:chat_message, chat_channel: channel, user: alice, message: "hello")
    end
