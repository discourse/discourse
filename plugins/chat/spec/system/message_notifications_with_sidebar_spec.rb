...
RSpec.describe "Message notifications - with sidebar" do
  ...
  before do
    PageObjects::Pages::Chat.new
    PageObjects::Pages::ChatChannel.new
    PageObjects::Pages::ChatThread.new
    PageObjects::Pages::ChatSidebar.new
  ...
  context "with public channel" do
    ...
    context "when a message is created" do
      it "doesn't show anything when the current user is not a channel member" do
        visit("/")
    ...
    context "when member of the channel" do
      ...
      context "when user chat_header_indicator_preference is set to 'never'" do
        before do
          current_user.user_option.update!(
            chat_header_indicator_preference: UserOption.chat_header_indicator_preferences[:never],
          )
        end

        it "doesn't show a header indicator for a new message" do
          visit("/")
          create_message(channel: channel_1, creator: user_1)
      ...
      context "when user chat_header_indicator_preference is set to 'dm_and_mentions'" do
        ...
        it "shows a header indicator for a new message with a mention" do
