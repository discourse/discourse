...
module PageObjects
  module Pages
    class ChatChannel < PageObjects::Pages::Base
      ...
      def send_message(text = nil, check_message_presence: true, persisted: true, staged: false)
        text ||= Faker::Lorem.characters(number: SiteSetting.chat_minimum_message_length)
        text = text.chomp if text.present? # having \n on the end of the string counts as an Enter keypress
        composer.fill_in(with: text)
        click_send_message
        expect(messages).to have_message(text: text, persisted:, staged:) if check_message_presence
        click_composer
