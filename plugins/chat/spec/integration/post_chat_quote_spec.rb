...
describe "chat bbcode quoting in posts" do
  ...
  it "renders with the reactions attribute" do
    ...
    expect(post.cooked.chomp).to eq(<<~COOKED.chomp)
      ...
      <img width="20" height="20" src="/images/emoji/twitter/+1.png?v=#{EMOJI_VERSION}" title="+1" loading="lazy" alt="+1" class="emoji"> 1</div>
      <div class="chat-transcript-reaction">
      <img width="20" height="20" src="/images/emoji/twitter/heart.png?v=#{EMOJI_VERSION}" title="heart" loading="lazy" alt="heart" class="emoji"> 2</div>
      </div>
      </div>
      </div>
    COOKED
