...
after_initialize do
  ...
  on(:chat_message_created) do |chat_message|
    Chat::DiscourseAutomation::EventHandlers.handle_chat_message_created_edited(
      chat_message,
      :create,
    )
  end
  ...
  on(:chat_message_edited) do |chat_message|
    Chat::DiscourseAutomation::EventHandlers.handle_chat_message_created_edited(chat_message, :edit)
  end

  add_automation_triggerable(Chat::DiscourseAutomation::Triggers::CHAT_MESSAGE_CREATED_EDITED) do
    field :action_type,
          component: :choices,
          extra: {
            content: [
