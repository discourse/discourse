    ...
    def self.call(thread_ids:)
      ...
      thread_messager_stats.each do |thread_messager_stat|
        ...
        if thread_participants[thread_id][:users].length < 2 &&
             thread_messager_stat.user_id != most_recent_messagers[thread_id][:id]
          thread_participants[thread_id][:users].push(
            {
              id: thread_messager_stat.user_id,
              username: thread_messager_stat.username,
              name: thread_messager_stat.name,
              uploaded_avatar_id: thread_messager_stat.uploaded_avatar_id,
            },
          )
        end
      ...
      most_recent_messagers.each do |thread_id, user|
