...
module Chat
  ...
  class SearchChatable
    ...
    def search_users(params, guardian)
      ...
      escaped_prefix = User.connection.quote("#{filter_term}%")

      user_search.select(Arel.sql(<<~SQL)).order("match_quality ASC")
          users.*,
          CASE
    ...
    def search_groups(params, guardian)
      ...
      escaped_prefix = Group.connection.quote("#{filter_term}%")

      Group
        .visible_groups(guardian.user)
        .members_visible_groups(guardian.user)
        .where(<<~SQL, exact: filter_term, prefix: "#{filter_term}%", partial: "%#{filter_term}%")
