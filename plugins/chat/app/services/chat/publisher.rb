...
module Chat
  module Publisher
    class << self
      def new_messages_message_bus_channel(chat_channel_id)
        "#{root_message_bus_channel(chat_channel_id)}/new-messages"
      end

      def root_message_bus_channel(chat_channel_id)
        "/chat/#{chat_channel_id}"
      end

      def thread_message_bus_channel(chat_channel_id, thread_id)
        "#{root_message_bus_channel(chat_channel_id)}/thread/#{thread_id}"
      end

      def calculate_publish_targets(channel, message)
      ...
      def allow_publish_to_thread?(channel, message)
        channel.threading_enabled || message.thread&.force
      end

      def publish_new!(chat_channel, chat_message, staged_id)
      ...
      def publish_thread_original_message_metadata!(thread)
        preview =
      ...
      def publish_thread_created!(chat_channel, chat_message, thread_id)
        publish_to_channel!(
          chat_channel,
      ...
      def user_has_threads_message_bus_channel(user_id)
        "/chat/user-has-threads/#{user_id}"
      end

      def publish_user_has_threads!(user)
        MessageBus.publish(
      ...
      def publish_processed!(chat_message)
      ...
      def publish_edit!(chat_channel, chat_message)
        message_bus_targets = calculate_publish_targets(chat_channel, chat_message)
        publish_to_targets!(
          message_bus_targets,
          chat_channel,
      ...
      def publish_refresh!(chat_channel, chat_message)
        message_bus_targets = calculate_publish_targets(chat_channel, chat_message)
        publish_to_targets!(
          message_bus_targets,
          chat_channel,
      ...
      def publish_reaction!(chat_channel, chat_message, action, user, emoji)
        message_bus_targets = calculate_publish_targets(chat_channel, chat_message)
        publish_to_targets!(
          message_bus_targets,
          chat_channel,
      ...
      def publish_presence!(chat_channel, user, typ)
        raise NotImplementedError
      end

      def publish_delete!(chat_channel, chat_message)
        message_bus_targets = calculate_publish_targets(chat_channel, chat_message)
      ...
      def publish_bulk_delete!(chat_channel, deleted_message_ids)
      ...
      def publish_restore!(chat_channel, chat_message)
        message_bus_targets = calculate_publish_targets(chat_channel, chat_message)
        publish_to_targets!(
          message_bus_targets,
          chat_channel,
      ...
      def publish_flag!(chat_message, user, reviewable, score)
      ...
      def publish_to_channel!(channel, payload)
        MessageBus.publish(
          root_message_bus_channel(channel.id),
          payload.as_json,
          permissions(channel),
      ...
      def publish_to_targets!(targets, channel, payload, permissions: nil)
      ...
      def serialize_message_with_type(chat_message, type, options = {})
      ...
      def user_tracking_state_message_bus_channel(user_id)
        "/chat/user-tracking-state/#{user_id}"
      end

      def publish_user_tracking_state!(user, channel, message)
        data = {
      ...
      def bulk_user_tracking_state_message_bus_channel(user_id)
        "/chat/bulk-user-tracking-state/#{user_id}"
      end

      def publish_bulk_user_tracking_state!(user, channel_last_read_map)
        tracking_data =
      ...
      def new_mentions_message_bus_channel(chat_channel_id)
        "/chat/#{chat_channel_id}/new-mentions"
      end

      def kick_users_message_bus_channel(chat_channel_id)
        "/chat/#{chat_channel_id}/kick"
      end

      def publish_new_mention(user_id, chat_channel_id, chat_message_id)
        MessageBus.publish(
      ...
      def publish_kick_users(channel_id, user_ids)
        MessageBus.publish(
      ...
      def publish_pin!(chat_channel, chat_message, pin)
        publish_to_channel!(
          chat_channel,
          {
            type: :pin,
      ...
      def publish_unpin!(chat_channel, chat_message, unpinned_by)
        publish_to_channel!(
          chat_channel,
          {
            type: :unpin,
      ...
      def publish_notice(user_id:, channel_id:, text_content: nil, type: nil, data: nil)
