...
module Chat
  class AutoJoinChannels
    ...
    def create_memberships(contract:)
      ...
      user_sql = contract.user_id ? "AND u.id = #{contract.user_id}" : ""
      channel_sql = contract.channel_id ? "AND cc.id = #{contract.channel_id}" : ""
      category_sql = contract.category_id ? "AND c.id = #{contract.category_id}" : ""
