...
module("Discourse Chat | Component | chat-channel-status", function (hooks) {
  ...
  test("renders nothing when channel is opened", async function (assert) {const self = this;

    this.channel = new ChatFabricators(getOwner(this)).channel();

    await render(<template><ChatChannelStatus @channel={{self.channel}} /></template>);

    assert.dom(".chat-channel-status").doesNotExist();
  });

  test("defaults to long format", async function (assert) {const self = this;
  ...
  test("accepts a format argument", async function (assert) {const self = this;
    ...
    await render(
      <template><ChatChannelStatus @channel={{self.channel}} @format="short" /></template>
    );
  ...
  test("renders the correct icon", async function (assert) {const self = this;
  ...
  test("renders archive status", async function (assert) {const self = this;
