...
module("Discourse Chat | Component | chat-message-text", function (hooks) {
  ...
  test("yields", async function (assert) {const self = this;
  ...
  test("shows collapsed", async function (assert) {const self = this;
    ...
    await render(
      <template><ChatMessageText @cooked={{self.cooked}} @uploads={{self.uploads}} /></template>
    );

    assert.dom(".chat-message-collapser").exists();
  });

  test("does not collapse a non-image onebox", async function (assert) {const self = this;

    this.set("cooked", '<p><a href="http://cat1.com" class="onebox"></a></p>');

    await render(<template><ChatMessageText @cooked={{self.cooked}} /></template>);

    assert.dom(".chat-message-collapser").doesNotExist();
  });

  test("shows edits - regular message", async function (assert) {const self = this;

    this.set("cooked", "<p></p>");

    await render(
      <template><ChatMessageText @cooked={{self.cooked}} @edited={{true}} /></template>
    );

    assert.dom(".chat-message-edited").exists();
  });

  test("shows edits - collapsible message", async function (assert) {const self = this;
