...
module(
  ...
  function (hooks) {
    ...
    test("when setting is set on 0", async function (assert) {const self = this;

      this.channel = new ChatFabricators(getOwner(this)).channel();
      this.siteSettings.chat_channel_retention_days = 0;

      await render(
        <template><ChatRetentionReminderText @channel={{self.channel}} /></template>
      );
      ...
      await render(
        <template><ChatRetentionReminderText @channel={{self.channel}} @type="short" /></template>
      );
    ...
    test("when channel is a public channel", async function (assert) {const self = this;
    ...
    test("when channel is a DM channel", async function (assert) {const self = this;
