  ...
  class ChannelFetcher
    ...
    def self.secured_public_channel_search(guardian, options = {})
      ...
      if options[:filter].present?
        ...
        escaped_prefix = Chat::Channel.connection.quote("#{filter_term}%")
        category_filter =
        ...
        channels = channels.select(Arel.sql(<<~SQL))
          chat_channels.*,
          CASE
        ...
        channels = channels.order("match_quality ASC, chat_channels.name ASC, categories.name ASC")
      end
    ...
    def self.secured_direct_message_channels_search(user_id, guardian, options = {})
      ...
      if options[:filter]
        ...
        query = query.select(Arel.sql(<<~SQL))
          chat_channels.*,
          MIN(
            CASE
