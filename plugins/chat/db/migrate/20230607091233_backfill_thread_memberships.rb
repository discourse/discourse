...
class BackfillThreadMemberships < ActiveRecord::Migration[7.0]
  def up
    sql = <<~SQL
    INSERT INTO user_chat_thread_memberships (user_id, thread_id, notification_level, last_read_message_id, created_at, updated_at)
    SELECT thread_messager_stats.user_id, thread_messager_stats.thread_id, 2, (
      SELECT id FROM chat_messages
      WHERE thread_id = thread_messager_stats.thread_id
    ...
    LEFT JOIN user_chat_thread_memberships ON user_chat_thread_memberships.thread_id = thread_messager_stats.thread_id
    AND user_chat_thread_memberships.user_id = thread_messager_stats.user_id
