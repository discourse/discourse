...
3. on developer.amazon.com, Under "Web Settings", set the following settings:
   ...
   - Allowed Return URLs: http://localhost:3000/auth/amazon/callback
...
6. start your local rails server as following:
   - ```% REDIRECT_URL_ORIGIN=http://localhost:3000 rails s```
7. at this point, when you go to localhost:3000, you should be redirected to /login and be presented with a "Login With Amazon" button. Clicking on it should
take you to an authentication flow on amazon.com and redirect you in the end, to a logged in session at localhost: 3000.
