...
describe DiscourseAi::Automation::LlmAgentTriage do
  fab!(:user)
  fab!(:bot_user, :user)
  ...
  fab!(:ai_agent) do
    agent =
      Fabricate(
        :ai_agent,
        name: "Triage Helper",
        description: "A agent that helps with triaging posts",
        system_prompt: "You are a helpful assistant that triages posts",
        default_llm: llm_model,
      )

    # Create the user for this agent
    agent.update!(user_id: bot_user.id)
    agent
  end

  let(:automation) do
    Fabricate(:automation, name: "my automation", script: "llm_agent_triage", enabled: true)
  end
  ...
  before do
    ...
    add_automation_field("agent", ai_agent.id, type: "choices")
    add_automation_field("whisper", false, type: "boolean")
  end

  it "can respond to a post using the specified agent" do
    ...
    # Verify that the response was posted by the agent's user
  ...
  it "hides thinking output when the agent is configured to do so" do
    post = Fabricate(:post, raw: "This is a test post that needs triage")
  ...
  it "interacts correctly with PMs" do
    ...
    # should not inject agent into allowed users
    expect(topic.topic_allowed_users.pluck(:user_id).sort).to eq(original_user_ids.sort)
  end

  describe "LLM Agent Triage with Chat Message Creation" do
    fab!(:user)
    fab!(:bot_user, :user)
    ...
    before do
      ...
      ai_agent.update!(tools: ["custom-#{custom_tool.id}"])
