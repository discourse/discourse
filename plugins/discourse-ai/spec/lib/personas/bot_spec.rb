...
RSpec.describe DiscourseAi::Personas::Bot do
  ...
  describe "#reply" do
    ...
    context "with max_turn_tokens token budget" do
      ...
      let(:persona_class) { persona_record.class_instance }
      ...
      it "sets tool_choice to :none when 85% of budget is consumed" do
        ...
        klass = big_budget_persona.class_instance
        ...
        DiscourseAi::Completions::Llm.with_prepared_responses(responses) do
          bot = described_class.as(bot_user, persona: klass.new)
          context =
            DiscourseAi::Personas::BotContext.new(
              messages: [{ type: :user, content: "List categories" }],
            )
      ...
      it "preserves legacy MAX_COMPLETIONS behavior when max_turn_tokens is nil" do
        no_budget_persona =
          Fabricate(:ai_persona, max_turn_tokens: nil, tools: [["ListCategories", nil, false]])

        klass = no_budget_persona.class_instance
      ...
      it "forces a final text-only call with budget hint when budget exhausted after tool execution" do
        ...
        klass = small_budget_persona.class_instance
      ...
      it "forces a final text-only call in legacy MAX_COMPLETIONS mode too" do
        ...
        klass = one_turn_persona.class_instance
    ...
    describe "#maybe_compress_context" do
      ...
      it "uses persona compression_threshold to control when compression triggers" do
        persona_record.update!(compression_threshold: 50)
        persona_class_with_threshold = persona_record.class_instance
        bot = described_class.as(bot_user, persona: persona_class_with_threshold.new)
