...
module PageObjects
  module Components
    class AiPostImageDescriptions < PageObjects::Components::Base
      PREVIEW_EDITOR_BUTTON = ".d-editor-preview .ai-post-image-description-editor__button"
      MODAL = ".ai-post-image-description-editor-modal"

      def has_post_image_count?(post, count:)
        post_component(post).cooked_content.has_css?("img", count: count, visible: :all)
      end

      def has_image_description?(post, image:, description:)
      ...
      def edit_preview_image_description(image:, description:)
