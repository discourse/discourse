    ...
    class Discover < Agent
      ...
      def system_prompt
        <<~PROMPT.strip
          ...
          When answerable, select the smallest sufficient set of source references that materially support the answer, normally one or two and never more than related_count. Each selected source must support a claim in the answer. Prefer authoritative guides and documentation over support questions, feature requests, and discussions that only repeat the query. Use category as a relevance signal. For broad queries, prefer general guides over discussions about one specific product or plugin when both could help. Use last_updated_at to judge freshness only when the query is time-sensitive; newer does not automatically mean better.

          Before returning an answer, silently check every factual claim and procedure against the selected candidates. Remove anything the selected candidates do not support. If this leaves no useful answer, set answerable to false. Never replace a missing procedure with adjacent advice.
