  ...
  module Agents
    module Tools
      class FlagPost < Tool
        ...
        def flag_type
          agent_flag_type = agent_option(:flag_type)
          (agent_flag_type.presence || feature_context[:flag_type].presence || "review").to_s
        end
        ...
        def agent_option(name)
          return nil if !agent_options.is_a?(Hash)
          return nil if !agent_options.key?(name)
