  ...
  module AiModeration
    class SpamScanner
      ...
      class << self
        ...
        def approved_from_review_queue?(post)
          ReviewableQueuedPost.approved.exists?(target: post)
        end

        def ensure_flagging_user!
        ...
        def ensure_safe_flagging_user!(user)
        ...
        def flag_post_for_scanning(post)
        ...
        def scanned_max_times?(post)
          AiSpamLog.where(post_id: post.id).count >= 3
        end

        def significant_change?(previous_version, current_version)
          return true if previous_version.nil? # First edit should be scanned

          # Use Discourse's built-in levenshtein implementation
          distance =
            ScreenedEmail.levenshtein(
              previous_version.to_s[0...1000],
              current_version.to_s[0...1000],
            )

          distance >= MINIMUM_EDIT_DIFFERENCE
        end

        def test_post(post, custom_instructions: nil, llm_id: nil)
        ...
        def perform_scan(post, triggering_user_id: nil)
          return if !should_scan_post?(post)
        ...
        def fix_spam_scanner_not_admin
        ...
        def handle_spam(post, log, triggering_user_id: nil)
        ...
        def post_has_existing_flag?(post)
