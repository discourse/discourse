...
module DiscourseAi::AiBot::UserFlair
  ...
  class << self
    def sync_all!
      ...
      group = Group.find_by(name: DiscourseAi::AiBot::UserFlair::GROUP_NAME)
    ...
    def sync_user!(user, associated:)
      if associated
        group = ensure_group!
        group.add(user, automatic: true)
        user.update!(flair_group_id: group.id) if user.flair_group_id != group.id
      elsif group = Group.find_by(name: DiscourseAi::AiBot::UserFlair::GROUP_NAME)
        group.remove(user)
      end
    end

    def associated_user_ids(user_ids: nil)
    ...
    def ensure_group!
      group = Group.find_or_initialize_by(name: DiscourseAi::AiBot::UserFlair::GROUP_NAME)
      group.assign_attributes(
        automatic: true,
        flair_icon: DiscourseAi::AiBot::UserFlair::FLAIR_ICON,
