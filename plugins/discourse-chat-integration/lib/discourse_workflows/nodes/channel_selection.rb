    ...
    module Nodes
      module ChatIntegrationChannelSelection
        ...
        class << self
          ...
          def channel_identifier(channel, provider = nil)
            provider ||= DiscourseChatIntegration::Provider.get_by_name(channel.provider)
          ...
          def selectable_channels
            ...
            DiscourseChatIntegration::Channel.where(
              "value::json->>'provider' IN (?)",
              provider_names,
            )
          end

          def load_options(context)
            options =
