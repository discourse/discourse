...
after_initialize do
  ...
  on(:topic_created) { |topic| handle_pm_created(topic) if topic.private_message? }

  on(:post_created) { |post| handle_post_created_edited(post, :create) }

  on(:post_edited) { |post| handle_post_created_edited(post, :edit) }

  Plugin::Filter.register(:after_post_cook) { |post, cooked| handle_after_post_cook(post, cooked) }
