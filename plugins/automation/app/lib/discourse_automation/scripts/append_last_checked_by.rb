...
DiscourseAutomation::Scriptable.add(DiscourseAutomation::Scriptable::APPEND_LAST_CHECKED_BY) do
  ...
  script do |context|
    ...
    if username.present? && checked_at.present?
      ...
      date_time = "[date=#{checked_at.to_date.to_s} time=#{checked_at.strftime("%H:%M:%S")} timezone=UTC]"
      node.inner_html += PrettyText.cook(I18n.t("discourse_automation.scriptables.append_last_checked_by.text", username: username, date_time: date_time)).html_safe
    end
