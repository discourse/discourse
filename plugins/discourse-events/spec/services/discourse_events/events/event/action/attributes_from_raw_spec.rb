...
RSpec.describe DiscourseEvents::Events::Event::Action::AttributesFromRaw do
  subject(:attributes) { described_class.call(raw_event:, current_status:) }

  let(:current_status) { DiscourseEvents::Events::Event.statuses[:public] }
  ...
  context "with timezone parsing" do
    ...
    it "falls back to the default timezone when none is provided" do
      ...
      utc = ActiveSupport::TimeZone[DiscourseEvents::Events::Event::DEFAULT_TIMEZONE]
  ...
  context "with status resolution" do
    it "resolves a known raw status to its enum value" do
      ...
      expect(attributes[:status]).to eq(DiscourseEvents::Events::Event.statuses[:private])
    end
