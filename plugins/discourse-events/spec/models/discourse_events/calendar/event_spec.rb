...
describe DiscourseEvents::Calendar::Event do
  ...
  it "removes the event if post does not contain dates anymore" do
    ...
    expect(DiscourseEvents::Calendar::Event.find_by(post_id: post.id)).to be_present

    post.update(raw: "Not sure about the dates anymore")
    CookedPostProcessor.new(post).post_process

    expect(DiscourseEvents::Calendar::Event.find_by(post_id: post.id)).not_to be_present
  end
  ...
  it "does not allow whispers to create events in calendar topics" do
    ...
    expect(DiscourseEvents::Calendar::Event.where(topic_id: calendar_post.topic_id)).to be_empty
  end
  ...
  it "recreates calendar event for all posts" do
    ...
    expect(DiscourseEvents::Calendar::Event.count).to eq(1)

    PostDestroyer.new(Discourse.system_user, calendar_post.reload, context: "spec").destroy
    expect(DiscourseEvents::Calendar::Event.count).to eq(0)
  ...
  describe "all day event site settings" do
    ...
    it "works with timezone" do
      ...
      event = DiscourseEvents::Calendar::Event.find_by(post_id: post.id)
  ...
  describe "#destroy" do
    it "removes event when a post is deleted" do
      ...
      expect(DiscourseEvents::Calendar::Event.find_by(post_id: post.id)).to_not be_present
    end
