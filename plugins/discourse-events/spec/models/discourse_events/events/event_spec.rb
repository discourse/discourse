...
describe DiscourseEvents::Events::Event do
  ...
  describe "#create_livestream_chat_channel" do
    ...
    it "creates the chat channel after the livestream event is committed" do
      ...
      }.to change(DiscourseEvents::Livestream::TopicChatChannel, :count).by(1)
    ...
    it "does not create a chat channel when chat is disabled" do
      ...
      }.not_to change(DiscourseEvents::Livestream::TopicChatChannel, :count)
    end
  ...
  describe "#raw_invitees_are_groups" do
    ...
    it "is valid when an invitee is a group whose name collides with a username" do
      Fabricate(:user).update_columns(
        username: DiscourseEvents::Events::Event::PUBLIC_GROUP,
        username_lower: DiscourseEvents::Events::Event::PUBLIC_GROUP,
      )
      event =
        Fabricate.build(
          :event,
          post: post,
          original_starts_at: Time.now,
          raw_invitees: [DiscourseEvents::Events::Event::PUBLIC_GROUP],
        )
  ...
  describe "#ongoing?" do
    ...
    context "without ends_at date" do
      ...
      context "when starts_at == current date" do
        it "is ongoing" do
          post_event =
            DiscourseEvents::Events::Event.create!(original_starts_at: Time.now, post: first_post)
  ...
  describe "#missing_users" do
    ...
    before { DiscourseEvents::Events::Invitee.create_attendance!(user_3.id, post_1.id, :going) }
