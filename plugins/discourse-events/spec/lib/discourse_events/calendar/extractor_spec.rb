...
describe DiscourseEvents::Calendar::Extractor do
  before do
  ...
  it "defaults to dynamic" do
    expect(calendar_post.reload.custom_fields[DiscourseEvents::CALENDAR_CUSTOM_FIELD]).to eq(
      "dynamic",
    )
  end
  ...
  it "adds an entry with a single date event" do
    ...
    calendar_event = DiscourseEvents::Calendar::Event.find_by(post_id: post.id)
