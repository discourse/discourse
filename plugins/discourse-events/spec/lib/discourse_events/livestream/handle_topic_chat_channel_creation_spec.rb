...
RSpec.describe DiscourseEvents::Livestream do
    ...
    context "when the topic has a category" do
      ...
      context "when the first post is a livestream event" do
        ...
        it "creates a topic chat channel" do
          ...
          expect(DiscourseEvents::Livestream::TopicChatChannel.count).to eq(1)
          expect(DiscourseEvents::Livestream::TopicChatChannel.first.topic).to eq(topic)
        end
        ...
        it "deletes the chat channel and the topic chat channel when the topic is destroyed" do
          ...
          expect(Chat::Channel.count).to eq(0)
          expect(DiscourseEvents::Livestream::TopicChatChannel.count).to eq(0)
        end
        ...
        it "deletes the chat channel when topic chat channel is destroyed" do
          ...
          DiscourseEvents::Livestream::TopicChatChannel.first.destroy!
        ...
        it "posts a pinned message referencing the topic when pinned messages are enabled" do
          ...
          expect(DiscourseEvents::Livestream::TopicChatChannel.first.reference_message_id).to eq(
            message.id,
          )
        end
