...
describe DiscourseEvents::Events::Finder do
  fab!(:current_user, :user)
  fab!(:user)

  subject(:finder) { DiscourseEvents::Events::Finder }
  ...
  describe "by attending user" do
    ...
    fab!(:not_attending_event) do
      DiscourseEvents::Events::Invitee.create!(
        user: attending_user,
        event: another_event,
        status: DiscourseEvents::Events::Invitee.statuses[:not_going],
      )
    end
  ...
  describe "by topic id" do
    fab!(:event_one) { Fabricate(:event, status: DiscourseEvents::Events::Event.statuses[:public]) }
    fab!(:event_two) { Fabricate(:event, status: DiscourseEvents::Events::Event.statuses[:public]) }
    ...
    context "with a closed event" do
      fab!(:closed_event) do
        Fabricate(:event, status: DiscourseEvents::Events::Event.statuses[:public], closed: true)
      end
