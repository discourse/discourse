...
describe DiscourseEvents::Events::ChatChannelSync do
  fab!(:user)
  fab!(:admin)
  ...
  it "does nothing when the user cannot create a channel" do
  ...
  describe "user filtering" do
    ...
    it "adds users whose suspension has expired to the chat channel" do
      ...
      event = Fabricate(:event, chat_enabled: true, post: admin_post, name: "Test Event")
      event.create_invitees(
        [user_id: formerly_suspended.id, status: DiscourseEvents::Events::Invitee.statuses[:going]],
      )
      event.save!
