      ...
      def valid_value?(val)
        ...
        attributes =
          names.index_with do |name|
            DiscourseEvents::Events::Parser.custom_field_data_attribute(name)
          end
        errors = []
        ...
        colliding =
          names
            .group_by { |name| attributes[name] }
            .values
            .select { |group| group.size > 1 }
            .flatten
        errors << error(:collision, colliding) if colliding.any?

        reserved = DiscourseEvents::Events::Parser.valid_option_attributes
