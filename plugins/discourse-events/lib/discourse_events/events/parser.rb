  ...
  module Events
    class Parser
      ...
      class << self
        ...
        def valid_option_attributes
          VALID_OPTIONS.map { |option| "data-#{option}" }
        end

        def custom_field_data_attribute(setting)
        ...
        def cook_location(text, post: nil)
          text = text.to_s
          add_nofollow = post.nil? || post.add_nofollow?
        ...
        def url_restates_location?(url, location, cooked_location: nil)
