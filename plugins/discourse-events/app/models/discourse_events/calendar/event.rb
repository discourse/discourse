...
module DiscourseEvents
  module Calendar
    class Event < ActiveRecord::Base
      ...
      # "discourse_events_events" from the namespace.
      ...
      after_save do
        if SiteSetting.enable_user_status && is_holiday? && underway?
          DiscourseEvents::Holidays::Status.set!(user, ends_at)
        end
      end

      after_destroy do
        DiscourseEvents::Holidays::Status.clear!(user) if SiteSetting.enable_user_status
      end

      class << self
        def update(post)
          Event.where(post_id: post.id).destroy_all

          dates = post.local_dates
          return if !dates || dates.size < 1 || dates.size > 2

          first_post = post.topic&.first_post
          return if !first_post || !first_post.custom_fields[DiscourseEvents::CALENDAR_CUSTOM_FIELD]
          ...
          if SiteSetting.all_day_event_start_time.present? &&
               SiteSetting.all_day_event_end_time.present?
            from = from.change(hour_adjustment(SiteSetting.all_day_event_start_time)) if !dates[0][
              "time"
            ]
            to =
              (to || from).change(
                hour_adjustment(SiteSetting.all_day_event_end_time),
              ) if adjust_to && !artificial_to
          end
