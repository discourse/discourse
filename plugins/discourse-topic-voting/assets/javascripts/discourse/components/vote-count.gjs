...
export default class VoteCount extends Component {
  ...
  <template>
    {{#if this.showVoterMenu}}
      ...
      >
        <:trigger>
          <span class="vote-count">{{this.displayCount}}</span>
        </:trigger>
        <:content>
          {{#if this.isLoading}}
            <div class="vote-count-loading">
              {{i18n "loading"}}
            </div>
          {{else if (eq @topic.vote_count 0)}}
            <div class="vote-count-empty-state">
              {{i18n "topic_voting.no_votes_yet"}}
            </div>
          {{else if this.voters}}
            <div class="vote-count__voter-list">
              {{#each this.voters as |voter|}}
                <a
                  class="vote-count__voter-avatar trigger-user-card"
              ...
              {{#if (gt this.totalVoters this.maxDisplayedVoters)}}
                <div class="vote-count-overflow-notice">
                  {{i18n
