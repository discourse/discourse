...
export default class VoteBox extends Component {
  ...
  get buttonContent() {
    ...
    if (this.currentUser) {
      ...
      } else if (this.currentUser.votes_exceeded && !this.topic.user_voted) {
  ...
  get buttonClasses() {
    if (this.currentUser?.vote_limit === 0) {
      return "btn-default btn-small vote-button";
    }

    return this.topic.user_voted
      ? "btn-success btn-small vote-button"
      : "btn-default btn-small vote-button";
  }
