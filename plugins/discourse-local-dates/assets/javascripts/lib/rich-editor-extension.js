...
/**
 * Pre-processes HTML to wrap adjacent date-range spans in a container element.
 * This is needed because ProseMirror's parseDOM processes each element independently.
 */
function wrapDateRangeSpans(element) {
  ...
  for (const span of dateSpans) {
    ...
    // Skip spans already inside a date-range wrapper (e.g., HTML generated by ProseMirror)
    if (span.closest(".discourse-local-date-range")) {
      continue;
    }

    const range = span.dataset.range;
    // Only process the "from" span (data-range="true" or "from")
    if (range !== "true" && range !== "from") {
      continue;
    }

    // Look for the next sibling span that's part of this range
    let nextSibling = span.nextSibling;
    // Skip text nodes (usually the arrow character)
    while (nextSibling && nextSibling.nodeType === Node.TEXT_NODE) {
      nextSibling = nextSibling.nextSibling;
    }

    // Check if the next element is the "to" date
    if (
      !nextSibling ||
      !nextSibling.classList?.contains("discourse-local-date") ||
      !nextSibling.dataset?.range
    ) {
      continue;
    }

    const toSpan = nextSibling;
    processed.add(span);
    processed.add(toSpan);

    // Update the data-range attributes for proper parsing
    span.dataset.range = "from";
    toSpan.dataset.range = "to";

    // Create wrapper and insert it
    // Use the document itself if element is a Document, otherwise use ownerDocument
    const doc =
    ...
    // Move the spans and arrow text into the wrapper
    wrapper.appendChild(span);

    // Move any text nodes (arrow) between the spans
