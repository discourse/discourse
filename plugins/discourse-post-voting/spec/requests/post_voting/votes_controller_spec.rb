...
RSpec.describe PostVoting::VotesController do
  ...
  describe "#create" do
    ...
    it "creates a vote when the user has not voted on the post" do
      post "/post_voting/vote.json", params: { post_id: answer.id }
    ...
    it "rejects a duplicate vote" do
      post "/post_voting/vote.json", params: { post_id: answer.id }
    ...
    it "returns 403 when voting on the user's own post" do
      post "/post_voting/vote.json", params: { post_id: answer_3.id }

      expect(response.status).to eq(403)
    end

    it "returns 403 after the vote undo window expires" do
  ...
  describe "#destroy" do
    ...
    it "returns 403 when the user has never voted on the post" do
      delete "/post_voting/vote.json", params: { post_id: answer.id }

      expect(response.status).to eq(403)
    end

    it "rejects undoing a vote after the undo window expires" do
      SiteSetting.post_voting_undo_vote_action_window = 1
  ...
  describe "#voters" do
    ...
    it "returns voters within the requested limit" do
      sign_in(user)
    ...
    it "returns total_voters_count for computing the remaining voters" do
      sign_in(user)
  ...
  describe "#create_comment_vote" do
    ...
    it "returns 404 if comment_id is invalid" do
      sign_in(user)
    ...
    it "returns 403 if the user cannot see the comment" do
      sign_in(user)
    ...
    it "returns 403 when voting on the user's own comment" do
      sign_in(user)
  ...
  describe "#destroy_comment_vote" do
    ...
    it "returns 403 if the user has not voted on the comment" do
      sign_in(user)
    ...
    it "removes the user's vote from the comment" do
