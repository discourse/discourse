...
module DiscourseWorkflows
  module Nodes
    module DataTable
      module Operations
        class Base
          def initialize(facade, filter_resolver, columns_resolver)
            @facade = facade
          ...
          def build_query(config, optional_filter:)
            query =
              @facade.build_query(
                filter: @filter_resolver.resolve(config),
                limit: config["limit"]&.to_i,
                sort_by: @filter_resolver.resolve_sort_column_name(config["sort_column"]),
