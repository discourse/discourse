  ...
  module Nodes
    module HttpRequest
      class Authenticator
        ...
        class << self
          def apply(config, headers, exec_ctx, item_index: 0)
          ...
          def fetch_credential_data(exec_ctx, auth_mode, item_index)
            if exec_ctx.nil?
              raise_node_error!(
                I18n.t(
                  "discourse_workflows.errors.http_request.exec_ctx_required",
                  mode: auth_mode,
                ),
              )
            end
          ...
          def apply_basic_auth(headers, cred_data)
            user = cred_data["user"]
          ...
          def apply_bearer_token(headers, cred_data)
            token = cred_data["token"]
            headers["Authorization"] = "Bearer #{token}"
            ["Authorization"]
          end

          def apply_header_auth(headers, cred_data)
            name = cred_data["name"].to_s
            unless name.match?(HEADER_NAME_PATTERN)
              raise_node_error!(
                I18n.t("discourse_workflows.errors.http_request.header_name_invalid"),
              )
            end
