...
module DiscourseWorkflows
  class FormTriggerToken
    ...
    class << self
      def generate(workflow_id:, trigger_node_id:, uuid:, form_query_parameters: nil)
        payload = {
      ...
      def valid?(token, workflow_id:, trigger_node_id:, uuid:)
        payload(
          token,
          workflow_id: workflow_id,
      ...
      def payload(token, workflow_id:, trigger_node_id:, uuid:)
