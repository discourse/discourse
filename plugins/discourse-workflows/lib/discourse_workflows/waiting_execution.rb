...
module DiscourseWorkflows
  class WaitingExecution
    ...
    class << self
      def find(execution_id:, signature:, expected_node_type: nil)
        execution =
      ...
      def valid_signature?(execution, signature)
        stored =
      ...
      def resume_claimed(execution, response_items, user: nil, webhook_context: nil)
      ...
      def form_waiting_url(execution, absolute: false)
      ...
      def form_waiting_url_for(execution_id:, resume_token:, absolute: false)
        signed_url(
      ...
      def form_status_url(execution, absolute: false)
        form_status_url_for(
          execution_id: execution.id,
          resume_token: execution.resume_token,
          absolute: absolute,
      ...
      def form_status_url_for(execution_id:, resume_token:, absolute: false)
        signed_url(
      ...
      def webhook_url(execution_id:, resume_token:, suffix: nil, absolute: true)
      ...
      def webhook_url_with_signature(execution_id:, signature:, suffix: nil, absolute: true)
        path = "/workflows/waiting/#{execution_id}/webhook"
        path = "#{path}/#{suffix}" if suffix.present?
        signed_url(path, signature, absolute: absolute)
      end

      def action_token(execution_id:, resume_token:, action:, target_user_id: nil)
        action = action.to_s
      ...
      def action_token_payload(token)
      ...
      def find_by_action_token(token, expected_node_type:)
