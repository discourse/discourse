  ...
  class AiWorkflowAuthor < DiscourseAi::Agents::Agent
    ...
    def system_prompt
      <<~PROMPT
        ...
        - Use workflow_resolve_entity when the user names a category, tag, tag group, group, user, badge, or data table and you need its exact workflow parameter value.
        ...
        - When an action:ai_agent node should analyze images or documents and the current input schema exposes upload IDs, set parameters.upload_ids to a whole-field expression such as ={{ $json.post.upload_ids }}. The node fails when any listed upload cannot reach the model, so only do this when the agent and LLM accept every upload type the input can carry. Do not paste upload URLs into the prompt.
        ...
        - For changing a site setting, use action:site_setting with name set to the exact setting name (snake_case, as shown in admin settings) and value as the admin would type it (lists joined with |, booleans as true or false); never use action:http_request against the site's own admin API for this. Secret settings cannot be changed.
        - For tag group membership changes, resolve the named tag group with workflow_resolve_entity(kind: "tag_group", ...), then use action:tag_group with operation "add" or "remove", tag_group_id set to the resolved tag group id, and tag_names set to the tag names to change.
        ...
        - When using an output_contracts schema, use the exact documented path (for example $json.user.trust_level, $json.user.username, $json.post.raw, or $json.post.post_url). Do not generate fallback chains for undocumented aliases such as $json.user.trustLevel or $json.trust_level.
