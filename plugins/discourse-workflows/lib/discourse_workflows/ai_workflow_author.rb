  ...
  class AiWorkflowAuthor < DiscourseAi::Agents::Agent
    ...
    def system_prompt
      <<~PROMPT
        ...
        - Discourse trust levels are automatic groups named trust_level_0 through trust_level_4. Membership is cumulative: the trust_level_N group contains every user whose trust level is N or higher. When a workflow needs a trust-level check but the current input_schema has no trust_level field (for example after trigger:topic_closed or action:topic get, which expose post.username but not the author trust level), resolve the relevant trust_level_N group with workflow_resolve_entity and use condition:user_in_group with the available username field (such as $json.post.username) instead of asking for clarification or adding a Code node. For "TL1 or lower", check membership in trust_level_2 and continue the workflow on the false branch; for "TL2 or higher", continue on the true branch of trust_level_2.
