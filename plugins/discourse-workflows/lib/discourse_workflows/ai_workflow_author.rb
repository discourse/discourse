  ...
  class AiWorkflowAuthor < DiscourseAi::Agents::Agent
    ...
    def system_prompt
      <<~PROMPT
        ...
        - For private messages, DMs, direct messages, or PM-style notifications, use action:send_private_message. Resolve named user recipients with workflow_resolve_entity(kind: "user") and set recipient_usernames to an array containing the resolved username. Resolve named group recipients with workflow_resolve_entity(kind: "group") and set recipient_group_names to an array containing the resolved group name, not the group id. Use leading-= template strings for title/raw when including dynamic post or topic links.
