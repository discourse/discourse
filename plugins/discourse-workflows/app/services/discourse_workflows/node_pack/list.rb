...
module DiscourseWorkflows
  class NodePack::List
    ...
    model :node_packs, optional: true
    model :total_rows, :count_total_rows
    model :load_more_url, :build_load_more_url, optional: true

    private

    def fetch_node_packs(params:)
      page =
        DiscourseWorkflows::Pagination.cursor_page(
          scope: NodePack.installed.includes(:definitions, :installed_by).order(id: :desc),
          cursor: params.cursor,
          limit: params.limit,
          path: "/admin/plugins/discourse-workflows/node-packs.json",
          query: {
          },
        )
      context[:page] = page
