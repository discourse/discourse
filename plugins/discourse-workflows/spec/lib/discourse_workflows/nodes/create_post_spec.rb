...
RSpec.describe DiscourseWorkflows::Nodes::CreatePost::V1 do
  ...
  describe "#execute" do
    ...
    it "raises when the author_username cannot be found" do
      expect do
        execute_node(
          configuration: {
            "topic_id" => topic.id.to_s,
    ...
    it "raises when the author cannot see the topic" do
      group = Fabricate(:group)
      category = Fabricate(:private_category, group: group)
      hidden_topic = create_post(user: admin, category: category).topic
    ...
    it "resolves expressions for each input item" do
      ...
      result =
        described_class.new(parameters: config).execute(
