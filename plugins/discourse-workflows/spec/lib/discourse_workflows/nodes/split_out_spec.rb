...
RSpec.describe DiscourseWorkflows::Nodes::SplitOut::V1 do
  describe "basic array splitting" do
    ...
    it "emits no items when field is missing" do
      ...
      hints = nil
      result =
        execute_node_output(configuration: { "field" => "items" }, input_items: input) do |ctx|
          hints = ctx.execution_hints
        end.first
  ...
  describe "include modes" do
    it "includes all other fields" do
      ...
      configuration = { "field" => "items", "include" => "all_other_fields" }
      result = execute_node_output(configuration: configuration, input_items: input).first
    ...
    it "includes selected other fields" do
      ...
      configuration = {
        "field" => "items",
        "include" => "selected_other_fields",
        "fields_to_include" => "company",
      }
      result = execute_node_output(configuration: configuration, input_items: input).first
    ...
    it "raises DiscourseWorkflows::NodeError when selected fields are blank" do
      ...
      configuration = { "field" => "items", "include" => "selected_other_fields" }
      expect {
        execute_node_output(configuration: configuration, input_items: input)
      }.to raise_error(DiscourseWorkflows::NodeError, /No fields specified/)
    end
  ...
  describe "dot notation" do
    it "accesses nested fields" do
      ...
      configuration = { "field" => "data.items" }
      result = execute_node_output(configuration: configuration, input_items: input).first
    ...
    it "strips a leading $json prefix from field names" do
      input = [{ "json" => { "items" => [1] } }]
      configuration = { "field" => "$json.items" }
      result = execute_node_output(configuration: configuration, input_items: input).first
  ...
  describe "multiple fields" do
    it "splits multiple fields simultaneously" do
      ...
      configuration = { "field" => "names, ages" }
      result = execute_node_output(configuration: configuration, input_items: input).first
  ...
  describe "destination field name" do
    it "renames the output field" do
      ...
      configuration = { "field" => "items", "destination_field_name" => "result" }
      result = execute_node_output(configuration: configuration, input_items: input).first
  ...
  describe "item limit" do
    it "raises DiscourseWorkflows::NodeError when array exceeds MAX_SPLIT_ITEMS" do
      ...
      expect {
        execute_node_output(configuration: { "field" => "items" }, input_items: input)
      }.to raise_error(DiscourseWorkflows::NodeError, /Cannot split into more than/)
    end
  end
end
