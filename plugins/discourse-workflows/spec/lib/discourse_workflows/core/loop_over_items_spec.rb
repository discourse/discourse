...
RSpec.describe DiscourseWorkflows::Core::LoopOverItems::V1 do
  ...
  describe "#execute" do
    it "sends first batch to loop output on first execution" do
      ...
      expect(result["loop"][0]["json"]).to include("id" => 1)
      expect(result["loop"][1]["json"]).to include("id" => 2)
    ...
    it "sends next batch on subsequent execution" do
      ...
      expect(result).to include("done" => [])
      expect(result["loop"].length).to eq(2)
      expect(result["loop"][0]["json"]).to include("id" => 3)
      expect(result["loop"][1]["json"]).to include("id" => 4)
    ...
    it "handles empty input" do
      ...
      expect(result).to include("loop" => [], "done" => [])
    end
