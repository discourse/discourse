...
RSpec.describe DiscourseWorkflows::Credential::List do
  describe described_class::Contract, type: :model do
    it { is_expected.to allow_values(nil, 1, 50).for(:cursor) }
    it { is_expected.to allow_values(nil, 1, 25, 100).for(:limit) }
    it { is_expected.to allow_values(nil, "basic_auth", "bearer_token").for(:type) }
  end
  ...
  describe ".call" do
    ...
    context "when credentials exist" do
      ...
      it "returns credentials ordered by id desc" do
        expect(result[:truncated_credentials].map(&:name)).to eq(%w[Second First])
      end
    ...
    context "when paginating" do
      ...
      it "returns load_more_url when more results exist" do
        expect(result[:truncated_credentials].length).to eq(2)
    ...
    context "when filtering by type" do
      ...
      it "returns only matching credentials" do
        expect(result[:truncated_credentials].map(&:id)).to eq([basic_credential.id])
      end
    ...
    context "when limit is below minimum" do
      fab!(:credential_1, :discourse_workflows_credential)
      fab!(:credential_2, :discourse_workflows_credential)

      let(:params) { { limit: 0 } }

      it "clamps limit to 1" do
        expect(result[:truncated_credentials].length).to eq(1)
      end
    end
  end
end
