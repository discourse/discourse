...
RSpec.describe DiscourseWorkflows::Workflow::List do
  describe ".call" do
    ...
    context "when limit is below minimum" do
      fab!(:workflow_1) { Fabricate(:discourse_workflows_workflow, created_by: user) }
      fab!(:workflow_2) { Fabricate(:discourse_workflows_workflow, created_by: user) }

      let(:params) { { limit: 0 } }

      it "clamps limit to 1" do
        expect(result[:workflows].length).to eq(1)
      end
    end
  end
end
