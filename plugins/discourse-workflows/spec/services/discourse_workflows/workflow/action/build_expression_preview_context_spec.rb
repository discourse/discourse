  ...
  context "with a workflow" do
    ...
    context "with a last successful execution" do
      ...
      context "with a successful trigger run" do
        ...
        it "uses the connected upstream output when the current node has no input data" do
        ...
        it "uses the current node's recorded input data for $json" do
          current_items = [{ "json" => { "title" => "From current input" } }]
          execution.execution_data.update!(
            data: {
              "entries" => {
              },
        ...
        it "uses the default input context when recorded input came from another source" do
          stale_items = [{ "json" => { "reviewable" => { "id" => 1 } } }]
          execution.execution_data.update!(
            data: {
              "entries" => {
              },
