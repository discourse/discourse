...
RSpec.describe DiscourseWorkflows::WorkflowVariable::UpdateValue do
  describe described_class::Contract, type: :model do
    ...
    it do
      is_expected.to validate_length_of(:value).is_at_most(
        DiscourseWorkflows::WorkflowVariable::UpdateValue::MAX_VALUE_LENGTH,
      )
    end
  end
  ...
  describe ".call" do
    ...
    let(:params) { { workflow_id: workflow.id, variable_id: variable.id, value: "medium" } }
    ...
    context "when the variable is a boolean and the value isn't true/false" do
      fab!(:variable) do
        Fabricate(
          :discourse_workflows_workflow_variable,
          workflow:,
    ...
    context "when the variable is a category_list and the id doesn't exist" do
      fab!(:variable) do
        Fabricate(
          :discourse_workflows_workflow_variable,
          workflow:,
    ...
    context "when the variable is a category_list and the id exists" do
      fab!(:category)
      fab!(:variable) do
        Fabricate(
          :discourse_workflows_workflow_variable,
    ...
    context "when the variable is a category and the id doesn't exist" do
      fab!(:variable) do
        Fabricate(
          :discourse_workflows_workflow_variable,
          workflow:,
    ...
    context "when the variable is a category and the id exists" do
      fab!(:category)
      fab!(:variable) do
        Fabricate(
          :discourse_workflows_workflow_variable,
    ...
    context "when the variable is a group and the id doesn't exist" do
      fab!(:variable) do
        Fabricate(
          :discourse_workflows_workflow_variable,
          workflow:,
    ...
    context "when the variable is a group and the id exists" do
      fab!(:group)
      fab!(:variable) do
        Fabricate(
          :discourse_workflows_workflow_variable,
    ...
    context "when everything's ok" do
      ...
      it "updates the value" do
        expect { result }.to change { variable.reload.value }.to("medium")
      end
      ...
      it "embeds the new value in the version snapshot" do
        ...
        entry = new_version.variables_schema.find { |f| f["key"] == "priority" }
    ...
    context "when the workflow is published and the draft was clean" do
      ...
      context "when another active workflow already owns the same webhook route" do
        ...
        it "still saves the value as a draft, without publishing it" do
          ...
          expect(variable.reload.value).to eq("medium")
