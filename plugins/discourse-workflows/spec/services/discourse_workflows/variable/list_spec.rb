...
RSpec.describe DiscourseWorkflows::Variable::List do
  describe described_class::Contract, type: :model do
    ...
    it { is_expected.to allow_value(42).for(:cursor) }
    it { is_expected.to allow_value(nil).for(:limit) }
    it { is_expected.to allow_value(10).for(:limit) }
  end
  ...
  describe ".call" do
    ...
    context "when there are variables" do
      fab!(:variable_a) { Fabricate(:discourse_workflows_variable, key: "ALPHA") }
      fab!(:variable_b) { Fabricate(:discourse_workflows_variable, key: "BRAVO") }
    ...
    context "with pagination" do
      ...
      fab!(:variable_1) { Fabricate(:discourse_workflows_variable, key: "FIRST") }
      fab!(:variable_2) { Fabricate(:discourse_workflows_variable, key: "SECOND") }
      fab!(:variable_3) { Fabricate(:discourse_workflows_variable, key: "THIRD") }
    ...
    context "when limit is below minimum" do
      fab!(:variable_1, :discourse_workflows_variable)
      fab!(:variable_2, :discourse_workflows_variable)
