...
function createCustomRenderer(Scope, iconHTML, callbacks) {
  return class CustomRenderer extends Scope {
    constructor() {
      ...
      this.addPipe((context) => {
        ...
        switch (context.type) {
          ...
          case "unmount": {
            const payloadId = this.connectionElementIds.get(
              context.data.element
            );
            if (payloadId != null) {
              this.connectionElements.delete(payloadId);
              this.connectionElementIds.delete(context.data.element);
            }
            break;
          }
          case "nodetranslated":
    ...
    renderNode(element, node) {
      const data = node.workflowData;
      const iconInfo = getNodeIcons()[data.type];
      const iconColor = iconInfo?.color || "var(--primary-medium)";
      const outputKeys = Object.keys(node.outputs);
      const isBranching = outputKeys.length > 1;
      ...
      wrapper.className = `workflow-rete-node${isTrigger ? " --trigger" : ""}${isBranching ? " --branching" : ""}${node.selected ? " --selected" : ""}`;
      wrapper.style.width = `${node.width}px`;
      wrapper.style.height = `${node.height}px`;
      wrapper.dataset.clientId = data.clientId;

      // Input socket
      if (node.inputs.input) {
        const inputSocket = document.createElement("div");
        inputSocket.className = "workflow-rete-node__socket --input";
        wrapper.appendChild(inputSocket);
      }

      // Node body
      const body = document.createElement("div");
      body.className = "workflow-rete-node__body";

      const titleRow = document.createElement("div");
      titleRow.className = "workflow-rete-node__title-row";

      if (data.type === "trigger:manual") {
        const playIcon = document.createElement("span");
        playIcon.className = "workflow-rete-node__icon";
        playIcon.style.color = iconColor;
        playIcon.innerHTML = this.iconHTML("play");
        titleRow.appendChild(playIcon);

        body.classList.add("--clickable");
        body.addEventListener("pointerdown", (e) => {
          e.stopPropagation();
          e.preventDefault();
          this.onManualTrigger?.(data.clientId, body);
        });
      } else if (iconInfo?.icon) {
        const iconSpan = document.createElement("span");
        iconSpan.className = "workflow-rete-node__icon";
        iconSpan.style.color = iconColor;
        iconSpan.innerHTML = this.iconHTML(iconInfo.icon);
        titleRow.appendChild(iconSpan);
      }

      const labelText = nodeLabel(data);
      if (labelText) {
        const label = document.createElement("span");
        label.className = "workflow-rete-node__label";
      ...
      if (data.configuration?.description) {
        ...
        desc.className = "workflow-rete-node__description";
        desc.textContent = data.configuration.description;
        body.appendChild(desc);
      }

      const deleteBtn = document.createElement("button");
      deleteBtn.type = "button";
      deleteBtn.className = "workflow-rete-node__delete-btn";
      ...
      deleteBtn.addEventListener("click", (e) => {
        e.stopPropagation();
        e.preventDefault();
        this.onNodeDelete?.(data.clientId);
      });
      ...
      if (outputKeys.length > 0) {
        ...
        outputsContainer.className = "workflow-rete-node__outputs";

        for (const key of outputKeys) {
          const outputSocket = document.createElement("div");
          outputSocket.className = `workflow-rete-node__socket --output${key === "loop" ? " --loop" : ""}`;
          outputSocket.dataset.socketKey = key;

          if (isBranching) {
            const pill = document.createElement("span");
            const isPositive =
              key === "true" || key === "loop" || key === "done";
            pill.className = `workflow-rete-node__port-pill ${isPositive ? "--positive" : "--negative"}`;
            pill.textContent = portLabel(data.type, key);
            outputSocket.appendChild(pill);
          }

          outputsContainer.appendChild(outputSocket);
        }

        wrapper.appendChild(outputsContainer);
      }

      element.appendChild(wrapper);

      // Emit socket render signals so connection plugin can track them.
      // nodeId, side, key must be directly on data (not nested under payload)
      // because the connection plugin stores context.data in socketsCache
      // and the classic flow accesses socket.side, socket.nodeId, socket.key.
      const area = this.parentScope();
      queueMicrotask(() => {
        const inputEl = wrapper.querySelector(
          ".workflow-rete-node__socket.--input"
        );
        if (inputEl && node.inputs.input) {
          area.emit({
            type: "render",
            data: {
              element: inputEl,
              ...
              side: "input",
              key: "input",
              socket: node.inputs.input.socket,
            },
          });
        }

        wrapper
          .querySelectorAll(".workflow-rete-node__socket.--output")
          .forEach((el) => {
            const key = el.dataset.socketKey;
            // "loop" output is a visual-only self-loop indicator, not interactive
            if (key === "loop" || !node.outputs[key]) {
              return;
            }
            area.emit({
              type: "render",
              data: {
                ...
                side: "output",
                key,
                socket: node.outputs[key].socket,
              },
            });
          });
      });
...
export async function createReteEditor(container, { iconHTML, callbacks }) {
  ...
  const CustomRenderer = createCustomRenderer(Scope, iconHTML, {
    ...
    onManualTrigger: (clientId, btnEl) => {
      callbacks.onManualTrigger?.(clientId, btnEl);
    },
  });
  ...
  area.addPipe((context) => {
    ...
    switch (context.type) {
      ...
      case "nodepicked": {
        ...
        // Re-render previously selected nodes so they lose --selected
