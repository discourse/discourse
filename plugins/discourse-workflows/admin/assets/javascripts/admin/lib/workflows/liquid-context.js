...
export function buildLiquidScope({
  ...
  const base = {
    items: [templateItem],
    inputs: Array.from(branchFields, (fields) =>
      fields ? [buildTemplateItem(buildScopeFromFields(fields))] : []
    ),
    items_count: 0,
...
const LOOP_TAG_RE =
  /\{%-?\s*(for|endfor|comment|endcomment|raw|endraw)\b([^%]*)%\}/g;
...
export function enclosingLoopsAt(text, pos) {
  ...
  while ((match = LOOP_TAG_RE.exec(before))) {
    const tag = match[1];
    if (ignoredBlock) {
      if (ignoredBlock === "comment" && tag === "comment") {
        commentDepth++;
      } else if (tag === `end${ignoredBlock}`) {
        if (commentDepth > 0) {
          commentDepth--;
        } else {
          ignoredBlock = null;
    ...
    const loop = { name: binding[1], path: binding[2] };
    const options = match[2].slice(binding[0].length);
    if (/\boffset\s*:/.test(options)) {
      loop.index = null;
    } else if (/\breversed\b/.test(options)) {
      loop.index = "rindex0";
    }
    stack.push(loop);
  }
...
/**
 * Resolves an input item's prefix without rebasing it onto a nested
 * collection's loop variable.
 */
export function itemPrefixAt(text, pos, { perItem = false } = {}) {
  ...
  for (let i = loops.length - 1; i >= 0; i--) {
    ...
    if (loop.path === "items" || loop.path === "inputs[0]") {
      if (!shadowedNames.has(loop.name)) {
        return loop.name;
      }

      // Liquid exposes iteration indexes, but not the source offset.
      if (loop.index === null) {
        return null;
      }
      const parentLoops = ".parentloop".repeat(loops.length - i - 1);
      return `${loop.path}[forloop${parentLoops}.${loop.index || "index0"}]`;
    }
    shadowedNames.add(loop.name);
  }
...
export function liquidPathForVariable(id, itemPrefix) {
  ...
  if (!id.startsWith("$")) {
    return itemPrefix ? `${itemPrefix}.${id}` : null;
  }
  ...
  if (root === "$json") {
    return itemPrefix && rest ? `${itemPrefix}${rest}` : itemPrefix;
  }
