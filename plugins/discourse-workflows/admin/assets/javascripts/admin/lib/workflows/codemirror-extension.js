...
function lookupDollarVarDoc(name) {
...
function buildExpressionDecorations({ cmState, cmView }) {
  ...
  return { field, setSegmentsEffect };
}
...
function buildExpressionValidator(
  { cmView },
  { setSegmentsEffect, workflowId, nodeId, onSegmentsResolved }
) {
  ...
  return ViewPlugin.fromClass(
    class {
      ...
      scheduleEvaluation() {
        ...
        if (!template) {
          this.timeout = setTimeout(() => this.dispatch([]), 0);
          return;
        }
        this.timeout = setTimeout(() => this.evaluate(), 500);
      }
...
export default function workflowExtension(cmParams, domainOpts = {}) {
  ...
  const { field: decorationsField, setSegmentsEffect } =
    buildExpressionDecorations(cmParams);
  ...
  return [
    ...
    decorationsField,
    buildExpressionValidator(cmParams, {
      setSegmentsEffect,
