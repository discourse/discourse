...
| `friend-group-post-dm-admin` | `when anyone in the friend group posts send admin a dm with a link to the post` | `trigger:post_created -> action:group (check_membership) -> condition:if -> action:send_private_message` | Resolves the `friend` group and `admin` user; sets resolved `group_id`, uses `$json.post.username` for membership, branches on `$json.group_membership.in_group`, and uses `$json.post.post_url` in a leading-`=` private message body; no Code node. |
