...
| `closed-general-tl1-chat` | `when a topic by a tl1 or lower is closed in the general category message the general channel with a link to the post` | `trigger:topic_closed -> action:topic (get) -> condition:user_in_group -> action:send_chat_message` | Uses `action:topic` get to hydrate first-post fields (the topic payload has no `trust_level`). Resolves the `trust_level_2` automatic group and uses `condition:user_in_group` with `username: ={{ $json.post.username }}`, connecting the chat node on the **false** branch (TL1-or-lower = not in `trust_level_2`, since `trust_level_N` is cumulative for TL ≥ N). Uses `$json.post.post_url`; no Code node; no clarification needed. |
...
| `reply-only` | `when a new topic is created in Support, reply Thanks, we will review this shortly.` | `trigger:topic_created -> action:create_post` | Uses `$json.topic.id`; no unnecessary tag lookup. |
...
- Trust level is only exposed as `$json.user.trust_level` on triggers with a user object (e.g. `trigger:post_created`); the post/topic payloads have no `trust_level`. When a trust-level check is needed but no `trust_level` field is in scope (topic-only triggers), resolve the cumulative `trust_level_N` automatic group and use `condition:user_in_group` — do not ask for clarification or add a Code node.
