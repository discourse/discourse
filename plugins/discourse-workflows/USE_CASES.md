...
| --- | --- | --- | --- |
| `tl1-cake-chat` | `if a tl1 posts a post with the word cake make a post in the general chat channel linking to it` | `trigger:post_created -> condition:filter -> action:send_chat_message` | Uses `$json.post.trust_level`, `$json.post.raw`, `$json.post.post_url`; no Code node; chat template starts with `=`. |
