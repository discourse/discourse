...
module Boards
  class TopicSync
    ...
    class << self
      ...
      def build_column_topic_map(board, columns)
        result = {}
        scope_base =
          Topic
            .listable_topics
        ...
        scope_base =
          scope_base.where(category_id: board.category_ids) if board.category_ids.present?
      ...
      def apply_bulk_changes(board, to_create:, to_delete:)
      ...
      def compute_sync_plan(existing_cards:, matching_placements:)
        ...
        all_board_ids.each do |board_id|
          ...
          board_tagged.each do |col_id|
            if existing_column_ids.exclude?(col_id)
              create_targets << { board_id:, column_id: col_id }
            end
          end
      ...
      def execute_sync_changes(topic:, plan:, existing_cards:, max_positions:, board_groups:)
      ...
      def trim_excess_cards(column_ids)
      ...
      def publish_sync_changes(board_groups:, deleted_by_board:, created_cards:)
      ...
      def publish_to_board(board_groups, board_id, data)
