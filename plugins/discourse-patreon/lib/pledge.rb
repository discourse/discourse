...
module Patreon
  class Pledge
    class << self
      def create!(data, adapter: ApiVersion.current)
        save!([data], true, adapter: adapter)
      end

      def update!(data, adapter: ApiVersion.current)
        delete!(data, adapter: adapter)
        create!(data, adapter: adapter)
      end

      def delete!(data, adapter: ApiVersion.current)
        entry = data["data"]
      ...
      def save!(pledges_data, is_append = false, adapter: ApiVersion.current)
      ...
      def get_patreon_id(data, adapter: ApiVersion.adapter_for_payload(data))
