# Visual Editor — responsive layouts + design tokens integration plan
...
- Block args are **scalar** (one value per arg) — `ve-heading.gjs:16-39`, `ve-layout.gjs:33-99`. No per-viewport shape.
- `containerArgs` is a per-child namespace bag (`block-outlet.gjs:200-211`) holding things like `{ grid: { column, row, align, justify } }`. Layout reads it via `cellStyle()` (`ve-layout.gjs:243-256`).
...
.ve-layout {
  ...
  container-name: ve-layout;
}
.ve-layout--grid {
  /* default: respect author's columns */
  grid-template-columns: repeat(var(--ve-layout-cols, 12), 1fr);
}
@container ve-layout (max-width: 40rem) {
  .ve-layout--grid {
    /* collapse to 1 column below `sm` */
    grid-template-columns: 1fr;
  }
  /* row layouts wrap to stack below sm */
  .ve-layout--row { flex-direction: column; }
}
```
...
The structural change in `ve-layout.gjs:243-256` is small: `cellStyle()` picks the active breakpoint's overrides instead of reading the namespace bag directly.
...
- VE blocks today are inconsistent:
  - `ve-cta-banner.gjs:21-81` — reads `--primary-low`, `--primary-medium` directly in its stylesheet. Stable, but not adjustable per-block.
  - `ve-media-card.gjs:85-94, 117-125` — exposes `backgroundColor` as a **raw hex string** via FormKit's `color` control, inlines it via `trustHTML`. Bypasses the palette entirely.
  - `ve-category-banner.gjs:102-106` — pulls colors from the category model via `--ve-category-banner-background` etc. Best of the three: indirected, source-of-truth-driven.
...
```js
// New: plugins/.../services/visual-editor-tokens.js
@service tokens;
...
```scss
.ve-media-card {
  --ve-media-card-background: var(--sys-surface-default);
  --ve-media-card-title-color: var(--sys-text-strong);
  --ve-media-card-cta-color: var(--sys-text-default);
  /* etc. */

  background: var(--ve-media-card-background);
  .ve-media-card__title { color: var(--ve-media-card-title-color); }
}
```
...
- Existing `ve-media-card.backgroundColor: "#aabbcc"` migrates to `{type:"custom", value:"#aabbcc"}` via a one-time `serializeEntryForSave` pass, OR stays as-is and the runtime accepts both shapes.
...
- Auto-adaptation primitives in `ve:layout` (grid → stack, gap shrink) baked into the layout block itself.
...
- **Responsive**:
  - `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-layout.gjs` — container-query SCSS + per-viewport `containerArgs.grid` reader in `cellStyle()`
  - `plugins/discourse-visual-editor/assets/javascripts/discourse/lib/mutate-layout.js` — `serializeEntryForSave` collapse logic + responsive-arg helpers
  - `plugins/discourse-visual-editor/assets/javascripts/discourse/components/editor/inspector-form.gjs` — responsive override row renderer
  - `plugins/discourse-visual-editor/assets/stylesheets/visual-editor.scss` — container-query rules for `.ve-layout`
- **Tokens**:
  - New: `plugins/.../services/visual-editor-tokens.js` — token catalog service
  ...
  - `plugins/.../blocks/ve-media-card.gjs` (and others) — migrate `control: "color"` → `control: "token-color"`
...
2. **Stop adding raw `control: "color"` to new blocks.** Default to component-token CSS vars (like `ve-category-banner` does today) until the token picker exists.
...
4. **Build the container-query foundation in `ve-layout`** soon — it's a small, decoupled change that pays off immediately and doesn't depend on schema decisions.
...
- Responsive: place a 4-column `ve:layout` in an outlet, narrow the browser → grid collapses to 1 column under 40rem (container query, not viewport). Same layout inside a sidebar collapses at the sidebar's width, not the viewport's.
- Responsive arg overrides: set `title` on a `ve:heading` with an `sm` override, simulate mobile in the toolbar, see the override render; switch to desktop, see the default render.
- Token picker: open a `ve:media-card`'s inspector → background color shows the curated list with swatches; pick "Surface subtle" → persisted as `"--sys-surface-subtle"`, rendered as `style="background: var(--sys-surface-subtle)"`.
