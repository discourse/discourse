...
**Design principle (matches the image-block pattern):** block components are pure renderers. They don't know about selection, edit mode, or the visual-editor service. The editor chrome wraps blocks and overlays editing UI on top.

The components split into two groups:

**Shared / live-site (dumb, no service):**
```
ve-heading.gjs / ve-paragraph.gjs       (chooses <h1..h6> / <p>, applies block styles)
...
**Edit path (admin-only).** The editor chrome wraps each block in a shell with `data-ve-block-id={{entry.id}}` (the wrapper already exists for selection/drag chrome). A canvas-level click handler does the gesture:

```js
onCanvasClick(event) {
  const target = event.target.closest("[data-ve-inline-edit-arg]");
  if (!target) return;
  const blockEl = target.closest("[data-ve-block-id]");
  if (!blockEl) return;
  const blockKey = blockEl.dataset.veBlockId;
  if (this.visualEditor.selectedBlockKey !== blockKey) return;  // click-on-selected gesture
  this.visualEditor.startEditingArg(blockKey, target.dataset.veInlineEditArg);
}
```

A new `<InlineEditController>`, mounted once at the canvas level, watches `(editingBlockKey, editingArgName)`. When set, it locates the active renderer span, reads `data-ve-inline-edit-schema` to pick the PM extension list, and mounts a `<ProsemirrorEditor>` into that span via `{{in-element}}`. The renderer's content is hidden (via a class toggle) while the editor is mounted, so the user sees the PM editor exactly where the rendered text was — same parent (`<h2>`, `<p>`), same styles.
...
`handleChange` calls `visualEditor.updateBlockArg(blockKey, argName, toStorage(docJson))`. Escape, blur to outside the block, and outside-clicks call `visualEditor.stopEditing()`. The toolbar is mounted as a child of the controller, so it lives and dies with the editor.
...
`VisualEditorService` already tracks `selectedBlockKey` and exposes a full undo/redo system (`_undoStack`, `_redoStack`, `undo()`, `redo()`, `_recordStructural()`, and args-shape undo entries `{ kind: "args", entry, prev, next }`). Inline edit integrates with this:
...
| `"heading"` | strong, em, link | none (Enter commits) | Single-line rich content (`ve:heading` text, `ve-media-card` title) |
| `"paragraph"` | strong, em, link | `hard_break` (Enter inserts) | Multi-line rich content (`ve:paragraph` text, `ve-cta-banner` content, `ve-callout` body) |
...
- Block selected → click on an element with `data-ve-inline-edit-arg` → canvas calls `visualEditor.startEditingArg(blockKey, argName)`. This implicitly commits + exits any other arg currently being edited.
- Escape, blur to outside the block, or click outside → canvas calls `visualEditor.stopEditing()`.
...
- `plugins/discourse-visual-editor/assets/javascripts/discourse/components/inline-rich-text-renderer.gjs` — Props: `@arg`, `@schema`, `@value` (`string | doc-JSON`). Emits `<span data-ve-inline-edit-arg data-ve-inline-edit-schema>` and walks runs via `<MarkedText>`.
- `plugins/discourse-visual-editor/assets/javascripts/discourse/components/marked-text.gjs` — recursive mark wrapper. Renders `<strong>`, `<em>`, `<a>`. `href` passes through `safeHref`.
- `plugins/discourse-visual-editor/assets/javascripts/discourse/lib/inline-rich-text.js` — `isInlineDoc`, `toDoc`, `toStorage`, `safeHref`, and a `SCHEMAS` map of `{ plain, heading, paragraph } → ProseMirror extension list`.
- `plugins/discourse-visual-editor/assets/stylesheets/common/inline-rich-text.scss` — renderer span styling, the `is-editing` class that hides renderer content while the editor mounts inside it, focus ring, placeholder.

**New — admin-only (editor chrome):**
- `plugins/discourse-visual-editor/admin/assets/javascripts/discourse/components/editor/inline-edit-controller.gjs` — mounted once at canvas level. Watches `(editingBlockKey, editingArgName)`; locates the matching renderer span; mounts `<ProsemirrorEditor>` into it via `{{in-element}}`. Reads `@schema` off the renderer's data-attr to pick the extension list. Handles `@change → updateBlockArg`, Escape / outside-click → `stopEditing`.
- `plugins/discourse-visual-editor/admin/assets/javascripts/discourse/components/editor/inline-edit-toolbar.gjs` — bold / italic / link bubble menu, subclasses `ToolbarBase`. Mounted by the controller; only when `schema !== "plain"`.
...
**Modified — plugin:**
- `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-heading.gjs` — replace `{{@text}}` with `<InlineRichTextRenderer @arg="text" @schema="heading" @value={{@text}} />` (wrapped in the heading tag). Change `args.text.type` from `"string"` to `"richInline"`.
- `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-paragraph.gjs` — same change with `@schema="paragraph"` (the outer `<p>` stays in the block template).
- `plugins/discourse-visual-editor/admin/assets/javascripts/discourse/services/visual-editor.js` — add `@tracked editingBlockKey`, `@tracked editingArgName`, `_editingPrevSnapshot`; methods `startEditingArg`, `stopEditing` (with undo-entry push on commit), `applyInlineEditChange` (per-keystroke, no undo push), `updateBlockArg` (split from `updateSelectedArg`).
- `plugins/discourse-visual-editor/admin/assets/javascripts/discourse/lib/schema-to-fields.js` — add `"rich-inline"` branch in `pickControl()`.
- `plugins/discourse-visual-editor/admin/assets/javascripts/discourse/components/editor/canvas.gjs` — add the canvas-level click handler that matches `[data-ve-inline-edit-arg]` inside the selected block; render `<InlineEditController />` once at canvas level; wire Escape / outside-click to `stopEditing`; suspend the canvas-level Cmd+Z handler while `editingBlockKey` is set.
...
2. **Renderer component test** — `plugins/discourse-visual-editor/test/javascripts/components/inline-rich-text-renderer-test.gjs`:
   - Renders a plain string as text.
   - Renders a doc-JSON with marks → `<strong>`, `<em>`, `<a>` wrapping.
   - Emits `data-ve-inline-edit-arg` / `data-ve-inline-edit-schema` on the root span.
   - `safeHref` returns `#` for invalid URLs; valid URLs flow through.
3. **Editor controller component test** — `plugins/discourse-visual-editor/test/javascripts/components/editor/inline-edit-controller-test.gjs`:
...
4. **System spec** — `plugins/discourse-visual-editor/spec/system/inline_text_editing_spec.rb`:
...
5. **Undo integration spec** — `plugins/discourse-visual-editor/spec/system/inline_text_editing_undo_spec.rb`:
...
**Modified — plugin:**
- `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-media-card.gjs` — adopt for `name` (`plain`), `role` (`plain`), `badgeLabel` (`plain`), `title` (`paragraph`), `ctaLabel` (`plain`). Switch each arg's `type` to `"richInline"`.
- `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-cta-banner.gjs` — `title` (`heading`), `content` (`paragraph`), `linkLabel` (`plain`).
- `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-callout.gjs` — `body` (`paragraph`).
- `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-button-link.gjs` — `label` (`plain`).
- `plugins/discourse-visual-editor/admin/assets/javascripts/discourse/components/editor/inline-edit-controller.gjs` — add a `Tab` / `Shift+Tab` keymap to the PM editor: on Tab, find the next `[data-ve-inline-edit-arg]` in DOM order within the same `[data-ve-block-id]`, call `startEditingArg` for it (which implicitly commits the current one). Shift+Tab walks backwards. If no next/previous exists in the current block, leave the keystroke alone (default browser behavior).

**Example adoption (ve-media-card):**

```gjs
<template>
  <div class="ve-media-card">
    <Image @src={{@avatarUrl}} />
    <div class="ve-media-card__content">
...
**Verification:**
1. **Multi-field system spec** — `plugins/discourse-visual-editor/spec/system/inline_text_editing_multi_field_spec.rb`:
   - Open a layout with a `ve-media-card`.
...
Brings the editor in line with Notion / Discourse-composer conventions for paragraph-flow editing. The visual editor's existing `_recordStructural` mechanism wraps these layout mutations into single undo entries automatically.
...
- **On Enter:** intercept the PM keymap. Compute the cursor offset in the doc; split `entry.args.text` into a `before` doc and an `after` doc (PM has helpers for this). Mutate the layout: replace the current entry's `args.text` with `before`; insert a new sibling `ve:paragraph` entry immediately after with `args.text = after`. Move edit focus to the new entry, place the PM cursor at position 0. Wrap the whole mutation in `_recordStructural` so it's one undo entry.
- **On Backspace at position 0:** if the previous sibling in the layout is a `ve:paragraph`, merge: concatenate previous's `text` doc with the current's, replace previous's `args.text` with the merged doc, remove the current entry, move edit focus to previous with cursor placed at the join point. Also wrapped in `_recordStructural`.
...
**Modified — plugin:**
- `plugins/discourse-visual-editor/admin/assets/javascripts/discourse/components/editor/inline-edit-controller.gjs` — extend the PM keymap with Enter / Backspace / arrow handlers as above. Each handler calls into the service for the layout mutation, then re-focuses the editor on the destination block.
- `plugins/discourse-visual-editor/admin/assets/javascripts/discourse/services/visual-editor.js` — new methods: `splitInlineEditAt(cursorPos)`, `mergeInlineEditWithPrevious()`, `enterEditFromNeighbor({ direction, blockKey, cursorHint })`. All wrap layout mutations in `_recordStructural`.
- `plugins/discourse-visual-editor/admin/assets/javascripts/discourse/lib/mutate-layout.js` — small helpers for "find previous/next text-bearing sibling in layout order" if not already present.

**Verification:**
1. **Enter-split system spec** — `plugins/discourse-visual-editor/spec/system/inline_text_editing_block_flow_spec.rb`:
   - Edit a paragraph with content "Hello world", place cursor between "Hello " and "world", press Enter.
   - Verify two `ve:paragraph` blocks exist: first with "Hello ", second with "world"; cursor visible in second block at position 0.
