# Container query foundation for `ve:layout`

Detailed execution plan for step 4 of `RESPONSIVE_AND_TOKENS_PLAN.md`: give `ve:layout` automatic adaptation to narrow widths via CSS container queries — zero author burden, no schema commitments, decoupled from later per-arg responsive overrides.

## Context

Today `ve:layout` renders a single layout regardless of available width:
...
## Current state (verified from `ve-layout.gjs`)
...
Each grid child gets a `.ve-layout__cell` wrapper with `cellStyle()` (lines 243-257):
...
"--ve-layout-cols: repeat(6, 1fr); --ve-layout-rows: repeat(2, minmax(80px, auto)); --ve-layout-gap: 1rem; --ve-layout-align: stretch; padding: ...; position: relative;"
```

```scss
// In visual-editor.scss:
.ve-layout--grid {
  display: grid;
  grid-template-columns: var(--ve-layout-cols, repeat(6, 1fr));
  grid-template-rows: var(--ve-layout-rows, repeat(2, minmax(80px, auto)));
  gap: var(--ve-layout-gap, 1rem);
  align-items: var(--ve-layout-align, stretch);
...
The inline custom property `--ve-layout-cols` is only consumed by the base rule's `var(...)` lookup. The `@container` rule sets `grid-template-columns` directly, bypassing the custom-property — so it wins by cascade order, no `!important` needed.
...
`ve-layout.gjs` lines 284-331 → emit only custom-property setters:
...
Move `display: grid`, `display: flex`, `flex-direction`, `padding`, `position: relative`, and the `transition` rule into the stylesheet against `.ve-layout--grid` / `.ve-layout--row` / `.ve-layout--stack` selectors.

### B. Refactor `cellStyle()` similarly

`ve-layout.gjs` lines 243-257:
...
cellStyle = (containerArgs) => {
  ...
  return trustHTML(
    `--ve-cell-column: ${grid.column ?? "auto"}; ` +
      `--ve-cell-row: ${grid.row ?? "auto"}; ` +
      `--ve-cell-align: ${grid.align ?? "stretch"}; ` +
      `--ve-cell-justify: ${grid.justify ?? "stretch"};`
  );
};
```
...
```scss
.ve-layout--grid > .ve-layout__cell {
  grid-column: var(--ve-cell-column, auto);
  grid-row: var(--ve-cell-row, auto);
  display: grid;
  place-items: var(--ve-cell-align, stretch) var(--ve-cell-justify, stretch);
  min-width: 0;
  min-height: 0;
}
```
...
The editor-only `padding: var(--visual-editor-container-margin)` and `position: relative` for grid mode → move to a CSS rule scoped to `.ve-layout--grid` (no reason to keep them inline). The CSS variable resolves to empty on the live page (it's defined in the editor's chrome scope), so the rule is a no-op outside the editor.

```scss
.ve-layout--grid {
  position: relative;
  padding: var(--visual-editor-container-margin, 0);
}
```

## Files to modify

- `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-layout.gjs`
  - `containerStyle` getter (lines 284-331) → custom-property hand-off
  - `cellStyle` arrow fn (lines 243-257) → custom-property hand-off
- `plugins/discourse-visual-editor/assets/stylesheets/visual-editor.scss`
  - New `.ve-layout`, `.ve-layout--grid`, `.ve-layout--row`, `.ve-layout--stack` rules with `container-type` / layout properties / transitions
  - New `@container ve-layout` block with collapse rules
  - New `.ve-layout--grid > .ve-layout__cell` rule

No JS modifier or component changes outside `ve-layout.gjs` itself. The grid overlay needs no edits (it reads computed styles, which already reflect the collapsed state).

## Implementation steps (ordered)

1. **Add the stylesheet rules first** (with no behavioural change). Define `.ve-layout--grid` / `.ve-layout--row` / `.ve-layout--stack` reading from custom properties with sensible defaults. Don't add `@container` rules yet. Verify the existing live page renders identically (the inline styles still win at this stage, so visually nothing changes; this is a defence-in-depth step).
...
4. **Add `container-type: inline-size; container-name: ve-layout` to `.ve-layout`.** Still no behavioural change.
5. **Add the `@container ve-layout (max-width: 40rem)` block** with the collapse rules. NOW the layout adapts at narrow widths.
...
- **Nested layouts.** A `ve:layout` inside another `ve:layout`'s child slot also has `container-name: ve-layout`. The inner query matches the *innermost* containing element with that name — so the inner layout reads its own width, not the outer's. Correct behaviour by spec; just worth noting.
...
1. `bin/lint --fix plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-layout.gjs plugins/discourse-visual-editor/assets/stylesheets/visual-editor.scss`
2. `bin/qunit plugins/discourse-visual-editor/test/javascripts/unit/lib/mutate-layout-test.gjs` — existing unit tests green.
3. Live page (after step 5):
   - Place a 4-column `ve:layout` with four children at outlet root. Narrow the browser to < 640px. Expect grid → 1 column, each child full-width.
...
5. Row-mode layout:
   - `ve:layout` mode=row with three children in an outlet. Narrow → flex-direction: column. Children stack.

## Risks & rollback

- **Risk**: an existing site theme has CSS overrides that target `.ve-layout` with their own `grid-template-columns`. Those overrides would now win regardless of inline style (since we moved away from inline). **Mitigation**: low likelihood for a feature this new; we control the only published consumer.
- **Risk**: the custom-property fallback chain has a typo and a layout's `grid-template-columns` resolves to the literal string `var(--ve-layout-cols, ...)` because the custom property isn't set. **Mitigation**: each step is independently testable; defaults inside `var(...)` mean the base rule still renders something sensible even without the custom property.
