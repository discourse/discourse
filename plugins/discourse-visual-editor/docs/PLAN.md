# Discourse Visual Editor — Phased Plan
...
We want a **drag-and-drop visual editor** as a Discourse plugin (`discourse-visual-editor`) that lets admins compose layouts without writing code, with parity for the rest of the industry (Puck, Gutenberg/FSE, Plasmic, Webflow patterns).
...
- **The visual editor is a *layout* editor, not a CSS editor.** Block args (variant/tone/direction/gap/align) and design tokens from the active theme palette are the user-facing styling surface. Raw CSS stays in the existing Ace-based theme editor.
...
Each theme's edit page (`/admin/customize/themes/:id`) gets a **"Visual Editor"** button alongside the existing Ace-based code editor for CSS/JS. Clicking opens the editor in the context of that theme; a page-picker (homepage / categories / topic / user profile / …) chooses which page mounts in the canvas. Edits land on that theme's `block_layout` ThemeFields, or its auto-created `<theme-name>-customizations` child component if the theme is Git-imported.

This is the canonical entry. Theme authors and power admins manage all customizations (CSS/JS via Ace, layouts via the visual editor) from the same place. It mirrors the existing customization pattern: `admin → customize → themes → show → [edit something]`.

**Secondary — in-context "Edit page" pill** (Phase 1+).
A small floating pill at the bottom-right of any page with at least one mounted `<BlockOutlet>`, visible only to users in `visual_editor_allowed_groups` (default: admins). Click → enters editor mode on the current page, editing the active theme.
...
**Permissions**:
- Site setting `visual_editor_enabled` (default: off during early phases, on once stable).
- Site setting `visual_editor_allowed_groups` (default: admins). The implicit `admins` group covers staff; other groups are opt-in.
...
2. Body class `body.visual-editor-active` toggles editor CSS.
...
@block("hero-banner", {
  ...
  thumbnail: "/plugins/discourse-visual-editor/images/hero-banner.svg",
...
**Ships**:
- Plugin scaffold; gated by `visual_editor_enabled` site setting + `visual_editor_allowed_groups`.
- **Plan archive**: `plugins/discourse-visual-editor/docs/PLAN.md` — copy of this planning document, committed alongside the scaffold so the architectural intent travels with the code regardless of session state. Updated as the plan evolves.
...
- **Test backfill for Phase 1's surface** (deferred from Phase 1):
  - qunit unit tests for `VisualEditorService` (`canEdit` matrix, `isBlockSelected`, `selectBlock`).
...
- **Theme-admin entry**: "Visual Editor" button on `/admin/customize/themes/:id` with a page-picker.
...
- Optional dedicated `/admin/customize/visual-editor` route with a page-picker sidebar — still uses in-place rendering.
...
2. **Replace the admin theme editor or live alongside?** Theme editor handles CSS/JS source code; visual editor handles layout. Recommend alongside, with a unified `/admin/customize` landing surfacing both.
