...
Templates write slot entries directly into `ve:layout.children` — applying "hero + 3" produces four `ve:slot` entries with `containerArgs.grid` matching the template's areas. The author fills them one by one; the layout's shape persists through fill/delete cycles.

## Approach

### 1. New block: `ve:slot`

**File**: `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-slot.gjs`.
...
export default class VESlot extends Component {
  <template>
    {{! Renders nothing on the live page. The editor's BlockChrome
        ...
        placeholder UI) when it sees `blockName === "ve:slot"`. }}
  </template>
}
```
...
- Validator accepts leaf blocks freely — `ve:slot` passes validation without changes to `validateContainerChildren`.
...
**File**: `plugins/discourse-visual-editor/assets/javascripts/discourse/components/editor/block-chrome.gjs`.

When the wrapped block is `ve:slot` (`@blockName === "ve:slot"`), the chrome:
...
The placeholder's "Pick a block" picker calls `visualEditor.fillSlot({slotKey, blockName, previewArgs})` (new service action below).

### 3. Service: fill-on-drop + restore-on-delete

**File**: `plugins/discourse-visual-editor/assets/javascripts/discourse/services/visual-editor.js`.
...
// it also inserts a `ve:slot` entry at the same position with the
...
```js
{
  ...
  slotEntries: [ // children to insert
    { block: "ve:slot", containerArgs: { grid: { column, row, align: "stretch", justify: "stretch" } } },
    ...
  ],
}
```
...
- **Template with slots**, where `n = existing children count`, `s = template slot count`:
  ...
  - `0 < n ≤ s`: **preserve existing children, fit them into the template**. Walk children in document order, assigning each to the next slot — overwrite that child's `containerArgs.grid` with the slot's rect. Append `ve:slot` entries for the remaining `(s - n)` slots. Apply silently.
...
- **No `args.slots` on `ve:layout`.** Reverted.
- **No new dragging affordances in the grid overlay.** The chrome already handles drops on positioned entries.
- **No validator changes.** `ve:slot` is a leaf block — `validateContainerChildren` is satisfied.
- **No live-page rendering changes.** A `ve:slot` on a published page renders nothing visible inside its grid cell, just as if the author left an empty positioned div there. Themes can `display: none` the `.block-ve-slot` class if they want to remove the slot's cell allocation in production.

## Files to add

- `plugins/discourse-visual-editor/assets/javascripts/discourse/blocks/ve-slot.gjs` — the slot block (no-op render).
- `plugins/discourse-visual-editor/assets/javascripts/discourse/components/editor/empty-cell-placeholder.gjs` — extracted `+` UI + palette picker, used by both `block-chrome.gjs` (for `ve:slot` entries) and `grid-overlay.gjs` (for auto-detected empty cells). Avoids duplicating the picker markup.

## Files to modify

- `plugins/discourse-visual-editor/assets/javascripts/discourse/lib/mutate-layout.js` — add `replaceEntry(layout, key, newEntry)`. Extend `removeEntry` so that when the removed entry has `__fromSlot === true`, the returned layout includes a freshly-minted `ve:slot` entry at the same `containerArgs.grid`. This keeps the restore logic in one place rather than scattered across delete code paths.
- `plugins/discourse-visual-editor/assets/javascripts/discourse/services/visual-editor.js` — add `fillSlot({slotKey, blockName, defaultArgs})`. Rewrite `applyGridTemplate` per the apply-algorithm section above (frame-only vs. slot-template; preserve-and-fit when `0 < n ≤ s`; refuse when `n > s`).
- `plugins/discourse-visual-editor/assets/javascripts/discourse/lib/grid-templates.js` — restructure each template with an optional `areas` string; export `resolveTemplateLayout(template)`. Add the 6 new entries.
- `plugins/discourse-visual-editor/assets/javascripts/discourse/components/editor/block-chrome.gjs` — branch on `@blockName === "ve:slot"`: render the placeholder instead of `<@WrappedComponent />`, route drops to `replaceEntry`. Keep resize handle / drag handle / selection.
- `plugins/discourse-visual-editor/assets/javascripts/discourse/components/editor/grid-overlay.gjs` — replace inline cell-picker markup with `<EmptyCellPlaceholder>`.
- `plugins/discourse-visual-editor/assets/javascripts/discourse/components/editor/inspector-layout-form.gjs` — `TemplatePreview` thumbnail reads `resolveTemplateLayout(template)` to render slot rects (same as last attempt's `parseGridAreas` flow, but the parser lives in `grid-templates.js` now).
- `plugins/discourse-visual-editor/plugin.rb` — `register_svg_icon "border-none"` is already there.
- `plugins/discourse-visual-editor/config/locales/client.en.yml` — strings for the 6 new template names + a `slot.placeholder_label` ("Pick a block for this slot").
- `plugins/discourse-visual-editor/assets/stylesheets/visual-editor.scss` — `.ve-slot` chrome styling (same dashed amber as empty cells, but at the slot's full rect via `containerArgs.grid`).
...
- **`insertBlockAtCell` service action**: `services/visual-editor.js:2871` — model for the structural-undo wrapping around `fillSlot`.
...
2. `bin/qunit plugins/discourse-visual-editor/test/javascripts/unit/lib/mutate-layout-test.gjs` — add cases:
   - `replaceEntry` swaps in a new entry at a key, preserves siblings.
   - `removeEntry` of an entry with `__fromSlot: true` returns a layout containing a `ve:slot` at the same `containerArgs.grid`.
...
3. Open the editor on a homepage with a registered layout. Select the `ve:layout`. Click the "Hero + 3" template chip.
   - Confirm the thumbnail matches what gets applied.
   - Confirm 4 `ve:slot` entries appear on the canvas, each rendering the `+` placeholder.
   - Confirm the hero slot spans 3 columns; the bottom row has 3 single-cell slots.
4. Click the hero slot's `+`, pick a `ve:cta-banner`. Confirm the slot is replaced by the cta-banner, sized to span the same 3 columns. The other 3 slots remain.
...
9. Save the layout. Reload the page. Confirm slots persist as `ve:slot` entries in the saved JSON. Confirm the live page renders the filled cells normally and renders nothing visible (but allocates the grid cell) for empty slots.

## Decisions locked

- **Save semantics**: slots persist as `ve:slot` entries. Live page renders empty cells (no visible content; grid cell is still allocated by `containerArgs.grid`). Themes can `display: none .block-ve-slot` if they want production pages to collapse empty cells.
