...
@block("ve:category-banner", {
  ...
  args: {
    showLogo: {
      ...
      ui: {
        control: "toggle",
        label: i18n("visual_editor.inspector.category_banner.show_logo"),
      },
    },
    showIcon: {
      type: "boolean",
      ...
      ui: {
        control: "toggle",
        label: i18n("visual_editor.inspector.category_banner.show_icon"),
      },
    },
    showDescription: {
      type: "boolean",
      ...
      ui: {
        control: "toggle",
        label: i18n("visual_editor.inspector.category_banner.show_description"),
      },
    },
  },
})
export default class VECategoryBanner extends Component {
  ...
  get safeStyle() {
    return trustHTML(
      `--ve-category-banner-background: #${this.category.color}; ` +
        `--ve-category-banner-color: #${this.category.text_color};`
    );
  }
  ...
  <template>
    {{#if this.shouldRender}}
      ...
      >
        {{#if this.category}}
          <div class="ve-category-banner__content">
            {{#if @showLogo}}
              <CategoryLogo
                class="ve-category-banner__logo"
                @category={{this.category}}
              />
            {{/if}}

            <h2 class="ve-category-banner__title">
            ...
            {{#if this.displayCategoryDescription}}
              <div class="ve-category-banner__description">
                <div class="cooked">
            ...
            {{#if this.hasSubcategories}}
              <ul
                class="ve-category-banner__subcategories"
                aria-label="Subcategories"
              >
                {{#each this.category.subcategories as |subcategory|}}
                  <li class="ve-category-banner__subcategory">
                    <CategoryCard
                      @category={{subcategory}}
                      @href={{subcategory.url}}
                      @class="ve-category-banner__subcategory-card"
                    />
                  </li>
                {{/each}}
              </ul>
