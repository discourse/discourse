 ...
 *
 * Receives the curry args from `api-initializers/visual-editor.js`:
 ...
 */
export default class VEGhostBlock extends Component {
  get iconName() {
  ...
  get hintMessage() {
    ...
    switch (this.args.failureType) {
      case FAILURE_TYPE.OPTIONAL_MISSING:
        return i18n("visual_editor.canvas.ghost.optional_missing");
      case FAILURE_TYPE.UNKNOWN_BLOCK:
        return i18n("visual_editor.canvas.ghost.unknown_block");
      case FAILURE_TYPE.NO_VISIBLE_CHILDREN:
        return i18n("visual_editor.canvas.ghost.no_visible_children");
      default:
        return i18n("visual_editor.canvas.ghost.condition_failed");
    }
  }

  <template>
    <div class={{dConcatClass "ve-ghost-block" (if this.isError "--error")}}>
      <div class="ve-ghost-block__header">
        <span class="ve-ghost-block__icon" aria-hidden="true">
          {{dIcon this.iconName}}
        </span>
        <span class="ve-ghost-block__name">{{@blockName}}</span>
        {{#if @blockId}}
          <span class="ve-ghost-block__id">#{{@blockId}}</span>
        {{/if}}
      </div>
      <p class="ve-ghost-block__hint">{{this.hintMessage}}</p>
      {{#if @ghostChildren.length}}
        <div class="ve-ghost-block__children">
