...
const NAMESPACE_LABEL_KEYS = {
  core: "visual_editor.palette.category_core",
  plugin: "visual_editor.palette.category_plugin",
  theme: "visual_editor.palette.category_theme",
};
...
export default class PalettePanel extends Component {
  ...
  <template>
    <div class="visual-editor-palette">
      <input
        type="search"
        class="visual-editor-palette__search"
        placeholder={{i18n "visual_editor.palette.search_placeholder"}}
        value={{this.searchTerm}}
        {{on "input" this.updateSearchTerm}}
      />

      <div class="visual-editor-palette__chips">
        {{#each this.categories as |category|}}
          <button
            type="button"
            class={{dConcatClass
              "visual-editor-palette__chip"
              (if (this.isCategoryActive category) "--active")
            }}
            {{on "click" (fn this.toggleCategory category)}}
          >
            {{this.labelFor category}}
          </button>
        {{/each}}
      </div>
