 ...
 * dropdowns thread their picked value into the visual-editor service's
...
export default class SimulationControls extends Component {
  ...
  get currentPersona() {
    const sim = this.visualEditor.simulation;
    if (!sim || !("user" in sim)) {
      return "real";
    }
    if (sim.user === null) {
  ...
  handlePersonaChange(event) {
    ...
    if (key === "real") {
      this.visualEditor.setSimulatedUser(undefined);
      return;
    }
    ...
    this.visualEditor.setSimulatedUser(PERSONAS[key]);
  }
  ...
  handleViewportChange(event) {
    const key = event.target.value;
    if (key === "real") {
      this.visualEditor.setSimulatedViewport(undefined);
      return;
    }
    ...
    this.visualEditor.setSimulatedViewport(buildSimulatedViewport(pick));
  }

  @action
  clear() {
    this.visualEditor.clearSimulation();
  }
  ...
  <template>
    <div
      class={{dConcatClass
        ...
        (if this.visualEditor.isSimulating "--active")
      }}
    >
      <select
        class="visual-editor-simulation__persona"
        aria-label={{i18n "visual_editor.chrome.simulation.persona_label"}}
        title={{i18n "visual_editor.chrome.simulation.persona_label"}}
        {{on "change" this.handlePersonaChange}}
      >
        <option value="real" selected={{eq this.currentPersona "real"}}>
          {{i18n "visual_editor.chrome.simulation.persona_real"}}
        </option>
        <option
          value="anonymous"
          selected={{eq this.currentPersona "anonymous"}}
        >
          {{i18n "visual_editor.chrome.simulation.persona_anonymous"}}
        </option>
        <option value="tl0" selected={{eq this.currentPersona "tl0"}}>
          {{i18n "visual_editor.chrome.simulation.persona_tl0"}}
        </option>
        <option value="tl1" selected={{eq this.currentPersona "tl1"}}>
          {{i18n "visual_editor.chrome.simulation.persona_tl1"}}
        </option>
        <option value="tl2" selected={{eq this.currentPersona "tl2"}}>
          {{i18n "visual_editor.chrome.simulation.persona_tl2"}}
        </option>
        <option value="tl3" selected={{eq this.currentPersona "tl3"}}>
          {{i18n "visual_editor.chrome.simulation.persona_tl3"}}
        </option>
        <option value="tl4" selected={{eq this.currentPersona "tl4"}}>
          {{i18n "visual_editor.chrome.simulation.persona_tl4"}}
        </option>
        <option value="admin" selected={{eq this.currentPersona "admin"}}>
          {{i18n "visual_editor.chrome.simulation.persona_admin"}}
        </option>
      </select>

      <select
        class="visual-editor-simulation__viewport"
        aria-label={{i18n "visual_editor.chrome.simulation.viewport_label"}}
        title={{i18n "visual_editor.chrome.simulation.viewport_label"}}
        {{on "change" this.handleViewportChange}}
      >
        <option value="real" selected={{eq this.currentViewport "real"}}>
          {{i18n "visual_editor.chrome.simulation.viewport_real"}}
        </option>
        <option value="mobile" selected={{eq this.currentViewport "mobile"}}>
          {{i18n "visual_editor.chrome.simulation.viewport_mobile"}}
        </option>
        <option value="tablet" selected={{eq this.currentViewport "tablet"}}>
          {{i18n "visual_editor.chrome.simulation.viewport_tablet"}}
        </option>
        <option value="desktop" selected={{eq this.currentViewport "desktop"}}>
          {{i18n "visual_editor.chrome.simulation.viewport_desktop"}}
        </option>
      </select>

      {{#if this.visualEditor.isSimulating}}
        <span
          class="visual-editor-simulation__dot"
          role="status"
          title={{i18n "visual_editor.chrome.simulation.disclosure_tooltip"}}
        >
          {{dIcon "circle"}}
        </span>
      {{/if}}
