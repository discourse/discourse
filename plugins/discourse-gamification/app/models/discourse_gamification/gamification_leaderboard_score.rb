...
module DiscourseGamification
  class GamificationLeaderboardScore < ::ActiveRecord::Base
    ...
    class << self
      def enabled_scorables(leaderboard:)
        Scorable.subclasses.filter { it.enabled?(leaderboard:) }
      end

      def scorables_queries(leaderboard:)
        enabled_scorables(leaderboard:).map { "( #{it.query(leaderboard:)} )" }.join(" UNION ALL ")
      end

      def calculate_scores(leaderboard, since_date: Date.today, only_subclass: nil)
        queries =
          if only_subclass
