...
module("Integration | Wireframe | wf:image block", function (hooks) {
  ...
  test("renders a plain image when only the light variant is set", async function (assert) {
    withPluginApi((api) =>
      api.renderBlocks("hero-blocks", [
        { block: WFImage, args: { image: LIGHT_IMG, alt: "Light" } },
      ])
    );

    await render(<template><BlockOutlet @name="hero-blocks" /></template>);

    assert.dom("img.wf-image").exists();
  ...
  test("wraps the image in an anchor when link is set", async function (assert) {
    ...
    assert.dom("a.wf-image img").exists("image is the anchor's child");
  });
  ...
  test("wraps the image in a figure with figcaption when caption is set", async function (assert) {
    ...
    assert.dom("figure.wf-image").exists();
    assert.dom("figure.wf-image img").exists();
    assert
      .dom("figure.wf-image figcaption.wf-image__caption")
  ...
  test("supports caption + link together", async function (assert) {
    ...
    assert.dom("figure.wf-image").exists();
    assert
      .dom("figure.wf-image > a")
      .exists("anchor wraps the image, inside the figure");
    assert
      .dom("figure.wf-image > figcaption")
