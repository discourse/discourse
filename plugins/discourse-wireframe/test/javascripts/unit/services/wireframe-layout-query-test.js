...
// Looks up the layout-query service and stubs its four core-reader seams with
// controllable fixtures. `layouts` is an outlet->layout map; `metaByOutlet` is
// an outlet->resolved-meta map (provenance); `blocksByName` maps a registry
// name to its class (what the blocks service would return).
function buildQuery(
  owner,
  {
    layouts = {},
  ...
  const query = owner.lookup("service:wireframe-layout-query");
  sinon
    .stub(query, "_resolvedLayout")
    .callsFake((outletName) => layouts[outletName] ?? null);
  sinon.stub(query, "_resolvedLayouts").callsFake(() => layoutMap);
  sinon
    .stub(query, "_resolvedLayoutMeta")
    .callsFake((outletName) => metaByOutlet[outletName] ?? null);
  sinon.stub(query, "_block").callsFake((name) => blocksByName[name] ?? null);
  return query;
}

module(
  "Unit | Discourse Wireframe | service:wireframe-layout-query",
  function (hooks) {
    setupTest(hooks);
    ...
    module("readResolvedLayout", function () {
      test("returns the outlet's layout, or null when absent", function (assert) {
        ...
        const query = buildQuery(this.owner, { layouts: { home: layout } });
    ...
    module("findEntryAndOutletSync / findEntryByKey", function () {
      test("locates a top-level entry and its outlet", function (assert) {
        ...
        const query = buildQuery(this.owner, { layouts: { home: [tile] } });
      ...
      test("returns null when not found, and skips records with no layout", function (assert) {
        const query = buildQuery(this.owner, { layouts: { home: [] } });
    ...
    module("findEntryParent / isAncestorOf", function () {
      ...
      test("isAncestorOf walks the full ancestry path", function (assert) {
        ...
        const query = buildQuery(this.owner, { layouts: { home: [root] } });
    ...
    module("resolvePartContext", function () {
      ...
      test("returns null when the composite can't be found", function (assert) {
        const query = buildQuery(this.owner, { layouts: { home: [] } });
        assert.strictEqual(
          query.resolvePartContext("card:7::part::title"),
          null
        );
      });
    });
    ...
    module("grid predicates", function () {
      ...
      test("isCellInGrid checks the entry's parent is the named grid", function (assert) {
        ...
        const query = buildQuery(this.owner, { layouts: { home: [grid] } });
    ...
    module("outlet-root identity", function () {
      ...
      test("recordOutletRoot is a no-op when the outlet has no layout", function (assert) {
        const query = buildQuery(this.owner, { layouts: {} });
        query.recordOutletRoot("home");
        assert.strictEqual(query.outletRootKey("home"), null);
      });
    });
