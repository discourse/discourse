...
// block-chrome-click integration test (through the orchestrator facade). These cases
// cover the service's wiring and its guard paths, which need no rendered layout.
module(
  "Unit | Discourse Wireframe | service:wireframe-inplace-link",
  function (hooks) {
    setupTest(hooks);

    hooks.beforeEach(function () {
      this.linkEdit = getOwner(this).lookup("service:wireframe-inplace-link");
    });
