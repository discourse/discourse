...
// block-chrome (through the orchestrator facade). These cases cover the service's
// wiring and its guard paths, which need no rendered layout or open menu.
module(
  "Unit | Discourse Wireframe | service:wireframe-inplace-icon",
  function (hooks) {
    setupTest(hooks);

    hooks.beforeEach(function () {
      this.iconEdit = getOwner(this).lookup("service:wireframe-inplace-icon");
    });
