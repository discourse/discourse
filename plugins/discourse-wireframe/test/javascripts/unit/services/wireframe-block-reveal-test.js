...
// Looks up the block-reveal service and stubs the layout-query reads it uses on
// the tab-reveal path (`located`/`layout` fixtures). The flash path needs
// neither, so they default to "nothing found".
function buildReveal(owner, { located = null, layout = null } = {}) {
  const layoutQuery = owner.lookup("service:wireframe-layout-query");
  sinon.stub(layoutQuery, "findEntryAndOutletSync").callsFake(() => located);
  sinon.stub(layoutQuery, "readResolvedLayout").callsFake(() => layout);
  return owner.lookup("service:wireframe-block-reveal");
}

module(
  "Unit | Discourse Wireframe | service:wireframe-block-reveal",
  function (hooks) {
    setupTest(hooks);
    ...
    module("flash", function () {
      ...
      test("flashes an already-rendered element", async function (assert) {
        const reveal = buildReveal(this.owner);
    ...
    module("revealSelection — containing tabs", function () {
      ...
      test("no-ops when the entry can't be located", function (assert) {
        const reveal = buildReveal(this.owner, { located: null });
        const btn = tabButton("tabs:1", false);
        let clicked = false;
        btn.addEventListener("click", () => (clicked = true));
