...
/** Result of attempting to update the selected entry's identifier. */
export type UpdateSelectedEntryIdResult = {
  /** Whether the identifier was committed. */
  ok: boolean;
  /** Validation or lookup failure, or `null` after a valid attempt. */
  error: "no-selection" | "invalid-format" | "not-found" | null;
};
...
export default class WireframeEntryConfigService extends Service {
   ...
   *
   * @returns Whether a composite was detached.
   */
  detachSelectedComposite(): boolean {
   ...
   *
   * @param namespace - The childArgs namespace key (e.g. "grid").
   * @param name - The field name inside the namespace.
   * @param value - Replacement value for the namespaced field.
   * @returns Whether the field was updated.
   */
  updateSelectedContainerArg(
    namespace: string,
    name: string,
    value: unknown
  ): boolean {
   ...
   * refresh.
   *
   * @param parsed - The parsed JSON, already validated by the caller
   *   (`InspectorRawJson` rejects invalid JSON without calling us).
   * @returns Whether the selected entry was replaced.
   */
  replaceSelectedEntryRaw(parsed: LayoutEntry): boolean {
    const key = this.wireframeSelection.selectedBlockKey;
    if (!key) {
      return false;
    }
   ...
   *
   * @param newConditions - Replacement visibility conditions, or `null` to clear them.
   * @returns `true` on success, `false` when no block is selected or the
   *   selection isn't locatable in the live layout.
   */
  updateSelectedConditions(
    newConditions: LayoutEntry["conditions"] | null
  ): boolean {
    const key = this.wireframeSelection.selectedBlockKey;
    if (!key) {
      return false;
   ...
   *
   * @param nextId - Replacement identifier, or `null` to clear it.
   * @returns Whether the update committed and any validation or lookup error.
   */
  updateSelectedEntryId(nextId: string | null): UpdateSelectedEntryIdResult {
    const key = this.wireframeSelection.selectedBlockKey;
    if (!key) {
