...
import type WireframeGridPlacementService from "./wireframe-grid-placement";
...
export default class WireframeDropDispatchService extends Service {
  /** Structural block commands dispatched for ordinary container drops. */
  @service declare wireframeBlockMutations: WireframeBlockMutationsService;

  /** Drag-overlay service that owns the pending dispatch payload. */
  @service declare wireframeDragOverlay: WireframeDragOverlayService;

  /** Grid command service dispatched for grid-specific drops. */
  @service declare wireframeGridPlacement: WireframeGridPlacementService;

  /**
   * Creates the dispatcher and registers it with the drag overlay.
   *
   ...
   *
   * @param payload - Structural command selected by the active drop target.
   * @returns `true` when the named action ran.
   */
  run(payload: DropDispatch): boolean {
    switch (payload.action) {
      case "insertBlock":
        this.wireframeBlockMutations.insertBlock(payload.args);
        return true;
      case "moveBlock":
        this.wireframeBlockMutations.moveBlock(payload.args);
        return true;
      case "applyGridDrop":
        this.wireframeGridPlacement.drop(payload.args);
        return true;
      case "moveBlockIntoCell":
        this.wireframeGridPlacement.moveIntoCell(payload.args);
        return true;
      case "placeBlockInCell":
        this.wireframeGridPlacement.placeInCell(payload.args);
        return true;
      default:
        return false;
    }
