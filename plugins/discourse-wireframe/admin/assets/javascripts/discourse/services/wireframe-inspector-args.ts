...
export default class WireframeInspectorArgsService extends Service {
  ...
  /**
   * Creates the service and registers its pre-selection-change flush boundary.
   *
   ...
   *
   * @param argName - Name of the selected block argument being edited.
   * @param value - Latest value entered for the argument.
   */
  @action
  updateSelectedArg(argName: string, value: unknown): void {
   ...
   *
   * @returns `true` when the flush touched an entry.
   */
  async #flushPendingArgs(): Promise<boolean> {
   ...
   *
   * @param partContext - Composite and override path owning the selected part.
   * @param pending - Latest pending value for each edited argument.
   * @returns `true` when the override map changed.
   */
  #flushPendingPartArgs(
    {
      compositeKey,
      outletName,
      partPath,
    }: NonNullable<
      ReturnType<WireframeLayoutQueryService["resolvePartContext"]>
    >,
