 ...
 * selected). It never reaches back up into the orchestrator; its consumers (the
 * inspector tabs, the condition tree, the toolbar) inject it directly.
 */
export default class WireframeEntryConfigService extends Service {
   ...
   * (structural commit) rather than the keystroke-debounced inspector-args pipeline.
   *
  ...
  updateSelectedEntryId(nextId) {
    ...
    const committed = this.wireframeMutationEngine.recordStructural(
      [located.outletName],
      () => {
        const layout = this.wireframeLayoutQuery.readResolvedLayout(
          located.outletName
