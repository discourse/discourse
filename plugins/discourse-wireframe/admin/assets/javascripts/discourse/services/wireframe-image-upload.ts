...
export default class WireframeImageUploadService extends Service {
   ...
   */
  #pendingDropFiles = new Map<string, File>();
   ...
   *
   * @param file - The image file to upload into the new block.
   * @returns `true` when a block was created and the file staged.
   */
  completeExternalImageDrop(file: File): boolean {
    if (!file) {
      return false;
    }
   ...
   */
  #installImagePasteListener(): void {
