 ...
 * services that sit downstream of it — the revision beacon (`bump`), the
 ...
 * the kernel that drives it; the kernel keeps thin facade delegators so its
 ...
 */
export default class WireframeEditEngineService extends Service {
   ...
   * persistence service's post-publish cleanup. Leaves the layout on the canvas.
   *
   * @param {string} outletName
   */
   ...
   * persisted-draft baseline (owned by the kernel) to the caller.
   *
   * @param {string} outletName
   */
   ...
   * kernel can clear their `session-draft` layers.
   *
  ...
  publishStructuralChange(outletName, newLayout) {
    ...
    _setLayoutLayer(
      ...
      // the kernel's draft materialisation. Without this, dragging the only
