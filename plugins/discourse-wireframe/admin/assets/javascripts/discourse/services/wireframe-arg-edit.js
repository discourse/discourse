 ...
 * the kernel that drives it; the kernel keeps a thin `updateSelectedArg` facade
 ...
 * constructor).
 */
export default class WireframeArgEditService extends Service {
   ...
   * the kernel's `exit()` so a burst left mid-debounce can't bleed into a later
  ...
  #flushPendingPartArgs({ compositeKey, outletName, partPath }, pending) {
    return this.wireframeEditEngine.recordStructural([outletName], () => {
