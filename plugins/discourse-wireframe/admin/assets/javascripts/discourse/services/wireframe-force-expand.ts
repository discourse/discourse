...
export default class WireframeForceExpandService extends Service {
  /** Composite keys of layout blocks expanded for the current edit session. */
  #keys = trackedSet<string>();

  /**
   * Whether `blockKey` is currently force-expanded.
   *
   * @param blockKey - The composite block key to inspect.
   */
  isForceExpanded(blockKey: string): boolean {
   ...
   * its next dragover.
   *
   * @param blockKey - The composite key of the layout block to toggle.
   */
  toggleForceExpand(blockKey: string): void {
    if (!blockKey) {
      return;
    }
