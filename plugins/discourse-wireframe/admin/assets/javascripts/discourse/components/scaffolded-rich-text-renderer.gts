...
/**
 * The scaffolded renderer is the edit-aware variant of the same `R.Content`
 * contract the minimal renderer implements, so it shares that component's
 * `Args`. It adds an `Element` because it renders a real outer span and
