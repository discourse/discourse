 ...
 * isn't rendering.
 *
 * Filters `wireframe.validationWarnings` (the flat, cross-outlet
...
export default class InspectorValidationBanner extends Component {
  ...
  get warnings() {
    ...
    const all = this.wireframe.validationWarnings;
    if (!all.length) {
      return [];
    }
    const key = this.wireframe.selectedBlockKey;
    if (!key) {
      return [];
    }
    const located = this.wireframe._findEntryAndOutletSync(key);
  ...
  <template>
    {{#if this.warnings.length}}
      <div
        class="wireframe-inspector-validation-banner"
        role="alert"
        aria-live="polite"
      >
        <div class="wireframe-inspector-validation-banner__header">
          {{dIcon "triangle-exclamation"}}
          <span>{{i18n "wireframe.inspector.validation.banner_title"}}</span>
        </div>
        <ul class="wireframe-inspector-validation-banner__list">
