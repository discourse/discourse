...
interface EmptyArgPromptSignature {
  /** Prompt presentation and activation callback. */
  Args: {
    /** Icon ID shown above the prompt. */
    icon?: string;
    /** Called when the author activates the prompt. */
    onActivate?: () => void;
    /** Pre-translated prompt and accessible label. */
    prompt: string;
  };
}
 ...
 */
export default class EmptyArgPrompt extends Component<EmptyArgPromptSignature> {
  /**
   * Prevents chrome activation and forwards prompt activation.
   *
