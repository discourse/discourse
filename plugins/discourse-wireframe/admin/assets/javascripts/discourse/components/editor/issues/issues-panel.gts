...
import WireframeValidationService, {
  type ValidationIssue,
} from "discourse/plugins/discourse-wireframe/discourse/services/wireframe-validation";

type IssueGroup = {
  /** Outlet containing the grouped issues. */
  outletName: string;
  /** Validation issues belonging to the outlet. */
...
export default class IssuesPanel extends Component {
   ...
   */
  get groups(): IssueGroup[] {
    const byOutlet = new Map<string, ValidationIssue[]>();
   ...
   *
   * @param issue - Validation issue whose block should be selected.
   */
  @action
   ...
   *
   * @param issue - Validation issue represented by the focused row.
   * @param event - Keyboard event to handle.
   */
  @action
  onRowKeydown(issue: ValidationIssue, event: KeyboardEvent): void {
