import Component from "@glimmer/component";
import { tracked } from "@glimmer/tracking";
import { on } from "@ember/modifier";
import { action } from "@ember/object";
import didInsert from "@ember/render-modifiers/modifiers/did-insert";
import willDestroy from "@ember/render-modifiers/modifiers/will-destroy";
import { service } from "@ember/service";
import DButton from "discourse/ui-kit/d-button";
import dIcon from "discourse/ui-kit/helpers/d-icon";
import { i18n } from "discourse-i18n";
import type WireframeInplaceLinkService from "discourse/plugins/discourse-wireframe/discourse/services/wireframe-inplace-link";
import type WireframeLayoutQueryService from "discourse/plugins/discourse-wireframe/discourse/services/wireframe-layout-query";

export type InplaceLinkPopoverData = {
  /** Composite key of the block whose URL is being edited. */
  blockKey: string;
  /** Name of the URL argument being edited. */
  argName: string;
};

interface InplaceLinkPopoverSignature {
  /** FloatKit data identifying the editable URL argument. */
  Args: {
    /** Block and argument identifiers supplied by the tooltip. */
    data?: InplaceLinkPopoverData;
  };
}

/**
 * Anchored URL editor for in-place-editable links on the canvas. Shown
 * via FloatKit when the author hovers a link rendered from a block's
 * `data-block-arg`.
 *
 * Two modes:
 *   - chip mode (default): a small "Edit URL" pill. Clicking switches
 *     into editing mode by opening a `linkEdit` session on the service.
 *   - editing mode: URL input + apply / remove / cancel. FloatKit's
 *     focus-lock (engaged via the `hoverGracePeriod` machinery in
 *     `d-float-body`) keeps the popover open while the input is
 *     focused, so the author can move the mouse away without the
 *     popover dismissing under them.
 *
 * Which mode is shown is derived from the service's `linkEdit` state
 * rather than tracked locally. That makes the popover responsive to
 * any path that opens a session — both the chip's own click and the
 * chrome's "click a URL arg of an already-selected block" gesture
 * (which programmatically opens this same popover) end up in the
 * same `editing` state through one source of truth.
 *
 * The popover owns its UI directly; it does NOT route through the
 * block-toolbar's `fieldEditor` slot. The toolbar's URL-edit surface
 * remains the right home for the rich text link mark (which
 * has no DOM element to anchor a popover to), but block-arg URLs are
 * edited where they live.
 */
export default class InplaceLinkPopover extends Component<InplaceLinkPopoverSignature> {
  /** Resolves the edited block's live argument value. */
  @service declare wireframeLayoutQuery: WireframeLayoutQueryService;

  /** Owns the active in-place URL edit session. */
  @service declare wireframeInplaceLink: WireframeInplaceLinkService;

  /**
   * Live working copy of the URL while editing. Seeded from the
   * current arg value when the input mounts (`seedInputValue`).
   */
  @tracked value: string = "";

  /**
   * `true` when the service has an active `linkEdit` session for THIS
   * popover's `(blockKey, argName)`. Derived rather than tracked
   * locally so external entry points (e.g. the chrome's click-arg
   * handler) can drive this popover into edit mode by calling
   * `linkEdit.start` directly.
   *
   * @returns Whether this popover owns the active edit session.
   */
  get editing(): boolean {
    const { blockKey, argName } = this.args.data ?? {};
    if (!blockKey || !argName) {
      return false;
    }
    return (
      this.wireframeInplaceLink.blockKey === blockKey &&
      this.wireframeInplaceLink.argName === argName
    );
  }

  /**
   * The live arg value for this link, read from the entry on every
   * access so the chip's "remove" affordance reflects whether the
   * link currently has a target.
   *
   * @returns Current URL value, or an empty string when unavailable.
   */
  get currentValue(): string {
    const { blockKey, argName } = this.args.data ?? {};
    if (!blockKey || !argName) {
      return "";
    }
    const located = this.wireframeLayoutQuery.findEntryAndOutletSync(blockKey);
    // TODO(devxp-typescript-pending): the stored arg value is `unknown`; the
    // raw read forwards it (or "") directly rather than narrowing to a string
    // at this boundary.
    return (located?.entry?.args?.[argName] ?? "") as string;
  }

  /**
   * Starts editing this URL argument.
   *
   * @param event - Edit-chip click event.
   */
  @action
  startEdit(event: MouseEvent): void {
    event.preventDefault();
    event.stopPropagation();

    const { blockKey, argName } = this.args.data ?? {};
    if (!blockKey || !argName) {
      return;
    }
    this.wireframeInplaceLink.start({ blockKey, argName });
  }

  /** Applies the current URL draft. */
  @action
  applyEdit(): void {
    this.wireframeInplaceLink.applyChange(this.value || null);
    this.value = "";
  }

  /** Removes the current URL value. */
  @action
  removeEdit(): void {
    this.wireframeInplaceLink.applyChange(null);
    this.value = "";
  }

  /** Cancels the URL edit session. */
  @action
  cancelEdit(): void {
    this.wireframeInplaceLink.stop();
    this.value = "";
  }

  /**
   * Updates the live URL draft.
   *
   * @param event - URL-input event.
   */
  @action
  onInput(event: Event): void {
    if (!(event.currentTarget instanceof HTMLInputElement)) {
      return;
    }
    this.value = event.currentTarget.value;
  }

  /**
   * Applies or cancels the edit from the keyboard.
   *
   * @param event - URL-input keyboard event.
   */
  @action
  onKeydown(event: KeyboardEvent): void {
    if (event.key === "Enter") {
      event.preventDefault();
      this.applyEdit();
    } else if (event.key === "Escape") {
      event.preventDefault();
      this.cancelEdit();
    }
  }

  /**
   * Seeds the working value from the live arg and focuses the input
   * when it mounts. Auto-select so the author can immediately type
   * over the current URL.
   *
   * @param element - Mounted URL input.
   */
  @action
  seedInputValue(element: HTMLInputElement): void {
    this.value = this.currentValue;
    element.focus();
    element.select();
  }

  /**
   * If the popover unmounts mid-edit (e.g. FloatKit closed the tooltip
   * because the user clicked outside while typing), drop the in-flight
   * session so the service doesn't carry stale `linkEdit.blockKey`
   * state into the next interaction.
   */
  @action
  onTeardown(): void {
    if (this.editing) {
      this.wireframeInplaceLink.stop();
    }
  }

  <template>
    <div class="wf-inplace-link-popover" {{willDestroy this.onTeardown}}>
      {{#if this.editing}}
        <input
          type="url"
          class="wf-inplace-link-popover__input"
          placeholder="https://..."
          value={{this.value}}
          {{didInsert this.seedInputValue}}
          {{on "input" this.onInput}}
          {{on "keydown" this.onKeydown}}
        />
        <DButton
          class="btn-flat wf-inplace-link-popover__btn"
          @icon="check"
          @title="wireframe.canvas.toolbar.link_apply"
          @ariaLabel="wireframe.canvas.toolbar.link_apply"
          @action={{this.applyEdit}}
          @preventFocus={{true}}
        />
        {{#if this.currentValue}}
          <DButton
            class="btn-flat wf-inplace-link-popover__btn"
            @icon="link-slash"
            @title="wireframe.canvas.toolbar.link_remove"
            @ariaLabel="wireframe.canvas.toolbar.link_remove"
            @action={{this.removeEdit}}
            @preventFocus={{true}}
          />
        {{/if}}
        <DButton
          class="btn-flat wf-inplace-link-popover__btn"
          @icon="xmark"
          @title="wireframe.canvas.toolbar.link_cancel"
          @ariaLabel="wireframe.canvas.toolbar.link_cancel"
          @action={{this.cancelEdit}}
          @preventFocus={{true}}
        />
      {{else}}
        <button
          type="button"
          class="wf-inplace-link-popover__chip"
          {{on "click" this.startEdit}}
        >
          {{dIcon "link"}}
          <span>{{i18n "wireframe.canvas.edit_url"}}</span>
        </button>
      {{/if}}
    </div>
  </template>
}
