 ...
 *  2. The block chrome wrapping a `wf:slot` entry
...
const EmptyCellPlaceholder = <template>
  <DButton
    class="wireframe-grid-cell__plus"
    @icon="plus"
    @title="wireframe.canvas.grid_overlay.add_at_cell"
    @action={{@onOpen}}
  />
  {{#if @isOpen}}
    <div class="wireframe-grid-cell__picker">
      <div class="wireframe-grid-cell__picker-header">
        <span>{{i18n "wireframe.canvas.grid_overlay.pick_block"}}</span>
        <DButton
          class="wireframe-grid-cell__picker-close"
          @icon="xmark"
          @title="wireframe.canvas.grid_overlay.cancel"
          @action={{@onClose}}
        />
      </div>
      <div class="wireframe-grid-cell__picker-grid" role="menu">
        {{#each @palette as |blockEntry|}}
          <DButton
            class="wireframe-grid-cell__picker-chip"
            role="menuitem"
