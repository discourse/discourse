...
export default class BlockBreadcrumb extends Component {
  /** Owns the current selection and its ancestry path. */
   ...
   */
  @cached
  get segments(): BlockAncestrySegment[] {
   ...
   *
   * @param segment - Breadcrumb segment to select.
   */
  @action
  pickSegment(segment: BlockAncestrySegment): void {
    if (segment.isOutlet) {
