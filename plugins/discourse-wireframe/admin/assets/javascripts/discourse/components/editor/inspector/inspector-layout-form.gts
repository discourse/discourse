...
import type WireframeGridTemplateService from "discourse/plugins/discourse-wireframe/discourse/services/wireframe-grid-template";
 ...
 * types for it. Cast it to the contract this form uses — its element (for
 * `...attributes` such as `class`) and the `content` block's yielded value.
 *
 * TODO(devxp-typescript-pending): drop once `DMenu` is authored in `.gts`
...
export default class InspectorLayoutForm extends Component {
   ...
   */
  #canApplyTemplate = (template: GridTemplate): boolean => {
   ...
   */
  get #activeTemplate(): GridTemplate | null {
    const data = this.wireframeSelection.selectedBlockData;
    if (!data?.key) {
      return null;
    }
   ...
   */
  @cached
  get argsSnapshot(): Record<string, unknown> {
    const snapshot =
  ...
  /** `true` for the flex modes (stack / row). */
  get isFlex() {
    return this.mode === "stack" || this.mode === "row";
  }

  /** `true` only in row mode (where wrap is meaningful). */
  get isRow() {
   ...
   */
  get #glyphAxis(): "horizontal" | "vertical" {
  ...
  @action
  applyTemplate(template: GridTemplate) {
    const data = this.wireframeSelection.selectedBlockData;
    if (!data?.key) {
      return;
    }
   ...
   */
  @action
  validationRuleFor(argName: string): string | undefined {
    const argDef = this.#schema?.[argName];
    if (!argDef) {
      return undefined;
    }
    // `buildValidationRule` reads only `required` and `schema`; hand it those
    // two from the arg's own schema entry.
