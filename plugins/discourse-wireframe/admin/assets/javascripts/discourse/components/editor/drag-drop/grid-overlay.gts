...
/**
 * The five-zone hit-test result within a cell. Center maps to a rect drop;
 * the four edges map to insert lines. The collapsed hit-test only ever emits
 * the vertical subset (`up` / `center` / `down`).
 */
type GridZone = "center" | "left" | "right" | "up" | "down";
 ...
 * cursor position before mirroring it into the unified slot-insert shape.
 *
 * `kind` selects the paint: a `rect` covers a cell (occupy / swap / replace);
 * a `line-column` / `line-row` is a thin insert line in a grid gap. The
 * leading-underscore fields are transient sentinels:
 *  - `_invalid` marks a shape the validity gate rejected (painted in danger
 *    tones, dispatches nothing).
...
interface GridOverlaySignature {
  ...
  Args: {
    ...
    gridKey: string;
    /** The outlet the layout lives in. */
    outletName: string;
  };
}
...
function descriptorsEqual(
  a: GridDropDescriptor | null,
  b: GridDropDescriptor | null
): boolean {
  if (a === b) {
    return true;
  }
...
export default class GridOverlay extends Component<GridOverlaySignature> {
   ...
   */
  @tracked gridElement: HTMLElement | null = null;
   ...
   */
  getGridElement = (): HTMLElement | null => this.gridElement;
   ...
   */
  #dragCache: DragGeometryCache | null = null;
   ...
   */
  #lastIntermediate: GridDropDescriptor | null = null;
   ...
   */
  #gridDropTargetCleanup: (() => void) | null = null;
   ...
   */
  #gridExternalDropTargetCleanup: (() => void) | null = null;
   ...
   * `onTrackResizeStart` and cleared on end/cancel.
   */
  #trackResize: TrackResizeSession | null = null;
   ...
   */
  #cellMerge: CellMergeSession | null = null;
   ...
   */
  @tracked _selectedEmptyCell: Cell | null = null;
  ...
  get columnHandles(): DResizeHandleDescriptor<number>[] {
    ...
    const spansAcross = (line: number, row: number) =>
      children.some((child) => {
   ...
   * layout model — and commits the final value in `onTrackResizeEnd`. Holding
   * Alt shrinks the columns to the right proportionally instead of split-pane.
   ...
   *
   * @param leftTrack - Index of the track left of the dragged divider.
   * @param dragInfo - The drag delta and originating event.
   */
  @action
  onTrackResize(leftTrack: number, dragInfo: ResizeDragInfo) {
   ...
   */
  @action
  commitColumnFractions(fractions: number[]) {
  ...
  #readColumnWidths(el: HTMLElement): number[] {
   ...
   */
  get selectedEmptyCell(): Cell | null {
  ...
  @action
  captureGridElement(element: HTMLElement) {
    ...
    // children.
    const gridEl =
      element.parentElement?.querySelector<HTMLElement>(GRID_LAYOUT_SELECTOR) ??
      null;
  ...
  @action
  pickBlockForCell(cell: Cell, blockEntry: BlockPaletteEntry) {
   ...
   *
   * @param cell - The origin cell (unused; the origin is held in the session).
   * @param direction - The handle's compass direction.
   * @param dragInfo - The `DResizeHandles` drag payload.
   */
  @action
  ...
  #applyGhostStyle(ghost: HTMLElement, placement: EdgeRect) {
   ...
   */
  #activeDragContext(): DragGeometryCache | null {
   ...
   */
  #publishUnified(
    intermediate: GridDropDescriptor | null,
    source: DragSource
  ): void {
  ...
  #unifiedKindFor(descriptor: GridDropDescriptor): string {
  ...
  #labelFor(descriptor: GridDropDescriptor, source: DragSource): string {
    if (!source) {
      return "";
    }
   ...
   */
  #publishFromDrag(source: DragSource, location: DragLocation): void {
  ...
  #descriptorFromCursor(
    ...
    let descriptor: GridDropDescriptor | null;
    if (slot) {
   ...
   */
  #attachCollapsedHit(
    descriptor: GridDropDescriptor | null,
    rect: DOMRect,
    zone: GridZone
  ): GridDropDescriptor | null {
    if (!descriptor) {
   ...
   */
  #cursorToCell(x: number, y: number, tracks: GridTracks): Cell | null {
   ...
   * track's index.
   */
  #findTrackIndex(pos: number, sizes: number[], gap: number): number | null {
    if (pos < 0) {
      return null;
    }
    let acc = 0;
   ...
   */
  #cellBounds(cell: Cell, tracks: GridTracks) {
   ...
   */
  #slotAtCell(cell: Cell): LayoutEntry | null {
   ...
   * side.
   */
  #cellDescriptorForZone(cell: Cell, zone: GridZone): GridDropDescriptor {
    if (zone === "left") {
      return {
        kind: "line-column",
        line: cell.column,
   ...
   * fallback only.
   */
  #computeOverlayGeometryCollapsed(
    descriptor: GridDropDescriptor
  ): OverlayGeometry | null {
    if (!this.gridElement) {
      return null;
