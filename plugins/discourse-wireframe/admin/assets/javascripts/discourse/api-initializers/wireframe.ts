...
import type { PluginApi } from "discourse/lib/plugin-api";
...
/**
 * The extra hierarchy field the block pipeline layers onto the debug context:
 * `rootOutletName` is the registry-level outlet the layout indexes by, distinct
...
/**
 * The BLOCK_DEBUG payload plus the fields the render pipeline and any
...
/**
 * Resolves a block reference (string name or class) to a block class, an
 * optional-missing marker, or null when unresolved. Matches the real contract
 * of `tryResolveBlock()`, the only implementation the pipeline ever passes in.
 */
type ResolveBlockFn = (
    ...
    function wfCreateGhostChildren(
      ...
      for (const childEntry of childEntries) {
        ...
        if (!resolvedBlock) {
          ...
          let nestedGhostChildren: ChildBlockResult[] | null = null;
