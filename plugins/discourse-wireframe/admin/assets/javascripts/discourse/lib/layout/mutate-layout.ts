...
// live-page `grid-math.ts` can use it without dragging mutate-layout
...
import {
  entryKey,
  type KeyedLayoutEntry,
} from "discourse/plugins/discourse-wireframe/discourse/lib/entry-key";
import { friendlyErrorMessage } from "discourse/plugins/discourse-wireframe/discourse/lib/friendly-error-message";

export { entryKey };

// TODO(devxp-typescript-pending): use core's type directly once it exports the
// stable identity and permissive-validation fields attached to layout entries.
/** A layout entry carrying editor-only runtime state. */
export interface ValidatedLayoutEntry extends LayoutEntry, KeyedLayoutEntry {
  /** Nested editor entries carrying the same runtime state. */
  children?: ValidatedLayoutEntry[];
  ...
  /** Whether the validator considers the entry renderable. */
  __visible?: boolean;
}

/** Placement of an entry relative to an existing target. */
export type InsertPosition = "after" | "before" | "inside" | "inside-end";

/** Result of an immutable layout mutation. */
export type LayoutMutationResult = {
  /** Whether the requested mutation changed the layout. */
  changed: boolean;
  /** Original or updated layout according to `changed`. */
  layout: LayoutEntry[];
};

/** Location of an entry within its direct sibling array. */
export type EntrySiblingLocation = {
  /** Direct sibling array containing the matched entry. */
  siblings: LayoutEntry[];
  /** Zero-based index of the matched entry. */
  index: number;
};

/** Result of removing one entry from a layout. */
export type RemoveEntryResult = LayoutMutationResult & {
  /** Removed entry, or `null` when no key matched. */
  removed: ValidatedLayoutEntry | null;
};

/** Resolves editor-relevant metadata for a block reference. */
type BlockMetadataLookup = (
  blockRef: LayoutEntry["block"]
) => BlockMetadata | null;

/** A JSON-safe layout entry with string-keyed transport fields. */
export type SerializedLayoutEntry = LayoutEntry &
  Record<string, unknown> & {
    /** Nested serialized entries. */
 ...
 *
 * @param layout - Layout whose entries should be searched recursively.
 * @param key - Composite key of the requested entry.
 ...
 *
 * @param layout - Layout whose entries should be searched recursively.
 * @param stableKey - The composite entry's stable key, as a string.
 * @returns Matching keyed entry, or `null` when absent.
 */
export function findEntryByStableKey(
  layout: ValidatedLayoutEntry[] | null | undefined,
  stableKey: string
): ValidatedLayoutEntry | null {
  if (!layout) {
    return null;
  }
  for (const entry of layout) {
 ...
 *
 * @param layout - Layout containing the entry to update.
 * @param key - Composite key of the entry to update.
 * @param updater - Function deriving replacement arguments from current values.
 * @returns Updated layout and whether an entry matched.
 */
export function replaceEntryArgs(
  layout: LayoutEntry[],
  key: string,
  updater: (currentArgs: Record<string, unknown>) => Record<string, unknown>
): LayoutMutationResult {
  let changed = false;

  /** @param entries - Current subtree being updated. */
  function walk(entries: LayoutEntry[]): LayoutEntry[] {
 ...
 * @param updater - Produces the replacement namespace bag.
 * @returns The updated layout and whether an entry matched.
 */
export function replaceEntryContainerArgs(
  layout: LayoutEntry[],
  key: string,
  namespace: string,
  updater: (currentBag: Record<string, unknown>) => Record<string, unknown>
): LayoutMutationResult {
  let changed = false;
  ...
  function walk(entries: LayoutEntry[]): LayoutEntry[] {
    ...
    const result = entries.map((entry) => {
      if (entryKey(entry) === key) {
        ...
        // TODO(devxp-typescript-pending): the stored bag is `unknown` at this
        // boundary; the original runtime passes it through to the updater as-is
        // (only substituting `{}` when nullish), so cast rather than coercing a
        // malformed bag to `{}`.
        const currentBag = (currentContainerArgs[namespace] ?? {}) as Record<
          string,
          unknown
        >;
 ...
 * remount the block.
 *
 * @param layout - Layout containing the entry to replace.
 * @param key - Composite key of the entry to replace.
 * @param newEntry - Replacement entry data.
 * @returns The updated layout, whether an entry matched, and the clone placed
 *   into the layout (or `null` when nothing matched).
 */
export function replaceEntryInPlace(
  layout: LayoutEntry[],
  key: string,
  newEntry: LayoutEntry
): LayoutMutationResult & {
  /** Clone inserted into the layout, or `null` when no entry matched. */
  ...
  let inserted: LayoutEntry | null = null;

  /** @param entries - Current subtree being searched for the insertion point. */
  function walk(entries: LayoutEntry[]): LayoutEntry[] {
    let subtreeChanged = false;
    const result = entries.map((entry) => {
      if (entryKey(entry) === key) {
        ...
        inserted = cloneEntryShell(newEntry, {
          // TODO(devxp-typescript-pending): remove this extension cast once
          // core's `LayoutEntry` includes the runtime stable identity field.
          __stableKey: (entry as ValidatedLayoutEntry).__stableKey,
        });
        return inserted;
      }
      if (entry.children?.length) {
 ...
 *
 * @param layout - Layout whose ancestry should be searched.
 * @param key - Composite key of the descendant entry.
 * @returns Entry path from the root through the matched descendant.
 */
export function findAncestryPath(
  ...
  /**
   * Searches a subtree while carrying its ancestor chain.
   *
   * @param entries - Current subtree being searched.
   * @param trail - Ancestors preceding the current subtree.
   */
  function walk(
    entries: LayoutEntry[],
    trail: LayoutEntry[]
 ...
 * @param key - Composite key of the entry whose siblings are requested.
 * @returns Direct siblings and matched index, or `null` when absent.
 */
export function findEntrySiblings(
  layout: LayoutEntry[],
  key: string
): EntrySiblingLocation | null {
  /** @param entries - Current subtree being searched for sibling context. */
  function walk(entries: LayoutEntry[]): EntrySiblingLocation | null {
 ...
 * @param layout - Layout containing the entry to update.
 * @param key - Composite key of the entry to update.
 * @param newConditions - Replacement conditions, or `null` to clear them.
 * @returns The updated layout and whether an entry matched.
 */
export function replaceEntryConditions(
  layout: LayoutEntry[],
  key: string,
 ...
 * @param layout - Layout containing the entry to update.
 * @param key - Composite key of the entry to update.
 * @param argName - Argument name to set.
 * @param value - Replacement argument value.
 * @returns Updated layout and whether an entry matched.
 */
export function setEntryArg(
  layout: LayoutEntry[],
  key: string,
  argName: string,
  value: unknown
 ...
 *
 * @param layout - Layout containing the entry to remove.
 * @param key - Composite key of the entry to remove.
 * @returns Updated layout, removed entry, and whether a key matched.
 */
export function removeEntry(
  layout: LayoutEntry[],
  key: string
): RemoveEntryResult {
  let removed: ValidatedLayoutEntry | null = null;

  /** @param entries - Current subtree being searched for removal. */
  function walk(entries: LayoutEntry[]): LayoutEntry[] {
    let subtreeChanged = false;
 ...
 *
 * @param layout - Layout that should receive the entry.
 * @param targetKey - Composite key used as the insertion target.
 * @param entry - Entry inserted into the layout.
 * @param position - Placement relative to the target.
 * @returns Updated layout and whether insertion occurred.
 */
 ...
 *
 * @param layout - Layout containing the source and destination.
 * @param sourceKey - Composite key of the entry to move.
 * @param targetKey - Composite key of the destination, or `null` for root.
 * @param position - Placement relative to the destination.
 * @returns Updated layout and whether the entry moved.
 */
 ...
 * self-nesting moves.
 *
 * @param entry - Entry subtree to inspect.
 * @param key - Composite key to find beneath the entry.
 * @returns Whether the key exists in the entry's descendants.
 */
function containsKey(entry: LayoutEntry, key: string | null): boolean {
  if (!entry?.children?.length) {
    return false;
  }
 ...
 *
 * @param entry - Composite layout entry whose part metadata should be walked.
 * @param idPath - Part-id path, such as `["actions", "primary"]`.
 * @returns Resolved part definition, or `null` when the path is invalid.
 */
export function resolvePartDef(
  entry: LayoutEntry | null | undefined,
  idPath: string[]
): BlockPartDefinition | null {
  ...
  for (const id of idPath) {
    ...
    part =
      meta?.parts?.find(
        (candidate: BlockPartDefinition) => candidate.id === id
      ) ?? null;
    if (!part) {
      return null;
    }
 ...
 *
 * @param layout - Layout containing the composite entry.
 * @param compositeKey - Composite entry key.
 * @param partPath - Dot-delimited part-id path, such as `"actions.primary"`.
 * @param updater - Function that derives the part's next argument overrides.
 * @returns Updated layout and whether the composite was found.
 */
  ...
  function walk(entries: LayoutEntry[]): LayoutEntry[] {
    ...
    const result = entries.map((entry) => {
      if (entryKey(entry) === compositeKey) {
        changed = true;
        subtreeChanged = true;
        // TODO(devxp-typescript-pending): the stored override is `unknown` at
        // this boundary; the original runtime passes it straight through the
        // spread to the updater (only substituting `{}` when nullish), so cast
        // rather than coercing a malformed override to `{}`.
        const current = (entry.overrides?.[partPath] ?? {}) as Record<
          string,
          unknown
        >;
 ...
 *
 * @param layout - Layout containing the composite entry.
 * @param compositeKey - Composite key of the entry to detach.
 * @returns The updated layout and whether a composite was detached.
 */
export function detachComposite(
  layout: LayoutEntry[],
  compositeKey: string
): LayoutMutationResult {
  let changed = false;

  /** @param entries - Current subtree being expanded. */
  function walk(entries: LayoutEntry[]): LayoutEntry[] {
    let subtreeChanged = false;
    const result = entries.map((entry) => {
      if (entryKey(entry) === compositeKey) {
        ...
        subtreeChanged = true;
        // TODO(devxp-typescript-pending): remove this extension cast once
        // core's `LayoutEntry.overrides` exposes the composite override shape.
        const children = synthesizePartEntries(
          entry as Omit<LayoutEntry, "overrides"> & {
            /** Composite override bags keyed by part path. */
            overrides?: Record<string, Record<string, unknown>>;
            /** Runtime stable identity used to derive part keys. */
            __stableKey?: string | number;
          },
          meta
        ).map(detachedChildFromSynthesized);
 ...
 *
 * @param synthesized - Synthesized part entry to materialize.
 * @returns Persistable child entry without synthetic identity.
 */
function detachedChildFromSynthesized(synthesized: LayoutEntry): LayoutEntry {
  const child: LayoutEntry = {
 ...
 *
 * @param layout - Source layout to clone for an editor draft.
 * @returns Structurally independent draft layout.
 */
export function cloneLayoutForDraft(layout: LayoutEntry[]): LayoutEntry[] {
  return layout.map(cloneEntryForDraft);
}

/**
 * Clones one entry and its descendants for an editor draft.
 *
 * @param entry - Source entry to clone.
 * @returns Structurally independent draft entry.
 */
function cloneEntryForDraft(entry: LayoutEntry): ValidatedLayoutEntry {
  const clone: ValidatedLayoutEntry = { ...entry };
 ...
 *
 * @param obj - Record mutated in place and returned for chaining.
 * @returns The same record without nullish-valued keys.
 */
function stripNullish<T extends Record<string, unknown>>(obj: T): T {
 ...
 *
 * @param entry - Layout entry mutated in place.
 */
export function clearValidatorStamps(entry: ValidatedLayoutEntry): void {
 ...
 *
 * @param entry - Entry whose validation stamps are updated in place.
 * @param options - Validation context options.
 */
export function revalidateEntryStamps(
  entry: ValidatedLayoutEntry | null | undefined,
  {
    owner,
  }: {
    /** Ember owner used for model-instance validation. */
    owner?: Owner;
  } = {}
): void {
  if (!entry) {
 ...
 *
 * @param entry - Source layout entry.
 * @param overrides - Fields replaced on the cloned entry.
 * @returns A clean cloned entry shell.
 */
function cloneEntryShell(
  entry: LayoutEntry,
  overrides: Partial<ValidatedLayoutEntry>
): ValidatedLayoutEntry {
  const clone: ValidatedLayoutEntry = { ...entry, ...overrides };
  clearValidatorStamps(clone);
  return clone;
}
 ...
 *
 * @param entry - Entry subtree copied into the clipboard.
 * @returns Deep clone without stable identity or validation stamps.
 */
export function cloneEntryForPaste(entry: LayoutEntry): LayoutEntry {
  const clone: ValidatedLayoutEntry = { ...entry };
 ...
 *
 * @param containerArgs - Container-argument namespaces to clone.
 * @returns Fresh top-level and namespace records.
 */
function cloneContainerArgs(
 ...
 *
 * @param layout - Layout entries to normalize under an outlet root.
 * @returns The existing or newly wrapped outlet-root layout.
 */
export function wrapAsOutletRoot(
  layout: LayoutEntry[] | null | undefined
): LayoutEntry[] {
 ...
 *
 * @param entry - Layout entry whose registered block name should resolve.
 * @returns Registered block name, or `null` when unavailable.
 */
function blockEntryName(entry: LayoutEntry): string | null {
  if (typeof entry?.block === "string") {
    return entry.block;
  }
  if (entry?.block) {
 ...
 *
 * @param entry - Container entry whose child contract should be inspected.
 * @param lookupMetadata - Metadata resolver for string or class block refs.
 * @returns Forced child block kind, or `null` when none is declared.
 */
function singleImplicitChildKind(
  entry: LayoutEntry,
  lookupMetadata: BlockMetadataLookup
): string | null {
 ...
 *
 * @param layout - Layout whose implicit child containers should normalize.
 * @param lookupMetadata - Metadata resolver for string or class block refs.
 * @returns Original or normalized layout with implicit wrappers applied.
 */
export function normalizeImplicitChildren(
  layout: LayoutEntry[],
  lookupMetadata: BlockMetadataLookup
): LayoutEntry[] {
 ...
 * nothing changed.
 *
 * @param entry - Entry subtree to normalize.
 * @param lookupMetadata - Metadata resolver for string or class block refs.
 * @returns Original or normalized entry.
 */
function normalizeEntryImplicitChildren(
  entry: LayoutEntry,
  lookupMetadata: BlockMetadataLookup
): LayoutEntry {
  if (!entry?.children?.length) {
 ...
 * @returns JSON-safe serialized layout.
 */
export function serializeLayoutForSave(
  layout: LayoutEntry[]
): SerializedLayoutEntry[] {
 ...
 *
 * @param prev - Previous argument value.
 * @param next - Next argument value.
 * @returns Whether both values represent the same stored value.
 */
export function sameValue(prev: unknown, next: unknown): boolean {
 ...
 * @returns JSON-safe serialized entry.
 */
export function serializeEntryForSave(
  entry: LayoutEntry
): SerializedLayoutEntry {
  // TODO(devxp-typescript-pending): the serialized shape requires `block`, but
  // the original runtime starts from an empty object and only writes `block`
  // when the reference is a string or truthy, omitting the key otherwise. Cast
  // to preserve that exact omission behavior.
  const out = {} as SerializedLayoutEntry;
  if (typeof entry.block === "string") {
 ...
 *
 * @param overrides - Composite override map to clone.
 * @returns Fresh top-level and per-part override records.
 */
