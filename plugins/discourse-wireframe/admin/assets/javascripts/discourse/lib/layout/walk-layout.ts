 ...
 * callback — see `api-initializers/wireframe.ts`. Falls back to the
...
/** An absolute path from an outlet layout to an outline row. */
export type OutlinePath = OutlinePathSegment[];

/** One flattened layout entry rendered by the outline. */
export type OutlineRow = {
  /** Nesting level within the outlet. */
  depth: number;
  ...
  /** Visibility conditions attached to the entry. */
  conditions?: LayoutEntry["conditions"];
  /** Whether the entry declares visibility conditions. */
  hasConditions: boolean;
  /** Whether the block reference is absent or unregistered. */
  isUnknown: boolean;
  /** Whether the row represents a synthesized composite part. */
  isPart: boolean;
  /** Permissive-validation failure category. */
  validationFailure: string | null;
  /** Human-readable permissive-validation failure. */
  validationReason: string | null;
  /** Structured permissive-validation details. */
  validationDetails: ValidationErrorDetails[] | null;
  /** One-based ordinal within a noun-framed parent. */
  slideOrdinal: number | null;
  /** Translation key used to render the child ordinal. */
  slideNumberKey: string | null;
  /** Optional author-supplied child label. */
  childLabel: string | null;
  /** Whether the entry has persisted children or synthesized parts. */
  hasChildren: boolean;
  /** Number of direct persisted children or synthesized parts. */
  childCount: number;
  /** Absolute source-layout path for the entry. */
  path: OutlinePath;
};

/** Flattened outline rows and root metadata for one outlet. */
export type OutletOutline = {
  /** Registered outlet name. */
  outletName: string;
  /** Flattened rows below the outlet root. */
  rows: OutlineRow[];
  /** Composite key of the implicit layout root, when present. */
  rootKey: string | null;
  /** Normalized layout mode of the implicit root, when present. */
  mode: string | null;
};

/** A block reference resolved for outline display. */
type ResolvedBlockName = {
  /** Canonical or unresolved block name. */
  name: string;
  /** Whether the reference failed registry resolution. */
  unknown: boolean;
};

// TODO(devxp-typescript-pending): use core's resolved block-entry type once it
// includes structured permissive-validation details and synthesized-part
// markers alongside the existing stable-key and failure fields.
type OutlineLayoutEntry = Omit<BlockEntry, "children"> & {
  /** Nested entries carrying the same resolved runtime fields. */
  children?: OutlineLayoutEntry[];
  /** Structured permissive-validation details. */
  __failureDetails?: ValidationErrorDetails[];
  /** Whether this entry is a render-only composite part. */
  __fromComposite?: boolean;
};
 ...
 *
 * @param blockRef - A registry name or a `@block` class.
 * @param blocksService - The Discourse `blocks` service; used to
 *   verify that string refs resolve to registered blocks.
 * @returns The resolved display name and unknown-state flag.
 */
function resolveBlockName(
  blockRef: BlockEntry["block"] | null | undefined,
...
export async function walkAllOutlets({
  ...
  /** Canonical blocks registry/layout service. */
  blocksService: BlocksService;
  /** Outlet names retained even while temporarily unmounted. */
  alwaysInclude?: Set<string>;
}): Promise<OutletOutline[]> {
  const result: OutletOutline[] = [];
  ...
  for (const outletName of outlets) {
    ...
    if (rootIsLayout) {
      ...
      result.push({
        ...
        // TODO(devxp-typescript-pending): `args.mode` is `unknown` at this
        // boundary; the original runtime passes it straight through, so cast
        // rather than narrowing (which would drop non-string truthy values).
        mode: normalizeLayoutMode(root.args?.mode as string | undefined),
      });
    } else {
 ...
 *
 * @param mode - The persisted layout mode.
 * @returns The current canonical layout mode.
 */
export function normalizeLayoutMode(mode: string | undefined): string {
  if (mode === "free-grid") {
    return "grid";
  }
  return mode ?? "stack";
 ...
 *
 * @param entries - Resolved entries whose tracked stamps should be read.
 */
function touchStamps(entries: OutlineLayoutEntry[]): void {
...
export const CHILD_NUMBER_KEY_BY_PARENT: Readonly<Record<string, string>> =
  Object.freeze({
...
export const CHILD_LABEL_NAMESPACE_BY_PARENT: Readonly<Record<string, string>> =
  Object.freeze({
    tabs: "tab",
  });
...
export const CHILD_NOUN_KEY_BY_PARENT: Readonly<Record<string, string>> =
  Object.freeze({
 ...
 *
 * @param value - The rich-inline argument value.
 * @returns Its concatenated text runs, or an empty string.
 */
export function richInlineToPlainText(value: unknown): string {
...
/**
 * Appends a depth-first flattened entry tree to the outline rows.
 *
 * @param entries - Entries at the current tree level.
 * @param depth - Current outline nesting depth.
 * @param path - Absolute path to the current entry list.
 * @param rows - Accumulated flattened outline rows.
 * @param blocksService - Canonical blocks registry/layout service.
 * @param metadataByName - Block metadata indexed by registered name.
 * @param parentBlockName - Parent block name used for child presentation.
 */
function walkEntries(
  entries: OutlineLayoutEntry[],
  depth: number,
  path: OutlinePath,
  rows: OutlineRow[],
  blocksService: BlocksService,
  metadataByName: Map<string, BlockMetadata | null>,
  ...
  entries.forEach((entry, index) => {
    ...
    const hasParts = !!(parts && entry.children == null);
    ...
    const partEntries =
      parts && entry.children == null
        ? synthesizePartEntries(entry, { ...metadata, parts })
        : null;
    ...
    const childLabel = childLabelNamespace
      ? richInlineToPlainText(
          nestedArgument(entry.containerArgs, childLabelNamespace, "label")
        ).trim() || null
      : null;
    rows.push({
      depth,
...
/**
 * Checks whether a rich-inline content item is a text run.
 *
 * @param value - The content item to check.
 * @returns Whether the item carries a string text value.
 */
function isTextRun(value: unknown): value is {
  /** Rich-inline node type. */
  type: "text";
  /** Plain text carried by the run. */
  text: string;
} {
  return (
    typeof value === "object" &&
...
/**
 * Reads a property from one namespaced container-argument bag.
 *
 * @param containerArgs - Container arguments keyed by namespace.
 * @param namespace - Namespace containing the argument.
 * @param name - Argument property to read.
 * @returns The nested argument value, or `undefined` for a missing bag.
 */
function nestedArgument(
  containerArgs: Record<string, unknown> | undefined,
  namespace: string,
  name: string
): unknown {
  const values = containerArgs?.[namespace];
  return typeof values === "object" && values !== null
    ? Reflect.get(values, name)
    : undefined;
}
