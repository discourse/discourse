...
export const FIT_TIERS = Object.freeze({
  /** Every toolbar action remains inline. */
  full: "full",

  /** Actions move into the overflow menu. */
  narrow: "narrow",

  /** The handle label also collapses. */
  narrower: "narrower",
});

/** A responsive toolbar presentation tier. */
export type FitTier = (typeof FIT_TIERS)[keyof typeof FIT_TIERS];
 ...
 * coordinator.
 *
 * @param avail - The block's available content width (`chrome.clientWidth`).
 * @param naturalFull - The bar's natural width with all actions inline.
 * @param naturalCompact - The bar's natural width with the actions folded
 *   into the hamburger (handle + hamburger).
 * @returns The narrowest tier needed to fit the available width.
 */
export function computeTier(
  avail: number,
  naturalFull: number,
  naturalCompact: number
): FitTier {
