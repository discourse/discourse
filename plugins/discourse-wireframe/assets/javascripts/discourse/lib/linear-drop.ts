...
/** A child's extent along the active axis. */
export type Segment = {
  /** The leading edge on the active axis. */
  near: number;

  /** The trailing edge on the active axis. */
  far: number;
};

/** A landing on a boundary between siblings. */
type LinearGapDrop = {
  /** Identifies a boundary landing. */
  kind: "gap";
  /** Boundary index from zero through the number of children. */
  gap: number;
};

/** A landing in the middle third of a child. */
type LinearMiddleDrop = {
  /** Identifies a child-middle landing. */
  kind: "middle";
  /** Index of the child under the cursor. */
  index: number;
};

/** A resolved boundary or child-middle landing. */
export type LinearDropResult = LinearGapDrop | LinearMiddleDrop;
 ...
 *
 * @param segments - Ordered child extents along the axis.
 * @param cursor - The cursor's coordinate on the same axis.
 * @returns The resolved boundary or child-middle landing.
 */
export function resolveLinearDrop(
  segments: Segment[],
  cursor: number
): LinearDropResult {
