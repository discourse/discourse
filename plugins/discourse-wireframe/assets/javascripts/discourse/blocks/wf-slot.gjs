 ...
 * grid layout (`wf:layout`) that hasn't been filled yet — applying
 * a template like "Hero + 3" produces a `wf:slot` per intended
 ...
 * can apply `display: none` to `.block-wf-slot`.
 *
 * In the editor, `block-chrome.gjs` detects the `wf:slot` block
