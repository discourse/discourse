...
@block("wf:layout", {
  ...
  args: {
    ...
    // The `@container` rules in `wireframe.scss` key off the
    // `wf-layout--collapse-<value>` modifier class emitted by
    // `className` below.
    ...
    autoCollapse: {
      ...
      ui: {
        control: "radio-group",
        label: i18n("wireframe.inspector.layout.auto_collapse_label"),
      },
    },
  },
  ...
  childArgs: {
    grid: {
      ...
      properties: {
        column: {
          ...
          default: "auto",
          ui: {
            label: i18n("wireframe.inspector.layout.placement.grid_column"),
          },
        },
        row: {
          type: "string",
          default: "auto",
          ui: {
            label: i18n("wireframe.inspector.layout.placement.grid_row"),
          },
        },
        align: {
          type: "string",
          ...
          ui: {
            control: "radio-group",
            label: i18n("wireframe.inspector.layout.placement.grid_align"),
            optionIcons: {
              start: "up-long",
              center: "align-center",
              end: "down-long",
        ...
        justify: {
          ...
          ui: {
            control: "radio-group",
            label: i18n("wireframe.inspector.layout.placement.grid_justify"),
            optionIcons: {
              start: "align-left",
              center: "align-center",
              end: "align-right",
      ...
      ui: {
        label: i18n("wireframe.inspector.layout.placement.grid_section"),
    ...
    stack: {
      ...
      properties: {
        alignSelf: {
          ...
          ui: {
            control: "radio-group",
            label: i18n("wireframe.inspector.layout.placement.align_self"),
        ...
        flexGrow: {
          ...
          min: 0,
          ui: {
            label: i18n("wireframe.inspector.layout.placement.flex_grow"),
          },
        },
      },
      ui: {
        label: i18n("wireframe.inspector.layout.placement.stack_section"),
    ...
    row: {
      ...
      ui: {
        label: i18n("wireframe.inspector.layout.placement.row_section"),
...
export default class WFLayout extends Component {
  /**
   * Inline style for the `.wf-layout__cell` wrapper this layout renders
   * around each grid-mode child. The cell wrapper IS the grid item.
   *
   * Emits ONLY CSS custom properties (`--wf-cell-column`, etc.) — the
   * actual `grid-column`, `grid-row`, `display: grid`, `place-items`,
   * and `min-*` declarations live in the stylesheet on
   * `.wf-layout--grid > .wf-layout__cell`. Same rationale as
   * `containerStyle`: a parent `@container wf-layout` rule can then
  ...
  cellStyle = (containerArgs) => {
    ...
    // `order`. Setting it here keeps slot chromes interleaved correctly
    // with the editor's empty-cell placeholders (which set the same
    // key) in the stacked view. Harmless in the expanded grid: explicit
    ...
    return trustHTML(
      `--wf-cell-column: ${column}; --wf-cell-row: ${row}; ` +
        `--wf-cell-align: ${align}; --wf-cell-justify: ${justify}; ` +
        `order: ${order};`
    );
  };
   ...
   *
   * Emits ONLY CSS custom properties (`--wf-layout-cols`, etc.) — the
   * actual layout declarations (`display: grid`, `grid-template-*`,
   * `flex-direction`, `transition`, etc.) live in the stylesheet on
   * `.wf-layout--{mode}` rules. This separation lets a parent
   * `@container wf-layout` rule override the actual `grid-template-
  ...
  get containerStyle() {
    ...
    if (mode === "grid") {
      ...
      return trustHTML(
        `--wf-layout-cols: ${gridTemplateColumns}; ` +
          `--wf-layout-rows: ${gridTemplateRows}; ` +
   ...
   * `wireframe.scss` applies (40rem for `--collapse-default`, 20rem for
  ...
  get className() {
    return (
      `wf-layout wf-layout--${this.resolvedMode} ` +
      `wf-layout--collapse-${this.args.autoCollapse ?? "default"}`
    );
  }
   ...
   *    column at narrow widths (`@container wf-layout (max-width:
   *    40rem)` in wireframe.scss), every cell gets
