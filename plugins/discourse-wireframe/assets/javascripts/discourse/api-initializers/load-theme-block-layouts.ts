...
/** One server-preloaded theme layout. */
export type ThemeBlockLayout = {
  /** Theme owning the layout. */
  theme_id: number;
  /** Block outlet receiving the layout. */
  outlet: string;
  /** Layout entries published into the theme layer. */
  layout: LayoutEntry[];
  /** Persisted layout schema version. */
  schema_version?: number;
};

/** Stack metadata for one theme. */
export type ThemeBlockLayoutMeta = {
  /** Server-assigned rank in the active theme stack. */
  stack_index: number;
};

/** Theme metadata keyed by theme id. */
export type ThemeBlockLayoutMetaMap = Record<
  string,
  ThemeBlockLayoutMeta | undefined
>;

/** Minimal MessageBus boundary used by live layout hydration. */
type MessageBus = {
  /**
   * Subscribes to a server-pushed channel.
   *
   * @param channel - MessageBus channel name.
   * @param callback - Handler for each untrusted payload.
   */
  subscribe(channel: string, callback: (data: unknown) => void): void;
};

// TODO(devxp-typescript-pending): replace `MessageBus` with the canonical core
// service type once `discourse/services/message-bus` exposes the instance
// returned by its JavaScript factory rather than only the factory class.
 ...
 *
 * @param api - Plugin API used to publish the theme layer.
 * @param layouts - Server-preloaded theme layouts.
 * @param metaByThemeId - Per-theme
 ...
 */
export function hydrateThemeBlockLayouts(
  api: PluginApi,
  layouts: ThemeBlockLayout[] | null | undefined,
  metaByThemeId: ThemeBlockLayoutMetaMap = {}
): void {
 ...
 * reload.
 *
 * @param api - Plugin API used to publish live updates.
 * @param themeIds - The ids of every theme currently in the
...
export function subscribeToBlockLayoutUpdates(
  ...
  // TODO(devxp-typescript-pending): remove this cast once
  // `getOwnerWithFallback` is converted and exposes Ember's canonical Owner
  // return type instead of leaking `any` from JavaScript inference.
  const owner = getOwnerWithFallback(undefined) as Owner;
  const messageBus: unknown = owner.lookup("service:message-bus");
  if (!isMessageBus(messageBus) || !Array.isArray(themeIds)) {
    return;
  }
  for (const themeId of themeIds) {
    ...
    messageBus.subscribe(`/block-layouts/${themeId}`, (payload) => {
      // The MessageBus payload arrives untyped; the raw envelope is handed
      // straight to `hydrateThemeBlockLayouts`, whose per-row guard drops
      // anything malformed.
      const data = payload as
        | ThemeBlockLayout
        | undefined; /* TODO(devxp-typescript-pending) */
...
export default apiInitializer((api: PluginApi): void => {
  // The preload boundary is untyped; the raw values are handed straight to
  // `hydrateThemeBlockLayouts`, whose per-row guard drops anything malformed.
  const layouts = PreloadStore.get("themeBlockLayouts") as
    | ThemeBlockLayout[]
    | null
    | undefined; /* TODO(devxp-typescript-pending) */
  const meta = (PreloadStore.get("themeBlockLayoutMeta") ??
    {}) as ThemeBlockLayoutMetaMap; /* TODO(devxp-typescript-pending) */
...
/**
 * Checks the untyped core MessageBus service instance boundary.
 *
 * @param value - Service instance returned by the Ember owner.
 * @returns Whether the instance supplies the subscription method used here.
 */
function isMessageBus(value: unknown): value is MessageBus {
  return (
    typeof value === "object" &&
    value !== null &&
