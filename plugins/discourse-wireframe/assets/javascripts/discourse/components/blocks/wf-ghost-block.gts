...
type FailureType = (typeof FAILURE_TYPE)[keyof typeof FAILURE_TYPE];

interface WFGhostBlockSignature {
  /** Failed block data supplied by the debug rendering pipeline. */
  Args: {
    /** Optional author-assigned block ID. */
    blockId?: string;
    /** Registered name of the block that failed to render. */
    blockName: string;
    /** Arguments supplied to the failed block. */
    blockArgs?: Record<string, unknown>;
    /** Conditions governing the failed block. */
    conditions?: object | object[];
    /** Optional explanation overriding the default failure message. */
    failureReason?: string;
    /** Category of render failure. */
    failureType: FailureType;
    /** Child ghosts rendered inside a failed container. */
    ghostChildren?: Array<{
      /** Curried ghost component for the child. */
      Component: ComponentLike;
      /** Stable render key for the child. */
      key: string;
    }>;
  };
}
 ...
 *
 * Receives the curry args from `api-initializers/wireframe.ts`:
 ...
 */
export default class WFGhostBlock extends Component<WFGhostBlockSignature> {
  /**
