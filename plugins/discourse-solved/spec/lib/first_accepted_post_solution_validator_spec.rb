...
describe DiscourseSolved::FirstAcceptedPostSolutionValidator do
  ...
  context "when trust level is 'any'" do
    it "validates the post" do
      ...
      Fabricate(:solved_topic, topic: post.topic, answer_post: post)
    ...
    it "invalidates if post user already has an accepted post" do
      ...
      Fabricate(:solved_topic, topic: previously_accepted_post.topic, answer_post: previously_accepted_post)

      newly_accepted_post = Fabricate(:post, user: user_tl1)
      Fabricate(:solved_topic, topic: newly_accepted_post.topic, answer_post: newly_accepted_post)
