...
module PageObjects
  module Pages
    class Rewind < PageObjects::Pages::Base
      ...
      def has_no_most_viewed_tags_report?
        has_no_css?(".rewind-report-page.--most-viewed-tags")
      end

      def has_tag_in_report?(tag)
        has_css?(".most-viewed-tags__tag", text: "##{tag.name}")
      end

      def has_tag_link?(tag)
        has_css?("a.folder-wrapper[href='/tag/#{tag.name}']")
      end
    end
  end
end
