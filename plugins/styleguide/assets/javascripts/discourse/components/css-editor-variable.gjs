...
export default class CssEditorVariable extends Component {
  ...
  get swatchStyle() {
    return htmlSafe(`background-color: ${this.colorHex}`);
  }
  ...
  <template>
    <div
      class="css-editor-variable
        {{if this.isModified 'is-modified'}}
        {{unless this.isEditable 'is-readonly'}}"
    >
      ...
      {{else}}
        <div class="css-editor-variable__controls">
          {{#if this.isColor}}
            <span
              class="css-editor-variable__color-swatch"
              style={{this.swatchStyle}}
            ></span>
          {{/if}}
          <span class="css-editor-variable__value">{{this.currentValue}}</span>
        </div>
      {{/if}}
    </div>
  </template>
