...
module(
  ...
  function (hooks) {
    ...
    test("arrows walk the bar and skip both spellings of disabled", async function (assert) {
      await render(<template><RovingFocusToolbarExample /></template>);
      ...
      assert.deepEqual(
        visited,
        ["Bold", "Italic", "Underline", "Copy", "Authoring practices"],
        "Cut is skipped as a disabled button and Paste as an aria-disabled one"
      );
    });
