...
RSpec.describe Checklist::Checkbox do
  ...
  describe "#replace_in" do
    ...
    it "rejects replacement for a non-toggleable checkbox" do
      expect { described_class.permanent.replace_in("[X] fixed", checked: false) }.to raise_error(
        ArgumentError,
        "checkbox is not toggleable",
      )
    end
