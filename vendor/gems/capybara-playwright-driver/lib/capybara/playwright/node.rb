  ...
  module Playwright
    ...
    class Node < ::Capybara::Driver::Node
      def initialize(driver, internal_logger, page, element, initial_cache = {})
        super(driver, element, initial_cache)
      ...
      # Throws the same message Playwright's own actionability check throws for
      # a detached element, so the rescue below maps it to StaleReferenceError.
      STALENESS_GUARD_JS = <<~JAVASCRIPT
        el => {
          if (!el.isConnected) {
      ...
      private def assert_element_not_stale(probe: true, &block)
        ...
        #
        # A plain isConnected evaluation detects detachment with a cheaper IPC
        # round trip than ElementHandle#enabled? (which runs Playwright's
        # injected-script locator machinery). Callers whose own payload JS
        # already starts with the staleness guard skip the probe entirely via
        # probe: false, collapsing the staleness check and the read into a
        # single round trip.
        @element.evaluate(STALENESS_GUARD_JS) if probe
      ...
      def visible_text
        assert_element_not_stale(probe: false) {
          # One round trip: staleness guard, the visibility walk from
          # #visible? (null when hidden), and the text extraction.
          text = @element.evaluate(<<~JAVASCRIPT)
            function(el){
              ...
              var vis = el;
              if (vis.tagName == 'AREA'){
                const map_name = document.evaluate('./ancestor::map/@name', vis, null, XPathResult.STRING_TYPE, null).stringValue;
                vis = document.querySelector(`img[usemap='#${map_name}']`);
                if (!vis){
                  return null;
                }
              }
              ...
              while (vis) {
                const style = window.getComputedStyle(vis);
                ...
                var parent = vis.parentElement;
                if (parent && (parent.tagName == 'DETAILS') && !parent.open && (vis.tagName != 'SUMMARY')) {
                  return null;
                }
                vis = parent;
              }
      ...
      def [](name)
        assert_element_not_stale(probe: false) {
          # One round trip for `property(name) || attribute(name)`: a falsy
          # (in the Ruby sense) or object/function property falls back to the
          # attribute, exactly like the two-step original.
          @element.evaluate(<<~JAVASCRIPT, arg: name.to_s)
            (el, name) => {
              if (!el.isConnected) {
                throw new Error('Element is not attached to the DOM');
              }
      ...
      def find_xpath(query, **options)
        # ShadowRootNode scopes stay on the stock engine, which pierces open
        # shadow roots; the fast engine resolves with native DOM queries. The
        # engine performs the root staleness check in-page (probe: false).
        selector, initial_cache =
          instance_of?(Node) ? FastFinder.plan(options, xpath: query, scoped: true) : nil
        if selector
          assert_element_not_stale(probe: false) {
            @element.query_selector_all(selector).map do |el|
              Node.new(@driver, @internal_logger, @page, el, initial_cache)
            end
          }
        else
          assert_element_not_stale {
