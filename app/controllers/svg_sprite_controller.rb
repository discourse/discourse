  ...
  def show
    ...
    RailsMultisite::ConnectionManagement.with_hostname(params[:hostname]) do
      ...
      if SvgSprite.version(theme_id) != params[:version]
        return(
          redirect_to UrlHelper.absolute((SvgSprite.path(theme_id))),
                      allow_other_host: true
        )
      end

      svg_sprite =
        "window.__svg_sprite = #{SvgSprite.bundle(theme_id).inspect};"

      response.headers['Last-Modified'] = 10.years.ago.httpdate
      response.headers['Content-Length'] = svg_sprite.bytesize.to_s
      immutable_for 1.year

      render plain: svg_sprite,
             disposition: nil,
  ...
  def svg_icon
    ...
    RailsMultisite::ConnectionManagement.with_hostname(params[:hostname]) do
      ...
      if icon.blank?
        ...
        doc.at_xpath('svg')['fill'] = adjust_hex(params[:color]) if params[
          :color
        ]
