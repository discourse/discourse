...
class ApplicationController < ActionController::Base
  ...
  if Rails.env == 'development'
    ...
    def remember_theme_id
      if @theme_ids.present?
        if defined?(Stylesheet::Watcher)
          Stylesheet::Watcher.theme_id = @theme_ids.first
        end
      end
    end
  end
      ...
      request.user_agent &&
        ...
        (
          has_escaped_fragment? ||
            CrawlerDetection.crawler?(request.user_agent) ||
            params.key?('print')
        )
  end
  ...
  def immutable_for(duration)
    ...
    response.cache_control[:extras] = %w[immutable]
  end

  def dont_cache_page
    if !response.headers['Cache-Control'] && response.cache_control.blank?
      response.cache_control[:no_cache] = true
      response.cache_control[:extras] = %w[no-store]
    end
  end
  ...
  rescue_from Discourse::InvalidParameters do |e|
    ...
    if (request.format && request.format.json?) || request.xhr? || !request.get?
      rescue_discourse_actions(
        :invalid_parameters,
        400,
        include_ember: true, custom_message_translated: message
      )
    else
      rescue_discourse_actions(
        :not_found,
        400,
        custom_message_translated: message
      )
    end
  end
  ...
  rescue_from Discourse::NotFound do |e|
    rescue_discourse_actions(
      :not_found,
      e.status,
      check_permalinks: e.check_permalinks, original_path: e.original_path
    )
  end
  ...
  rescue_from Discourse::InvalidAccess do |e|
    ...
    rescue_discourse_actions(
      :invalid_access,
      403,
      include_ember: true, custom_message: e.custom_message
    )
  end

  rescue_from Discourse::ReadOnly do
    render_json_error I18n.t('read_only_mode_enabled'),
                      type: :read_only, status: 503
  end
  ...
  def rescue_discourse_actions(type, status_code, opts = nil)
    ...
    message =
      opts[:custom_message_translated] || I18n.t(opts[:custom_message] || type)

    if show_json_errors
      # HACK: do not use render_json_error for topics#show
      if request.params[:controller] == 'topics' &&
         request.params[:action] == 'show'
        return render(
          status: status_code,
          layout: false,
          plain:
            if (status_code == 404 || status_code == 410)
              build_not_found_page(status_code)
            else
              message
            end
        )
      ...
      render html:
               build_not_found_page(
                 status_code,
                 opts[:include_ember] ? 'application' : 'no_ember'
               )
    end
  end
  ...
  def self.requires_plugin(plugin_name)
    before_action do
      if Discourse.disabled_plugin_names.include?(plugin_name)
        raise PluginDisabled.new
      end
    end
  end
  ...
  def fetch_user_from_params(opts = nil, eager_load = [])
    ...
    user =
      if params[:username]
        ...
        unless opts[:include_inactive] || current_user.try(:staff?)
          find_opts[:active] = true
        end
        result = User
  ...
  def post_ids_including_replies
    ...
    if params[:reply_post_ids]
      post_ids |=
        PostReply.where(post_id: params[:reply_post_ids]).pluck(:reply_id)
    end
    post_ids
  end
  ...
  def preload_anonymous_data
    ...
    store_preloaded(
      ...
      I18n.client_overrides_json(I18n.locale)
    )
  end
  ...
  def preload_current_user_data
    store_preloaded(
      'currentUser',
      MultiJson.dump(
        CurrentUserSerializer.new(current_user, scope: guardian, root: false)
      )
    )
  ...
  def custom_html_json
    ...
    DiscoursePluginRegistry.html_builders.each do |name, _|
      if name.start_with?('client:')
        data[name.sub(/^client:/, '')] =
          DiscoursePluginRegistry.build_html(name, self)
      end
    end
  ...
  def banner_json
    ...
    unless json
      ...
      ApplicationController.banner_json_cache['json'] =
        json = MultiJson.dump(banner)
    end
  ...
  def custom_emoji
    serializer =
      ActiveModel::ArraySerializer.new(
        Emoji.custom,
        each_serializer: EmojiSerializer
      )
  ...
  def render_json_error(obj, opts = {})
    ...
    render json: MultiJson.dump(create_errors_json(obj, opts)),
  ...
  def json_result(obj, opts = {})
    if yield(obj)
      ...
      if opts[:serializer].present?
        json[obj.class.name.underscore] =
          opts[:serializer].new(obj, scope: guardian).serializable_hash
      end
  ...
  def mini_profiler_enabled?
    defined?(Rack::MiniProfiler) &&
      (guardian.is_developer? || Rails.env.development?)
  end
  ...
  def redirect_to_login_if_required
    ...
    if current_user && !current_user.totp_enabled? && !request.format.json? &&
       !is_api? &&
       (
         (
      ...
      redirect_path =
        "#{GlobalSetting.relative_url_root}/u/#{current_user
          .username}/preferences/second-factor"
  ...
  def block_if_readonly_mode
    ...
    if !(request.get? || request.head?) && @readonly_mode
  ...
  def build_not_found_page(status = 404, layout = false)
    ...
    if !SiteSetting.login_required? || current_user
      ...
      if @topics_partial = $redis.get(key)
        ...
        @top_viewed =
          TopicQuery.new(nil, except_topic_ids: category_topic_ids)
            .list_top_for('monthly')
            .topics
            .first(10)
        @recent =
          Topic.includes(:category).where.not(id: category_topic_ids).recent(10)
        @topics_partial =
          render_to_string partial: '/exceptions/not_found_topics',
                           formats: %i[html]
    ...
    render_to_string status: status,
                     layout: layout,
                     formats: %i[html],
  ...
  def render_post_json(post, add_raw = true)
    ...
    post_serializer.post_actions = counts if counts && counts = counts[post.id]
