...
class ApplicationController < ActionController::Base
  ...
  after_action :add_noindex_header,
               if: -> {
                 is_feed_request? || !SiteSetting.allow_index_in_robots_txt
               }
  after_action :add_noindex_header_to_non_canonical,
               if: -> {
                 request.get? && !(request.format && request.format.json?) &&
                   !request.xhr?
               }
  ...
  def show_browser_update?
    @show_browser_update ||=
      CrawlerDetection.show_browser_update?(request.user_agent)
  end
      ...
      request.user_agent &&
        ...
        (
          has_escaped_fragment? || params.key?('print') ||
            show_browser_update? ||
            CrawlerDetection.crawler?(
              request.user_agent,
              request.headers['HTTP_VIA']
            )
        )
  end
  ...
  def ember_cli_required?
    Rails.env.development? && ENV['NO_EMBER_CLI'] != '1' &&
  ...
  rescue_from Discourse::SiteSettingMissing do |e|
    render_json_error I18n.t('site_setting_missing', name: e.message),
                      status: 500
  end
  ...
  rescue_from SecondFactor::BadChallenge do |e|
    render json: {
             error: I18n.t(e.error_translation_key)
           },
  ...
  def rescue_discourse_actions(type, status_code, opts = nil)
    ...
    if show_json_errors
      ...
      with_resolved_locale(check_current_user: false) do
        ...
        if (
             request.params[:controller] == 'topics' &&
      ...
      with_resolved_locale do
        error_page_opts[:layout] = (
          if opts[:include_ember]
            'application'
          else
            'no_ember'
          end
  ...
  def set_mobile_view
    session[:mobile_view] = params[:mobile_view] if params.has_key?(
      :mobile_view
    )
  end
    ...
    user =
      if params[:username]
        ...
        if provider_name = params[:external_provider]
          ...
          provider =
            Discourse.enabled_authenticators.find do |a|
  ...
  def post_ids_including_replies
    ...
    post_ids |=
      PostReply.where(post_id: params[:reply_post_ids]).pluck(
        :reply_post_id
  ...
  def rate_limit_second_factor!(user)
    ...
    if user
      RateLimiter.new(
        nil,
        "second-factor-min-#{user.username}",
        6,
        1.minute
      ).performed!
    end
  ...
  def custom_html_json
    ...
    DiscoursePluginRegistry.html_builders.each do |name, _|
      if name.start_with?('client:')
        data[name.sub(/^client:/, '')] = DiscoursePluginRegistry.build_html(
          name,
          self
        )
      end
  ...
  def banner_json
    ...
    unless json
      ...
      ApplicationController.banner_json_cache['json'] = json =
  ...
  def redirect_to_login_if_required
    ...
    if !current_user && params.has_key?(:user_api_public_key) &&
    ...
    redirect_path =
      path("/u/#{current_user.encoded_username}/preferences/second-factor")
  ...
  def build_not_found_page(opts = {})
    ...
    @slug =
      (params[:slug].presence || params[:id].presence || '').to_s.tr('-', ' ')
