  ...
  def index
    ...
    user_badges =
      user_badges.includes(
        :user,
        :granted_by,
        badge: :badge_type, post: :topic, user: :primary_group
      )
    ...
    if params[:username]
      user_id =
        User.where(username_lower: params[:username].downcase).pluck(:id).first
      user_badges = user_badges.where(user_id: user_id) if user_id
      grant_count = badge.user_badges.where(user_id: user_id).count
    end

    user_badges = user_badges.offset(offset.to_i) if offset = params[:offset]
    ...
    render_serialized(
      ...
      root: :user_badge_info, include_long_description: true
    )
  end

  def username
    params.permit %i[grouped]
    ...
    if params[:grouped]
      user_badges =
        user_badges.group(:badge_id).select(
          UserBadge.attribute_names.map { |x| "MAX(#{x}) AS #{x}" },
          'COUNT(*) AS "count"'
        )
    end

    user_badges =
      user_badges.includes(badge: %i[badge_grouping badge_type]).includes(
        post: :topic
      )
        .includes(:granted_by)
  ...
  def create
    ...
    if params[:reason].present?
      ...
      if route = Discourse.route_for(params[:reason])
        ...
        if topic_id > 0
          post_id =
            Post.find_by(topic_id: topic_id, post_number: post_number).try(:id)
        end
      end
    end
