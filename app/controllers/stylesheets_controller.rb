class StylesheetsController < ApplicationController
  ...
  def show_resource(source_map: false)
    ...
    location =
      "#{Rails
        .root}/#{Stylesheet::Manager::CACHE_PATH}/#{target}#{underscore_digest}#{extension}"
    ...
    unless File.exist?(location)
      if current =
         query.limit(1).pluck(source_map ? :source_map : :content).first
