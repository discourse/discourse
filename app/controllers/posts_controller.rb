...
class PostsController < ApplicationController
  ...
  def update
    ...
    if post.is_first_post?
      ...
      changes[:category_id] = params[:post][:category_id] if params[:post][
        :category_id
      ]
    ...
    post_serializer.single_post_link_counts =
      link_counts[post.id] if link_counts.present?
  ...
  def all_reply_ids
    Discourse.deprecate(
      '/posts/:id/reply-ids/all is deprecated.',
      drop_from: '3.0'
    )
    ...
    render json:
             post.reply_ids(
               guardian,
               only_replies_to_single_post: false
             ).to_json
  end
  ...
  def destroy
    ...
    if params[:force_destroy].present?
      if !guardian.can_permanently_delete?(post)
        return(
          render_json_error post.cannot_permanently_delete_reason(current_user),
                            status: 403
        )
      end
  ...
  def revert
    ...
    changes = {}
    changes[:raw] = post_revision.modifications['raw'][
      0
    ] if post_revision.modifications['raw'].present? &&
      post_revision.modifications['raw'][0] != post.raw
    if post.is_first_post?
      changes[:title] = post_revision.modifications['title'][
        0
  ...
  def post_type
    ...
    if Post.types[params[:post_type].to_i].blank?
  ...
  def create_params
    ...
    params[:auto_track] = !(params[:auto_track].to_s == 'false') if params[
      :auto_track
    ]
    params[:visible] = (params[:unlist_topic].to_s == 'false') if params[
      :unlist_topic
    ]
    ...
    if recipients
      ...
      groups =
        Group
          ...
          .where('lower(name) in (?)', recipients)
          .pluck('lower(name)')
      recipients -= groups
