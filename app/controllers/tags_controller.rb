...
class TagsController < ::ApplicationController
  ...
  skip_before_action :check_xhr, only: %i[tag_feed show index]

  before_action :set_category_from_params,
                except: %i[
                  index
                  update
                  destroy
    ...
    respond_to do |format|
      ...
      format.json do
        ...
        if SiteSetting.tags_listed_by_group
          ...
          grouped_tag_counts =
            TagGroup.visible(guardian).order('name ASC').includes(:tags)
              .map do |tag_group|
              {
                id: tag_group.id,
                name: tag_group.name,
                tags: self.class.tag_counts_json(tag_group.tags)
              }
            end
          ...
          categories =
            Category.where('id IN (SELECT category_id FROM category_tags)')
  ...
  Discourse.filters.each do |filter|
    define_method("show_#{filter}") do
      ...
      @additional_tags =
        params[:additional_tag_ids].to_s.split('/').map do |t|
      ...
      if current_user
        @list.draft =
          Draft.get(current_user, @list.draft_key, @list.draft_sequence)
      end
      ...
      canonical_url "#{Discourse.base_url_no_prefix}#{public_send(
                      path_name,
                      *(
                        params.slice(:parent_category, :category, :tag_id)
                          .values
                          .map { |t| t.force_encoding('UTF-8') }
                      )
                    )}"

      if @list.topics.size == 0 && params[:tag_id] != 'none' &&
         !Tag.where_name(@tag_id).exists?
  ...
  def update
    ...
    if tag.save
      StaffActionLogger.new(current_user).log_custom(
        'renamed_tag',
        previous_value: params[:tag_id], new_value: new_tag_name
      )
          ...
          CSV.foreach(file.tempfile) do |row|
            if row.length > 2
              raise Discourse::InvalidParameters.new(
                      I18n.t('tags.upload_row_too_long')
                    )
            end
            ...
            if tag_group_name
              ...
              unless tag.tag_groups.include?(tag_group)
                tag.tag_groups << tag_group
              end
            end
          end
        end
  ...
  def tag_feed
    ...
    render 'list/list', formats: %i[rss]
  end

  def search
    clean_name = DiscourseTagging.clean_tag(params[:q])
    category =
      params[:categoryId] ? Category.find_by_id(params[:categoryId]) : nil

    # Prioritize exact matches when ordering
    order_query =
      Tag.sanitize_sql_for_order(
    ...
    if !tags.find { |h| h[:id].downcase == clean_name.downcase } &&
      ...
      category_names =
        tag.categories.where(id: guardian.allowed_category_ids).pluck(:name)
      category_names +=
        Category.joins(tag_groups: :tags).where(
          id: guardian.allowed_category_ids, :"tags.id" => tag.id
        )
          .pluck(:name)

      if category_names.present?
        category_names.uniq!
        json_response[:forbidden_message] =
          I18n.t(
  ...
  def notifications
    ...
    render json: {
             tag_notification: { id: tag.name, notification_level: level.to_i }
           }
  end
  ...
  def check_hashtag
    valid_tags =
      Tag.where_name(params[:tag_values]).map do |tag|
  ...
  def personal_messages
    ...
    if current_user.id != allowed_user.id && !@guardian.is_admin?
      raise Discourse::NotFound
    end
  ...
  def set_category_from_params
    ...
    if slug_or_id == 'none' && params[:parent_category]
      @filter_on_category =
        Category.query_category(params[:parent_category], nil)
      ...
      @filter_on_category =
        Category.query_category(slug_or_id, parent_category_id)
    end
  ...
  def page_params(opts = nil)
    ...
    if @filter_on_category && @filter_on_category.parent_category
      route_params[:parent_category] =
        @filter_on_category.parent_category.slug_for_url
    end
  ...
  def build_topic_list_options
    options = {
      ...
      category:
        @filter_on_category ? @filter_on_category.id : params[:category],
      order: params[:order],
  ...
  def category_redirect_or_not_found
    ...
    url =
      if params[:parent_category]
        "c/#{params[:parent_category]}/#{params[:category]}"
      else
        "c/#{params[:category]}"
      end
    ...
    if permalink.present? && permalink.category_id
      redirect_to "#{Discourse.base_uri}/tags#{permalink.target_url}/#{params[
                    :tag_id
                  ]}",
