...
class TagsController < ::ApplicationController
  ...
  def index
    ...
    if SiteSetting.tags_listed_by_group
      ungrouped_tags =
        Tag.where('tags.id NOT IN (SELECT tag_id FROM tag_group_memberships)')
      ungrouped_tags =
        ungrouped_tags.where('tags.topic_count > 0') unless show_all_tags
        ...
        categories
          .map do |c|
            category_tags =
              self.class.tag_counts_json(
                DiscourseTagging.filter_visible(
                  c.tags.where(target_tag_id: nil),
                  guardian
                )
              )
  ...
  Discourse.filters.each do |filter|
    define_method("show_#{filter}") do
      ...
      if filter == :top
        period =
          params[:period] || SiteSetting.top_page_default_timeframe.to_sym
        TopTopic.validate_period(period)

        @list =
          TopicQuery.new(current_user, list_opts).public_send(
            'list_top_for',
            period
          )
  ...
  def update
    ...
    tag.description = params[:tag][:description] if params[:tag]&.has_key?(
      :description
    )
    if tag.save
  ...
  def search
    ...
    if clean_name &&
         !tags.find { |h| h[:id].downcase == clean_name.downcase } &&
  ...
  def create_synonyms
    guardian.ensure_can_admin_tags!
    value =
      DiscourseTagging.add_or_create_synonyms_by_name(@tag, params[:synonyms])
    if value.is_a?(Array)
      render json:
               failed_json.merge(
                 failed_tags:
  ...
  def self.tag_counts_json(tags, show_pm_tags: true)
    target_tags =
      Tag.where(id: tags.map(&:target_tag_id).compact.uniq).select(:id, :name)
    tags
      .map do |t|
...

        {
          ...
          target_tag:
            (
              if t.target_tag_id
                target_tags.find { |x| x.id == t.target_tag_id }&.name
              else
                nil
              end
            )
  ...
  def set_category
    ...
    if !@filter_on_category
      permalink =
        Permalink.find_by_url("c/#{params[:category_slug_path_with_id]}")
  ...
  def build_topic_list_options
    ...
    options[:per_page] = params[:per_page].to_i.clamp(1, 30) if params[
      :per_page
    ].present?
