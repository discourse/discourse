...
class ListController < ApplicationController
  ...
  before_action :set_category,
                only: [
                  ...
                  Discourse.filters.map do |f|
                    :"parent_category_category_#{f}"
                  end,
                  Discourse.filters.map do |f|
                    :"parent_category_category_none_#{f}"
                  end,
                  # top summaries
                  :category_top,
                  :category_none_top,
                  ...
                  TopTopic
                    .periods
                    .map { |p| :"category_top_#{p}" },
                  TopTopic.periods.map { |p| :"category_none_top_#{p}" },
                  TopTopic.periods.map do |p|
                    :"parent_category_category_top_#{p}"
                  end,
                  # category feeds
                  :category_feed
                ]
  ...
  before_action :ensure_logged_in,
                  ...
                  Discourse.anonymous_filters.map do |f|
                    :"parent_category_category_#{f}"
                  end,
                  Discourse.anonymous_filters.map do |f|
                    :"parent_category_category_none_#{f}"
  ...
  Discourse.filters.each do |filter|
    define_method(filter) do |options = nil|
      ...
      if params[:category].blank?
        list_opts[:no_definitions] = true if filter == :latest
        if %i[latest categories].include?(filter) &&
           list_opts[:exclude_category_ids].blank?
          list_opts[:exclude_category_ids] =
            get_excluded_category_ids(list_opts[:category])
        end
      end
      ...
      if guardian.can_create_shared_draft? && @category.present?
        if @category.id == SiteSetting.shared_drafts_category.to_i
          ...
          list.topics.each { |t| t.includes_destination_category = true }
        else
          ...
          if list_opts[:category] && @category
            @title =
              I18n.t(
                ...
                filter: filter_title, category: @category.name
              )
          else
            @title = I18n.t('js.filters.with_topics', filter: filter_title)
          end
          @title << " - #{SiteSetting.title}"
        elsif (filter.to_s == current_homepage) &&
  ...
  def category_default
    ...
    view_method == 'top' ? top(category: @category.id) : self.send(view_method)
  end
  ...
  def self.generate_message_route(action)
    define_method("#{action}") do
      ...
      list.more_topics_url =
        url_for(construct_url_with(:next, list_opts, url_prefix))
      list.prev_topics_url =
        url_for(construct_url_with(:prev, list_opts, url_prefix))
      respond_with_list(list)
    end
  end
  ...
  def top_feed
    ...
    @topic_list =
      TopicQuery.new(nil).list_top_for(
  ...
  def category_feed
    ...
    @description =
      "#{I18n.t('topics_in_category', category: @category.name)} #{@category
        .description}"
  ...
  def user_topics_feed
    ...
    @title =
      "#{SiteSetting.title} - #{I18n.t(
        'rss_description.user_topics',
        username: target_user.username
      )}"
    @link = "#{Discourse.base_url}/u/#{target_user.username}/activity/topics"
    @atom_link =
      "#{Discourse.base_url}/u/#{target_user.username}/activity/topics.rss"
    @description =
      I18n.t('rss_description.user_topics', username: target_user.username)
    @topic_list =
      TopicQuery.new(nil, order: 'created').send('list_topics_by', target_user)
  ...
  def top(options = nil)
    options ||= {}
    period =
      ListController.best_period_for(
        current_user.try(:previous_visit_at),
        options[:category]
      )
  ...
  TopTopic.periods.each do |period|
    define_method("top_#{period}") do |options = nil|
      ...
      if 'top'.freeze == current_homepage &&
         top_options[:exclude_category_ids].blank?
        top_options[:exclude_category_ids] =
          get_excluded_category_ids(top_options[:category])
      end
      ...
      if use_crawler_layout?
        @title =
          I18n.t("js.filters.top.#{period}.title") + " - #{SiteSetting.title}"
      end
  ...
  def page_params(opts = nil)
    ...
    if @category && @category.parent_category
      route_params[:parent_category] = @category.parent_category.slug_for_url
    end
    route_params[:order] = opts[:order] if opts[:order].present?
    route_params[:ascending] = opts[:ascending] if opts[:ascending].present?
    if params[:username].present?
      route_params[:username] = UrlHelper.escape_uri(params[:username])
    end
    route_params
  end
  ...
  def set_category
    ...
    if parent_slug_or_id.present?
      ...
      if parent_category_id.blank? && !id
    ...
    if use_crawler_layout?
      @subcategories =
        @category.subcategories.select { |c| guardian.can_see?(c) }
    end
  end

  def build_topic_list_options
    options = {}
    if params[:tag_id].present? && guardian.can_tag_pms?
      params[:tags] = [params[:tag_id].parameterize]
    end
  ...
  def construct_url_with(action, opts, url_prefix = nil)
    method =
      if url_prefix.blank?
        ...
        "#{url_prefix}_#{action_name}_path"
      end
    url =
      if action == :prev
  ...
  def get_excluded_category_ids(current_category = nil)
    ...
    if current_category
      exclude_category_ids =
        exclude_category_ids.where.not(id: current_category)
    end
  ...
  def self.best_period_for(previous_visit_at, category_id = nil)
    default_period =
      (
        (
          category_id &&
            Category.where(id: category_id).pluck(:default_top_period).first
        ) ||
          SiteSetting.top_page_default_timeframe
      )
        .to_sym

    best_period_with_topics_for(
      previous_visit_at,
      category_id,
      default_period
    ) ||
  ...
  def self.best_period_with_topics_for(
    ...
    best_periods_for(previous_visit_at, default_period.to_sym).each do |period|
      ...
      if category_id
        top_topics =
          top_topics.joins(:topic).where('topics.category_id = ?', category_id)
      end
      top_topics = top_topics.limit(SiteSetting.topics_per_period_in_top_page)
      if top_topics.count == SiteSetting.topics_per_period_in_top_page
        return period
      end
    end
