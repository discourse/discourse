class Admin::WebHooksController < Admin::AdminController
  before_action :fetch_web_hook,
                only: %i[show update destroy list_events bulk_events ping]
  ...
  def index
    ...
    web_hooks =
      WebHook.limit(limit).offset(offset).includes(:web_hook_event_types)
        .includes(:categories)
        .includes(:groups)
    ...
    json = {
      ...
      extras: {
        ...
        content_types:
          WebHook.content_types.map { |name, id| { id: id, name: name } },
        delivery_statuses:
          WebHook.last_delivery_statuses.map do |name, id|
            { id: id, name: name.to_s }
          end
      },
      total_rows_web_hooks: WebHook.count,
      load_more_web_hooks:
        admin_web_hooks_path(
          limit: limit, offset: offset + limit, format: :json
        )
    }
  ...
  def list_events
    ...
    json = {
      ...
      extras: { web_hook_id: @web_hook.id }
    }
  ...
  def redeliver_event
    ...
    if web_hook_event
      ...
      web_hook_event.update_attributes!(
  ...
  def ping
    Jobs.enqueue(
      :emit_web_hook_event,
      web_hook_id: @web_hook.id, event_type: 'ping', event_name: 'ping'
    )
    render json: success_json
  end
