...
class Admin::SiteTextsController < Admin::AdminController
  ...
  def index
    ...
    if query.blank? && !overridden
      ...
      translations.each { |k, v| results << record_for(k, v) }

      unless translations.empty?
        extras[:regex] =
          I18n::Backend::DiscourseI18n.create_search_regexp(
            query,
            as_string: true
          )
      end
    ...
    render_serialized(
      results[0..49],
      SiteTextSerializer,
      root: 'site_texts', rest_serializer: true, extras: extras
    )
  end
  ...
  def show
    ...
    render_serialized(
      site_text,
      SiteTextSerializer,
      root: 'site_text', rest_serializer: true
    )
  end
  ...
  def record_for(k, value = nil)
    if k.ends_with?('_MF')
      ovr =
        TranslationOverride.where(translation_key: k, locale: I18n.locale)
          .pluck(:value)
  ...
  def find_site_text
    unless I18n.exists?(params[:id]) &&
           !self.class.restricted_keys.include?(params[:id])
