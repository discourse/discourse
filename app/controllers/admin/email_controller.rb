...
class Admin::EmailController < Admin::AdminController
  ...
  def test
    ...
    begin
      ...
      if SiteSetting.disable_emails == 'yes'
        render json: {
                 sent_test_email_message:
                   I18n.t('admin.email.sent_test_disabled')
               }
      else
        render json: {
                 sent_test_email_message: I18n.t('admin.email.sent_test')
               }
      end
    rescue => e
  ...
  def sent
    ...
    tuples =
      email_logs.map { |email_log| [email_log.post_id, email_log.user_id] }

    reply_keys = {}

    if tuples.present?
      PostReplyKey.where(
        "(post_id,user_id) IN (#{(%w[(?)] * tuples.size).join(', ')})",
        *tuples
      )
  ...
  def preview_digest
    ...
    renderer =
      Email::Renderer.new(
        UserNotifications.digest(user, since: params[:last_seen_at])
      )
    render json:
             MultiJson.dump(
               html_content: renderer.html, text_content: renderer.text
             )
  end
  ...
  def send_digest
    ...
    user = User.find_by_username(params[:username])
    message, skip_reason =
      UserNotifications.send(:digest, user, since: params[:last_seen_at])
    if message
  ...
  def smtp_should_reject
    ...
    if !User.with_email(Email.downcase(params[:from])).exists? &&
       !SiteSetting.enable_staged_users
      render json: {
               reject: true,
               reason:
                 'Mail from your address is not accepted. Do you have an account here?'
             }
  ...
  def handle_mail
    ...
    begin
      Jobs.enqueue(
        :process_email,
        mail: params[:email], retry_on_rate_limit: true
      )
    rescue JSON::GeneratorError => e
      if retry_count == 0
        params[:email] =
          params[:email].force_encoding('iso-8859-1').encode('UTF-8')
        retry_count += 1
        retry
      else
        raise e
  ...
  def raw_email
    ...
    render json: {
             raw_email: incoming_email.raw, text_part: text, html_part: html
           }
  end
  ...
  def filter_logs(logs, params)
    ...
    logs =
      logs.includes(:user, post: :topic).references(:user).order(
        created_at: :desc
      )
    ...
    if params[:user].present?
      logs = logs.where('users.username ILIKE ?', "%#{params[:user]}%")
    end
    if params[:address].present?
      logs =
        logs.where("#{table_name}.to_address ILIKE ?", "%#{params[:address]}%")
    end
    if params[:type].present?
      logs =
        logs.where("#{table_name}.email_type ILIKE ?", "%#{params[:type]}%")
    end
    logs
  end
  ...
  def filter_incoming_emails(incoming_emails, params)
    incoming_emails =
      incoming_emails.includes(:user, post: :topic).order(created_at: :desc)
        .offset(params[:offset] || 0)
        .limit(50)

    if params[:from].present?
      incoming_emails =
        incoming_emails.where('from_address ILIKE ?', "%#{params[:from]}%")
    end
    ...
    if params[:subject].present?
      incoming_emails =
        incoming_emails.where('subject ILIKE ?', "%#{params[:subject]}%")
    end
    if params[:error].present?
      incoming_emails =
        incoming_emails.where('error ILIKE ?', "%#{params[:error]}%")
    end
  ...
  def delivery_settings
    action_mailer_settings.reject { |k, _| k == :password }.map do |k, v|
      { name: k, value: v }
    end
  end
