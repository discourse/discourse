class Admin::EmailTemplatesController < Admin::AdminController
  ...
  def update
    ...
    if error_messages.blank?
      ...
      render_serialized(
        ...
        root: 'email_template', rest_serializer: true
      )
    else
      ...
      TranslationOverride.upsert!(
        I18n.locale,
        "#{key}.text_body_template",
        body_result[:old_value]
      )
  ...
  def index
    render_serialized(
      ...
      AdminEmailTemplateSerializer,
      root: 'email_templates', rest_serializer: true
    )
  end
  ...
  def update_key(key, value)
    ...
    unless old_value.is_a?(Hash)
      translation_override =
        TranslationOverride.upsert!(I18n.locale, key, value)
    end
  ...
  def format_error_message(update_result, attribute_key)
    attribute =
      I18n.t("admin_js.admin.customize.email_templates.#{attribute_key}")
    message = update_result[:error_messages].join('<br>')
    I18n.t(
      'errors.format_with_full_message',
