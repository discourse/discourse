...
class UploadsController < ApplicationController
  ...
  def create
    ...
    if type == 'avatar' && !me.admin? &&
       (SiteSetting.sso_overrides_avatar || !SiteSetting.allow_uploaded_avatars)
  ...
  def lookup_urls
    ...
    if (params[:short_urls] && params[:short_urls].length > 0)
      PrettyText::Helpers.lookup_image_urls(params[:short_urls])
        .each { |short_url, url| uploads << { short_url: short_url, url: url } }
    end
  ...
  def self.serialize_upload(data)
    ...
    # as strings here
    if Upload === data
      serialized = UploadSerializer.new(data, root: nil).as_json.as_json
    end
