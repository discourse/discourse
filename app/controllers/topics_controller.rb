...
class TopicsController < ApplicationController
  ...
  def show
    ...
    if username_filters.is_a?(String)
      opts[:username_filters] = username_filters.split(',')
    end
    ...
    if opts[:print]
      unless SiteSetting.max_prints_per_hour_per_user > 0
    ...
    begin
      @topic_view =
        TopicView.new(params[:id] || params[:topic_id], current_user, opts)
    ...
    if (page < 0) ||
       (
         (page - 1) * @topic_view.chunk_size >
           @topic_view.topic.highest_post_number
       )
      raise Discourse::NotFound
    end
    ...
    if should_track_visit_to_topic?
      @topic_view.draft =
        Draft.get(
          current_user,
          @topic_view.draft_key,
          @topic_view.draft_sequence
        )
    end
    ...
    if use_crawler_layout?
      if last_modified = @topic_view.posts&.map(&:updated_at)&.max&.httpdate
    ...
    if ex.obj && Topic === ex.obj &&
       guardian.can_see_topic_if_not_deleted?(ex.obj)
  ...
  def wordpress
    ...
    # safe cause 0 means ignore
    only_moderator_liked = params[:only_moderator_liked].to_s == 'true'

    opts = {
      best: params[:best].to_i,
      min_trust_level:
        params[:min_trust_level] ? params[:min_trust_level].to_i : 1,
      min_score: params[:min_score].to_i,
      min_replies: params[:min_replies].to_i,
      bypass_trust_level_score: params[:bypass_trust_level_score].to_i,
      only_moderator_liked: only_moderator_liked
    }
    ...
    wordpress_serializer =
      TopicViewWordpressSerializer.new(
        @topic_view,
  ...
  def posts
    ...
    render_json_dump(
      TopicViewPostsSerializer.new(
        @topic_view,
        scope: guardian, root: false, include_raw: !!params[:include_raw]
      )
    )
  end
  ...
  def excerpts
    ...
    @posts =
      Post.where(hidden: false, deleted_at: nil, topic_id: @topic.id).where(
        'posts.id in (?)',
        post_ids
      )
  ...
  def update_shared_draft
    ...
    row_count =
      SharedDraft.where(topic_id: topic.id).update_all(category_id: category.id)
    if row_count == 0
  ...
  def update
    ...
    if params[:category_id] &&
       (params[:category_id].to_i != topic.category_id.to_i)
      ...
      if category && topic_tags = (params[:tags] || topic.tags.pluck(:name))
        ...
        category_tag_groups =
          category.tag_groups.joins(:tags).pluck('tags.name')
        ...
        if topic_tags.present? && allowed_tags.present?
          ...
          if !invalid_tags.empty?
            return render_json_error(
              I18n.t(
                'category.errors.disallowed_topic_tags',
                tags: invalid_tags.join(', ')
              )
            )
          end
        end
    ...
    if topic.category_id.to_i == changes[:category_id].to_i
      changes.delete(:category_id)
    end
  ...
  def feature_stats
    ...
    render json: {
             pinned_in_category_count:
               visible_topics.where(category_id: category_id).where(
             ...
             pinned_globally_count:
               visible_topics.where(pinned_globally: true).where.not(
                 pinned_at: nil
               )
                 .count,
             banner_count:
               Topic.listable_topics.where(archetype: Archetype.banner).count
           }
  end
             ...
             success_json.merge!(
               topic_status_update:
                 TopicTimerSerializer.new(
                   TopicTimer.find_by(topic: @topic),
                   root: false
                 )
             )
  end
  ...
  def timer
    ...
    options = {
      by_user: current_user, based_on_last_post: params[:based_on_last_post]
    }
    ...
    topic_status_update =
      topic.set_or_create_timer(status_type, params[:time], options)
  ...
  def remove_bookmarks
    ...
    PostAction.joins(:post).where(user_id: current_user.id).where(
      'topic_id = ?',
      topic.id
    )
      .each do |pa|
      PostAction.remove_act(
        current_user,
        pa.post,
  ...
  def toggle_archive_message(archive)
    ...
    if group_ids.present?
      allowed_groups =
        topic.allowed_groups.where(
          'topic_allowed_groups.group_id IN (?)',
          group_ids
        )
          .pluck(:id)
  ...
  def destroy
    ...
    first_post = topic.ordered_posts.first
    PostDestroyer.new(current_user, first_post, context: params[:context])
      .destroy
  ...
  def invite_group
    ...
    if topic.private_message?
      ...
      render_json_dump BasicGroupSerializer.new(
                         group,
                         scope: guardian, root: 'group'
                       )
    else
      render json: failed_json, status: 422
    end
  ...
  def invite
    ...
    groups =
      Group.lookup_groups(
        group_ids: params[:group_ids], group_names: params[:group_names]
      )
    ...
    begin
      if topic.invite(
        ...
        if user
          render_json_dump BasicUserSerializer.new(
                             user,
                             scope: guardian, root: 'user'
                           )
        else
          render json: success_json
        end
        ...
        unless topic.private_message?
          group_names =
            topic.category.visible_group_names(current_user).where(
              automatic: false
            )
              .pluck(:name)
              .join(', ')
  ...
  def merge_topic
    ...
    if params[:archetype] == 'private_message' && !guardian.is_staff?
      raise Discourse::InvalidAccess
    end
    ...
    if params[:archetype].present?
      ...
      if params[:participants].present? &&
        ...
        args[:participants] = params[:participants]
      end
    end

    destination_topic =
      topic.move_posts(current_user, topic.posts.pluck(:id), args)
  ...
  def move_posts
    ...
    if params[:title].present? &&
       Post.where(topic: topic, id: post_ids).order(:post_number).pluck(
         :post_type
       )
         .first !=
         Post.types[:regular]
  ...
  def change_timestamps
    ...
    begin
      ...
      render json: success_json
           ...
           TopicTimestampChanger::InvalidTimestampError
      render json: failed_json, status: 422
    end
  end
  ...
  def feed
    ...
    discourse_expires_in 1.minute
    render 'topics/show', formats: %i[rss]
  end
  ...
  def bulk
    ...
    if operation[:type].blank?
      raise ActionController::ParameterMissing.new(:operation_type)
    end
    operator =
  ...
  def perform_show_response
    ...
    respond_to do |format|
      format.html do
        @description_meta =
          if @topic_view.topic.excerpt.present?
            @topic_view.topic.excerpt
          else
            @topic_view.summary
          end
        store_preloaded(
          "topic_#{@topic_view.topic.id}",
          MultiJson.dump(topic_view_serializer)
        )
        render :show
      end
  ...
  def move_posts_to_destination(topic)
    ...
    if params[:destination_topic_id].present?
      args[:destination_topic_id] = params[:destination_topic_id].to_i
    end
    ...
    if params[:archetype].present?
      ...
      if params[:category_id].present?
        args[:category_id] = params[:category_id].to_i
      end
    end
  ...
  def check_for_status_presence(key, attr)
    unless %w[pinned pinned_globally visible closed archived].include?(attr)
      invalid_param(key)
    end
  end
