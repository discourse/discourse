...
class TopicsController < ApplicationController
  ...
  def show
    ...
    begin
        ...
        guardian.can_see_topic?(ex.obj, false) ||
          (
            !guardian.can_see_topic?(ex.obj) &&
              ex.obj&.access_topic_via_group && ex.obj.deleted_at
          )
      ...
      if SiteSetting.detailed_404
        ...
        elsif !guardian.can_see_topic?(ex.obj) &&
              group = ex.obj&.access_topic_via_group
    ...
    if use_crawler_layout?
      if last_modified =
           @topic_view.posts&.map { |p| p.updated_at }&.max&.httpdate
  ...
  def update
    ...
    if params[:category_id] &&
      ...
      if topic.shared_draft
        ...
        if category &&
             topic_tags =
               (params[:tags] || topic.tags.pluck(:name)).reject { |c|
                 c.empty?
               }
  ...
  def timer
    ...
    if params[:duration].present?
      options.merge!(duration: params[:duration].to_i)
    end
  ...
  def destroy
    ...
    if params[:force_destroy].present?
      if !guardian.can_permanently_delete?(topic)
        return(
          render_json_error topic.cannot_permanently_delete_reason(
                              current_user
                            ),
                            status: 403
        )
  ...
  def merge_topic
    ...
    if params[:archetype].present?
      ...
      args[:participants] = params[:participants] if params[
        :participants
      ].present? && params[:archetype] == 'private_message'
    end
  ...
  def move_posts
    ...
    if params[:title].present?
      ...
      if Post
           .where(topic: topic, id: post_ids)
           .order(:post_number)
           .pluck_first(:post_type) != Post.types[:regular]
        return(
          render_json_error(
  ...
  def reset_new
    topic_scope =
      if params[:category_id].present?
        ...
        if params[:include_subcategories] == 'true'
          category_ids =
            category_ids.concat(
              Category.where(parent_category_id: params[:category_id]).pluck(
                :id
              )
            )
        end
        ...
        scope =
          scope.joins(:tags).where(tags: { name: params[:tag_id] }) if params[
          :tag_id
        ]
        scope
    ...
    TopicTrackingState.publish_dismiss_new(
      current_user.id,
      topic_ids: dismissed_topic_ids
    )
  ...
  def set_slow_mode
    ...
    time =
      enabled && params[:enabled_until].present? ? params[:enabled_until] : nil
    ...
    topic_view_serializer =
      TopicViewSerializer.new(
        ...
        exclude_suggested_and_related:
          !!params[:replies_to_post_number] ||
            !!params[:filter_upwards_post_id] ||
  ...
  def move_posts_to_destination(topic)
    ...
    if params[:archetype].present?
      ...
      args[:category_id] = params[:category_id].to_i if params[
        :category_id
      ].present?
    end
  ...
  def bulk_unread_topic_ids
    ...
    if inbox = params[:private_message_inbox]
      ...
      topics = TopicQuery.tracked_filter(topics, current_user.id) if params[
        :tracked
      ].to_s == 'true'
