...
class MetadataController < ApplicationController
  ...
  def app_association_android
    unless SiteSetting.app_association_android.present?
      raise Discourse::NotFound
    end
    expires_in 1.minutes
    render plain: SiteSetting.app_association_android,
  ...
  def app_association_ios
    ...
    expires_in 1.minutes
    render plain: SiteSetting.app_association_ios,
           content_type: 'application/json'
  end
  ...
  def default_manifest
    ...
    icon_url_base =
      UrlHelper.absolute(
        "/svg-sprite/#{Discourse.current_hostname}/icon/#{primary_color}"
      )
      ...
      manifest =
        manifest.merge(
          ...
          related_applications: [
            { platform: 'play', id: SiteSetting.android_app_id }
          ]
        )
    end
