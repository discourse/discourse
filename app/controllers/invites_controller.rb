...
class InvitesController < ApplicationController
  ...
  skip_before_action :check_xhr, except: %i[perform_accept_invitation]
  skip_before_action :preload_json, except: %i[show]
  skip_before_action :redirect_to_login_if_required

  before_action :ensure_new_registrations_allowed,
                only: %i[show perform_accept_invitation]
  before_action :ensure_not_logged_in, only: %i[show perform_accept_invitation]
      ...
      if !invite.redeemed?
        ...
        flash.now[:error] =
          I18n.t(
            ...
            site_name: SiteSetting.title, base_url: Discourse.base_url
          )
      ...
      begin
        ...
        if user.present? && user.active?
          ...
          response[:redirect_to] =
            topic.present? ? path("#{topic.relative_url}") : path('/')
        else
  ...
  def create
    ...
    invite_exists =
      Invite.where(email: params[:email], invited_by_id: current_user.id).first
    if invite_exists && !guardian.can_send_multiple_invites?(current_user)
      return render json: failed_json, status: 422
    end

    begin
      if Invite.invite_by_email(
         params[:email],
         current_user,
         nil,
         group_ids,
  ...
  def resend_invite
    params.require(:email)
    RateLimiter.new(current_user, 'resend-invite-per-hour', 10, 1.hour)
      .performed!
    ...
    begin
      data =
        if extension.downcase == '.csv'
          ...
          Jobs.enqueue(
            :bulk_invite,
            filename: "#{name}#{extension}", current_user_id: current_user.id
          )
          { url: path }
        else
  ...
  def ensure_not_logged_in
    if current_user
      flash[:error] =
        I18n.t('login.already_logged_in', current_user: current_user.username)
      render layout: 'no_ember'
      false
    end
  end
