...
class EditDirectoryColumnsController < ApplicationController
  ...
  def index
    ...
    render_json_dump(
      directory_columns: serialize_data(columns, EditDirectoryColumnSerializer)
    )
  end
  ...
  def update
    ...
    unless has_enabled_column
      raise Discourse::InvalidParameters,
            'Must have at least one column enabled'
    end

    directory_column_params[:directory_columns].values.each do |column_data|
      existing_column =
        directory_columns.detect { |c| c.id == column_data[:id].to_i }
      if (
