...
class StaticController < ApplicationController
  ...
  def enter
    ...
    if redirect_location.present? && !redirect_location.is_a?(String)
      ...
      begin
        ...
        if uri.path.present? &&
             (uri.host.blank? || uri.host == forum_uri.host) && uri.path !~ /\./
    ...
    hijack do
      data =
        DistributedMemoizer.memoize(
          "FAVICON#{SiteIconManager.favicon_url}",
          60 * 30
        ) do
          ...
          if Discourse.store.external?
            begin
              ...
              Rails.logger.debug(
                "Failed to fetch favicon #{favicon.url}: #{e}\n#{e.backtrace}"
              )
              ''
            ensure
              file&.unlink
      ...
      format.js do
        ...
        content =
          Rails
            .application
            .assets_manifest
            .find_sources('service-worker.js')
            .first
  ...
  def serve_asset(suffix = nil)
    ...
    response.headers['Expires'] = 1.year.from_now.httpdate
    response.headers['Access-Control-Allow-Origin'] = params[:origin] if params[
      :origin
    ]
