...
class WebArtifactKeyValuesController < ApplicationController
    ...
    render json: {
             key_values:
               ActiveModel::ArraySerializer.new(
                 key_values,
                 each_serializer: WebArtifactKeyValueSerializer,
  ...
  def set
    key_value =
      @artifact.key_values.find_or_initialize_by(user: current_user, key: key_value_params[:key])

    key_value.assign_attributes(key_value_params.except(:key))

    if key_value.save
      render json: WebArtifactKeyValueSerializer.new(key_value).as_json
    else
      render json: { errors: key_value.errors.full_messages }, status: :unprocessable_entity
    end
  end
  ...
  def find_artifact
    @artifact = WebArtifact.find_by(id: params[:artifact_id])
    raise Discourse::NotFound if !@artifact
    raise Discourse::NotFound if !@artifact.public? && guardian.anonymous?
    raise Discourse::NotFound if !@artifact.public? && !guardian.can_view_web_artifact?(@artifact)
  end
end
