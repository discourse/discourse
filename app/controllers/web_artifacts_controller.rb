...
class WebArtifactsController < ApplicationController
  before_action :require_web_artifacts_enabled!
  ...
  def show
    ...
    if artifact.public?
      ...
      raise Discourse::NotFound if !guardian.can_view_web_artifact?(artifact)
    end
  ...
  def create
    guardian.ensure_can_create_web_artifact!
    RateLimiter.new(current_user, "create-web-artifact", 20, 1.minute).performed!

    artifact =
      WebArtifact.new(
        user: current_user,
        name: artifact_params[:name].to_s[0...255],
    ...
    if artifact.save
      render json: { id: artifact.id }, status: :created
    else
    ...
    <<~JAVASCRIPT
      <script>
        ...
        async function handleKeyValueRequest(action, data, artifactId) {
          const baseUrl = '/web-artifact-key-values/' + artifactId + ".json";
  ...
  def set_security_headers
    ...
    ] = "script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval' #{WebArtifact::ALLOWED_CDN_SOURCES.join(" ")};"
    response.headers["X-Robots-Tag"] = "noindex"
  end

  def require_web_artifacts_enabled!
    raise Discourse::NotFound if !SiteSetting.web_artifact_security.in?(%w[lax hybrid strict])
  end
end
