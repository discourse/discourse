class BadgesController < ApplicationController
  ...
  def index
    ...
    if (params[:only_listable] == 'true') || !request.xhr?
      ...
      badges =
        badges.includes(:badge_grouping).includes(:badge_type).where(
          enabled: true, listable: true
        )
    end
    ...
    if current_user
      user_badges =
        Set.new(
          current_user.user_badges.select('distinct badge_id').pluck(:badge_id)
        )
    end
    serialized =
      MultiJson.dump(
  ...
  def show
    ...
    @rss_title =
      I18n.t(
        ...
        display_name: @badge.display_name, site_title: SiteSetting.title
      )
    @rss_link = "#{Discourse.base_url}/badges/#{@badge.id}/#{@badge.slug}"

    if current_user
      user_badge =
        UserBadge.find_by(user_id: current_user.id, badge_id: @badge.id)
    ...
    serialized =
      MultiJson.dump(
        serialize_data(
          @badge,
          BadgeSerializer,
          root: 'badge', include_long_description: true
        )
      )
    respond_to do |format|
