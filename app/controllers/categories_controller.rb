...
class CategoriesController < ApplicationController
  ...
  def update
    ...
    json_result(@category, serializer: CategorySerializer) do |cat|
      ...
      category_params[:minimum_required_tags] = 0 if category_params[
  ...
  def update_slug
    ...
    if custom_slug.blank?
      error =
        @category.errors.full_message(:slug, I18n.t('errors.messages.blank'))
    ...
    @category_params ||=
      begin
        ...
        if SiteSetting.tagging_enabled
          params[:allowed_tags] = params[:allowed_tags].presence ||
            [] if params[:allowed_tags]
          params[:allowed_tag_groups] = params[:allowed_tag_groups].presence ||
            [] if params[:allowed_tag_groups]
          params[:required_tag_groups] = params[
            :required_tag_groups
          ].presence || [] if params[:required_tag_groups]
        end
        ...
        if result[:required_tag_groups] &&
             !result[:required_tag_groups].is_a?(Array)
