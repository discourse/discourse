...
class CategoriesController < ApplicationController
  ...
  def hierarchical_search
    ...
    only_ids = params[:only].to_a.map(&:to_i) if params[:only].present?
    except_ids = params[:except].to_a.map(&:to_i)
    limit = fetch_limit_from_params(default: MAX_CATEGORIES_LIMIT, max: MAX_CATEGORIES_LIMIT)

    categories =
      CategoryHierarchySearch.call(
        guardian: guardian,
        params: {
          term: term,
          only_ids: only_ids,
