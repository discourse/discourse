...
class CategoriesController < ApplicationController
  ...
  def index
    ...
    parent_category =
      Category.find_by(slug: params[:parent_category_id]) ||
    ...
    @category_list.draft_sequence =
      DraftSequence.current(current_user, Draft::NEW_TOPIC)
    if current_user
      @category_list.draft =
        Draft.get(current_user, Draft::NEW_TOPIC, @category_list.draft_sequence)
    end

    if category_options[:is_homepage] &&
    ...
    respond_to do |format|
      format.html do
        store_preloaded(
          ...
          MultiJson.dump(
            CategoryListSerializer.new(@category_list, scope: guardian)
          )
        )
        ...
        topic_options = {
          ...
          exclude_category_ids:
            Category.where(suppress_from_latest: true).pluck(:id)
        }
        ...
        if style == 'categories_and_latest_topics'.freeze
          ...
          @topic_list =
            TopicQuery.new(nil, topic_options).list_top_for(
        ...
        if @topic_list.present? && @topic_list.topics.present?
          store_preloaded(
            ...
            MultiJson.dump(
              TopicListSerializer.new(@topic_list, scope: guardian)
            )
          )
        end
  ...
  def reorder
    ...
    by_category =
      Hash[change_requests.map { |cat, pos| [Category.find(cat.to_i), pos] }]
  ...
  def update
    ...
    json_result(@category, serializer: CategorySerializer) do |cat|
      ...
      if category_params[:minimum_required_tags]&.blank?
        category_params[:minimum_required_tags] = 0
      end
  ...
  def categories_and_topics(topics_filter)
    ...
    if topics_filter == :latest
      result.topic_list =
        TopicQuery.new(current_user, topic_options).list_latest
  ...
  def category_params
    @category_params ||=
      begin
        ...
        p.each { |k, v| p[k] = v.to_i } if p = params[:permissions]
  ...
  def fetch_category
    @category =
      Category.find_by(slug: params[:id]) ||
        Category.find_by(id: params[:id].to_i)
  end
