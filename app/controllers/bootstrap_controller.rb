...
class BootstrapController < ApplicationController
  ...
  def index
    ...
    if theme_id.present?
      add_style(mobile_view? ? :mobile_theme : :desktop_theme)
    end
    ...
    bootstrap = {
      theme_id: theme_id,
      theme_color:
        "##{ColorScheme.hex_for_name('header_background', scheme_id)}",
  ...
  def add_scheme(scheme_id, media, css_class)
    ...
    if style =
         Stylesheet::Manager.new(
           theme_id: theme_id
         ).color_scheme_stylesheet_details(scheme_id, media)
  ...
  def add_style(target, opts = nil)
    if styles =
         Stylesheet::Manager.new(theme_id: theme_id).stylesheet_details(
           target,
           'all'
         )
  ...
  def create_theme_html
    ...
    add_if_present(
      theme_html,
      :body_tag,
      Theme.lookup_field(theme_id, theme_view, 'body_tag')
    )
    add_if_present(
      theme_html,
      :head_tag,
      Theme.lookup_field(theme_id, theme_view, 'head_tag')
    )
    add_if_present(
      theme_html,
      :header,
      Theme.lookup_field(theme_id, theme_view, 'header')
    )
    add_if_present(
      theme_html,
      :translations,
    ...
    add_if_present(
      theme_html,
      :js,
      Theme.lookup_field(theme_id, :extra_js, nil)
    )
