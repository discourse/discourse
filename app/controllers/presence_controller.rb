...
class PresenceController < ApplicationController
  ...
  def get
    ...
    names.each do |name|
      ...
      if channel.can_view?(user_id: current_user&.id, group_ids: user_group_ids)
        result[name] = PresenceChannelStateSerializer.new(
          channel.state,
          root: nil
        )
      else
  ...
  def update
    ...
    RateLimiter.new(
      nil,
      "update-presence-#{current_user.id}",
      20,
      10.seconds
    ).performed!
    ...
    if present_channels &&
         !(
           present_channels.is_a?(Array) &&
             present_channels.all? { |c| c.is_a? String }
         )
    ...
    if leave_channels &&
         !(
           leave_channels.is_a?(Array) &&
             leave_channels.all? { |c| c.is_a? String }
         )
    ...
    present_channels&.each do |name|
      PresenceChannel.new(name).present(
    ...
    leave_channels&.each do |name|
      PresenceChannel.new(name).leave(
        user_id: current_user&.id,
        client_id: params[:client_id]
      )
    rescue PresenceChannel::NotFound
