...
class Users::OmniauthCallbacksController < ApplicationController
  ...
  def self.types
    @types ||= Enum.new(:facebook, :twitter, :google, :yahoo, :github, :persona, :linkedin)
  end
  ...
  def create_or_sign_on_user_using_linkedin(auth_token)

    data = auth_token[:info][:name]
    screen_name = data["nickname"]
    linkedin_user_id = auth_token["uid"]

    session[:authentication] = {
      linkedin_user_id: linkedin_user_id,
      linkedin_screen_name: screen_name
    }

    user_info = LinkedinUserInfo.where(:linkedin_user_id => linkedin_user_id).first

    @data = {
      username: screen_name,
      auth_provider: "Linked_in"
    }
