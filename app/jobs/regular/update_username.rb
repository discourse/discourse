module Jobs
  class UpdateUsername < Jobs::Base
    ...
    def execute(args)
      ...
      @cooked_mention_user_path_regex = %r{^\/u(?:sers)?\/#{cooked_username}$}i
    ...
    def update_posts
      ...
      Post.with_deleted.joins(mentioned('posts.id')).where(
        'a.user_id = :user_id',
        user_id: @user_id
      )
        .find_each do |post|
      ...
      Post.with_deleted.joins(quoted('posts.id')).where(
        'p.user_id = :user_id',
        user_id: @user_id
      )
        .find_each do |post|
        update_post(post) unless updated_post_ids.include?(post.id)
      end
    end

    def update_revisions
      PostRevision.joins(mentioned('post_revisions.post_id')).where(
        'a.user_id = :user_id',
        user_id: @user_id
      )
        .find_each { |revision| update_revision(revision) }

      PostRevision.joins(quoted('post_revisions.post_id')).where(
        'p.user_id = :user_id',
        user_id: @user_id
      )
        .find_each { |revision| update_revision(revision) }
    ...
    def update_revision(revision)
      if revision.modifications.key?('raw') ||
         revision.modifications.key?('cooked')
        revision.modifications['raw']&.map! { |raw| update_raw(raw) }
        revision.modifications['cooked']&.map! do |cooked|
          update_cooked(cooked)
        end
        revision.save!
      end
    rescue => e
    ...
    def update_cooked(cooked)
      ...
      doc.css('a.mention').each do |a|
        a.content =
          a.content.gsub(@cooked_mention_username_regex, "@#{@new_username}")
        if a['href']
          a['href'] =
            a['href'].gsub(
      ...
      doc.css('aside.quote').each do |aside|
        ...
        div.children.each do |child|
          if child.text?
            ...
            username_replaced =
              content.gsub!(@cooked_quote_username_regex, @new_username)
                .present?
    ...
    def quotes_correct_user?(aside)
      Post.where(topic_id: aside['data-topic'], post_number: aside['data-post'])
        .pluck(:user_id)
        .first ==
        @user_id
    end
