module Jobs
  class AnonymizeUser < Jobs::Base
    ...
    def make_anonymous
      ...
      IncomingEmail.where(
        'user_id = ? OR from_address = ?',
        @user_id,
        @prev_email
      )
        .delete_all

      Post.with_deleted.where(user_id: @user_id).where.not(raw_email: nil)
    ...
    def anonymize_ips(new_ip)
      IncomingLink.where(ip_where('current_user_id')).update_all(
        ip_address: new_ip
      )
      ...
      UserHistory.where(ip_where('acting_user_id')).update_all(
        ip_address: new_ip
      )
