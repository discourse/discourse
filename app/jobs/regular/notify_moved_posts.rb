        ...
        posts.each do |p|
          unless users_notified.include?(p.user_id)
            p.user.notifications.create(
              ...
              data: {
                topic_title: p.topic.title, display_username: moved_by.username
              }
                .to_json
            )
