...
module Jobs
  ...
  class UserEmail < Jobs::Base
    ...
    def execute(args)
      ...
      to_address =
        args[:to_address].presence || user.try(:email).presence ||
          'no_email_found'
      ...
      message, skip_reason_type =
        message_for_email(user, post, type, notification, args)
      ...
      if message
        ...
        if (b = user.user_stat.bounce_score) >
           SiteSetting.bounce_score_erode_on_send
          ...
          user.user_stat.update(
            bounce_score: b - SiteSetting.bounce_score_erode_on_send
          )
        end
      else
        skip_reason_type
    ...
    def set_skip_context(type, user_id, to_address, post_id)
      @skip_context = {
        type: type, user_id: user_id, to_address: to_address, post_id: post_id
      }
    end
    ...
    def message_for_email(user, post, type, notification, args = nil)
      ...
      if user.anonymous?
        return skip_message(
          SkippedEmailLog.reason_types[:user_email_anonymous_user]
        )
      end

      if user.suspended? &&
         !%w[user_private_message account_suspended].include?(type.to_s)
        return skip_message(
      ...
      if always_email_regular?(user, type) ||
         always_email_private_message?(user, type) ||
         user.staged
        seen_recently = false
      end
      ...
      if (post || notification || notification_type) &&
        ...
        return skip_message(
          SkippedEmailLog.reason_types[:user_email_seen_recently]
        )
      end
      ...
      if notification || notification_type
        email_args[:notification_type] ||=
          notification_type || notification.try(:notification_type)
        email_args[:notification_data_hash] ||=
          notification_data_hash || notification.try(:data_hash)

        unless String === email_args[:notification_type]
          if Numeric === email_args[:notification_type]
            email_args[:notification_type] =
              Notification.types[email_args[:notification_type]]
          end
          email_args[:notification_type] = email_args[:notification_type].to_s
        end

        if user.user_option.mailing_list_mode? &&
           user.user_option.mailing_list_mode_frequency > 0 &&
           (!post.try(:topic).try(:private_message?)) &&
           NOTIFICATIONS_SENT_BY_MAILING_LIST.include?(
             email_args[:notification_type]
           )
        ...
        unless always_email_regular?(user, type) ||
               always_email_private_message?(user, type)
      ...
      if args[:user_history_id]
        email_args[:user_history] =
          UserHistory.where(id: args[:user_history_id]).first
      end
