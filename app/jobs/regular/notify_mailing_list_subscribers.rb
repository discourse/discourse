...
module Jobs
  class NotifyMailingListSubscribers < Jobs::Base
    ...
    def execute(args)
      ...
      users =
        User.activated.not_silenced.not_suspended.real.joins(:user_option)
          .where(
          'user_options.mailing_list_mode AND user_options.mailing_list_mode_frequency > 0'
        )
          .where(
          'NOT EXISTS (
                      SELECT 1
      ...
      users.find_each do |user|
        if Guardian.new(user).can_see?(post)
          ...
          if (user.id == post.user_id) &&
             (user.user_option.mailing_list_mode_frequency == 2)
            skip(
              user.email,
              user.id,
              post.id,
