          ...
          begin
            ...
            if object&.exists?
              begin
                ...
                temp_file = Tempfile.new(%w[converted .mp4])
                begin
                  ...
                  if thumbnail_upload
                    ...
                    new_thumbnail_path =
                      "original/1X/#{new_upload.sha1}#{File.extname(thumbnail_upload.original_filename)}"
                  ...
                  ref_count =
                    UploadReference.where(
                      upload_id: [upload.id, thumbnail_upload&.id].compact,
                    ).count
                  ...
                  UploadReference.where(
                    ...
                    target_id: target_ids.map(&:first),
                    target_type: target_ids.map(&:last).uniq,
                  ).delete_all
                  ...
                  if new_thumbnail_upload && thumbnail_upload
                    ...
                    UploadReference.where(
                      upload_id: new_thumbnail_upload.id,
                      target_id: thumbnail_target_ids.map(&:first),
                      target_type: thumbnail_target_ids.map(&:last).uniq,
                    ).delete_all

                    # Update the references
                    thumbnail_updated =
                      thumbnail_refs.update_all(upload_id: new_thumbnail_upload.id)
                  end
