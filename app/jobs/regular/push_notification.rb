module Jobs
  class PushNotification < Jobs::Base
    def execute(args)
      ...
      notification['url'] =
        UrlHelper.absolute_without_cdn(notification['post_url'])
      ...
      clients.group_by { |r| r[1] }.each do |push_url, group|
        notifications =
          group.map { |client_id, _| notification.merge(client_id: client_id) }
        ...
        if result.status != 200
          ...
          Rails.logger.warn(
            "Failed to push a notification to #{push_url} Status: #{result
