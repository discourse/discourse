module Jobs
  class CleanUpUploads < Jobs::Scheduled
    ...
    def execute(args)
      grace_period = [SiteSetting.clean_orphan_uploads_grace_period_hours, 1]
        .max

      # always remove invalid upload records
      Upload.by_users.where(
      ...
      result =
        Upload.by_users.where(
          ...
          AND ss.data_type = #{SiteSettings::TypeSupervisor
            .types[
            :upload
          ]
            .to_i}
        SQL
          ...
          .joins(
          'LEFT JOIN user_profiles up ON up.profile_background = uploads.url OR up.card_background = uploads.url'
        )
          .joins(
          ...
          .where(
          'ua.gravatar_upload_id IS NULL AND ua.custom_upload_id IS NULL'
        )
      ...
      if ignore_urls.present?
        result = result.where('uploads.url NOT IN (?)', ignore_urls)
      end
      ...
      result.find_each do |upload|
        if upload.sha1.present?
          ...
          if QueuedPost.where(
             "raw LIKE '%#{upload.sha1}%' OR raw LIKE '%#{encoded_sha}%'"
           )
             .exists?
            next
          end
          if Draft.where(
             "data LIKE '%#{upload.sha1}%' OR data LIKE '%#{encoded_sha}%'"
           )
             .exists?
            next
          end
