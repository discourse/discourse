module Jobs
  class EnqueueDigestEmails < Jobs::Scheduled
    ...
    def target_user_ids
      ...
      query =
        User.real.not_suspended.activated.where(staged: false).joins(
          :user_option,
          :user_stat
        )
          .where('user_options.email_digests')
          .where(
          "user_stats.bounce_score < #{SiteSetting.bounce_score_threshold}"
        )
          .where(
          ...
          .where(
          "COALESCE(last_seen_at, '2010-01-01') >= CURRENT_TIMESTAMP - ('1 DAY'::INTERVAL * #{SiteSetting
