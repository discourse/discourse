module Jobs
  class MigrateUploadScheme < Jobs::Scheduled
    ...
    def execute(args)
      ...
      Upload.where('created_at < ?', 1.hour.ago).where(
        "LENGTH(COALESCE(url, '')) = 0"
      )
        .destroy_all
      ...
      OptimizedImage.where('upload_id NOT IN (SELECT id FROM uploads)')
        .destroy_all
      ...
      problems.each do |hash|
        ...
        Discourse.handle_job_exception(
          ...
          error_context(
            args,
            "Migrating optimized_image id #{optimized_image_id}",
            optimized_image_id: optimized_image_id
          )
        )
      end
    end
