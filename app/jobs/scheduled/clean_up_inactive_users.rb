module Jobs
  class CleanUpInactiveUsers < Jobs::Scheduled
    ...
    def execute(args)
      ...
      User.joins('LEFT JOIN posts ON posts.user_id = users.id').where(
        last_posted_at: nil, trust_level: TrustLevel.levels[:newuser]
      )
        .where(
        ...
        begin
          ...
          Discourse.handle_job_exception(
            e,
            message: 'Cleaning up inactive users', extra: { user_id: user.id }
          )
        end
      end
    end
