  ...
  class PendingFlagsReminder < Jobs::Scheduled
      ...
      if SiteSetting.notify_about_flags_after > 0
        ...
        if flag_ids.size > 0 && last_notified_id.to_i < flag_ids.max
          ...
          PostCreator.create(
            ...
            title:
              I18n.t(
                'flags_reminder.subject_template',
                count: flagged_posts_count
              ),
            raw:
              mentions +
                I18n.t(
                  'flags_reminder.flags_were_submitted',
    ...
    def pending_flag_ids
      ...
      FlagQuery.flagged_post_actions(filter: 'active').where(
        ...
        SiteSetting.notify_about_flags_after.to_i.hours.ago
      )
        .pluck(:post_id, :id)
        .each do |row|
        by_post[row[0]] ||= []
      ...
      by_post.delete_if do |post_id, flags|
        flags.size < SiteSetting.min_flags_staff_visibility
      end
    ...
    def active_moderator_usernames
      User.where(moderator: true).human_users.order('last_seen_at DESC').limit(
        3
      )
        .pluck(:username)
    end
