  ...
  class PendingUsersReminder < Jobs::Scheduled
    ...
    def execute(args)
      if SiteSetting.must_approve_users &&
         SiteSetting.pending_users_reminder_delay >= 0
        query =
          AdminUserIndexQuery.new(query: 'pending', stats: false)
            .find_users_query # default order is: users.created_at DESC
        if SiteSetting.pending_users_reminder_delay > 0
          query =
            query.where(
              'users.created_at < ?',
              SiteSetting.pending_users_reminder_delay.hours.ago
            )
        end
        ...
        if count > 0
          target_usernames =
            Group[:moderators].users.map do |user|
              next if user.bot?

              unseen_count =
                user.notifications.joins(:topic).where(
                  'notifications.id > ?',
                  ...
                  .where(
                  'topics.subtype = ?',
                  TopicSubtype.pending_users_reminder
                )
                  .count
          ...
          unless target_usernames.empty?
            PostCreator.create(
              ...
              title:
                I18n.t(
                  'system_messages.pending_users_reminder.subject_template',
                  count: count
                ),
              raw:
                I18n.t(
                  'system_messages.pending_users_reminder.text_body_template',
                  count: count, base_url: Discourse.base_url
                )
            )
