...
module Jobs
  class AutoQueueHandler < Jobs::Scheduled
    ...
    def execute(args)
      ...
      # Flags
      flags =
        FlagQuery.flagged_post_actions(filter: 'active').where(
          'post_actions.created_at < ?',
          SiteSetting.auto_handle_queued_age.to_i.days.ago
        )
      ...
      # Posts
      queued_posts =
        QueuedPost.visible.where(state: QueuedPost.states[:new]).where(
          'created_at < ?',
          SiteSetting.auto_handle_queued_age.to_i.days.ago
        )
