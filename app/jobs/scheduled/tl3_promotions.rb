  ...
  class Tl3Promotions < Jobs::Scheduled
    ...
    def execute(args)
      ...
      User.real.not_suspended.where(
        trust_level: TrustLevel[2], manual_locked_trust_level: nil
      )
        .where
        .not(id: demoted_user_ids)
        .joins(:user_stat)
        .where(
        'user_stats.days_visited >= ?',
        SiteSetting.tl3_requires_days_visited
      )
        .where(
        'user_stats.topic_reply_count >= ?',
        SiteSetting.tl3_requires_topics_replied_to
      )
        .where(
        'user_stats.topics_entered >= ?',
        SiteSetting.tl3_requires_topics_viewed_all_time
      )
        .where(
        'user_stats.posts_read_count >= ?',
