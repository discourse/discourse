  ...
  class RebakeGifImages < ::Jobs::Onceoff
    def execute_onceoff(args)
      ...
      User
        ...
        if user.uploaded_avatar.animated
          if path = Discourse.store.path_for(user.uploaded_avatar)
            file = File.new(path)
          else
            find = Discourse.store.download(user.uploaded_avatar)
          end

          # We want to reupload the avatar, but we want to keep the avatar
          # until the new one is ready, so we will have to keep it under a
          # different hash (like we do for secure media).
      ...
      # Destroy all optimized image for animated GIFs and let the user
