module Jobs
  ...
  class Base
    class JobInstrumenter
      def initialize(job_class:, opts:, db:, jid:)
        ...
        self.class.mutex.synchronize do
          ...
          @data['job_type'] =
            job_class.try(:scheduled?) ? 'scheduled' : 'regular' # Job Type - either s for scheduled or r for regular
          ...
          Thread.new do
            begin
              ...
              Discourse.warn_exception(
                e,
                message: 'Sidekiq logging thread terminated unexpectedly'
              )
            end
          end
          ...
          Thread.new do
            begin
              loop do
                ...
                mutex.synchronize do
                  @@active_jobs.each do |j|
                    j.write_to_log if j.current_duration > interval
                  end
                end
              end
            rescue Exception => e
              Discourse.warn_exception(
                e,
                message:
                  'Sidekiq interval logging thread terminated unexpectedly'
              )
            end
          end
      end
    ...
    def log(*args)
      args.each do |arg|
        Rails.logger.info "#{Time.now.to_formatted_s(:db)}: [#{self.class.name
                .upcase}] #{arg}"
      end
      true
    end
    ...
    def perform(*args)
      ...
      if Jobs.run_later?
        Sidekiq.redis { |r| r.set('last_job_perform_at', Time.now.to_i) }
      end
      ...
      dbs.each do |db|
        begin
          ...
          RailsMultisite::ConnectionManagement.with_connection(db) do
            job_instrumenter =
              JobInstrumenter.new(
                job_class: self.class, opts: opts, db: db, jid: jid
              )
            ...
            begin
              ...
              exception[:ex] = e
              exception[:message] =
                "While establishing database connection to #{db}"
              exception[:other] = { problem_db: db }
            ensure
  ...
  def self.enqueue(job_name, opts = {})
    ...
    if Jobs.run_later?
      ...
      hash['queue'] = queue if queue = opts.delete(:queue)
...
Dir["#{Rails.root}/app/jobs/onceoff/*.rb"].each do |file|
  require_dependency file
end
Dir["#{Rails.root}/app/jobs/regular/*.rb"].each do |file|
  require_dependency file
end
Dir["#{Rails.root}/app/jobs/scheduled/*.rb"].each do |file|
  require_dependency file
end
