...
class UserDestroyer
  ...
  def initialize(actor)
    @actor = actor
    unless @actor && @actor.is_a?(User)
      raise Discourse::InvalidParameters.new('acting user is nil')
    end
  ...
  def destroy(user, opts = {})
    unless user && user.is_a?(User)
      raise Discourse::InvalidParameters.new('user is nil')
    end
      ...
      if opts[:delete_posts]
        user.posts.each do |post|
          if opts[:delete_as_spammer]
            # agree with flags
            PostAction.agree_flags!(post, @actor)
          end
        ...
        if u
          if opts[:block_email]
            user_emails.each do |email|
              ScreenedEmail.block(email, ip_address: u.ip_address)
                &.record_match!
            end
          end
          ...
          if opts[:block_ip] && u.ip_address
            ...
            if u.registration_ip_address &&
               u.ip_address != u.registration_ip_address
          ...
          MessageBus.publish '/file-change', %w[refresh], user_ids: [u.id]
        end
      end
    end
  end
