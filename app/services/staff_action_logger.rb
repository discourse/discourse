...
class StaffActionLogger
  ...
  def initialize(admin)
    @admin = admin
    unless @admin && @admin.is_a?(User)
      raise Discourse::InvalidParameters.new(:admin)
    end
  end

  USER_FIELDS ||=
    %i[id username name created_at trust_level last_seen_at last_emailed_at]
  ...
  def log_post_deletion(deleted_post, opts = {})
    ...
    topic =
      deleted_post.topic ||
        Topic.with_deleted.find_by(id: deleted_post.topic_id)

    username =
      deleted_post.user.try(:username) || I18n.t('staff_action_logs.unknown')
  ...
  def log_topic_delete_recover(topic, action = 'delete_topic', opts = {})
    unless topic && topic.is_a?(Topic)
      raise Discourse::InvalidParameters.new(:topic)
    end
  ...
  def log_trust_level_change(user, old_trust_level, new_trust_level, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        ...
        details:
          "old trust level: #{old_trust_level}\nnew trust level: #{new_trust_level}"
      )
    )
  end
  ...
  def log_lock_trust_level(user, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(action: action, target_user_id: user.id)
    )
  end
  ...
  def log_topic_published(topic, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:topic_published], topic_id: topic.id
      )
    )
  end

  def log_topic_timestamps_changed(
    topic, new_timestamp, previous_timestamp, opts = {}
  )
    unless topic && topic.is_a?(Topic)
      raise Discourse::InvalidParameters.new(:topic)
    end
  ...
  def log_post_lock(post, opts = {})
    unless post && post.is_a?(Post)
      raise Discourse::InvalidParameters.new(:post)
    end
    UserHistory.create!(
      params(opts).merge(
        action:
          UserHistory.actions[opts[:locked] ? :post_locked : :post_unlocked],
        post_id: post.id
      )
    )
  end
  ...
  def log_site_setting_change(
    setting_name, previous_value, new_value, opts = {}
  )
  ...
  def log_user_unsuspend(user, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:unsuspend_user], target_user_id: user.id
      )
    )
  end
  ...
  def log_user_merge(user, source_username, source_email, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        ...
        target_user_id: user.id,
        context:
          I18n.t('staff_action_logs.user_merged', username: source_username),
        email: source_email
      )
    )
  end
  ...
  def log_badge_creation(badge)
    ...
    details =
      BADGE_FIELDS.map { |f| [f, badge.send(f)] }.select { |f, v| v.present? }
        .map { |f, v| "#{f}: #{v}" }
    UserHistory.create!(
      params.merge(
        action: UserHistory.actions[:create_badge], details: details.join("\n")
      )
    )
  end
  ...
  def log_badge_change(badge)
    ...
    UserHistory.create!(
      params.merge(
        action: UserHistory.actions[:change_badge], details: details.join("\n")
      )
    )
  end
  ...
  def log_badge_deletion(badge)
    ...
    UserHistory.create!(
      params.merge(
        action: UserHistory.actions[:delete_badge], details: details.join("\n")
      )
    )
  end
  ...
  def log_check_email(user, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:check_email], target_user_id: user.id
      )
    )
  end
  ...
  def log_impersonate(user, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:impersonate], target_user_id: user.id
      )
    )
  end
  ...
  def log_roll_up(subnets, opts = {})
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:roll_up], details: subnets.join(', ')
      )
    )
  end

  def log_category_settings_change(
    category, category_params, old_permissions = nil
  )
    validate_category(category)

    changed_attributes = category.previous_changes.slice(*category_params.keys)

    if !old_permissions.empty? &&
       (old_permissions != category_params[:permissions])
      changed_attributes.merge!(
        permissions: [
          old_permissions.to_json,
          category_params[:permissions].to_json
        ]
      )
    end
  ...
  def log_unsilence_user(user, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:unsilence_user], target_user_id: user.id
      )
    )
  end
  ...
  def log_revoke_admin(user, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:revoke_admin], target_user_id: user.id
      )
    )
  end
  ...
  def log_grant_moderation(user, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:grant_moderation], target_user_id: user.id
      )
    )
  end
  ...
  def log_revoke_moderation(user, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:revoke_moderation], target_user_id: user.id
      )
    )
  end
  ...
  def log_user_approve(user, opts = {})
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:approve_user], target_user_id: user.id
      )
    )
  end
  ...
  def log_wizard_step(step, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:wizard_step], context: step.id
      )
    )
  end
  ...
  def log_post_approved(post, opts = {})
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:post_approved], post_id: post.id
      )
    )
  end

  def log_post_rejected(rejected_post, opts = {})
    unless rejected_post && rejected_post.is_a?(QueuedPost)
      raise Discourse::InvalidParameters.new(:rejected_post)
    end

    topic =
      rejected_post.topic ||
        Topic.with_deleted.find_by(id: rejected_post.topic_id)
    topic_title = topic&.title || I18n.t('staff_action_logs.not_found')
    username =
      rejected_post.user&.username || I18n.t('staff_action_logs.unknown')
    ...
    UserHistory.create!(
      params(opts).merge(
        action: UserHistory.actions[:post_rejected], details: details.join("\n")
      )
    )
  end
  ...
  def validate_category(category)
    unless category && category.is_a?(Category)
