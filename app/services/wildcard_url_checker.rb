...
module WildcardUrlChecker
  ...
  class << self
    def check_url(url, url_to_check)
      ...
      uri.scheme.casecmp?(uri_to_check.scheme) && uri.port == uri_to_check.port &&
        ...
        wildcard_match?(
          path_query(uri),
          path_query(uri_to_check),
          URL_COMPONENT_WILDCARD_PATTERN,
        ) &&
        wildcard_match?(
          uri.fragment.to_s,
          uri_to_check.fragment.to_s,
          URL_COMPONENT_WILDCARD_PATTERN,
        )
    end
    ...
    def hosts_match?(host, host_to_check)
      host = host.downcase
    ...
    def wildcard_match?(pattern, value, wildcard_pattern)
