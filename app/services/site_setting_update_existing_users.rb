...
class SiteSettingUpdateExistingUsers
  class << self
    def call(id, value, previous_value)
    ...
    def is_sidebar_default_setting?(setting_name)
      %w[default_navigation_menu_categories default_navigation_menu_tags].include?(
        setting_name.to_s,
      )
    end

    def default_categories(id, value, previous_value)
      new_value = value.nil? ? "" : value
    ...
    def default_tags(id, value, previous_value)
      new_value = value.nil? ? "" : value
