...
class BadgeGranter
  ...
  class << self
    ...
    def enqueue_mass_grant_for_users(
      badge,
      emails: [],
      usernames: [],
      ...
      if usernames.size > 0
        usernames_map_to_ids =
          User.where(username_lower: usernames).pluck(:username_lower, :id).to_h
      end
      if emails.size > 0
    ...
    def mass_grant(badge, user, count:)
    ...
    def queue_badge_grant(type, opt)
    ...
    def revoke_ungranted_titles!(user_ids = nil)
    ...
    def suppress_notification?(badge, granted_at, skip_new_user_tips)
