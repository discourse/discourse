...
class CategoryHierarchySearch
  ...
  def fetch_categories(guardian:, params:)
    ...
    allowed_categories_sql = Category.secured(guardian).distinct.to_sql
    ...
    sql = <<~SQL
      ...
      matched AS (
        ...
        WHERE
          ...
          #{params.only_ids.present? ? "AND allowed_categories.id IN (:only_ids)" : ""}
          #{params.except_ids.present? ? "AND allowed_categories.id NOT IN (:except_ids)" : ""}
      ),
      ancestors AS (
      ...
      INNER JOIN (SELECT DISTINCT id FROM ancestors) a ON a.id = ct.id
      LEFT JOIN topics ON topics.id = categories.topic_id
