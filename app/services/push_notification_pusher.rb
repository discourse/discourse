...
class PushNotificationPusher
  ...
  class << self
      ...
      I18n.with_locale(user.effective_locale) do
        ...
        notification_icon =
          ActionController::Base.helpers.image_url(
            "push-notifications/#{notification_icon_name}.png",
          )
    ...
    def clear_subscriptions(user)
      user.push_subscriptions.clear
    end

    def subscribe(user, push_params, send_confirmation)
    ...
    def unsubscribe(user, subscription)
  ...
  class << self
    ...
    def send_notification(user, subscription, message)
