...
class SearchIndexer
  ...
  class << self
    ...
    def update_index(table:, id:, a_weight: nil, b_weight: nil, c_weight: nil, d_weight: nil)
    ...
    def update_topics_index(topic_id, title, cooked)
    ...
    def update_categories_index(category_id, name)
      update_index(table: "category", id: category_id, a_weight: name)
    end

    def update_tags_index(tag_id, name)
      update_index(table: "tag", id: tag_id, a_weight: name.downcase)
    end

    def queue_category_posts_reindex(category_id)
      return if @disabled
    ...
    def queue_users_reindex(user_ids)
      return if @disabled
    ...
    def queue_post_reindex(topic_id)
      return if @disabled
    ...
    def clean_post_raw_data!(raw_data)
      urls = Set.new
