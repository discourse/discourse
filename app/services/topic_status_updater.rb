  ...
  Struct.new(:topic, :user) do
    ...
    def change(status, opts = {})
      ...
      if status.pinned? || status.pinned_globally?
        ...
        rc =
          Topic.where(id: topic.id, closed: !status.enabled?).update_all(
      ...
      if (
         (
           status.enabled? &&
             (status.autoclosed? || status.closed? || status.archived?)
         ) ||
    ...
    Status =
      Struct.new(:name, :enabled) do
        %w[pinned_globally pinned autoclosed closed visible archived]
          .each { |status| define_method("#{status}?") { name == status } }
