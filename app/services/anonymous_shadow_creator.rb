    ...
    if (shadow_id = user.custom_fields['shadow_id'].to_i) > 0
      ...
      if shadow && (shadow.post_count + shadow.topic_count) > 0 &&
         shadow.last_posted_at <
           SiteSetting.anonymous_account_duration_minutes.minutes.ago
        shadow = nil
      end
    ...
    User.transaction do
      shadow =
        User.create!(
          ...
          name: username,
          # prevents error when names are required
          username: username,
          active: true,
          trust_level: 1,
      ...
      UserCustomField.create!(
        user_id: user.id, name: 'shadow_id', value: shadow.id
      )
      UserCustomField.create!(
        user_id: shadow.id, name: 'master_id', value: user.id
      )
