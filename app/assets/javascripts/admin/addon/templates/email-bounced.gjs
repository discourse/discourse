  ...
  <table class="table email-list">
    ...
    <tbody>
      <tr class="filters">
        ...
        <td colspan="2"><TextField @value={{@controller.filter.type}} @placeholderKey="admin.email.logs.filters.type_placeholder" /></td>
      </tr>
      ...
      {{#each @controller.model as |l|}}
        <tr>
          ...
          <td>
            {{#if l.has_bounce_key}}
              <a href {{on "click" (fn @controller.handleShowIncomingEmail l.id)}}>
                {{l.email_type}}
              </a>
            {{else}}
              {{l.email_type}}
          ...
          <td class="email-details">
            {{#if l.has_bounce_key}}
              <a href {{on "click" (fn @controller.handleShowIncomingEmail l.id)}} title={{i18n "admin.email.details_title"}}>
                {{dIcon "circle-info"}}
              </a>
            {{/if}}
          </td>
