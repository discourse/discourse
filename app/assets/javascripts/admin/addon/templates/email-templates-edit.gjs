...
<div class="email-template">
  ...
  {{#if @controller.hasMultipleSubjects}}
    <h3><LinkTo @route="adminSiteText" @query={{hash q=@controller.hasMultipleSubjects}}>{{i18n "admin.customize.email_templates.multiple_subjects"}}</LinkTo></h3>
  {{else}}
  ...
  <SaveControls @model={{@controller.emailTemplate}} @action={{action "saveChanges"}} @saved={{@controller.saved}} @saveDisabled={{@controller.saveDisabled}}>
