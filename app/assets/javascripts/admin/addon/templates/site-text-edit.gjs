...
export default RouteTemplate(<template><div class="edit-site-text">
  ...
  <div class="title">
    <h4>{{i18n "admin.site_text.locale"}} {{@controller.localeFullName}}</h4>
  </div>
  ...
  {{#if @controller.isOutdated}}
    <div class="outdated">
      ...
      <DButton @action={{@controller.dismissOutdated}} @label="admin.site_text.outdated.dismiss" class="btn-default" />
    </div>
  {{/if}}

  <ExpandingTextArea {{on "input" (withEventValue (fn (mut @controller.buffered.value)))}} value={{@controller.buffered.value}} rows="1" class="site-text-value" />
  ...
  <SaveControls @model={{@controller.siteText}} @action={{action "saveChanges"}} @saved={{@controller.saved}} @saveDisabled={{@controller.saveDisabled}}>
    {{#if @controller.siteText.can_revert}}
      <DButton @action={{@controller.revertChanges}} @label="admin.site_text.revert" class="revert-site-text" />
    {{/if}}
  </SaveControls>

  <LinkTo @route="adminSiteText.index" @query={{hash locale=@controller.locale}} class="go-back">
    {{dIcon "arrow-left"}}
