  ...
  <table class="table email-list">
    ...
    <tbody>
      <tr class="filters">
        ...
        <td colspan="2"><TextField @value={{@controller.filter.error}} @placeholderKey="admin.email.incoming_emails.filters.error_placeholder" /></td>
      </tr>
      ...
      {{#each @controller.model as |email|}}
        <tr>
          ...
          <td>{{email.subject}}</td>
          <td class="error">
            <a href {{on "click" (fn @controller.handleShowIncomingEmail email.id)}}>{{email.error}}</a>
          </td>
          <td class="email-details">
            <a href {{on "click" (fn @controller.handleShowIncomingEmail email.id)}} title={{i18n "admin.email.details_title"}}>
              {{dIcon "circle-info"}}
            </a>
          </td>
        </tr>
