...
export default RouteTemplate(<template><section class="details {{unless @controller.model.active "not-activated"}}">
  <div class="user-controls">
    {{#if @controller.model.canViewProfile}}
      <LinkTo @route="user" @model={{@controller.model}} class="btn btn-default">
        {{dIcon "user"}}
    ...
    {{#if @controller.model.can_view_action_logs}}
      <DButton @action={{fn @controller.viewActionLogs @controller.model.username}} @icon="far-rectangle-list" @label="admin.user.action_logs" class="btn-default" />
    {{/if}}
    {{#if @controller.model.active}}
      {{#if @controller.currentUser.admin}}
        <DButton @action={{@controller.logOut}} @icon="power-off" @label="admin.user.log_out" class="btn-default" />
      {{/if}}
    {{/if}}
    <PluginOutlet @name="admin-user-controls-after" @outletArgs={{hash model=@controller.model}} />
  </div>

  <div class="display-row username">
    <AdminEditableField @name="user.username.title" @value={{@controller.model.username}} @action={{action "saveUsername"}} @editing={{@controller.editingUsername}} />
  </div>

  <div class="display-row name">
    <AdminEditableField @name="user.name.title" @value={{@controller.model.name}} @action={{action "saveName"}} @editing={{@controller.editingName}} />
  </div>

  <PluginOutlet @name="admin-user-below-names" @outletArgs={{hash user=@controller.model}} />
  ...
  {{#if @controller.canCheckEmails}}
    <div class="display-row email">
      ...
      <div class="value">
        ...
        {{else}}
          <DButton @action={{fn (routeAction "checkEmail") @controller.model}} @icon="envelope" @label="admin.users.check_email.text" @title="admin.users.check_email.title" class="btn-default" />
        {{/if}}
      </div>
      <div class="controls">
        ...
        {{else if @controller.model.email}}
          {{htmlSafe (i18n "admin.user.visit_profile" url=@controller.preferencesPath)}}
        {{/if}}
      </div>
    </div>
    ...
    <div class="display-row bounce-score">
      <div class="field"><a href={{@controller.model.bounceLink}}>{{i18n "admin.user.bounce_score"}}</a></div>
      <div class="value">{{@controller.model.bounceScore}}</div>
      <div class="controls">
        {{#if @controller.model.canResetBounceScore}}
          <DButton @action={{@controller.resetBounceScore}} @label="admin.user.reset_bounce_score.label" @title="admin.user.reset_bounce_score.title" class="btn-default" />
        {{/if}}
    ...
    <div class="display-row associations">
      ...
      {{#if (and @controller.currentUser.admin @controller.associatedAccounts)}}
        <div class="controls">
          <DButton @action={{@controller.deleteAssociatedAccounts}} @icon="trash-can" @label="admin.users.delete_associated_accounts.text" @title="admin.users.delete_associated_accounts.title" class="btn-danger" />
        </div>
      {{/if}}
    </div>
  {{/if}}
  ...
  <div class="display-row">
    <AdminEditableField @name="user.title.title" @value={{@controller.model.title}} @action={{action "saveTitle"}} @editing={{@controller.editingTitle}} />
  </div>
    ...
    <div class="controls">
      {{#if @controller.currentUser.staff}}
        {{#if @controller.model.ip_address}}
          <IpLookup @ip={{@controller.model.ip_address}} @userId={{@controller.model.id}} />
        {{/if}}
      {{/if}}
    </div>
  </div>
    ...
    <div class="controls">
      {{#if @controller.currentUser.staff}}
        {{#if @controller.model.registration_ip_address}}
          <IpLookup @ip={{@controller.model.registration_ip_address}} @userId={{@controller.model.id}} />
        {{/if}}
      {{/if}}
    </div>
  </div>
  ...
  {{#if @controller.showBadges}}
    <div class="display-row">
      ...
      <div class="controls">
        <LinkTo @route="adminUser.badges" @model={{@controller.model}} class="btn btn-default">
          {{dIcon "certificate"}}
  ...
  <div class="display-row second-factor">
    ...
    <div class="controls">
      {{#if @controller.canDisableSecondFactor}}
        <DButton @action={{@controller.disableSecondFactor}} @icon="unlock-keyhole" @label="user.second_factor.disable" class="btn-default disable-second-factor" />
      {{/if}}
    </div>
  </div>
</section>
...
<span>
  <PluginOutlet @name="admin-user-details" @connectorTagName="div" @outletArgs={{hash model=@controller.model}} />
</span>
...
<section class="details">
      ...
      <div class="controls">
        ...
        {{else}}
          {{#if @controller.model.can_approve}}
            <DButton @action={{@controller.approve}} @icon="check" @label="admin.user.approve" class="btn-default" />
          {{/if}}
        {{/if}}
      </div>
    </div>
    ...
    <div class="controls">
      {{#if @controller.model.active}}
        {{#if @controller.model.can_deactivate}}
          <DButton @action={{@controller.deactivate}} @label="admin.user.deactivate_account" class="btn-default" />
          {{i18n "admin.user.deactivate_explanation"}}
        {{/if}}
      {{else}}
        {{#if @controller.model.can_send_activation_email}}
          <DButton @action={{@controller.sendActivationEmail}} @icon="envelope" @label="admin.user.send_activation_email" class="btn-default" />
        {{/if}}
        {{#if @controller.model.can_activate}}
          <DButton @action={{@controller.activate}} @icon="check" @label="admin.user.activate" class="btn-default" />
        {{/if}}
      {{/if}}
    </div>
  </div>
  ...
  {{#if @controller.currentUser.admin}}
    <div class="display-row">
      ...
      <div class="controls">
        <DButton @href="/admin/api/keys" @label="admin.api.manage_keys" class="btn-default" />
      </div>
    </div>
  {{/if}}
  ...
  <div class="display-row">
    ...
    <div class="controls">
      {{#if @controller.model.can_revoke_admin}}
        <DButton @action={{@controller.revokeAdmin}} @icon="shield-halved" @label="admin.user.revoke_admin" class="btn-default" />
      {{/if}}
      {{#if @controller.model.can_grant_admin}}
        <DButton @action={{@controller.grantAdmin}} @icon="shield-halved" @label="admin.user.grant_admin" class="btn-default grant-admin" />
      {{/if}}
    </div>
  </div>
  ...
  <div class="display-row">
    ...
    <div class="controls">
      {{#if @controller.model.can_revoke_moderation}}
        <DButton @action={{@controller.revokeModeration}} @icon="shield-halved" @label="admin.user.revoke_moderation" class="btn-default" />
      {{/if}}
      {{#if @controller.model.can_grant_moderation}}
        <DButton @action={{@controller.grantModeration}} @icon="shield-halved" @label="admin.user.grant_moderation" class="btn-default" />
      {{/if}}
    </div>
  </div>
  ...
  <div class="display-row">
    ...
    <div class="value">
      <ComboBox @content={{@controller.site.trustLevels}} @nameProperty="detailedName" @value={{@controller.model.trustLevel.id}} @onChange={{fn (mut @controller.model.trust_level)}} />

      {{#if @controller.model.dirty}}
        <div>
          <DButton @action={{@controller.saveTrustLevel}} @icon="check" class="ok no-text" />
          <DButton @action={{@controller.restoreTrustLevel}} @icon="xmark" class="cancel no-text" />
        </div>
      {{/if}}
    </div>
    <div class="controls">
      {{#if @controller.model.canLockTrustLevel}}
        {{#if @controller.hasLockedTrustLevel}}
          ...
          <DButton @action={{fn @controller.lockTrustLevel false}} @label="admin.user.unlock_trust_level" class="btn-default" />
        {{else}}
          {{dIcon "unlock" title="admin.user.trust_level_unlocked_tip"}}
          <DButton @action={{fn @controller.lockTrustLevel true}} @label="admin.user.lock_trust_level" class="btn-default" />
        {{/if}}
      {{/if}}
      {{#if @controller.model.tl3Requirements}}
        <LinkTo @route="adminUser.tl3Requirements" @model={{@controller.model}} class="btn btn-default">
  ...
  <div class="user-suspended display-row
    ...
    <div class="value">
      ...
      {{#if @controller.model.suspended}}
        {{#unless @controller.model.suspendedForever}}
          {{i18n "admin.user.suspended_until" until=@controller.model.suspendedTillDate}}
        {{/unless}}
      {{/if}}
    </div>
    <div class="controls">
      {{#if @controller.model.suspended}}
        <DButton @action={{@controller.unsuspend}} @icon="ban" @label="admin.user.unsuspend" class="btn-danger unsuspend-user" />
        {{i18n "admin.user.suspended_explanation"}}
      {{else}}
        {{#if @controller.model.canSuspend}}
          <DButton @action={{@controller.showSuspendModal}} @icon="ban" @label="admin.user.suspend" class="btn-danger suspend-user" />
          {{i18n "admin.user.suspended_explanation"}}
        {{/if}}
      {{/if}}
    </div>
  ...
  {{#if @controller.model.suspended}}
    <div class="display-row highlight-danger suspension-info">
      ...
      <div class="controls">
        ...
        <div class="full-reason">{{@controller.model.full_suspend_reason}}</div>
      </div>
    </div>
  {{/if}}

  <div class="display-row {{if @controller.model.silenced "highlight-danger"}}">
    ...
    <div class="value">
      ...
      {{#if @controller.model.silenced}}
        {{#unless @controller.model.silencedForever}}
          {{i18n "admin.user.suspended_until" until=@controller.model.silencedTillDate}}
        {{/unless}}
      {{/if}}
    </div>
    <div class="controls">
      <ConditionalLoadingSpinner @size="small" @condition={{@controller.model.silencingUser}}>
        {{#if @controller.model.silenced}}
          <DButton @action={{@controller.unsilence}} @icon="microphone-slash" @label="admin.user.unsilence" class="btn-danger unsilence-user" />
          {{i18n "admin.user.silence_explanation"}}
        {{else}}
          {{#if @controller.model.canSilence}}
            <DButton @action={{@controller.showSilenceModal}} @icon="microphone-slash" @label="admin.user.silence" class="btn-danger silence-user" />
            {{i18n "admin.user.silence_explanation"}}
          {{/if}}
        {{/if}}
  ...
  {{#if @controller.model.silenced}}
    <div class="display-row highlight-danger silence-info">
      ...
      <div class="controls">
        ...
        <div class="full-reason">{{htmlSafe @controller.model.silence_reason}}</div>
      </div>
    </div>
  {{/if}}
  ...
  {{#if @controller.model.tl3_requirements.penalty_counts.total}}
    <div class="display-row clear-penalty-history">
      ...
      <div class="value">{{@controller.model.tl3_requirements.penalty_counts.total}}</div>
      {{#if @controller.currentUser.admin}}
        <div class="controls">
          <DButton @label="admin.user.clear_penalty_history.title" @icon="xmark" @action={{@controller.clearPenaltyHistory}} class="btn-default" />
...
{{#if @controller.currentUser.admin}}
  <section class="details">
    ...
    <div class="display-row">
      ...
      <div class="value">
        <GroupChooser @content={{@controller.availableGroups}} @value={{@controller.customGroupIdsBuffer}} @labelProperty="name" @onChange={{fn (mut @controller.customGroupIdsBuffer)}} />
      </div>
      {{#if @controller.customGroupsDirty}}
        <div class="controls">
          <DButton @icon="check" @action={{@controller.saveCustomGroups}} class="ok" />
          <DButton @icon="xmark" @action={{@controller.resetCustomGroups}} class="cancel" />
        </div>
      {{/if}}
    </div>
    {{#if @controller.model.customGroups}}
      <div class="display-row">
        ...
        <div class="value">
          <ComboBox @content={{@controller.model.customGroups}} @value={{@controller.model.primary_group_id}} @onChange={{fn (mut @controller.model.primary_group_id)}} @options={{hash none="admin.groups.no_primary"}} />
        </div>
        {{#if @controller.primaryGroupDirty}}
          <div class="controls">
            <DButton @icon="check" @action={{@controller.savePrimaryGroup}} class="ok" />
            <DButton @icon="xmark" @action={{@controller.resetPrimaryGroup}} class="cancel" />
          </div>
        {{/if}}
      </div>
    {{/if}}
...
<section class="details">
  ...
  <div class="display-row">
    ...
    <div class="value">{{formatDate @controller.model.created_at leaveAgo="true"}}</div>
  </div>
  <div class="display-row">
  ...
  <div class="display-row">
    ...
    <div class="value">{{formatDate @controller.model.last_seen_at leaveAgo="true"}}</div>
  </div>
  <div class="display-row">
    ...
    <div class="controls">
      {{#if @controller.model.can_delete_all_posts}}
        {{#if @controller.model.post_count}}
          <DButton @action={{@controller.showDeletePostsConfirmation}} @icon="trash-can" @label="admin.user.delete_posts.button" class="btn-danger" />
        {{/if}}
      {{else}}
  ...
  <div class="display-row">
    ...
    <div class="controls">
      {{#if @controller.model.flags_received_count}}
        <LinkTo @route="review" @query={{hash username=@controller.model.username type="ReviewableFlaggedPost" status="all"}} class="btn">
  ...
  <div class="display-row post-edits-count">
    ...
    <div class="value">
      {{if (gt @controller.model.post_edits_count 0) @controller.model.post_edits_count "0"}}
    </div>
    <div class="controls">
      {{#if (gt @controller.model.post_edits_count 0)}}
        <LinkTo @route="adminReports.show" @model="post_edits" @query={{hash filters=@controller.postEditsByEditorFilter}} class="btn btn-icon">
          {{dIcon "far-eye"}}
    ...
    {{#let @controller.model.single_sign_on_record as |sso|}}
      <div class="display-row">
        ...
        {{#if @controller.model.can_delete_sso_record}}
          <div class="controls">
            <DButton @action={{@controller.deleteSSORecord}} @icon="trash-can" @label="admin.user.discourse_connect.delete_sso_record" class="btn-danger" />
          </div>
        {{/if}}
      </div>
      <div class="display-row">
      ...
      {{#if @controller.canAdminCheckEmails}}
        <div class="display-row">
          ...
          {{else}}
            <DButton @action={{fn @controller.checkSsoEmail @controller.model}} @icon="envelope" @label="admin.users.check_email.text" @title="admin.users.check_email.title" class="btn-default" />
          {{/if}}
        </div>
      {{/if}}
      <div class="display-row">
      ...
      {{#if @controller.canAdminCheckEmails}}
        <div class="display-row">
          ...
          {{else}}
            <DButton @action={{fn @controller.checkSsoPayload @controller.model}} @icon="far-rectangle-list" @label="admin.users.check_sso.text" @title="admin.users.check_sso.title" class="btn-default" />
          {{/if}}
        </div>
      {{/if}}
    {{/let}}
...
<span>
  <PluginOutlet @name="after-user-details" @connectorTagName="div" @outletArgs={{hash model=@controller.model}} />
</span>
  ...
  <div class="pull-right">
    {{#if @controller.model.active}}
      {{#if @controller.model.can_impersonate}}
        <DButton @action={{@controller.impersonate}} @icon="crosshairs" @label="admin.impersonate.title" @title="admin.impersonate.help" class="btn-danger btn-impersonate" />
      {{/if}}
    {{/if}}

    {{#if @controller.model.can_be_anonymized}}
      <DButton @label="admin.user.anonymize" @icon="triangle-exclamation" @action={{@controller.anonymize}} class="btn-danger btn-anonymize" />
    {{/if}}

    {{#if @controller.model.canBeDeleted}}
      <DButton @label="admin.user.delete" @icon="trash-can" @action={{@controller.destroyUser}} class="btn-danger btn-user-delete" />
    {{/if}}

    {{#if @controller.model.can_be_merged}}
      <DButton @label="admin.user.merge.button" @icon="left-right" @action={{@controller.promptTargetUser}} class="btn-danger btn-user-merge" />
    {{/if}}
  </div>
