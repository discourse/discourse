  ...
  <table class="table email-list">
    ...
    <tbody>
      <tr class="filters">
        ...
        <td><TextField @value={{@controller.filter.user}} @placeholderKey="admin.email.logs.filters.user_placeholder" /></td>
        <td><TextField @value={{@controller.filter.address}} @placeholderKey="admin.email.logs.filters.address_placeholder" /></td>
        <td><TextField @value={{@controller.filter.type}} @placeholderKey="admin.email.logs.filters.type_placeholder" /></td>
        <td><TextField @value={{@controller.filter.reply_key}} @placeholderKey="admin.email.logs.filters.reply_key_placeholder" /></td>
        <td></td>
      </tr>
      ...
      {{#each @controller.model as |l|}}
        <tr class="sent-email-item">
          ...
          <td class="sent-email-address">
            {{#if l.bounced}}{{dIcon "arrow-rotate-right" title="admin.email.bounced"}}{{/if}}
            <p><a href="mailto:{{l.to_address}}" title="TO">{{l.to_address}}</a></p>
            {{#if l.cc_addresses}}
              {{#if (gt l.cc_addresses.length @controller.ccAddressDisplayThreshold)}}
                {{#each (slice 0 @controller.ccAddressDisplayThreshold (fn @controller.sortWithAddressFilter l.cc_addresses)) as |cc|}}
                  <p><a href="mailto:{{cc}}" title="CC">{{cc}}</a></p>
                {{/each}}
                <DTooltip @placement="right-start" @arrow={{true}} @identifier="email-log-cc-addresses" @interactive={{true}}>
                  <:trigger>
                    {{i18n "admin.email.logs.email_addresses.see_more"}}
                  </:trigger>
                  <:content>
                    <ul>
                      {{#each (slice @controller.ccAddressDisplayThreshold l.cc_addresses) as |cc|}}
                        <li>
                          <span>
