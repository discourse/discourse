...
<div class="screened-ip-controls">
  <div class="filter-screened-ip-address inline-form">
    <TextField @value={{@controller.filter}} @placeholderKey="admin.logs.screened_ips.form.filter" @autocorrect="off" @autocapitalize="off" class="ip-address-input" />
    <DButton @action={{@controller.exportScreenedIpList}} @icon="download" @title="admin.export_csv.button_title.screened_ip" @label="admin.export_csv.button_text" class="btn-default" />
  </div>
        ...
        {{#each @controller.model as |item|}}
          <tr class="admin-list-item">
            <td class="col first ip_address">
              ...
              {{else}}
                <a href {{on "click" (fn @controller.edit item)}} class="inline-editable-field">
                  {{#if item.isRange}}
            ...
            <td class="col actions">
              {{#if item.editing}}
                <DButton @action={{fn @controller.save item}} @label="admin.logs.save" class="btn-default" />
                <DButton @action={{fn @controller.cancel item}} @translatedLabel={{i18n "cancel"}} class="btn-flat" />
              {{else}}
                <DButton @action={{fn @controller.destroyRecord item}} @icon="trash-can" class="btn-default btn-danger" />
                <DButton @action={{fn @controller.edit item}} @icon="pencil" class="btn-default" />
                {{#if item.isBlocked}}
                  <DButton @action={{fn @controller.allow item}} @icon="check" @label="admin.logs.screened_ips.actions.do_nothing" class="btn-default" />
                {{else}}
                  <DButton @action={{fn @controller.block item}} @icon="ban" @label="admin.logs.screened_ips.actions.block" class="btn-default" />
                {{/if}}
              {{/if}}
            </td>
          </tr>
