      ...
      <tbody>
        {{#each @controller.model as |item|}}
          <tr class="admin-list-item">
            ...
            <td class="last_match_at"><div class="label">{{i18n "admin.logs.last_match_at"}}</div>{{ageWithTooltip item.last_match_at}}</td>
            <td class="created_at"><div class="label">{{i18n "admin.logs.created_at"}}</div>{{ageWithTooltip item.created_at}}</td>
            <td class="ip_address">{{item.ip_address}}</td>
            <td class="action">
              <DButton @action={{fn @controller.clearBlock item}} @icon="check" @label="admin.logs.screened_emails.actions.allow" />
            </td>
          </tr>
        {{/each}}
      </tbody>
