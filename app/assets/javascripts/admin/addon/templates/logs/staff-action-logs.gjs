...
export default RouteTemplate(<template><div class="staff-action-logs-controls">
  {{#if @controller.filtersExists}}
    <div class="staff-action-logs-filters">
      <a href {{on "click" @controller.clearAllFilters}} class="clear-filters filter btn">
        <span class="label">{{i18n "admin.logs.staff_actions.clear_filters"}}</span>
      </a>
      {{#if @controller.actionFilter}}
        <a href {{on "click" (fn @controller.clearFilter "actionFilter")}} class="filter btn">
      ...
      {{#if @controller.filters.acting_user}}
        <a href {{on "click" (fn @controller.clearFilter "acting_user")}} class="filter btn">
          <span class="label">{{i18n "admin.logs.staff_actions.staff_user"}}</span>:
          {{@controller.filters.acting_user}}
          {{dIcon "circle-xmark"}}
        </a>
      {{/if}}
      {{#if @controller.filters.target_user}}
        <a href {{on "click" (fn @controller.clearFilter "target_user")}} class="filter btn">
          <span class="label">{{i18n "admin.logs.staff_actions.target_user"}}</span>:
          {{@controller.filters.target_user}}
          {{dIcon "circle-xmark"}}
        </a>
      {{/if}}
      {{#if @controller.filters.subject}}
        <a href {{on "click" (fn @controller.clearFilter "subject")}} class="filter btn">
          <span class="label">{{i18n "admin.logs.staff_actions.subject"}}</span>:
  ...
  {{else}}
    ...
    <ComboBox @content={{@controller.userHistoryActions}} @value={{@controller.filterActionId}} @onChange={{action "filterActionIdChanged"}} @options={{hash none="admin.logs.staff_actions.all"}} @id="staff-action-logs-action-filter" />
  {{/if}}

  <DButton @action={{@controller.exportStaffActionLogs}} @label="admin.export_csv.button_text" @icon="download" class="btn-default" />
</div>
          ...
          {{#each @controller.model.content as |item|}}
            <tr class="admin-list-item" data-user-history-id={{item.id}}>
              ...
              <td class="col value action">
                <a href {{on "click" (fn @controller.filterByAction item)}}>{{item.actionName}}</a>
              </td>
              <td class="col value subject">
                <div class="subject">
                  {{#if item.target_user}}
                    <LinkTo @route="adminUser" @model={{item.target_user}}>{{avatar item.target_user imageSize="tiny"}}</LinkTo>
                    <a href {{on "click" (fn @controller.filterByTargetUser item.target_user)}}>{{item.target_user.username}}</a>
                  {{/if}}
                  {{#if item.subject}}
                    <a href {{on "click" (fn @controller.filterBySubject item.subject)}} title={{item.subject}}>{{item.subject}}</a>
                  {{/if}}
                </div>
              </td>
              <td class="col value created-at">{{ageWithTooltip item.created_at}}</td>
              <td class="col value details">
                <div>
                  {{htmlSafe item.formattedDetails}}
                  {{#if item.useCustomModalForDetails}}
                    <a href {{on "click" (fn @controller.showCustomDetailsModal item)}}>{{dIcon "circle-info"}}
                      {{i18n "admin.logs.staff_actions.show"}}</a>
                  {{/if}}
                  {{#if item.useModalForDetails}}
                    <a href {{on "click" (fn @controller.showDetailsModal item)}}>{{dIcon "circle-info"}}
                      {{i18n "admin.logs.staff_actions.show"}}</a>
                  {{/if}}
                </div>
              </td>
