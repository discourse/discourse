  ...
  {{#if @controller.model}}
    <form class="form-horizontal">
      ...
      <div class="control-group">
        <label class="checkbox-label">
          <Input @type="checkbox" @checked={{@controller.replaceBadgeOwners}} />
          {{i18n "admin.badges.mass_award.replace_owners"}}
        </label>
        {{#if @controller.model.multiple_grant}}
          <label class="grant-existing-holders">
            <Input @type="checkbox" @checked={{@controller.grantExistingHolders}} class="grant-existing-holders-checkbox" />
            {{i18n "admin.badges.mass_award.grant_existing_holders"}}
          </label>
        {{/if}}
      </div>
      <DButton @action={{@controller.massAward}} @disabled={{@controller.massAwardButtonDisabled}} @icon="certificate" @label="admin.badges.mass_award.perform" type="submit" class="btn-primary" />
    ...
    {{#if @controller.resultsMessage}}
      ...
      {{#if @controller.unmatchedEntries.length}}
        <p>
          {{dIcon "triangle-exclamation" class="bulk-award-status-icon failure"}}
          <span>
            {{#if @controller.unmatchedEntriesTruncated}}
              {{i18n "admin.badges.mass_award.csv_has_unmatched_users_truncated_list" count=@controller.unmatchedEntriesCount}}
            {{else}}
