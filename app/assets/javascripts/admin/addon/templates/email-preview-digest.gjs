...
<div class="admin-controls email-preview">
  <div class="controls">
    <div class="inline-form">
      ...
      <EmailGroupUserChooser @value={{@controller.username}} @onChange={{action "updateUsername"}} @options={{hash maximum=1 caretDownIcon="caret-down" caretUpIcon="caret-up"}} />
      <DButton @action={{@controller.refresh}} @label="admin.email.refresh" class="btn-primary digest-refresh-button" />
      <div class="toggle">
        <label>{{i18n "admin.email.format"}}</label>
        {{#if @controller.showHtml}}
          <span>{{i18n "admin.email.html"}}</span>
          |
          <a href {{on "click" @controller.toggleShowHtml}} class="show-text-link">
            {{i18n "admin.email.text"}}
          </a>
        {{else}}
          <a href {{on "click" @controller.toggleShowHtml}} class="show-html-link">{{i18n "admin.email.html"}}</a>
          |
  ...
  <div class="email-preview-digest">
      ...
      <div class="controls">
        <div class="inline-form">
          ...
          {{else}}
            ...
            <TextField @value={{@controller.email}} @placeholderKey="admin.email.test_email_address" />
            <DButton @action={{@controller.sendEmail}} @disabled={{@controller.sendEmailDisabled}} @label="admin.email.send_digest" class="btn-default" />
    ...
    <div class="preview-output">
      {{#if @controller.showHtml}}
        ...
        {{else}}
          <iframe title={{i18n "admin.email.html_preview"}} srcdoc={{@controller.model.html_content}}></iframe>
        {{/if}}
      {{else}}
