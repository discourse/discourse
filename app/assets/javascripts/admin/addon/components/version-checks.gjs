...
<div class="dashboard-stats version-check
  ...
  {{else}}
    ...
    <div class="version-status">
      <div class="face">
        ...
        {{else}}
          <span class="icon
              {{if this.versionCheck.critical_updates "critical-updates-available" "updates-available"}}">
  ...
  <CustomHtml @name="update-header" @versionCheck={{this.versionCheck}} @tagName="div" @classNames="update-header" />

  <PluginOutlet @name="admin-upgrade-header" @outletArgs={{hash versionCheck=this.versionCheck}} />
</div></template>}
