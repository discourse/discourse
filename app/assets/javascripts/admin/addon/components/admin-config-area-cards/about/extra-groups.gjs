...
export default class AdminConfigAreasAboutExtraGroups extends Component {
  ...
  get data() {
    return {
      aboutPageExtraGroups: this.args.extraGroups.aboutPageExtraGroups.value.split("|").map(Number) || [],
      aboutPageExtraGroupsInitialMembers: this.args.extraGroups.aboutPageExtraGroupsInitialMembers.value,
      aboutPageExtraGroupsOrder: this.args.extraGroups.aboutPageExtraGroupsOrder.value,
      aboutPageExtraGroupsShowDescription: this.args.extraGroups.aboutPageExtraGroupsShowDescription.value === "true",
    };
  }
  ...
  async save(data) {
    ...
    try {
      ...
      this.toasts.success({
        ...
        data: {
          message: i18n(
            "admin.config_areas.about.toasts.extra_groups_saved"
          ),
        },
      });
    } catch (err) {
  ...
  <template>
    <Form @data={{this.data}} @onSubmit={{this.save}} as |form|>
      ...
      <form.Field
        ...
        @description={{i18n "admin.config_areas.about.extra_groups.initial_members_description"}}
        @validation="required"
        @format="large"
        as |field|
      >
