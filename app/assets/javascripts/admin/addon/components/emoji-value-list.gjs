  ...
  <ul class="values emoji-value-list">
    {{#each this.collection as |data index|}}
      <li class="value" data-index={{index}}>
        <DButton @action={{fn this.removeValue data}} @icon="xmark" @disabled={{not data.isEditable}} class="btn-default remove-value-btn btn-small" />

        <div class="value-input emoji-details
            {{if data.isEditable "can-edit"}}
            {{if data.isEditing "d-editor-textarea-wrapper"}}" {{on "click" (fn this.editValue index)}} role="button">
...
<div class="value">
  <EmojiPicker @label={{i18n0 "admin.site_settings.emoji_list.add_emoji_button.label"}} @didSelectEmoji={{this.emojiSelected}} />
</div></template>}
