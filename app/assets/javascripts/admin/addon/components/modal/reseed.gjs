...
<template><DModal @closeModal={{@closeModal}} @title={{i18n0 "admin.reseed.modal.title"}} @subtitle={{i18n0 "admin.reseed.modal.subtitle"}} class="reseed-modal" @flash={{this.flash}}>
  <:body>
    <ConditionalLoadingSpinner @condition={{this.loading}}>
      {{#if this.categories}}
        <fieldset>
          <legend class="options-group-title">
            {{i18n0 "admin.reseed.modal.categories"}}
          </legend>
      ...
      {{#if this.topics}}
        <fieldset>
          <legend class="options-group-title">
            {{i18n0 "admin.reseed.modal.topics"}}
          </legend>
  ...
  <:footer>
    <DButton @action={{this.reseed}} @label="admin.reseed.modal.replace" @isLoading={{this.reseeding}} class="btn-danger" />
