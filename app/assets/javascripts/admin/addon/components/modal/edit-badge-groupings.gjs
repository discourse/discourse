...
<template><DModal @title={{i18n0 "admin.badges.badge_groupings.modal_title"}} @bodyClass="badge-groupings-modal" @closeModal={{@closeModal}}>
  <:body>
        ...
        {{#each this.workingCopy as |wc|}}
          <li class="badge-grouping-item">
            ...
            <div class="actions">
              ...
              {{else}}
                <DButton @action={{fn (mut wc.editing) true}} @disabled={{wc.system}} @icon="pencil" />
              {{/if}}
              <DButton @action={{fn this.up wc}} @icon="chevron-up" />
              <DButton @action={{fn this.down wc}} @icon="chevron-down" />
              <DButton @action={{fn this.delete wc}} @disabled={{wc.system}} @icon="xmark" />
            </div>
          </li>
        {{/each}}
      </ul>
    </div>
    <DButton @action={{this.add}} class="badge-groupings__add-grouping" @label="admin.badges.new" />
  </:body>
  <:footer>
    <DButton @action={{this.saveAll}} @label="admin.badges.save" class="btn-primary badge-groupings__save" @disabled={{this.submitDisabled}} />
