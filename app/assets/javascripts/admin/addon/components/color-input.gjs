...
export default class ColorInput extends Component {
  @tracked hexValue = this.args?.hexValue || "";
  @tracked brightnessValue = this.args?.brightnessValue || 0;
  @tracked valid = this._valid();

  onlyHex = this.args?.onlyHex ?? true;
  styleSelection = this.args?.styleSelection ?? true;
  ...
  @action
  onPickerInput(event) {
    this.hexValue = event.target.value.replace("#", "");
  }
  ...
  hexValueChanged() {
    ...
    if (this._valid()) {
      document.querySelector(".picker").value = this.normalize(hex);
    }
  }
