...
module("Discourse Chat | Component | emoji-picker", function (hooks) {
  ...
  hooks.beforeEach(function () {
    ...
    this.emojiPickerManager = this.container.lookup(
      "service:emoji-picker-manager"
    );
    this.emojiPickerManager.open(() => {});
    this.emojiPickerManager.addVisibleSections([
      "smileys_&_emotion",
      "people_&_body",
      "objects",
    ]);
  ...
  test("When selecting an emoji", async function (assert) {
    ...
    await render(hbs`<EmojiPicker @didSelectEmoji={{this.didSelectEmoji}} />`);
