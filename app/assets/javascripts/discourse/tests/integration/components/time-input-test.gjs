...
module("Integration | Component | time-input", function (hooks) {
  ...
  test("default", async function (assert) {const self = this;
    ...
    await render(
      <template><TimeInput @hours={{self.hours}} @minutes={{self.minutes}} /></template>
    );
  ...
  test("allows mutations through actions", async function (assert) {const self = this;
    ...
    await render(
      <template><TimeInput @hours={{self.hours}} @minutes={{self.minutes}} @onChange={{self.onChange}} /></template>
    );
