...
module("Integration | Component | time-gap", function (hooks) {
  ...
  test("it renders days correctly", async function (assert) {const self = this;

    this.set("daysSince", 5);
    await render(<template><TimeGap @daysSince={{self.daysSince}} /></template>);
    assert
      .dom(".small-action-desc.timegap")
      .hasText(i18n("dates.later.x_days", { count: 5 }));
  });

  test("it renders months correctly", async function (assert) {const self = this;
  ...
  test("it renders years correctly", async function (assert) {const self = this;
  ...
  test("it renders the correct elements", async function (assert) {const self = this;
