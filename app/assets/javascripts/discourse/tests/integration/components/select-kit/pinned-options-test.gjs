...
module("Integration | Component | select-kit/pinned-options", function (hooks) {
  ...
  test("unpinning", async function (assert) {const self = this;
    ...
    await render(
      <template><PinnedOptions @value={{self.topic.pinned}} @topic={{self.topic}} /></template>
    );
  ...
  test("pinning", async function (assert) {const self = this;
