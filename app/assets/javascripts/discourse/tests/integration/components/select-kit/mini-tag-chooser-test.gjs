...
module(
  ...
  function (hooks) {
    ...
    test("displays tags", async function (assert) {const self = this;

      this.set("value", ["foo", "bar"]);

      await render(<template><MiniTagChooser @value={{self.value}} /></template>);

      assert.strictEqual(this.subject.header().value(), "foo,bar");
    });

    test("create a tag", async function (assert) {const self = this;
    ...
    test("max_tags_per_topic", async function (assert) {const self = this;
    ...
    test("disables search and shows limit when max_tags_per_topic is zero", async function (assert) {const self = this;
    ...
    test("required_tag_group", async function (assert) {const self = this;

      this.set("value", ["foo", "bar"]);

      await render(
        <template><MiniTagChooser @value={{self.value}} @options={{hash categoryId=1}} /></template>
      );
    ...
    test("creating a tag using invalid character", async function (assert) {
      await render(<template><MiniTagChooser @options={{hash allowAny=true}} /></template>);
      await this.subject.expand();
    ...
    test("values in hiddenFromPreview will not display in preview", async function (assert) {const self = this;

      this.set("value", ["foo", "bar"]);
      this.set("hiddenValues", ["foo"]);

      await render(
        <template><MiniTagChooser @options={{hash allowAny=true hiddenValues=self.hiddenValues}} @value={{self.value}} /></template>
      );
...
module(
  ...
  function (hooks) {
    ...
    test("displays tags and filter in header", async function (assert) {const self = this;

      this.set("value", ["apple", "orange", "potato"]);

      await render(
        <template><MiniTagChooser @value={{self.value}} @options={{hash filterable=true useHeaderFilter=true}} /></template>
      );
    ...
    test("removing a tag does not display the dropdown", async function (assert) {const self = this;
