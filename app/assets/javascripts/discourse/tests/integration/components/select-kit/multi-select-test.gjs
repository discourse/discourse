...
module("Integration | Component | select-kit/multi-select", function (hooks) {
  ...
  test("maximum=1", async function (assert) {const self = this;

    setDefaultState(this);

    await render(<template>
      <MultiSelect @value={{self.value}} @content={{self.content}} @options={{hash maximum=1}} />
    </template>);
  ...
  test("maximum=2", async function (assert) {const self = this;

    setDefaultState(this);

    await render(<template>
      <MultiSelect @value={{self.value}} @content={{self.content}} @options={{hash maximum=2}} />
    </template>);
  ...
  test("pasting", async function (assert) {const self = this;
