...
module("Integration | Component | select-kit/user-chooser", function (hooks) {
  ...
  test("displays usernames", async function (assert) {const self = this;
  ...
  test("can remove a username", async function (assert) {const self = this;
  ...
  test("can display default search results", async function (assert) {const self = this;
    ...
    await render(<template><UserChooser @options={{self.options}} /></template>);
