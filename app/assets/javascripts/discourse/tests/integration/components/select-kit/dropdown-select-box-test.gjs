...
module(
  ...
  function (hooks) {
    ...
    test("selection behavior", async function (assert) {const self = this;
    ...
    test("options.showFullTitle=false", async function (assert) {const self = this;
      ...
      await render(<template>
        <DropdownSelectBox @value={{self.value}} @content={{self.content}} @options={{hash icon="xmark" showFullTitle=self.showFullTitle none=self.none}} />
      </template>);
    ...
    test("options.showFullTitle=true", async function (assert) {const self = this;

      setDefaultState(this, { showFullTitle: true });

      await render(<template>
        <DropdownSelectBox @value={{self.value}} @content={{self.content}} @options={{hash showFullTitle=self.showFullTitle}} />
      </template>);
