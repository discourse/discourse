...
import selectKit, { DEFAULT_CONTENT, setDefaultState } from "discourse/tests/helpers/select-kit-helper";
...
module("Integration | Component | select-kit/api", function (hooks) {
  ...
  test("modifySelectKit(identifier).appendContent", async function (assert) {const self = this;
    ...
    await render(<template>
      <ComboBox @value={{self.value}} @content={{self.content}} @onChange={{self.onChange}} />
      <SingleSelect @value={{self.value}} @content={{self.content}} @onChange={{self.onChange}} />
    </template>);
  ...
  test("modifySelectKit(identifier).prependContent", async function (assert) {const self = this;
  ...
  test("modifySelectKit(identifier).onChange", async function (assert) {const self = this;
  ...
  test("modifySelectKit(identifier).replaceContent", async function (assert) {const self = this;
