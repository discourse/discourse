...
module(
  ...
  function (hooks) {
    ...
    test("excluding usernames", async function (assert) {const self = this;
      ...
      await render(
        <template><EmailGroupUserChooser @options={{hash excludedUsernames=self.excludedUsernames}} /></template>
      );
    ...
    test("shows user status if enabled", async function (assert) {
      ...
      await render(<template><EmailGroupUserChooser @showUserStatus={{true}} /></template>);
      await this.subject.expand();
      await fillIn(".filter-input", "test-user");
