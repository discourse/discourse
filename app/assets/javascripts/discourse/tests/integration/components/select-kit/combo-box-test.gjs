...
module("Integration | Component | select-kit/combo-box", function (hooks) {
  ...
  test("options.clearable", async function (assert) {const self = this;
    ...
    await render(<template>
      <ComboBox @value={{self.value}} @content={{self.content}} @onChange={{self.onChange}} @options={{hash clearable=self.clearable}} />
    </template>);
  ...
  test("options.{caretUpIcon,caretDownIcon}", async function (assert) {const self = this;
    ...
    await render(<template>
      <ComboBox @value={{self.value}} @content={{self.content}} @options={{hash caretUpIcon=self.caretUpIcon caretDownIcon=self.caretDownIcon}} />
    </template>);
