...
import selectKit, { setDefaultState } from "discourse/tests/helpers/select-kit-helper";
  ...
  function (hooks) {
    ...
    test("default", async function (assert) {const self = this;
      ...
      await render(<template>
        <NotificationsButton @value={{self.value}} @options={{hash i18nPrefix=self.i18nPrefix i18nPostfix=self.i18nPostfix}} />
      </template>);
