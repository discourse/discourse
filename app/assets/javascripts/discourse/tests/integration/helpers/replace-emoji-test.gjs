...
module("Integration | Helper | replace-emoji", function (hooks) {
  ...
  test("it replaces the emoji", async function (assert) {
    await render(<template><span>{{replaceEmoji "some text :heart:"}}</span></template>);
  ...
  test("it escapes the text", async function (assert) {
    await render(
      <template><span>{{replaceEmoji "<style>body: {background: red;}</style>"}}</span></template>
    );
  ...
  test("it renders html-safe text", async function (assert) {
    await render(<template><span>{{replaceEmoji (htmlSafe "safe text")}}</span></template>);
