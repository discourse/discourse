...
module("Addons | truth-helpers | Integration | includes", function (hooks) {
  ...
  test("when using an array", async function (assert) {const self = this;

    this.foo = [1];
    this.bar = 1;
    await render(
      <template>{{#if (includes self.foo self.bar)}}<span class="test"></span>{{/if}}</template>
    );
  ...
  test("when using a string", async function (assert) {const self = this;
