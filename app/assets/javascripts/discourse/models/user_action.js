...
Discourse.UserAction = Discourse.Model.extend({
  ...
  descriptionHtml: (function() {
    ...
    if (action === null || actions.indexOf(action) >= 0) {
      ...
      params = { u1: this.get('userUrl'), u2: this.get('targetUserUrl') };
      if (sameUser) {
        sentence = fill(Em.String.i18n('user_action.you_mentioned_user', { user: this.get('target_name') }), params);
      } else {
        if (this.get('target_username') === Discourse.get('currentUser.username')) {
          sentence = fill(Em.String.i18n('user_action.user_mentioned_you', { user: this.get('name') }), params);
        } else {
          sentence = fill(Em.String.i18n('user_action.user_mentioned_user', { user: this.get('name'), another_user: this.get('target_name') }), params);
        }
      }
    } else {
