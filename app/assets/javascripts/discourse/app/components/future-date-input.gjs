...
import { extendedDefaultTimeShortcuts, formatTime, hideDynamicTimeShortcuts, TIME_SHORTCUT_TYPES, timeShortcuts } from "discourse/lib/time-shortcut";
import { i18n } from "discourse-i18n";
import FutureDateInputSelector, { FORMAT } from "select-kit/components/future-date-input-selector";
...
<template><div class="future-date-input">
  {{#unless this.noRelativeOptions}}
    <div class="control-group">
      <label class={{this.labelClasses}}>
        {{#if this.displayLabelIcon}}{{icon this.displayLabelIcon}}{{/if}}{{this.displayLabel}}
      </label>
      <FutureDateInputSelector @value={{readonly this.selection}} @content={{this.shortcuts}} @clearable={{this.clearable}} @onChangeInput={{this.onChangeInput}} @onChange={{fn (mut this.selection)}} @options={{hash none="time_shortcut.select_timeframe"}} />
    </div>
  {{/unless}}
  ...
  {{#if this.displayDateAndTimePicker}}
    <div class="control-group future-date-input-date-picker">
      {{icon "calendar-days"}}
      <DatePickerFuture @value={{this._date}} @defaultDate={{this._date}} @onSelect={{action "onChangeDate"}} />
    </div>

    <div class="control-group future-date-input-time-picker">
      {{icon "far-clock"}}
      <Input placeholder="--:--" @type="time" class="time-input" @value={{this._time}} disabled={{this.timeInputDisabled}} {{on "input" (withEventValue this.onChangeTime)}} />
    </div>
  {{/if}}
</div></template>}
