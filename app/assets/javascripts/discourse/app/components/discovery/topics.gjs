...
{{#if @model.sharedDrafts}}
  {{#if this.site.useGlimmerTopicList}}
    <List @listTitle="shared_drafts.title" @top={{this.top}} @hideCategory="true" @category={{@category}} @topics={{@model.sharedDrafts}} @discoveryList={{true}} class="shared-drafts" />
  {{else}}
    <TopicList @listTitle="shared_drafts.title" @top={{this.top}} @hideCategory="true" @category={{@category}} @topics={{@model.sharedDrafts}} @discoveryList={{true}} class="shared-drafts" />
  {{/if}}
{{/if}}

<DiscoveryTopicsList @model={{@model}} @incomingCount={{this.topicTrackingState.incomingCount}} @bulkSelectHelper={{@bulkSelectHelper}} @class={{if this.footerEducation "--no-topics-education"}}>
  {{#if this.top}}
    <div class="top-lists">
      <PeriodChooser @period={{@period}} @action={{@changePeriod}} @fullDay={{false}} />
    </div>
  {{else}}
    {{#if (or this.topicTrackingState.hasIncoming @model.loadingBefore)}}
      <div class="show-more {{if this.hasTopics "has-topics"}}">
        <a tabindex="0" href {{on "click" this.showInserted}} class="alert alert-info clickable
          ...
          <CountI18n @key="topic_count_" @suffix={{this.topicTrackingState.filter}} @count={{or @model.loadingBefore this.topicTrackingState.incomingCount}} />
  ...
  <span class="after-topic-list-plugin-outlet-wrapper">
    <PluginOutlet @name="after-topic-list" @connectorTagName="div" @outletArgs={{hash category=@category tag=@tag loadingMore=@model.loadingMore canLoadMore=@model.canLoadMore}} />
  </span>
</DiscoveryTopicsList>
...
<footer class="topic-list-bottom">
  ...
  {{#if this.allLoaded}}
    <PluginOutlet @name="topic-list-bottom" @outletArgs={{hash category=@category tag=@tag allLoaded=this.allLoaded model=@model}}>
      <TopicDismissButtons @position="bottom" @selectedTopics={{@bulkSelectHelper.selected}} @model={{@model}} @showResetNew={{@showResetNew}} @showDismissRead={{@showDismissRead}} @resetNew={{@resetNew}} @dismissRead={{@dismissRead}} />

      <FooterMessage @education={{this.footerEducation}} @message={{this.footerMessage}}>
        {{#if @tag}}
          {{htmlSafe (i18n0 "topic.browse_all_tags_or_latest" basePath=(basePath))}}
        {{else if this.latest}}
          {{#if @category.canCreateTopic}}
            <DiscourseLinkedText @action={{fn this.composer.openNewTopic (hash category=@category)}} @text="topic.suggest_create_topic" />
          {{/if}}
        {{else if this.top}}
          {{htmlSafe (i18n0 "topic.browse_all_categories_latest_or_top" basePath=(basePath))}}
          <TopPeriodButtons @period={{@period}} @action={{@changePeriod}} />
        {{else}}
          {{htmlSafe (i18n0 "topic.browse_all_categories_latest" basePath=(basePath))}}
        {{/if}}
      </FooterMessage>
    </PluginOutlet>
  {{/if}}
