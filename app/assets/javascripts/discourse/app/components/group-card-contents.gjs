...
export default class GroupCardContents extends CardContentsBase {<template>{{#if this.visible}}
  <div class="card-content">
    ...
    {{else}}
      <div class="card-row first-row">
        <div class="group-card-avatar">
          <a {{on0 "click" this.handleShowGroup}} href={{this.groupPath}} class="card-huge-avatar">
        ...
        <div class="names">
          <span>
            <div class="names__primary {{this.group.name}}">
              <a {{on0 "click" this.handleShowGroup}} href={{this.groupPath}} class="group-page-link">{{this.group.name}}</a>
            </div>
        ...
        <ul class="usercard-controls group-details-button">
          <li>
            <GroupMembershipButton @model={{this.group}} @showLogin={{routeAction "showLogin"}} />
          </li>
          {{#if this.group.messageable}}
            <li>
              <DButton @action={{action "messageGroup"}} @icon="envelope" @label="groups.message" class="btn-primary group-message-button inline" />
            </li>
          {{/if}}
        </ul>
      </div>
        ...
        <div class="card-row third-row">
          <div class="members metadata">
            {{#each this.highlightedMembers as |user|}}
              <a {{on0 "click" this.close}} href={{user.path}} class="card-tiny-avatar">{{boundAvatar user "tiny"}}</a>
            {{/each}}
            {{#if this.showMoreMembers}}
              <a {{on0 "click" this.handleShowGroup}} href={{this.groupPath}} class="more-members-link">
