...
<template><a role="tab" class={{this.classNames}} id={{this.id}} tabindex={{this.tabIndex}} title={{@tab.title}} aria-label={{@tab.title}} aria-selected={{if this.isActive "true" "false"}} aria-controls={{this.ariaControls}} data-tab-number={{@tab.position}} href={{@tab.linkWhenActive}} {{on "click" @onTabClick}} {{on "keydown" @onTabClick}}>
  {{icon @tab.icon}}
  {{#if @tab.count}}
