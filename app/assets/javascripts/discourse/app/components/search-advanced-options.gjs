...
<template><summary>
  {{i18n0 "search.advanced.title"}}
</summary>
<div class="search-advanced-filters">
  <div class="search-advanced-options">
    <PluginOutlet @name="advanced-search-options-above" @connectorTagName="div" @outletArgs={{hash searchedTerms=this.searchedTerms onChangeSearchedTermField=this.onChangeSearchedTermField}} />

    <div class="control-group advanced-search-category">
      <label class="control-label">{{i18n0 "search.advanced.in_category.label"}}</label>
      <div class="controls">
        <SearchAdvancedCategoryChooser @id="search-in-category" @value={{this.searchedTerms.category.id}} @onChange={{action "onChangeSearchTermForCategory"}} />
      </div>
    </div>

    {{#if this.siteSettings.tagging_enabled}}
      <div class="control-group advanced-search-tags">
        <label class="control-label">{{i18n0 "search.advanced.with_tags.label"}}</label>
        <div class="controls">
          <TagChooser @id="search-with-tags" @tags={{this.searchedTerms.tags}} @everyTag={{true}} @unlimitedTagCount={{true}} @onChange={{action "onChangeSearchTermForTags"}} @options={{hash allowAny=false headerAriaLabel=(i18n0 "search.advanced.with_tags.aria_label")}} />
          {{#if this.showAllTagsCheckbox}}
            <section class="field">
              <label>
                ...
                {{i18n0 "search.advanced.filters.all_tags"}}
              </label>
            </section>
          {{/if}}
        </div>
    ...
    <div class="control-group advanced-search-topics-posts">
      <div class="controls">
        <fieldset class="grouped-control">
          <legend class="grouped-control-label">{{i18n0 "search.advanced.filters.label"}}</legend>
          ...
          {{#if this.currentUser}}
            <div class="grouped-control-field">
              ...
              <label class="checkbox-label" for="matching-title-only">
                {{i18n0 "search.advanced.filters.title"}}
              </label>
            </div>
            ...
            <div class="grouped-control-field">
              ...
              <label class="checkbox-label" for="matching-liked">{{i18n0 "search.advanced.filters.likes"}}</label>
            </div>
            ...
            <div class="grouped-control-field">
              ...
              <label class="checkbox-label" for="matching-in-messages">{{i18n0 "search.advanced.filters.private"}}</label>
            </div>
            ...
            <div class="grouped-control-field">
              ...
              <label class="checkbox-label" for="matching-seen">{{i18n0 "search.advanced.filters.seen"}}</label>
            </div>
          {{/if}}

          <ComboBox @id="in" @valueProperty="value" @content={{this.inOptions}} @value={{this.searchedTerms.in}} @onChange={{action "onChangeSearchTermForIn"}} @options={{hash none="user.locale.any" clearable=true}} />
        </fieldset>
      </div>
    </div>

    <div class="control-group advanced-search-topic-status">
      <label class="control-label">{{i18n0 "search.advanced.statuses.label"}}</label>
      <div class="controls">
        <ComboBox @id="search-status-options" @valueProperty="value" @content={{this.statusOptions}} @value={{this.searchedTerms.status}} @onChange={{action "onChangeSearchTermForStatus"}} @options={{hash none="user.locale.any" headerAriaLabel=(i18n0 "search.advanced.statuses.label") clearable=true}} />
      </div>
    </div>

    <div class="control-group advanced-search-posted-by">
      <label class="control-label">
        {{i18n0 "search.advanced.posted_by.label"}}
      </label>
      <div class="controls">
        <UserChooser @id="search-posted-by" @value={{this.searchedTerms.username}} @onChange={{action "onChangeSearchTermForUsername"}} @options={{hash headerAriaLabel=(i18n0 "search.advanced.posted_by.aria_label") maximum=1 excludeCurrentUser=false}} />
      </div>
    </div>

    <div class="control-group advanced-search-posted-date">
      <label class="control-label">{{i18n0 "search.advanced.post.time.label"}}</label>
      <div class="controls inline-form full-width">
        <ComboBox @id="postTime" @valueProperty="value" @content={{this.postTimeOptions}} @value={{this.searchedTerms.time.when}} @onChange={{action "onChangeWhenTime"}} @options={{hash headerAriaLabel=(i18n0 "search.advanced.post.time.aria_label")}} />
        <DateInput @date={{this.searchedTerms.time.days}} @onChange={{action "onChangeWhenDate"}} @inputId="search-post-date" />
      </div>
    </div>

    <PluginOutlet @name="advanced-search-options-below" @connectorTagName="div" @outletArgs={{hash searchedTerms=this.searchedTerms onChangeSearchedTermField=this.onChangeSearchedTermField}} />
  </div>

  <details class="search-advanced-additional-options">
    <summary>
      {{i18n0 "search.advanced.additional_options.label"}}
    </summary>
    <div class="count-group control-group">
      {{!-- TODO: Using a label here fails no-nested-interactive lint rule --}}
      <span class="control-label">{{i18n0 "search.advanced.post.count.label"}}</span>
      <div class="controls">
        <Input @type="number" @value={{readonly this.searchedTerms.min_posts}} class="input-small" id="search-min-post-count" placeholder={{i18n0 "search.advanced.post.min.placeholder"}} aria-label={{i18n0 "search.advanced.post.min.aria_label"}} {{on "input" (withEventValue this.onChangeSearchTermMinPostCount)}} />
        {{icon "left-right"}}
        <Input @type="number" @value={{readonly this.searchedTerms.max_posts}} class="input-small" id="search-max-post-count" placeholder={{i18n0 "search.advanced.post.max.placeholder"}} aria-label={{i18n0 "search.advanced.post.max.aria_label"}} {{on "input" (withEventValue this.onChangeSearchTermMaxPostCount)}} />
      </div>
    </div>

    <div class="count-group control-group">
      {{!-- TODO: Using a label here fails no-nested-interactive lint rule --}}
      <span class="control-label">{{i18n0 "search.advanced.views.label"}}</span>
      <div class="controls">
        <Input @type="number" @value={{readonly this.searchedTerms.min_views}} class="input-small" id="search-min-views" placeholder={{i18n0 "search.advanced.min_views.placeholder"}} aria-label={{i18n0 "search.advanced.min_views.aria_label"}} {{on "input" (withEventValue this.onChangeSearchTermMinViews)}} />
        {{icon "left-right"}}
        <Input @type="number" @value={{readonly this.searchedTerms.max_views}} class="input-small" id="search-max-views" placeholder={{i18n0 "search.advanced.max_views.placeholder"}} aria-label={{i18n0 "search.advanced.max_views.aria_label"}} {{on "input" (withEventValue this.onChangeSearchTermMaxViews)}} />
      </div>
    </div>
  </details>
  ...
  {{#if this.site.mobileView}}
    <div class="second-search-button">
      <DButton @action={{this.search}} @icon="magnifying-glass" @label="search.search_button" @ariaLabel="search.search_button" @disabled={{this.searchButtonDisabled}} class="btn-primary search-cta" />
    </div>
  {{/if}}
</div></template>}
