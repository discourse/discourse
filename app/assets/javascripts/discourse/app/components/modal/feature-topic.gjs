...
<template><DModal class="feature-topic" @title={{i18n0 "topic.feature_topic.title"}} @closeModal={{@closeModal}}>
  <:body>
            ...
            <p>
              <ConditionalLoadingSpinner @size="small" @condition={{this.loading}}>
                {{#if this.pinnedGloballyCount}}
                  {{htmlSafe (i18n0 "topic.feature_topic.already_pinned_globally" count=this.pinnedGloballyCount)}}
                {{else}}
                  {{htmlSafe (i18n0 "topic.feature_topic.not_pinned_globally")}}
                {{/if}}
