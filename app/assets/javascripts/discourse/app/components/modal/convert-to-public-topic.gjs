...
<template><DModal @title={{i18n "topic.make_public.title"}} @closeModal={{@closeModal}} class="convert-to-public-topic" @flash={{this.flash}}>
  <:body>
    <div class="instructions">
      {{i18n "topic.make_public.choose_category"}}
    </div>
    <CategoryChooser @value={{this.publicCategoryId}} @onChange={{fn (mut this.publicCategoryId)}} />
  </:body>
  <:footer>
    <DButton class="btn-primary" @action={{this.makePublic}} @label="composer.modal_ok" @disabled={{this.saving}} />
