...
export default class ShareTopicModal extends Component.extend(
  ...
  <template>
    <DModal
      @title={{if
        this.post
        (iN "post.share.title" post_number=this.post.post_number)
        (iN "topic.share.title")
      }}
      ...
      <form>
        <div class="input-group invite-link">
          <label for="invite-link">
            {{if
              this.post
              (iN "post.share.instructions" post_number=this.post.post_number)
              (iN "topic.share.instructions")
            }}
          </label>
