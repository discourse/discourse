...
<template><DModal @closeModal={{@closeModal}} @title={{i18n0 "user.second_factor.totp.add"}} {{didInsert this.totpRequested}}>
  <:body>
    <ConditionalLoadingSpinner @condition={{this.loading}}>
      ...
      <div class="control-group">
        <div class="controls">
          ...
          <p>
            ...
            {{else}}
              <a href class="show-second-factor-key" {{on "click" this.enableShowSecondFactorKey}}>{{i18n0 "user.second_factor.show_key_description"}}</a>
            {{/if}}
          </p>
        </div>
      </div>
      ...
      <div class="control-group">
        <label class="control-label input-prepend">{{i18n0 "user.second_factor.name"}}</label>
        <div class="controls">
          <input {{on "input" (withEventValue (fn (mut this.secondFactorName)))}} value={{this.secondFactorName}} type="text" placeholder={{i18n0 "user.second_factor.totp.default_name"}} maxlength={{this.maxSecondFactorNameLength}} id="second-factor-name" />
        </div>
