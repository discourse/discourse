...
const Revisions = <template><div id="revisions" data-post-id={{@model.post_id}} class={{@hiddenClasses}}>
  ...
  {{#if @mobileView}}
    ...
    {{#if @model.wiki_changes}}
      <div class="row">
        {{icon "far-pen-to-square" class=(if @model.wiki_changes.current "diff-ins" "diff-del")}}
      </div>
    {{/if}}
    {{#if @model.archetype_changes}}
      <div class="row">
        {{icon (if (eq @model.archetype_changes.current "private_message") "envelope" "comment")}}
      </div>
    {{/if}}
  ...
  <span>
    <PluginOutlet @name="post-revisions" @connectorTagName="div" @outletArgs={{hash model=@model}} />
  </span>

  <LinksRedirect {{didInsert @calculateBodyDiff @bodyDiffHTML}} {{didUpdate @calculateBodyDiff @bodyDiffHTML}} class="row body-diff">
    {{htmlSafe @bodyDiff}}
  </LinksRedirect>
</div></template>;
