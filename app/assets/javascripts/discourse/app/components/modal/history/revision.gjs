    ...
    {{#if this.site.desktopView}}
      <span>
        ...
        {{#if @model.post_type_changes}}
          {{icon "shield-halved" class=(if (eq @model.post_type_changes.current @site.post_types.moderator_action) "diff-del" "diff-ins")}}
        {{/if}}
  ...
  {{#if this.site.desktopView}}
    <div id="display-modes">
      <ul class="nav nav-pills">
        <li>
          <a href class={{concatClass "inline-mode" (if (eq @viewMode "inline") "active")}} {{on "click" @displayInline}} title={{i18n "post.revisions.displays.inline.title"}} aria-label={{i18n "post.revisions.displays.inline.title"}}>
            {{icon "far-square"}}
            {{i18n "post.revisions.displays.inline.button"}}
          </a>
        </li>
        <li>
          <a href class={{concatClass "side-by-side-mode" (if (eq @viewMode "side_by_side") "active")}} {{on "click" @displaySideBySide}} title={{i18n "post.revisions.displays.side_by_side.title"}} aria-label={{i18n "post.revisions.displays.side_by_side.title"}}>
            {{icon "table-columns"}}
            {{i18n "post.revisions.displays.side_by_side.button"}}
          </a>
        </li>
        <li>
          <a href class={{concatClass "side-by-side-markdown-mode" (if (eq @viewMode "side_by_side_markdown") "active")}} {{on "click" @displaySideBySideMarkdown}} title={{i18n "post.revisions.displays.side_by_side_markdown.title"}} aria-label={{i18n "post.revisions.displays.side_by_side_markdown.title"}}>
            {{icon "table-columns"}}
