...
<template><DModal id="choosing-topic" @title={{i18n0 "topic.move_to.title"}} @closeModal={{@closeModal}} class="choose-topic-modal" @flash={{this.flash}} @flashType="error">
  <:body>
    {{#if @model.topic.isPrivateMessage}}
      <div class="radios">
        {{#if this.canSplitToPM}}
          <label class="radio-label" for="move-to-new-message">
            ...
            <b>{{i18n0 "topic.move_to_new_message.radio_label"}}</b>
          </label>
        {{/if}}
        ...
        <label class="radio-label" for="move-to-existing-message">
          ...
          <b>{{i18n0 "topic.move_to_existing_message.radio_label"}}</b>
        </label>
      </div>
      ...
      {{#if this.canSplitTopic}}
        {{#if this.newMessage}}
          <p>
            {{htmlSafe (i18n0 "topic.move_to_new_message.instructions" count=@model.selectedPostsCount)}}
          </p>
          <form>
            <label>{{i18n0 "topic.move_to_new_message.message_title"}}</label>
            <TextField @value={{this.topicName}} @placeholderKey="composer.title_placeholder" id="split-topic-name" />

            {{#if this.canTagMessages}}
              <label>{{i18n0 "tagging.tags"}}</label>
      ...
      {{#if this.existingMessage}}
        <p>
          {{htmlSafe (i18n0 "topic.move_to_existing_message.instructions" count=@model.selectedPostsCount)}}
        </p>
        <form>
          <ChooseMessage @currentTopicId={{@model.topic.id}} @setSelectedTopicId={{fn (mut this.selectedTopic)}} @selectedTopicId={{this.selectedTopic.id}} />

          <label>{{i18n0 "topic.move_to_new_message.participants"}}</label>
          <EmailGroupUserChooser class="participant-selector" @value={{this.participants}} @onChange={{fn (mut this.participants)}} />

          {{#if this.selectedTopic}}
            <hr />
            <label for="chronological-order" class="checkbox-label">
              <Input id="chronological-order" @type="checkbox" @checked={{this.chronologicalOrder}} />
              {{i18n0 "topic.merge_topic.chronological_order"}}
            </label>
          {{/if}}
        </form>
      {{/if}}
    ...
    {{else}}
      <div class="radios">
        {{#if this.canSplitTopic}}
          <label class="radio-label" for="move-to-new-topic">
            ...
            <b>{{i18n0 "topic.split_topic.radio_label"}}</b>
          </label>
        {{/if}}
        ...
        <label class="radio-label" for="move-to-existing-topic">
          ...
          <b>{{i18n0 "topic.merge_topic.radio_label"}}</b>
        </label>
      ...
      {{#if this.existingTopic}}
        <p>
          {{htmlSafe (i18n0 "topic.merge_topic.instructions" count=@model.selectedPostsCount)}}
        </p>
        <form>
          <ChooseTopic @topicChangedCallback={{this.newTopicSelected}} @currentTopicId={{@model.topic.id}} @selectedTopicId={{this.selectedTopic.id}} />
      ...
      {{#if this.canSplitTopic}}
        {{#if this.newTopic}}
          <p>
            {{htmlSafe (i18n0 "topic.split_topic.instructions" count=@model.selectedPostsCount)}}
          </p>
          <form class="split-new-topic-form">
            <div class="control-group">
              <label>{{i18n0 "topic.split_topic.topic_name"}}</label>
              <TextField @value={{this.topicName}} @placeholderKey="composer.title_placeholder" id="split-topic-name" />
              <PluginOutlet @name="split-new-topic-title-after" @outletArgs={{hash selectedPosts=@model.selectedPosts updateTopicName=this.updateTopicName}} />
            </div>

            <div class="control-group">
              <label>{{i18n0 "categories.category"}}</label>
              <CategoryChooser @value={{this.categoryId}} class="small" @onChange={{fn (mut this.categoryId)}} />
              <PluginOutlet @name="split-new-topic-category-after" @outletArgs={{hash selectedPosts=@model.selectedPosts updateCategoryId=this.updateCategoryId}} />
            </div>
            ...
            {{#if this.canAddTags}}
              <div class="control-group">
                ...
                <PluginOutlet @name="split-new-topic-tag-after" @outletArgs={{hash selectedPosts=@model.selectedPosts updateTags=this.updateTags tags=this.tags}} />
              </div>
            {{/if}}
          </form>
        {{/if}}
  ...
  <:footer>
    <DButton class="btn-primary" @disabled={{this.buttonDisabled}} @action={{this.performMove}} @icon="right-from-bracket" @label={{this.buttonTitle}} />
  </:footer>
</DModal></template>}
