...
<template><ConditionalLoadingSpinner @condition={{this.loading}}>
  <LoadMore @selector=".bookmark-list .bookmark-list-item" @action={{this.loadMore}}>
          ...
          <PluginOutlet @name="bookmark-list-table-header">
            {{#if this.bulkSelectEnabled}}
              <th class="bulk-select topic-list-data">
                <FlatButton @action={{this.toggleBulkSelect}} @class="bulk-select" @icon="list-check" @title="bookmarks.bulk.toggle" />
              </th>
            {{/if}}
                  ...
                  {{~#if this.canDoBulkActions}}
                    <div class="bulk-select-bookmarks-dropdown">
                      <span class="bulk-select-bookmark-dropdown__count">
                        {{i18n0 "bookmarks.bulk.selected_count" count=this.selectedCount}}
                      </span>
                      <BulkSelectBookmarksDropdown @bulkSelectHelper={{this.bulkSelectHelper}} />
                    </div>

                  {{/if~}}
                  <DButton @action={{this.selectAll}} class="btn btn-default bulk-select-all" @label="bookmarks.bulk.select_all" />
                  <DButton @action={{this.clearAll}} class="btn btn-default bulk-clear-all" @label="bookmarks.bulk.clear_all" />
                </span>
              {{else}}
                <FlatButton @action={{this.toggleBulkSelect}} @class="bulk-select" @icon="list-check" @title="bookmarks.bulk.toggle" />
                {{i18n0 "topic.title"}}
            ...
            <th class="post-metadata topic-list-data">{{i18n0 "post.bookmarks.updated"}}</th>
            <th class="post-metadata topic-list-data">{{i18n0 "activity"}}</th>
            <th>&nbsp;</th>
          </PluginOutlet>
        {{/if}}
      </thead>
            ...
            {{#if this.bulkSelectEnabled}}
              <td class="bulk-select bookmark-list-data">
                <label for="bulk-select-{{bookmark.id}}">
                  <input type="checkbox" class="bulk-select" id="bulk-select-{{bookmark.id}}" data-id={{bookmark.id}} />
                </label>
              </td>
            {{/if}}
            <td class="main-link topic-list-data">
              <PluginOutlet @name="bookmark-list-before-link" @outletArgs={{hash bookmark=bookmark}} />

              <span class="link-top-line">
                <div class="bookmark-metadata">
                  {{#if bookmark.reminder_at}}
                    <span class="bookmark-metadata-item bookmark-reminder
                        {{if bookmark.reminderAtExpired "bookmark-expired-reminder"}}">
                ...
                <div class="bookmark-status-with-link">
                  ...
                  {{else}}
                    <a href={{bookmark.bookmarkable_url}} role="heading" aria-level="2" class="title" data-topic-id="${topic.id}">
              ...
              {{#if bookmark.bookmarkableTopicAlike}}
                <div class="link-bottom-line">
                  ...
                  {{discourseTags bookmark mode="list" tagsForUser=this.tagsForUser}}
                </div>
              {{/if}}
              {{#if (and this.site.mobileView bookmark.excerpt bookmark.user.avatar_template)}}
              ...
              <p class="post-excerpt" {{on "click" this.screenExcerptForExternalLink}}>{{htmlSafe bookmark.excerpt}}</p>
            </td>
            {{#if this.site.desktopView}}
              ...
              <td class="post-metadata topic-list-data updated-at">{{formatDate bookmark.updated_at format="tiny"}}</td>
              {{raw "list/activity-column" topic=bookmark class="num post-metadata" tagName="td"}}
            {{/if}}
            <td class="topic-list-data">
              <BookmarkActionsDropdown @bookmark={{bookmark}} @removeBookmark={{action "removeBookmark"}} @editBookmark={{action "editBookmark"}} @clearBookmarkReminder={{action "clearBookmarkReminder"}} @togglePinBookmark={{action "togglePinBookmark"}} />
            </td>
          </tr>
        {{/each}}
      </tbody>
