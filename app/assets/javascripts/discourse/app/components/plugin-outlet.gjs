...
export default class PluginOutletComponent extends GlimmerComponentWithDeprecatedParentView {<template>{{~#if @tagName~}}
  {{!--
  ...
  <this.wrapperComponent @tagName={{@tagName}}>
    {{~#each (this.getConnectors) as |c|~}}
      ...
      {{~else~}}
        <PluginConnector @connector={{c}} @args={{this.outletArgs}} @deprecatedArgs={{@deprecatedArgs}} @outletArgs={{this.outletArgsWithDeprecations}} @tagName={{or @connectorTagName ""}} @layout={{c.template}} class={{c.classicClassNames}} />
      {{~/if~}}
    {{~/each~}}
...
{{~else if (this.connectorsExist hasBlock=(has-block))~}}
  ...
  {{~#if (has-block)~}}
    <PluginOutlet @name={{concat @name "__before"}} @outletArgs={{this.outletArgsWithDeprecations}} />
  {{~/if~}}

  {{~#each (this.getConnectors hasBlock=(has-block)) as |c|~}}
    {{~#if c.componentClass~}}
      <c.componentClass @outletArgs={{this.outletArgsWithDeprecations}}>{{yield}}</c.componentClass>
    {{~else if @defaultGlimmer~}}
      <c.templateOnly @outletArgs={{this.outletArgsWithDeprecations}}>{{yield}}</c.templateOnly>
    {{~else~}}
      <PluginConnector @connector={{c}} @args={{this.outletArgs}} @deprecatedArgs={{@deprecatedArgs}} @outletArgs={{this.outletArgsWithDeprecations}} @tagName={{or @connectorTagName ""}} @layout={{c.template}} class={{c.classicClassNames}}>{{yield}}</PluginConnector>
    {{~/if~}}
  {{~else~}}
    {{yield}}
  {{~/each~}}
  ...
  {{~#if (has-block)~}}
    <PluginOutlet @name={{concat @name "__after"}} @outletArgs={{this.outletArgsWithDeprecations}} />
  {{~/if~}}
{{~else~}}
  {{yield}}
{{~/if~}}</template>
