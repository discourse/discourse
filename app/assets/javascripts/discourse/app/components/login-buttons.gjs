...
<template>{{#each this.buttons as |b|}}
  <button type="button" class="btn btn-social {{b.name}}" {{on "click" (action this.externalLogin b)}} aria-label={{b.screenReaderTitle}}>
    {{#if b.isGoogle}}
      <GoogleIcon />
    {{else if b.icon}}
      {{icon b.icon}}
...
<PluginOutlet @name="after-login-buttons" /></template>}
