...
export default class SubcategoriesWithFeaturedTopics extends Component {<template>{{#each this.categories as |category|}}
  {{#if this.site.mobileView}}
    <PluginOutlet @name="mobile-subcategories-with-featured-topics-list" @outletArgs={{hash category=category}}>
      <div class="category-list subcategory-list with-topics">
        <div class="parent-category">
          <CategoryTitleLink @category={{category}} />
          <span class="stat" title={{category.statTitle}}>{{htmlSafe category.stat}}</span>
        </div>
  ...
  {{else}}
    <PluginOutlet @name="subcategories-with-featured-topics-list" @outletArgs={{hash category=category}}>
