    ...
    {{#if this.filteredCategories}}
      {{#if this.site.mobileView}}
        <div class="category-list {{if this.showTopics "with-topics"}}">
          <PluginOutlet @name="mobile-categories" @outletArgs={{hash categories=this.filteredCategories}}>
      ...
      {{else}}
        <table class="category-list {{if this.showTopics "with-topics"}}">
          <thead>
            <tr>
              <th class="category"><span role="heading" aria-level="2" id="categories-only-category">{{iN "categories.category"}}</span></th>
              <th class="topics">{{iN "categories.topics"}}</th>
              {{#if this.showTopics}}
                <th class="latest">{{iN "categories.latest"}}</th>
              {{/if}}
            </tr>
          </thead>
    ...
    {{#if this.mutedCategories}}
      <div class="muted-categories">
        <a href class="muted-categories-link" {{on "click" this.toggleShowMuted}}>
          <h3 class="muted-categories-heading">{{iN "categories.muted"}}</h3>
        ...
        {{#if this.site.mobileView}}
          <div class="category-list
            ...
            {{#each this.mutedCategories as |c|}}
              <ParentCategoryRow @category={{c}} @showTopics={{this.showTopics}} @listType="muted" />
            {{/each}}
          </div>
        {{else}}
          <table class="category-list
            ...
            <thead>
              <tr>
                <th class="category"><span role="heading" aria-level="2" id="categories-only-category-muted">{{iN "categories.category"}}</span></th>
                <th class="topics">{{iN "categories.topics"}}</th>
                {{#if this.showTopics}}
                  <th class="latest">{{iN "categories.latest"}}</th>
                {{/if}}
...
<PluginOutlet @name="below-categories-only" @connectorTagName="div" @outletArgs={{hash categories=this.categories showTopics=this.showTopics}} /></template>
  showMuted = false;
