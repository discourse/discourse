...
<template><EmailGroupUserChooser @id="private-message-users" @value={{this.splitRecipients}} @onChange={{action "updateRecipients"}} @options={{hash topicId=this.topicId none="composer.users_placeholder" includeMessageableGroups=true allowEmails=this.currentUser.can_send_private_email_messages autoWrap=true}} /></template>}
