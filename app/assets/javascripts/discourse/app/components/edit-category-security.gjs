  ...
  {{#unless this.category.is_special}}
    <div class="category-permissions-table">
      ...
      {{#each this.category.permissions as |p|}}
        <CategoryPermissionRow @groupName={{p.group_name}} @type={{p.permission_type}} @category={{this.category}} @everyonePermission={{this.everyonePermission}} @onChangeEveryonePermission={{action "onChangeEveryonePermission"}} />
      {{/each}}
      ...
      {{#if this.category.availableGroups}}
        <div class="add-group">
          <span class="group-name">
            <ComboBox @content={{this.category.availableGroups}} @onChange={{action "onSelectGroup"}} @value={{null}} @valueProperty={{null}} @nameProperty={{null}} @options={{hash none="category.security_add_group"}} class="available-groups" />
          </span>
        </div>
      {{/if}}
    </div>
...
<section>
  <PluginOutlet @name="category-custom-security" @outletArgs={{hash category=this.category}} />
</section></template>}
