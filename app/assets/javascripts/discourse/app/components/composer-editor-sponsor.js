...
export default Component.extend(ComposerUploadUppy, {
  ...
  composerModelContentKey: "replySponsor",
  ...
  replyPlaceholder(requiredCategoryMissing) {
    if (requiredCategoryMissing) {
      return "composer.reply_sponsor_placeholder_choose_category";
    } else {
      const key = authorizesOneOrMoreImageExtensions(
        this.currentUser.staff,
        this.siteSettings
      )
        ? "reply_optional_placeholder"
        : "reply_optional_placeholder_no_images";
      return `composer.${key}`;
    }
  },
  ...
  _afterMentionComplete(value) {
    this.composer.set("replySponsor", value);
  ...
  @on("didInsertElement")
  _composerEditorInit() {
    this.toggleProperty('isTextBoxVisible')
  ...
  @discourseComputed(
    "composer.replySponsor",
    "composer.replySponsorLength",
  ...
  _handleImageScaleButtonClick(event) {
    ...
    const matchingPlaceholder =
      this.get("composer.replySponsor").match(IMAGE_MARKDOWN_REGEX);
  ...
  actions: {
    ...
    showTextBox() {
      console.log(`is textbox visible: ${this.isTextBoxVisible}`)
      ...
      if (currentHeight == 'var(--new-topic-composer-height, 500px)' 
          || currentHeight == '500px') {
			...
			if (currentHeight == '800px' && this.isTextBoxVisible) {
				document.documentElement.style.setProperty(
					"--composer-height",
					'600px'
				);
