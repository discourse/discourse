...
export default RouteTemplate(<template>{{#if @controller.canChangePassword}}
  <div class="control-group pref-password" data-setting-name="user-password">
    ...
    <div class="controls">
      <a href {{on "click" @controller.changePassword}} class="btn btn-default">
        {{icon "envelope"}}
  ...
  {{#if @controller.isCurrentUser}}
    <div class="control-group pref-second-factor" data-setting-name="user-second-factor">
      ...
      <div class="controls pref-second-factor">
        <DButton @action={{@controller.manage2FA}} @icon="lock" @label="user.second_factor.enable" class="btn-default btn-second-factor" />
      </div>
    </div>
  {{/if}}
{{/if}}
...
{{#if @controller.canCheckEmails}}
  <div class="control-group pref-auth-tokens" data-setting-name="user-auth-tokens">
      ...
      {{#each @controller.authTokens as |token|}}
        <div class="row auth-token">
          ...
          <div class="auth-token-first">
            {{htmlSafe (i18n "user.auth_tokens.device_location" device=token.device ip=token.client_ip location=token.location)}}
          </div>
          <div class="auth-token-second">
            {{#if token.is_active}}
              {{htmlSafe (i18n "user.auth_tokens.browser_active" browser=token.browser)}}
            {{else}}
              {{htmlSafe (i18n "user.auth_tokens.browser_last_seen" browser=token.browser date=(formatDate token.seen_at))}}
            {{/if}}
          </div>
        </div>
      {{/each}}
    ...
    {{#if @controller.canShowAllAuthTokens}}
      <a href {{on "click" @controller.toggleShowAllAuthTokens}}>
        ...
        {{else}}
          {{icon "caret-down"}}
          <span>
            {{i18n "user.auth_tokens.show_all" count=@controller.model.user_auth_tokens.length}}
          </span>
        {{/if}}
      </a>
    {{/if}}

    <a href {{on "click" (fn @controller.revokeAuthToken null)}} class="pull-right text-danger">
...
<span>
  <PluginOutlet @name="user-preferences-security" @connectorTagName="div" @outletArgs={{hash model=@controller.model save=(action "save")}} />
</span>
