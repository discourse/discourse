      ...
      {{#each @controller.authTokens as |token|}}
        <div class="row auth-token">
          ...
          <div class="auth-token-first">
            {{htmlSafe (iN "user.auth_tokens.device_location" device=token.device ip=token.client_ip location=token.location)}}
          </div>
          <div class="auth-token-second">
            {{#if token.is_active}}
              {{htmlSafe (iN "user.auth_tokens.browser_active" browser=token.browser)}}
            {{else}}
              {{htmlSafe (iN "user.auth_tokens.browser_last_seen" browser=token.browser date=(formatDate token.seen_at))}}
            {{/if}}
          </div>
        </div>
      {{/each}}
    ...
    {{#if @controller.canShowAllAuthTokens}}
      <a href {{on "click" @controller.toggleShowAllAuthTokens}}>
        ...
        {{else}}
          {{dIcon "caret-down"}}
          <span>
            {{iN "user.auth_tokens.show_all" count=@controller.model.user_auth_tokens.length}}
          </span>
        {{/if}}
      </a>
    {{/if}}
