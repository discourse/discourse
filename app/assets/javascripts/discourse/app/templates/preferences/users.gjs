...
export default RouteTemplate(<template><label class="control-label">{{i18n "user.users"}}</label>
...
{{#if @controller.model.can_ignore_users}}
  <div class="control-group user-ignore" data-setting-name="user-ignored-users">
    <div class="controls tracking-controls user-notifications">
      ...
      <IgnoredUserList @model={{@controller.model}} @items={{@controller.model.ignored_usernames}} />
    </div>
  </div>
{{/if}}
...
{{#if @controller.model.can_mute_users}}
  <div class="control-group user-mute" data-setting-name="user-muted-users">
    <div class="controls tracking-controls">
      ...
      <UserChooser @value={{@controller.mutedUsernames}} @onChange={{action "onChangeMutedUsernames"}} @options={{hash excludeCurrentUser=true}} />
    </div>
...
{{#if @controller.model.can_send_private_messages}}
  <div class="control-group private-messages">
    ...
    <div class="control-group user-allow-pm" data-setting-name="user-allow-private-messages">
      <div class="controls">
        <PreferenceCheckbox @labelKey="user.allow_private_messages" @checked={{@controller.model.user_option.allow_private_messages}} />
      </div>
    </div>

    <div class="control-group user-allow-pm" data-setting-name="user-allow-private-messages-from-specific-users">
      <div class="controls">
        <PreferenceCheckbox @labelKey="user.allow_private_messages_from_specific_users" @checked={{@controller.model.user_option.enable_allowed_pm_users}} @disabled={{@controller.disableAllowPmUsersSetting}} />
      </div>
      {{#if @controller.allowPmUsersEnabled}}
        <div class="controls tracking-controls">
          <UserChooser @value={{@controller.allowedPmUsernames}} @onChange={{action "onChangeAllowedPmUsernames"}} @options={{hash excludeCurrentUser=true}} />
        </div>
