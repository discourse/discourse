...
export default RouteTemplate(<template><div class="control-group pref-username" data-setting-name="user-username">
...
{{#unless @controller.siteSettings.discourse_connect_overrides_avatar}}
  <div class="control-group pref-avatar" data-setting-name="user-avatar">
    ...
    <input type="hidden" id="user-avatar-uploads" data-custom-avatar-upload-id={{@controller.model.custom_avatar_upload_id}} data-system-avatar-upload-id={{@controller.model.system_avatar_upload_id}} />
    <div class="controls">
      {{!-- we want the "huge" version even though we're downsizing it in CSS --}}
      {{boundAvatar @controller.model "huge"}}
      <DButton @action={{fn (routeAction "showAvatarSelector") @controller.model}} @icon="pencil" id="edit-avatar" class="btn-default pad-left" />
    </div>
  </div>
{{/unless}}
...
{{#if @controller.canCheckEmails}}
  <div class="control-group pref-email" data-setting-name="user-email">
    ...
    {{#if @controller.model.email}}
            ...
            <div class="row email">
              ...
              <div class="email-second">
                ...
                {{#unless email.confirmed}}
                  ...
                  {{else}}
                    <button type="button" class="resend-email-confirmation" {{on "click" (fn @controller.resendConfirmationEmail email)}}>
      ...
      {{else}}
        <div class="controls">
          ...
          {{#if @controller.model.can_edit_email}}
            <LinkTo @route="preferences.email" class="btn btn-default btn-small btn-icon pad-left no-text">
              {{icon "pencil"}}
            </LinkTo>
          {{/if}}
        </div>
    ...
    {{else}}
      <div class="controls">
        <DButton @action={{fn (routeAction "checkEmail") @controller.model}} @title="admin.users.check_email.title" @icon="envelope" @label="admin.users.check_email.text" class="btn-default" />
      </div>
    {{/if}}
  </div>
{{/if}}
...
{{#if @controller.canUpdateAssociatedAccounts}}
  <div class="control-group pref-associated-accounts" data-setting-name="user-associated-accounts">
              ...
              <tr class="{{dasherize authProvider.method.name}} account-connected">
                ...
                <td>
                  {{#if authProvider.method.can_revoke}}
                    <DButton @action={{fn (action "revokeAccount") authProvider.account}} @title="user.associated_accounts.revoke" @icon="trash-can" @disabled={{get @controller.revoking authProvider.method.name}} class="btn-danger no-text" />
                  {{/if}}
                </td>
              </tr>
            {{else}}
              <tr class={{dasherize authProvider.method.name}}>
                ...
                <td colspan="2">
                  {{#if authProvider.method.can_connect}}
                    <DButton @action={{fn (action "connectAccount") authProvider.method}} @label="user.associated_accounts.connect" @icon="plug" @disabled={{@controller.disableConnectButtons}} class="btn-default" />
                  {{else}}
    ...
    {{else}}
      <div class="controls">
        <DButton @action={{fn (routeAction "checkEmail") @controller.model}} @title="admin.users.check_email.title" @icon="envelope" @label="admin.users.check_email.text" />
      </div>
    {{/if}}
  </div>
{{/if}}
  ...
  <div class="control-group pref-name" data-setting-name="user-name">
    ...
    <div class="controls">
      {{#if @controller.model.can_edit_name}}
        <TextField @value={{@controller.newNameInput}} @classNames="input-xxlarge" @maxlength="255" />
      {{else}}
...
{{#if @controller.canSelectTitle}}
  <div class="control-group pref-title" data-setting-name="user-title">
    ...
    <div class="controls">
      <ComboBox @value={{@controller.newTitleInput}} @content={{@controller.model.availableTitles}} @onChange={{fn (mut @controller.newTitleInput)}} @options={{hash none="user.title.none"}} />
    </div>
    <div class="instructions">
...
{{#if @controller.canSelectFlair}}
  <div class="control-group pref-flair" data-setting-name="user-flair">
    ...
    <div class="controls">
      <FlairChooser @value={{@controller.newFlairGroupId}} @content={{@controller.model.availableFlairs}} @onChange={{fn (mut @controller.newFlairGroupId)}} @options={{hash none="user.flair.none"}} />
    </div>
    <div class="instructions">
  ...
  <div class="control-group pref-user-status" data-setting-name="user-status">
    ...
    <div class="controls">
      {{#if @controller.newStatus}}
        <UserStatusMessage @status={{@controller.newStatus}} @showDescription={{true}} />
      {{else}}
        <span class="static">{{i18n "user.status.not_set"}}</span>
      {{/if}}
      <DButton @action={{fn (action "showUserStatusModal") @controller.newStatus}} @icon="pencil" class="btn-default btn-small pad-left" />
    </div>
  </div>
{{/if}}

{{#if @controller.canSelectPrimaryGroup}}
  <div class="control-group pref-primary-group" data-setting-name="user-primary-group">
    <label class="control-label">{{i18n "user.primary_group.title"}}</label>
    <div class="controls">
      <ComboBox @value={{@controller.newPrimaryGroupInput}} @content={{@controller.model.filteredGroups}} @options={{hash none="user.primary_group.none"}} />
    </div>
  </div>
{{/if}}

{{#if @controller.canDownloadPosts}}
  <div class="control-group pref-data-export" data-setting-name="user-data-export">
    <label class="control-label">{{i18n "user.download_archive.title"}}</label>
    <div class="controls">
      <DButton @action={{action "exportUserArchive"}} @label="user.download_archive.button_text" @icon="download" class="btn-default btn-request-archive" />
    </div>
    <div class="instructions">
...
<span>
  <PluginOutlet @name="user-preferences-account" @connectorTagName="div" @outletArgs={{hash model=@controller.model save=(action "save")}} />
</span>
...
{{#if @controller.model.canDeleteAccount}}
  <div class="control-group delete-account">
    <br />
    <div class="controls">
      <DButton @action={{action "delete"}} @disabled={{@controller.deleteDisabled}} @icon="trash-can" @label="user.delete_account" class="btn-danger" />
    </div>
  </div>
{{/if}}</template>)