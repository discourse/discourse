  ...
  <ConditionalLoadingSpinner @condition={{@controller.loading}}>
    <form class="form-vertical">
      ...
      {{#if @controller.displayOAuthWarning}}
        <div class="alert alert-warning">{{i18n "user.second_factor.oauth_enabled_warning"}}</div>
      {{/if}}
            ...
            {{#if @controller.model.second_factor_enabled}}
              <div class="details">
                {{#if @controller.model.second_factor_backup_enabled}}
                  {{htmlSafe (i18n "user.second_factor_backup.manage" count=@controller.model.second_factor_remaining_backup_codes)}}
                {{else}}
