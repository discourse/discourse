...
{{#if @controller.canInviteToForum}}
  <LoadMore @id="user-content" @selector=".user-invite-list tr" @action={{action "loadMore"}} class="user-content">
    <section class="user-additional-controls">
      {{#if @controller.showSearch}}
        <div class="user-invite-search">
          <form><TextField @value={{@controller.searchTerm}} @placeholderKey="user.invited.search" /></form>
        </div>
      {{/if}}
      <section class="user-invite-buttons">
        <DButton @icon="plus" @action={{@controller.createInvite}} @label="user.invited.create" class="btn-default invite-button" />
        {{#if @controller.canBulkInvite}}
          {{#if @controller.siteSettings.allow_bulk_invite}}
            {{#if @controller.site.desktopView}}
              <DButton @icon="upload" @action={{@controller.createInviteCsv}} @label="user.invited.bulk_invite.text" class="btn-default" />
            {{/if}}
          {{/if}}
        {{/if}}
        {{#if @controller.showBulkActionButtons}}
          {{#if @controller.inviteExpired}}
            ...
            {{else}}
              <DButton @icon="xmark" @action={{@controller.destroyAllExpired}} @label="user.invited.remove_all" />
            {{/if}}
          {{/if}}
          ...
          {{#if @controller.invitePending}}
            {{#if @controller.reinvitedAll}}
              <span class="reinvited-all">
                <DButton @icon="check" @disabled={{true}} @label="user.invited.reinvited_all" />
              </span>
            {{else if @controller.hasEmailInvites}}
              <DButton @icon="arrows-rotate" @action={{@controller.reinviteAll}} @label="user.invited.reinvite_all" class="btn-default" />
            {{/if}}
          {{/if}}
        {{/if}}
      </section>
    ...
    <section>
      {{#if @controller.model.invites}}
          ...
          <table class="table user-invite-list">
            ...
            <tbody>
              {{#each @controller.model.invites as |invite|}}
                <tr>
                  <td>
                    <LinkTo @route="user" @model={{invite.user}}>{{avatar invite.user imageSize="tiny"}}</LinkTo>
                    <LinkTo @route="user" @model={{invite.user}}>{{invite.user.username}}</LinkTo>
                  </td>
                  ...
                  {{#if @controller.model.can_see_invite_details}}
                    ...
                    <td>
                      <span title={{iN "user.invited.days_visited"}}>{{htmlSafe invite.user.days_visited}}</span>
                      /
                      <span title={{iN "user.invited.account_age_days"}}>{{htmlSafe invite.user.days_since_created}}</span>
                    </td>
              ...
              {{#each @controller.model.invites as |invite|}}
                <tr>
                  <td class="invite-type">
                    ...
                    {{else}}
                      {{dIcon "link"}}
                      {{iN "user.invited.invited_via_link" key=invite.shortKey count=invite.redemption_count max=invite.max_redemptions_allowed}}
                    {{/if}}

                    {{#each invite.groups as |g|}}
                      <p class="invite-extra"><a href="/g/{{g.name}}">{{dIcon "users"}}
                          {{g.name}}</a></p>
                    {{/each}}

                    {{#if invite.topic}}
                      <p class="invite-extra"><a href={{invite.topic.url}}>{{dIcon "file"}} {{invite.topic.title}}</a></p>
                    {{/if}}
                  </td>
                  ...
                  {{#if invite.can_delete_invite}}
                    <td class="invite-actions">
                      <DButton @icon="pencil" @action={{fn @controller.editInvite invite}} @title="user.invited.edit" class="btn-default" />
                      <DButton @icon="trash-can" @action={{fn @controller.destroyInvite invite}} @title={{if invite.destroyed "user.invited.removed" "user.invited.remove"}} class="cancel" />
                    </td>
                  {{/if}}
                </tr>
              {{/each}}
        ...
        <ConditionalLoadingSpinner @condition={{@controller.invitesLoading}} />
      {{else}}
