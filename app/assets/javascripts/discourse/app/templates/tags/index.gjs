...
<div class="container tags-index">
  ...
  <div class="tag-sort-options">
    ...
    <span class="tag-sort-count {{if @controller.sortedByCount "active"}}"><a href {{on "click" @controller.sortByCount}}>{{i18n "tagging.sort_by_count"}}</a></span>
    <span class="tag-sort-name {{if @controller.sortedByName "active"}}"><a href {{on "click" @controller.sortById}}>{{i18n "tagging.sort_by_name"}}</a></span>
  </div>
