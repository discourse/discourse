...
export default RouteTemplate(<template><section class="user-secondary-navigation">
  <HorizontalOverflowNav class="activity-nav">
    {{#if @controller.model.can_see_members}}
      <GroupActivityFilter @filter="posts" @categoryId={{@controller.category_id}} />
      <GroupActivityFilter @filter="topics" @categoryId={{@controller.category_id}} />
    {{/if}}
    {{#if @controller.siteSettings.enable_mentions}}
      <GroupActivityFilter @filter="mentions" @categoryId={{@controller.category_id}} />
    {{/if}}
