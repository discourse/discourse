...
<ConditionalLoadingSpinner @condition={{@controller.loading}}>
  ...
  {{else if @controller.userDoesNotHaveBookmarks}}
    <EmptyState @title={{i18n "user.no_bookmarks_title"}} @body={{@controller.emptyStateBody}} />
  {{else}}
    <div class="inline-form full-width bookmark-search-form">
      <Input @type="text" @value={{@controller.searchTerm}} placeholder={{i18n "bookmarks.search_placeholder"}} @enter={{@controller.search}} id="bookmark-search" autocomplete="off" />
