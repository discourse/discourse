...
{{#unless @controller.loadError}}
  ...
  {{#if @controller.showSecurityKeyForm}}
    <div id="security-key">
      <DButton @action={{@controller.authenticateSecurityKey}} @icon="key" @label="login.security_key_authenticate" id="security-key-authenticate-button" class="btn-large btn-primary" />
    </div>
  {{else if (or @controller.showTotpForm @controller.showBackupCodesForm)}}
    <form class={{@controller.inputFormClass}}>
      <SecondFactorInput {{on "input" (withEventValue (fn (mut @controller.secondFactorToken)))}} @secondFactorMethod={{@controller.shownSecondFactorMethod}} value={{@controller.secondFactorToken}} />
      <DButton @action={{@controller.authenticateToken}} @label="submit" type="submit" class="btn-primary" />
    </form>
  {{/if}}
  ...
  {{#if @controller.alternativeMethods.length}}
    <p>
      {{#each @controller.alternativeMethods as |method index|}}
        ...
        {{/if}}
        <span>
          <a href class="toggle-second-factor-method {{method.class}}" {{on "click" (fn @controller.useAnotherMethod method.id)}}>
            {{iN method.translationKey}}
          </a>
        </span>
      {{/each}}
    </p>
