    ...
    {{else}}
      ...
      <div class="control-group">
        <label class="control-label">
          {{i18n (if @controller.new "user.add_email.title" "user.change_email.title")}}
        </label>
        <div class="controls">
          <TextField @value={{@controller.newEmail}} @id="change-email" @classNames="input-xxlarge" @autofocus="autofocus" />
      ...
      <div class="controls save-button">
        <DButton @action={{action "saveEmail"}} @disabled={{@controller.saveDisabled}} @translatedLabel={{@controller.saveButtonText}} type="submit" class="btn-primary" />
        <LinkTo @route="preferences.account" @model={{@controller.model.username}} class="cancel">
          {{i18n "cancel"}}
        </LinkTo>
      </div>
    {{/if}}
