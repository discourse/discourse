...
class Notification < ActiveRecord::Base
  ...
  before_create do
    ...
    self.high_priority =
      self.high_priority ||
        Notification.high_priority_types.include?(self.notification_type)
  end
  ...
  def self.consolidate_or_create!(notification_params)
    ...
    consolidated_notification =
      consolidation_planner.consolidate_or_save!(notification)

    if consolidated_notification == :no_plan
  ...
  def self.normal_priority_types
    @normal_priority_types ||=
      types.reject { |_k, v| high_priority_types.include?(v) }.values
  end
