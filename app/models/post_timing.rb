...
class PostTiming < ActiveRecord::Base
  ...
  def self.pretend_read(
    topic_id,
    actual_read_post_number,
    pretend_read_post_number
  )
  ...
  def self.record_new_timing(args)
    ...
    if Topic.exists?(id: args[:topic_id], archetype: Archetype.private_message)
      return
    end
    UserStat.where(
  ...
  def self.set_minimum_first_unread_pm!(topic:, user_id:, date:)
    if topic.topic_allowed_users.exists?(user_id: user_id)
      UserStat.where(
        'first_unread_pm_at > ? AND user_id = ?',
        date,
        user_id
  ...
  def self.process_timings(
    ...
    max_time_per_post = MAX_READ_TIME_PER_BATCH if max_time_per_post >
