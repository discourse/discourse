...
class PostTiming < ActiveRecord::Base
  ...
  class << self
    def pretend_read(topic_id, actual_read_post_number, pretend_read_post_number, user_ids = nil)
    ...
    def destroy_last_for(user, topic_id: nil, topic: nil)
    ...
    def destroy_for(user_id, topic_ids)
      PostTiming.transaction do
    ...
    def set_minimum_first_unread_pm!(topic:, user_id:, date:)
    ...
    def set_minimum_first_unread!(user_id:, date:)
...
SQL
      ...
      if timings.length > 0
        total_changed =
          Notification.mark_posts_read(current_user, topic_id, timings.map { |t| t[0] })
      end
