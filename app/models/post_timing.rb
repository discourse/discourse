...
class PostTiming < ActiveRecord::Base
  ...
  def self.pretend_read(
    topic_id, actual_read_post_number, pretend_read_post_number
  )
  ...
  def self.record_new_timing(args)
    ...
    ).update_all 'posts_read_count = posts_read_count + 1'
  end
  ...
  def self.destroy_last_for(user, topic_id)
    ...
    post_number =
      user.staff? ? topic.highest_staff_post_number : topic.highest_post_number
    ...
    PostTiming.transaction do
      ...
      TopicUser.where(user_id: user.id, topic_id: topic.id).update_all(
        highest_seen_post_number: last_read, last_read_post_number: last_read
      )
    end
  end
  ...
  def self.destroy_for(user_id, topic_ids)
    PostTiming.transaction do
      PostTiming.where('user_id = ? and topic_id in (?)', user_id, topic_ids)
        .delete_all

      TopicUser.where('user_id = ? and topic_id in (?)', user_id, topic_ids)
        .delete_all
    end
  end
  ...
  def self.process_timings(
    current_user, topic_id, topic_time, timings, opts = {}
  )
    UserStat.update_time_read!(current_user.id)

    max_time_per_post = ((Time.now - current_user.created_at) * 1000.0)
    if max_time_per_post > MAX_READ_TIME_PER_BATCH
      max_time_per_post = MAX_READ_TIME_PER_BATCH
    end
    ...
    timings.each_with_index do |(post_number, time), index|
      join_table <<
        "SELECT #{topic_id.to_i} topic_id, #{post_number
          .to_i} post_number,
                     #{current_user.id
          .to_i} user_id, #{time.to_i} msecs, #{index} idx"

      highest_seen =
        post_number.to_i > highest_seen ? post_number.to_i : highest_seen
    end
