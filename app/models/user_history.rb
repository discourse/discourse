...
class UserHistory < ActiveRecord::Base
  ...
  def self.with_filters(filters)
    query = self
    query = query.where(action: filters[:action_id]) if filters[
      :action_id
    ].present?
    query = query.where(custom_type: filters[:custom_type]) if filters[
      :custom_type
    ].present?
    ...
    %i[acting_user target_user].each do |key|
      if filters[key] &&
           (
             obj_id =
               User.where(username_lower: filters[key].downcase).pluck(:id)
           )
  ...
  def self.for(user, action_type)
    self.where(
      ...
      action: UserHistory.actions[action_type]
    )
  end
  ...
  def self.staff_action_records(viewer, opts = nil)
    ...
    if custom_staff
      ...
      opts[:action_id] = self.actions[opts[:action_name].to_sym] if opts[
        :action_name
      ]
    end
  ...
  def new_value_is_json?
    [
      UserHistory.actions[:change_theme],
      UserHistory.actions[:delete_theme]
    ].include?(action)
  end
