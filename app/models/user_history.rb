...
class UserHistory < ActiveRecord::Base
  ...
  class << self
    ...
    def staff_action_ids
      @staff_action_ids ||= staff_actions.map { |a| actions[a] }
    end

    def moderator_visible_actions
    ...
    def for(user, action_type)
      where(target_user_id: user.id, action: UserHistory.actions[action_type])
    end

    def exists_for_user?(user, action_type, opts = nil)
      opts = opts || {}
    ...
    def staff_action_records(viewer, opts = nil)
      opts ||= {}
