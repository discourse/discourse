...
module ReviewableActionBuilder
  ...
  def reject_all_related_reviewables(performed_by:, extra_delete_options: {})
    ...
    reviewable_ids = [id]
    Reviewable.where(target_created_by_id: user.id).where.not(id: id).find_each do |reviewable|
      reviewable_ids << reviewable.id
      reviewable.update!(target_created_by_id: nil)
      reviewable.perform(performed_by, :reject_due_to_user_deletion, skip_validate_action: true)
    end

    create_result(:success, :rejected) { |result| result.remove_reviewable_ids = reviewable_ids }
  end
