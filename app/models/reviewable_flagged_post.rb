...
class ReviewableFlaggedPost < Reviewable
  ...
  def perform_ignore(performed_by, args)
    ...
    actions.each do |action|
      ...
      action.save
      unless args[:expired]
        action.add_moderator_post_if_needed(
          performed_by,
          :ignored,
          args[:post_was_deleted]
        )
      end
    end
  ...
  def perform_disagree(performed_by, args)
    ...
    action_type_ids.each do |atid|
      ...
      cached[column] = 0 if ActiveRecord::Base.connection.column_exists?(
        :posts,
        column
      )
    end
    ...
    actions.each do |action|
      ActiveRecord::Base.transaction do
        ...
        DB.after_commit do
          ...
          trigger_spam = true if action.post_action_type_id ==
