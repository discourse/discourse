...
class TrustLevel3Requirements
  ...
  def days_visited
    @user.user_visits.where(
      'visited_at > ? and posts_read > 0',
  ...
  def num_topics_replied_to
    @user.posts.select('distinct topic_id').where(
      'created_at > ? AND post_number > 1',
      time_period.days.ago
    )
      .count
  end
  ...
  def posts_read
    @user.user_visits.where('visited_at > ?', time_period.days.ago).pluck(
      :posts_read
    )
      .sum
  end
  ...
  def num_flagged_posts
    PostAction.with_deleted.where(post_id: flagged_post_ids).where.not(
      user_id: @user.id
    )
      .where
      .not(agreed_at: nil)
  ...
  def num_likes_given
    UserAction.where(user_id: @user.id, action_type: UserAction::LIKE).where(
      'created_at > ?',
      time_period.days.ago
    )
      .count
  ...
  def num_likes_received_query
    UserAction.where(user_id: @user.id, action_type: UserAction::WAS_LIKED)
      .where('created_at > ?', time_period.days.ago)
  end
  ...
  def self.num_topics_in_time_period
    $redis.get(NUM_TOPICS_KEY) ||
      begin
        count =
          Topic.listable_topics.visible.created_since(
            SiteSetting.tl3_time_period.days.ago
          )
            .count
  ...
  def self.num_posts_in_time_period
    $redis.get(NUM_POSTS_KEY) ||
      begin
        count =
          Post.public_posts.visible.created_since(
            SiteSetting.tl3_time_period.days.ago
          )
            .count
