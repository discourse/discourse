...
class TopicThumbnail < ActiveRecord::Base
  ...
  def self.find_or_create_for!(original, max_width:, max_height:)
    ...
    if target_width < original.width && target_height < original.height
      optimized =
        OptimizedImage.create_for(original, target_width, target_height)
    end
  ...
  def self.ensure_consistency!
    ...
    TopicThumbnail
      ...
      .joins(
        'LEFT JOIN optimized_images on optimized_image_id = optimized_images.id'
      )
      .where(<<~SQL)
    ...
    sizes =
      Topic.thumbnail_sizes +
        ThemeModifierHelper.new(
          theme_ids: Theme.pluck(:id)
        ).topic_thumbnail_sizes
    sizes_sql =
      sizes
        .map { |s| "(max_width = #{s[0].to_i} AND max_height = #{s[1].to_i})" }
        .join(' OR ')
