...
class RemoteTheme < ActiveRecord::Base
  ...
  scope :joined_remotes,
        -> {
          joins(
            'JOIN themes ON themes.remote_theme_id = remote_themes.id'
  ...
  def self.update_zipped_theme(
    ...
    theme.child_components =
      child_components = theme_info['components'].presence || []
    ...
    if existing && update_components.present? && update_components != 'none'
      child_components =
        child_components.map do |url|
          ThemeStore::GitImporter.new(url.strip).url
        end
  ...
  def self.unreachable_themes
    self
      .joined_remotes
      .where('last_error_text IS NOT NULL')
      .pluck('themes.name', 'themes.id')
  end
  ...
  def update_from_remote(importer = nil, skip_update: false)
    ...
    field_ids_to_destroy =
      theme.theme_fields.pluck(:id) - updated_fields.map { |tf| tf&.id }
  ...
  def update_theme_color_schemes(theme, schemes)
    ...
    schemes&.each do |name, colors|
      ...
      scheme =
        theme.color_schemes.find_by(name: name) ||
          theme.color_schemes.build(name: name)
      ...
      ColorScheme.base.colors_hashes.each do |color|
        ...
        if color_scheme_color.hex_changed?
      ...
      ColorScheme.color_transformation_variables.each do |variable_name|
        ...
        color_scheme_color =
          scheme.color_scheme_colors.to_a.find { |c| c.name == variable_name }
        if override
          color_scheme_color ||=
            scheme.color_scheme_colors.build(name: variable_name)
          color_scheme_color.hex = override
          if color_scheme_color.hex_changed?
            theme.notify_color_change(color_scheme_color)
          end
        elsif color_scheme_color
          # No longer specified in about.json, delete record
