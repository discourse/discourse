...
class RemoteTheme < ActiveRecord::Base
  ...
  has_one :theme
  scope :joined_remotes, lambda do
  ...
  validates_format_of :minimum_discourse_version,
                      :maximum_discourse_version,
                      with: Discourse::VERSION_REGEXP, allow_nil: true
  ...
  def self.update_tgz_theme(
    filename, match_theme: false, user: Discourse.system_user, theme_id: nil
  )
  ...
  def self.import_theme(
    url, user = Discourse.system_user, private_key: nil, branch: nil
  )
    importer =
      ThemeStore::GitImporter.new(
        url.strip,
        private_key: private_key, branch: branch
      )
    importer.import!
    ...
    theme =
      Theme.new(
        user_id: user&.id || -1, name: theme_info['name'], component: component
      )
  ...
  def self.out_of_date_themes
    self.joined_remotes.where(
      'commits_behind > 0 OR remote_version <> local_version'
    )
      .pluck('themes.name', 'themes.id')
  end

  def self.unreachable_themes
    self.joined_remotes.where('last_error_text IS NOT NULL').pluck(
      'themes.name',
      'themes.id'
    )
  end
  ...
  def update_remote_version
    ...
    begin
      ...
      self.remote_version, self.commits_behind =
        importer.commits_since(local_version)
  ...
  def update_from_remote(importer = nil, skip_update: false)
    ...
    theme_info['assets']&.each do |name, relative_path|
      if path = importer.real_path(relative_path)
        new_path =
          "#{File.dirname(path)}/#{SecureRandom.hex}#{File.extname(path)}"
    ...
    schemes&.each do |name, colors|
      ...
      if existing
        ...
        ColorScheme.base.colors_hashes.each do |color|
          ...
          scheme.color_scheme_colors <<
            ColorSchemeColor.new(
              name: color[:name], hex: override || color[:hex]
            )
        end
  ...
  def github_diff_link
    if github_repo_url.present? && local_version != remote_version
      "#{github_repo_url.gsub(
        /\.git$/,
        ''
      )}/compare/#{local_version}...#{remote_version}"
    end
  end
