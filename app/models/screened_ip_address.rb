...
class ScreenedIpAddress < ActiveRecord::Base
  ...
  class << self
    ...
    def exists_for_ip_address_and_action?(ip_address, action_type, opts = {})
    ...
    def block_admin_login?(user, ip_address)
    ...
    def subnets(family, from_masklen, to_masklen)
      sql = <<~SQL
    ...
    def roll_up(current_user = Discourse.system_user)
