class GroupHistory < ActiveRecord::Base
  ...
  def self.with_filters(group, params = {})
    records =
      self.includes(:acting_user, :target_user).where(group_id: group.id).order(
        'group_histories.created_at DESC'
      )

    if !params.blank?
      params = params.slice(*filters)
      unless params[:action].blank?
        records = records.where(action: self.actions[params[:action].to_sym])
      end
      unless params[:subject].blank?
        records = records.where(subject: params[:subject])
      end
