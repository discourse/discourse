...
class Site
  ...
  def categories
    @categories ||=
      begin
        categories =
          Category.includes(:uploaded_logo, :uploaded_background).secured(
            @guardian
          )
        ...
        categories.each do |c|
          with_children << c.parent_category_id if c.parent_category_id
        end
        ...
        unless @guardian.is_admin?
          allowed_topic_create_ids =
            if @guardian.anonymous?
              ...
              Category.topic_create_allowed(@guardian).pluck(:id)
            end
        ...
        categories.reject! do |c|
          c.parent_category_id && !by_id[c.parent_category_id]
        end
        categories
      end
  end
  ...
  def self.json_for(guardian)
    if guardian.anonymous? && SiteSetting.login_required
      return {
        ...
        user_fields:
          UserField.all.map do |userfield|
    ...
    if guardian.anonymous?
      ...
      cached_json, cached_seq, cached_version =
        $redis.mget('site_json', 'site_json_seq', 'site_json_version')

      if cached_json && seq == cached_seq.to_i &&
         Discourse.git_version == cached_version
        return cached_json
      end
    end
