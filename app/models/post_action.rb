...
class PostAction < ActiveRecord::Base
  ...
  scope :active, lambda do
    where(disagreed_at: nil, deferred_at: nil, agreed_at: nil, deleted_at: nil)
  end
  ...
  def self.flag_count_by_date(start_date, end_date, category_id = nil)
    result =
      where(
        'post_actions.created_at >= ? AND post_actions.created_at <= ?',
        start_date,
        end_date
      )
    result =
    ...
    if category_id
      result =
        result.joins(post: :topic).where('topics.category_id = ?', category_id)
    end
    result.group('date(post_actions.created_at)').order(
      'date(post_actions.created_at)'
    )
      .count
  end
  ...
  def self.apply_minimum_visibility(relation)
    ...
    params = {
      ...
      hidden_reasons:
        Post.hidden_reasons.only(:flagged_by_tl3_user, :flagged_by_tl4_user)
          .values
    }
  ...
  def self.update_flagged_posts_count
    flagged_relation =
      PostAction.active.flags.joins(post: :topic).where(
    ...
    posts_flagged_count = flagged_relation.pluck('posts.id').count

    $redis.set('posts_flagged_count', posts_flagged_count)
    user_ids = User.staff.pluck(:id)
    MessageBus.publish(
      '/flagged_counts',
      { total: posts_flagged_count },
      user_ids: user_ids
    )
  end
  ...
  def self.count_per_day_for_type(post_action_type, opts = nil)
    ...
    if opts[:end_date]
      result = result.where('post_actions.created_at <= ?', opts[:end_date])
    end
    if opts[:category_id]
      result =
        result.joins(post: :topic).merge(
  ...
  def self.agree_flags!(post, moderator, delete_post = false)
    actions =
      PostAction.active.where(post_id: post.id).where(
        post_action_type_id: PostActionType.notify_flag_types.values
      )
    ...
    actions.each do |action|
      ...
      if action.post_action_type_id == PostActionType.types[:spam]
        trigger_spam = true
      end
    end

    # Update the flags_agreed user stat
    UserStat.where(user_id: actions.map(&:user_id)).update_all(
      'flags_agreed = flags_agreed + 1'
    )
  ...
  def self.clear_flags!(post, moderator)
    ...
    actions =
      PostAction.active.where(post_id: post.id).where(
        post_action_type_id: action_type_ids
      )
    ...
    UserStat.where(user_id: actions.map(&:user_id)).update_all(
      'flags_disagreed = flags_disagreed + 1'
    )
  ...
  def self.create_message_for_post_action(user, post, post_action_type_id, opts)
    ...
    unless opts[:message] &&
           %i[notify_moderators notify_user spam].include?(post_action_type)
      return
    end

    title =
      I18n.t(
        "post_action_types.#{post_action_type}.email_title",
        title: post.topic.title, locale: SiteSetting.default_locale
      )
    body =
      I18n.t(
        "post_action_types.#{post_action_type}.email_body",
    ...
    if %i[notify_moderators spam].include?(post_action_type)
  ...
  def self.act(user, post, post_action_type_id, opts = {})
    ...
    begin
      related_post_id =
        create_message_for_post_action(user, post, post_action_type_id, opts)
    ...
    if opts[:flag_topic] && post.topic
      targets_topic = post.topic.reload.posts_count != 1
    end
    ...
    post_action =
      PostAction.where(where_attrs).with_deleted.where('deleted_at IS NOT NULL')
        .first
  ...
  def self.copy(original_post, target_post)
    ...
    SELECT #{target_post
              .id}, #{cols_to_copy}
    FROM post_actions
    WHERE post_id = #{original_post
              .id}
    SQL

    target_post.post_actions.each(&:update_counters)
  end
    ...
    %w[like flag bookmark].each do |type|
      if send("is_#{type}?")
        ...
        if is_like? && user && user.trust_level >= 2
          multiplier =
            SiteSetting.send(
              "tl#{user.trust_level}_additional_likes_per_day_multiplier"
            )
              .to_f
  ...
  before_create do
    ...
    if PostAction.where(user_id: user_id).where(post_id: post_id).where(
  ...
  def update_counters
    ...
    count =
      PostAction.where(post_id: post_id).where(
        post_action_type_id: post_action_type_id
      )
        .count
    ...
    when :like
      ...
      score =
        PostAction.joins(:user).where(post_id: post_id).sum(
          "CASE WHEN users.moderator OR users.admin THEN #{SiteSetting
            .staff_like_weight} ELSE 1 END"
        )
    ...
    # topic_user
    if %i[like bookmark].include? post_action_type_key
  ...
  def create_user_action
    UserActionCreator.log_post_action(self) if is_bookmark? || is_like?
  end

  def update_notifications
    PostActionNotifier.post_action_deleted(self) if self.deleted_at.present?
  end
  ...
  def notify_subscribers
    post.publish_change_to_clients! :acted if (is_like? || is_flag?) && post
  end
  ...
  def self.auto_close_threshold_reached?(topic)
    ...
    flags =
      PostAction.active.flags.joins(:post).where('posts.topic_id = ?', topic.id)
  ...
  def self.auto_hide_if_needed(acting_user, post, post_action_type)
    ...
    if post_action_type == :spam &&
      ...
      hide_post!(
        post,
        post_action_type,
        Post.hidden_reasons[:flagged_by_tl3_user]
      )
    elsif PostActionType.auto_action_flag_types.include?(post_action_type)
      if acting_user.has_trust_level?(TrustLevel[4]) && !acting_user.staff? &&
         post.user&.trust_level != TrustLevel[4]
        hide_post!(
          post,
          post_action_type,
          Post.hidden_reasons[:flagged_by_tl4_user]
        )
  ...
  def self.hide_post!(post, post_action_type, reason = nil)
    ...
    reason = guess_hide_reason(post) unless reason
      ...
      options = {
        ...
        flag_reason:
          I18n.t(
            ...
            locale: SiteSetting.default_locale, base_path: Discourse.base_path
          )
      }
