...
class Draft < ActiveRecord::Base
  ...
  class << self
    def set(user, key, sequence, data, owner = nil, force_save: false)
      ...
      draft_id, current_owner, current_sequence =
        DB.query_single(<<~SQL, user_id: user.id, key: key)
      WITH draft AS (
    ...
    def allowed_draft_topics_for_user(user)
      topics = Topic.listable_topics.secured(Guardian.new(user))
      pms = Topic.private_messages_for_user(user)
      topics.or(pms)
    end

    def allowed_draft_posts_for_user(user)
    ...
    def backup_draft(user, key, sequence, data)
