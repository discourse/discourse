...
class AdminDashboardData
  ...
  HTTP_REPORTS ||=
    ApplicationRequest.req_types.keys.select { |r| r =~ /^http_/ }.map do |r|
      r + '_reqs'
    end
      .sort

  USER_REPORTS ||= %w[users_by_trust_level]

  MOBILE_REPORTS ||=
    %w[mobile_visits] +
      ApplicationRequest.req_types.keys.select { |r| r =~ /mobile/ }.map do |r|
        r + '_reqs'
      end
  ...
  class << self
    attr_reader :problem_syms, :problem_blocks, :problem_messages
  end
  ...
  def problems
    problems = []
    AdminDashboardData.problem_syms.each { |sym| problems << send(sym) }
  ...
  def self.set_problems_started
    ...
    $redis.setex(
      problems_started_key,
      14.days.to_i,
      existing_time || Time.zone.now.to_s
    )
  end
  ...
  def self.problem_message_check(i18n_key)
    if $redis.get(problem_message_key(i18n_key))
  ...
  def rails_env_check
    unless Rails.env.production?
      I18n.t('dashboard.rails_env_warning', env: Rails.env)
    end
  end
  ...
  def sidekiq_check
    ...
    if Jobs.queued > 0 &&
       (last_job_performed_at.nil? || last_job_performed_at < 2.minutes.ago)
  ...
  def queue_size_check
    ...
    unless queue_size < 100_000
      I18n.t('dashboard.queue_size_warning', queue_size: queue_size)
    end
  end

  def ram_check
    if MemInfo.new.mem_total && MemInfo.new.mem_total < 1_000_000
      I18n.t('dashboard.memory_warning')
    end
  end
  ...
  def facebook_config_check
    if SiteSetting.enable_facebook_logins &&
       (
         SiteSetting.facebook_app_id.blank? ||
           SiteSetting.facebook_app_secret.blank?
       )
      I18n.t(
  ...
  def twitter_config_check
    if SiteSetting.enable_twitter_logins &&
       (
         SiteSetting.twitter_consumer_key.blank? ||
           SiteSetting.twitter_consumer_secret.blank?
       )
  ...
  def github_config_check
    if SiteSetting.enable_github_logins &&
       (
         SiteSetting.github_client_id.blank? ||
           SiteSetting.github_client_secret.blank?
       )
  ...
  def pwa_config_check
    unless SiteSetting.large_icon.present? &&
    ...
    unless SiteSetting.short_title.present? &&
           SiteSetting.short_title.size <= 12
      return I18n.t(
  ...
  def s3_config_check
    ...
    if !GlobalSetting.use_s3?
      bad_keys =
        (
          SiteSetting.s3_access_key_id.blank? ||
            SiteSetting.s3_secret_access_key.blank?
        ) &&
          !SiteSetting.s3_use_iam_profile

      if SiteSetting.enable_s3_uploads &&
         (bad_keys || SiteSetting.s3_upload_bucket.blank?)
        return I18n.t(
  ...
  def image_magick_check
    if SiteSetting.create_thumbnails &&
       !system('command -v convert >/dev/null;')
  ...
  def failing_emails_check
    ...
    if num_failed_jobs > 0
      I18n.t(
        ...
        num_failed_jobs: num_failed_jobs, base_path: Discourse.base_path
      )
    end
  end

  def subfolder_ends_in_slash_check
    I18n.t('dashboard.subfolder_ends_in_slash') if Discourse.base_uri =~ %r{\/$}
  end
  ...
  def email_polling_errored_recently
    ...
    if errors > 0
      I18n.t(
        ...
        count: errors, base_path: Discourse.base_path
      )
    end
  end
  ...
  def themes_html_format(themes, i18n_key)
    html =
      themes.map do |name, id|
        "<li><a href=\"/admin/customize/themes/#{id}\">#{CGI.escapeHTML(
          name
        )}</a></li>"
      end
        .join("\n")
