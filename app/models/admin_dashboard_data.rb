...
class AdminDashboardData
  ...
  def self.load_found_scheduled_check_problems
    ...
    begin
      ...
      Discourse.warn_exception(
        err,
        message:
          "Error parsing found problem JSON in admin dashboard: #{found_problems_json}"
      )
      []
    end
  end
    ...
    problem_scheduled_check_blocks.each do |check_identifier, blk|
      ...
      begin
        ...
        Discourse.warn_exception(
          err,
          message:
            "A scheduled admin dashboard problem check (#{check_identifier}) errored."
        )
  ...
  def self.add_problem_message(i18n_key, expire_seconds = nil)
    if expire_seconds.to_i > 0
      Discourse.redis.setex problem_message_key(i18n_key),
                            expire_seconds.to_i,
                            1
    else
  ...
  def ram_check
    if MemInfo.new.mem_total && MemInfo.new.mem_total < 950_000
  ...
  def s3_config_check
    ...
    if !GlobalSetting.use_s3?
      ...
      if SiteSetting.enable_s3_uploads &&
        ...
        return(
          I18n.t('dashboard.s3_config_warning', base_path: Discourse.base_path)
        )
      end
