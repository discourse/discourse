class IncomingLinksReport
  ...
  def self.report_top_referrers(report)
    ...
    user_id_lookup =
      User.where(username: num_clicks.keys).select(
        :id,
        :username,
        :uploaded_avatar_id
      )
        ...
        sum[v.username] = {
          ...
          user_avatar_template:
            User.avatar_template(v.username, v.uploaded_avatar_id)
        }
        sum
      end
  ...
  def self.per_user(start_date:, end_date:, category_id:)
    public_incoming_links(category_id: category_id).where(
  ...
  def self.report_top_traffic_sources(report)
    ...
    num_topics =
      topic_count_per_domain(num_clicks.keys, category_id: report.category_id)
    report.data = []
  ...
  def self.link_count_per_domain(
    limit: 10, start_date:, end_date:, category_id:
  )
    public_incoming_links(category_id: category_id).where(
  ...
  def self.per_domain(domains, options = {})
    public_incoming_links(category_id: options[:category_id]).joins(
      incoming_referer: :incoming_domain
    )
      .where(
      'incoming_links.created_at > ? AND incoming_domains.name IN (?)',
      30.days.ago,
      domains
    )
      .group('incoming_domains.name')
  ...
  def self.report_top_referred_topics(report)
    report.y_titles[:num_clicks] =
      I18n.t("reports.#{report.type}.labels.num_clicks")
    num_clicks =
    ...
    num_clicks =
      num_clicks.to_a.sort_by { |x| x[1] }.last(report.limit || 10).reverse
    report.data = []
    topics =
      Topic.select('id, slug, title').where(
        'id in (?)',
        num_clicks.map { |z| z[0] }
      )
  ...
  def self.public_incoming_links(category_id: nil)
    IncomingLink.joins(post: :topic).where(
      'topics.archetype = ?',
      Archetype.default
    )
