...
class TopicViewItem < ActiveRecord::Base
  ...
  def self.add(topic_id, ip, user_id = nil, at = nil, skip_redis = false)
    ...
    user_id ? redis_key << ":user-#{user_id}" : redis_key << ":ip-#{ip}"

    if skip_redis || $redis.setnx(redis_key, '1')
      skip_redis ||
        $redis.expire(redis_key, SiteSetting.topic_view_duration_hours.hours)
      ...
      TopicViewItem.transaction do
        ...
        if !user_id
          builder.where(
            'ip_address = :ip_address AND topic_id = :topic_id AND user_id IS NULL'
          )
        else
        ...
        result =
          builder.exec(
            topic_id: topic_id, ip_address: ip, viewed_at: at, user_id: user_id
          )
        ...
        if result > 0
          if user_id
            UserStat.where(
              user_id: user_id
            ).update_all 'topics_entered = topics_entered + 1'
          end
        end
