...
class TopicTrackingState
  ...
  def self.publish_unread(post)
    ...
    TopicUser.tracking(post.topic_id).select(
      ...
      MessageBus.publish(
        self.unread_channel_key(tu.user_id),
        message.as_json,
        group_ids: group_ids
      )
    end
  ...
  def self.publish_recover(topic)
    ...
    message = { topic_id: topic.id, message_type: 'recover' }
  ...
  def self.publish_delete(topic)
    ...
    message = { topic_id: topic.id, message_type: 'delete' }

    MessageBus.publish('/delete', message.as_json, group_ids: group_ids)
  end

  def self.publish_read(
    topic_id, last_read_post_number, user_id, notification_level = nil
  )
    highest_post_number =
      Topic.where(id: topic_id).pluck(:highest_post_number).first
    ...
    MessageBus.publish(
      self.unread_channel_key(user_id),
      message.as_json,
      user_ids: [user_id]
    )
  end
  ...
  def self.report(user, topic_id = nil)
    ...
    sql <<
      report_raw_sql(
        topic_id: topic_id, skip_new: true, skip_order: true, staff: user.staff?
      )
  ...
  def self.report_raw_sql(opts = nil)
    unread =
      if opts && opts[:skip_unread]
        ...
        TopicQuery.unread_filter(Topic, -999, staff: opts && opts[:staff])
          .where_clause
          .send(:predicates)
          .join(' AND ')
          .gsub('-999', ':user_id')
    ...
    new =
      if opts && opts[:skip_new]
        ...
        TopicQuery.new_filter(Topic, 'xxx').where_clause.send(:predicates).join(
          ' AND '
        )
          .gsub!("'xxx'", treat_as_new_topic_clause) +
    ...
    WHERE u.id = :user_id AND
          ...
          AND NOT EXISTS( SELECT 1 FROM category_users cu
                          WHERE last_read_post_number IS NULL AND
                               ...
                               cu.notification_level = #{CategoryUser
        .notification_levels[
        :muted
      ]})
...
SQL
    ...
    sql << ' AND topics.id = :topic_id' if opts && opts[:topic_id]
  ...
  def self.publish_private_message(
    topic, archive_user_id: nil, post: nil, group_archive: false
  )
    ...
    if archive_user_id
      ...
      %w[
        ...
        .each { |channel| channels[channel] = user_ids }
    end
    ...
    channels.each do |channel, ids|
      MessageBus.publish(channel, message.as_json, user_ids: ids)
    end
  end
end
