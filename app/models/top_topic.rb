...
class TopTopic < ActiveRecord::Base
    ...
    sql =
      "SELECT topic_id, GREATEST(COUNT(*), 1) AS count
        ...
        :regular
               ...
               AND user_id <> #{Discourse.system_user
        .id}
             GROUP BY topic_id"
  ...
  def self.compute_top_score_for(period)
    log_views_multiplier =
      SiteSetting.top_topics_formula_log_views_multiplier.to_f
    log_views_multiplier = 2 if log_views_multiplier == 0

    first_post_likes_multiplier =
      SiteSetting.top_topics_formula_first_post_likes_multiplier.to_f
