...
class TopTopic < ActiveRecord::Base
  ...
  class << self
    ...
    def sorted_periods
      ascending_periods ||=
        Enum.new(daily: 1, weekly: 2, monthly: 3, quarterly: 4, yearly: 5, all: 6)
    end

    def score_column_for_period(period)
    ...
    def update_posts_count_for(period)
      sql =
    ...
    def update_views_count_for(period)
      sql =
    ...
    def update_likes_count_for(period)
      sql =
    ...
    def update_op_likes_count_for(period)
      sql =
    ...
    def compute_top_score_for(period)
    ...
    def update_top_topics(period, sort, inner_join)
      DB.exec(
        "UPDATE top_topics
