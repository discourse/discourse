...
class ApiKeyScope < ActiveRecord::Base
  ...
  class << self
    def list_actions
      ...
      %i[latest unread new top].each do |f|
        actions.concat(["list#category_#{f}", "list##{f}"])
      end
    ...
    def default_mappings
      ...
      mappings = {
        ...
        email: {
          receive_emails: {
            actions: %w[admin/email#handle_mail]
          }
        },
        badges: {
          create: {
      ...
      if actions.present?
        ...
        route_sets.each do |set|
          ...
          set.routes.each do |route|
            ...
            if actions.include?(action) && api_supported_path &&
  ...
  def permits?(env)
    RouteMatcher.new(
      **mapping.except(:urls),
      allowed_param_values: allowed_parameters
    ).match?(env: env)
  end
