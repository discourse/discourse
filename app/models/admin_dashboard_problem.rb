...
class AdminDashboardProblem
  ...
  def problems
    problems = []
    AdminDashboardProblem.problem_syms.each do |sym|
      problems << send(sym)
    end
    AdminDashboardProblem.problem_blocks.each do |blk|
      problems << instance_exec(&blk)
    end
    AdminDashboardProblem.problem_messages.each do |i18n_key|
      problems << AdminDashboardProblem.problem_message_check(i18n_key)
    end
    problems.compact!
  ...
  def self.fetch_problems(opts = {})
    AdminDashboardProblem.new(opts).problems
  end
