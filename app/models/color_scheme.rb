...
class ColorScheme < ActiveRecord::Base
  ...
  class LightAndDarkPaletteDoNotMatchError < StandardError
  end
  ...
  def self.base_color_scheme_colors
    ...
    base_colors.each do |name, color|
      base_with_hash << { name: name, hex: color, dark_hex: nil } # TODO: Do we need a core palette for dark mode?
    end

    list = [{ id: LIGHT_THEME_ID, colors: base_with_hash }]

    BUILT_IN_SCHEMES.each do |k, v|
      list.push(id: k.to_s, colors: self.create_colors_list_from_palette(v))
    end
  ...
  def self.create_colors_list_from_palette(palette)
    palette = palette.symbolize_keys
    colors = []

    if palette.has_key?(:dark) && palette.has_key?(:light)
      if palette[:light].keys.sort != palette[:dark].keys.sort
        raise LightAndDarkPaletteDoNotMatchError.new
      end

      palette[:light].each do |name, color|
        colors << { name: name, hex: color, dark_hex: palette[:dark][name] }
      end
    elsif palette.has_key?(:dark)
      palette[:dark].each { |name, color| colors << { name: name, hex: nil, dark_hex: color } }
    elsif palette.has_key?(:light)
      palette[:light].each { |name, color| colors << { name: name, hex: color, dark_hex: nil } }
    else
      raise "Invalid color scheme definition for #{k}"
    end
  ...
  def self.base
    ...
    @base_color_scheme.colors =
      base_colors.map { |name, hex| { name: name, hex: hex, dark_hex: nil } }
  ...
  def self.create_from_base(params)
    ...
    if params[:base_scheme_id]
      if palette = BUILT_IN_SCHEMES[params[:base_scheme_id].to_sym]
        colors = self.create_colors_list_from_palette(palette)
      end
    end
  ...
  def base_colors(dark: false)
    colors = nil
    if base_scheme_id && base_scheme_id != "Light"
      palette = BUILT_IN_SCHEMES[base_scheme_id.to_sym]
      if palette
        colors = dark ? (palette[:dark] || palette[:light]) : (palette[:light] || palette[:dark])
      end
    end
