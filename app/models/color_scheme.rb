...
class ColorScheme < ActiveRecord::Base
  ...
  def self.base_color_scheme_colors
    ...
    base_colors.each { |name, color| base_with_hash[name] = "#{color}" }

    list = [{ id: 'Light', colors: base_with_hash }]

    CUSTOM_SCHEMES.each { |k, v| list.push(id: k.to_s, colors: v) }
  ...
  def self.base_colors
    ...
    @mutex.synchronize do
      ...
      File.readlines(BASE_COLORS_FILE).each do |line|
        matches =
          /\$([\w]+):\s*#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})(?:[;]|\s)/.match(
            line.strip
          )
  ...
  def self.base_color_schemes
    base_color_scheme_colors.map do |hash|
      ...
      scheme.colors =
        hash[:colors].map { |k, v| { name: k.to_s, hex: v.sub('#', '') } }
      scheme.is_base = true
      scheme
    end
  end
  ...
  def self.base
    ...
    @base_color_scheme.colors =
      base_colors.map { |name, hex| { name: name, hex: hex } }
  ...
  def self.lookup_hex_for_name(name, scheme_id = nil)
    ...
    enabled_color_scheme ||=
      Theme.where(id: SiteSetting.default_theme_id).first&.color_scheme
    (enabled_color_scheme || base).colors.find { |c| c.name == name }.try(
      :hex
    ) ||
      'nil'
  end
  ...
  def colors=(arr)
    @colors_by_name = nil
    arr.each do |c|
      self.color_scheme_colors <<
        ColorSchemeColor.new(name: c[:name], hex: c[:hex])
    end
  end

  def colors_by_name
    @colors_by_name ||=
      self.colors.inject({}) do |sum, c|
        sum[c.name] = c
        sum
      end
  end
    ...
    if self.id
      ...
      if theme_ids.present?
        ...
        Theme.notify_theme_change(
          theme_ids,
          with_scheme: true, clear_manager_cache: false, all_themes: true
        )
      end
    end
  end
