...
class Group < ActiveRecord::Base
  ...
  def add(user, notify: false, automatic: false)
    ...
    added_ids = GroupManager.new(Discourse.system_user, self).bulk_add([user.id], automatic:)
  ...
  def remove(user)
    return false if user.nil?
    GroupManager.new(Discourse.system_user, self).bulk_remove([user.id]).present?
  end
  ...
  def bulk_add(user_ids, automatic: false)
    GroupManager.new(Discourse.system_user, self).bulk_add(user_ids, automatic:)
  end

  def bulk_remove(user_ids)
    GroupManager.new(Discourse.system_user, self).bulk_remove(user_ids)
  end
