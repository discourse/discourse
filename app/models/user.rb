...
class User < ActiveRecord::Base
  ...
  def unread_notifications_of_priority(high_priority:)
    if high_priority
      notification_query.unread_high_priority_count
    else
      notification_query.unread_low_priority_count
    end
  end

  MAX_UNREAD_BACKLOG = 400
  def grouped_unread_notifications
    notification_query.grouped_unread_counts
  end

  def unread_high_priority_notifications
    @unread_high_prios ||= notification_query.unread_high_priority_count
  end

  def new_personal_messages_notifications_count
    notification_query.new_personal_messages_count
  end
  ...
  def total_unread_notifications
    @unread_total_notifications ||= notification_query.total_count(filter: :unread)
  end
