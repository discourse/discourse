...
class User < ActiveRecord::Base
  ...
  has_many :default_featured_user_badges,
           -> {
             ...
             for_enabled_badges.grouped_with_count.where(
               'featured_rank <= ?',
               max_featured_rank
             )
           },
  ...
  has_many :associated_groups,
           through: :user_associated_groups,
           dependent: :destroy
  ...
  after_update :trigger_user_automatic_group_refresh,
               if: :saved_change_to_staged?
  ...
  before_destroy do
    ...
    MutedUser
      .where(user_id: self.id)
      .or(MutedUser.where(muted_user_id: self.id))
      .delete_all
    IgnoredUser
      .where(user_id: self.id)
      .or(IgnoredUser.where(ignored_user_id: self.id))
      .delete_all
  ...
  scope :silenced,
        -> {
          where(
            'silenced_till IS NOT NULL AND silenced_till > ?',
            Time.zone.now
          )
        }
  scope :not_silenced,
  ...
  scope :suspended,
        -> {
          where(
            'suspended_till IS NOT NULL AND suspended_till > ?',
            Time.zone.now
          )
        }
  scope :not_suspended,
  ...
  scope :watching_topic_when_mute_categories_by_default,
        ->(topic) {
          joins(
            DB.sql_fragment(
  ...
  def self.editable_user_custom_fields(by_staff: false)
    ...
    if by_staff
      fields.push(*DiscoursePluginRegistry.staff_editable_user_custom_fields)
    end
  ...
  def invited_by
    used_invite =
      Invite
        .with_deleted
        .joins(:invited_users)
        .where('invited_users.user_id = ?', self.id)
        .first
  ...
  def unread_notifications_of_type(notification_type)
    ...
    DB.query_single(sql, user_id: id, notification_type: notification_type)[
      0
    ].to_i
  end
         ...
         trust_level > TrustLevel[1] ||
           (
             first_seen_at.present? &&
               first_seen_at <
                 TRACK_FIRST_NOTIFICATION_READ_DURATION.seconds.ago
  ...
  def new_user?
    (created_at >= 24.hours.ago || trust_level == TrustLevel[0]) &&
      trust_level < TrustLevel[2] && !staff?
  end
  ...
  def update_timezone_if_missing(timezone)
    ...
    UserOption.where(user_id: self.id, timezone: nil).update_all(
      timezone: timezone
    )
  end
  ...
  def update_posts_read!(num_posts, opts = {})
    ...
    if user_visit = visit_record_for(now.to_date)
      ...
      begin
        create_visit_record!(
          now.to_date,
          posts_read: num_posts,
          mobile: opts.fetch(:mobile, false)
        )
  ...
  def self.update_ip_address!(user_id, new_ip:, old_ip:)
    unless old_ip == new_ip || new_ip.blank?
      ...
      if SiteSetting.keep_old_ip_address_count > 0
        ...
        SQL
          user_id: user_id,
          offset: SiteSetting.keep_old_ip_address_count
        )
      end
    end
  end
  ...
  def self.system_avatar_template(username)
    ...
    if SiteSetting.external_system_avatars_enabled
      ...
      url.gsub! '{first_letter}',
                UrlHelper.encode_component(
                  normalized_username.grapheme_clusters.first
                )
  ...
  def avatar_template
    use_small_logo =
      is_system_user? && SiteSetting.logo_small &&
  ...
  def posted_too_much_in_topic?(topic_id)
    ...
    since_reply =
      since_reply.where('id > ?', last_action_in_topic) if last_action_in_topic
  ...
  def flag_linked_posts_as_spam
    ...
    topic_links
      ...
      .each do |tl|
        ...
        results << PostActionCreator.create(
  ...
  def email=(new_email)
    if primary_email
      ...
      build_primary_email email: new_email,
                          skip_validate_email: !should_validate_email_address?
    end
  ...
  def next_best_title
    ...
    group_titles_query =
      group_titles_query.order(
        "groups.id = #{primary_group_id} DESC"
      ) if primary_group_id
    group_titles_query =
  ...
  def has_more_posts_than?(max_post_count)
    if user_stat &&
         (user_stat.topic_count + user_stat.post_count) > max_post_count
      return true
    end
  ...
  def shelved_notifications
    ShelvedNotification.joins(:notification).where(
  ...
  def set_default_categories_preferences
    ...
    %w[watching watching_first_post tracking regular muted].each do |setting|
      category_ids =
        SiteSetting.get("default_categories_#{setting}").split('|').map(&:to_i)
      ...
      badge_matching_title =
        title &&
          badges.find do |badge|
            badge.allow_title? &&
              (badge.display_name == title || badge.name == title)
          end
      user_profile.update(
