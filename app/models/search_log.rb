...
class SearchLog < ActiveRecord::Base
  ...
  def self.redis_key(ip_address:, user_id: nil)
    user_id ? "__SEARCH__LOG_#{user_id}" : "__SEARCH__LOG_#{ip_address}"
  end

  # for testing
  def self.clear_debounce_cache!
    $redis.keys('__SEARCH__LOG_*').each { |k| $redis.del(k) }
  end

  def self.log(term:, search_type:, ip_address:, user_id: nil)
    return %i[error] if term.blank?

    search_type = search_types[search_type]
    return %i[error] unless search_type.present? && ip_address.present?
  ...
  def self.term_details(term, period = :weekly, search_type = :all)
    ...
    if search_type == :header || search_type == :full_page
      result = result.where('search_type = ?', search_types[search_type])
    end
    if search_type == :click_through_only
      result = result.where('search_result_id IS NOT NULL')
    end

    result.group(:term).order('date').group('date').each do |record|
  ...
  def self.trending_from(start_date, options = {})
    ...
    result.group('lower(term)').order(
      'searches DESC, click_through DESC, term ASC'
    )
      .limit(limit)
  end
  ...
  def self.clean_up
    search_id =
      SearchLog.order(:id).offset(SiteSetting.search_query_log_max_size).limit(
        1
      )
        .pluck(:id)
