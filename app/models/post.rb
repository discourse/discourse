...
class Post < ActiveRecord::Base
  ...
  cattr_accessor :plugin_permitted_create_params,
                 :plugin_permitted_update_params
  ...
  def with_secure_media?
    ...
    SiteSetting.login_required? ||
      (
        topic.present? &&
          (topic.private_message? || topic.category&.read_restricted)
      )
  end
  ...
  def self.estimate_posts_per_day
    ...
    posts_per_day =
      Topic
        ...
        .secured
        .joins(:posts)
        .merge(Post.created_since(30.days.ago))
        .count / 30
    Discourse.redis.setex(
  ...
  def reply_ids(guardian = nil, only_replies_to_single_post: true)
    ...
    replies =
      builder.query_hash(
        post_id: id,
        max_reply_level: MAX_REPLY_LEVEL,
        topic_id: topic_id
      )
  ...
  def link_post_uploads(fragments: nil)
    ...
    post_uploads =
      upload_ids.map { |upload_id| { post_id: self.id, upload_id: upload_id } }

    PostUpload.transaction do
      PostUpload.where(post_id: self.id).delete_all

      PostUpload.insert_all(post_uploads) if post_uploads.size > 0
  ...
  def each_upload_url(fragments: nil, include_local_upload: true)
    ...
    links.each do |src|
      ...
      src =
        "#{SiteSetting.force_https ? 'https' : 'http'}:#{src}" if src.start_with?(
        '//'
      )
      unless Discourse.store.has_been_uploaded?(src) ||
               Upload.secure_media_url?(src) ||
  ...
  def self.find_missing_uploads(include_local_upload: true)
      ...
      query.find_in_batches do |posts|
        ...
        sha1s =
          Upload
            .joins(:post_uploads)
            .where(
              'post_uploads.post_id >= ? AND post_uploads.post_id <= ?',
              ids.min,
              ids.max
            )
            .pluck(:sha1)
        ...
        posts.each do |post|
          post.each_upload_url do |src, path, sha1|
            ...
            upload_id = nil
            upload_id =
              Upload.where(sha1: sha1).pluck_first(:id) if sha1.present?
    ...
    {
      uploads: missing_uploads,
      post_uploads: missing_post_uploads,
      count: count
    }
  end
  ...
  def parse_quote_into_arguments(quote)
    ...
    quote
      .first
      .scan(/([a-z]+)\:(\d+)/)
      .each { |arg| args[arg[0]] = arg[1].to_i }
    args
  end
