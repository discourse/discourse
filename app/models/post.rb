...
class Post < ActiveRecord::Base
  ...
  def self.allow_listed_image_classes
    @allow_listed_image_classes ||= ['avatar', 'favicon', 'thumbnail', 'emoji', 'ytp-thumbnail-image']
  end
  ...
  def allowlisted_spam_hosts
    hosts = SiteSetting
      .allow_listed_spam_host_domains
      .split('|')
      .map { |h| h.strip }
      .reject { |h| !h.include?('.') }
  ...
  def total_hosts_usage
    ...
    allowlisted = allowlisted_spam_hosts
