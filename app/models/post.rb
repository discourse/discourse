...
class Post < ActiveRecord::Base
  ...
  has_many :revisions,
           -> { order(:number) },
           foreign_key: :post_id, class_name: 'PostRevision'
  ...
  scope :private_posts_for_user, lambda do |user|
    where(
      'posts.topic_id IN (SELECT topic_id
  ...
  scope :public_posts, lambda do
    joins(:topic).where('topics.archetype <> ?', Archetype.private_message)
  end
  scope :private_posts, lambda do
    joins(:topic).where('topics.archetype = ?', Archetype.private_message)
  end
  scope :with_topic_subtype,
        ->(subtype) { joins(:topic).where('topics.subtype = ?', subtype) }
  scope :visible,
        -> { joins(:topic).where('topics.visible = true').where(hidden: false) }
  scope :secured, lambda do |guardian|
    where('posts.post_type IN (?)', Topic.visible_post_types(guardian&.user))
  end
  scope :for_mailing_list, lambda do |user, since|
    q =
      created_since(since).joins(:topic).where(
        topic: Topic.for_digest(user, Time.at(0))
  ...
  scope :raw_match, lambda do |pattern, type = 'string'|
    type = type&.downcase
  ...
  def self.types
    @types ||=
      Enum.new(regular: 1, moderator_action: 2, small_action: 3, whisper: 4)
  end
  ...
  def limit_posts_per_day
    if user && user.new_user_posting_on_first_day? && post_number &&
       post_number > 1
      RateLimiter.new(
        user,
  ...
  def publish_message!(channel, message, opts = {})
    ...
    if Topic.visible_post_types.include?(post_type)
      if topic.private_message?
        opts[:user_ids] =
          User.human_users.where('admin OR moderator').pluck(:id)
      ...
      opts[:user_ids] =
        User.human_users.where('admin OR moderator OR id = ?', user_id).pluck(
          :id
        )
    end
  ...
  def store_unique_post_key
    if SiteSetting.unique_posts_mins > 0
      $redis.setex(
        unique_post_key,
        SiteSetting.unique_posts_mins.minutes.to_i,
        id
      )
    end
  end
  ...
  def self.white_listed_image_classes
    @white_listed_image_classes ||= %w[avatar favicon thumbnail emoji]
  end
  ...
  %w[
    ...
    .each { |attr| define_method(attr) { post_analyzer.send(attr) } }

  def add_nofollow?
    return false if user&.staff?
    user.blank? || SiteSetting.tl3_links_no_follow? ||
  ...
  def cook(raw, opts = {})
    ...
    if post_type == Post.types[:regular]
      if new_cooked != cooked && new_cooked.blank?
        Rails.logger.debug(
          "Plugin is blanking out post: #{self.url}\nraw: #{raw}"
        )
  ...
  def whitelisted_spam_hosts
    hosts =
      SiteSetting.white_listed_spam_host_domains.split('|').map(&:strip)
        .reject { |h| !h.include?('.') }
  ...
  def total_hosts_usage
    ...
    hosts.reject! { |h| whitelisted.any? { |w| h.end_with?(w) } }

    return hosts if hosts.length == 0

    TopicLink.where(domain: hosts.keys, user_id: acting_user.id).group(
      :domain,
      :post_id
    )
      .count
  ...
  def has_host_spam?
    ...
    total_hosts_usage.values.any? do |count|
      count >= SiteSetting.newuser_spam_host_threshold
    end
  end
  ...
  def recover_public_post_actions
    PostAction.publics.with_deleted.where(
      post_id: self.id, id: self.custom_fields['deleted_public_actions']
    )
  ...
  def reply_to_post
    ...
    @reply_to_post ||=
      Post.find_by(
        ...
        topic_id: topic_id, post_number: reply_to_post_number
      )
  end
  ...
  def reply_notification_target
    ...
    Post.find_by(
      ...
      topic_id: topic_id, post_number: reply_to_post_number, user_id: user_id
    )
      .try(:user)
  end
  ...
  def url(opts = nil)
    ...
    topic ? Post.url(topic.slug, topic.id, post_number, opts) : '/404'
  end

  def unsubscribe_url(user)
    "#{Discourse.base_url}/email/unsubscribe/#{UnsubscribeKey.create_key_for(
      user,
      self
    )}"
  end
  ...
  def self.urls(post_ids)
    ...
    if ids.length > 0
      urls = {}
      Topic.joins(:posts).where('posts.id' => ids).select(
        [
          'posts.id as post_id',
          'post_number',
          'topics.slug',
      ...
      begin
        ...
        if attempts > 3
          ...
          Discourse.warn_exception(
            e,
            message:
              "Can not rebake post# #{post.id} after 3 attempts, giving up"
          )
        else
  ...
  def rebake!(
    invalidate_broken_images: false, invalidate_oneboxes: false, priority: nil
  )
    new_cooked =
      cook(raw, topic_id: topic_id, invalidate_oneboxes: invalidate_oneboxes)
    old_cooked = cooked

    update_columns(
      cooked: new_cooked, baked_at: Time.new, baked_version: BAKED_VERSION
    )
  ...
  def self.estimate_posts_per_day
    ...
    posts_per_day =
      Topic.listable_topics.secured.joins(:posts).merge(
        Post.created_since(30.days.ago)
      )
        .count /
        30
  ...
  def self.calculate_avg_time(min_topic_age = nil)
    retry_lock_error do
      ...
      if min_topic_age
        builder.where(
          'posts.topic_id IN (SELECT id FROM topics where bumped_at > :bumped_at)',
  ...
  def extract_quoted_post_numbers
    ...
    raw.scan(/\[quote=\"([^"]+)"\]/).each do |quote|
      ...
      if !(args[:topic].present? && topic_id != args[:topic]) &&
         args[:post] != post_number
  ...
  def trigger_post_process(
    bypass_bump: false, priority: :normal, new_post: false
  )
    args = { post_id: id, bypass_bump: bypass_bump, new_post: new_post }
  ...
  def self.private_messages_count_per_day(start_date, end_date, topic_subtype)
    private_posts.with_topic_subtype(topic_subtype).where(
      'posts.created_at >= ? AND posts.created_at <= ?',
      start_date,
      end_date
    )
  ...
  def reply_history(max_replies = 100, guardian = nil)
    ...
    ORDER by id
    SQL,
        post_id: id, topic_id: topic_id
      )

    # [1,2,3][-10,-1] => nil
    post_ids = (post_ids[(0 - max_replies)..-1] || post_ids)

    Post.secured(guardian).where(id: post_ids).includes(:user, :topic).order(
      :id
    )
      .to_a
  end
  ...
  def reply_ids(guardian = nil, only_replies_to_single_post: true)
    ...
    replies.each(&:symbolize_keys!)

    secured_ids =
      Post.secured(guardian).where(id: replies.map { |r| r[:id] }).pluck(:id)
        .to_set
  ...
  def seen?(user)
    PostTiming.where(
      topic_id: topic_id, post_number: post_number, user_id: user.id
    )
      .exists?
  end
  ...
  def link_post_uploads(fragments: nil)
    ...
    fragments.css('a/@href', 'img/@src').each do |media|
      upload_ids << upload.id if upload = Upload.get_from_url(media.value)
    end

    upload_ids |= Upload.where(id: downloaded_images.values).pluck(:id)
    values =
      upload_ids.map! { |upload_id| "(#{self.id},#{upload_id})" }.join(',')
    ...
    PostUpload.transaction do
      ...
      if values.size > 0
        DB.exec(
          "INSERT INTO post_uploads (post_id, upload_id) VALUES #{values}"
        )
      end
    end
  end
