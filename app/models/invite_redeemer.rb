...
InviteRedeemer =
  Struct.new(
    ...
    def self.create_user_from_invite(
      ...
      authenticator =
        UserAuthenticator.new(user, session, require_password: false)
    ...
    def mark_invite_redeemed
      ...
      if existing_user.present? &&
           InvitedUser.exists?(user_id: existing_user.id, invite_id: invite.id)
        return false
      end

      @invited_user_record =
        InvitedUser.create!(invite_id: invite.id, redeemed_at: Time.zone.now)
    ...
    def add_to_private_topics_if_invited
      ...
      topic_ids.each do |id|
        unless TopicAllowedUser.exists?(user_id: invited_user.id, topic_id: id)
    ...
    def delete_duplicate_invites
      Invite
        ...
        .joins(
          'LEFT JOIN invited_users ON invites.id = invited_users.invite_id'
        )
