InviteRedeemer =
  Struct.new(:invite, :username, :name, :password, :user_custom_fields) do
    def redeem
    ...
    def self.create_user_from_invite(
      invite, username, name, password = nil, user_custom_fields = nil
    )
      if username && UsernameValidator.new(username).valid_format? &&
         User.username_available?(username)
    ...
    def add_user_to_groups
      new_group_ids =
        invite.groups.pluck(:id) - invited_user.group_users.pluck(:group_id)
      new_group_ids.each { |id| invited_user.group_users.create(group_id: id) }
    end

    def send_welcome_message
      if Invite.where('email = ?', invite.email).update_all(
    ...
    def approve_account_if_needed
      invited_user.approve(invite.invited_by, false) if get_existing_user
    end
