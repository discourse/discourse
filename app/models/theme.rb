...
class Theme < ActiveRecord::Base
  ...
  after_save do
    ...
    if saved_change_to_color_scheme_id? || saved_change_to_user_selectable? ||
  ...
  def self.lookup_field(theme_id, target, field, skip_transformation: false)
    ...
    cache_key =
      "#{theme_ids.join(',')}:#{target}:#{field}:#{Theme.compiler_version}"
  ...
  def self.lookup_modifier(theme_ids, modifier_name)
    ...
    get_set_cache(
      "#{theme_ids.join(',')}:modifier:#{modifier_name}:#{Theme.compiler_version}"
  ...
  def self.notify_theme_change(
    ...
    if with_scheme
      ...
      targets.append(
        *Discourse.find_plugin_css_assets(mobile_view: true, desktop_view: true)
      )
  ...
  def self.refresh_message_for_targets(targets, theme_ids)
    ...
    targets.each_with_object([]) do |target, data|
      theme_ids.each do |theme_id|
        data << Stylesheet::Manager.new(theme_id: theme_id).stylesheet_data(
          target.to_sym
        )
      end
    end
  ...
  def self.resolve_baked_field(theme_ids, target, name)
    if target == :extra_js
      ...
      return(
        caches
          .map do |c|
            "<script src='#{c.url}' data-theme-id='#{c.theme_id}'></script>"
          end
          .join("\n")
      )
    end
    list_baked_fields(theme_ids, target, name)
      .map { |f| f.value_baked || f.value }
      .join("\n")
  end
  ...
  def build_settings_hash
    ...
    hash[
      'theme_uploads_local'
    ] = theme_uploads_local if theme_uploads_local.present?
  ...
  def convert_settings
    settings.each do |setting|
      setting_row =
        ThemeSetting.where(theme_id: self.id, name: setting.name.to_s).first
