...
class TopicUser < ActiveRecord::Base
  ...
  scope :tracking, lambda do |topic_id|
    where(topic_id: topic_id).where(
      'COALESCE(topic_users.notification_level, :regular) >= :tracking',
  ...
  class << self
    ...
    def auto_notification(user_id, topic_id, reason, notification_level)
      should_change =
        TopicUser.where(user_id: user_id, topic_id: topic_id).where(
          'notifications_reason_id IS NULL OR (notification_level < :max AND notification_level > :min)',
          max: notification_level, min: notification_levels[:regular]
        )
          .exists?
    ...
    def auto_notification_for_staging(
      ...
      change(
        user_id,
        topic_id,
        notification_level: notification_level, notifications_reason_id: reason
      )
    end
    ...
    def change(user_id, topic_id, attrs)
      ...
      TopicUser.transaction do
        ...
        rows =
          TopicUser.where(topic_id: topic_id, user_id: user_id).update_all(
            [attrs_sql, *vals]
          )
    ...
    def notification_level_change(
      user_id, topic_id, notification_level, reason_id
    )
    ...
    def create_missing_record(user_id, topic_id, attrs)
      ...
      unless attrs[:notification_level]
        category_notification_level =
          CategoryUser.where(user_id: user_id).where(
            'category_id IN (SELECT category_id FROM topics WHERE id = :id)',
            id: topic_id
          )
            .where(
        ...
        tag_notification_level =
          TagUser.where(user_id: user_id).where(
            'tag_id IN (SELECT tag_id FROM topic_tags WHERE topic_id = :id)',
            id: topic_id
          )
            .where(
            'notification_level IN (:levels)',
        ...
        if category_notification_level &&
           !(
             tag_notification_level &&
               (tag_notification_level > category_notification_level)
           )
        ...
        if Topic.private_messages.where(id: topic_id).exists? &&
          ...
          group_notification_level =
            Group.joins(
              ...
              .joins(
              "LEFT OUTER JOIN topic_allowed_groups tag ON tag.topic_id = #{topic_id}"
            )
          ...
          auto_track_after =
            UserOption.where(user_id: user_id).pluck(
          ...
          if auto_track_after >= 0 &&
             auto_track_after <= (attrs[:total_msecs_viewed].to_i || 0)
    ...
    UPDATE_TOPIC_USER_SQL_STAFF =
      UPDATE_TOPIC_USER_SQL.gsub(
        'highest_post_number',
    ...
    INSERT_TOPIC_USER_SQL_STAFF =
      INSERT_TOPIC_USER_SQL.gsub(
        'highest_post_number',
        'highest_staff_post_number'
      )

    def update_last_read(
      user, topic_id, post_number, new_posts_read, msecs, opts = {}
    )
  ...
  def self.update_post_action_cache(opts = {})
    ...
    builder.where('tu2.user_id = :user_id', user_id: user_id) if user_id

    builder.where('tu2.topic_id = :topic_id', topic_id: topic_id) if topic_id

    if post_id
      builder.where(
        'tu2.topic_id IN (SELECT topic_id FROM posts WHERE id = :post_id)',
        post_id: post_id
      )
      builder.where(
        'tu2.user_id IN (SELECT user_id FROM post_actions
  ...
  def self.cap_unread!(user_id, count)
    ...
    WHERE tu.user_id = :user_id AND
          ...
          tu.topic_id IN (
            #{TopicTrackingState
      .report_raw_sql(
      skip_new: true, select: 'topics.id'
    )}
            offset :count
          )
SQL
