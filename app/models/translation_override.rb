...
class TranslationOverride < ActiveRecord::Base
  ...
  def self.upsert!(locale, key, value)
    ...
    sanitized_value =
      translation_override.sanitize_field(
        value,
        additional_attributes: ['data-auto-route']
      )
    ...
    if key.end_with?('_MF')
      ...
      data[:compiled_js] = JsLocaleHelper.compile_message_format(
        filename,
        locale,
        sanitized_value
      )
  ...
  def self.expire_cache(locale, key)
    if key.starts_with?('post_action_types.')
      ApplicationSerializer.expire_cache_fragment!(
        "post_action_types_#{locale}"
      )
    elsif key.starts_with?('topic_flag_types.')
      ApplicationSerializer.expire_cache_fragment!(
        "post_action_flag_types_#{locale}"
      )
    else
      return false
    end
  ...
  def check_interpolation_keys
    ...
    if original_text
      original_interpolation_keys =
        I18nInterpolationKeysFinder.find(original_text)
      ...
      invalid_keys =
        (original_interpolation_keys | new_interpolation_keys) -
          original_interpolation_keys - custom_interpolation_keys
