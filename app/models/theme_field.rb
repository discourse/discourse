...
class ThemeField < ActiveRecord::Base
  ...
  scope :find_by_theme_ids, lambda do |theme_ids|
    return none unless theme_ids.present?

    where(theme_id: theme_ids).joins(
      "JOIN (
          SELECT #{theme_ids.map.with_index do |id, idx|
  ...
  scope :filter_locale_fields, lambda do |locale_codes|
    ...
    where(target_id: Theme.targets[:translations], name: locale_codes).joins(
      self.sanitize_sql_array(
        [
          "JOIN (
        SELECT * FROM (VALUES #{locale_codes.map { '(?)' }
            .join(
            ','
          )}) as Y (locale_code, locale_sort_column)
  ...
  scope :find_first_locale_fields, lambda do |theme_ids, locale_codes|
    find_by_theme_ids(theme_ids).filter_locale_fields(locale_codes).reorder(
      'X.theme_sort_column',
      'Y.locale_sort_column'
    )
  ...
  validates :name,
            format: { with: /\A[a-z_][a-z0-9_-]*\z/i }, if: Proc.new do |field|
    ThemeField.theme_var_type_ids.include?(field.type_id)
  end
  ...
  def process_html(html)
    ...
    errors.each { |error| js_compiler.append_js_error(error) }

    if js_compiler.content.present? && theme.cached_settings.present?
      js_compiler.prepend_settings(theme.cached_settings)
    end
    javascript_cache.content = js_compiler.content
    javascript_cache.save!

    if javascript_cache.content.present?
      doc.add_child("<script src='#{javascript_cache.url}'></script>")
    end
  ...
  def translation_data(
    with_overrides: true, internal: false, fallback_fields: nil
  )
    fallback_fields ||=
      theme.theme_fields.filter_locale_fields(I18n.fallbacks[name])
  ...
  def process_translation
    ...
    begin
      ...
      js = <<~JS
        /* Translation data for theme #{self.theme_id} (#{self
        .name})*/
        const data = #{data
        .to_json};
    ...
    doc = ''
    if javascript_cache.content.present?
      doc = "<script src='#{javascript_cache.url}'></script>"
    end
    [doc, errors&.join("\n")]
  end
  ...
  def validate_yaml!
    ...
    begin
      ThemeSettingsParser.new(self).load do |name, default, type, opts|
        ...
        if setting.invalid?
          setting.errors.details.each_pair do |attribute, _errors|
            _errors.each do |hash|
              errors <<
                I18n.t(
                  "#{translation_key}.#{attribute}_#{hash[:error]}",
                  name: name
                )
            end
          end
        ...
        if default.nil?
          errors <<
            I18n.t("#{translation_key}.default_value_missing", name: name)
        end
        ...
        unless ThemeSetting.acceptable_value_for_type?(default, type)
          errors <<
            I18n.t("#{translation_key}.default_not_match_type", name: name)
        end
      end
  ...
  def ensure_baked!
    if ThemeField.html_fields.include?(self.name) ||
       translation = Theme.targets[:translations] == self.target_id
      if !self.value_baked || compiler_version != COMPILER_VERSION
        self.value_baked, self.error =
          translation ? process_translation : process_html(self.value)
    ...
    if ThemeField.scss_fields.include?(self.name)
      begin
        Stylesheet::Compiler.compile(
          '@import \"common/foundation/variables\"; @import \"theme_variables\"; @import \"theme_field\";',
          'theme.scss',
          theme_field: self.value.dup, theme: self.theme
        )
      ...
      update_columns(error: self.error) if will_save_change_to_error?
    end
  end
  ...
  class ThemeFileMatcher
    ...
    def opts_from_filename(filename)
      ...
      OPTIONS.each do |option|
        plural = :"#{option}s"
        if @allowed_values[plural].length == 1
          hash[option] = @allowed_values[plural][0]
        end
  ...
  FILE_MATCHERS = [
    ThemeFileMatcher.new(
      regex: %r{^(?<target>(?:mobile|desktop|common))\/(?<name>(?:head_tag|header|after_header|body_tag|footer))\.html$},
    ...
    ThemeFileMatcher.new(
      regex: %r{^locales\/(?<name>(?:#{I18n.available_locales.join(
        '|'
      )}))\.yml$},
    ...
    ThemeFileMatcher.new(
      regex: /(?!)/,
      # Never match uploads by filename, they must be named in about.json
      names: nil,
  ...
  def self.opts_from_file_path(filename)
    FILE_MATCHERS.each do |matcher|
      return opts if opts = matcher.opts_from_filename(filename)
    end
    nil
  end
  ...
  after_commit do
    ...
    if theme && self.name == 'header'
      MessageBus.publish "/header-change/#{theme.id}", self.value
    end
    if theme && self.name == 'footer'
      MessageBus.publish "/footer-change/#{theme.id}", self.value
    end
  end
