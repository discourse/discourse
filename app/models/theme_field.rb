...
class ThemeField < ActiveRecord::Base
  ...
  def process_html(html)
    ...
    if contains_optimized_link?(html)
      errors << I18n.t('themes.errors.optimized_link')
    end
    ...
    if js_compiler.content.present? && settings_hash.present?
    ...
    if javascript_cache.content.present?
      doc.add_child(
        "<script src='#{javascript_cache.url}' data-theme-id='#{theme_id}'></script>"
      )
    end
    ...
    begin
      ...
      when 'hbs'
        js_compiler.append_ember_template(
          filename.sub('discourse/templates/', ''),
          content
        )
      when 'hbr', 'raw.hbs'
        js_compiler.append_raw_template(
          filename.sub('discourse/templates/', ''),
          content
        )
      else
        raise ThemeJavascriptCompiler::CompileError.new(
                I18n.t(
                  'themes.compile_error.unrecognized_extension',
                  extension: extension
                )
              )
      end
  ...
  def process_translation
    ...
    doc = ''
    doc =
      "<script src='#{javascript_cache.url}' data-theme-id='#{theme_id}'></script>" if javascript_cache.content.present?
    [doc, errors&.join("\n")]
  end
    ...
    begin
      ThemeSettingsParser
        ...
        .load do |name, default, type, opts|
          ...
          if default.nil?
            errors << I18n.t(
              "#{translation_key}.default_value_missing",
              name: name
            )
          end
          ...
          if (min = opts[:min]) && (max = opts[:max])
            unless ThemeSetting.value_in_range?(default, (min..max), type)
              errors << I18n.t(
                "#{translation_key}.default_out_range",
                name: name
              )
            end
          end
          ...
          unless ThemeSetting.acceptable_value_for_type?(default, type)
            errors << I18n.t(
              "#{translation_key}.default_not_match_type",
              name: name
            )
          end
        end
  ...
  def ensure_baked!
    needs_baking =
      !self.value_baked || compiler_version != Theme.compiler_version
  ...
  class ThemeFileMatcher
    ...
    def opts_from_filename(filename)
      ...
      OPTIONS.each do |option|
        plural = :"#{option}s"
        hash[option] = @allowed_values[plural][0] if @allowed_values[
          plural
        ]&.length == 1
  ...
  FILE_MATCHERS = [
    ...
    ThemeFileMatcher.new(
      regex:
        %r{^locales/(?<name>(?:#{I18n.available_locales.join('|')}))\.yml$},
    ...
    if extra_scss_field?
      return(
        theme.theme_fields.where(
          target_id:
            ThemeField.basic_targets.map { |t| Theme.targets[t.to_sym] },
  ...
  after_destroy do
    DB.after_commit { SvgSprite.expire_cache } if svg_sprite_field?
  end
