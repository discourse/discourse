...
class GlobalSetting
  ...
  def self.load_defaults
    default_provider =
      FileProvider.from(
        File.expand_path('../../../config/discourse_defaults.conf', __FILE__)
      )
    default_provider
      .keys
  ...
  def self.database_config
    ...
    hash[
      'idle_timeout'
    ] = connection_reaper_age if connection_reaper_age.present?
    hash[
      'reaping_frequency'
    ] = connection_reaper_interval if connection_reaper_interval.present?
    hash['advisory_locks'] = !!self.db_advisory_locks

    db_variables =
      provider.keys.filter { |k| k.to_s.starts_with? 'db_variables_' }
    if db_variables.length > 0
      hash['variables'] = {}
      db_variables.each do |k|
        hash['variables'][
          k.slice(('db_variables_'.length)..)
        ] = self.public_send(k)
      end
    end
  ...
  def self.get_message_bus_redis_replica_host
    if message_bus_redis_replica_host.present?
      return message_bus_redis_replica_host
    end
    if respond_to?(:message_bus_redis_slave_host) &&
         message_bus_redis_slave_host.present?
      message_bus_redis_slave_host
    end
  end

  def self.get_message_bus_redis_replica_port
    if message_bus_redis_replica_port.present?
      return message_bus_redis_replica_port
    end
    if respond_to?(:message_bus_redis_slave_port) &&
         message_bus_redis_slave_port.present?
  ...
  def self.redis_config
    @config ||=
      begin
        ...
        if get_redis_replica_host && get_redis_replica_port &&
             defined?(RailsFailover)
  ...
  def self.message_bus_redis_config
    ...
    @message_bus_config ||=
      begin
        ...
        if get_message_bus_redis_replica_host &&
             get_message_bus_redis_replica_port
        ...
        c[
          :password
        ] = message_bus_redis_password if message_bus_redis_password.present?
  ...
  def self.configure!
    if Rails.env == 'test'
      ...
      @provider =
        FileProvider.from(
          File.expand_path('../../../config/discourse.conf', __FILE__)
        ) || EnvProvider.new
    end
  end
