class Emoji
  ...
  def self.search_aliases
    Discourse.cache.fetch(cache_key('search_aliases_emojis')) do
      db['searchAliases']
    end
  end

  def self.translations
    Discourse.cache.fetch(cache_key('translations_emojis')) do
      load_translations
    end
  end
  ...
  def self.url_for(name)
    "#{Discourse.base_uri}/images/emoji/#{SiteSetting
      .emoji_set}/#{name}.png?v=#{EMOJI_VERSION}"
  end
  ...
  def self.clear_cache
    %w[custom standard aliases search_aliases translations all tonable]
      .each { |key| Discourse.cache.delete(cache_key("#{key}_emojis")) }
  end
  ...
  def self.load_custom
    ...
    Plugin::CustomEmoji.emojis.each do |name, url|
      result <<
        Emoji.new.tap do |e|
          e.name = name
          url = (Discourse.base_uri + url) if url[%r{^\/[^\/]}]
          e.url = url
        end
    end
        ...
        db['emojis'].each do |e|
          ...
          if is_tonable_emojis.include?(e['name'])
            FITZPATRICK_SCALE.each_with_index do |scale, index|
              toned_code =
                (code.codepoints.insert(1, scale.to_i(16))).pack('U*')
