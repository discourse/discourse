...
- `enabled?(setting_name)` — Determines the *effective* value of a change. This is where auto-promotion logic lives: if a setting's status meets/exceeds `promote_upcoming_changes_on_status`, it resolves to `true` even if the DB default is `false`. Permanent settings always resolve to `true` (admins can't disable them). It returns `false` outright when the owning plugin is not configurable, or — for changes declaring `requires_plugin_enabled: true` — when the owning plugin is disabled. Both take precedence over the above. See [Requiring the Owning Plugin](#requiring-the-owning-plugin).
...
2. **Resolution** — `should_display?` first hides the change outright if its owning plugin is not configurable (`owning_plugin_configurable?`), or if the change declares `requires_plugin_enabled: true` and the owning plugin is disabled (`owning_plugin_enabled?`). Then it checks for a class method named `should_display_<upcoming_change_name>?` on `ConditionalDisplay`. If defined, its return value is used. Otherwise it evaluates enabled plugin callbacks registered for that setting. If no method or callback is defined, the change is always displayed (returns `true`).
...
- **Never gate a change on its own plugin's `enabled_site_setting`**: use `requires_plugin_enabled: true` instead. A conditional display callback cannot express it — `DiscoursePluginRegistry` filters callbacks from disabled plugins, so it would never run in the only case that matters. See [Requiring the Owning Plugin](#requiring-the-owning-plugin).
...
A plugin-owned change that gates a feature *inside* its plugin is useless while that plugin is disabled — the plugin's code isn't running, so the admin is being pitched (and notified about) a toggle that does nothing. Such a change declares:
...
`UpcomingChanges.owning_plugin_enabled?` then returns false while the plugin is off, which both hides the change (`should_display?`) and stops it resolving (`enabled?`), so its `hide_settings` and default overrides don't apply either. The admin's opt-in stays in the database and resumes when they re-enable the plugin.

#### It is opt-in for a reason

Plenty of plugin-owned changes are the *opposite*: they exist to get the plugin adopted, and only make sense while it is disabled. Declaring `requires_plugin_enabled` on one of these makes it unreachable — it would be hidden from exactly the sites it targets. Current examples, none of which may ever declare it:
...
| `enable_discourse_workflows` (workflows) | *Is* the plugin's `enabled_site_setting`. That row is how an admin opts into the plugin at all. |

Rule of thumb: if the change's purpose is *"try this feature we added to the plugin you already run"*, declare it. If its purpose is *"start using this plugin"*, don't.

#### Guardrails

The integrity spec (`spec/integrity/upcoming_change_metadata_spec.rb`) enforces that `requires_plugin_enabled` is a boolean, is only set on plugin-owned changes, and is **never** set on the plugin's own `enabled_site_setting` — that combination is self-gating and would recurse (`Plugin::Instance#enabled?` reads the setting, which resolves back through `enabled?`). `owning_plugin_enabled?` also guards against the recursion at runtime, so a mistake surfaces as a hidden change rather than a stack overflow.
