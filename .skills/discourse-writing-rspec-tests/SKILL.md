...
- **Never define Ruby methods within RSpec scope** — no form of Ruby method definition may appear beneath an example group, shared example, or shared context, including definitions nested in examples, hooks, or inline fixture/interface classes, modules, and singleton classes. Put reusable fixture and interface behavior in discoverable support fixtures, helpers, page objects, custom matchers, or derived fabricators. Use `fab!`, `let`, a named `subject`, and explicit scenario setup and calls for ordinary setup.
