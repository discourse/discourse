...
- **Abstract concepts, not syntax** — an abstraction in test code should express a meaningful testing concept and make the example understandable without inspecting its implementation. Repetition alone does not justify a helper. Keep ordinary setup and actions visible with RSpec primitives or direct calls when an extra name would only add indirection.
...
- **Preserve assertion strength while refactoring tests** — identify every guarantee made by the original assertions, including existence, identity, cardinality, ordering, and shape. A more concise matcher is only equivalent when it preserves all of those guarantees and cannot pass vacuously.
