...
- **Avoid helper methods inside example groups** — express shared records with `fab!`; use `let` when record creation must remain lazy because it depends on per-example setup. Express other shared values and configuration with `let`, and the public operation under test with a named `subject`. Keep scenario-specific setup and parameterized calls explicit in the example. Extract substantial reusable behavior into a page object, custom matcher, support helper, or derived fabricator.
