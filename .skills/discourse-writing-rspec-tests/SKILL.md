...
- Use `stub_const(target, :CONSTANT, value) { ... }` for temporary constants rather than `const_set` and manual cleanup. Use an `around` hook when the constant is needed throughout an example; the helper restores existing constants and removes newly introduced ones.
