...
- **Eliminate trivial wrappers instead of relocating them** — do not move a method to `spec/support` merely to remove it from RSpec scope. If it only wraps `Fabricate`, `create!`, `update!`, a lookup, a value, or one production call, replace it with `fab!`, `let`, a named `subject`, or explicit setup at the call site. Prefer a little visible duplication over a support method that hides ordinary setup.
- **Reserve support objects for substantial abstractions** — use support fixtures, helpers, page objects, custom matchers, or derived fabricators only when they own a reusable interface, external boundary, cohesive multi-step fixture, or assertion protocol. During cleanup, inspect newly extracted support code and delete trivial or unused methods rather than preserving them under a new owner.
- **Use auto-loaded support files** — `rails_helper` requires core and plugin `spec/support/**/*.rb`; do not add per-spec requires for those paths.
...
- **Preserve presence guarantees when consolidating assertions** — matchers such as `all` pass for an empty collection. When replacing per-record assertions, first assert the expected count or identities, then assert the shared shape.
...
`fab!` uses TestProf `let_it_be` and runs outside per-example `let` setup. A `fab!` block cannot depend on `let` values or other per-example state. Keep static attributes directly in the `fab!` declaration. If a record genuinely depends on per-example setup, use `let!`; do not create a partial record with `fab!` and mutate it in a `before` block as a workaround.
