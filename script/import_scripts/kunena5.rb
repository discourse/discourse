  ...
  def import_posts
    ...
    batches(batch_size) do |offset|
      ...
      create_posts(results, total: total_count, offset: offset) do |m|
        ...
        while m["message"] =~ /\[attachment=[0-9]+\](.+?)\[\/attachment\]/ do
          attachmatch = /\[attachment=[0-9]+\](.+?)\[\/attachment\]/.match(m["message"])
          attachmentid = /\[attachment=[0-9]+\]/.match(m["message"]).to_a[0].delete('^0-9')
          attachmentrec = @client.query("
            SELECT a.id a_id, 
                   a.protected, 
                   a.folder, 
                   a.filename, 
                   a.filetype,
                   a.filename_real 
            FROM  #{KUNENA_PREFIX}kunena_attachments a
            WHERE a.id = #{attachmentid}
            ", cache_rows: false).first

          if attachmentrec == nil 
            break
          end
          #puts attachmentrec["folder"] + "\/" + attachmentrec["filename_real"]

          a_url = URI.encode("#{IMAGE_PREFIX}/" + attachmentrec["folder"] + "\/" + attachmentrec["filename_real"])
          if ["jpg", "image/png", "image/jpg", "image/gif", "image/jpeg"].include?(attachmentrec["filetype"])
            m["message"] = m["message"].gsub(attachmatch[0], "![" + attachmentrec["filename_real"] + "](" + a_url + ")")
          else
            m["message"] = m["message"].gsub(attachmatch[0], a_url)
          end
        end
