...
class ImportScripts::FluxBB < ImportScripts::Base
  ...
  def import_groups
    ...
    results = postgres_query(
      "SELECT g_id id, g_title as name, g_user_title as title
       FROM #{FLUXBB_PREFIX}groups")

    customgroups = results.select { |group| group['id'].to_i > 2 }
  ...
  def import_users
    ...
    total_count = postgres_query("SELECT count(*) count FROM #{FLUXBB_PREFIX}users;").first['count']

    batches(BATCH_SIZE) do |offset|
      results = postgres_query(
        "SELECT id, username, realname as name, url as website, email as email, registered as created_at,
                registration_ip as registration_ip_address, last_visit as last_visit_time,
                last_email_sent as last_emailed_at, location, group_id
         FROM #{FLUXBB_PREFIX}users
         LIMIT #{BATCH_SIZE}
         OFFSET #{offset};")

      break if results.cmd_tuples() < 1
      ...
      create_users(results, total: total_count, offset: offset) do |user|
        { id: user['id'],
          ...
          name: user['name'],
          created_at: Time.zone.at(user['created_at'].to_i),
          website: user['website'],
          registration_ip_address: user['registration_ip_address'],
          last_seen_at: Time.zone.at(user['last_visit_time'].to_i),
          last_emailed_at: user['last_emailed_at'] == nil ? 0 : Time.zone.at(user['last_emailed_at'].to_i),
          location: user['location'],
          moderator: user['group_id'] == 2,
          admin: user['group_id'] == 1 }
      end

      groupusers = results.select { |user| user['group_id'].to_i > 2 }
  ...
  def import_categories
    ...
    categories = postgres_query("
                              SELECT id, cat_name as name, disp_position as position
    ...
    children_categories = postgres_query("
                                       SELECT id, forum_name as name, forum_desc as description, disp_position as position, cat_id as parent_category_id
  ...
  def import_posts
    ...
    total_count = postgres_query("SELECT count(*) as count from #{FLUXBB_PREFIX}posts").first["count"]

    batches(BATCH_SIZE) do |offset|
      results = postgres_query("
        SELECT p.id as id,
               t.id as topic_id,
               t.forum_id as category_id,
               t.subject as title,
               t.first_post_id as first_post_id,
               p.poster_id as user_id,
               p.message as raw,
               p.posted as created_at
  ...
  def suspend_users
    ...
    banned = 0
    failed = 0
    total = postgres_query("SELECT count(*) count FROM #{FLUXBB_PREFIX}bans").first['count']

    system_user = Discourse.system_user

    postgres_query("SELECT username, email FROM #{FLUXBB_PREFIX}bans").each do |b|
