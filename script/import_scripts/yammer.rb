...
class ImportScripts::Yammer < ImportScripts::Base
  ...
  def create_developer_users
    GlobalSetting.developer_emails.split(',').each do |e|
      User.create(email: e, active: true, username: e.split('@')[0])
    end
  end
      ...
      create_posts(rows, total: total_count, offset: offset) do |row|
        {
          ...
          title: row['title'].present? ? row['title'] : row['raw'].split(/\W/)[0..(NUM_WORDS_IN_TITLE - 1)].join(' '),
          raw: normalize_raw(row['raw']),
          category: IMPORT_GROUPS_AS_TAGS ? base_category.id : category_id_from_imported_category_id(row['category_id']),
          user_id: user_id_from_imported_user_id(row['user_id']) || Discourse.system_user.id,
          created_at: row['created_at'],
          closed: row['closed'] == 1,
