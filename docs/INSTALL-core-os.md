**Set up Discourse on CoreOS in under 30 minutes** with zero knowledge of Rails or Linux shell using our [Discourse Docker image][dd]. We recommend [DigitalOcean][do], but these steps will work on a CoreOS server.

# Create New CoreOS Server
...
Create your new Droplet. You must setup a SSH key because CoreOS is all about security so it uses no password but a SSH key auth instead.

# Access Your Cloud Server

Connect to your server via SSH, or use [Putty][put] on Windows:

    ssh core@192.168.1.1

Replace `192.168.1.1` with the IP address of your server.

You will be asked for permission to connect, type `yes`, then enter the ssh key passphrase from when you generated your SSH key.


# Install Docker / Git

    Docker is already installed on CoreOS
...
Create a `/var/discourse` folder, clone the [Official Discourse Docker Image][dd] into it, and make a copy of the config file as `app.yml`:
   ...
   /bin/toolbox (This will enter a docker instance to write and edit files on your server located at (/media/root/ on the node)

   dnf -y install nano

   dnf -y install git
   
   mkdir /media/root/var/discourse
   
   git clone https://github.com/discourse/discourse_docker.git /media/root/var/discourse
   
   cd /media/root/var/discourse
...
- For proper email deliverability, you must set the [SPF and DKIM records](http://help.mandrill.com/entries/21751322-What-are-SPF-and-DKIM-and-do-I-need-to-set-them-up-) in your DNS. See your email provider instructions for specifics.
   ...
   Now press CTRL +  three times to exit the toolbox
