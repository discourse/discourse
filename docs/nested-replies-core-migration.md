...
| What | Core location |
|---|---|
| Module + constants + reactions batch | `lib/nested_replies.rb` |
...
- [x] **Decompose the initializer entirely**: The monolithic initializer has been broken up:
  ...
  - **PostSerializer reactions patch** -> `lib/nested_replies/post_serializer_reactions_patch.rb`
  - **DiscourseEvent handler** -> slim initializer `config/initializers/300-nested-replies.rb`
  - **Reactions batch precompute** -> moved to `TopicView.on_preload` hook in
    `lib/nested_replies/topic_view_preload.rb`
...
- [ ] **discourse-reactions coupling**: ~90 lines of SQL in `lib/nested_replies.rb`
  (`batch_precompute_reactions`) that replicate `discourse-reactions` internals, plus a
  `PostSerializer.prepend` in `lib/nested_replies/post_serializer_reactions_patch.rb`.
  Options:
  1. Request a batch preload API from discourse-reactions
  2. Test whether `TopicView.on_preload` + `reset_post_collection` lets discourse-reactions'
     own preload hook handle it naturally
...
| `spec/requests/nested_topics_controller_spec.rb` | 65 | passing |
| `spec/serializers/nested_replies_basic_category_serializer_spec.rb` | 2 | passing |
| JS unit tests (3 files) | 17 | passing |
