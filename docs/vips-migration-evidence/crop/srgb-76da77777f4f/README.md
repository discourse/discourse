...
Memory: process-tree smaps_rollup RSS/PSS at 1 ms intervals; extra memory is peak minus per-call idle, clipped at zero. Short peaks may be missed. Worker idle medians: 29.16 MiB RSS / 15.45 MiB PSS, excluded from extra memory. Warm worker PID remains constant across all cases. Owned DV Rails/Ember remain running; no fixed CPU/memory caps.
