...
result_path = ENV.fetch("RESULT_PATH", Rails.root.join("results.json").to_s)
report = {
  ...
  caller_integration: "not included; tempfile adoption and quality estimator selection remain separate",
  iterations: 31,
...
cases.each do |example|
  ...
  expected_dimensions = example.fetch("orientation") >= 5 ? input_metadata[:dimensions].reverse : input_metadata[:dimensions]
