...
directory = Rails.root.join("outputs")
...
cases.each_with_index do |example, index|
  ...
  if paths.size == 2
    ...
    if [first.width, first.height] == [second.width, second.height]
      ...
      if %w[convert_to_jpeg auto_orient].include?(operation)
        source_pixels = Vips::Image.new_from_file(source).colourspace(:srgb)
        entry[:source_visible_rgb_difference] = {}
        { "imagemagick" => first, "libvips" => second }.each do |backend, decoded|
          source_stats = (source_pixels - decoded).abs.stats
          entry[:source_visible_rgb_difference][backend] = { mean: source_stats.getpoint(4, 0).first, max: source_stats.getpoint(1, 0).first }
        end
      end
    else
