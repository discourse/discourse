...
module API
  class << self
    ...
    def edit_post(post_id:, raw:, title: nil, category: nil)
      if dry_run?
    ...
    def create_topic(external_id:, raw:, category:, title:)
      if dry_run?
      ...
      client.post(
        "/posts",
        { title: title, raw: raw, external_id: external_id, category: category },
      )
    end

    def trash_topic(topic_id:)
      if dry_run?
