...
`scan_batches` batching, and give `RawExtractor` its first production caller.
...
## 2. Prepared-body / batch API — done

Shipped in the tiering PR after all: `RawExtractor#prepare` normalizes and
classifies a body once, `scan_batches` partitions the engine-bound ones by
post count and aggregate bytes, and `extract_prepared` consumes the scan
data. The harness drives exactly that loop, so the posts step can copy it.
