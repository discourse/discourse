class SqlBuilder
  def initialize(template, klass = nil)
    Discourse.deprecate(
      'SqlBuilder is deprecated and will be removed, please use DB.build instead!'
    )
  ...
  class ActiveRecordTypeMap < PG::BasicTypeMapForResults
    def initialize(connection)
      ...
      add_coder RailsDateTimeDecoder.new(
                  name: 'timestamp', oid: 1114, format: 0
                )
  ...
  def map_exec(klass = OpenStruct, args = {})
    ...
    setters = results.fields.each_with_index.map { |f, index| f.dup << '=' }

    values = results.values
    values.map! do |row|
      mapped = klass.new
      setters.each_with_index { |name, index| mapped.send name, row[index] }
      mapped
    end
  end
end
