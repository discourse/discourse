...
module Onebox
  module Helpers
    ...
    class << self
      ...
      def fetch_content_length(location)
        uri = URI(location)
      ...
      def click_to_scroll_div(width = 690, height = 400)
      ...
      def sanitize(value, length = 50)
        return nil if value.blank?
        Sanitize.fragment(value).strip
      end

      def normalize_url_for_output(url)
        return "" unless url
        url = url.dup
      ...
      def get_absolute_image_url(src, url)
