...
module FlagQuery
  ...
  def self.flagged_posts_report(current_user, opts = nil)
    ...
    post_ids_relation =
      actions.limit(per_page).offset(offset).group(:post_id).order(
        'MIN(post_actions.created_at) DESC'
      )
    ...
    post_actions =
      actions.order('post_actions.created_at DESC').includes(
        related_post: { topic: { ordered_posts: :user } }
      )
      ...
      if pa.related_post && pa.related_post.topic
        ...
        if response = related_topic.ordered_posts[0]
          conversation[:response] = {
            excerpt: excerpt(response.cooked), user_id: response.user_id
          }
          user_ids << response.user_id
          if reply = related_topic.ordered_posts[1]
            conversation[:reply] = {
              excerpt: excerpt(reply.cooked), user_id: reply.user_id
            }
        ...
        action.merge!(
          permalink: related_topic.relative_url, conversation: conversation
        )
      end
    ...
    post_custom_fields =
      Post.custom_fields_for_ids(post_ids, post_custom_field_names)
    ...
    posts.map! do |post|
      result = post.to_h
      result[:custom_fields] = cfs if cfs = post_custom_fields[post.id]
      result
    end
    ...
    User.preload_custom_fields(
      users,
      User.whitelisted_user_custom_fields(guardian)
    )
  ...
  def self.flagged_post_actions(opts = nil)
    ...
    if opts[:filter] == 'old'
      post_actions.where(
        'post_actions.disagreed_at IS NOT NULL OR
                          post_actions.deferred_at IS NOT NULL OR
                          post_actions.agreed_at IS NOT NULL'
      )
    else
      post_actions.active.where('posts.deleted_at' => nil).where(
  ...
  def self.flagged_topics
    ...
    flagged_topics = ft_by_id.values.select(&:meets_minimum)
    Topic.preload_custom_fields(all_topics, TopicList.preloaded_custom_fields)

    { flagged_topics: flagged_topics, users: User.where(id: user_ids) }
  end
