...
class S3Inventory
    ...
    DistributedMutex.synchronize("s3_inventory_list_missing_#{inventory_id}") do
      ...
      ActiveRecord::Base.transaction do
        begin
          ...
          connection.exec(
            "CREATE TEMP TABLE #{table_name}(key text UNIQUE, etag text, PRIMARY KEY(etag, key))"
          )
          connection.copy_data("COPY #{table_name} FROM STDIN CSV") do
            files.each do |file|
              CSV.foreach(file[:filename][0...-3], headers: false) do |row|
                connection.put_copy_data(
                  "#{row[CSV_KEY_INDEX]},#{row[CSV_ETAG_INDEX]}\n"
                )
              end
            end
          end
          ...
          connection.async_exec(
            ...
            FROM #{table_name}
            WHERE #{model
              .table_name}.etag IS NULL
              AND url ILIKE '%' || #{table_name}.key"
          )

          uploads =
            if (model == Upload)
              model.by_users.where('created_at < ?', inventory_date)
            else
              model
            end
          missing_uploads =
            uploads.joins(
              "LEFT JOIN #{table_name} ON #{table_name}.etag = #{model
                .table_name}.etag"
            )
          ...
          if (missing_count = missing_uploads.count) > 0
            ...
            log "#{missing_count} of #{uploads.count} #{model.name.underscore
  ...
  def download_inventory_files_to_tmp_directory
    files.each do |file|
      ...
      failure_message =
        "Failed to inventory file '#{file[:key]}' to tmp directory."
  ...
  def decompress_inventory_files
    FileUtils.cd(tmp_directory) do
      files.each do |file|
        log "Decompressing inventory file '#{file[
              :filename
        ...
        Discourse::Utils.execute_command(
          ...
          file[:filename],
          failure_message:
            "Failed to decompress inventory file '#{file[:filename]}'."
        )
      end
    end
  end
  ...
  def files
    @files ||=
      begin
        symlink_file =
          unsorted_files.sort_by { |file| -file.last_modified.to_i }.first
  ...
  def tmp_directory
    @tmp_directory ||=
      begin
        ...
        directory =
          File.join(Rails.root, 'tmp', INVENTORY_PREFIX, current_db, timestamp)
...

    {
      destination: {
        s3_bucket_destination: {
          bucket: bucket_arn, prefix: inventory_path, format: 'CSV'
        }
      },
  ...
  def unsorted_files
    ...
    @s3_helper.list(hive_path).each do |obj|
      objects << obj if obj.key.match?(/symlink\.txt$/i)
    end
