...
class TextCleaner
  def self.title_options
...

    {
      ...
      replace_all_upper_case:
        SiteSetting.title_prettify && !SiteSetting.allow_uppercase_posts,
      ...
      remove_extraneous_space:
        SiteSetting.title_prettify && SiteSetting.default_locale == 'en',
  ...
  def self.clean(text, opts = {})
    ...
    if opts[:replace_all_upper_case] && (text == text.mb_chars.upcase)
    ...
    if first && opts[:capitalize_first_letter] &&
       first == first.mb_chars.downcase
      text = "#{first.mb_chars.capitalize}#{rest ? ' ' + rest : ''}"
    end
    # Remove unnecessary periods at the end
    if opts[:remove_all_periods_from_the_end]
      text.sub!(/([^.])\.+(\s*)\z/, "\1\2")
    end
