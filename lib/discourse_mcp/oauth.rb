  ...
  module OAuth
    class ClientResolver
      ...
      def self.resolve!(client_id, force: false, user:)
        ...
        if SiteSetting.mcp_oauth_client_id_metadata_policy == "approved_domains" &&
             !approved_domain?(uri.host)
          raise Discourse::InvalidAccess
        end
      ...
      def self.revoke_disallowed_clients!
        McpOauthClient
          ...
          .in_batches do |clients|
            disallowed_ids =
              clients.select { |client| !allowed_by_current_policy?(client) }.map(&:id)
      ...
      def self.valid_client_id_uri?(uri)
        path_has_dot_segment =
          uri.path.split("/", -1).any? { |segment| %w[. ..].include?(segment.gsub(/%2e/i, ".")) }
        uri.scheme == "https" && uri.host.present? && uri.userinfo.blank? && uri.fragment.blank? &&
          uri.path.present? && uri.path != "/" && !path_has_dot_segment
      end
    ...
    class AuthorizationGrant
      def self.grantable_scopes(user:, requested_scopes:)
        ...
        raise Discourse::InvalidAccess if !scopes.include?(DiscourseMcp::INITIAL_SCOPE)
