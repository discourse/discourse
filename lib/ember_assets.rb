...
class EmberAssets < ActiveSupport::CurrentAttributes
  ...
  class << self
    ...
    def script_chunks(exception: true)
    ...
    def deep_imports_for(chunk_filename:, chunks:, seen: Set.new)
          ...
          if Process.clock_gettime(Process::CLOCK_MONOTONIC) >= deadline
            return(
              data.merge(
                ...
                "error" =>
                  plain_error(
                    "Frontend build did not complete within #{BUILD_WAIT_TIMEOUT.to_i}s.",
                  ),
              )
            )
          end
