...
class AdminUserIndexQuery
  ...
  SORTABLE_MAPPING = {
    'created' => 'created_at',
    'last_emailed' =>
      "COALESCE(last_emailed_at, to_date('1970-01-01', 'YYYY-MM-DD'))",
  ...
  def initialize_query_with_order(klass)
    ...
    if custom_order.present? &&
       without_dir =
         SORTABLE_MAPPING[custom_order.downcase.sub(/ (asc|desc)$/, '')]
    ...
    if !custom_order.present?
      if params[:query] == 'active'
        order <<
          "COALESCE(users.last_seen_at, to_date('1970-01-01', 'YYYY-MM-DD')) DESC"
      else
  ...
  def suspect_users
    @query.activated.human_users.joins(:user_profile, :user_stat).where(
      'users.created_at <= ?',
      1.day.ago
    )
      .where("LENGTH(COALESCE(user_profiles.bio_raw, '')) > 0")
      .where(
      'user_stats.posts_read_count <= 1 AND user_stats.topics_entered <= 1'
    )
  end
  ...
  def filter_by_search
    ...
    if filter.present?
      ...
      if ip =
        ...
        @query.where(
          'ip_address <<= :ip OR registration_ip_address <<= :ip',
          ip: ip.to_cidr_s
        )
      else
