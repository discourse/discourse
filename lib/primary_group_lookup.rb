...
class PrimaryGroupLookup
  ...
  def self.lookup_columns
    @lookup_columns ||= %i{id name flair_icon flair_image_id flair_bg_color flair_color}
  end
  ...
  def user_lookup_hash
    ...
    Group.includes(:flair_image).where(id: group_ids).select(self.class.lookup_columns)
