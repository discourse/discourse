...
module BackupRestore
  ...
  class Restorer
    ...
    def self.pg_produces_portable_dump?(version)
      ...
      %w[10.3 9.6.8 9.5.12 9.4.17 9.3.22].each do |unportable_version|
        if Gem::Dependency.new('', "~> #{unportable_version}").match?(
           '',
           version
         )
          return false
    ...
    def ensure_restore_is_enabled
      unless Rails.env.development? || SiteSetting.allow_restore?
    ...
    def initialize_state
      ...
      @tmp_directory =
        File.join(Rails.root, 'tmp', 'restores', @current_db, @timestamp)
    ...
    def wait_for_sidekiq
      ...
      while sidekiq_has_running_jobs?
        ...
        iterations += 1
        if iterations > 6
          raise 'Sidekiq did not finish running all the jobs in the allowed time!'
        end
      end
    end
    ...
    def unzip_archive
      ...
      FileUtils.cd(@tmp_directory) do
        Discourse::Utils.execute_command(
          ...
          '--decompress',
          @archive_filename,
          failure_message: 'Failed to unzip archive.'
        )
      end
    end
    ...
    def get_dumped_by_version
      output =
        Discourse::Utils.execute_command(
          ...
          failure_message:
            'Failed to check version of pg_dump used to generate the dump file'
        )
    ...
    def restore_dump
      ...
      IO.popen(restore_dump_command) do |pipe|
        begin
          logs << line while line = pipe.readline
        rescue EOFError
          # finished reading...
        ensure
          psql_running = false
    ...
    def psql_command
      ...
      if db_conf.password.present?
        password_argument = "PGPASSWORD='#{db_conf.password}'"
      end
      host_argument = "--host=#{db_conf.host}" if db_conf.host.present?
      port_argument = "--port=#{db_conf.port}" if db_conf.port.present?
      if db_conf.username.present?
        username_argument = "--username=#{db_conf.username}"
      end

      [
        password_argument,
        # pass the password to psql (if any)
        'psql',
        # the psql command
        "--dbname='#{db_conf.database}'",
        # connect to database *dbname*
        '--single-transaction',
        # all or nothing (also runs COPY commands faster)
        host_argument,
        # the hostname to connect to (if any)
        port_argument,
    ...
    def migrate_database
      ...
      if Discourse.skip_post_deployment_migrations?
        ...
        Rails.application.config.paths['db/migrate'] <<
          Rails.root.join(Discourse::DB_POST_MIGRATE_PATH).to_s
      end
    ...
    def extract_uploads
      if system(
        ...
        FileUtils.cd(public_uploads_path) do
          ...
          tmp_uploads_path =
            Dir.glob(File.join(@tmp_directory, 'uploads', '*')).first
          previous_db_name = File.basename(tmp_uploads_path)
          current_db_name = RailsMultisite::ConnectionManagement.current_db
          optimized_images_exist =
            File.exist?(File.join(tmp_uploads_path, 'optimized'))
    ...
    def generate_optimized_images
      ...
      User.where('uploaded_avatar_id IS NOT NULL').find_each do |user|
        Jobs.enqueue(
          :create_avatar_thumbnails,
          upload_id: user.uploaded_avatar_id, user_id: user.id
        )
      end
    end
    ...
    def notify_user
      if user = User.find_by_email(@user_info[:email])
        ...
        log "Could not send notification to '#{@user_info[
              :username
            ]}' (#{@user_info[:email]}), because the user does not exists..."
      end
    rescue => ex
    ...
    def publish_log(message, timestamp)
      ...
      MessageBus.publish(
        ...
        data,
        user_ids: [@user_id], client_ids: [@client_id]
      )
    end
