...
# ~3s on this dev box (and similar on CI under bootsnap), all on the critical
# path before workers can be spawned. Skipping it shaves ~2.5s of master boot
# wall — measurable in `step_sec` because workers can't start `bundle exec
# rspec` until master returns from `check_for_migrations`. The few
# Rails-coupled pieces in master (the `Rails.root.to_s` references in
# `BaseFormatter` and `Flaky::Manager`, plus `check_for_migrations`'s
# ActiveRecord usage) are handled in their respective files: the formatters
# now use `Dir.pwd` (correct because `bin/turbo_rspec` always runs from the
# Discourse project root), and the migration check is opt-out via
...
#
# We still need `ActiveSupport::CoreExt` for `.present?`/`.blank?` calls in
# the formatters; that's a 0.3s require vs Rails' full 3s app boot.
