...
# ~2.5–3 s on the dev box (3–5 s on a cold-cache CI runner), all on the
# critical path before workers can be spawned. The few Rails-coupled pieces in
# master are handled in their files: `lib/turbo_tests/base_formatter.rb` and
# `lib/turbo_tests/flaky/manager.rb` switched from `Rails.root` to `Dir.pwd`
# (correct because `bin/turbo_rspec` always runs from the Discourse project
# root), and `lib/turbo_tests/runner.rb`'s migration check is opt-out via
# `DISCOURSE_TURBO_SKIP_MIGRATION_CHECK=1` (set in `.github/workflows/tests.yml`
# because CI runs `bin/rake parallel:migrate` before tests start). Local
# `bin/turbo_rspec` invocations still load Rails lazily inside
# `check_for_migrations` so the developer-friendly "pending migrations"
# diagnostic is preserved when the env var is unset.
#
# `active_support` is still needed for `.present?` / `.blank?` /
# `.symbolize_keys` calls in the formatters, runner, and `FakeExample`
# below — that's a ~0.3 s require, not Rails' full ~2.5 s app boot.
...
# Progress); JsonExample is referenced by JsonRowsFormatter; Flaky::Manager is
# referenced by FailuresLoggerFormatter.
