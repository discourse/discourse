...
module LlmsTxt
  ...
  class << self
    def generate
      I18n.with_locale(SiteSetting.default_locale) do
        ...
        if !SiteSetting.login_required
          ...
          if Guardian.new.can_search?
            public_links.unshift(translated_local_link(:search, "/search"))
          end
    ...
    def section(heading, links)
      "## #{heading}\n\n#{links.join("\n")}"
    end

    def translated_link(key, target)
      label = translated_text("links.#{key}.label")
      note = translated_text("links.#{key}.note")
      "- [#{label}](#{target}): #{note}"
    end

    def translated_local_link(key, route)
