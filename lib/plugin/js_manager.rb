...
module Plugin
  class JsManager
    ...
    # Written next to each plugin's manifest, then merged into one digested asset the
    # bundle analyzer reads. Plugins compile independently, so the merge is what gives
    # the analyzer a single, current view of all of them.
    BUNDLE_ANALYSIS_NAME = "bundle-analysis.json"
    BUNDLE_ANALYSIS_DIR = "app/assets/generated/bundle-analysis"
    ...
    def compile!
      ...
      merge_bundle_analysis!

      log "Finished initial compilation of plugins in #{(Time.now - start).round(2)}s"
    end

    # Merges each plugin's report into one asset, named by a digest of its contents so the
    ...
    def compile_js_bundle(plugin)
      ...
      files_exist =
        File.exist?("#{base_output_dir}/#{BUNDLE_ANALYSIS_NAME}") &&
      ...
      if !cache? || !files_exist
        ...
        File.write(
          "#{base_output_dir}/#{BUNDLE_ANALYSIS_NAME}",
          JSON.generate(
