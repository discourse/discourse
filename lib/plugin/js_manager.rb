...
module Plugin
  class JsManager
    ...
    # Stub for a missing plugin that was imported `with { optional: "true" }`.
    # Every lookup resolves to null so optional consumers can guard at runtime.
    FAKE_PLUGIN_MODULE =
      "data:text/javascript," +
        ERB::Util.url_encode(
          "/* autogenerated missing optional plugin stub */const m=new Proxy({},{get:()=>null});export default new Proxy({},{get:()=>m});",
        )

    # Stub for a missing plugin that was imported without `optional: "true"`.
    # It declares a default export so the importing module still links, then
    # throws as soon as it is evaluated, surfacing a helpful message at boot
    # instead of a cryptic module-resolution error.
    def self.missing_required_plugin_module(plugin_name)
      message =
        "Plugin '#{plugin_name}' is imported by another bundle, but it is not installed on this site. " \
          "If this dependency is optional, import it with `with { optional: \"true\" }`."
      "data:text/javascript," +
        ERB::Util.url_encode(
          "/* autogenerated missing required plugin stub */" \
            "export default null;throw new Error(#{message.to_json});",
        )
    end
