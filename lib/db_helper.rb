...
class DbHelper
  ...
  TRUNCATABLE_COLUMNS = %w[bookmarks.name topic_links.url]
  ...
  class << self
    def remap(
      ...
      text_columns.each do |table, columns|
        ...
        begin
          ...
          raise_contextualized_transform_exception(
            e,
            table,
            query_parts[:length_constrained_columns],
          )
        end
    ...
    def find(needle, anchor_left: false, anchor_right: false, excluded_tables: [])
      found = {}
  ...
  class << self
    ...
    def find_text_columns(excluded_tables)
    ...
    def build_transform_query_parts(table, columns, skip_max_length_violations, transforms)
    ...
    def skipped_transform_counts(table, query_parts, skip_max_length_violations, params)
    ...
    def log_transform_result(table, rows_updated, skipped_counts)
    ...
    def execute_transform(table, query_parts, params)
      DB.exec(<<~SQL, params)
      UPDATE "#{table}"
    ...
    def skipped_remap_counts(table, from, to, pattern, query_parts, skip_max_length_violations)
      return unless skip_max_length_violations && query_parts[:skipped_sums].any?
    ...
    def raise_contextualized_transform_exception(error, table, columns)
