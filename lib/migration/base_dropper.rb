...
module Migration
  class BaseDropper
    ...
    class << self
      def create_readonly_function(table_name, column_name = nil)
        DB.exec <<~SQL
      ...
      def drop_readonly_function(table_name, column_name = nil)
        DB.exec(
          "DROP FUNCTION IF EXISTS #{readonly_function_name(table_name, column_name)} CASCADE",
        )
      end

      def readonly_function_name(table_name, column_name = nil, with_schema: true)
      ...
      def old_readonly_function_name(table_name, column_name = nil)
        readonly_function_name(table_name, column_name).sub("#{FUNCTION_SCHEMA_NAME}.", "")
      end

      def readonly_trigger_name(table_name, column_name = nil)
        [table_name, column_name, "readonly"].compact.join("_")
      end

      def function_schema_exists?
        DB.exec(<<~SQL).to_s == "1"
        SELECT schema_name
      ...
      def existing_discourse_function_names
        DB.query_single(<<~SQL)
