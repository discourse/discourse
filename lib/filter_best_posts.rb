class FilterBestPosts
  ...
  def filter_posts_liked_by_moderators
    ...
    liked_by_moderators =
      liked_by_moderators.joins(:user).where('users.moderator').pluck(:post_id)
    @filtered_posts = @filtered_posts.where(id: liked_by_moderators)
  end

  def setup_posts
    @posts =
      @filtered_posts.order('percent_rank asc, sort_order asc').where(
        'post_number > 1'
      )
    @posts =
      @posts.includes(:reply_to_user).includes(:topic).joins(:user).limit(
        @limit
      )
  end
