...
class TextSentinel
  ...
  def self.body_sentinel(text, opts = {})
    ...
    if opts[:private_message]
      scale_entropy =
        SiteSetting.min_personal_message_post_length.to_f /
          SiteSetting.min_post_length.to_f
      entropy = (entropy * scale_entropy).to_i
      if entropy > SiteSetting.min_personal_message_post_length
        entropy =
          (SiteSetting.min_personal_message_post_length.to_f * ENTROPY_SCALE)
            .to_i
      end
    else
      if entropy > SiteSetting.min_post_length
        entropy = (SiteSetting.min_post_length.to_f * ENTROPY_SCALE).to_i
      end
    end
  ...
  def self.title_sentinel(text)
    ...
    TextSentinel.new(
      text,
      min_entropy: entropy, max_word_length: SiteSetting.title_max_word_length
    )
  end
  ...
  def entropy
    ...
    @entropy ||=
      chars.pack('M*' * chars.size).gsub("\n", '').split('=').uniq.size
  end

  def valid?
    @text.present? && seems_meaningful? && seems_pronounceable? &&
      seems_unpretentious? &&
      seems_quiet?
  end
  ...
  def seems_unpretentious?
    ...
    @opts[:max_word_length].blank? ||
      @text.split(%r{\s|\/|-|\.|:}).map(&:size).max <= @opts[:max_word_length]
  end

  def seems_quiet?
    # We don't allow all upper case content
    SiteSetting.allow_uppercase_posts ||
