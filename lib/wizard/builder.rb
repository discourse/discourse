...
class Wizard
  class Builder
    ...
    def build
      unless SiteSetting.wizard_enabled? && @wizard.user.try(:staff?)
        return @wizard
      end
      ...
      @wizard.append_step('forum-title') do |step|
        step.add_field(
          id: 'title', type: 'text', required: true, value: SiteSetting.title
        )
        step.add_field(
          id: 'site_description',
          type: 'text',
        ...
        step.add_field(
          id: 'short_site_description',
          type: 'text',
          required: false,
          value: SiteSetting.short_site_description
        )
      ...
      @wizard.append_step('introduction') do |step|
        ...
        if @wizard.completed_steps?('introduction') && !introduction.get_summary
          step.disabled = true
          step.description_vars = {
            topic_title: I18n.t('discourse_welcome_topic.title')
          }
        else
          step.add_field(
            id: 'welcome',
            type: 'textarea',
            required: true,
            value: introduction.get_summary
          )
          ...
          step.on_update do |updater|
            ...
            if value.index("\n")
              updater.errors.add(
                :welcome,
                I18n.t('wizard.step.introduction.fields.welcome.one_paragraph')
              )
            else
      ...
      @wizard.append_step('privacy') do |step|
        ...
        step.on_update do |updater|
          updater.update_setting(
            :login_required,
            updater.fields[:privacy] == 'restricted'
          )
          updater.update_setting(
            :invite_only,
            updater.fields[:privacy] == 'restricted'
      ...
      @wizard.append_step('contact') do |step|
        step.add_field(
          ...
          type: 'text',
          required: true,
          value: SiteSetting.contact_email
        )
        step.add_field(
          id: 'contact_url', type: 'text', value: SiteSetting.contact_url
        )

        username = SiteSetting.site_contact_username
        username = Discourse.system_user.username if username.blank?
        contact =
          step.add_field(id: 'site_contact', type: 'dropdown', value: username)
      ...
      @wizard.append_step('corporate') do |step|
        ...
        step.add_field(
          id: 'company_name', type: 'text', value: SiteSetting.company_name
        )
        step.add_field(
          id: 'governing_law', type: 'text', value: SiteSetting.governing_law
        )
        step.add_field(
          id: 'city_for_disputes',
          type: 'text',
          value: SiteSetting.city_for_disputes
        )
      ...
      @wizard.append_step('colors') do |step|
        ...
        step.on_update do |updater|
          ...
          name =
            I18n.t(
              "color_schemes.#{scheme_name.downcase.gsub(' ', '_')}_theme_name"
            )

          theme = nil
          scheme =
            ColorScheme.find_by(base_scheme_id: scheme_name, via_wizard: true)
          scheme ||=
            ColorScheme.create_from_base(
              name: name, via_wizard: true, base_scheme_id: scheme_name
            )
          ...
          theme ||=
            Theme.create(
              name: name, user_id: @wizard.user.id, color_scheme_id: scheme.id
            )
          theme.set_default!
        end
      end
      ...
      @wizard.append_step('logos') do |step|
        step.add_field(
          id: 'logo', type: 'image', value: SiteSetting.site_logo_url
        )
        step.add_field(
          id: 'logo_small',
          type: 'image',
          value: SiteSetting.site_logo_small_url
        )

        step.on_update { |updater| updater.apply_settings(:logo, :logo_small) }
      end

      @wizard.append_step('icons') do |step|
        step.add_field(
          id: 'favicon', type: 'image', value: SiteSetting.site_favicon_url
        )
        step.add_field(
          id: 'apple_touch_icon',
          type: 'image',
          value: SiteSetting.site_apple_touch_icon_url
        )
        ...
        step.on_update do |updater|
          ...
          if updater.fields[:apple_touch_icon] != SiteSetting.apple_touch_icon
            ...
            if upload && upload.width > dimensions && upload.height > dimensions
              ...
              apple_touch_icon_optimized =
                OptimizedImage.create_for(upload, dimensions, dimensions)

              original_file =
                begin
                  File.new(Discourse.store.path_for(apple_touch_icon_optimized))
              ...
              if original_file
                apple_touch_icon_upload =
                  UploadCreator.new(original_file, upload.original_filename)
                    .create_for(@wizard.user.id)
                updater.update_setting(
                  :apple_touch_icon,
                  apple_touch_icon_upload
                )
      ...
      @wizard.append_step('emoji') do |step|
        ...
        emoji = %w[smile +1 tada poop]

        EmojiSetSiteSetting.values.each do |set|
          imgs =
            emoji.map do |e|
              "<img src='#{Discourse.base_uri}/images/emoji/#{set[
                :value
              ]}/#{e}.png'>"
            end
          ...
          step.on_update { |updater| updater.apply_settings(:emoji_set) }
        end
      end
      ...
      @wizard.append_step('invites') do |step|
        if SiteSetting.enable_local_logins
          staff_count =
            User.staff.human_users.where(
              'username_lower not in (?)',
              reserved_usernames
            )
              .count
          step.add_field(
            id: 'staff_count', type: 'component', value: staff_count
          )
    ...
    def replace_setting_value(updater, raw, field_name)
      ...
      raw.gsub!("<ins>#{old_value}</ins>", new_value) ||
        raw.gsub!(old_value, new_value)
    end

    def reserved_usernames
      @reserved_usernames ||=
        SiteSetting.defaults[:reserved_usernames].split('|')
    end

    def update_tos
      tos_post =
        Post.find_by(topic_id: SiteSetting.tos_topic_id, post_number: 1)
