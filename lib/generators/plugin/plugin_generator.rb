...
class PluginGenerator < Rails::Generators::NamedBase
  ...
  class_option :controller,
               type: :boolean, desc: 'Generate controller', default: true
  class_option :spec, type: :boolean, desc: 'Generate spec', default: true
  class_option :acceptance,
               type: :boolean, desc: 'Generate acceptance test', default: true
  class_option :stylesheet,
               type: :boolean, desc: 'Generate Stylesheet', default: true
  class_option :javascript,
               type: :boolean,
               desc: 'Generate Javascript initializer',
               default: true
  class_option :scheduled_job,
               type: :boolean, desc: 'Generate scheduled job', default: false
  class_option :help,
               type: :boolean,
               desc: 'Adds help comments in generated files',
               default: true

  def create_acceptance_file
    return unless @options['acceptance']

    template 'acceptance-test.js.es6.erb',
             File.join(
               'plugins',
               dasherized_name,
               'test/javascripts/acceptance',
               "#{dasherized_name}-test.js.es6"
             )
  end
  ...
  def create_stylesheet_file
    ...
    template 'stylesheet.scss.erb',
             File.join(
               'plugins',
               dasherized_name,
               'assets/stylesheets/common',
               "#{dasherized_name}.scss"
             )
  end
  ...
  def create_settings_file
    template 'settings.yml.erb',
             File.join('plugins', dasherized_name, 'config', 'settings.yml')
  end

  def create_locales_file
    template 'client.en.yml.erb',
             File.join(
               'plugins',
               dasherized_name,
               'config/locales',
