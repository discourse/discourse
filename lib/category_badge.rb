...
module CategoryBadge
  class << self
    def html_for(category, opts = nil)
      opts ||= {}
    ...
    def fetch_parent_category(category)
      Category.find_by(id: category.parent_category_id) if category.parent_category_id
    end

    def map_styles_to_string(styles)
      styles.map { |k, v| "#{k}: #{ERB::Util.html_escape(v)};" }.join(" ")
    end

    def wrap_in_link(content, url, extra_classes = "", style_value = nil)
    ...
    def style_for_browser(category, opts)
    ...
    def style_for_email(category, opts)
      data = shared_data(category, opts)
