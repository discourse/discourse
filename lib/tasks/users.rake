...
task 'users:change_post_ownership',
     %i[old_username new_username archetype] => %i[environment] do |_, args|
...
task 'users:merge',
     %i[source_username target_username] => %i[environment] do |_, args|
...
desc 'Rename a user'
task 'users:rename',
     %i[old_username new_username] => %i[environment] do |_, args|
  old_username = args[:old_username]
  new_username = args[:new_username]
...
task 'users:update_posts',
     %i[old_username current_username] => %i[environment] do |_, args|
  old_username = args[:old_username]
...
task 'users:disable_2fa', %i[username] => %i[environment] do |_, args|
  username = args[:username]
  user = find_user(username)
  UserSecondFactor.where(
    user_id: user.id, method: UserSecondFactor.methods[:totp]
  )
    .each(&:destroy!)
...
task 'users:anonymize_all' => :environment do
  ...
  confirm_anonymize =
    ask("Are you sure you want to anonymize #{total} users? (Y/n)")
...
task 'users:list_recent_staff' => :environment do
  ...
  recent_admin_ids =
    recent_actions.where(action: UserHistory.actions[:revoke_admin]).pluck(
      :target_user_id
    )
  recent_moderator_ids =
    recent_actions.where(action: UserHistory.actions[:revoke_moderation]).pluck(
      :target_user_id
    )
