...
task "annotate:clean" => :environment do |task, args|
  ...
  db.start
  db.with_env do
    system("RAILS_ENV=test LOAD_PLUGINS=0 bin/rails db:migrate:reset", exception: true)
...
task "annotate:clean:plugins", [:plugin] => :environment do |task, args|
  ...
  db.start
  db.with_env do
    system("RAILS_ENV=test LOAD_PLUGINS=1 bin/rails db:migrate:reset", exception: true)
