task 'assets:precompile:before' do
  ...
  if Rails.configuration.assets.js_compressor == :uglifier &&
     !`which uglifyjs`.empty? &&
     !ENV['SKIP_NODE_UGLIFY']
    $node_uglify = true
  end

  $bypass_sprockets_uglify = true unless ENV['USE_SPROCKETS_UGLIFY']
...
def compress_node(from, to)
  ...
  cmd =
    "uglifyjs '#{assets_path}/#{from}' -p relative -c -m -o '#{to_path}' --source-map-root '#{source_map_root}' --source-map '#{assets_path}/#{to}.map' --source-map-url '#{source_map_url}'"
...
def compress_ruby(from, to)
  ...
  uglified, map =
    Uglifier.new(
      comments: :none,
      source_map: {
        filename: File.basename(from), output_filename: File.basename(to)
      }
    )
...
def compress(from, to)
  $node_uglify ? compress_node(from, to) : compress_ruby(from, to)
end
...
def concurrent?
  ...
  if ENV['SPROCKETS_CONCURRENT'] == '1'
    ...
    yield(
      Proc.new do |&block|
        concurrent_compressors <<
          Concurrent::Future.execute(executor: executor) { block.call }
      end
    )
...
task 'assets:precompile' => 'assets:precompile:before' do
  if $bypass_sprockets_uglify
    ...
    concurrent? do |proc|
      ...
      manifest.files.select { |k, v| k =~ /\.js$/ }.each do |file, info|
        ...
        if File.exists?(_path)
          ...
          proc.call do
            ...
            unless (ENV['DONT_MINIFY'] == '1') ||
                   to_skip.include?(info['logical_path'])
            ...
            STDERR.puts "Done compressing #{file} : #{(
    ...
    if GlobalSetting.fallback_assets_path.present?
      begin
        ...
        STDERR.puts "Failed to backup assets to #{GlobalSetting
