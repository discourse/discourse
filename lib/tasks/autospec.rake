...
task 'autospec' => :environment do
  ...
  latency =
    ((ARGV.find { |a| a =~ /l=|latency=/ } || '').split('=')[1] || 3).to_i

  if force_polling
    puts "Polling has been forced (slower) - checking every #{latency} #{'second'
           .pluralize(latency)}"
  else
  ...
  Autospec::Manager.run(
    force_polling: force_polling, latency: latency, debug: debug
  )
end
