...
def write_yaml(path, task_name, template)
  header = <<~HEADER
    # DO NOT EDIT THIS FILE!!!
    # Update it by running `rake javascript:#{task_name}`
  HEADER
  ...
  basename = File.basename(path)
  output_path = "#{Rails.root}/config/#{path}"
...
task 'javascript:update' => 'clean_up' do
  ...
  dependencies.each do |f|
    ...
    if f[:public_root]
      ...
      if f[:skip_versioning]
        ...
        if package_dir_name == 'workbox'
          versions[filename] = "#{package_dir_name}/#{package_version}"
          dest = path
        else
  ...
  write_yaml("public_js_versions.yml", "update", versions.to_yaml)
