...
DISCOURSE_GIFS_REPO ||= "https://github.com/discourse/discourse-gifs"
...
desc "Migrate #{COMPONENT_NAME} theme component settings to core site settings. " \
...
task "themes:discourse_gifs:migrate" => :environment do
  ...
  components = find_discourse_gifs_components

  if components.any?
    puts "\nMigrating settings..."
    puts "---------------------"
    components.each { |c| migrate_discourse_gifs_component(c, enable_gifs: enable_gifs) }
  else
    puts "\nNo #{COMPONENT_NAME} theme component found. Nothing to migrate."
  end
end

def find_discourse_gifs_components
...
def find_component_in_db(db)
  ...
  themes =
    RemoteTheme
      .where(remote_url: [DISCOURSE_GIFS_REPO, "#{DISCOURSE_GIFS_REPO}.git"])
      .includes(theme: :theme_settings)
      .map(&:theme)

  if themes.length > 1
    puts "  \e[33mMultiple (#{themes.length}) #{COMPONENT_NAME} components found:\e[0m"
...
def migrate_discourse_gifs_component(theme, enable_gifs:)
