...
class HtmlToMarkdown
  ...
  def fix_span_elements(node)
    if (
       INLINE_ELEMENTS.include?(node.name) &&
         BLOCK_ELEMENTS.any? { |e| node.at(e) }
     )
  ...
  def remove_whitespaces!
    @doc.traverse do |node|
      if node.is_a? Nokogiri::XML::Text
        if node.previous_element&.description&.block?
          node.content = node.content.gsub(/\A[[:space:]]+/, '')
        end
        if node.previous_element.nil? && node.parent.description&.block?
          node.content = node.content.gsub(/\A[[:space:]]+/, '')
        end
        if node.next_element&.description&.block?
          node.content = node.content.gsub(/[[:space:]]+\z/, '')
        end
        if node.next_element.nil? && node.parent.description&.block?
          node.content = node.content.gsub(/[[:space:]]+\z/, '')
        end
  ...
  def visit(node)
    ...
    if node.description&.block? && node.parent&.description&.block? &&
       @stack[-1].markdown.size > 0
      block = @stack[-1].dup
      @markdown << format_block
      block.markdown = ''
      block.opened = true
  ...
  def visit_abbr(node)
    @stack[-1].markdown <<
      (node['title'].present? ? "<abbr title=\"#{node['title']}\">" : '<abbr>')
    traverse(node)
  ...
  def visit_text(node)
    if node.previous_element.nil? && EMPHASIS.include?(node.parent.name)
      node.content = node.content.gsub(/\A[[:space:]]+/, '')
    end
    indent = node.text[/^\s+/] || ''
  ...
  def format_block
    lines =
      @stack[-1].markdown.each_line.map do |line|
  ...
  def is_visible_img?(img)
    ...
    if img['style'].present? && img['style'][/(width|height)\s*:\s*0/]
      return false
    end
    true
  end
