...
module UpcomingChanges
  ...
  class ConditionalDisplay
    class << self
      def should_display?(upcoming_change_name)
      ...
      def should_display_enable_horizon_high_context_topic_cards?
      ...
      def should_display_remove_and_replace_uncategorized?
      ...
      def should_display_enable_local_logins_via_code?
      ...
      def should_display_update_pending_users_reminder_default?
  ...
  class << self
    ...
    def previous_status_value(status)
    ...
    def image_exists?(change_setting_name)
      File.exist?(File.join(Rails.public_path, image_path(change_setting_name)))
    end

    def image_path(change_setting_name)
    ...
    def image_data(change_setting_name, include_file_path: false)
    ...
    def exists?(change_setting_name)
    ...
    def owning_plugin_configurable?(change_setting_name)
      plugin_name = settings_provider.plugins[change_setting_name.to_sym]
    ...
    def requires_plugin_enabled?(change_setting_name)
      change_metadata(change_setting_name)[:requires_plugin_enabled] != false
    end

    def owning_plugin_enabled?(change_setting_name)
      change_setting_name = change_setting_name.to_sym
    ...
    # @return [Boolean]
    def enabled?(change_setting_name)
      change_setting_name = change_setting_name.to_sym
    ...
    def allow_enabled_for(change_setting_name)
    ...
    def target_allowed?(change_setting_name, target)
    ...
    def groups_target_allowed?(change_setting_name)
      allow = allow_enabled_for(change_setting_name)
      return true if allow.nil?
      allow.include?(:staff) || allow.include?(:specific_groups)
    end

    def has_groups?(change_setting_name)
      group_ids_for(change_setting_name).present?
    end

    def group_ids_for(change_setting_name)
      change_setting_name = change_setting_name.to_sym
    ...
    # @return [Boolean]
    def enabled_for_user?(change_setting_name, user)
      change_setting_name = change_setting_name.to_sym
      ...
      if user.blank?
        ...
        if UpcomingChanges.has_groups?(change_setting_name)
          return(
            setting_enabled &&
              user.in_any_groups?(UpcomingChanges.group_ids_for(change_setting_name))
          )
        end
      end
    ...
    #   #   },
    #   # ]
    def stats_for_user(user:, acting_guardian:)
    ...
    def enabled_for_with_groups(setting_name, setting_value, upcoming_change_selected_groups)
    ...
    def current_statuses_cache_key
    ...
    def current_statuses
      Discourse
        ...
        .fetch(current_statuses_cache_key) do
          results =
            DB.query(<<-SQL, status_changed: UpcomingChangeEvent.event_types[:status_changed])
    ...
    def permanent_upcoming_changes_cache_key
    ...
    def permanent_upcoming_changes
      Discourse
        .cache
    ...
    def find_dependents_for_change(change_setting_name)
    ...
    def change_dependencies_met?(change_setting_name)
    ...
    def settings_hidden_while_enabled
