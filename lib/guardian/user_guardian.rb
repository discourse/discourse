...
module UserGuardian
  def can_pick_avatar?(user_avatar, upload)
    ...
    if upload.user_id == user_avatar.user_id || upload.user_id == user.id
      return true
    end
  ...
  def can_edit_username?(user)
    if (SiteSetting.sso_overrides_username? && SiteSetting.enable_sso?)
      return false
    end
    ...
    is_me?(user) &&
      (
        (user.post_count + user.topic_count) == 0 ||
          user.created_at > SiteSetting.username_change_period.days.ago
      )
  end

  def can_edit_email?(user)
    if (SiteSetting.sso_overrides_email? && SiteSetting.enable_sso?)
      return false
    end
  ...
  def can_delete_user?(user)
    ...
    if is_me?(user)
      ...
      is_staff? &&
        (
          user.first_post_created_at.nil? || user.post_count <= 5 ||
            user.first_post_created_at >
              SiteSetting.delete_user_max_post_age.to_i.days.ago
        )
    end
  end
