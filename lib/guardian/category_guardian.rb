...
module CategoryGuardian
  ...
  def can_create_category?(parent = nil)
    is_admin? || (SiteSetting.moderators_create_categories && is_moderator?)
  end
  ...
  def can_delete_category?(category)
    can_edit_category?(category) && category.topic_count <= 0 &&
  ...
  def cannot_delete_category_reason(category)
    if category.uncategorized?
      return I18n.t('category.cannot_delete.uncategorized')
    end
    ...
    if category.topic_count != 0
      oldest_topic =
        category.topics.where.not(id: category.topic_id).order('created_at ASC')
          .limit(1)
          .first
      if oldest_topic
        return I18n.t(
          ...
          topic_link:
            "<a href=\"#{oldest_topic.url}\">#{CGI.escapeHTML(
              oldest_topic.title
            )}</a>"
        )
      else
  ...
  def can_see_category?(category)
    ...
    if is_staged? && category.email_in.present? &&
       category.email_in_allow_strangers
      return true
    end
  ...
  def topic_create_allowed_category_ids
    @topic_create_allowed_category_ids ||=
      @user.topic_create_allowed_category_ids
  end
