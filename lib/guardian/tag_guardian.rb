...
module TagGuardian
  def can_create_tag?
    ...
    user && SiteSetting.tagging_enabled &&
      user.has_trust_level?(SiteSetting.min_trust_to_create_tag.to_i)
  end

  def can_tag_topics?
    if SiteSetting.min_trust_level_to_tag_topics.to_s == 'admin'
      return is_admin?
    end
    if SiteSetting.min_trust_level_to_tag_topics.to_s == 'staff'
      return is_staff?
    end
    user && SiteSetting.tagging_enabled &&
      user.has_trust_level?(SiteSetting.min_trust_level_to_tag_topics.to_i)
  end

  def can_tag_pms?
    is_staff? && SiteSetting.tagging_enabled &&
      SiteSetting.allow_staff_to_tag_pms
  end
