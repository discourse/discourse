...
class DiscourseConnectProvider < DiscourseConnectBase
  ...
  class << self
    def parse(payload, sso_secret = nil, **init_kwargs)
      ...
      if (sso.prompt != nil) && (sso.prompt != "none")
        raise InvalidParameterValueError.new("prompt")
      end

      sso
    end

    def lookup_return_sso_url(parsed_payload)
      decoded = Base64.decode64(parsed_payload["sso"])
      decoded_hash = Rack::Utils.parse_query(decoded)
      decoded_hash["return_sso_url"]
    end

    def lookup_sso_secret(return_sso_url, parsed_payload)
