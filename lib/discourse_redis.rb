...
class DiscourseRedis
  class FallbackHandler
    ...
    def initiate_fallback_to_master
      ...
      begin
        ...
        info = master_client.call(%i[info])

        if info.include?(MASTER_LOADED_STATUS) &&
           info.include?(MASTER_ROLE_STATUS)
          begin
        ...
        logger.warn "#{log_prefix}: Connection to Master server failed with '#{e
                      .message}'"
      ensure
  ...
  class Connector < Redis::Client::Connector
    ...
    def resolve(client = nil)
      ...
      begin
        ...
        loading =
          client.call(%i[info persistence]).include?(
        ...
        loading ? @slave_options : @options
             ...
             RuntimeError => ex
        if ex.class == RuntimeError && ex.message != 'Name or service not known'
          raise ex
        end
  ...
  def scan_each(options = {}, &block)
    DiscourseRedis.ignore_readonly do
      ...
      options[:match] = @namespace ? "#{namespace}:#{match}" : match
  ...
  def flushdb
    DiscourseRedis.ignore_readonly { keys.each { |k| del(k) } }
  end
  ...
  def self.namespace
    Rails.logger.warn(
      'DiscourseRedis.namespace is going to be deprecated, do not use it!'
    )
    RailsMultisite::ConnectionManagement.current_db
  end
