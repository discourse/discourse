...
class NotificationQuery
  ...
  def compute_bulk_counts
   ...
   LEFT JOIN categories c ON c.id = t.category_id
    ...
    badges_disabled = !SiteSetting.enable_badges

    rows.each do |row|
      # Disabled badges are filtered from the displayed list, so they must not
      # be counted either — otherwise counts and the list disagree.
      next if badges_disabled && row.notification_type == badge_type
  ...
  def topic_visibility_sql(notifications: "n", topics: "t")
  ...
  def base_scope
    scope =
      Notification
        ...
        .where(topic_visibility_sql(notifications: "notifications", topics: "topics"))
