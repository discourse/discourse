module DiscourseTagging
  ...
  TAGS_FILTER_REGEXP = %r{[\/\?#\[\]@!\$&'\(\)\*\+,;=\.%\\`^\s|\{\}"<>]+} # /?#[]@!$&'()*+,;=.%\`^|{}"<>
  TAGS_STAFF_CACHE_KEY = 'staff_tag_names'

  def self.tag_topic_by_names(topic, guardian, tag_names_arg, append: false)
    if guardian.can_tag?(topic)
      tag_names =
        DiscourseTagging.tags_for_saving(tag_names_arg, guardian) || []
      ...
      unless guardian.is_staff?
        ...
        if staff_tags.present?
          topic.errors[:base] <<
            I18n.t('tags.staff_tag_disallowed', tag: staff_tags.join(' '))
          return false
        end
        ...
        if staff_tags.present?
          topic.errors[:base] <<
            I18n.t(
              'tags.staff_tag_remove_disallowed',
              tag: staff_tags.join(' ')
            )
          return false
        end
  ...
  def self.filter_allowed_tags(query, guardian, opts = {})
    ...
    if category && (category.tags.count > 0 || category.tag_groups.count > 0)
      if category.tags.count > 0 && category.tag_groups.count > 0
        ...
        # category.tag_groups.count > 0
        tag_group_ids = category.tag_groups.pluck(:id)

        query =
          query.where(
            'tags.id IN (SELECT tag_id FROM tag_group_memberships WHERE tag_group_id IN (?))',
            tag_group_ids
          )
      end
      ...
      if CategoryTagGroup.exists?
        ...
        query =
          query.where(
            'tags.id NOT IN (SELECT tag_id FROM tag_group_memberships WHERE tag_group_id IN (?))',
            tag_group_ids
          )
      end
    end
    ...
    if opts[:for_input] || opts[:for_topic]
      unless guardian.nil? || guardian.is_staff?
        all_staff_tags = DiscourseTagging.staff_tag_names
        if all_staff_tags.present?
          query = query.where('tags.name NOT IN (?)', all_staff_tags)
        end
      end
      ...
      if selected_tag_ids.empty?
        sql =
          "tags.id NOT IN (#{select_sql} WHERE tg.parent_tag_id IS NOT NULL)"
        query = query.where(sql)

        # One tag per group restriction
      else
        exclude_group_ids =
          TagGroup.where(one_per_topic: true).joins(:tag_group_memberships)
            .where('tag_group_memberships.tag_id in (?)', selected_tag_ids)
            .pluck(:id)

        if exclude_group_ids.empty?
          sql =
            "tags.id NOT IN (#{select_sql} WHERE tg.parent_tag_id NOT IN (?))"
          ...
          limit_tag_ids =
            TagGroupMembership.select('distinct on (tag_group_id) tag_id')
              .where(tag_id: selected_tag_ids)
              .where(tag_group_id: exclude_group_ids)
              .map(&:tag_id)
          sql =
            "(tags.id NOT IN (#{select_sql} WHERE (tg.parent_tag_id NOT IN (?) OR tg.id in (?))) OR tags.id IN (?))"
          query =
            query.where(sql, selected_tag_ids, exclude_group_ids, limit_tag_ids)
        end
      end
    end
  ...
  def self.filter_visible(query, guardian = nil)
    if guardian&.is_staff?
      ...
      query.where("tags.id NOT IN (#{hidden_tags_query.select(:id).to_sql})")
    end
  end
  ...
  def self.hidden_tags_query
    Tag.joins(:tag_groups).where(
      'tag_groups.id NOT IN (
        SELECT tag_group_id
    ...
    if !tag_names
      tag_names =
        Tag.joins(tag_groups: :tag_group_permissions).where(
          'tag_group_permissions.group_id = ? AND tag_group_permissions.permission_type = ?',
          Group::AUTO_GROUPS[:everyone],
