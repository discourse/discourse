...
module DiscourseTagging
  ...
  class << self
    ...
    def tag_topic(topic, guardian, tags_param)
    ...
    def tag_topic_by_names(topic, guardian, tag_names_arg, append: false)
      if guardian.can_tag?(topic)
        ...
        "
          ...
          unless validate_min_required_tags_for_category(guardian, topic, category, tags)
            return false
          end
    ...
    def tag_topic_by_ids(topic, guardian, tag_ids, append: false)
      tag_names = Tag.where(id: tag_ids).pluck(:name)
      tag_topic_by_names(topic, guardian, tag_names, append: append)
    end

    def validate_category_tags(guardian, model, category, tags = [])
    ...
    def validate_min_required_tags_for_category(guardian, model, category, tags = [])
    ...
    def validate_required_tags_from_group(guardian, model, category, tags = [])
    ...
    def restricted_category_ids_by_tag_name(tag_names)
    ...
    def tag_names_not_allowed_in_category(category, tag_names, restricted_to: nil)
    ...
    def validate_category_restricted_tags(guardian, model, category, tags = [])
    ...
    def validate_one_tag_from_group_per_topic(guardian, model, category, tags = [])
      tags_cant_be_used =
        filter_tags_violating_one_tag_from_group_per_topic(guardian, category, tags)
    ...
    def filter_tags_violating_one_tag_from_group_per_topic(guardian, category, tags = [])
    ...
    def filter_allowed_tags(guardian, opts = {})
      ...
      category_has_restricted_tags =
        category_has_tag_groups || (category && category.tags.count > 0)
    ...
    SQL
      ...
      if filter_for_non_admin
        builder.where("t.id IN (#{visible_tags(guardian).select(:id).to_sql})")
      end
      ...
      if opts[:order_recent_tag_ids].present?
        ...
        builder.order_by(
          "lower(name) = lower(:cleaned_term) DESC, #{topic_count_column} DESC, name",
        )
      end
    ...
    def visible_tags(guardian)
      ...
      permitted =
        Tag
          .where.not(
            id: TagGroupMembership.joins(tag_group: :tag_group_permissions).select(:tag_id),
          )
          .or(
            Tag.where(
              id:
    ...
    def without_pm_only_tags(tags, guardian)
    ...
    def hidden_tag_names(guardian = nil)
      hidden_tags(guardian).pluck(:name)
    end

    def permitted_group_ids_query(guardian = nil)
    ...
    def permitted_group_ids(guardian = nil)
      permitted_group_ids_query(guardian).pluck(:id)
    end

    # read-only tags for this user
    def readonly_tag_names(guardian = nil)
    ...
    def permitted_tag_names(guardian = nil)
      query =
        Tag.joins(tag_groups: :tag_group_permissions).where(
    ...
    def clean_tag(tag, truncate: true)
      tag = tag.dup
    ...
    def tag_ids_matching_localizations(term, term_type:)
    ...
    def tags_for_saving(tags_arg, guardian, opts = {})
    ...
    def find_or_create_tags!(tag_names, guardian)
    ...
    def add_or_create_tags_by_name(taggable, tag_names_arg, opts = {})
      tag_names =
    ...
    def add_or_create_synonyms(target_tag, synonym_tag_ids: [], new_synonym_names: [])
