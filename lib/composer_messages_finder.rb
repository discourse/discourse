class ComposerMessagesFinder
          ...
          PrettyText.cook(
            I18n.t(
              ...
              education_posts_text:
                I18n.t(
                  'education.until_posts',
                  count: SiteSetting.educate_until_posts
                ),
  ...
  def check_new_user_many_replies
    unless replying? && @user.posted_too_much_in_topic?(@details[:topic_id])
      return
    end
        ...
        PrettyText.cook(
          I18n.t(
            ...
            newuser_max_replies_per_topic:
              SiteSetting.newuser_max_replies_per_topic
          )
        )
    }
  end
  ...
  def check_avatar_notification
    ...
    if SiteSetting.disable_avatar_education_message ||
       SiteSetting.sso_overrides_avatar ||
...

    {
      ...
      body:
        PrettyText.cook(
          I18n.t('education.avatar', profile_path: "/u/#{@user.username_lower}")
        )
    }
  end
  ...
  def check_sequential_replies
    ...
    recent_posts_user_ids =
      Post.where(topic_id: @details[:topic_id]).where(
        'created_at > ?',
        1.day.ago
      )
  ...
  def check_dominating_topic
    ...
    if ratio < (SiteSetting.dominating_topic_minimum_percent.to_f / 100.0)
      return
    end
...

    {
      ...
      body:
        PrettyText.cook(
          I18n.t('education.dominating_topic', percent: (ratio * 100).round)
        )
    }
  end
  ...
  def check_get_a_room(min_users_posted: 5)
    ...
    last_x_replies =
      @topic.posts.where(user_id: @user.id).order('created_at desc').limit(
        SiteSetting.get_a_room_threshold
      )
  ...
  def check_reviving_old_topic
    ...
    if @topic.nil? || SiteSetting.warn_reviving_old_topic_age < 1 ||
