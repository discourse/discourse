...
module SvgSprite
  ...
  def self.svgs_for(theme_id)
    ...
    if icon_set
      ...
      svgs.merge!(source.slice(*explicitly_requested_icons(theme_id)))
...
" \
  ...
  def self.log_unresolved_icon_set_targets(theme_id, icon_set)
    ...
    Rails.logger.warn(
      "Icon set (#{icon_set["plugin"] || "theme #{icon_set["theme_id"]}"}): " \
        ...
        "#{"icon".pluralize(missing.size)}, falling back to the default: " \
  ...
  # Core icon names are alphanumerics, dashes and underscores, plus dots for
  # the "notification.*" aliases (see REPLACEMENTS in icon-library.js).
  ...
  def self.plugin_icon_set_decl(registered)
    ...
    values =
      icon_set_placeholders(map).index_with do |setting|
        SiteSetting.public_send(setting) if SiteSetting.has_setting?(setting)
      end
    decl = { "map" => map, "values" => values, "plugin" => registered[:plugin_name] }
    if (setting = registered[:ignore_setting].to_s).present? && SiteSetting.has_setting?(setting)
  ...
  def self.icon_set_theme_setting?(theme_id, setting_name)
    ...
    affected_settings = icon_set_placeholders(declaration["map"]) << ICON_SET_IGNORE_SETTING
    affected_settings.include?(setting_name.to_s)
  end
  ...
  # Icons something asked for by name, as opposed to ones that merely exist in a
  # sprite. Cached because `bundle` consults it outside the `all_icons` cache
  # block, and `theme_icons` is the one member that isn't memoized on its own.
  # Returns an Array: it round-trips through DistributedCache/MessageBus, and a
  # Set arrives on other app servers as an Array anyway.
