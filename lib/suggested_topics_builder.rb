...
class SuggestedTopicsBuilder
  ...
  def add_results(results, priority = :low)
    ...
    results =
      results.where('topics.id NOT IN (?)', @excluded_topic_ids).where(
        visible: true
      )
  ...
  def splice_results(results, priority)
    if priority == :ultra_high
      ...
      other_category_index =
        @results.index { |r| r.category_id != @category_id }
  ...
  def category_results_left
    SiteSetting.suggested_topics -
      @results.count { |r| r.category_id == @category_id }
  end
