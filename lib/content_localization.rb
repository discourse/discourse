...
class ContentLocalization
  ...
  class << self
    ...
    def show_translated_topic?(topic, scope)
      SiteSetting.content_localization_enabled && topic&.locale.present? &&
        !topic.in_user_locale? && automatically_translate?(scope) &&
        !understands?(topic.locale, scope)
    end

    def translated_topic_title(topic, scope)
      return if !show_translated_topic?(topic, scope)

      topic.get_localization&.title.presence
    end

    def translated_topic_fancy_title(topic, scope)
      return if !show_translated_topic?(topic, scope)
