...
module JsLocaleHelper
  class << self
    ...
    def reloadable_plugins(locale_sym, ctx)
    ...
    def plugin_translations(locale_str)
    ...
    def deep_delete_matches(deleting_from, checking_hashes)
    ...
    def load_translations_merged(*locales)
    ...
    def output_client_overrides(main_locale)
    ...
    def output_extra_locales(bundle, locale)
    ...
    def find_locale(locale_chain, path, type, fallback_to_english:)
    ...
    def remove_message_formats!(translations, locale)
      message_formats = {}
      I18n.fallbacks[locale]
        .map(&:to_s)
        .each do |l|
    ...
    def strip_out_message_formats!(hash, prefix = "", message_formats = {})
      if hash.is_a?(Hash)
