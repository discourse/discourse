class LocaleFileWalker
  ...
  def handle_nodes(nodes, depth, parents)
    ...
    nodes.each do |node|
      consecutive_scalars =
        handle_node(node, depth, parents, consecutive_scalars)
    end
  end
  ...
  def handle_value(value, parents); end

  def handle_scalar(node, depth, parents)
    parents[depth] = node.value
  end

  def handle_alias(node, depth, parents); end

  def handle_mapping(node, depth, parents); end
end
