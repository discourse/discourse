...
class LocaleFileChecker
  ...
  def check(locale)
    ...
    locale_files.each do |locale_path|
      ...
      @relative_locale_path =
        Pathname.new(locale_path).relative_path_from(Pathname.new(Rails.root))
          .to_s
  ...
  FORCE_PLURAL_COUNT_LOCALES = %w[bs lt lv ru sl sr uk]

  def locale_files
    YML_DIRS.map do |dir|
      Dir["#{Rails.root}/#{dir}/{client,server}.#{@locale}.yml"]
    end
      .flatten
  end
  ...
  def check_interpolation_keys
    traverse_hash(@locale_yaml, []) do |keys, value|
      ...
      reference_interpolation_keys =
        I18nInterpolationKeysFinder.find(reference_value.to_s)
      ...
      unsupported_keys =
        locale_interpolation_keys - reference_interpolation_keys
      ...
      if pluralized && keys.last == 'one' &&
         !FORCE_PLURAL_COUNT_LOCALES.include?(@locale)
      ...
      unless missing_keys.empty?
        add_error(
          keys,
          TYPE_MISSING_INTERPOLATION_KEYS,
          missing_keys,
  ...
  def check_plural_keys
    ...
    traverse_hash(@locale_yaml, []) do |keys, _, parent|
      ...
      reference_value = reference_value(keys)
      if reference_value.nil? || !reference_value_pluralized?(reference_value)
        next
      end
  ...
  def add_error(keys, type, details, pluralized:)
    ...
    @errors[@relative_locale_path] <<
      { key: joined_key, type: type, details: details.to_s }
  end
end
