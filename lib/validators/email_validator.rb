class EmailValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    ...
    if record.errors[attribute].blank? && value &&
       ScreenedEmail.should_block?(value)
  ...
  def self.allowed?(email)
    if (setting = SiteSetting.email_domains_whitelist).present?
      return email_in_restriction_setting?(setting, email) ||
        is_developer?(email)
    elsif (setting = SiteSetting.email_domains_blacklist).present?
      return !(
        email_in_restriction_setting?(setting, email) && !is_developer?(email)
      )
    end
  ...
  def self.email_regex
    %r{\A[a-zA-Z0-9!#\$%&'*+\/=?\^_`{|}~\-]+(?:\.[a-zA-Z0-9!#\$%&'\*+\/=?\^_`{|}~\-]+)*@(?:[a-zA-Z0-9](?:[a-zA-Z0-9\-]*[a-zA-Z0-9])?\.)+[a-zA-Z0-9](?:[a-zA-Z0-9\-]*[a-zA-Z0-9])?$\z}
  end
end
