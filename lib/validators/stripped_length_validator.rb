...
class Validators::StrippedLengthValidator < ActiveModel::EachValidator
  def self.validate(record, attribute, value, range)
    unless value.nil?
      ...
      unless stripped_length >= range.begin
        record.errors.add attribute,
             (I18n.t('errors.messages.too_short', count: range.begin))
      end
      unless stripped_length <= range.end
        record.errors.add attribute,
             (
               I18n.t(
                 'errors.messages.too_long_validation',
                 max: range.end, length: stripped_length
               )
             )
      end
    else
