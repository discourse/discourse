...
class Validators::PostValidator < ActiveModel::Validator
  def validate(record)
    ...
    if record.acting_user.try(:admin?) &&
       Discourse.static_doc_topic_ids.include?(record.topic_id)
      return
    end
  ...
  def stripped_length(post)
    range =
      ...
      elsif post.is_first_post? ||
            (post.topic.present? && post.topic.posts_count == 0)
  ...
  def raw_quality(post)
    sentinel =
      TextSentinel.body_sentinel(
        post.raw,
        private_message: private_message?(post)
      )
  ...
  def watched_words(post)
    if !post.acting_user&.staff? && !post.acting_user&.staged &&
       matches = WordWatcher.new(post.raw).should_block?
  ...
  def max_posts_validator(post)
    if post.new_record? && post.acting_user.present? &&
      ...
      post.errors.add(
        ...
        I18n.t(
          :too_many_replies,
          count: SiteSetting.newuser_max_replies_per_topic
        )
      )
    end
  end
  ...
  def max_images_validator(post)
    ...
    if post.acting_user.trust_level <
       TrustLevel[SiteSetting.min_trust_to_post_images]
      add_error_if_count_exceeded(
        post,
  ...
  def force_edit_last_validator(post)
    if SiteSetting.max_consecutive_replies == 0 || post.id ||
    ...
    if guardian.can_edit?(topic.ordered_posts.last)
      post.errors.add(
        ...
        I18n.t(
          ...
          count: SiteSetting.max_consecutive_replies
        )
      )
    end
  end
