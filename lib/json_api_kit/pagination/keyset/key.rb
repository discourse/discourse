  ...
  module Pagination
    class Keyset
      class Key
        ...
        def valued_condition = ->(scope) { joins.apply(scope).where("#{value_sql} IS NOT NULL") }

        def null_condition = ->(scope) { joins.apply(scope).where("#{value_sql} IS NULL") }
