...
module FileStore
  class LocalStore < BaseStore
    ...
    def remove_file(url, _)
      ...
      destination =
        "#{public_dir}#{url.sub('/uploads/', '/uploads/tombstone/')}"
    ...
    def is_local?(url)
      ...
      absolute_url =
        url.start_with?('//') ? SiteSetting.scheme + ':' + url : url
      absolute_url.start_with?(absolute_base_url) ||
        absolute_url.start_with?(absolute_base_cdn_url)
    end
    ...
    def tombstone_dir
      "#{public_dir}#{relative_base_url.sub(
        '/uploads/',
        '/uploads/tombstone/'
      )}"
    end
    ...
    def list_missing(model)
      ...
      if count > 0
        puts "#{count} of #{model.count} #{model.name.underscore
