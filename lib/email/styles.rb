  ...
  class Styles
    ...
    def format_basic
      ...
      @fragment.css('img').each do |img|
        ...
        if (img['class'] && img['class']['emoji']) ||
           (img['src'] && img['src'][%r{\/_?emoji\/}])
      ...
      big_images =
        @fragment.css('img[width="auto"][height="auto"]') -
      ...
      @fragment.css('a.attachment').each do |a|
        if a['href'] =~ %r{^\/[^\/]} # ensure all urls are absolute
          a['href'] = "#{Discourse.base_url}#{a['href']}"
        end

        if a['href'] && a['href'].starts_with?('//') # ensure no schemaless urls
    ...
    def format_notification
      style(
        '.previous-discussion',
        'font-size: 17px; color: #444; margin-bottom:10px;'
      )
      style(
        '.notification-date',
      ...
      style(
        '.username',
        "font-size:13px;font-family:'lucida grande',tahoma,verdana,arial,sans-serif;color:#{SiteSetting
          .email_link_color};text-decoration:none;font-weight:bold"
      )
      style(
        '.user-title',
        "font-size:13px;font-family:'lucida grande',tahoma,verdana,arial,sans-serif;text-decoration:none;margin-left:7px;color: #999;"
      )
      style(
        '.user-name',
        "font-size:13px;font-family:'lucida grande',tahoma,verdana,arial,sans-serif;text-decoration:none;margin-left:7px;color: #{SiteSetting
          .email_link_color};font-weight:normal;"
      )
    ...
    def onebox_styles
      ...
      style(
        'blockquote',
        'border-left: 5px solid #e9e9e9; background-color: #f8f8f8; margin: 0;'
      )
      ...
      style(
        'aside.onebox',
        'border: 5px solid #e9e9e9; padding: 12px 25px 12px 12px;'
      )
      style(
        'aside.onebox header img.site-icon',
      ...
      @fragment.css('aside.quote').each do |n|
        ...
        blockquote =
          if original_node.css('blockquote').inner_html.strip.start_with?('<p')
            ...
            "<p style='padding: 0;'>#{original_node.css('blockquote')
              .inner_html}</p>"
          end
      ...
      @fragment.css('iframe').each do |i|
        begin
          ...
          display_src =
            "#{src_uri.scheme || 'https'}://#{src_uri.host}#{src_uri
              .path}#{src_uri.query.nil? ? '' : '?' + src_uri.query}#{if src_uri
               .fragment
               .nil?
              ''
            else
          ...
          i.replace "<p><a href='#{src_uri.to_s}'>#{CGI.escapeHTML(
                      display_src
                    )}</a><p>"
        rescue URI::Error
    ...
    def format_html
      style(
        '.with-accent-colors',
        "background-color: #{SiteSetting
          .email_accent_bg_color}; color: #{SiteSetting.email_accent_fg_color};"
      )
      style('h4', 'color: #222;')
      ...
      style(
        'a',
        "text-decoration: none; font-weight: bold; color: #{SiteSetting
          .email_link_color};"
      )
      ...
      style(
        'div.footer',
        'color:#666; font-size:95%; text-align:center; padding-top:15px;'
      )
      ...
      style(
        'pre code',
        'display: block; background-color: #f1f1ff; padding: 5px;'
      )
      style(
        '.featured-topic a',
        "text-decoration: none; font-weight: bold; color: #{SiteSetting
          .email_link_color}; line-height:1.5em;"
      )
      ...
      @fragment.search('img').each do |img|
        ...
        if img['src'][/_avatar/]
          if img.parent&.name == 'td'
            img.parent['style'] = 'vertical-align: top;'
          end
          img.remove
        end
    ...
    def make_all_links_absolute
      ...
      @fragment.css('a').each do |link|
        begin
          unless URI(link['href'].to_s).host.present?
      ...
      @fragment.css('.footer.hilight').each do |element|
        ...
        element.css('a').each do |inner|
          if footernum == (0) && linknum == (0)
            ...
            inner['style'] =
              "background-color: #{bg_color}; color: #{SiteSetting
                .email_accent_fg_color}; border-top: 4px solid #{bg_color}; border-right: 6px solid #{bg_color}; border-bottom: 4px solid #{bg_color}; border-left: 6px solid #{bg_color}; display: inline-block; font-weight: bold;"
          end
          return
        end
        return
