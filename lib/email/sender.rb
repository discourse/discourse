...
module Email
  class Sender
    ...
    def send(is_critical: false)
      ...
      if @message.blank?
        return skip(SkippedEmailLog.reason_types[:sender_message_blank])
      end
      if @message.to.blank?
        return skip(SkippedEmailLog.reason_types[:sender_message_to_blank])
      end
      ...
      if @message.text_part
        if @message.text_part.body.to_s.blank?
          return skip(
            SkippedEmailLog.reason_types[:sender_text_part_body_blank]
          )
        end
      else
      ...
      email_log =
        EmailLog.new(
          email_type: @email_type, to_address: to_address, user_id: user_id
        )
      ...
      if topic_id.present? && post_id.present?
        ...
        unless post
          return skip(SkippedEmailLog.reason_types[:sender_post_deleted])
        end
        ...
        referenced_posts =
          Post.includes(:incoming_email).where(
            id: PostReply.where(reply_id: post_id).select(:post_id)
          )
            .order(id: :desc)

        referenced_post_message_ids =
          referenced_posts.map do |referenced_post|
        ...
        if topic&.category && !topic.category.uncategorized?
          list_id =
            "#{SiteSetting.title} | #{topic.category.name} <#{topic.category
              .name
              .downcase
              .tr(' ', '-')}.#{host}>"
          ...
          if !topic.category.parent_category_id.nil?
            parent_category_name =
              Category.find_by(id: topic.category.parent_category_id).name
            list_id =
              "#{SiteSetting.title} | #{parent_category_name} #{topic.category
                .name} <#{topic.category.name.downcase.tr(
                ' ',
                '-'
              )}.#{parent_category_name.downcase.tr(' ', '-')}.#{host}>"
          end
        else
        ...
        if topic
          if SiteSetting.private_email?
            @message.header['List-Archive'] =
              "#{Discourse.base_url}#{topic.slugless_url}"
          else
      ...
      if SiteSetting.reply_by_email_address.present? &&
        ...
        @message.header[:return_path] =
          SiteSetting.reply_by_email_address.sub(
            '%{reply_key}',
            "verp-#{email_log.bounce_key}"
          )
      end
      ...
      if reply_key.present?
        @message.header[Email::MessageBuilder::ALLOW_REPLY_BY_EMAIL_HEADER] =
          nil
      end
      ...
      if SiteSetting.strip_images_from_short_emails &&
         @message.html_part.body.to_s.bytesize <=
           SiteSetting.short_email_length &&
      ...
      begin
        ...
        return skip(
          SkippedEmailLog.reason_types[:custom],
          custom_reason: e.message
        )
      end
    ...
    def set_reply_key(post_id, user_id)
      unless user_id && post_id &&
             header_value(Email::MessageBuilder::ALLOW_REPLY_BY_EMAIL_HEADER)
               .present?
        return
      end
      ...
      reply_key =
        PostReplyKey.find_or_create_by_safe!(post_id: post_id, user_id: user_id)
          .reply_key
