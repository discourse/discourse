  ...
  class Processor
    ...
    def handle_failure(mail_string, e)
      ...
      if message_template == :email_reject_unrecognized_error
        msg =
          "Unrecognized error type (#{e.class}: #{e
            .message}) when processing incoming email"
      ...
      if message_template == :email_reject_old_destination
        ...
        template_args[:number_of_days] =
          SiteSetting.disallow_reply_by_email_after_days
      end
    ...
    def log_email_process_failure(mail_string, exception)
      if SiteSetting.log_mail_processing_failures
        Rails.logger.warn(
          "Email can not be processed: #{exception}\n\n#{mail_string}"
        )
      end
    end
  end
