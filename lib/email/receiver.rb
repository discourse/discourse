...
module Email
  class Receiver
    ...
    def initialize(mail_string, opts = {})
      ...
      @message_id =
        @mail.message_id.presence || Digest::MD5.hexdigest(mail_string)
      @opts = opts
    end
    ...
    def ensure_valid_address_lists
      %i[to cc bcc].each do |field|
        ...
        if addresses&.errors.present?
          @mail[field] =
            addresses.to_s.scan(/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i)
        end
      end
    end
    ...
    def is_blacklisted?
      ...
      Regexp.new(SiteSetting.ignore_by_title, Regexp::IGNORECASE) =~
        @mail.subject
    end
    ...
    def process_internal
      ...
      if body.blank? && attachments.empty? && !is_bounce?
    ...
    def hidden_reason_id
      @hidden_reason_id ||=
        is_spam? ? Post.hidden_reasons[:email_spam_header_found] : nil
    end
    ...
    def handle_bounce
      ...
      if @mail.error_status.present? &&
         Array.wrap(@mail.error_status).any? { |s| s.start_with?('4.') }
    ...
    def self.update_bounce_score(email, score)
      if user = User.find_by_email(email)
        ...
        if user.active &&
           range === SiteSetting.bounce_score_threshold_deactivate
          ...
          reason =
            I18n.t(
              'user.email.revoked',
              email: user.email, date: user.user_stat.reset_bounce_score_after
            )
          StaffActionLogger.new(Discourse.system_user).log_revoke_email(
            user,
            reason
          )
          # ... and PM the user
    ...
    def is_auto_generated?
      if SiteSetting.auto_generated_whitelist.split('|').include?(@from_email)
        return false
      end
      @mail[:precedence].to_s[/list|junk|bulk|auto_reply/i] ||
        @mail[:from].to_s[
          /(mailer[\-_]?daemon|post[\-_]?master|no[\-_]?reply)@/i
        ] ||
        @mail[:subject].to_s[
    ...
    def select_body
      ...
      if text.present?
        ...
        if @opts[:convert_plaintext] || sent_to_mailinglist_mirror?
          ...
          converter_opts = {
            format_flowed:
              !!(text_content_type =~ /format\s*=\s*["']?flowed["']?/i),
            delete_flowed_space:
              !!(text_content_type =~ /DelSp\s*=\s*["']?yes["']?/i)
          }
          text = PlainTextToMarkdown.new(text, converter_opts).to_markdown
          elided_text =
            PlainTextToMarkdown.new(elided_text, converter_opts).to_markdown
        end
      end
      ...
      markdown, elided_markdown =
        if html.present?
          ...
          if html_extracter =
             HTML_EXTRACTERS.select { |_, r| html[r] }.min_by do |_, r|
               html =~ r
             end
            ...
            markdown =
              HtmlToMarkdown.new(html, keep_img_tags: true, keep_cid_imgs: true)
                .to_markdown
      ...
      if text.blank? ||
         (SiteSetting.incoming_email_prefer_html && markdown.present?)
    ...
    def to_markdown(html, elided_html)
      ...
      [
        EmailReplyTrimmer.trim(markdown),
        HtmlToMarkdown.new(elided_html).to_markdown
      ]
    end
    ...
    def extract_from_outlook(doc)
      ...
      elided =
        doc.css(
          '#Signature, #Signature ~ *, hr, #divRplyFwdMsg, #divRplyFwdMsg ~ *'
        )
          .remove
      to_markdown(doc.to_html, elided.to_html)
    end
    ...
    def extract_from_exchange(doc)
      ...
      to_markdown(
        doc.css("div[name='messageReplySection']").to_html,
        elided.to_html
      )
    end
    ...
    def extract_from_protonmail(doc)
      ...
      elided =
        doc.css("*[class^='protonmail_'], *[class^='protonmail_'] ~ *").remove
      to_markdown(doc.to_html, elided.to_html)
    end
    ...
    def extract_from_address_and_name(value)
      ...
      if (from_address.blank? || !from_address['@']) &&
         value[/\[mailto:[^\]]+\]/]
    ...
    def destinations
      @destinations ||=
        all_destinations.map do |d|
          Email::Receiver.check_address(d, is_bounce?)
        end
          .reject(&:blank?)
    end
    ...
    def self.check_address(address, include_verp = false)
      ...
      # reply
      match =
        Email::Receiver.reply_by_email_address_regex(true, include_verp).match(
          address
        )
    ...
    def process_destination(destination, user, body, elided, hidden_reason_id)
      if SiteSetting.enable_forwarded_emails && has_been_forwarded? &&
      ...
      when :category
        ...
        if user.staged? && !category.email_in_allow_strangers
          raise StrangersNotAllowedError
        end
        if !user.has_trust_level?(SiteSetting.email_in_min_trust) &&
           !sent_to_mailinglist_mirror?
      ...
      when :reply
        ...
        if post_reply_key.user_id != user.id &&
           !forwarded_reply_key?(post_reply_key, user)
          raise ReplyUserNotMatchingError,
                "post_reply_key.user_id => #{post_reply_key.user_id
                  .inspect}, user.id => #{user.id.inspect}"
        end
    ...
    def create_group_post(group, user, body, elided, hidden_reason_id)
      message_ids =
        Email::Receiver.extract_reply_message_ids(
          @mail,
          max_message_id_count: 5
        )
      post_ids = []

      incoming_emails =
        IncomingEmail.where(message_id: message_ids).addressed_to_user(user)
          .pluck(:post_id, :to_addresses, :cc_addresses)

      incoming_emails.each do |post_id, to_addresses, cc_addresses|
        if contains_email_address_of_user?(to_addresses, user) ||
      ...
      if post_ids.any? &&
         post = Post.where(id: post_ids).order(:created_at).last
        create_reply(
          user: user,
          raw: body,
          elided: elided,
    ...
    def forwarded_reply_key?(post_reply_key, user)
      ...
      incoming_emails.each do |to_addresses, cc_addresses|
        ...
        if contains_reply_by_email_address(
           to_addresses,
           post_reply_key.reply_key
         ) ||
           contains_reply_by_email_address(
             cc_addresses,
             post_reply_key.reply_key
           )
          return true
    ...
    def contains_email_address_of_user?(addresses, user)
      ...
      user.user_emails.any? do |user_email|
        addresses.include?(user_email.email)
      end
    end
    ...
    def has_been_forwarded?
      subject[/^[[:blank:]]*(fwd?|tr)[[:blank:]]?:/i] &&
        embedded_email_raw.present?
    end

    def embedded_email_raw
      return @embedded_email_raw if @embedded_email_raw
      text = fix_charset(@mail.multipart? ? @mail.text_part : @mail)
      @embedded_email_raw, @before_embedded =
        EmailReplyTrimmer.extract_embedded_email(text)
      @embedded_email_raw
    end
    ...
    def process_forwarded_email(destination, user)
      ...
      if post&.topic
        ...
        if @before_embedded.present?
          ...
          if post.topic.private_message? && group.usernames[user.username]
    ...
    def self.reply_by_email_address_regex(
      extract_reply_key = true, include_verp = false
    )
      ...
      if include_verp && SiteSetting.reply_by_email_address.present? &&
        ...
        reply_addresses <<
          SiteSetting.reply_by_email_address.sub(
            '%{reply_key}',
            'verp-%{reply_key}'
          )
      end
      ...
      reply_addresses.map! do |a|
        a.gsub(Regexp.escape('%{reply_key}'), "(\\h{32})?")
      end
      reply_addresses.empty? ? /$a/ : /#{reply_addresses.join('|')}/ # a regex that can never match
    end

    def group_incoming_emails_regex
      @group_incoming_emails_regex ||=
        Regexp.union Group.pluck(:incoming_email).select(&:present?).map do |e|
          e.split('|')
        end
          .flatten
          .uniq
    ...
    def category_email_in_regex
      @category_email_in_regex ||=
        Regexp.union Category.pluck(:email_in).select(&:present?).map do |e|
          e.split('|')
        end
          .flatten
          .uniq
    ...
    def find_related_post(force: false)
      ...
      post_ids =
        message_ids.map { |message_id| message_id[post_id_regexp, 1] }.compact
          .map(&:to_i)
      post_ids <<
        Post.where(
          topic_id:
            message_ids.map { |message_id| message_id[topic_id_regexp, 1] }
              .compact,
          post_number: 1
        )
          .pluck(:id)
    ...
    def self.extract_reply_message_ids(mail, max_message_id_count:)
      message_ids = [
        mail.in_reply_to,
        Email::Receiver.extract_references(mail.references)
      ]
      message_ids.flatten!
    ...
    def likes
      @likes ||=
        Set.new [
                  ...
                  '<3',
                  '❤',
                  I18n.t('post_action_types.like.title').downcase
                ]
    end
    ...
    def subscription_action_for(body, subject)
      ...
      if ([subject, body].compact.map(&:to_s).map(&:downcase) & %w[unsubscribe])
         .any?
        :confirm_unsubscribe
      end
    end
    ...
    def create_reply(options = {})
      if options[:topic].nil? || options[:topic].trashed?
        raise TopicNotFoundError
      end
      if options[:bounce] &&
         options[:topic].archetype != Archetype.private_message
        raise BouncedEmailError
      end
      ...
      if post_action_type = post_action_for(options[:raw])
        ...
        options[:is_group_message] =
          options[:topic].private_message? &&
    ...
    def is_whitelisted_attachment?(attachment)
      attachment.content_type !~
        SiteSetting.attachment_content_type_blacklist_regex &&
        ...
        begin
          attachments =
            @mail.attachments.select do |attachment|
              is_whitelisted_attachment?(attachment)
            end
          if @mail.attachment? && is_whitelisted_attachment?(@mail)
            attachments << @mail
          end

          @mail.parts.each do |part|
            if part.attachment? && is_whitelisted_attachment?(part)
              attachments << part
            end
          end
    ...
    def add_attachments(raw, user, options = {})
      ...
      attachments.each do |attachment|
        tmp =
          Tempfile.new(
            ['discourse-email-attachment', File.extname(attachment.filename)]
          )
        begin
          # read attachment
          ...
          upload =
            UploadCreator.new(tmp, attachment.filename, opts).create_for(
              user.id
            )
          if upload&.valid?
            ...
            raw <<
              "\n\n#{I18n.t(
                'emails.incoming.missing_attachment',
    ...
    def attachment_markdown(upload)
      if FileHelper.is_supported_image?(upload.original_filename)
        "<img src='#{upload.url}' width='#{upload.width}' height='#{upload
          .height}'>"
      else
        "<a class='attachment' href='#{upload.url}'>#{upload
          .original_filename}</a> (#{number_to_human_size(upload.filesize)})"
      end
    end
    ...
    def create_post(options = {})
      ...
      if options[:created_at].nil?
        raise InvalidPost,
              'No post creation date found. Is the e-mail missing a Date: header?'
      end
      ...
      if options[:bounce]
        options[:raw] =
          I18n.t(
            ...
            email: user.email, raw: options[:raw]
          )
      ...
      if result.post
        @incoming_email.update_columns(
          topic_id: result.post.topic_id, post_id: result.post.id
        )
    ...
    def self.elided_html(elided)
      ...
      html <<
        "<summary title='#{I18n.t(
          'emails.incoming.show_trimmed_content'
        )}'>&#183;&#183;&#183;</summary>" <<
        "\n\n"
      html << elided << "\n\n"
      html << '</details>' << "\n"
          ...
          @mail[d].address_list.addresses.each do |address_field|
            begin
              ...
              if should_invite?(email)
                ...
                if @staged_users.count >
                   SiteSetting.maximum_staged_users_per_email
                  post.topic.add_moderator_post(
                    sender,
                    I18n.t(
                      'emails.incoming.maximum_staged_user_per_email_reached'
                    )
                  )
                  return
                end
    ...
    def can_invite?(topic, user)
      !topic.topic_allowed_users.where(user_id: user.id).exists? &&
        !topic.topic_allowed_groups.where(
    ...
    def enable_email_pm_setting(user)
      ...
      if !user.staged &&
         user.user_option.email_messages_level ==
