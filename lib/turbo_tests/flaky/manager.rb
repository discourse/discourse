  ...
  module Flaky
    class Manager
      ...
      class << self
        ...
        def log_potential_flaky_tests(failed_examples)
          return if failed_examples.empty?

          File.open(PATH, "w") do |file|
            file.puts(
              failed_examples
                .map { |failed_example| FailedExample.new(failed_example).to_h }
                .to_json,
            )
          end
        end
        ...
        def remove_example(failed_examples)
          flaky_tests =
            JSON
              .parse(File.read(PATH))
