...
module PrettyText
  ...
  class << self
    ...
    def apply_es6_file(ctx:, path:, module_name:)
    ...
    def avatar_img(avatar_template, size)
      protect do
    ...
    def strip_hidden_unicode_bidirectional_characters(doc)
    ...
    def sanitize_hotlinked_media(doc)
    ...
    def add_rel_attributes_to_user_content(doc, add_nofollow)
      allowlist = []
    ...
    def add_video_placeholder_image(doc)
      doc
        ...
        .each do |video|
          ...
          thumbnail =
            Upload.where(
              ...
              "#{Upload.sanitize_sql_like(video_sha1)}.%",
            ).last
          if thumbnail
    ...
    def excerpt(html, max_length, options = {})
    ...
    def convert_hashtag_links_to_plaintext(doc)
      doc
        .css("a.hashtag-cooked")
        .each do |hashtag|
          hashtag.replace(doc.document.create_text_node("##{hashtag["data-slug"]}"))
        end
    end
    ...
    def make_all_links_absolute(doc)
      doc
        .css("a[href]")
        .each do |a|
          href = a["href"].to_s
    ...
    def strip_image_wrapping(doc)
      doc.css(".lightbox-wrapper .meta").remove
    end

    def strip_oneboxed_media(doc)
      doc.css("audio").remove
      doc.css(".video-onebox,video").remove
    end

    def convert_vimeo_iframes(doc)
      doc
    ...
    def vimeo_url_from_iframe(iframe)
    ...
    def secure_uploads_placeholder(doc, url, onebox_type: false, width: nil, height: nil)
    ...
    def format_for_email(html, post = nil)
      doc = Nokogiri::HTML5.fragment(html)
