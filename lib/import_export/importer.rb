  ...
  class Importer < ImportScripts::Base
    ...
    def import_categories
      ...
      @categories.each do |cat_attrs|
        begin
          ...
          if cat_attrs[:parent_category_id].present?
            category.parent_category_id =
              new_category_id(cat_attrs[:parent_category_id])
          end
          ...
          category.permissions =
            if permissions.present?
              ...
              { 'everyone' => CategoryGroup.permission_types[:full] }
            end
          category.save!
    ...
    def import_topics
      ...
      @topics.each do |t|
        ...
        first_post_attrs =
          t[:posts].first.merge(
            t.slice(*(TopicExporter::TOPIC_ATTRS - %i[id category_id]))
          )
        ...
        first_post =
          PostCustomField.where(name: 'import_id', value: import_id).first&.post
        ...
        t[:posts].each_with_index do |post_data, i|
          ...
          existing =
            PostCustomField.where(name: 'import_id', value: post_import_id)
              .first
              &.post
          unless existing
            ...
            create_post(
              post_data.merge(
                topic_id: topic_id, user_id: new_user_id(post_data[:user_id])
              ),
              post_import_id
            )
          end
    ...
    def new_user_id(external_user_id)
      ucf =
        UserCustomField.where(
          name: 'import_id', value: "#{external_user_id}#{import_source}"
        )
          .first
    ...
    def new_category_id(external_category_id)
      CategoryCustomField.where(
        name: 'import_id', value: "#{external_category_id}#{import_source}"
      )
        .first
        &.category_id
    end
