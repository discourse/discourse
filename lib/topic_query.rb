...
class TopicQuery
  ...
  def self.validators
    @validators ||=
      begin
        ...
        zero_up_to_max_int =
          lambda { |x| int.call(x) && x.to_i.between?(0, PG_MAX_INT) }

        array_int_or_int =
          lambda do |x|
            int.call(x) || (Array === x && x.length > 0 && x.all?(&int))
          end
  ...
  def self.validate?(option, value)
    fn = validators[option.to_sym] ? fn.call(value) : true
  end
  ...
  def joined_topic_user(list = nil)
    (list || Topic).joins(
      "LEFT OUTER JOIN topic_users AS tu ON (topics.id = tu.topic_id AND tu.user_id = #{@user
        .id
        .to_i})"
    )
  end
  ...
  def get_pm_params(topic)
    if topic.private_message?
      ...
      if my_group_ids.present?
        ...
        target_users =
          target_users.joins(
            "LEFT JOIN group_users gu ON gu.user_id = topic_allowed_users.user_id AND gu.group_id IN (#{sanitize_sql_array(
              my_group_ids
            )})"
          )
      ...
      target_user_ids =
        target_users.where('NOT topic_allowed_users.user_id = ?', @user.id)
          .pluck(:user_id)
  ...
  def list_suggested_for(topic, pm_params: nil)
    ...
    if @user
      if topic.private_message?
        unless builder.full?
          builder.add_results(
            new_messages(pm_params.merge(count: builder.results_left))
          )
        end

        unless builder.full?
          builder.add_results(
            unread_messages(pm_params.merge(count: builder.results_left))
          )
        end
      else
        builder.add_results(
          unread_results(topic: topic, per_page: builder.results_left),
          :high
        )
        unless builder.full?
          builder.add_results(
            new_results(topic: topic, per_page: builder.category_results_left)
          )
        end
      end
    end
    ...
    if !topic.private_message?
      unless builder.full?
        builder.add_results(
          random_suggested(
            topic,
            builder.results_left,
            builder.excluded_topic_ids
          )
        )
      end
    end
  ...
  def list_read
    create_list(:read, unordered: true) do |topics|
      topics.where('tu.last_visited_at IS NOT NULL').order(
  ...
  def not_archived(list, user)
    list.joins(
      "LEFT JOIN user_archived_messages um
                       ON um.user_id = #{user
        .id
        .to_i} AND um.topic_id = topics.id"
    )
  ...
  def list_group_topics(group)
    list =
      default_results.where(
        "
      topics.user_id IN (
        SELECT user_id FROM group_users gu WHERE gu.group_id = #{group
          .id
          .to_i}
      )
    "
  ...
  def list_private_messages(user)
    ...
    list =
      not_archived(list, user).where(
        'NOT (topics.participant_count = 1 AND topics.user_id = ?)',
        user.id
      )
  ...
  def list_private_messages_archive(user)
    list = private_messages_for(user, :user)
    list =
      list.joins(:user_archived_messages).where(
        'user_archived_messages.user_id = ?',
        user.id
      )
    create_list(:private_messages, {}, list)
  end
  ...
  def list_private_messages_group(user)
    ...
    group_id =
      Group.where('name ilike ?', @options[:group_name]).pluck(:id).first
    list =
      list.joins(
        "LEFT JOIN group_archived_messages gm ON gm.topic_id = topics.id AND
  ...
  def list_private_messages_group_archive(user)
    ...
    list =
      list.joins(
        "JOIN group_archived_messages gm ON gm.topic_id = topics.id AND
                      gm.group_id = #{group_id
          .to_i}"
      )
    create_list(:private_messages, {}, list)
    ...
    list =
      list.joins(
        "JOIN topic_tags tt ON tt.topic_id = topics.id
                      JOIN tags t ON t.id = tt.tag_id AND t.name = '#{@options[
          :tags
        ][
          0
        ]}'"
  ...
  def list_category_topic_ids(category)
    ...
    pinned_ids =
      query.where(
        'topics.pinned_at IS NOT NULL AND topics.category_id = ?',
        category.id
      )
        .limit(nil)
    ...
    non_pinned_ids =
      query.where(
        'topics.pinned_at IS NULL OR topics.category_id <> ?',
        category.id
      )
        .pluck(:id)
  ...
  def list_new_in_category(category)
    create_list(
      :new_in_category,
      unordered: true, category: category.id
    ) { |list| list.by_newest.first(25) }
  end

  def self.new_filter(list, treat_as_new_topic_start_date)
    list.where(
      'topics.created_at >= :created_at',
      created_at: treat_as_new_topic_start_date
    )
  ...
  def self.unread_filter(list, user_id, opts)
    col_name =
      opts[:staff] ? 'highest_staff_post_number' : 'highest_post_number'
  ...
  def create_list(filter, options = {}, topics = nil)
    ...
    if %w[activity default].include?(options[:order] || 'activity') &&
       !options[:unordered] &&
    ...
    if options[:preload_posters]
      user_ids = []
      topics.each do |ft|
        user_ids << ft.user_id << ft.last_post_user_id <<
          ft.featured_user_ids <<
          ft.allowed_user_ids
      end
    ...
    topics.each do |t|
      t.allowed_user_ids =
        filter == :private_messages ? t.allowed_users.map(&:id) : []
    end
  ...
  def latest_results(options = {})
    ...
    unless options && options[:state] == 'muted'.freeze
      result = remove_muted_topics(result, @user)
    end
    result = remove_muted_categories(result, @user, exclude: options[:category])
    result = remove_muted_tags(result, @user, options)
    result =
      apply_shared_drafts(result, get_category_id(options[:category]), options)
  ...
  def private_messages_for(user, type)
    ...
    if type == :group
      ...
      result =
        result.where(
          "topics.id IN (SELECT topic_id FROM topic_allowed_users WHERE user_id = #{user
            .id
            .to_i})"
        )
    elsif type == :all
      ...
      result =
        result.where(
          "topics.id IN (
            ...
            SELECT topic_id FROM topic_allowed_groups
            WHERE group_id IN (
              SELECT group_id FROM group_users WHERE user_id = #{user
            .id
            .to_i}
            )
    )"
  ...
  def apply_shared_drafts(result, category_id, options)
    ...
    if can_create_shared && options[:destination_category_id]
      destination_category_id =
        get_category_id(options[:destination_category_id])
      topic_ids =
        SharedDraft.where(category_id: destination_category_id).pluck(:topic_id)
  ...
  def apply_ordering(result, options)
    ...
    if sort_column == 'category_id'
      ...
      return result.references(:categories).order(
        TopicQuerySQL.order_by_category_sql(sort_dir)
      )
    end
  ...
  def get_category_id(category_id_or_slug)
    ...
    if category_id == 0
      category_id = Category.where(slug: category_id_or_slug).pluck(:id).first
    end
    category_id
  end
  ...
  def default_results(options = {})
    ...
    if category_id
      ...
      if !@options[:order]
        ...
        sort_order, sort_ascending =
          Category.where(id: category_id).pluck(:sort_order, :sort_ascending)
            .first
        if sort_order
    ...
    if SiteSetting.tagging_enabled
      ...
      if @options[:tags] && @options[:tags].size > 0
        unless @options[:tags].respond_to?('each')
        ...
        if @options[:match_all_tags]
          ...
          unless @options[:tags][0].is_a?(Integer)
            @options[:tags] = Tag.where_name(@options[:tags]).pluck(:id)
          end
    ...
    if options[:exclude_category_ids] &&
       options[:exclude_category_ids].is_a?(Array) &&
       options[:exclude_category_ids].size > 0
      result =
        result.where(
          'categories.id NOT IN (?)',
          options[:exclude_category_ids].map(&:to_i)
        )
    ...
    if options[:except_topic_ids]
      result =
        result.where.not(topics: { id: options[:except_topic_ids] }).references(
          :topics
        )
    end
    ...
    if options[:topic_ids]
      result =
        result.where('topics.id in (?)', options[:topic_ids]).references(
          :topics
        )
    end
    ...
    if search = options[:search]
      result =
        result.where(
          "topics.id in (select pp.topic_id from post_search_data pd join posts pp on pp.id = pd.post_id where pd.search_data @@ #{Search
            .ts_query(term: search.to_s)})"
        )
    end
    ...
    if bumped_before = options[:bumped_before]
      if (bumped_before = bumped_before.to_i) > 0
        result =
          result.where('topics.bumped_at < ?', bumped_before.to_i.days.ago)
      end
    end
    ...
    if options[:max_posts].present?
      result = result.where('topics.posts_count <= ?', options[:max_posts])
    end
    if options[:min_posts].present?
      result = result.where('topics.posts_count >= ?', options[:min_posts])
    end
  ...
  def remove_muted_tags(list, user, opts = nil)
    if user.nil? || !SiteSetting.tagging_enabled ||
       !SiteSetting.remove_muted_tags_from_latest
      list
    else
      if !TagUser.lookup(user, :muted).exists?
        ...
        if TagUser.lookup(user, :muted).joins(:tag).where(
           'tags.name = ?',
           showing_tag
         )
           .exists?
  ...
  def related_messages_group(params)
    messages =
      messages_for_groups_or_user(params[:my_group_ids]).limit(params[:count])
    messages = allowed_messages(messages, params)
  end
    ...
    if user_ids.present?
      messages =
        messages.joins(
          ...
          AND ta2.user_id IN (#{sanitize_sql_array(
            user_ids
          )})
        "
        )
    ...
    if group_ids.present?
      messages =
        messages.joins(
          ...
          AND tg2.group_id IN (#{sanitize_sql_array(
            group_ids
          )})
        "
        )
      ...
      base_messages.joins(
        "
          ...
          .to_i}
            ...
            WHERE gu.group_id IN (#{sanitize_sql_array(
          group_ids
        )})
  ...
  def base_messages
    query =
      Topic.where('topics.archetype = ?', Archetype.private_message).joins(
        "LEFT JOIN topic_users tu ON topics.id = tu.topic_id AND tu.user_id = #{@user
          .id
          .to_i}"
      )
  ...
  def random_suggested(topic, count, excluded_topic_ids = [])
    result =
      default_results(unordered: true, per_page: count).where(
    ...
    unless excluded_topic_ids.empty?
      result = result.where('topics.id NOT IN (?)', excluded_topic_ids)
    end
    ...
    if topic.category_id
      result =
        result.order(
          "CASE WHEN topics.category_id = #{topic.category_id
            .to_i} THEN 0 ELSE 1 END"
        )
    end
  ...
  def suggested_ordering(result, options)
    ...
    if options[:topic] && options[:topic].category_id
      result =
        result.order(
          "CASE WHEN topics.category_id = #{options[:topic].category_id
            .to_i} THEN 0 ELSE 1 END"
        )
    end
