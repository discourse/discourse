...
class Autospec::Manager
  ...
  def focus_on_failed_tests(current)
    ...
    if failed_specs.length > 0
      @queue.unshift ['focus', failed_specs.join(' '), runner]
    end
  end
    ...
    files.each do |file, line|
      @runners.each do |runner|
        ...
        runner
          ...
          if k.match(file)
            if @debug
              puts "@@@@@@@@@@@@ #{file} matched a reloader for #{runner}"
            end
            runner.reload
            return
          end
        ...
        runner.watchers.each do |k, v|
          if m = k.match(file)
            if @debug
              puts "@@@@@@@@@@@@ #{file} matched a watcher for #{runner}"
            end
            hit = true
  ...
  def queue_specs(specs)
    ...
    @mutex.synchronize do
      ...
      specs.each do |file, spec, runner|
        ...
        @queue
          .delete_if do |_, s, r|
          s.strip.start_with?(spec.strip) && r == runner
        end
