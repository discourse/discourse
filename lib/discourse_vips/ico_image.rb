...
module DiscourseVips
  class IcoImage
    module Native
      ...
      ffi_lib %w[gdk_pixbuf-2.0 libgdk_pixbuf-2.0.so.0]

      attach_function :gdk_pixbuf_loader_new_with_type, %i[string pointer], :pointer
      attach_function :gdk_pixbuf_loader_write, %i[pointer pointer size_t pointer], :int
      attach_function :gdk_pixbuf_loader_close, %i[pointer pointer], :int
          ...
          if payload.byteslice(14, 2).unpack1("v") == 1
            colors =
              payload
                ...
                .unpack("C8")
                .each_slice(4)
                .map { |color| color.first(3).reverse }
    ...
    def self.load_bitmap(ico_data)
      ...
      pixel_data =
        Array.new(height) { |row| pixels.get_bytes(row * rowstride, width * channels) }.join
      Vips::Image.new_from_memory(pixel_data, width, height, channels, :uchar).copy(
        interpretation: :srgb,
      )
    ensure
