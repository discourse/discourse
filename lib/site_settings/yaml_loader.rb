    ...
    yaml.each_key do |category|
      yaml[category].each do |setting_name, hash|
        if hash.is_a?(Hash)
          ...
          if value.is_a?(Hash)
            raise Discourse::Deprecation,
                  "The site setting `#{setting_name}` can no longer be set based on Rails environment. See also `config/environments/<env>.rb`."
          elsif value.nil?
            raise StandardError,
          ...
          if hash['hidden']&.is_a?(Hash)
            raise Discourse::Deprecation,
                  "The site setting `#{setting_name}`'s hidden property can no longer be set based on Rails environment. It can only be either `true` or `false`."
          end
