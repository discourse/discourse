...
module SiteSettings::Validations
  def validate_error(key, opts = {})
    raise Discourse::InvalidParameters.new(
            I18n.t("errors.site_settings.#{key}", opts)
          )
  end

  def validate_default_categories(new_val, default_categories_selected)
    if (new_val.split('|').to_set & default_categories_selected).size > 0
  ...
  def validate_enable_s3_uploads(new_val)
    if new_val == 't' && SiteSetting.s3_upload_bucket.blank?
      validate_error :s3_upload_bucket_is_required
    end
  end
  ...
  def validate_backup_location(new_val)
    ...
    unless SiteSetting.s3_use_iam_profile
      if SiteSetting.s3_access_key_id.blank?
        validate_error(
          :s3_backup_requires_s3_settings,
          setting_name: 's3_access_key_id'
        )
      end
      if SiteSetting.s3_secret_access_key.blank?
        validate_error(
          :s3_backup_requires_s3_settings,
          setting_name: 's3_secret_access_key'
        )
      end
    end
  end
