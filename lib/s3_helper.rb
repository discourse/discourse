...
class S3Helper
  ...
  def self.build_from_config(use_db_s3_config: false, for_backup: false, s3_client: nil)
    ...
    bucket =
      if use_db_s3_config
        for_backup ? SiteSetting.s3_backup_bucket : SiteSetting.s3_upload_bucket
      else
        for_backup ? GlobalSetting.s3_backup_bucket : GlobalSetting.s3_bucket
      end
...
​
  ...
  def default_s3_options
    if GlobalAwsCredentials.configured?
      ...
      SiteAwsCredentials.new(SiteSetting).tap(&:validate!).to_sdk_options
    else
      {}
    end
  end
