...
class DropOldShelvedNotificationsNotificationIdIndexes < ActiveRecord::Migration[7.0]
  ...
  def up
    ...
    results =
      execute(
        "SELECT indexname, indexdef FROM pg_indexes WHERE tablename = 'shelved_notifications' AND indexdef SIMILAR TO '%\\mold_notification_id\\M%'",
      )
    results.each do |res|
