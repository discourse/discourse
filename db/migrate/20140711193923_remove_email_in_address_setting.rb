class RemoveEmailInAddressSetting < ActiveRecord::Migration[4.2]
  def up
    ...
    cat_id_r =
      DB.query_single(
        "SELECT value FROM site_settings WHERE name = 'email_in_category'"
      )
        .first
    email_r =
      DB.query_single(
        "SELECT value FROM site_settings WHERE name = 'email_in_address'"
      )
        .first
    if email_r
      ...
      email = email_r['value']
      DB.exec(
        'UPDATE categories SET email_in = ? WHERE id = ?',
        email,
        category_id
      )
    end
