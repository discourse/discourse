class MigrateDefaultUserEmailOptions < ActiveRecord::Migration[5.2]
  def up
    ...
    email_always =
      DB.query_single(
        "SELECT value FROM site_settings WHERE name = 'default_email_always'"
      )
        .first
    email_direct =
      DB.query_single(
        "SELECT value FROM site_settings WHERE name = 'default_email_direct'"
      )
        .first
    ...
    if email_personal_messages != 'f' && email_always == 't'
      default_email_messages_level = 0
    end
