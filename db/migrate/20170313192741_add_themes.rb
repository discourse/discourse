class AddThemes < ActiveRecord::Migration[4.2]
  def up
    ...
    enabled_theme_count =
      execute('SELECT count(*) FROM themes WHERE enabled').to_a[0]['count'].to_i

    enabled_scheme_id =
      execute('SELECT id FROM color_schemes WHERE enabled').to_a[0]&.fetch('id')

    theme_key, theme_id =
      execute('SELECT key, id FROM themes WHERE enabled').to_a[0]&.values

    if (enabled_theme_count == 0 && enabled_scheme_id) ||
       enabled_theme_count > 1
...
SQL
    ...
    if enabled_theme_count > 1
      ...
      SELECT #{theme_id
                .to_i}, id, created_at, updated_at
      FROM themes WHERE enabled
SQL
    end

    if enabled_scheme_id
      execute "UPDATE themes SET color_scheme_id=#{enabled_scheme_id
                .to_i} WHERE id=#{theme_id.to_i}"
    end
