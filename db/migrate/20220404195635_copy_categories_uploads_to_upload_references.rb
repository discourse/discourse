...
class CopyCategoriesUploadsToUploadReferences < ActiveRecord::Migration[6.1]
  def up
    ...
    execute <<~SQL
    INSERT INTO upload_references(upload_id, target_type, target_id, created_at, updated_at)
    SELECT categories.uploaded_logo_dark_id, 'Category', categories.id, uploads.created_at, uploads.updated_at
    FROM categories
    JOIN uploads ON uploads.id = categories.uploaded_logo_dark_id
    WHERE categories.uploaded_logo_dark_id IS NOT NULL
    ON CONFLICT DO NOTHING
    SQL
