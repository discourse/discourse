class AddSubcategoryListStyleToCategories < ActiveRecord::Migration[4.2]
  def up
    add_column :categories,
               ...
               :string,
               limit: 50, default: 'rows_with_featured_topics'

    result =
      execute(
        "select value from site_settings where name = 'desktop_category_page_style' and value != 'categories_with_featured_topics'"
      )
    if result.count > 0
