...
class InsertCommunityToSidebarSections < ActiveRecord::Migration[7.0]
  def up
    ...
    result = DB.query <<~SQL
      ...
      VALUES
        ('everything', '/system_unread', 'layer-group', false, 'primary', now(), now()),
        ('my_posts', '/system_posts', 'user', false, 'primary', now(), now()),
        ('review', '/system_review', 'flag', false, 'primary', now(), now()),
        ('admin', '/system_admin', 'wrench', false, 'primary', now(), now()),
        ('users', '/system_users', 'users', false, 'secondary', now(), now()),
        ('info', '/system_about', 'info-circle', false, 'secondary', now(), now()),
        ('faq', '/system_faq', 'question-circle', false, 'secondary', now(), now()),
        ('groups', '/system_groups', 'user-friends', false, 'secondary', now(), now()),
        ('badges', '/system_badges', 'certificate', false, 'secondary', now(), now())
