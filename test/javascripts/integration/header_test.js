...
var locationLinksSelector = "#site-map-dropdown .location-links";
var categoryLinksSelector = "#site-map-dropdown .category-links";

test("sitemap is rendered correctly", function() {
  expect(35);
  ...
  this.stub(Discourse.Category, "list").returns([
  ...
  visit("/").then(function() {
    ...
    ok($firstCategoryLink.hasClass("badge-category"), "the category link has correct class");
    ...
    var $firstCategoryTopicCount = $categories.first().find(".topic-count");
    ok(exists($firstCategoryTopicCount), "a category item contains a tag with the number of topics");
    equal($firstCategoryTopicCount.text(), "123", "the tag with the number of topics has correct text");
  });
});
