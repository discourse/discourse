...
QUnit.test("Right filters are shown to anonymous users", assert => {
  ...
  expandSelectBox("#select-box-kit-id_in");

  andThen(() => {
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=first]'));
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=pinned]'));
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=unpinned]'));
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=wiki]'));
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=images]'));

    assert.notOk(exists('#select-box-kit-id_in .select-box-kit-row[data-value=unseen]'));
    assert.notOk(exists('#select-box-kit-id_in .select-box-kit-row[data-value=posted]'));
    assert.notOk(exists('#select-box-kit-id_in .select-box-kit-row[data-value=watching]'));
    assert.notOk(exists('#select-box-kit-id_in .select-box-kit-row[data-value=tracking]'));
    assert.notOk(exists('#select-box-kit-id_in .select-box-kit-row[data-value=bookmarks]'));
...
QUnit.test("Right filters are shown to logged-in users", assert => {
  ...
  andThen(() => {
    ...
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=unseen]'));
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=posted]'));
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=watching]'));
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=tracking]'));
    assert.ok(exists('#select-box-kit-id_in .select-box-kit-row[data-value=bookmarks]'));
