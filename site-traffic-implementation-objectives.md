...
7.10b **Partial buckets render fully**: when the selected period ends mid-bucket — e.g., today is a Wednesday and the period ends today, so the current week's Monday-Sunday bucket only has Mon-Wed of data, or today is mid-month for the monthly bucket — the partial bucket still renders as a bar in the chart with whatever data exists within it. The bar is positioned at the bucket's start (Monday for weekly, first-of-month for monthly) and visually extends to its full bucket width; the chart's x-axis is extended past the last bucket's start so the rightmost bar isn't clipped at the chart's right edge. The bucket's tooltip still describes the full bucket range (e.g., "4 May – 10 May 2026" for a partial week) since that is the bucket the bar represents.
