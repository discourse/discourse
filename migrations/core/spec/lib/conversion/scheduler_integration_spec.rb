...
RSpec.describe Migrations::Conversion::StepScheduler, :integration do
  around do |example|
    Dir.mktmpdir do |storage_path|
      ...
      db.execute("CREATE TABLE topics (id INTEGER)")
      ...
      # Build the shard template from the empty schema before any rows go in.
      @shard_manager = Migrations::Conversion::ShardManager.new(canonical_path: @db_path)
      Migrations::Database::IntermediateDB.setup(Migrations::Database::DbWriter.new(path: @db_path))
      begin
        example.run
      ensure
