...
RSpec.describe Migrations::Conversion::PipeProgressSink do
  subject(:sink) { described_class.new(io) }

  let(:io) { StringIO.new }

  # The sink owns the line format in both directions, so pin it with
  # round-trips: whatever the writer emits, `.parse` reads back.
  it "round-trips a progress batch" do
    sink.report_progress(progress: 25, warnings: 1, errors: 2)
  ...
  it "round-trips a result as one line, embedded newlines included" do
    sink.report_result("hosts" => { "evil.example\ncom" => 3 })
