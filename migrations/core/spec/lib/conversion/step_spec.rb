...
RSpec.describe Migrations::Conversion::Step do
  ...
  describe "reads_table defaults" do
    ...
    it "narrows the filter to the chunk when also partitioned" do
      step_class =
        define_step do
          source do
      ...
      source.items
      expect(source_db.select_args).to eq(["things", "active AND id >= 1 AND id < 5"])
    end
  ...
  describe ".partitionable?" do
    it "defaults to false" do
      ...
      expect(define_step.partition_key).to be_nil
    end
    ...
    it "is true when the source declares a partition key" do
      ...
      expect(step.partition_key).to eq(:id)
    end

    it "accepts a composite partition key" do
      step = define_step { source { partition_by %i[topic_id user_id], from: "topic_users" } }

      expect(step.partitionable?).to be(true)
      expect(step.partition_key).to eq(%i[topic_id user_id])
    end
  end
