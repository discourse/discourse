  ...
  module Reporting
    ...
    class Plain < Reporter
      ...
      def report_notice(id, message)
        @mutex.synchronize do
          step = @steps[id]
          @out.puts("    #{step[:title]} #{message}") if step
        end
      end
      ...
      def report_progress(id, current, skip_count, warning_count, error_count)
        @mutex.synchronize do
          ...
          step.merge!(current:, skip_count:, warning_count:, error_count:)
      ...
      def report_finish(id, outcome)
        @mutex.synchronize do
          ...
          line << " #{format_count(count)}" if count.positive?
          line << " (#{format_duration(monotonic - step[:started_at])})"
          line << " — #{outcome}" unless outcome == :done
      ...
      def log_heartbeat(step, current)
        ...
        @out.puts("    #{step[:title]} #{format_count(current)} processed#{annotations(step)}")
      end
