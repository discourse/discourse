  ...
  module Reporting
    class Tui
      ...
      class Renderer
        ...
        RATE_SMOOTHING = 0.3 # EMA weight for each throughput sample; lower = smoother
        ...
        def row_fields(step)
          ...
          if step.total&.positive?
            ...
            if step.rate && elapsed > ETA_AFTER_SECONDS
              remaining = [(step.total - step.current) / step.rate, 0].max
          ...
          fields[:elapsed] = "#{Ansi::DIM}#{format_duration(elapsed)}#{Ansi::RESET}"
          if step.rate && elapsed > RATE_AFTER_SECONDS
