    ...
    class Partitioner
      ...
      def ensure_source_can_partition!
        ...
        raise UnsupportedSourceError,
              "Can't partition `#{@from}`: its source can't compute chunk boundaries " \
                "(missing #{missing.join(", ")}). Implement the partition primitives, " \
                "or remove `partition_by` from the step."
      end

      # The primitives this key needs but the source doesn't provide. A scalar key
      # also strides by value (bounds + estimate); every key needs a way to scan,
      # either the fast `boundaries_by_scan` or `count_all` + `each_partition_key`.
      def missing_primitives
        needed = []
        needed.push(:partition_bounds, :estimated_row_count) unless @key.is_a?(Array)
        unless @source_db.respond_to?(:boundaries_by_scan)
          needed.push(:count_all, :each_partition_key)
        end
        needed.reject { |primitive| @source_db.respond_to?(primitive) }
      end

      def dense?(min, max)
        rows = @source_db.estimated_row_count(@from)
        rows > 0 && (max - min + 1) <= rows * DENSE_RANGE_FACTOR
      end
