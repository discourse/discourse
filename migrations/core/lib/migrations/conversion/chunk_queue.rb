...
module Migrations
  module Conversion
    ...
    class ChunkQueue
      ...
      # The whole bag is written before any worker reads it, so it has to fit in the
      # pipe buffer, or the write would block with no one draining. A real chunk
      # count (fork count times a small multiplier) is a few KB at most, far under
      # this and under the smallest pipe buffer we run on.
      MAX_BYTES = 8 * 1024
