    ...
    module Steps
      class Users < CopyStep
        ...
        requires_mapping :user_ids_by_email,
                         "SELECT LOWER(email) AS email, user_id FROM user_emails"
        ...
        def setup
          @unique_name_finder = UsernameFinder.new(@shared_data)
        ...
        def insert_sanitized_users
          ...
          rows.each do |row|
            ...
            if username != sanitized_username
              @intermediate_db.insert(insert_sql, [row[:original_id]])
            end
          end
        ...
        def transform_row(row)
          ...
          date_of_birth = Database.to_date(row[:date_of_birth])
