  ...
  module Importer
    module Steps
      class Posts < CopyStep
        MAPPED_POSTS_SQL = <<~SQL
          INSERT INTO mapped.posts (original_id, discourse_id, topic_id, post_number)
          VALUES (?, ?, ?, ?)
        SQL
        private_constant :MAPPED_POSTS_SQL
      ...
      FROM posts
           JOIN mapped.ids mapped_topic
             ON posts.topic_id = mapped_topic.original_id AND mapped_topic.type = ?1
           LEFT JOIN mapped.posts mapped_post
             ON posts.original_id = mapped_post.original_id
        ...
        FROM importable_posts
             ...
             LEFT JOIN normalized_posts parent_normalized_post
              ...
              LEFT JOIN mapped.ids mapped_user
                ON importable_posts.user_id = mapped_user.original_id AND mapped_user.type = ?2
              LEFT JOIN mapped.ids mapped_reply_to_user
                ON importable_posts.reply_to_user_id = mapped_reply_to_user.original_id AND mapped_reply_to_user.type = ?2
              LEFT JOIN mapped.ids mapped_last_editor
                ON importable_posts.last_editor_id = mapped_last_editor.original_id AND mapped_last_editor.type = ?2
              LEFT JOIN mapped.ids mapped_deleted_by_user
                ON importable_posts.deleted_by_id = mapped_deleted_by_user.original_id AND mapped_deleted_by_user.type = ?2
              LEFT JOIN mapped.ids mapped_locked_by_user
                ON importable_posts.locked_by_id = mapped_locked_by_user.original_id AND mapped_locked_by_user.type = ?2
    ...
    SQL
        ...
        def insert_mapped_posts(rows)
          rows.each do |row|
            next unless row[:id] && row[:original_id] && row[:topic_id] && row[:post_number]

            @intermediate_db.insert(
              MAPPED_POSTS_SQL,
              [row[:original_id], row[:id], row[:topic_id], row[:post_number]],
            )
          end
        ...
        GROUP BY topic_id
      SQL
            @max_post_number_by_topic_id[row[:topic_id]] = row[:max_post_number]
          end
        end
      end
    end
