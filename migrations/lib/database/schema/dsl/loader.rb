  ...
  class Loader
    ...
    def load_file(filename, optional: false)
      ...
      if File.exist?(path)
        ...
        raise Migrations::Database::Schema::ConfigError, "Required config file not found: #{path}"
      end
    end
    ...
    def load_with_error_handling(path)
      ...
      raise Migrations::Database::Schema::ConfigError, "Error loading #{path}: #{e.message}"
    end
  end
end
