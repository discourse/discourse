  ...
  class ColumnsValidator < BaseValidator
    def validate
      ...
      @schema_config[:tables].each do |table_name, table_config|
        validate_columns_of_table(table_name.to_s, table_config[:columns])
      end
    end

    private

    def validate_columns_of_table(table_name, columns)
      ...
      validated_added_columns(table_name)
      validate_included_columns(table_name)
      validate_excluded_columns(table_name)
      validate_modified_columns(table_name)
      validate_column_usage(table_name)
    end

    def validated_added_columns(table_name)
    ...
    def validate_included_columns(table_name)
    ...
    def validate_excluded_columns(table_name)
    ...
    def validate_modified_columns(table_name)
    ...
    def validate_column_usage(table_name)
