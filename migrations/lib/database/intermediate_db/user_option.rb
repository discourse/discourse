      ...
      module UserOption
        ...
        def self.create(
          ...
          IntermediateDB.insert(
            ...
            auto_track_topics_after_msecs,
            Database.format_boolean(automatically_unpin_topics),
            bookmark_auto_delete_preference,
            color_scheme_id,
            composition_mode,
            dark_scheme_id,
            default_calendar,
            digest_after_minutes,
            Database.format_boolean(dynamic_favicon),
            Database.format_boolean(email_digests),
            Database.format_boolean(email_in_reply_to),
            email_level,
            email_messages_level,
            email_previous_replies,
            Database.format_boolean(enable_allowed_pm_users),
            Database.format_boolean(enable_defer),
            Database.format_boolean(enable_markdown_monospace_font),
            Database.format_boolean(enable_quoting),
            Database.format_boolean(enable_smart_lists),
            Database.format_boolean(external_links_in_new_tab),
            Database.format_boolean(hide_presence),
            Database.format_boolean(hide_profile),
            Database.format_boolean(hide_profile_and_presence),
            homepage_id,
            Database.format_boolean(include_tl0_in_digests),
            interface_color_mode,
            Database.format_datetime(last_redirected_to_top_at),
            like_notification_frequency,
            Database.format_boolean(mailing_list_mode),
            mailing_list_mode_frequency,
            new_topic_duration_minutes,
            notification_level_when_replying,
            Database.format_boolean(notify_on_linked_posts),
            Database.format_datetime(oldest_search_log_date),
            seen_popups,
            Database.format_boolean(sidebar_link_to_filtered_list),
            Database.format_boolean(sidebar_show_count_of_new_items),
            Database.format_boolean(skip_new_user_tips),
            text_size_key,
            text_size_seq,
            theme_ids,
            theme_key_seq,
            timezone,
            title_count_mode_key,
            Database.format_boolean(topics_unread_when_closed),
            Database.format_boolean(watched_precedence_over_muted),
          )
        end
      end
    end
