      ...
      module User
        ...
        def self.create(
          ...
          IntermediateDB.insert(
            SQL,
            original_id,
            Database.format_boolean(active),
            Database.format_boolean(admin),
            Database.format_boolean(approved),
            Database.format_datetime(approved_at),
            approved_by_id,
            avatar_type,
            Database.format_datetime(created_at),
            Database.format_date(date_of_birth),
            Database.format_datetime(first_seen_at),
            flair_group_id,
            group_locked_trust_level,
            Database.format_ip_address(ip_address),
            Database.format_datetime(last_seen_at),
            locale,
            manual_locked_trust_level,
            Database.format_boolean(moderator),
            name,
            original_username,
            primary_group_id,
            Database.format_ip_address(registration_ip_address),
            Database.format_datetime(silenced_till),
            Database.format_boolean(staged),
            title,
            trust_level,
            uploaded_avatar_id,
            username,
