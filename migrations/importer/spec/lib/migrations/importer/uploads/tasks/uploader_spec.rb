...
RSpec.describe Migrations::Importer::Uploads::Tasks::Uploader do
  ...
  subject(:uploader) do
    described_class.allocate.tap do |task|
      ...
      task.instance_variable_set(:@upload_service, upload_service)
  ...
  describe "upload classification" do
    let(:service_class) { Migrations::Importer::Uploads::UploadCreationService }
    ...
    it "records the created upload's file type" do
      ...
      expected_types.each do |filename, file_type|
        allow(upload_service).to receive(:create).and_return(created(filename))

        result = uploader.process({ id: filename }, nil)
