...
module Migrations
  module Importer
    module Uploads
      ...
      # watches CPU, memory, and throughput. A change takes effect at the next
      # item, so no thread has to be stopped or started.
      #
      # `target` is clamped to `[min, max]`. `min` is 1, not the controller's
      # normal floor: a memory emergency may push the target below the floor, and
      ...
      class WorkerGate
        ...
        # A higher target wakes waiting workers, so they can take the new slots.
        # A lower target lets running workers finish their item and admits no new
        # ones until `active` is below it.
        def target=(value)
          @mutex.synchronize do
