  ...
  module Converters
    module Discourse
      class Posts < Conversion::Step
        source do
          # Posts is the heaviest step, so split it across forks. The key is the
          # composite `(topic_id, post_number)`, not `topic_id` alone: posts per
          # topic are wildly uneven (some topics have one post, some tens of
          # thousands), and a single-column key can't split one topic — a huge topic
          # would collapse the NTILE boundaries onto a single value and land whole on
          # one fork. The composite key splits a big topic at a `post_number`, so the
          # row-count-balanced boundaries the partitioner computes for composite keys
          # stay even. Ordering each chunk by `(topic_id, post_number)` also matches
          # the target's `idx_posts_topic_id_post_number`, so the shard and merge
