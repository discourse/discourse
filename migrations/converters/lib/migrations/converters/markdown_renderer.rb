...
module Migrations
  module Converters
    ...
    #
    # An instance is not thread-safe: Markbridge's Discourse renderer keeps an
    # `@interface_cache` for the duration of a render (verified in
    # markbridge-0.3.1, lib/markbridge/renderers/discourse/renderer.rb), so the
    # same instance must not run in two threads at once. Each worker holds its
    # own MarkdownRenderer.
    #
