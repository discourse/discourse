...
RSpec.describe Migrations::Converters::Adapter::Postgres do
  ...
  context "with a stubbed connection" do
    ...
    describe "#select_all and #count_all" do
      ...
      it "orders by the given key so the rows arrive sorted" do
        create_adapter do |adapter|
          allow(adapter).to receive(:query).with(
            'SELECT * FROM "things" WHERE active ORDER BY id',
          ).and_return([:row])
          expect(adapter.select_all("things", where: "active", order: :id)).to eq([:row])
        end
      end

      it "orders by every column of a composite key" do
        create_adapter do |adapter|
          allow(adapter).to receive(:query).with(
            'SELECT * FROM "things" ORDER BY topic_id, user_id',
          ).and_return([:row])
          expect(adapter.select_all("things", order: %i[topic_id user_id])).to eq([:row])
        end
      end
    end
