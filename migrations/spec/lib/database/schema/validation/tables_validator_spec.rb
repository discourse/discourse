...
RSpec.describe ::Migrations::Database::Schema::Validation::TablesValidator do
  ...
  describe "#validate" do
    ...
    context "when table is a copy of another table" do
      it "adds an error if the copied table does not exist" do
        schema_config[:tables][:topics] = {}
        schema_config[:tables][:anonymous_users] = { copy_of: "missing_table" }

        validator.validate
        expect(errors).to contain_exactly(
          I18n.t("schema.validator.tables.copy_table_not_found", table_name: "missing_table"),
        )
      end

      it "validates the columns of the copied table" do
        schema_config[:tables] = { anonymous_users: { copy_of: "users" } }

        validator.validate

        expect(columns_validator).to have_received(:validate).once
        expect(columns_validator).to have_received(:validate).with("anonymous_users", "users")
      end

      it "adds no error if the copied table exists" do
        schema_config[:tables][:topics] = {}
        schema_config[:tables][:anonymous_users] = { copy_of: "users" }
