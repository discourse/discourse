  ...
  module Tooling
    module Schema
      ...
      class << self
        ...
        def effective_ignored_table_names(database: :intermediate_db)
        ...
        def preflight(database: :intermediate_db)
          ensure_ready!(database:)
        ...
        def validate(database: :intermediate_db)
          preflight(database:).errors
        end

        def generate(database: :intermediate_db, output_root: nil)
          ensure_ready!(database:)
        ...
        def diff(database: :intermediate_db)
          ensure_ready!(database:)
          DSL::Differ.new(self).diff
        end

        def add_table(table_name, database: :intermediate_db)
          ensure_ready!(database:)
          DSL::Scaffolder.new(self, table_name, database:).scaffold!
        end

        def ignore_table(table_name, reason: nil, database: :intermediate_db)
          ensure_ready!(database:)
        ...
        def unignore_table(table_name, database: :intermediate_db)
          ensure_ready!(database:)
        ...
        def ensure_ready!(database: :intermediate_db, refresh_manifest: true)
