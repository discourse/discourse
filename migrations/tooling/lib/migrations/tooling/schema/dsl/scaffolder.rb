      ...
      module DSL
        class Scaffolder
          ...
          def generate_table_file
            ...
            lines << ""
            lines << "Migrations::Tooling::Schema.table :#{@table_name} do"
          ...
          def write_file(content)
            tables_dir = File.join(Schema.config_path(@database), "tables")
            FileUtils.mkdir_p(tables_dir)

            path = File.join(tables_dir, "#{@table_name}.rb")

            raise ConfigError, "Table config already exists at #{path}" if File.exist?(path)

            File.write(path, content)
            Helpers.format_ruby_file(path)
            path
          end
        end
      end
