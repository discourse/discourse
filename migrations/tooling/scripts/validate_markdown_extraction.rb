...
# the same production `prepare` / `scan_batches` / `extract_prepared` seam
# the posts step uses.
...
OptionParser
  .new do |parser|
    ...
    parser.on(
      "--keep-db DIR",
      "Keep each worker's intermediate database as DIR/worker-<n>.db instead of a temp dir",
    ) { |v| options[:keep_db] = v }
...
# The same inputs the converter's posts step builds: every username, every
# group, the site's here_mention value and `all` for the mention gate; every
# category slug plus each nested category's `parent:child` path for hashtags.
source_settings = corpus.exec("SELECT name, value FROM site_settings").values.to_h
here_mention = source_settings["here_mention"].presence || "here"
mention_name_list =
  corpus.exec("SELECT username FROM users").column_values(0) +
    corpus.exec("SELECT name FROM groups").column_values(0) + [here_mention, "all"]
category_slugs = []
corpus
  .exec(<<~SQL)
  ...
  SQL
  .each do |row|
    category_slugs << row["slug"]
    category_slugs << "#{row["parent_slug"]}:#{row["slug"]}" if row["parent_slug"]
  end
...
FileUtils.mkdir_p(options[:keep_db]) if options[:keep_db]
...
# A kept database lands at DIR/worker-<n>.db so its rows can be attached and
# diffed against a converter run's; otherwise it lives and dies with the worker.
def with_worker_db_dir(options, partition)
  return Dir.mktmpdir("markdown-validate") { |dir| yield dir } unless options[:keep_db]

  dir = File.join(options[:keep_db], "worker-#{partition[:index]}")
  FileUtils.rm_rf(dir)
...
def run_worker(options, partition, bundle, config_inputs, identity_data, pipe)
  ...
  with_worker_db_dir(options, partition) do |dir|
...
def process_batch(
  ...
  rows.each do |row|
    ...
    prepared_by_id[row["id"]] = extractor.prepare(raw:, id: row["id"], topic_id: row["topic_id"])
  end

  # The extractor partitions the engine-bound bodies itself; a terminated
  # partition yields nothing and its bodies fall back to the per-post path
  # through `extract_prepared(scan_data: nil)`.
  engine_bound = prepared_by_id.values.select(&:engine_bound?)
  expected_batches = (engine_bound.size.to_f / options[:batch]).ceil
  stats["batch_scans"] += expected_batches
  scan_data_by_id = extractor.scan_batches(engine_bound, max_posts: options[:batch])
  stats["batch_failures"] += 1 if engine_bound.any? && scan_data_by_id.empty?
  ...
  rows.each do |row|
    ...
    prepared = prepared_by_id[row["id"]]
    scan_data = scan_data_by_id[row["id"]]
    if prepared.engine_bound?
    ...
    output = extractor.extract_prepared(prepared, scan_data:)

    unless buffer.empty?
      stats["embed_rows"] += buffer.placeholders.size
      stats["written_posts"] += 1
      each_embed_row(buffer) { |kind, _row| stats["rows_by_kind"][kind.to_s] += 1 }
  ...
  partitions.each_with_index.map do |partition, index|
    ...
    pid =
      Migrations::ForkManager.fork do
        reader.close
        run_worker(options, partition.merge(index:), bundle, config_inputs, identity_data, writer)
      end
    writer.close
    { index:, pid:, reader: }
  end
...
# One database with every worker's rows, so a converter run's database can be
# attached next to it and the two compared post by post.
def merge_kept_databases(dir, worker_count)
  merged_path = File.join(dir, "harness.db")
  FileUtils.rm_f(merged_path)
  Migrations::Database.migrate(
    merged_path,
    migrations_path: Migrations::Database::INTERMEDIATE_DB_SCHEMA_PATH,
  )
  merged = Migrations::Database.connect(merged_path)
  tables = %w[
    embed_emojis
    embed_events
    embed_hashtags
  ...
  worker_count.times do |index|
    worker_db = File.join(dir, "worker-#{index}", "intermediate.db")
    next unless File.exist?(worker_db)

    merged.execute("ATTACH DATABASE ? AS worker", worker_db)
    tables.each { |table| merged.execute("INSERT INTO #{table} SELECT * FROM worker.#{table}") }
    merged.execute("DETACH DATABASE worker")
  end
  merged.close
  merged_path
end

merged_db = merge_kept_databases(options[:keep_db], children.size) if options[:keep_db]
...
rows_by_kind = Hash.new(0)
ok.each do |result|
  (result["rows_by_kind"] || {}).each { |kind, count| rows_by_kind[kind] += count }
end
puts "  by kind: #{rows_by_kind.sort.map { |kind, count| "#{kind} #{number(count)}" }.join(", ")}"
puts "  all rows kept in #{merged_db}" if merged_db
