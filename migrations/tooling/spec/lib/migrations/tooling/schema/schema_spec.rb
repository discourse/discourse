...
RSpec.describe Migrations::Tooling::Schema, :rails do
  after { described_class.reset! }

  describe ".validate" do
    it "includes resolved schema validation errors" do
      resolved =
        Migrations::Tooling::Schema::Definition.new(
          tables: [
    ...
    it "runs static validation before resolution in preflight" do
      validator =
        instance_double(
          Migrations::Tooling::Schema::DSL::Validator,
          validate: ["Table 'users': bad config"],
        )

      allow(described_class).to receive(:ensure_ready!).with(database: :test_db)
      allow(Migrations::Tooling::Schema::DSL::Validator).to receive(:new).with(
        described_class,
      ).and_return(validator)
      allow(Migrations::Tooling::Schema::DSL::SchemaResolver).to receive(:new)

      result = described_class.preflight(database: :test_db)

      expect(result.errors).to eq(["Table 'users': bad config"])
      expect(Migrations::Tooling::Schema::DSL::SchemaResolver).not_to have_received(:new)
    end
  end

  describe ".ignore_table" do
    let(:editor) do
      instance_double(Migrations::Tooling::Schema::DSL::IgnoredFileEditor, add_table: nil)
    end
    ...
    it "adds the table to ignored.rb after validating it" do
      ...
      allow(Migrations::Tooling::Schema::DSL::IgnoredFileEditor).to receive(:new).with(
        "/tmp/schema",
      ).and_return(editor)
