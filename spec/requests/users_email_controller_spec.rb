...
describe UsersEmailController do
  describe '#confirm' do
    ...
    context 'valid old address token' do
      ...
      before { updater.change_to('new.n.cool@example.com') }
      ...
      it 'confirms with a correct token' do
        ...
        expect(body).to include(
              I18n.t('change_email.authorizing_old.description')
            )
      end
    end
    ...
    context 'valid new address token' do
      ...
      it 'confirms with a correct token' do
        user.user_stat.update_columns(
          bounce_score: 42, reset_bounce_score_after: 1.week.from_now
        )
      ...
      it 'automatically adds the user to a group when the email matches' do
        group =
          Fabricate(:group, automatic_membership_email_domains: 'example.com')
      ...
      context 'second factor required' do
        ...
        it 'requires a second factor token' do
          ...
          expect(response_body).not_to include(
  ...
  describe '#update' do
    ...
    context 'when logged in' do
      ...
      it 'raises an error without an invalid email' do
        put "/u/#{user.username}/preferences/email.json",
            params: { email: "sam@not-email.com'" }
        expect(response.status).to eq(422)
        expect(response.body).to include('email is invalid')
      end

      it "raises an error if you can't edit the user's email" do
        Guardian.any_instance.expects(:can_edit_email?).with(user).returns(
          false
        )
      ...
      context 'when new email is different case of existing email' do
        let!(:other_user) do
          Fabricate(:user, email: 'case.insensitive@gmail.com')
        end

        it 'raises an error' do
          put "/u/#{user.username}/preferences/email.json",
              params: { email: other_user.email.upcase }
      ...
      it 'raises an error when new email domain is present in email_domains_blacklist site setting' do
        ...
        put "/u/#{user.username}/preferences/email.json",
            params: { email: 'not_good@mailinator.com' }
