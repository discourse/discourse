...
RSpec.describe Admin::FlagsController do
  ...
  context '#agree' do
    ...
    it 'should be able to hide spam' do
      ...
      agree_post =
        Topic.joins(:topic_allowed_users).where(
          'topic_allowed_users.user_id = ?',
          user.id
        )
          .order(:id)
          .last
          .posts
          .last
      expect(agree_post.raw).to eq(
            I18n.with_locale(:en) do
              I18n.t('flags_dispositions.agreed_and_deleted')
            end
          )
    ...
    it 'should not delete category topic' do
      ...
      post "/admin/flags/agree/#{first_post.id}.json",
           params: { action_on_post: 'delete' }
