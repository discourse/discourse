...
RSpec.describe Admin::SiteTextsController do
  ...
  context 'when not logged in as an admin' do
    ...
    it "raises an error if you aren't an admin" do
      ...
      put '/admin/customize/reseed.json',
          params: { category_ids: [], topic_ids: [] }
      expect(response.status).to eq(404)
    end
  end
  ...
  context 'when logged in as admin' do
    ...
    describe '#index' do
      it 'returns json' do
        ...
        expect(JSON.parse(response.body)['site_texts']).to include(
              include('id' => 'title')
            )
      end
      ...
      it 'normalizes ellipsis' do
        ...
        ['Loading Discussion', 'Loading Discussion...', 'Loading Discussion…']
          .each do |search_term|
          get '/admin/customize/site_texts.json', params: { q: search_term }
          expect(response.status).to eq(200)
          expect(JSON.parse(response.body)['site_texts']).to include(
    ...
    describe '#update & #revert' do
      ...
      it 'works as expectd with correct keys' do
        put '/admin/customize/site_texts/login_required.welcome_message.json',
            params: { site_text: { value: 'foo' } }
      ...
      it 'returns the right error message' do
        ...
        put '/admin/customize/site_texts/some_key.json',
            params: { site_text: { value: 'hello %{key} %{omg}' } }
      ...
      it 'returns site texts for the correct locale' do
        ...
        put '/admin/customize/site_texts/title.json',
            params: { site_text: { value: ru_title } }
        expect(response.status).to eq(200)
        put '/admin/customize/site_texts/js.topic.read_more_MF.json',
            params: { site_text: { value: ru_mf_text } }
        expect(response.status).to eq(200)
    ...
    context 'reseeding' do
      before do
        staff_category =
          Fabricate(:category, name: 'Staff EN', user: Discourse.system_user)
      ...
      describe '#reseed' do
        it 'reseeds categories and topics' do
          ...
          post '/admin/customize/reseed.json',
               params: {
                 category_ids: %w[staff_category_id],
                 topic_ids: %w[guidelines_topic_id]
               }
          expect(response.status).to eq(200)

          expect(Category.find(SiteSetting.staff_category_id).name).to eq(
                I18n.t('staff_category_name')
              )
