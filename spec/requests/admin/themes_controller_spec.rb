...
describe Admin::ThemesController do
  ...
  describe '#export' do
    it 'exports correctly' do
      ...
      theme.set_field(
        target: :common, name: :scss, value: '.body{color: black;}'
      )
      theme.set_field(
        target: :desktop, name: :after_header, value: '<b>test</b>'
      )
      theme.save!
      ...
      file.rewind
      uploaded_file =
        Rack::Test::UploadedFile.new(file.path, 'application/x-gzip')
  ...
  describe '#import' do
    ...
    let(:theme_archive) do
      Rack::Test::UploadedFile.new(
        file_from_fixtures('discourse-test-theme.tar.gz', 'themes'),
        'application/x-gzip'
      )
    end
    ...
    it 'imports a theme' do
      ...
      expect(json['theme']['theme_fields'].length).to eq(2)
      expect(
        UserHistory.where(action: UserHistory.actions[:change_theme]).count
      ).to eq(1)
    end
    ...
    it 'updates an existing theme from an archive by id' do
      ...
      expect do
        post '/admin/themes/import.json',
             params: {
               bundle: theme_archive, theme_id: other_existing_theme.id
             }
      end.to change { Theme.count }.by (0)
      expect(response.status).to eq(201)
      json = ::JSON.parse(response.body)
    ...
    it 'creates a new theme when id specified as nil' do
      ...
      expect do
        post '/admin/themes/import.json',
             params: { bundle: theme_archive, theme_id: nil }
      end.to change { Theme.count }.by (1)
      expect(response.status).to eq(201)
      json = ::JSON.parse(response.body)
      ...
      post '/admin/themes.json',
           params: {
             theme: {
               name: 'my test name',
               theme_fields: [
                 name: 'scss', target: 'common', value: 'body{color: red;}'
               ]
             }
           }
  ...
  describe '#update' do
    ...
    it 'can change default theme' do
      ...
      put "/admin/themes/#{theme.id}.json",
          params: { id: theme.id, theme: { default: true } }
    ...
    it 'updates a theme' do
      ...
      fields =
        json['theme']['theme_fields'].sort { |a, b| a['value'] <=> b['value'] }
    ...
    it 'can update translations' do
      theme.set_field(
        target: :translations,
        name: :en,
        value: { en: { somegroup: { somestring: 'defaultstring' } } }
          .deep_stringify_keys
          .to_yaml
      )
      theme.save!
      ...
      put "/admin/themes/#{theme.id}.json",
          params: {
            theme: {
              translations: { 'somegroup.somestring' => 'overridenstring' }
            }
          }
      ...
      put "/admin/themes/#{theme.id}.json",
          params: {
            theme: {
              translations: { 'somegroup.somestring' => 'defaultstring' }
            }
          }
      # Response correct
      expect(response.status).to eq(200)
    ...
    it 'handles import errors on update' do
      ...
      RemoteTheme.any_instance.stubs(:update_from_remote).raises(
        RemoteTheme::ImportError.new('error message')
      )
      put "/admin/themes/#{theme.id}.json",
          params: { theme: { remote_update: true } }
  ...
  describe '#destroy' do
    ...
    it "deletes the field's javascript cache" do
      ...
      expect { javascript_cache.reload }.to raise_error(
