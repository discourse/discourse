  ...
  context 'while logged in as an admin' do
    ...
    describe '#create' do
      ...
      it 'returns 404 when that user does not exist' do
        post '/admin/impersonate.json',
             params: { username_or_email: 'hedonismbot' }
      ...
      it "raises an invalid access error if the user can't be impersonated" do
        post '/admin/impersonate.json',
             params: { username_or_email: another_admin.email }
      ...
      context 'success' do
        it 'succeeds and logs the impersonation' do
          expect do
            post '/admin/impersonate.json',
                 params: { username_or_email: user.username }
          end.to change do
            UserHistory.where(action: UserHistory.actions[:impersonate]).count
          end
            .by(1)
        ...
        it 'also works with an email address' do
          post '/admin/impersonate.json',
               params: { username_or_email: user.email }
          expect(response.status).to eq(200)
          expect(session[:current_user_id]).to eq(user.id)
        end
      end
