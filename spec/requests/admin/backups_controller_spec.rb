...
RSpec.describe Admin::BackupsController do
    ...
    context 'json format' do
      it 'returns a list of all the backups' do
        begin
          ...
          filenames =
            JSON.parse(response.body).map { |backup| backup['filename'] }
  ...
  describe '#create' do
    it 'starts a backup' do
      ...
      post '/admin/backups.json',
           params: { with_uploads: false, client_id: 'foo' }
    ...
    it 'returns 422 when token is bad' do
      begin
        get "/admin/backups/#{backup_filename}.json",
            params: { token: 'bad_value' }

        expect(response.status).to eq(422)
        expect(response.headers['Content-Disposition']).not_to match(
                  /attachment; filename/
                )
      end
    end
  ...
  describe '#destroy' do
    it 'removes the backup if found' do
      begin
        ...
        expect { delete "/admin/backups/#{backup_filename}.json" }.to change do
  ...
  describe '#restore' do
    it 'starts a restore' do
      ...
      post "/admin/backups/#{backup_filename}/restore.json",
  ...
  describe '#readonly' do
    it 'enables readonly mode' do
      ...
      expect do
        ...
        UserHistory.where(
          action: UserHistory.actions[:change_readonly_mode], new_value: 't'
        )
          .count
      end
        .by(1)
    ...
    it 'disables readonly mode' do
      ...
      expect do
        put '/admin/backups/readonly.json', params: { enable: false }
      end.to change do
        UserHistory.where(
          action: UserHistory.actions[:change_readonly_mode], new_value: 'f'
        )
          .count
      end
        .by(1)
  ...
  describe '#email' do
    it 'enqueues email job' do
      ...
      expect { put "/admin/backups/#{backup_filename}.json" }.to change do
