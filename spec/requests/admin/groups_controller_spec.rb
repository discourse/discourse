...
RSpec.describe Admin::GroupsController do
  ...
  describe '#add_owners' do
    it 'should work' do
      put "/admin/groups/#{group.id}/owners.json",
          params: {
            group: { usernames: [user.username, admin.username].join(',') }
          }
      ...
      expect(
        group.group_users.where(owner: true).map(&:user)
  ...
  describe '#bulk_perform' do
    let(:group) do
      Fabricate(
        :group,
        name: 'test', primary_group: true, title: 'WAT', grant_trust_level: 3
      )
    end
    ...
    it 'can assign users to a group by email or username' do
      ...
      put '/admin/groups/bulk.json',
          params: {
            group_id: group.id,
            users: [user.username.upcase, user2.email, 'doesnt_exist']
          }
