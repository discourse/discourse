...
describe WebhooksController do
  ...
  context 'mailgun' do
    ...
    let(:signature) do
      OpenSSL::HMAC.hexdigest('SHA256', SiteSetting.mailgun_api_key, data)
    end
    ...
    it 'works (deprecated)' do
      ...
      email_log =
        Fabricate(
          :email_log,
          user: user, message_id: message_id, to_address: email
        )
      ...
      expect(email_log.user.user_stat.bounce_score).to eq(
             ...
             '_json' => [
               {
                 'msys' => {
                   'message_event' => {
                     'bounce_class' => 10,
                     'rcpt_to' => email,
                     'rcpt_meta' => { 'message_id' => message_id }
                   }
                 }
               }
             ]
