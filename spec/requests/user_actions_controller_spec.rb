...
describe UserActionsController do
  context 'index' do
    ...
    it 'can be filtered by acting_username' do
      ...
      get '/user_actions.json',
          params: {
            username: post.user.username, acting_username: user.username
          }
      ...
      expect(response_body['user_actions'].first['acting_username']).to eq(
            user.username
          )
    end
    ...
    it 'renders help text if provided for self' do
      ...
      expect(parsed['no_results_help']).to eq(
    ...
    it 'renders help text for others' do
      ...
      expect(parsed['no_results_help']).to eq(
            I18n.t('user_activity.no_bookmarks.others')
          )
    end
    ...
    context 'queued posts' do
      context 'without access' do
        ...
        it 'raises an exception' do
          get '/user_actions.json',
              params: { username: user.username, filter: UserAction::PENDING }
      ...
      context 'with access' do
        ...
        it 'finds queued posts' do
          queued_post =
            PostEnqueuer.new(user, 'default').enqueue(
              raw: 'this is the raw enqueued content'
            )
