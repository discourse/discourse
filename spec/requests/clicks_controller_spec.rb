    ...
    context 'correct params' do
      ...
      context 'with a made up url' do
        it "doesn't redirect" do
          get '/clicks/track',
              params: { url: 'https://fakewebsite.com', post_id: post.id },
              headers: headers
      ...
      context 'with a valid url' do
        it 'redirects' do
          get '/clicks/track',
              params: {
                url: 'https://discourse.org/?hello=123', post_id: post.id
              },
              headers: headers
      ...
      context 'with a post_id' do
        it 'redirects' do
          get '/clicks/track',
              params: { url: url, post_id: post.id }, headers: headers
        ...
        it 'redirects links in whispers to staff members' do
          ...
          get '/clicks/track',
              params: { url: url, post_id: whisper.id }, headers: headers
        ...
        it "doesn't redirect with the redirect=false param" do
          get '/clicks/track',
              params: { url: url, post_id: post.id, redirect: 'false' },
              headers: headers
          expect(response).not_to be_redirect
        end
      end
      ...
      context 'with a topic_id' do
        it 'redirects' do
          get '/clicks/track',
              params: { url: url, topic_id: post.topic.id }, headers: headers
          expect(response).to redirect_to(url)
        end
      end
    end
