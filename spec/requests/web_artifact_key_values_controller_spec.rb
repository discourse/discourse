...
RSpec.describe WebArtifactKeyValuesController do
  ...
  fab!(:artifact) do
    Fabricate(:web_artifact, post: private_message_post, metadata: { public: true })
  end
  fab!(:private_artifact) { Fabricate(:web_artifact, post: private_message_post) }

  describe "#index" do
    fab!(:public_key_value) do
      Fabricate(
        :web_artifact_key_value,
    ...
    context "when not logged in" do
      it "returns only public key values" do
        get "/web-artifact-key-values/#{artifact.id}.json", params: { all_users: true }
      ...
      it "returns 404 for non-existent artifact" do
        get "/web-artifact-key-values/999999.json"
        expect(response.status).to eq(404)
      end
    ...
    context "when logged in as regular user" do
      ...
      it "shows all users' key values when all_users=true" do
        get "/web-artifact-key-values/#{artifact.id}.json?all_users=true"
      ...
      it "filters by key when specified" do
        get "/web-artifact-key-values/#{artifact.id}.json?key=test_key"
      ...
      it "returns keys only when keys_only=true" do
        get "/web-artifact-key-values/#{artifact.id}.json?keys_only=true"
    ...
    context "when paginating" do
      ...
      it "paginates results correctly" do
        get "/web-artifact-key-values/#{artifact.id}.json?per_page=5&page=1"
      ...
      it "respects per_page limit" do
        get "/web-artifact-key-values/#{artifact.id}.json?per_page=200"
      ...
      it "defaults to page 1 for invalid page numbers" do
        get "/web-artifact-key-values/#{artifact.id}.json?page=0"
  ...
  describe "#destroy" do
    ...
    context "when not logged in" do
      it "returns 403 forbidden" do
        delete "/web-artifact-key-values/#{artifact.id}/delete_me.json"
    ...
    context "when logged in" do
      ...
      it "returns 404 for non-existent key" do
        delete "/web-artifact-key-values/#{artifact.id}/non_existent_key.json"
      ...
      it "returns 404 when trying to delete another user's key" do
        delete "/web-artifact-key-values/#{artifact.id}/other_delete_me.json"
      ...
      it "returns 404 for non-existent artifact" do
        delete "/web-artifact-key-values/999999/delete_me.json"
      ...
      it "returns 404 for private artifact user cannot see" do
        ...
        private_artifact = Fabricate(:web_artifact, post: private_post)
        Fabricate(
          :web_artifact_key_value,
          web_artifact: private_artifact,
          user: other_user,
          key: "private_key",
        )

        delete "/web-artifact-key-values/#{private_artifact.id}/private_key.json"
      ...
      context "with URL encoding" do
        ...
        it "handles URL encoded keys correctly" do
          expect {
            delete "/web-artifact-key-values/#{artifact.id}.json",
                   params: {
                     key: "key with spaces & symbols!",
                   }
            expect(response.status).to eq(200)
    ...
    context "when logged in as admin" do
      ...
      it "can delete own keys" do
        ...
        expect {
          delete "/web-artifact-key-values/#{artifact.id}/admin_key.json"
          expect(response.status).to eq(200)
        }.to change { artifact.key_values.count }.by(-1)
      end
      ...
      it "can access private artifacts" do
        ...
        expect {
          delete "/web-artifact-key-values/#{private_artifact.id}/private_admin_key.json"
          expect(response.status).to eq(200)
  ...
  describe "#set" do
    ...
    context "when not logged in" do
      it "returns 403 forbidden" do
        post "/web-artifact-key-values/#{artifact.id}.json", params: valid_params
    ...
    context "when logged in" do
      ...
      it "creates a new key value successfully" do
        ...
        json = response.parsed_body
        key_value = json["web_artifact_key_value"]
      ...
      it "returns 404 for non-existent artifact" do
        post "/web-artifact-key-values/999999.json", params: valid_params
      ...
      it "returns 404 for private artifact user cannot see" do
        ...
        post "/web-artifact-key-values/#{private_artifact.id}.json", params: valid_params
