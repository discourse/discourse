...
RSpec.describe SuperAdmin::EmailTemplatesController do
  fab!(:admin)
  fab!(:moderator)
  fab!(:user)
  ...
  describe "#index" do
    context "when logged in as an admin" do
      ...
      it "includes custom email template keys added via modifier" do
        ...
        modified_keys = SuperAdmin::EmailTemplatesController.email_keys
  ...
  it "uses only existing email templates" do
    SuperAdmin::EmailTemplatesController.email_keys.each do |key|
    ...
    it "returns all email templates except security-restricted ones" do
      expected_keys =
        EmailTemplatesFinder.list.reject do |key|
          SuperAdmin::EmailTemplatesController.restricted_key?(key)
        end
      expect(SuperAdmin::EmailTemplatesController.email_keys).to contain_exactly(*expected_keys)
    end
  end
end
