  ...
  describe "#show" do
    ...
    context "when the article is valid" do
      ...
      context "when login is required" do
        ...
        it "returns 200 when a user is connected" do
          sign_in(user)
          get published_page.path
          expect(response.status).to eq(200)
        end

        it "redirects a disconnected user to the login page" do
          expect(get(published_page.path)).to redirect_to("/login")
        end

        it "returns 200 when published pages are shown despite required login" do
