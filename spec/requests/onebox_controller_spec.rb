  ...
  describe 'logged in' do
    ...
    describe 'local onebox' do
      it 'does not cache local oneboxes' do
        ...
        get '/onebox.json',
            params: { url: url, category_id: post.topic.category_id }
    ...
    it 'does not allow onebox of PMs' do
      post =
        create_post(
          archetype: 'private_message', target_usernames: [user.username]
        )
      url = Discourse.base_url + post.url
    ...
    it 'does not allow whisper onebox' do
      post = create_post
      whisper =
        create_post(topic_id: post.topic_id, post_type: Post.types[:whisper])
