...
RSpec.describe TagsController do
  ...
  describe "#bulk_create" do
    ...
    context "while logged in" do
      it "fails if you can't manage tags" do
        sign_in(user)
        post "/tags/bulk_create.json", params: { tag_names: %w[tag1 tag2] }
        expect(response.status).to eq(403)
      end

      it "allows staff to bulk create tags" do
        sign_in(moderator)

        expect do
          post "/tags/bulk_create.json", params: { tag_names: %w[tag1 tag2 tag3] }
        end.to change { Tag.count }.by(3)

        expect(response.status).to eq(200)
        result = response.parsed_body

        expect(result["created"]).to contain_exactly("tag1", "tag2", "tag3")
        expect(result["existing"]).to be_empty
        expect(result["failed"]).to be_empty
      end

      it "reports existing tags" do
        Fabricate(:tag, name: "existing-tag")
        sign_in(moderator)

        expect do
          post "/tags/bulk_create.json", params: { tag_names: %w[existing-tag new-tag] }
        end.to change { Tag.count }.by(1)

        expect(response.status).to eq(200)
        result = response.parsed_body

        expect(result["created"]).to contain_exactly("new-tag")
        expect(result["existing"]).to contain_exactly("existing-tag")
        expect(result["failed"]).to be_empty
      end
      ...
      it "cleans tag names" do
        ...
        expect do
          post "/tags/bulk_create.json", params: { tag_names: ["Tag With Spaces", "UPPERCASE"] }
        end.to change { Tag.count }.by(2)

        expect(response.status).to eq(200)
        result = response.parsed_body

        expect(result["created"]).to contain_exactly("tag-with-spaces", "uppercase")
      end

      it "handles invalid tag names" do
        sign_in(moderator)

        post "/tags/bulk_create.json", params: { tag_names: ["", "valid-tag", "   "] }

        expect(response.status).to eq(200)
        result = response.parsed_body

        expect(result["created"]).to contain_exactly("valid-tag")
      ...
      it "returns error for invalid parameters" do
        ...
        post "/tags/bulk_create.json", params: { tag_names: "not an array" }

        expect(response.status).to eq(422)
      end

      it "handles empty tag_names array" do
        sign_in(moderator)

        post "/tags/bulk_create.json", params: { tag_names: [] }
      ...
      it "rejects tags with invalid characters instead of creating cleaned version" do
        sign_in(moderator)

        expect do
          post "/tags/bulk_create.json", params: { tag_names: %w[%^&ui^&* valid-tag !!!test] }
        end.to change { Tag.count }.by(1)
        ...
        expect(result["created"]).to contain_exactly("valid-tag")
        expect(result["existing"]).to be_empty
        expect(result["failed"].keys).to contain_exactly("%^&ui^&*", "!!!test")
      end

      it "provides specific error message for tags that are too long" do
        sign_in(moderator)
        long_tag = "a" * (SiteSetting.max_tag_length + 1)

        expect do
          post "/tags/bulk_create.json", params: { tag_names: [long_tag, "short-tag"] }
        end.to change { Tag.count }.by(1)

        expect(response.status).to eq(200)
        result = response.parsed_body

        expect(result["created"]).to contain_exactly("short-tag")
