...
describe TagsController do
  ...
  describe '#show' do
    before { Fabricate(:tag, name: 'test') }
    ...
    it 'does not show staff-only tags' do
      tag_group =
        Fabricate(
          :tag_group,
          permissions: { 'staff' => 1 }, tag_names: %w[test]
        )
    ...
    it 'triggers a extensibility event' do
      event =
        DiscourseEvent.track_events do
          put "/tags/#{tag.name}.json", params: { tag: { id: 'hello' } }
        end
          .last
  ...
  describe '#show_latest' do
    ...
    context 'tagging enabled' do
      ...
      it 'can filter by two tags' do
        ...
        get "/tags/#{tag.name}/l/latest.json",
            params: { additional_tag_ids: other_tag.name }

        expect(response.status).to eq(200)

        topic_ids =
          JSON.parse(response.body)['topic_list']['topics'].map do |topic|
            topic['id']
          end
      ...
      it "does not find any tags when a tag which doesn't exist is passed" do
        ...
        get "/tags/#{tag.name}/l/latest.json",
            params: { additional_tag_ids: 'notatag' }
      ...
      it 'can filter by category, sub-category, and tag' do
        get "/tags/c/#{category.slug}/#{subcategory.slug}/#{tag
              .name}/l/latest.json"
        expect(response.status).to eq(200)
      end
      ...
      it 'can handle subcategories with the same name' do
        ...
        get "/tags/c/#{category2.slug}/#{subcategory2.slug}/#{other_tag
              .name}/l/latest.json"
  ...
  describe '#search' do
    ...
    context 'tagging enabled' do
      ...
      it 'returns tags ordered by topic_count, and prioritises exact matches' do
        ...
        expect(response.status).to eq(200)
        json = ::JSON.parse(response.body)
        expect(json['results'].map { |j| j['id'] }).to eq(%w[tag tag2])
      end
      ...
      context 'with category restriction' do
        ...
        it 'can say if given tag is not allowed' do
          ...
          get '/tags/filter/search.json',
              params: { q: nope.name, categoryId: category.id }
          expect(response.status).to eq(200)
          json = ::JSON.parse(response.body)
          expect(json['results'].map { |j| j['id'] }.sort).to eq([])
          expect(json['forbidden']).to be_present
          expect(json['forbidden_message']).to eq(
        ...
        it 'can say if given tag is restricted to different category' do
          category
          get '/tags/filter/search.json',
              params: { q: yup.name, categoryId: Fabricate(:category).id }
          json = ::JSON.parse(response.body)
          expect(json['results'].map { |j| j['id'] }.sort).to eq([])
          expect(json['forbidden']).to be_present
          expect(json['forbidden_message']).to eq(
                I18n.t(
                  ...
                  count: 1, tag_name: yup.name, category_names: category.name
                )
              )
        end
      end
      ...
      it 'matches tags after sanitizing input' do
        ...
        expect(response.status).to eq(200)
        json = ::JSON.parse(response.body)
        expect(json['results'].map { |j| j['id'] }.sort).to eq(%w[nope])
      end
      ...
      it 'can return tags that are in secured categories but are allowed to be used' do
        ...
        expect(response.status).to eq(200)
        json = ::JSON.parse(response.body)
        expect(json['results'].map { |j| j['id'] }).to eq(%w[cooltag])
      end
      ...
      it 'supports Chinese and Russian' do
        ...
        expect(response.status).to eq(200)
        json = ::JSON.parse(response.body)
        expect(json['results'].map { |j| j['id'] }).to eq(%w[房地产])

        get '/tags/filter/search.json', params: { q: 'тема' }
        expect(response.status).to eq(200)
        json = ::JSON.parse(response.body)
        expect(json['results'].map { |j| j['id'] }).to eq(%w[тема-в-разработке])
      end
    end
  end
      ...
      context 'with some tags' do
        let!(:tags) do
          [
            Fabricate(
              :tag,
              name: 'used_publically', topic_count: 2, pm_topic_count: 0
            ),
            Fabricate(
              :tag,
              name: 'used_privately', topic_count: 0, pm_topic_count: 3
            ),
            Fabricate(
              :tag,
              name: 'used_everywhere', topic_count: 0, pm_topic_count: 3
            ),
  ...
  context '#upload_csv' do
    ...
    context 'while logged in' do
      ...
      let(:invalid_csv_file) do
        File.new("#{Rails.root}/spec/fixtures/csv/tags_invalid.csv")
      end
      ...
      it 'allows staff to bulk upload tags' do
        ...
        expect(
          Tag.find_by_name('tag3').tag_groups.pluck(:name)
        ).to contain_exactly('taggroup1')
        expect(
          Tag.find_by_name('tag4').tag_groups.pluck(:name)
        ).to contain_exactly('taggroup1')
      end
      ...
      it 'fails gracefully with invalid input' do
        ...
        expect do
          post '/tags/upload.json',
               params: { file: invalid_file, name: filename }
