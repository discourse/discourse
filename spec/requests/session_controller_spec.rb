...
RSpec.describe SessionController do
  ...
  describe '#email_login' do
    ...
    context 'valid token' do
      ...
      context 'when IP address is blocked' do
        ...
        it 'returns the right response' do
          ActionDispatch::Request.any_instance.stubs(:remote_ip).returns(
            permitted_ip_address
          )
          ...
          expect(CGI.unescapeHTML(response.body)).to include(
                I18n.t(
                  'login.not_allowed_from_ip_address',
                  username: user.username
                )
              )
        end
      ...
      it 'fails when user is suspended' do
        user.update!(
          suspended_till: 2.days.from_now, suspended_at: Time.zone.now
        )
      ...
      context 'user has 2-factor logins' do
        ...
        let!(:user_second_factor_backup) do
        ...
        describe 'requires second factor' do
          it 'should return a second factor prompt' do
            ...
            expect(response_body).to_not include(
        ...
        describe 'errors on incorrect 2-factor' do
          context 'when using totp method' do
            it 'does not log in with incorrect two factor' do
              post "/session/email-login/#{email_token.token}",
                   params: {
  ...
  describe '#sso_login' do
    ...
    it 'does not create superflous auth tokens when already logged in' do
      ...
      expect do
        get '/session/sso_login',
            params: Rack::Utils.parse_query(sso.payload), headers: headers
        logged_on_user =
          Discourse.current_user_provider.new(request.env).current_user
    ...
    it 'can take over an account' do
      ...
      expect(logged_on_user.single_sign_on_record.external_username).to eq(
            'sam'
          )
    end
    ...
    it 'respects IP restrictions on login' do
      ...
      get '/'
      sso = sso_for_ip_specs
      DiscourseSingleSignOn.parse(sso.payload).lookup_or_create_user(
        request.remote_ip
      )
    ...
    it 'allows you to create an account' do
      ...
      expect(logged_on_user.custom_fields['shop_url']).to eq(
            'http://my_shop.com'
          )
    ...
    it 'allows login to existing account with valid nonce' do
      ...
      expect(user.single_sign_on_record.last_payload).to eq(
            sso.unsigned_payload
          )
    ...
    it 'returns the correct error code for invalid signature' do
      ...
      get '/session/sso_login',
          params:
            correct_params.merge(:"sig" => 'thisisnotthesigyouarelookingfor'),
          headers: headers
      ...
      get '/session/sso_login',
          params:
            correct_params.merge(
              :"sig" => 'thisisasignaturewith@special!characters'
            ),
          headers: headers
      expect(response.status).to eq(422)
      expect(response.body).not_to include(correct_params['sig']) # Check we didn't send the real sig back to the client
    ...
    describe 'local attribute override from SSO payload' do
      before do
        ...
        @suggested_username =
          UserNameSuggester.suggest(@sso.username || @sso.name || @sso.email)
        @suggested_name =
          User.suggest_name(@sso.name || @sso.username || @sso.email)
        @user.create_single_sign_on_record(external_id: '997', last_payload: '')
      end
      ...
      it 'stores the external attributes' do
        get '/session/sso_login',
            params: Rack::Utils.parse_query(@sso.payload), headers: headers
        @user.single_sign_on_record.reload
        expect(@user.single_sign_on_record.external_username).to eq(
              @sso.username
            )
  ...
  describe '#sso_provider' do
    ...
    describe 'can act as an SSO provider' do
      before do
        stub_request(:any, %r{#{Discourse.current_hostname}\/uploads})
          .to_return(
          status: 200, body: lambda { |request| file_from_fixtures('logo.png') }
        )
        ...
        @user =
          Fabricate(
            :user,
            password: 'myfrogs123ADMIN', active: true, admin: true
          )
      ...
      it 'handles non local content correctly' do
        ...
        stub_request(:any, /test.s3.dualstack.us-east-1.amazonaws.com/)
          .to_return(status: 200, body: '', headers: { referer: 'fgdfds' })

        @user.create_user_avatar!
        upload =
          Fabricate(
            :upload,
            url: '//test.s3.dualstack.us-east-1.amazonaws.com/something'
          )
        ...
        @user.user_profile.update_columns(
          profile_background:
            '//test.s3.dualstack.us-east-1.amazonaws.com/something',
          card_background:
            '//test.s3.dualstack.us-east-1.amazonaws.com/something'
        )
        ...
        stub_request(:get, 'http://cdn.com/something/else').to_return(
          body:
            lambda do |request|
              File.new(Rails.root + 'spec/fixtures/images/logo.png')
            end
        )
        ...
        expect(sso2.avatar_url).to start_with(
              "#{SiteSetting.s3_cdn_url}/original"
            )
        expect(sso2.profile_background_url).to start_with(
  ...
  describe '#create' do
    ...
    context 'local login is disabled' do
      before do
        ...
        post '/session.json',
             params: { login: user.username, password: 'myawesomepassword' }
      end
    ...
    context 'when email is confirmed' do
      ...
      describe 'invalid password' do
        it 'should return an error with an invalid password' do
          post '/session.json',
               params: { login: user.username, password: 'sssss' }
      ...
      describe 'suspended user' do
        it 'should return an error' do
          ...
          user.save!
          StaffActionLogger.new(user).log_user_suspend(
            user,
            '<strike>banned</strike>'
          )
      ...
      describe 'success by username' do
        it 'logs in correctly' do
          ...
          expect(UserAuthToken.hash_token(cookies[:_t])).to eq(
      ...
      describe 'also allow login by email' do
        it 'sets a session id' do
          post '/session.json',
               params: { login: user.email, password: 'myawesomepassword' }
          expect(response.status).to eq(200)
          expect(session[:current_user_id]).to eq(user.id)
        end
      end
      ...
      context 'login has leading and trailing space' do
        ...
        it 'strips spaces from the username' do
          post '/session.json',
               params: { login: username, password: 'myawesomepassword' }
          expect(response.status).to eq(200)
        ...
        it 'strips spaces from the email' do
          post '/session.json',
               params: { login: email, password: 'myawesomepassword' }
          expect(response.status).to eq(200)
          expect(::JSON.parse(response.body)['error']).not_to be_present
        end
      end
