...
describe GroupsController do
  ...
  describe '#index' do
    let(:staff_group) do
      Fabricate(
        :group,
        name: '0000', visibility_level: Group.visibility_levels[:staff]
      )
    end
      ...
      %w[desc asc].each do |order|
        context "#{order} order" do
          it 'should return the right response' do
            ...
            expect(response_body['groups'].map { |group| group['id'] }).to eq(
                  group_ids
                )
    ...
    it 'should return the right response' do
      ...
      expect(response_body['extras']['type_filters'].map(&:to_sym)).to eq(
            described_class::TYPE_FILTERS.keys - %i[my owner automatic]
          )
    end
    ...
    context 'viewing as an admin' do
      ...
      context 'filterable by type' do
        def expect_type_to_return_right_groups(type, expected_group_ids)
          ...
          expect(response_body['total_rows_groups']).to eq(
                expected_group_ids.count
              )
  ...
  describe '#show' do
    ...
    context 'as an admin' do
      it 'returns the right response' do
        ...
        expect(
          response_body['extras']['visible_group_names']
    ...
    it 'should respond to HTML' do
      ...
      expect(response.body).to have_tag(
            :meta,
            with: { property: 'og:title', content: group.name }
          )

      expect(response.body).to have_tag(
            :meta,
            with: { property: 'og:description', content: group.bio_cooked }
          )
    end
  ...
  describe '#members' do
    ...
    it 'ensures that membership can be paginated' do
      ...
      usernames = group.users.map(&:username).sort
      ...
      get "/groups/#{group.name}/members.json",
          params: { order: 'added_at', desc: true }
      members = JSON.parse(response.body)['members']
  ...
  describe '#update' do
    let(:group) do
      Fabricate(
        :group,
        name: 'test', users: [user], public_admission: false, public_exit: false
      )
    end
    ...
    context 'when user is group owner' do
      ...
      it 'should not be allowed to update automatic groups' do
        ...
        put "/groups/#{group.id}.json",
            params: { group: { messageable_level: 1 } }
    ...
    context 'when user is group admin' do
      ...
      it 'should be able to update the group' do
        ...
        expect(
          Jobs::AutomaticGroupMembership.jobs.first['args'].first['group_id']
        ).to eq(group.id)
      end
  ...
  describe '#members' do
    ...
    let(:user3) do
      Fabricate(
        :user,
        last_seen_at: nil, last_posted_at: nil, email: 'c@test.org'
      )
    end
    ...
    it 'should allow members to be sorted by' do
      get "/groups/#{group.name}/members.json",
          params: { order: 'last_seen_at', desc: true }
      ...
      get "/groups/#{group.name}/members.json",
          params: { order: 'last_posted_at', desc: true }
    ...
    it 'should not allow members to be sorted by columns that are not allowed' do
      ...
      expect(members.map { |m| m['id'] }).to contain_exactly(
            user1.id,
            user2.id,
            user3.id
          )
  ...
  describe '#edit' do
    ...
    context 'when user is not signed in' do
      ...
      context 'public group' do
        it 'should be fobidden' do
          group.update_attributes!(public_admission: true, public_exit: true)
  ...
  describe 'membership edits' do
    ...
    context '#add_members' do
      it 'can make incremental adds' do
        ...
        expect do
          put "/groups/#{group.id}/members.json",
              params: { usernames: user2.username }
      ...
      it 'returns 422 if member already exists' do
        put "/groups/#{group.id}/members.json",
            params: { usernames: user.username }
      ...
      context 'is able to remove a member' do
        ...
        it 'removes by id with integer in json' do
          expect do
            ...
            delete "/groups/#{group.id}/members.json",
                   params: "{\"user_id\":#{user.id}}", headers: headers
          end.to change { group.users.count }.by(-1)
        ...
        context 'is able to remove several members from a group' do
          ...
          it 'removes by username' do
            expect do
              delete "/groups/#{group1.id}/members.json",
                     params: {
                       usernames:
                         [user1.username, user2.username.upcase].join(',')
                     }
          ...
          it 'removes by id with integer in json' do
            expect do
              headers = { :"CONTENT_TYPE" => 'application/json' }
              delete "/groups/#{group1.id}/members.json",
                     params: "{\"user_ids\":#{user1.id}}", headers: headers
  ...
  describe '#histories' do
    ...
    context 'when user is an admin' do
      ...
      it 'should be able to filter through the history' do
        ...
        get "/groups/#{group.name}/logs.json",
            params: { filters: { 'action' => 'add_user_to_group' } }
  ...
  describe '#search ' do
    ...
    before do
      group.update!(name: 'GOT', full_name: 'Daenerys Targaryen')
