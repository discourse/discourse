  ...
  context 'show' do
    it 'caches for 24 hours if version is provided and it matches current hash' do
      get '/extra-locales/admin',
          params: { v: ExtraLocalesController.bundle_js_hash('admin') }
    ...
    it 'does not cache at all if version is invalid' do
      ...
      expect(response.status).to eq(200)
      expect(response.headers['Cache-Control']).not_to eq(
                'max-age=86400, public, immutable'
              )
    end
    ...
    it 'includes plugin translations' do
      ...
      JsLocaleHelper.expects(:plugin_translations).with(I18n.locale.to_s)
        .returns(
        'admin_js' => {
          'admin' => {
            'site_settings' => {
              'categories' => { 'github_badges' => 'Github Badges' }
            }
          }
        }
      )
