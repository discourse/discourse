...
describe InvitesController do
  context 'show' do
    ...
    it 'returns error if invite not found' do
      ...
      body = response.body
      expect(body).to_not have_tag(
                :script,
                with: { src: '/assets/application.js' }
              )
      expect(CGI.unescapeHTML(body)).to include(
            I18n.t(
              'invite.not_found',
    ...
    it 'renders the accept invite page if invite exists' do
      ...
      expect(CGI.unescapeHTML(body)).to_not include(
                I18n.t(
  ...
  context '#destroy' do
    it 'requires you to be logged in' do
      delete '/invites.json', params: { email: 'jake@adventuretime.ooo' }
    ...
    context 'while logged in' do
      ...
      let(:another_invite) do
        Fabricate(:invite, email: 'anotheremail@address.com')
      end
  ...
  context '#create_invite_link' do
    it 'requires you to be logged in' do
      post '/invites/link.json', params: { email: 'jake@adventuretime.ooo' }
      expect(response.status).to eq(403)
    end
    ...
    context 'while logged in' do
      ...
      it 'fails for normal user if invite email already exists' do
        ...
        post '/invites/link.json', params: { email: invite.email }
      ...
      it 'verifies that inviter is authorized to invite new user to a group-private topic' do
        ...
        post '/invites/link.json',
             params: { email: email, topic_id: group_private_topic.id }
      ...
      it 'allows admins to invite to groups' do
        ...
        post '/invites/link.json',
             params: { email: email, group_names: group.name }
      ...
      it 'allows multiple group invite' do
        ...
        post '/invites/link.json',
             params: { email: email, group_names: 'security,support' }
  ...
  context '#perform_accept_invitation' do
          ...
          context 'with password' do
            context 'user was invited via email' do
              ...
              it "doesn't send an activation email and activates the user" do
                expect do
                  put "/invites/show/#{invite.invite_key}.json",
                      params: { password: 'verystrongpassword' }
            ...
            context 'user was invited via link' do
              ...
              it "sends an activation email and doesn't activate the user" do
                ...
                tokens =
                  EmailToken.where(
                    user_id: invited_user.id, confirmed: false, expired: false
                  )
                    .pluck(:token)
    ...
    context 'new registrations are disabled' do
      ...
      it "doesn't redeem the invite" do
        ...
        expect(response.body).to include(
              I18n.t('login.new_registrations_disabled')
            )
      end
    end
    ...
    context 'user is already logged in' do
      ...
      it "doesn't redeem the invite" do
        put "/invites/show/#{invite.invite_key}.json",
            params: { id: invite.invite_key }
        expect(response.status).to eq(200)
        invite.reload
        expect(invite.user_id).to be_blank
        expect(invite.redeemed?).to be_falsey
        expect(response.body).to include(
              I18n.t('login.already_logged_in', current_user: user.username)
            )
      end
    end
  end
  ...
  context '#resend_invite' do
    ...
    context 'while logged in' do
      ...
      let!(:invite) { Fabricate(:invite, invited_by: user) }
      let(:another_invite) do
        Fabricate(:invite, email: 'last_name@example.com')
      end
      ...
      it 'raises an error when the email cannot be found' do
        post '/invites/reinvite.json',
             params: { email: 'first_name@example.com' }
        expect(response.status).to eq(400)
      end
  ...
  context '#upload_csv' do
    ...
    context 'while logged in' do
      let(:csv_file) do
        File.new("#{Rails.root}/spec/fixtures/csv/discourse.csv")
      end
