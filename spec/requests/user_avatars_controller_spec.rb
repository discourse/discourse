  ...
  context 'show_proxy_letter' do
    it 'returns not found if external avatar is set somewhere else' do
      SiteSetting.external_system_avatars_url =
        'https://somewhere.else.com/avatar.png'
    ...
    it 'returns an avatar if we are allowing the proxy' do
      stub_request(
        :get,
        'https://avatars.discourse.org/v3/letter/a/aaaaaa/360.png'
      )
  ...
  context 'show' do
    ...
    it 'handles non local content correctly' do
      ...
      expect(response).to redirect_to(
            "http://awesome.com/boom/user_avatar/default/#{user
              .username_lower}/98/#{upload.id}_#{OptimizedImage::VERSION}.png"
          )
      ...
      expect(response.headers['Last-Modified']).to eq(
            optimized_image.upload.created_at.httpdate
          )
    end
