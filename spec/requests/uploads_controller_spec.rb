...
describe UploadsController do
  describe '#create' do
    ...
    context 'logged in' do
      ...
      it 'is successful with api' do
        ...
        expect(json['short_url']).to eq(
              'upload://qUm0DGR49PAZshIi7HxMd3cAlzn.png'
            )
      end
      ...
      it 'requires a file' do
        ...
        expect(message['errors']).to contain_exactly(
              I18n.t('upload.file_missing')
            )
      end
      ...
      it 'properly returns errors' do
        ...
        expect(errors.first).to eq(
              I18n.t('upload.attachments.too_large', max_size_kb: 1)
            )
      end
      ...
      it 'allows staff to upload any file in PM' do
        ...
        post '/uploads.json',
             params: {
               file: text_file, type: 'composer', for_private_message: 'true'
             }
      ...
      it 'allows staff to upload supported images for site settings' do
        ...
        post '/uploads.json',
             params: {
               file: logo, type: 'site_setting', for_site_setting: 'true'
             }
      ...
      it 'ignores `authorized_extensions_for_staff` setting when non-staff upload file' do
        ...
        data = JSON.parse(response.body)
        expect(data['errors'].first).to eq(
              I18n.t('upload.unauthorized', authorized_extensions: '')
            )
      end
      ...
      it 'returns an error when it could not determine the dimensions of an image' do
        ...
        expect(message).to contain_exactly(
  ...
  describe '#show' do
    ...
    it 'handles image without extension' do
      ...
      expect(response.status).to eq(200)
      expect(response.headers['Content-Disposition']).to eq(
            'attachment; filename=\"image_no_extension.png\"'
          )
    end
    ...
    it 'handles file without extension' do
      ...
      expect(response.status).to eq(200)
      expect(response.headers['Content-Disposition']).to eq(
            'attachment; filename=\"not_an_image\"'
          )
    end
  ...
  describe '#lookup_urls' do
    it 'can look up long urls' do
      ...
      post '/uploads/lookup-urls.json',
           params: { short_urls: [upload.short_url] }
      expect(response.status).to eq(200)
