...
describe PostsController do
  ...
  let(:topicless_post) do
    Fabricate(:post, user: user, raw: '<p>Car 54, where are you?</p>')
  end
  ...
  describe '#by_date' do
    include_examples 'finding and showing post' do
      let(:url) do
        "/posts/by-date/#{post.topic_id}/#{post.created_at.strftime(
          '%Y-%m-%d'
        )}.json"
      end
    end
    ...
    it 'returns the expected post' do
      ...
      second_post =
        Fabricate(:post, topic: first_post.topic, created_at: 4.days.ago)
      third_post =
        Fabricate(:post, topic: first_post.topic, created_at: 3.days.ago)

      get "/posts/by-date/#{second_post.topic_id}/#{(
            second_post.created_at - 2.days
          )
            .strftime('%Y-%m-%d')}.json"
  ...
  describe '#reply_history' do
    ...
    it 'returns the replies with whitelisted user custom fields' do
      ...
      child =
        Fabricate(
          :post,
          topic: parent.topic, reply_to_post_number: parent.post_number
        )
  ...
  describe '#destroy_many' do
    ...
    describe 'when logged in' do
      ...
      it "raises an error when the user doesn't have permission to delete the posts" do
        ...
        delete '/posts/destroy_many.json',
               params: { post_ids: [post1.id, post2.id] }
        expect(response).to be_forbidden
      end
      ...
      describe 'can delete replies' do
        before { PostReply.create(post_id: post1.id, reply_id: post2.id) }
    ...
    describe 'when logged in' do
      ...
      it "raises an error when the user doesn't have permission to see the post" do
        post =
          Fabricate(
            :post,
            topic: Fabricate(:private_message_topic), post_number: 3
          )
        sign_in(user)
  ...
  describe '#update' do
    ...
    let(:update_params) do
      {
        ...
        image_sizes: {
          'http://image.com/image.jpg' => { 'width' => 123, 'height' => 456 }
        }
      }
    end
    ...
    describe 'when logged in as a regular user' do
      ...
      it 'calls revise with valid parameters' do
        PostRevisor.any_instance.expects(:revise!).with(
          post.user,
          { raw: 'edited body', edit_reason: 'typo' },
          anything
        )
        put "/posts/#{post.id}.json", params: update_params
      end
      ...
      it 'extracts links from the new body' do
        param = update_params
        param[:post][:raw] =
          'I just visited this https://google.com so many cool links'
    ...
    it 'can not change category to a disallowed category' do
      ...
      put "/posts/#{post.id}.json",
          params: {
            post: {
              category_id: category.id, raw: 'this is a test edit to post'
            }
          }
  ...
  describe '#bookmark' do
    ...
    describe 'when logged in' do
      ...
      it "raises an error if the user doesn't have permission to see the post" do
        put "/posts/#{private_message.id}/bookmark.json",
      ...
      it 'creates a bookmark' do
        ...
        expect(post_action.post_action_type_id).to eq(
    ...
    context 'api' do
      ...
      it 'allows users with api key to bookmark posts' do
        put "/posts/#{post.id}/bookmark.json",
            params: { bookmarked: 'true', api_key: api_key.key }
      ...
      it 'raises an error with a user key that does not match an optionally specified username' do
        put "/posts/#{post.id}/bookmark.json",
            params: {
              bookmarked: 'true', api_key: api_key.key, api_username: 'made_up'
            }
      ...
      it 'disallows blank api' do
        put "/posts/#{post.id}/bookmark.json",
            params: {
              bookmarked: 'true', api_key: '', api_username: user.username
            }
  ...
  describe '#rebake' do
    ...
    describe 'when logged in' do
      ...
      it 'will invalidate broken images cache' do
        sign_in(Fabricate(:moderator))
        post.custom_fields[Post::BROKEN_IMAGES] = %w[
  ...
  describe '#create' do
    ...
    describe 'when logged in' do
      ...
      it 'can send a message to a group' do
        group =
          Group.create(
            name: 'test_group', messageable_level: Group::ALIAS_LEVELS[:nobody]
          )
      ...
      it 'can create a reply to a post' do
        ...
        expect(job_args['image_sizes']).to eq(
      ...
      describe 'as a staff user' do
        ...
        context 'with a shared category' do
          ...
          it 'will work if the shared draft category is present' do
            ...
            expect(topic.shared_draft.category_id).to eq(
    ...
    context 'topic bump' do
      shared_examples 'it works' do
        ...
        it 'should be able to skip topic bumping' do
          ...
          expect(response.status).to eq(200)
          expect(topic.reload.bumped_at).to be_within_one_second_of(
                original_bumped_at
              )
        end
        ...
        it 'should be able to post with topic bumping' do
          post '/posts.json',
               params: { raw: 'this is the test content', topic_id: topic.id }
  ...
  describe '#revisions' do
    ...
    context 'when edit history is not visible to the public' do
      ...
      it 'ensures trust level 4 can see the revisions' do
        ...
        get "/posts/#{post_revision.post_id}/revisions/#{post_revision
              .number}.json"
        expect(response.status).to eq(200)
      end
    end
    ...
    context 'deleted post' do
      ...
      let(:deleted_post_revision) do
        Fabricate(:post_revision, user: admin, post: deleted_post)
      end
      ...
      it 'also work on deleted post' do
        sign_in(admin)
        get "/posts/#{deleted_post_revision
              .post_id}/revisions/#{deleted_post_revision.number}.json"
        expect(response.status).to eq(200)
      end
    end
    ...
    context 'deleted topic' do
      ...
      let(:post) do
        Fabricate(:post, user: admin, topic: deleted_topic, version: 3)
      end
  ...
  describe '#revert' do
    include_examples 'action requires login',
                     :put,
                     '/posts/123/revisions/2/revert.json'
    ...
    let(:blank_post_revision) do
      Fabricate(
        :post_revision,
        post: post,
        modifications: { 'edit_reason' => ['edit reason #1', 'edit reason #2'] }
      )
    end
    ...
    describe 'when logged in as staff' do
      ...
      it 'fails when revision is blank' do
        put "/posts/#{post_id}/revisions/#{blank_post_revision
              .number}/revert.json"
        expect(response.status).to eq(422)
        expect(JSON.parse(response.body)['errors']).to include(
              I18n.t('revert_version_same')
            )
      end

      it 'fails when revised version is same as current version' do
        put "/posts/#{post_id}/revisions/#{same_post_revision
              .number}/revert.json"
        expect(response.status).to eq(422)
        expect(JSON.parse(response.body)['errors']).to include(
              I18n.t('revert_version_same')
