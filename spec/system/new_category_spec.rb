...
describe "New Category", type: :system do
  ...
  it "should create category with 0 in minimum_required_tags column when not defined" do
    ...
    category_page.find(".edit-category-tab-general input.category-name").fill_in(with: "Sales")
    find(".form-kit button[type=submit]").click

    expect(page).to have_current_path("/c/sales/edit/general")
