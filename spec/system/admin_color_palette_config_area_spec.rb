...
describe "Admin Color Palette Config Area Page" do
  ...
  after do
    ...
    # cache warm across examples. This file mutates the shared
    # `color_scheme` between examples — e.g. one example marks it as
    # the default theme palette (which causes the browser to fetch and
    # cache that scheme's stylesheet body) and a later example edits
    # the same scheme's secondary colour and asserts the rendered
    # `html { background-color }` reflects the edit. The browser
    # otherwise serves the pre-edit stylesheet body from its
    # response cache. Wipe that cache here.
