...
describe "Topic bulk select", type: :system do
  ...
  context "when changing category with restricted tags" do
    ...
    it "allows staff to move topics even when tags are restricted" do
      sign_in(admin)
      visit("/latest")
      ...
      expect(toasts).to have_success("completed successfully")
      expect(topics.first.reload.category).to eq(destination_category)
    end

    it "shows a warning for non-staff when topics cannot be moved due to tag restrictions" do
