...
shared_examples "login scenarios" do |login_page_object|
  ...
  context "when login is not required" do
    ...
    TXT
      ...
      it "completes signup and redirects to the user api key permissions page" do
        mock_facebook_auth
        visit("/user-api-key/new?#{args.to_query}")

        # once logged in, it should redirect back to `/user-api-key/new` with the authorization form
...
describe "Login", type: :system do
  # DO NOT COMMIT: commenting these out only to isolate the test above to 1 run
  #
