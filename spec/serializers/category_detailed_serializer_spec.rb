...
describe CategoryDetailedSerializer do
  describe 'counts' do
    it 'works for categories with no subcategories' do
      no_subcats =
        Fabricate(:category, topics_year: 10, topics_month: 5, topics_day: 2)
      json =
        CategoryDetailedSerializer.new(
          no_subcats,
          scope: Guardian.new, root: false
        )
          .as_json
    ...
    it 'includes counts from subcategories' do
      ...
      json =
        CategoryDetailedSerializer.new(parent, scope: Guardian.new, root: false)
          .as_json
