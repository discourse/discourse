...
describe PostSerializer do
  context 'a post with lots of actions' do
    ...
    def visible_actions_for(user)
      serializer =
        PostSerializer.new(post, scope: Guardian.new(user), root: false)
      # NOTE this is messy, we should extract all this logic elsewhere
      if user.try(:id) == actor.id
        serializer.post_actions = PostAction.counts_for([post], actor)[post.id]
      end
    ...
    before do
      acted_ids.each { |id| PostAction.act(actor, post, id) }
      post.reload
    end

    it 'displays the correct info' do
      expect(visible_actions_for(actor).sort).to eq(%i[like notify_user spam])
      expect(visible_actions_for(post.user).sort).to eq(%i[like])
      expect(visible_actions_for(nil).sort).to eq(%i[like])
      expect(visible_actions_for(admin).sort).to eq(%i[like notify_user spam])
    end

    it "can't flag your own post to notify yourself" do
      serializer =
        PostSerializer.new(post, scope: Guardian.new(post.user), root: false)
      notify_user_action =
        serializer.actions_summary.find do |a|
  ...
  context 'a post by a nuked user' do
    let!(:post) do
      Fabricate(:post, user: Fabricate(:user), deleted_at: Time.zone.now)
    end
    ...
    it 'serializes correctly' do
      %i[name username display_username avatar_template user_title trust_level]
        .each { |attr| expect(subject[attr]).to be_nil }
      %i[moderator staff yours].each do |attr|
  ...
  context 'display_username' do
    ...
    let(:serializer) do
      PostSerializer.new(post, scope: Guardian.new, root: false)
    end
  ...
  context 'a hidden post with add_raw enabled' do
    ...
    context 'a public post' do
      ...
      it 'includes the raw post for everyone' do
        [nil, user, Fabricate(:user), Fabricate(:moderator), Fabricate(:admin)]
          .each do |user|
    ...
    context 'a hidden post' do
      ...
      it 'can view edit history only if authorized' do
        expect(serialized_post_for_user(nil)[:can_view_edit_history]).to eq(
              false
            )
        expect(
          serialized_post_for_user(Fabricate(:user))[:can_view_edit_history]
        ).to eq(false)

        expect(serialized_post_for_user(user)[:can_view_edit_history]).to eq(
              true
            )
        expect(
          serialized_post_for_user(Fabricate(:moderator))[
            :can_view_edit_history
          ]
        ).to eq(true)
        expect(
          serialized_post_for_user(Fabricate(:admin))[:can_view_edit_history]
        ).to eq(true)
      end
    end
