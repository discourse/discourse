...
RSpec.describe ThemeSerializer do
  describe ".settings" do
    ...
    it "adds an error for an invalid settings format" do
      Theme
        .any_instance
        .stubs(:settings)
    ...
    it "adds error messages from theme fields" do
  ...
  describe "color schemes" do
    ...
    it "includes the color scheme ID" do
      expect(serialized[:color_scheme_id]).to eq(1)
    end

    it "includes the dark color scheme ID" do
  ...
  describe "screenshot_light_url" do
    ...
    it "includes screenshot_light_url for the light screenshot field" do
      Fabricate(
        :theme_field,
        theme: theme,
    ...
    it "omits screenshot URLs when no screenshot fields exist" do
      expect(serialized[:screenshot_light_url]).to be_nil
      expect(serialized[:screenshot_dark_url]).to be_nil
    end

    it "includes both dark and light screenshot URLs" do
