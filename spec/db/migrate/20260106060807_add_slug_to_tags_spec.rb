...
describe AddSlugToTags do
  ...
  it "strips special characters from names" do
    reset_slug_column
    tag_id =
      DB.query_single(
        "INSERT INTO tags (name, created_at, updated_at) VALUES ('hello@world!', NOW(), NOW()) RETURNING id",
    ...
    slug = DB.query_single("SELECT slug FROM tags WHERE id = ?", tag_id)[0]
    expect(slug).to eq("helloworld")
  end

  it "handles unicode names by stripping non-ASCII" do
    reset_slug_column
    tag_id =
      DB.query_single(
  ...
  it "resolves conflicts by appending -tag to newer tag" do
    reset_slug_column
    tag1_id =
      DB.query_single(
