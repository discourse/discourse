...
describe SpamRule::FlagSockpuppets do
  ...
  let(:post1) do
    Fabricate(:post, user: user1, topic: Fabricate(:topic, user: user1))
  end
  ...
  describe 'reply_is_from_sockpuppet?' do
    ...
    it 'is false if users have different IP addresses' do
      post2 =
        Fabricate(
          :post,
          user: Fabricate(:user, ip_address: '182.189.199.199'),
          topic: post1.topic
        )
    ...
    it 'is true if users have the same IP address and are new' do
      post2 =
        Fabricate(
          :post,
          user: Fabricate(:user, ip_address: user1.ip_address),
          topic: post1.topic
        )
    ...
    it 'is false if the ip address is whitelisted' do
      ScreenedIpAddress.stubs(:is_whitelisted?).with(user1.ip_address).returns(
        true
      )
      post2 =
        Fabricate(
    ...
    it 'is false if first post user is staff' do
      ...
      staff_post1 =
        Fabricate(:post, user: staff1, topic: Fabricate(:topic, user: staff1))
      post2 =
        Fabricate(
          :post,
    ...
    it 'is true if first post user was created over 24 hours ago and has trust level higher than 0' do
      ...
      first_post =
        Fabricate(
          :post,
          user: old_user, topic: Fabricate(:topic, user: old_user)
        )
      post2 =
        Fabricate(
          :post,
    ...
    it 'is true if first post user was created less that 24 hours ago and has trust level higher than 0' do
      ...
      first_post =
        Fabricate(
          :post,
          user: new_user, topic: Fabricate(:topic, user: new_user)
        )
      post2 =
        Fabricate(
          :post,
  ...
  describe 'flag_sockpuppet_users' do
    let(:post2) do
      Fabricate(
        :post,
        user: Fabricate(:user, ip_address: user1.ip_address), topic: post1.topic
      )
    end
    ...
    it 'flags post and first post if both users are new' do
      ...
      expect(
        PostAction.where(user: system, post: post1, post_action_type_id: spam)
          .exists?
      ).to eq(true)
      expect(
        PostAction.where(user: system, post: post2, post_action_type_id: spam)
          .exists?
      ).to eq(true)
    end
    ...
    it "doesn't flag the first post more than once" do
      ...
      expect(
        PostAction.where(post: post2, post_action_type_id: spam).count
      ).to eq(1)
    end
    ...
    it "doesn't flag the first post if the user is not new" do
      ...
      expect(
        PostAction.where(
          user: system, post: first_post, post_action_type_id: spam
        )
          .exists?
      ).to eq(false)
    end
