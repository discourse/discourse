...
RSpec.describe GroupActionLogger do
  ...
  describe '#log_make_user_group_owner' do
    it 'should create the right record' do
      ...
      expect(group_history.action).to eq(
            GroupHistory.actions[:make_user_group_owner]
          )
  ...
  describe '#log_remove_user_as_group_owner' do
    it 'should create the right record' do
      ...
      expect(group_history.action).to eq(
            GroupHistory.actions[:remove_user_as_group_owner]
          )
      expect(group_history.acting_user).to eq(group_owner)
      expect(group_history.target_user).to eq(user)
    end
    ...
    context 'as a normal user' do
      ...
      describe 'user cannot freely exit group' do
        it 'should not be allowed to log the action' do
          expect { subject.log_add_user_to_group(user) }.to raise_error(
  ...
  describe '#log_remove_user_from_group' do
    describe 'as group owner' do
      it 'should create the right record' do
        ...
        expect(group_history.action).to eq(
              GroupHistory.actions[:remove_user_from_group]
            )
        expect(group_history.acting_user).to eq(group_owner)
        expect(group_history.target_user).to eq(user)
      end
    ...
    context 'as a normal user' do
      ...
      describe 'user cannot freely exit group' do
        it 'should not be allowed to log the action' do
          expect { subject.log_remove_user_from_group(user) }.to raise_error(
                Discourse::InvalidParameters
              )
        end
      end
  ...
  describe '#log_change_group_settings' do
    it 'should create the right record' do
      group.update_attributes!(
        public_admission: true, created_at: Time.zone.now
      )

      expect { subject.log_change_group_settings }.to change do
        GroupHistory.count
      end
        .by(1)
      ...
      expect(group_history.action).to eq(
            GroupHistory.actions[:change_group_setting]
          )
      expect(group_history.acting_user).to eq(group_owner)
