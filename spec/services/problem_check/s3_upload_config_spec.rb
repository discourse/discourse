    ...
    context "when S3 uploads are locally enabled" do
      ...
      context "when not configured to use IAM profile" do
        ...
        context "when credentials are missing" do
          ...
          it do
            expect(check).to have_a_problem.with_priority("low").with_message(
              '{{setting:enable_s3_uploads}} is on, but {{setting:s3_secret_access_key}} must still be set. <a href="https://meta.discourse.org/t/how-to-set-up-image-uploads-to-s3/7229" target="_blank">See "How to set up image uploads to S3?" to learn more</a>.',
            )
          end
        end
        ...
        context "when bucket name is missing" do
          ...
          it do
            expect(check).to have_a_problem.with_priority("low").with_message(
              '{{setting:enable_s3_uploads}} is on, but {{setting:s3_upload_bucket}} must still be set. <a href="https://meta.discourse.org/t/how-to-set-up-image-uploads-to-s3/7229" target="_blank">See "How to set up image uploads to S3?" to learn more</a>.',
            )
          end
        end
      end
