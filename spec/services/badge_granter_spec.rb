...
describe BadgeGranter do
  ...
  describe 'backfill' do
    ...
    it 'should grant badges in the user locale' do
      ...
      notification_badge_name =
        JSON.parse(post.user.notifications.first.data)['badge_name']
  ...
  describe 'grant' do
    ...
    it "doesn't grant disabled badges" do
      badge =
        Fabricate(:badge, badge_type_id: BadgeType::Bronze, enabled: false)
      time = 1.year.ago
  ...
  context 'update_badges' do
    ...
    before { BadgeGranter.clear_queue! }
    ...
    it 'grants autobiographer' do
      ...
      expect(
        UserBadge.where(user_id: user.id, badge_id: Badge::Autobiographer).count
      ).to eq(1)
    end
    ...
    it 'grants read guidlines' do
      ...
      BadgeGranter.process_queue!
      expect(
        UserBadge.where(user_id: user.id, badge_id: Badge::ReadGuidelines).count
      ).to eq(1)
    end
    ...
    it 'grants first link' do
      ...
      BadgeGranter.process_queue!
      expect(
        UserBadge.where(user_id: post2.user.id, badge_id: Badge::FirstLink)
          .count
      ).to eq(1)
    end
    ...
    it 'grants first edit' do
      ...
      expect(
        UserBadge.where(user_id: user.id, badge_id: Badge::Editor).count
      ).to eq(0)
    ...
    it 'grants and revokes trust level badges' do
      ...
      BadgeGranter.process_queue!
      expect(
        UserBadge.where(user_id: user.id, badge_id: Badge.trust_level_badge_ids)
          .count
      ).to eq(4)
      ...
      expect(UserBadge.where(user_id: user.id, badge_id: 1).first).not_to eq(
                nil
              )
    ...
    it 'grants system like badges' do
      ...
      expect(
        UserBadge.find_by(user_id: user.id, badge_id: Badge::NicePost)
      ).not_to eq(nil)
      expect(
        UserBadge.where(user_id: user.id, badge_id: Badge::NicePost).count
      ).to eq(1)
      ...
      BadgeGranter.process_queue!
      expect(
        UserBadge.find_by(user_id: user.id, badge_id: Badge::GoodPost)
      ).not_to eq(nil)
      ...
      BadgeGranter.process_queue!
      expect(
        UserBadge.find_by(user_id: user.id, badge_id: Badge::GreatPost)
      ).not_to eq(nil)
