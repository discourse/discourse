...
describe SiteSettingsTask do
  ...
  describe 'export' do
    it 'creates a hash of all site settings' do
      ...
      expect(hash).to eq('enable_sso' => 'true', 'sso_url' => sso_url)
    end
  end
  ...
  describe 'import' do
    ...
    it "won't update a setting that doesn't exist" do
      ...
      expect(
        log[0]
      ).to eq 'NOT FOUND: existing site setting not found for fake_setting'
    ...
    it 'will log that an error has occured' do
      ...
      expect(
        log[0]
      ).to eq 'ERROR: min_password_length: Value must be between 8 and 500.'
