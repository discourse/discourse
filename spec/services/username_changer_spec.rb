  ...
  describe '#change' do
    ...
    describe 'allow custom minimum username length from site settings' do
      ...
      it 'should not allow a longer username than limit' do
        result =
          UsernameChanger.change(user, 'a' * (User.username_length.end + 1))
        expect(result).to eq(false)
      end
    end
    ...
    context 'posts and revisions' do
      ...
      context 'mentions' do
        it 'rewrites cooked correctly' do
          ...
          expect(post.cooked).to eq(
                '<p>Hello <a class="mention" href="/u/bar">@bar</a></p>'
              )
        ...
        it 'ignores case when replacing mentions' do
          post =
            create_post_and_change_username(
              raw: "There's no difference between @foo and @Foo"
            )

          expect(post.raw).to eq("There's no difference between @bar and @bar")
          expect(post.cooked).to eq(
                '<p>There’s no difference between <a class="mention" href="/u/bar">@bar</a> and <a class="mention" href="/u/bar">@bar</a></p>'
              )
        end
        ...
        it 'replaces mentions when there are leading symbols' do
          post =
            create_post_and_change_username(
              raw: '.@foo -@foo %@foo _@foo ,@foo ;@foo @@foo'
            )
        ...
        it 'replaces mentions within double and single quotes' do
          post = create_post_and_change_username(raw: "\"@foo\" '@foo'")

          expect(post.raw).to eq("\"@bar\" '@bar'")
          expect(post.cooked).to eq(
                '<p>“<a class="mention" href="/u/bar">@bar</a>” ‘<a class="mention" href="/u/bar">@bar</a>’</p>'
              )
        end
        ...
        it 'does not replace mention when followed by an underscore' do
          ...
          expect(post.cooked).to eq('<p><span class="mention">@foo_</span></p>')
        end
        ...
        it 'does not replace mentions when there are leading alphanumeric chars' do
          ...
          expect(post.cooked).to eq('<p>a@foo 2@foo</p>')
        end
        ...
        it 'does not replace username within email address' do
          ...
          expect(post.cooked).to eq(
                '<p><a href="mailto:mail@foo.com">mail@foo.com</a></p>'
              )
        end
        ...
        it 'does not replace username in a mention of a similar username' do
          ...
          post =
            create_post_and_change_username(
              raw: '@foo @foobar @foo-bar @foo_bar @foo1'
            )
        ...
        it 'replaces mentions within revisions' do
          ...
          post =
            create_post_and_change_username(
              raw: 'Hello @foo', revisions: revisions
            )
          ...
          expect(post.revisions[0].modifications['cooked'][1]).to eq(
                '<p>Hello Foo</p>'
              )
          ...
          expect(post.revisions[2].modifications['cooked'][0]).to eq(
                '<p>Hello Foo</p>'
              )
          ...
          expect(post.revisions[3].modifications['raw'][1]).to eq(
                'Hello @bar!!'
              )
        ...
        it 'works when users are mentioned with HTML' do
          post =
            create_post_and_change_username(
              raw:
                '<a class="mention">@foo</a> and <a class="mention">@someuser</a>'
            )

          expect(post.raw).to eq(
                '<a class="mention">@bar</a> and <a class="mention">@someuser</a>'
              )
          expect(post.cooked).to match_html(
      ...
      context 'quotes' do
        let(:quoted_post) do
          create_post(
            user: user, topic: topic, post_number: 1, raw: 'quoted post'
          )
        end
        ...
        it 'replaces the username in quote tags and updates avatar' do
          post =
            ...
            [quote="foo, post:1, topic:#{quoted_post
                .topic
                .id}"]
            quoted post
            [/quote]
          ...
          expect(post.raw).to eq(
            ...
            [quote="bar, post:1, topic:#{quoted_post
                  .topic
                  .id}"]
            quoted post
            [/quote]
          ...
          expect(post.cooked).to match_html(
                <<~HTML
            <p>Lorem ipsum</p>
            <aside class="quote no-group" data-post="1" data-topic="#{quoted_post
                  .topic
                  .id}">
            <div class="title">
      ...
      context 'oneboxes' do
        ...
        let(:another_quoted_post) do
          create_post(
            user: evil_trout, topic: topic, post_number: 2, raw: 'evil post'
          )
        end
        ...
        it 'updates avatar for linked topics and posts' do
          ...
          expect(post.cooked).to match_html(
                <<~HTML
            <p><aside class="quote" data-post="#{quoted_post
                  .post_number}" data-topic="#{quoted_post.topic
                  .id}">
              <div class="title">
                <div class="quote-controls"></div>
                <img alt="" width="20" height="20" src="#{avatar_url}" class="avatar">
                <a href="#{protocol_relative_url(
                  quoted_post.full_url
                )}">#{quoted_post.topic
                  .title}</a>
              </div>
              <blockquote>
            ...
            <br>
            <aside class="quote" data-post="#{quoted_post
                  .post_number}" data-topic="#{quoted_post.topic
                  .id}">
              <div class="title">
                <div class="quote-controls"></div>
        ...
        it 'does not update the wrong avatar' do
          ...
          expect(post.cooked).to match_html(
            ...
            <br>
            <aside class="quote" data-post="#{another_quoted_post
                  .post_number}" data-topic="#{another_quoted_post.topic
                  .id}">
              <div class="title">
                <div class="quote-controls"></div>
                <img alt="" width="20" height="20" src="#{user_avatar_url(
                  evil_trout
                )}" class="avatar">
                <a href="#{protocol_relative_url(
                  another_quoted_post.full_url
                )}">#{another_quoted_post.topic
                  .title}</a>
              </div>
              <blockquote>
                evil post
      ...
      it 'replaces usernames in notifications' do
        ...
        n01 =
          create_notification(
            ...
            notified_user,
            p1,
            original_and_display_username('alice')
          )
        n02 =
          create_notification(
            :mentioned,
        ...
        expect(notification_data(n01)).to eq(
              original_and_display_username('bob')
            )
        expect(notification_data(n02)).to eq(
              original_and_display_username('another_user')
            )
