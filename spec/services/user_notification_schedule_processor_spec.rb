  ...
  describe "#create_do_not_disturb_timings" do
    [
      ...
      it 'creates dnd timings correctly for each timezone' do
        ...
        travel_to Time.new(2020, 1, 4, 12, 0, 0, "UTC") do
    ...
    it 'extends previously scheduled dnd timings to remove gaps' do
      ...
      travel_to Time.new(2020, 1, 4, 12, 0, 0, "UTC") do
        existing_timing = user.do_not_disturb_timings.create(
          scheduled: true,
          starts_at: 1.day.ago,
          ends_at: Time.new(2020, 1, 03, 11, 0, 0, "UTC").end_of_day
        )
        UserNotificationScheduleProcessor.create_do_not_disturb_timings_for(standard_schedule)
        expect(existing_timing.reload.ends_at).to eq_time(Time.new(2020, 1, 4, 7, 59, 0, "UTC"))
      end
    end
    ...
    it 'creates the correct timings when the whole schedule is DND (-1)' do
      ...
      travel_to Time.new(2020, 1, 4, 12, 0, 0, "UTC") do
        ...
        expect(user.do_not_disturb_timings.first.starts_at).to eq_time(Time.new(2020, 1, 4, 0, 0, 0, "UTC"))
        expect(user.do_not_disturb_timings.first.ends_at).to be_within(1.second).of Time.new(2020, 1, 6, 23, 59, 59, "UTC")
      end
    end
    ...
    it 'creates the correct timings at the end of a month and year' do
      ...
      schedule.update(
        enabled: true,
        day_4_start_time: -1, # December 31, 2020 was a thursday. testing more cases.
      )

      travel_to Time.new(2020, 12, 31, 12, 0, 0, "UTC") do
        UserNotificationScheduleProcessor.create_do_not_disturb_timings_for(schedule)
        expect(user.do_not_disturb_timings[0].starts_at).to eq_time(Time.new(2020, 12, 31, 0, 0, 0, "UTC"))
        expect(user.do_not_disturb_timings[0].ends_at).to eq_time(Time.new(2021, 1, 1, 7, 59, 0, "UTC"))
        expect(user.do_not_disturb_timings[1].starts_at).to eq_time(Time.new(2021, 1, 1, 17, 0, 0, "UTC"))
        expect(user.do_not_disturb_timings[1].ends_at).to eq_time(Time.new(2021, 1, 2, 7, 59, 0, "UTC"))
        expect(user.do_not_disturb_timings[2].starts_at).to eq_time(Time.new(2021, 1, 2, 17, 0, 0, "UTC"))
        expect(user.do_not_disturb_timings[2].ends_at).to be_within(1.second).of Time.new(2021, 1, 2, 23, 59, 59, "UTC")
      end
    end
  end
end
