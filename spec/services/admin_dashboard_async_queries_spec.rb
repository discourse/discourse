...
RSpec.describe AdminDashboardSearch, type: :request do
  ...
  describe ".build" do
    ...
    it "records ordinary synchronous AR and MiniSql profiler SQL and snapshot metadata without duplicates" do
      sql =
      ...
      page_struct.root.add_sql("SELECT 1 /* snapshot-sync-ar */", 1.0, page_struct)
      page_struct.root.add_sql("SELECT 1 /* snapshot-minisql */", 1.0, page_struct)
      storage = Rack::MiniProfiler::MemoryStore.new
      storage.push_snapshot(page_struct, "ordinary", Rack::MiniProfiler.config)
      snapshot = storage.fetch_snapshots_group("ordinary").first
      snapshot_sql = snapshot.root.sql_timings.map { |timing| timing[:formatted_command_string] }

      expect(tagged_count(sql, "ordinary-sync-ar")).to eq(1)
      expect(tagged_count(sql, "ordinary-minisql")).to eq(1)
      expect(snapshot[:request_path]).to eq("/snapshot")
      expect(tagged_count(snapshot_sql, "snapshot-sync-ar")).to eq(1)
      expect(tagged_count(snapshot_sql, "snapshot-minisql")).to eq(1)
    end
