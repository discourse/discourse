...
describe StaffActionLogger do
  ...
  describe 'log_user_deletion' do
    ...
    subject(:log_user_deletion) do
      described_class.new(admin).log_user_deletion(deleted_user)
    end

    it 'raises an error when user is nil' do
      expect { logger.log_user_deletion(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end

    it 'raises an error when user is not a User' do
      expect { logger.log_user_deletion(1) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
  ...
  describe 'log_show_emails' do
    it 'logs the user history' do
      expect { logger.log_show_emails([admin]) }.to change(UserHistory, :count)
            .by(1)
    end
  ...
  describe 'log_post_deletion' do
    ...
    subject(:log_post_deletion) do
      described_class.new(admin).log_post_deletion(deleted_post)
    end

    it 'raises an error when post is nil' do
      expect { logger.log_post_deletion(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end

    it 'raises an error when post is not a Post' do
      expect { logger.log_post_deletion(1) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
  ...
  describe 'log_topic_delete_recover' do
    ...
    context 'when deleting topic' do
      ...
      it 'raises an error when topic is nil' do
        expect { logger.log_topic_delete_recover(nil) }.to raise_error(
              Discourse::InvalidParameters
            )
      end

      it 'raises an error when topic is not a Topic' do
        expect { logger.log_topic_delete_recover(1) }.to raise_error(
              Discourse::InvalidParameters
            )
      end

      it 'creates a new UserHistory record' do
        expect { log_topic_delete_recover }.to change { UserHistory.count }.by(
              1
            )
      end
    end
    ...
    context 'when recovering topic' do
      subject(:log_topic_delete_recover) do
        described_class.new(admin).log_topic_delete_recover(
          topic,
          'recover_topic'
        )
      end
      ...
      it 'raises an error when topic is nil' do
        expect do
          logger.log_topic_delete_recover(nil, 'recover_topic')
        end.to raise_error(Discourse::InvalidParameters)
      end

      it 'raises an error when topic is not a Topic' do
        expect do
          logger.log_topic_delete_recover(1, 'recover_topic')
        end.to raise_error(Discourse::InvalidParameters)
      end
  ...
  describe 'log_trust_level_change' do
    ...
    subject(:log_trust_level_change) do
      described_class.new(admin).log_trust_level_change(
        user,
        old_trust_level,
        new_trust_level
      )
    ...
    it 'raises an error when user or trust level is nil' do
      ...
      expect do
        logger.log_trust_level_change(user, nil, new_trust_level)
      end.to raise_error(Discourse::InvalidParameters)
      expect do
        logger.log_trust_level_change(user, old_trust_level, nil)
      end.to raise_error(Discourse::InvalidParameters)
    end

    it 'raises an error when user is not a User' do
      expect do
        logger.log_trust_level_change(1, old_trust_level, new_trust_level)
      end.to raise_error(Discourse::InvalidParameters)
    end
    ...
    it 'raises an error when new trust level is not a Trust Level' do
      ...
      expect do
        logger.log_trust_level_change(user, old_trust_level, max_level + 1)
      end.to raise_error(Discourse::InvalidParameters)
    end
    ...
    it 'creates a new UserHistory record' do
      ...
      expect(
        UserHistory.last.details
      ).to include "new trust level: #{new_trust_level}"
    end
  end
  ...
  describe 'log_site_setting_change' do
    it 'raises an error when params are invalid' do
      ...
      expect do
        logger.log_site_setting_change(
          'i_am_a_site_setting_that_will_never_exist',
          '1',
          '2'
        )
      end.to raise_error(Discourse::InvalidParameters)
    ...
    it 'creates a new UserHistory record' do
      expect do
        logger.log_site_setting_change('title', 'Discourse', 'My Site')
      end.to change { UserHistory.count }.by(1)
    end
  end
  ...
  describe 'log_theme_change' do
    it 'raises an error when params are invalid' do
      expect { logger.log_theme_change(nil, nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
    ...
    it 'logs updated site customizations' do
      ...
      theme.set_field(
        target: :common, name: :scss, value: 'body{margin: 10px;}'
      )
  ...
  describe 'log_theme_destroy' do
    it 'raises an error when params are invalid' do
      expect { logger.log_theme_destroy(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
  ...
  describe 'log_site_text_change' do
    it 'raises an error when params are invalid' do
      expect do
        logger.log_site_text_change(nil, 'new text', 'old text')
      end.to raise_error(Discourse::InvalidParameters)
    end

    it 'creates a new UserHistory record' do
      expect do
        logger.log_site_text_change('created', 'new text', 'old text')
      end.to change { UserHistory.count }.by(1)
    end
  end
  ...
  describe 'log_user_suspend' do
    let(:user) do
      Fabricate(
        :user,
        suspended_at: 10.minutes.ago, suspended_till: 1.day.from_now
      )
    end

    it 'raises an error when arguments are missing' do
      expect { logger.log_user_suspend(nil, nil) }.to raise_error(
            Discourse::InvalidParameters
          )
  ...
  describe 'log_user_unsuspend' do
    let(:user) do
      Fabricate(:user, suspended_at: 1.day.ago, suspended_till: 7.days.from_now)
    end

    it 'raises an error when argument is missing' do
      expect { logger.log_user_unsuspend(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
  ...
  describe 'log_badge_grant' do
    ...
    it 'raises an error when argument is missing' do
      expect { logger.log_badge_grant(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
  ...
  describe 'log_badge_revoke' do
    ...
    it 'raises an error when argument is missing' do
      expect { logger.log_badge_revoke(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
  ...
  describe 'log_roll_up' do
    let(:subnets) { %w[1.2.3.0/24 42.42.42.0/24] }
  ...
  describe 'log_custom' do
    it 'raises an error when `custom_type` is missing' do
      expect { logger.log_custom(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
    ...
    it 'creates the UserHistory record' do
      logged =
        logger.log_custom(
          'clicked_something',
          evil: 'trout', clicked_on: 'thing', topic_id: 1234
        )
  ...
  describe 'log_category_settings_change' do
    ...
    let(:category_group) do
      Fabricate(:category_group, category: category, permission_type: 1)
    end
    ...
    it 'creates new UserHistory records' do
      attributes = {
        name: 'new_name', permissions: { category_group.group_name => 2 }
      }
      ...
      expect(permission_user_history.new_value).to eq(
            { category_group.group_name => 2 }.to_json
          )
      expect(permission_user_history.action).to eq(
            UserHistory.actions[:change_category_settings]
          )
  ...
  describe 'log_category_deletion' do
    ...
    it 'raises an error when category is missing' do
      expect { logger.log_category_deletion(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
    ...
    it 'creates a new UserHistory record' do
      ...
      expect(user_history.details).to include(
  ...
  describe 'log_lock_trust_level' do
    ...
    it 'raises an error when argument is missing' do
      expect { logger.log_lock_trust_level(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
    ...
    it 'creates a new UserHistory record' do
      ...
      expect { logger.log_lock_trust_level(user) }.to change do
        UserHistory.count
      end
        .by(1)
      user_history = UserHistory.last
      ...
      expect(user_history.action).to eq(
            UserHistory.actions[:unlock_trust_level]
          )
    end
  end
  ...
  describe 'log_user_activate' do
    ...
    it 'raises an error when argument is missing' do
      expect { logger.log_user_activate(nil, nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
    ...
    it 'creates a new UserHistory record' do
      ...
      expect { logger.log_user_activate(user, reason) }.to change do
        UserHistory.count
      end
        .by(1)
      user_history = UserHistory.last
  ...
  describe '#log_readonly_mode' do
    it 'creates a new record' do
      expect { logger.log_change_readonly_mode(true) }.to change do
        UserHistory.count
      end
        .by(1)
      ...
      expect(user_history.action).to eq(
            UserHistory.actions[:change_readonly_mode]
          )
      expect(user_history.new_value).to eq('t')
      expect(user_history.previous_value).to eq('f')

      expect { logger.log_change_readonly_mode(false) }.to change do
        UserHistory.count
      end
        .by(1)
  ...
  describe 'log_check_personal_message' do
    ...
    it 'raises an error when topic is nil' do
      expect { logger.log_check_personal_message(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end

    it 'raises an error when topic is not a Topic' do
      expect { logger.log_check_personal_message(1) }.to raise_error(
            Discourse::InvalidParameters
          )
    end

    it 'creates a new UserHistory record' do
      expect { log_check_personal_message }.to change { UserHistory.count }.by(
            1
          )
    end
  end
  ...
  describe 'log_post_approved' do
    ...
    subject(:log_post_approved) do
      described_class.new(admin).log_post_approved(approved_post)
    end

    it 'raises an error when post is nil' do
      expect { logger.log_post_approved(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end

    it 'raises an error when post is not a Post' do
      expect { logger.log_post_approved(1) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
  ...
  describe 'log_post_rejected' do
    ...
    subject(:log_post_rejected) do
      described_class.new(admin).log_post_rejected(rejected_post)
    end

    it 'raises an error when post is nil' do
      expect { logger.log_post_rejected(nil) }.to raise_error(
            Discourse::InvalidParameters
          )
    end

    it 'raises an error when post is not a QueuedPosts' do
      expect { logger.log_post_rejected(1) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
