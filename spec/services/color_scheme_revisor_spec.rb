...
describe ColorSchemeRevisor do
  let(:color) do
    Fabricate.build(:color_scheme_color, hex: 'FFFFFF', color_scheme: nil)
  end
  let(:color_scheme) do
    Fabricate(
      :color_scheme,
      created_at: 1.day.ago, updated_at: 1.day.ago, color_scheme_colors: [color]
    )
  end
  ...
  describe 'revise' do
    it 'does nothing if there are no changes' do
      ...
      end.to_not change { color_scheme.reload.updated_at }
    end

    it 'can change the name' do
      ColorSchemeRevisor.revise(
        color_scheme,
        valid_params.merge(name: 'Changed Name')
      )
    ...
    it 'can update the base_scheme_id' do
      ColorSchemeRevisor.revise(
        color_scheme,
        valid_params.merge(base_scheme_id: 'test')
      )
    ...
    it "doesn't make changes when a color is invalid" do
      ...
      end.to_not change { color_scheme.reload.version }
