...
describe TopicStatusUpdater do
  ...
  it 'adds an autoclosed message' do
    ...
    expect(last_post.raw).to eq(
          I18n.t('topic_statuses.autoclosed_enabled_minutes', count: 0)
        )
  end
  ...
  describe 'repeat actions' do
    shared_examples "an action that doesn't repeat" do
      it 'does not perform the update twice' do
        ...
        updated =
          TopicStatusUpdater.new(topic, admin).update!(status_name, true)
        ...
        updated =
          TopicStatusUpdater.new(topic, admin).update!(status_name, false)
        expect(updated).to eq(true)
    ...
    it 'updates autoclosed' do
      ...
      updated =
        TopicStatusUpdater.new(topic, admin).update!('autoclosed', false)
      expect(updated).to eq(true)
