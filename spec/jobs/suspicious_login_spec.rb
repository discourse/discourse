...
RSpec.describe Jobs::SuspiciousLogin do
  ...
  it "computes the distance between login locations" do
  ...
  it "does not email the user on their first login" do
    expect do
  ...
  it "does not email the user after a login from a known location" do
  ...
  it "emails the user after a login from a new location" do
    UserAuthTokenLog.create!(action: "generate", user_id: user.id, client_ip: "1.1.1.1")
    ...
    expect(Jobs::CriticalUserEmail.jobs.first["args"].first["type"]).to eq("suspicious_login")
  end
end
