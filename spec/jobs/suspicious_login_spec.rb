...
describe Jobs::SuspiciousLogin do
  ...
  it 'will correctly compute distance' do
    def expect_distance(from, to, distance)
      expect(UserAuthToken.distance(from.values, to.values).to_i).to eq(
            distance
          )
      expect(UserAuthToken.distance(to.values, from.values).to_i).to eq(
            distance
          )
    end
  ...
  it 'will not send an email when user log in from a known location' do
    UserAuthTokenLog.create!(
      action: 'generate', user_id: user.id, client_ip: '1.1.1.1'
    )
  ...
  it 'will send an email when user logs in from a new location' do
    ...
    expect(Jobs::CriticalUserEmail.jobs.first['args'].first['type']).to eq(
          'suspicious_login'
        )
  end
end
