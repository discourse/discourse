...
describe Jobs::PollMailbox do
  ...
  describe '.poll_pop3' do
    context 'pop errors' do
      ...
      after { $redis.del(Jobs::PollMailbox::POLL_MAILBOX_TIMEOUT_ERROR_KEY) }

      it 'add an admin dashboard message on pop authentication error' do
        Net::POP3.any_instance.expects(:start).raises(
          Net::POPAuthenticationError.new
        )
          .at_least_once
      ...
      it 'logs an error on pop connection timeout error' do
        Net::POP3.any_instance.expects(:start).raises(Net::OpenTimeout.new)
          .at_least_once
  ...
  describe '#process_popmail' do
    ...
    it 'does not reply to a bounced email' do
      expect { process_popmail(:bounced_email) }.to_not change do
