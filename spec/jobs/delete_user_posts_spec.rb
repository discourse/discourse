...
RSpec.describe Jobs::DeleteUserPosts do
  ...
  it "deletes all posts for the user in batches" do
    ...
    described_class.new.execute(user_id: user.id, admin_id: admin.id)
    user.reload
  ...
  it "raises Discourse::InvalidAccess if admin is not an admin" do
    ...
    expect { described_class.new.execute(user_id: user.id, admin_id: non_admin.id) }.to raise_error(
  ...
  it "raises Discourse::InvalidAccess if admin cannot delete all posts for the user" do
    ...
    expect { described_class.new.execute(user_id: user.id, admin_id: admin.id) }.to raise_error(
      Discourse::InvalidAccess,
    )
  end
  ...
  it "does nothing if user has no posts" do
    ...
    user.reload
    expect { described_class.new.execute(user_id: user.id, admin_id: admin.id) }.not_to change {
      user.posts.count
    }
  end
