...
describe Jobs::ExportCsvFile do
  context '.execute' do
    ...
    it 'raises an error when the entity is missing' do
      expect do
        Jobs::ExportCsvFile.new.execute(user_id: user.id)
    ...
    it 'works' do
      begin
        ...
        expect(user.topics_allowed.last.title).to eq(
              I18n.t(
        ...
        expect(user.topics_allowed.last.first_post.raw).to include(
              'user-archive-john_doe-'
            )
      ensure
  ...
  it 'exports sso data' do
    ...
    user.create_single_sign_on_record(
      external_id: '123', last_payload: 'xxx', external_email: 'test@test.com'
    )
