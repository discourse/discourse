...
describe Jobs::PollFeed do
  ...
  context 'execute' do
    ...
    before { $redis.del('feed-polled-recently') }
  ...
  describe '#poll_feed' do
    ...
    let!(:feed_author) do
      Fabricate(:user, username: 'xrav3nz', email: 'hi@bye.com')
    end
    ...
    shared_examples 'topic creation based on the the feed' do
      ...
      it 'parses creates a new post correctly' do
        ...
        expect(Topic.last.topic_embed.embed_url).to eq(
              'https://blog.discourse.org/2017/09/poll-feed-spec-fixture'
            )
      end
    end
    ...
    context 'encodings' do
      ...
      it 'respects the charset in the Content-Type header' do
        ...
        expect { poller.poll_feed }.to change { Topic.count }.by(1)
        expect(Topic.last.first_post.raw).to include(
              '<p>This is the body &amp; content. 100€ </p>'
            )
      end
