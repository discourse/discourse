...
describe Jobs::Tl3Promotions do
  ...
  it 'promotes tl2 user who qualifies for tl3' do
    ...
    Promotion.any_instance.expects(:change_trust_level!).with(
      TrustLevel[3],
      anything
    )
      .once
  ...
  context "tl3 user who doesn't qualify for tl3 anymore" do
    def create_leader_user
      ...
      TrustLevel3Requirements.any_instance.stubs(:requirements_met?).returns(
        true
      )
    ...
    it 'demotes if was promoted more than X days ago' do
      ...
      TrustLevel3Requirements.any_instance.stubs(:requirements_lost?).returns(
        true
      )
      run_job
