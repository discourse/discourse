...
describe Jobs::NotifyMailingListSubscribers do
  ...
  before do
    mailing_list_user.user_option.update(
      mailing_list_mode: true, mailing_list_mode_frequency: 1
    )
  end
  ...
  shared_examples 'no emails' do
    it "doesn't send any emails" do
      UserNotifications.expects(:mailing_list_notify).with(
        mailing_list_user,
        post
      )
        .never
  ...
  context 'when mailing list mode is globally enabled' do
    ...
    context 'with a valid post from another user' do
      ...
      context 'to an user who has disabled mailing list mode' do
        before do
          mailing_list_user.user_option.update(mailing_list_mode: false)
        end
        include_examples 'no emails'
      end
      ...
      context "to an user who has frequency set to 'always'" do
        before do
          mailing_list_user.user_option.update(mailing_list_mode_frequency: 1)
        end
      ...
      context "to an user who has frequency set to 'no echo'" do
        before do
          mailing_list_user.user_option.update(mailing_list_mode_frequency: 2)
        end
        include_examples 'one email'
      end
      ...
      context 'from an ignored user' do
        before do
          Fabricate(:ignored_user, user: mailing_list_user, ignored_user: user)
        end
        include_examples 'no emails'
      end
        ...
        it "doesn't send any emails" do
          (SiteSetting.max_emails_per_day_per_user + 1).times do
            mailing_list_user.email_logs.create(
              email_type: 'foobar', to_address: mailing_list_user.email
            )
          end
      ...
      context 'bounce score was reached' do
        it "doesn't send any emails" do
          mailing_list_user.user_stat.update(
            bounce_score: SiteSetting.bounce_score_threshold + 1
          )
    ...
    context 'with a valid post from same user' do
      ...
      context "to an user who has frequency set to 'daily'" do
        before do
          mailing_list_user.user_option.update(mailing_list_mode_frequency: 0)
        end
        include_examples 'no emails'
      end
