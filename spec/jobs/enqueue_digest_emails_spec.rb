...
describe Jobs::EnqueueDigestEmails do
  describe '#target_users' do
    context 'disabled digests' do
      ...
      let!(:user_no_digests) do
        Fabricate(
          :active_user,
          last_emailed_at: 8.days.ago, last_seen_at: 10.days.ago
        )
      end

      it "doesn't return users with email disabled" do
        expect(
          Jobs::EnqueueDigestEmails.new.target_user_ids.include?(
            user_no_digests.id
          )
        ).to eq(false)
      end
    ...
    context 'unapproved users' do
      ...
      it 'should enqueue the right digest emails' do
        ...
        unapproved_user.update_attributes(
          admin: false, moderator: false, approved: true
        )
        expect(
          Jobs::EnqueueDigestEmails.new.target_user_ids.include?(
            unapproved_user.id
    ...
    context 'staged users' do
      let!(:staged_user) do
        Fabricate(
          :active_user,
          staged: true, last_emailed_at: 1.year.ago, last_seen_at: 1.year.ago
        )
      end

      it "doesn't return staged users" do
        expect(
          Jobs::EnqueueDigestEmails.new.target_user_ids.include?(staged_user.id)
        ).to eq(false)
      end
    end

    context 'recently emailed' do
      let!(:user_emailed_recently) do
        Fabricate(:active_user, last_emailed_at: 6.days.ago)
      end

      it "doesn't return users who have been emailed recently" do
        expect(
          Jobs::EnqueueDigestEmails.new.target_user_ids.include?(
            user_emailed_recently.id
          )
        ).to eq(false)
      end
    end
    ...
    context 'suspended user' do
      let!(:suspended_user) do
        Fabricate(
          :user,
          suspended_till: 1.week.from_now, suspended_at: 1.day.ago
        )
      end
    ...
    context 'visited the site this week' do
      let(:user_visited_this_week) do
        Fabricate(:active_user, last_seen_at: 6.days.ago)
      end
      let(:user_visited_this_week_email_always) do
        Fabricate(
          :active_user,
      ...
      it "doesn't return users who have been emailed recently" do
        ...
        expect(
          Jobs::EnqueueDigestEmails.new.target_user_ids.include?(user.id)
        ).to eq(false)
      end
    end

    context 'visited the site a year ago' do
      let!(:user_visited_a_year_ago) do
        Fabricate(:active_user, last_seen_at: 370.days.ago)
      end

      it "doesn't return the user who have not visited the site for more than 365 days" do
        expect(
          Jobs::EnqueueDigestEmails.new.target_user_ids.include?(
            user_visited_a_year_ago.id
          )
        ).to eq(false)
      end
    end
    ...
    context 'too many bounces' do
      ...
      it "doesn't return users with too many bounces" do
        bounce_user.user_stat.update(
          bounce_score: SiteSetting.bounce_score_threshold + 1
        )
        expect(
          Jobs::EnqueueDigestEmails.new.target_user_ids.include?(bounce_user.id)
        ).to eq(false)
      end
    end
  end
  ...
  describe '#execute' do
    ...
    context 'digest emails are enabled' do
      before do
        Jobs::EnqueueDigestEmails.any_instance.expects(:target_user_ids)
          .returns([user.id])
      end
