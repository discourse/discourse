...
describe Jobs::UserEmail do
  ...
  it "raises an error when the type doesn't exist" do
    expect do
      Jobs::UserEmail.new.execute(type: :no_method, user_id: user.id)
  ...
  context 'disable_emails setting' do
    ...
    it 'sends when critical' do
      SiteSetting.disable_emails = 'yes'
      Jobs::CriticalUserEmail.new.execute(
        type: :confirm_new_email, user_id: user.id
      )
  ...
  context 'recently seen' do
    ...
    it "doesn't send a PM email to a user that's been recently seen and has email_messages_level set to never" do
      ...
      Jobs::UserEmail.new.execute(
        type: :user_private_message, user_id: user.id, post_id: post.id
      )
    ...
    it 'creates a skipped email log when the mail is skipped' do
      ...
      expect(
        SkippedEmailLog.exists?(
          ...
          reason_type:
            SkippedEmailLog.reason_types[:user_email_user_suspended_not_pm]
        )
      ).to eq(true)
  ...
  context 'args' do
    it 'passes a token as an argument when a token is present' do
      Jobs::UserEmail.new.execute(
        type: :forgot_password, user_id: user.id, email_token: 'asdfasdf'
      )
    ...
    context 'post' do
      ...
      context 'user is suspended' do
        it "doesn't send email for a pm from a regular user" do
          Jobs::UserEmail.new.execute(
            type: :user_private_message, user_id: suspended.id, post_id: post.id
          )
      ...
      context 'user is anonymous' do
        ...
        it "doesn't send email for a pm from a regular user" do
          Jobs::UserEmail.new.execute(
            type: :user_private_message, user_id: anonymous.id, post_id: post.id
          )
    ...
    context 'notification' do
      ...
      it 'does send the email if the user is using daily mailing list mode' do
        user.user_option.update(
          mailing_list_mode: true, mailing_list_mode_frequency: 0
        )
      ...
      context 'max_emails_per_day_per_user limit is reached' do
        ...
        it 'sends critical email' do
          ...
          expect(
            EmailLog.exists?(email_type: 'forgot_password', user: user)
          ).to eq(true)
        end
      end
      ...
      it "doesn't send the mail if the user is using individual mailing list mode" do
        ...
        # When post is nil
        Jobs::UserEmail.new.execute(
          type: :user_mentioned, user_id: user.id, notification_type: 'posted'
        )
      ...
      it "doesn't send the mail if the user is using individual mailing list mode with no echo" do
        user.user_option.update(
          mailing_list_mode: true, mailing_list_mode_frequency: 2
        )
        # sometimes, we pass the notification_id
        Jobs::UserEmail.new.execute(
          type: :user_mentioned,
      ...
      context 'user is suspended' do
        ...
        context 'pm from staff' do
          before do
            @pm_from_staff = Fabricate(:post, user: Fabricate(:moderator))
            @pm_from_staff.topic.topic_allowed_users.create!(
              user_id: suspended.id
            )
            @pm_notification =
              Fabricate(
