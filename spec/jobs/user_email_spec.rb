  ...
  context 'args' do
    ...
    context "confirm_new_email" do
      ...
      context "when the change was requested by admin" do
        ...
        it "passes along true for the requested_by_admin param" do
          UserNotifications.expects(:confirm_new_email).with(user, email_token: email_token.token, requested_by_admin: true)
      ...
      context "when the change was requested by the user" do
        ...
        it "passes along true for the requested_by_admin param" do
          UserNotifications.expects(:confirm_new_email).with(user, email_token: email_token.token, requested_by_admin: false)
          Jobs::UserEmail.new.execute(type: :confirm_new_email, user_id: user.id, email_token: email_token.token)
        end
      end
    end
