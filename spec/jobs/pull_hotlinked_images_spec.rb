...
describe Jobs::PullHotlinkedImages do
  ...
  before do
    ...
    stub_request(:get, large_image_url).to_return(
      body: large_png, headers: { 'Content-Type' => 'image/png' }
    )
  ...
  describe '#execute' do
    ...
    it 'replaces images' do
      ...
      expect(post.raw).to match(%r{^<img src='\/uploads})
    end
    ...
    describe 'onebox' do
      ...
      it 'all combinations' do
        ...
        expect(post.cooked).to match(
              %r{<img src=.*\/uploads.*\ class="thumbnail"}
            )
  ...
  describe '#is_valid_image_url' do
    ...
    describe 'when url is valid' do
      it 'should return true' do
        expect(subject.is_valid_image_url('http://meta.discourse.org')).to eq(
              true
            )
