  ...
  context "with an admin who hasn't been seen recently" do
    ...
    context 'invalidate_inactive_admin_email_after_days = 365' do
      ...
      it 'marks email tokens as unconfirmed' do
        subject
        expect(
          not_seen_admin.reload.email_tokens.where(confirmed: true).exists?
        ).to eq(false)
      end
      ...
      context 'with social logins' do
        before do
          GithubUserInfo.create!(
            user_id: not_seen_admin.id, screen_name: 'bob', github_user_id: 100
          )
          UserOpenId.create!(
            url: 'https://me.yahoo.com/id/123',
          ...
          UserAssociatedAccount.create!(
            ...
            user_id: not_seen_admin.id,
            provider_uid: 100,
            info: { email: 'bob@google.account.com' }
          )
        end
        ...
        it 'removes the social logins' do
          subject
          expect(
            GithubUserInfo.where(user_id: not_seen_admin.id).exists?
          ).to eq(false)
          expect(
            UserAssociatedAccount.where(user_id: not_seen_admin.id).exists?
          ).to eq(false)
          expect(UserOpenId.where(user_id: not_seen_admin.id).exists?).to eq(
                false
              )
        end
      end
    ...
    context 'invalidate_inactive_admin_email_after_days = 0 to disable this feature' do
      ...
      it 'does nothing' do
        subject
        expect(active_admin.reload.active).to eq(true)
        expect(active_admin.email_tokens.where(confirmed: true).exists?).to eq(
              true
            )
        expect(not_seen_admin.reload.active).to eq(true)
        expect(
          not_seen_admin.email_tokens.where(confirmed: true).exists?
        ).to eq(true)
      end
    end
  end
