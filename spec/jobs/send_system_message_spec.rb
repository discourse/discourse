...
describe Jobs::SendSystemMessage do
  it 'raises an error without a user_id' do
    expect do
      Jobs::SendSystemMessage.new.execute(message_type: 'welcome_invite')
    end.to raise_error(Discourse::InvalidParameters)
  end

  it 'raises an error without a message_type' do
    expect do
      Jobs::SendSystemMessage.new.execute(user_id: 1234)
    end.to raise_error(Discourse::InvalidParameters)
  end
  ...
  context 'with valid parameters' do
    ...
    it 'should call SystemMessage.create' do
      ...
      Jobs::SendSystemMessage.new.execute(
        user_id: user.id, message_type: 'welcome_invite'
      )
    end
    ...
    it 'can send message parameters' do
      ...
      Jobs::SendSystemMessage.new.execute(
        user_id: user.id, message_type: 'post_hidden', message_options: options
      )
    end
  end
end
