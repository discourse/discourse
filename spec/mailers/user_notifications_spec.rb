...
describe UserNotifications do
  ...
  describe '#get_context_posts' do
    it 'does not include hidden/deleted/user_deleted posts in context' do
      ...
      _post5 =
        Fabricate(
          :post,
          topic: post1.topic, post_type: Post.types[:moderator_action]
        )
      _post6 =
        Fabricate(
          :post,
          topic: post1.topic, post_type: Post.types[:small_action]
        )
      _post7 =
        Fabricate(:post, topic: post1.topic, post_type: Post.types[:whisper])
      last = Fabricate(:post, topic: post1.topic)

      post1.user.user_option.email_previous_replies =
        UserOption.previous_replies_type[:always]

      # default is only post #1
      expect(
        UserNotifications.get_context_posts(last, nil, post1.user).count
      ).to eq(1)
      # staff members can also see the whisper
      moderator = build(:moderator)
      moderator.user_option = UserOption.new
      moderator.user_option.email_previous_replies =
        UserOption.previous_replies_type[:always]
      tu = TopicUser.new(topic: post1.topic, user: moderator)
      expect(
        UserNotifications.get_context_posts(last, tu, tu.user).count
      ).to eq(2)
    end
    ...
    it 'allows users to control context' do
      ...
      expect(
        UserNotifications.get_context_posts(post3, topic_user, user).count
      ).to eq(1)
  ...
  describe '.digest' do
      ...
      let!(:new_today) do
        Fabricate(
          ...
          user:
            Fabricate(
              :user,
              trust_level: TrustLevel[0], created_at: 10.minutes.ago
            ),
      ...
      let!(:new_yesterday) do
        Fabricate(
          ...
          user:
            Fabricate(
              :user,
              trust_level: TrustLevel[0], created_at: 25.hours.ago
            ),
    ...
    context 'with new topics' do
      let!(:popular_topic) do
        Fabricate(
          :topic,
          user: Fabricate(:coding_horror), created_at: 1.hour.ago
        )
      end
      ...
      it 'supports subfolder' do
        ...
        expect(subject.header['List-Unsubscribe'].to_s).to match(
              %r{http:\/\/test.localhost\/forum\/email\/unsubscribe\/\h{64}}
            )

        topic_url =
          "http://test.localhost/forum/t/#{popular_topic.slug}/#{popular_topic
            .id}"
  ...
  describe '.user_replied' do
    ...
    let(:topic) do
      Fabricate(
        :topic,
        category: category, tags: [tag1, tag2], title: 'Super cool topic'
      )
    end
    let(:post) do
      Fabricate(:post, topic: topic, raw: 'This is My super duper cool topic')
    end
    let(:response) do
      Fabricate(:basic_reply, topic: post.topic, user: response_by_user)
    end
    let(:user) { Fabricate(:user) }
    let(:notification) do
      Fabricate(:replied_notification, user: user, post: response)
    end
  ...
  describe '.user_posted' do
    let(:response_by_user) do
      Fabricate(:user, name: 'John Doe', username: 'john')
    end
    ...
    let(:user) { Fabricate(:user) }
    let(:notification) do
      Fabricate(:posted_notification, user: user, post: response)
    end
    ...
    it 'uses the original subject for staged users' do
      incoming_email =
        Fabricate(
          :incoming_email,
          subject: 'Original Subject', post: post, topic: post.topic, user: user
        )
  ...
  describe '.user_private_message' do
    ...
    let(:user) { Fabricate(:user) }
    let(:notification) do
      Fabricate(:private_message_notification, user: user, post: response)
    end
    ...
    it 'includes a list of participants, groups first with member lists' do
      ...
      expect(mail.body).to include(
            '[group1 (2)](http://test.localhost/groups/group1), [group2 (1)](http://test.localhost/groups/group2), [one](http://test.localhost/u/one), [two](http://test.localhost/u/two)'
          )
    end
    ...
    it 'uses the original subject for staged users when topic was started via email' do
      incoming_email =
        Fabricate(
          :incoming_email,
          subject: 'Original Subject', post: post, topic: topic, user: user
        )
  ...
  it 'adds a warning when mail limit is reached' do
    ...
    user.email_logs.create!(
      email_type: 'blah', to_address: user.email, user_id: user.id
    )
    ...
    expect(mail.html_part.to_s).to match(
          I18n.t('user_notifications.reached_limit', count: 2)
        )
  ...
  def expects_build_with(condition)
    UserNotifications.any_instance.expects(:build_email).with(
      user.email,
      condition
    )
    mailer =
  ...
  describe 'group invited to a private message' do
    include_examples 'notification email building' do
      ...
      let(:mail_template) do
        "user_notifications.user_#{notification_type}_pm_group"
      end
