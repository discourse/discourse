...
RSpec.describe "a refused request" do
  ...
  describe "the sort parameter" do
    ...
    context "when an undeclared sort comes with an anchor" do
      let(:params) { { sort: { secrets: :asc }, page: { anchor: { title: middle.title } } } }
  ...
  describe "the anchor parameter" do
    context "when the resource declares no such anchor" do
      let(:params) { { page: { anchor: { views: 1 } } } }
    ...
    context "when the request has two anchors" do
      let(:params) { { page: { anchor: { id: 1, title: "a topic" } } } }
        ...
        expect(document).to eq(
          errors: [
            refusal(
              ...
              detail: "page[anchor][id] must be a single value.",
    ...
    context "when the request has an anchor and a cursor" do
      let(:params) { { page: { anchor: { id: oldest.id }, after: cursor } } }
        ...
        expect(document).to eq(
          errors: [
            refusal(
              ...
              detail: "Use page[anchor] or page[after], not both.",
              parameter: "page[anchor]",
            ),
          ],
        )
