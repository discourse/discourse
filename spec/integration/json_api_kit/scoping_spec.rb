...
RSpec.describe "a listing kept to other rows" do
  ...
  describe "the rows a resource exposes" do
    ...
    context "when a row matches the scope" do
      ...
      it "renders that row" do
        expect(document).to eq(
          data: [topic_object(closed, fields: %w[title])],
          included: [],
          links: links_of,
        )
      end
    ...
    context "when the resource declares no scope of its own" do
      ...
      it "renders every row of its model" do
        expect(document).to eq(
          data: [topic_object(newest), topic_object(middle), topic_object(oldest)],
          included: [],
          links: links_of,
        )
      end
  ...
  describe "the rows a related resource exposes" do
    fab!(:open_category) { Fabricate(:category, name: "Open to everyone") }
    fab!(:closed_category) { Fabricate(:category, name: "Kept from everyone") }
    fab!(:filed_away) do
      Fabricate(:topic, title: "Filed in a closed category", category: closed_category)
    end
    ...
    let(:categories) do
      Class.new(JsonApiKit::Resource) do
        ...
        attribute :name
        scope do |guardian|
          Category.where(id: Category.where(name: "Open to everyone").select(:id))
        end
      end
    end
    let(:resource) do
      ...
      Class.new(JsonApiKit::Resource) do
        ...
        type :topics
        attribute :title
        has_one :category, resource: related
      end
    end
    let(:params) { { include: %w[category] } }
    let(:scoped_to) { Topic.where(id: filed_away.id) }

    it "renders no row that scope hides" do
      expect(document[:included]).to be_empty
    end

    it "links to no row that scope hides" do
      expect(document[:data].sole[:relationships]["category"][:data]).to be_nil
    end
  end
