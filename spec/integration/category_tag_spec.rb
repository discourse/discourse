...
describe 'category tag restrictions' do
  ...
  context 'tags restricted to one category' do
    ...
    it 'tags belonging to that category can only be used there' do
      post =
        create_post(
          category: category_with_tags, tags: [tag1.name, tag2.name, tag3.name]
        )
      expect(post.topic.tags).to contain_exactly(tag1, tag2)

      post =
        create_post(
          category: other_category, tags: [tag1.name, tag2.name, tag3.name]
        )
    ...
    it 'search can show only permitted tags' do
      ...
      ).to contain_exactly(tag1, tag2)
      expect(filter_allowed_tags(for_input: true)).to contain_exactly(
            tag3,
            tag4
          )
      expect(
    ...
    it "can't create new tags in a restricted category" do
      ...
      post =
        create_post(
          category: category_with_tags, tags: [tag1.name, 'newtag'], user: admin
        )
      expect(post.topic.tags).to contain_exactly(tag1)
    end
    ...
    it 'can create tags when changing category settings' do
      expect { other_category.update(allowed_tags: %w[newtag]) }.to change do
        Tag.count
      end
        .by(1)
      expect do
        other_category.update(
          allowed_tags: [tag1.name, 'tag-stuff', tag2.name, 'another-tag']
        )
  ...
  context 'tag groups restricted to a category' do
    ...
    before { tag_group1.tags = [tag1, tag2] }
    ...
    it 'tags in the group are used by category tag restrictions' do
      ...
      expect(
        filter_allowed_tags(for_input: true, category: category)
      ).to contain_exactly(tag2, tag3, tag4)
  ...
  context 'tag groups with parent tag' do
    it 'filter_allowed_tags returns results based on whether parent tag is present or not' do
      ...
      expect(
        filter_allowed_tags(
          for_input: true, selected_tags: [tag1.name, tag3.name]
        )
      ).to contain_exactly(tag2, tag4)
    end
    ...
    context 'and category restrictions' do
      ...
      before do
        ...
        car_category.allowed_tag_groups = [
          makes.name,
          honda_group.name,
          ford_group.name
        ]
      ...
      it 'handles all those rules' do
        ...
        expect(
          filter_allowed_tags(for_input: true, category: other_category)
        ).to contain_exactly(tag1, tag2, tag3, tag4)
        ...
        expect(
          sorted_tag_names(
            filter_allowed_tags(
              for_input: true, category: car_category, selected_tags: %w[honda]
            )
          )
        ...
        expect(
          sorted_tag_names(
            filter_allowed_tags(
              for_input: true, category: car_category, selected_tags: %w[ford]
            )
          )
...
describe 'tag topic counts per category' do
  ...
  it 'counts when a topic is created with tags' do
    ...
    end.to change { CategoryTagStat.count }.by(2)
    expect(
      CategoryTagStat.where(category: category, tag: tag1).sum(:topic_count)
    ).to eq(1)
    expect(
      CategoryTagStat.where(category: category, tag: tag2).sum(:topic_count)
    ).to eq(1)
  end
  ...
  it 'counts when tag is added to an existing topic' do
    ...
    expect do
      PostRevisor.new(post).revise!(
        topic.user,
        raw: post.raw, tags: [tag1.name, tag2.name]
      )
    end.to change { CategoryTagStat.count }.by(2)
    expect(
      CategoryTagStat.where(category: category, tag: tag1).sum(:topic_count)
  ...
  context 'topic with 2 tags' do
    ...
    it "has correct counts after a topic's category changes" do
      PostRevisor.new(post).revise!(
        topic.user,
        category_id: category2.id, raw: post.raw, tags: [tag1.name, tag2.name]
      )
      expect(
        CategoryTagStat.where(category: category, tag: tag1).sum(:topic_count)
      ).to eq(0)
      ...
      expect(
        CategoryTagStat.where(category: category2, tag: tag1).sum(:topic_count)
      ).to eq(1)
      expect(
        CategoryTagStat.where(category: category2, tag: tag2).sum(:topic_count)
      ).to eq(1)
    end
    ...
    it "has correct counts after topic's category AND tags changed" do
      PostRevisor.new(post).revise!(
        topic.user,
        raw: post.raw, tags: [tag2.name, tag3.name], category_id: category2.id
      )
      expect(
        CategoryTagStat.where(category: category, tag: tag1).sum(:topic_count)
      ).to eq(0)
      ...
      expect(
        CategoryTagStat.where(category: category, tag: tag3).sum(:topic_count)
      ).to eq(0)
      expect(
        CategoryTagStat.where(category: category2, tag: tag1).sum(:topic_count)
      ).to eq(0)
      ...
      expect(
        CategoryTagStat.where(category: category2, tag: tag3).sum(:topic_count)
      ).to eq(1)
    end
  end
