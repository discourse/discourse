...
describe WatchedWord do
  ...
  let(:flag_word) do
    Fabricate(:watched_word, action: WatchedWord.actions[:flag])
  end
  let(:block_word) do
    Fabricate(:watched_word, action: WatchedWord.actions[:block])
  end
  ...
  context 'block' do
    def should_block_post(manager)
      expect do
        ...
        expect(result.errors[:base]&.first).to eq(
              I18n.t('contains_blocked_words', word: block_word.word)
            )
    ...
    it 'should prevent the post from being created' do
      manager =
        NewPostManager.new(
          tl2_user,
          raw: "Want some #{block_word.word} for cheap?", topic_id: topic.id
        )
    ...
    it 'should block in a private message too' do
      manager =
        NewPostManager.new(
          ...
          target_usernames:
            Fabricate(:user, trust_level: TrustLevel[2]).username
        )
      should_block_post(manager)
    end
    ...
    it 'blocks on revisions' do
      post =
        Fabricate(
          :post,
          topic: Fabricate(:topic, user: tl2_user), user: tl2_user
        )
      expect do
    ...
    it 'looks at title too' do
      manager =
        NewPostManager.new(
          tl2_user,
          title:
            "You won't believe these #{require_approval_word.word} dog names!",
  ...
  context 'flag' do
    ...
    def should_not_flag_post(author, raw, topic)
      ...
      end.to_not change { PostAction.count }
    end
    ...
    it 'should look at the title too' do
      should_flag_post(
        ...
        Fabricate(
          :topic,
          user: tl2_user, title: "Read my #{flag_word.word} review!"
        )
      )
    end
