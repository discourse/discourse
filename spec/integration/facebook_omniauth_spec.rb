...
describe "Facebook OAuth2" do
  ...
  def setup_facebook_email_stub(email:)
    ...
    stub_request(
      :get,
      "https://graph.facebook.com/v26.0/me?appsecret_proof=#{appsecret_proof}&fields=name,first_name,last_name,email",
  ...
  before do
    ...
    stub_request(:post, "https://graph.facebook.com/v26.0/oauth/access_token").with(
      body:
        hash_including(
          "client_id" => app_id,
  ...
  it "signs in the user if the API response from facebook includes an email (implies it's verified) and the email matches an existing user's" do
    ...
    expect(response.location).to start_with("https://www.facebook.com/v26.0/dialog/oauth")
