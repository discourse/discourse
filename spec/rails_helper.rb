...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    ...
    # Replace `Capybara::Playwright::Driver#reset!`'s default
    # `clear_browser_contexts` (which closes the BrowserContext, nils
    # `@browser`, and forces the next test to rebuild Browser+Context+
    # Page from scratch) with a soft-reset that **keeps the primary
    # BrowserContext alive between examples** but **closes and recreates
    # the primary Page**. The kept-alive context preserves DNS/TLS
    # connection pool warmth and the browser HTTP response cache (where
    # most of the per-test cost goes); recreating the page ensures the
    # next example starts with a genuinely-clean JS context (no stale
    # Ember app, MessageBus subscriptions, EventTarget listeners) and
    # auto-detaches every CDP session attached to the old page (e.g.
    # the `Network.emulateNetworkConditions` session that
    # `cdp.with_network_disconnected` opens but never closes — iter-7
    # identified this as the root cause of iter-6's network_disconnected
    # → admin_color_palette_config_area cross-spec failure).
    #
    # Per-reset cleanup steps (order matters — the CDP session attaches
    # to the primary page, so storage clears must happen before the
    # primary page is closed):
    #   1. Storage.clearDataForOrigin (storageTypes: "all") for each
    #      real origin pages in the primary context were last on. Wipes
    #      cookies, localStorage, sessionStorage, IndexedDB, service
    #      workers, cache_storage on a per-origin basis.
    #   2. Close every auxiliary BrowserContext (created by
    #      `open_new_window(:window)`); typically zero per test.
    #   3. Close every auxiliary page in the primary context (created
    #      by `open_new_window(:tab)`).
    #   4. Clear context-level permissions (clipboard / notification /
    #      etc. granted by `cdp.allow_clipboard`) and remaining cookies
    #      on the primary context.
    #   5. Close the primary page — drops all in-memory JS state and
    #      auto-detaches every CDP session attached to that page.
    #   6. Create a fresh primary page on the surviving BrowserContext.
    #      The context's `'page'` event handler (registered by the gem
    #      in `Browser#create_browser_context`) reassigns
    #      `@playwright_page` to the new page when the prior reference
    #      was nilled by the old page's `'close'` callback.
    #
    # Tracing (`:trace` metadata) needs the full BrowserContext lifecycle
    # to flush a trace zip, so that path falls through to the upstream
    # `clear_browser_contexts` reset. Any unexpected failure in the
    # soft-reset path also falls through, so a single broken reset costs
    # at most one example's full reset rather than cascading.
    #
    # Loop history of this mechanism family:
    #   * iter-2 used `goto("about:blank", timeout: 5000)` with wildcard
    #     origin for storage clear; CI step jumped to 1681s (+1081s)
    #     because service workers from the prior origin stalled the
    #     load event.
    #   * iter-6 kept primary page+context alive (no aux cleanup). CI
    #     step 507s (-22% baseline) but tests failed: stale CDP
    #     network-throttle sessions + stale clipboard permissions broke
    #     several specs.
    #   * iter-8 added auxiliary cleanup AND `Network.clearBrowserCache`
    #     globally; the global cache wipe undid iter-6's perf win
    #     (asset cache reset on every test). CI step 643s ~= baseline.
    #   * iter-9 dropped global cache wipe, ran with kept-alive
    #     page+context. CI step 517s (-10% best, the cleanest signal in
    #     this family) but tests still failed — the kept-alive page
    #     accumulated per-page state (CDP sessions, JS context, etc.)
    #     this iteration's close+recreate-page step is the surgical fix.
    #   * iter-10 tried `goto("about:blank")` again with `waitUntil:
    #     "commit"` and a 1500ms timeout cap. CI cancelled at 1694s —
    #     the goto cascaded similarly to iter-2 despite the bounded
    #     timeout. Closing the page is bounded (synchronous to the gem)
    #     and doesn't trigger beforeunload handlers (the iter-10
    #     cascade hypothesis), so this iteration avoids that failure
    #     mode entirely.
    #
    # The color-palette case (admin_color_palette_config_area_spec.rb)
    # depends on a fresh browser HTTP response cache between examples
    # because that file mutates a shared color_scheme between tests and
    # asserts the rendered html background reflects the new palette.
    # Closing the page does not flush per-context HTTP cache — that's a
    # separate Network-domain entity. So a per-spec
    # `cdp.clear_browser_cache` after-hook is added there only.
    ...
    module CapybaraPlaywrightBrowserSoftResetPatch
      ...
      def soft_reset_browser_state
        ...
        # Wipe per-origin storage on the primary context BEFORE closing
        # the primary page (the CDP session attaches to the primary
        # page; closing the page would tear it down mid-call).
        ...
        begin
          ...
          # Page or context torn down mid-reset; the upstream fallback
          # in Driver#reset! rebuilds from scratch on the next example.
        end

        # Close every auxiliary BrowserContext (e.g. from
        # `open_new_window(:window)`). Their pages and any CDP sessions
        # are torn down with them.
        ...
        # Close every auxiliary page in the primary context (e.g. tabs
        # opened by `open_new_window(:tab)`). The primary page is closed
        # in a later step, after context-level state is cleared.
        primary_ctx.pages.each do |pg|
        ...
        # Close the primary page. This drops all in-memory JS state
        # (Ember app, MessageBus client + subscriptions, EventTarget
        # listeners, in-flight long-polls) and auto-detaches every CDP
        # session attached to that page — including the
        # `Network.emulateNetworkConditions` sessions opened by
        # `cdp.with_network_disconnected` / `with_slow_download` /
        # `with_slow_upload`, which are never explicitly detached and
        # were the documented leak vector breaking iter-6's
        # network_disconnected → admin_color_palette_config_area chain.
        # Playwright's default page.close behaviour does not run
        # beforeunload handlers, so this never blocks on a confirm
        # dialog (avoiding the iter-2 / iter-10 about:blank navigation
        # cascade).
        begin
          primary.close
        rescue ::Playwright::Error
          # Page already torn down; the next step recreates it anyway.
        end

        # Create a fresh primary page on the surviving BrowserContext.
        # The context's `'page'` event handler set in
        # `Browser#create_browser_context` reassigns `@playwright_page`
        # to the new page (the prior page's `'close'` handler nilled it
        # one step earlier). The `create_page` private method also
        # registers a fresh `'close'` handler on the new page so the
        # next reset works.
        send(:create_page, primary_ctx)
      end
    end
