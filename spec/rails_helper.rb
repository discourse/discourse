...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    ...
    # The upstream `Capybara::Playwright::Driver#reset!` calls
    # `@browser.clear_browser_contexts`, which closes every BrowserContext
    # and nils `@browser`. The next test's first interaction lazily
    # rebuilds a fresh `Browser` (via `@browser ||= ...`) which creates a
    # new BrowserContext **and** a new Page from scratch. That cost is
    # paid ~1,800 times per CI run (every system spec hits `reset!` via
    # `Capybara.reset_session!` in the `after(:each, type: :system)`
    # hook). iter-6 measured CI step time at 507s vs the 656s baseline
    # (-22%) by skipping that close+recreate dance.
    #
    # The fix: keep the primary BrowserContext + primary Page alive
    # between examples (preserving DNS/TLS/HTTP-connection-pool warmth +
    # avoiding the new_context/new_page IPC roundtrips), and only wipe
    # the per-test state Capybara's session reset is responsible for —
    # storage, cookies, permissions, browser HTTP cache, and any
    # auxiliary contexts/pages created by `open_new_window`.
    #
    # Lessons folded in from prior iterations:
    #
    # * iter-2 (CI hang at 1681s vs ~600s baseline): `goto("about:blank",
    #   timeout: 5000)` per page on every reset multiplied to ~1,125s
    #   extra wall across the suite. The navigation isn't needed because
    #   every spec calls `visit(...)` (or `sign_in(...)`, which visits)
    #   before interacting, and that real navigation tears down the old
    #   Ember app. iter-2 also used `origin: "*"` for
    #   `Storage.clearDataForOrigin`; CDP doesn't document `*` as a glob
    #   so it can be a silent no-op on CI's Chromium. We use real
    #   per-page origins parsed from `pg.url`.
    #
    # * iter-6 (CI step 507s = -22% but tests failed): cleared per-origin
    #   storage and per-context cookies, but missed three categories of
    #   per-test state:
    #     1. **Browser HTTP cache** — not covered by
    #        `Storage.clearDataForOrigin`'s `storageTypes: "all"` (that
    #        flag covers per-origin storage like cache_storage /
    #        service_workers / IndexedDB; the HTTP response cache is a
    #        separate Network-domain entity). The locally-reproducible
    #        `admin_color_palette_config_area_spec.rb:162` failure with
    #        seed 22429 is precisely this: example 1:12 colours the
    #        default theme with the shared `color_scheme`, the browser
    #        caches that scheme's stylesheet body, and example 1:9 then
    #        edits secondary on the same scheme — the new `<link>` ends
    #        up in the DOM but the cached stylesheet body is the
    #        pre-edit one, so html backgroundColor stays white. Fixed
    #        here by also calling `Network.clearBrowserCache`.
    #     2. **Context-level permissions** (clipboard/notification/etc.)
    #        granted by `cdp.allow_clipboard`. Fixed by
    #        `clear_permissions` on the surviving context.
    #     3. **Auxiliary tabs/windows** opened by
    #        `open_new_window(:tab|:window)` — the original iter-6
    #        reset left these alive, which corrupted `current_window`
    #        inventory. Fixed by closing every page in the primary
    #        context that isn't the primary page, and closing every
    #        non-primary context.
    #
    # Tracing (`:trace` metadata) still requires the full context
    # lifecycle to flush a trace zip, so that path falls back to the
    # upstream `clear_browser_contexts` behaviour. An error during the
    # soft-reset (e.g. broken CDP session, dead context) also falls back
    # to the upstream path so the next example always starts clean.
      ...
      def reset!
        ...
        if @browser
          begin
            ...
            # Fall through to the original close-everything path.
          end
        end
    ...
    module CapybaraPlaywrightBrowserSoftResetPatch
      # Raised internally to signal that the soft-reset can't safely
      # proceed and the caller should fall back to clear_browser_contexts.
      ...
      def soft_reset_browser_state
        ...
        # Wipe per-origin storage and the per-context HTTP response cache
        # on the surviving primary BrowserContext, so the next example
        # starts with a clean slate (localStorage/sessionStorage/IndexedDB
        # /SW/cache_storage via Storage.clearDataForOrigin, plus the
        # Network-domain HTTP cache via Network.clearBrowserCache).
        #
        # `Storage.clearDataForOrigin` wants a real origin string, so we
        # collect unique origins from every page in the primary context
        # (covers tests that opened a `:tab` window before this reset
        # fires).
        origins = []
        ...
        begin
          ...
          # Network.clearBrowserCache wipes the HTTP response cache,
          # which Storage.clearDataForOrigin's `storageTypes: "all"`
          # does NOT cover (the latter handles per-origin storage APIs
          # — IndexedDB, cache_storage, service_workers etc.). Without
          # this wipe, a stylesheet body fetched by an earlier test
          # gets served back to the next test even when the URL has
          # the same hash — see the iter-6 / color-palette repro
          # documented above.
          cdp.send_message("Network.clearBrowserCache")
          cdp.detach
        rescue ::Playwright::Error
          # Page or context torn down mid-reset; the upstream fallback
          # in Driver#reset! will rebuild from scratch on the next
          # example, so a single failed soft-reset doesn't cascade.
        end

        # Close every auxiliary BrowserContext (e.g. from
        # `open_new_window(:window)`). Their pages and any CDP sessions
        # are torn down with them, and contexts don't accumulate across
        # the suite.
        ...
        # opened by `open_new_window(:tab)`). Keep the primary page alive
        # — that's the dominant perf gain over upstream's
        # `clear_browser_contexts` — but drop the extra tabs so
        # `current_window` inventory is reset.
        primary_ctx.pages.each do |pg|
