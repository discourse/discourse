...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    ...
    # BrowserContext + primary Page alive between examples and only
    # wipes per-test state. This is the same mechanism iter-6 measured
    # at -22% CI step time (507s vs 656s baseline) and iter-9 measured
    # at -10% CI step time (517s vs 577s best). Page-mutation refinements
    # (close+recreate page in iter-11, bounded `goto("about:blank")` in
    # iter-10) both cascaded in CI to ~1700s, so this revision keeps
    # the page alive entirely and addresses iter-7's named CDP-session
    # leak vector at the helper source (see
    # `spec/system/page_objects/cdp.rb`'s `with_network_disconnected` /
    # `with_slow_download` / `with_slow_upload` ensure blocks) instead
    # of trying to clean up sessions during the page-level reset.
    #
    # Per-reset cleanup steps:
    ...
    #      by `open_new_window(:tab)`), keep the primary page itself
    #      alive. Typically zero per test, so this is a no-op loop.
    ...
    # soft-reset path also falls through, so a single broken reset
    # costs at most one example's full reset rather than cascading.
    ...
    module CapybaraPlaywrightBrowserSoftResetPatch
      ...
      def soft_reset_browser_state
        ...
        # Quiesce the old Ember app *before* we wipe cookies/storage. With the
        # primary page kept alive, the previous test's app keeps long-polling
        # `/message-bus/poll` and other endpoints; after `clear_cookies` /
        # `Storage.clearDataForOrigin` strip its session, those in-flight and
        # subsequent requests come back as 401/403/302. The old app's
        # error handlers can react by mutating DOM (toasts, redirect modals)
        # and queuing more XHRs, which races with the next example's
        # `visit(...)` and the server-side `RspecErrorTracker` capture window
        # in `before(:each)`. iter-6/iter-9/iter-12/iter-13 all measured large
        # CI-step wins from kept-alive pages but failed on tests that we never
        # could repro locally — this is the one category of state none of the
        # prior soft-reset variants addressed.
        #
        # Concretely: stop MessageBus and detach Ember-portal nodes that were
        # appended directly to <body> (toasts, modals, popovers, dropdowns).
        # The script is best-effort; any exception leaves cleanup for the
        # next test's `visit(...)` to complete naturally on page navigation.
        begin
          primary.evaluate(<<~JAVASCRIPT)
            () => {
              try {
                if (window.MessageBus) {
                  if (typeof window.MessageBus.stop === 'function') {
                    window.MessageBus.stop();
                  }
                  if (Array.isArray(window.MessageBus.callbacks)) {
              ...
              try {
                document
                  .querySelectorAll(
                    '.fk-d-menu, .fk-d-tooltip, .toast-container, .toaster, ' +
                    '.modal-outer-container, .d-modal, .modal-overlay, ' +
                    '.tippy-box, .tippy-popper, [data-popper-placement], ' +
                    '[role="dialog"], [data-portal]'
                  )
          ...
          # Page torn down mid-script. The storage clear below catches the
          # same condition; on second failure Driver#reset! falls through
          # to the upstream `clear_browser_contexts` rebuild.
        end
