...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    ...
    # new BrowserContext **and** a new Page from scratch. That cost is paid
    # ~1,800 times per CI run (every system spec hits `reset!` via
    # `Capybara.reset_session!` in the `after(:each, type: :system)`
    # hook), and `Capybara::Playwright::Browser#initialize` shows up at
    # ~1.7% wall on stackprof for representative slices.
    #
    # The fix: keep the BrowserContext + Page alive between examples and
    # only wipe per-origin storage state (cookies, localStorage,
    # sessionStorage, IndexedDB, service workers, cache storage) — i.e.
    # exactly the state Capybara's session reset is responsible for.
    # `Storage.clearDataForOrigin` over CDP is the native API for this.
    #
    # Two changes vs the iter-2 attempt that hung CI at the 30-min step
    # cap (1681s vs ~600s baseline, +1081s):
    #
    # 1. **Use real per-page origins** rather than the `origin: "*"`
    #    wildcard. CDP's `Storage.clearDataForOrigin` documents `origin`
    #    as a security-origin string, not a glob; `"*"` may be treated
    #    as a literal in CI's Chromium and silently no-op (leaving SW +
    #    storage unchanged). Parsing `pg.url` per page gives an actual
    #    origin string that's guaranteed to match what the page used.
    #
    # 2. **Skip the `goto("about:blank", timeout: 5000)` step**. iter-2
    #    drove every page to about:blank to drop the previous origin's
    #    in-memory JS state (Ember instance, etc.), but in CI the
    #    navigation appears to have hung on every test (5s timeout x
    #    ~1,800 specs / 8 workers ≈ 1,125s extra wall, matching the
    #    observed +1,081s overage). The navigation isn't actually needed:
    #    every system spec calls `visit(...)` (or `sign_in(...)`, which
    #    visits) before interacting with the page, and that real
    #    navigation tears down the old Ember app and mounts the new one.
    #    Storage clearing is what handles the localStorage-leak failures
    #    iter-2 fixed (login_spec redirect tests) — the about:blank goto
    #    was belt-and-braces.
    #
    # Tracing (`:trace` metadata) still requires the full context
    # lifecycle to flush a trace zip, so that path falls back to the
    # original `clear_browser_contexts` behaviour.
      ...
      def soft_reset_browser_state
        ...
        @playwright_browser.contexts.each do |ctx|
          ctx.pages.each do |pg|
            ...
            next if url == "about:blank" || url.start_with?("chrome:", "data:")
            ...
            begin
              ...
              # Page or context already closed mid-reset; nothing to clear.
            end
          end
