...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    ...
    module CapybaraPlaywrightBrowserSoftResetPatch
      ...
      def soft_reset_browser_state
        ...
        begin
          ...
          # See `BrowserCacheTracker` defined just below the patches. The
          # tracker's `after_commit` hooks fire from the in-process Rails
          # server thread when a test mutates one of a tightly scoped set
          # of cache-affecting models or SiteSettings. Consuming and
          # clearing the browser HTTP cache here (vs. in every soft-reset)
          # keeps the perf win on the ~80%+ of system tests that don't
          # mutate that state while preventing stale-cache failures on
          # the rest.
          cdp.send_message("Network.clearBrowserCache") if BrowserCacheTracker.consume!
    ...
    # Automatic browser HTTP cache invalidation for the soft-reset path.
    # The soft-reset preserves the BrowserContext between examples; its
    # HTTP cache (keyed by URL) survives, so when an example mutates DB
    # state served under a URL whose digest doesn't change (e.g.,
    # `ColorScheme` palette colors served by the stylesheet endpoint, or
    # `TranslationOverride`s embedded in `index.html`) the NEXT example
    # may see stale content. The iter-20 patch enumerates known polluting
    # files and tags them `:hard_reset`; this tracker is the
    # complementary mechanism — it auto-detects mutations to a tightly
    # scoped set of cache-affecting models AND a regex-matched subset of
    # cache-affecting SiteSettings, and arranges for `soft_reset_browser_state`
    # to send a `Network.clearBrowserCache` CDP call on the way out of
    # such examples. Tests in the 28 `:hard_reset`-tagged files still go
    # through the upstream `clear_browser_contexts` path and don't pay
    # this cost; un-tagged tests that don't mutate cache-affecting state
    # pay zero (tracker stays clean); un-tagged tests that DO mutate pay
    # one extra CDP round-trip (~80 ms per past iter-8 measurement). Each
    # `after_commit` hook only registers a flag-set; the mutex makes
    # tracker state visible to the soft-reset path which may run in a
    # different thread than the controller that performed the save.
    module BrowserCacheTracker
      MUTEX = Mutex.new
      @dirty = false
      class << self
        def mark_dirty
    ...
    # Models whose persisted state directly affects the bytes of a
    # response served under a URL that doesn't include a content-derived
    # digest. Saves to ColorScheme/ColorSchemeColor change the bytes
    # served by the color-scheme stylesheet endpoint (URL keyed by
    # color_scheme_id, not content hash). Saves to Theme,
    # ThemeField, ThemeModifierSet, ThemeSetting, ThemeSiteSetting and
    # ThemeTranslationOverride change theme-rendered HTML and the
    # theme's CSS/JS bundles. Saves to TranslationOverride change MF
    # translations embedded in `index.html` and the
    # `/extra-locales/<digest>/<locale>/<bundle>` JS (ExtraLocalesController
    # invalidates its in-process digest cache on TranslationOverride save,
    # but the BROWSER may already have fetched the previous-digest URL).
    cache_affecting_models = [
      ColorScheme,
      ColorSchemeColor,
      Theme,
      ThemeField,
    ...
    cache_affecting_models.each { |klass| klass.after_commit { BrowserCacheTracker.mark_dirty } }

    # A subset of SiteSetting names that affect browser-rendered HTML/
    # JS/CSS — e.g., `default_locale` (HTML + JS bundles),
    # `default_theme_id`, `top_menu`/`navigation_menu` (header HTML),
    # `tagging_enabled` (HTML), `enable_*` toggles that gate UI surfaces,
    # `site_logo_*`, `site_favicon_*`, `site_name`, `site_description`,
    # `max_*` that bound visible widget counts/sizes,
    # `dashboard_*` / `admin_dashboard_*` (admin dashboard layout +
    # presence of the redesigned UI; missed leak class diagnosed from
    # `tmp/capybara/*.png` filenames after the iter-19/20 local runs).
    # SiteSetting changes outside this regex (e.g., email tuning, search
    # internals) don't affect any cached browser response so the tracker
    # stays clean and the soft-reset perf win is preserved.
    cache_affecting_setting_regex =
      /\A(default_|enable_|disable_|allow_|tagging|navigation_menu|top_menu|max_|site_name|site_description|logo|favicon|powered_by|welcome_banner|dashboard_|admin_dashboard_)/
    Module
      .new do
        define_method(:add_override!) do |name, *rest|
          BrowserCacheTracker.mark_dirty if cache_affecting_setting_regex.match?(name.to_s)
          super(name, *rest)
        end
      end
      .tap { |m| SiteSetting.singleton_class.prepend(m) }
