...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    ...
    # `clear_browser_contexts` (closes BrowserContext, nils @browser, forces
    # next test to rebuild Browser+Context+Page from scratch) with a
    # soft-reset that keeps the primary BrowserContext + primary Page
    # alive between examples and only wipes per-test state. Iter-6's
    # measurement of this mechanism: -22% CI step time (507s vs 656s
    # baseline). Two prior iterations of the same mechanism failed CI:
    #
    # - Iter-6 failed because the kept-alive page accumulates per-page
    #   state (CDP-network-throttle session leak from
    #   `cdp.with_network_disconnected`, multi-tab inventory from
    #   `open_new_window`, clipboard permission grants, browser HTTP
    #   cache) that breaks specific specs.
    #
    # - Iter-8 added the `Network.clearBrowserCache` global cleanup but
    #   that wipes the asset cache on every reset (~1,800 times per CI
    #   run), undoing the cache-warmth half of iter-6's gain — CI step
    #   regressed back to 643s.
    #
    # - Iter-9 added iter-8's auxiliary cleanup (clear_permissions,
    #   clear_cookies, close auxiliary tabs/contexts), moved the
    #   color-palette cache bust into that single spec's after-hook, and
    #   measured 517s in CI (-22% baseline / -10% best). But CI still
    #   reported test failures — likely due to stale per-page JS state
    #   (e.g. MessageBus subscription state holding over a long-poll
    #   connection bound to the prior example's user session).
    #
    # This iteration adds one cleanup primitive iter-9 was missing: a
    # CDP `Page.navigate` to about:blank with `waitUntil: "commit"` and
    # a 1500ms timeout. `commit` waits only for the URL change, not for
    # any load event, so service-worker / network races (iter-2's
    # failure mode) can't stall the reset. The about:blank navigation
    # tears down the prior example's JS context (Ember app, MessageBus
    # client, EventTarget listeners), so the next example's `visit`
    # bootstraps from a genuinely-clean page. The BrowserContext stays
    # alive across the navigation — DNS, TLS connection pool, and the
    # browser HTTP response cache remain warm.
    #
    # Per-reset steps:
    #   1. Storage.clearDataForOrigin (storageTypes: "all") for each
    #      real origin pages in the primary context were last on.
    #      Wipes cookies, localStorage, sessionStorage, IndexedDB,
    #      service workers, cache_storage on a per-origin basis.
    #   2. Close every auxiliary BrowserContext (from
    #      `open_new_window(:window)`); typically zero per test.
    #   3. Close every auxiliary page in the primary context (from
    #      `open_new_window(:tab)`), keep the primary page alive.
    #   4. Clear context-level permissions and remaining cookies on the
    #      primary context.
    #   5. CDP `Page.navigate` the primary page to about:blank with
    #      `waitUntil: "commit"` and a 1500ms timeout — strictly bounded
    #      so a runaway navigation can't cascade. Detaches per-page CDP
    #      sessions and drops the prior example's JS state.
    #
    ...
    module CapybaraPlaywrightBrowserSoftResetPatch
      ...
      ABOUT_BLANK_NAVIGATE_TIMEOUT_MS = 1500
      ...
      def soft_reset_browser_state
        ...
        # opened by `open_new_window(:tab)`). Keep the primary page
        # alive — that's the dominant perf gain — but drop extra tabs so
        ...
        # Navigate the primary page to about:blank with a bounded
        # timeout and waitUntil: "commit" — return as soon as the URL
        # is committed, don't wait for any load event. Iter-2's failure
        # mode was waiting for the load event to fire when service
        # workers from the previous origin intercepted the navigation;
        # we already cleared service workers above (storageTypes: "all"
        # covers them) and "commit" doesn't depend on the load event
        # firing at all, so the iter-2 cascade can't happen here.
        # The navigation drops the prior example's JS state (Ember app,
        # MessageBus subscriptions, in-flight long-polls) and detaches
        # any per-page CDP sessions opened by helpers like
        # `cdp.with_network_disconnected`, addressing iter-9's likely
        # CI failure mode (stale MessageBus client carried into the
        # next example).
        begin
          primary.goto("about:blank", waitUntil: "commit", timeout: ABOUT_BLANK_NAVIGATE_TIMEOUT_MS)
        rescue ::Playwright::Error, ::Playwright::TimeoutError
          # Bounded by ABOUT_BLANK_NAVIGATE_TIMEOUT_MS — at worst this
          # one example pays that latency. The next visit() proceeds
          # anyway and replaces whatever state we left behind.
        end
      end
    end
