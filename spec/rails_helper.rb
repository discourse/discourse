...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    ...
    # wipes per-test state.
    #
    # The kept-alive page mechanism alone (iter-6 / iter-9 / iter-12 in
    # the autoresearch loop) reliably measures -10..-26% CI step time vs
    # baseline, but every CI run in that family hit unidentified test
    # failures that did not reproduce in 100-example local validation.
    # The most defensible "accumulation bound" hypothesis is that some
    # subset of failures is driven by per-page state that grows over the
    # ~225 examples each worker handles. To bound that, this revision
    # forces a periodic hard reset (the upstream
    # `clear_browser_contexts` path that nils `@browser`) every
    # `SOFT_RESET_LIMIT` examples per Driver instance. Each hard reset
    # costs ~250-700 ms (close ctx + lazy-recreate ctx+page on next
    # test); at limit=20 that's ~10 hard resets per worker × ~500 ms ≈
    # 5 s of added wall, vs 130-170 s saved by the soft-reset on the
    # other 215 examples. If the failure mode is accumulation, the
    # bound caps it. If it's a specific cross-test interaction, this
    # patch performs identically to iter-12 and produces a clean
    # negative signal for that hypothesis.
    #
    # Per-reset cleanup steps (unchanged from iter-12):
    ...
    #
    # Tracing (`:trace` metadata) needs the full BrowserContext
    # lifecycle to flush a trace zip, so that path falls through to the
    # upstream `clear_browser_contexts` reset. Any unexpected failure
    # in the soft-reset path also falls through, so a single broken
    # reset costs at most one example's full reset rather than
    # cascading.
    module CapybaraPlaywrightDriverSoftResetPatch
      ...
      def reset!
        ...
        if @browser && @soft_reset_count < SOFT_RESET_LIMIT
          begin
