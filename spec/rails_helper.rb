...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    ...
    # at -22% CI step time (507s vs 656s baseline). Two prior iterations
    # of this mechanism failed CI because of incomplete per-test cleanup;
    # this revision lands the cleanup that's known to be cheap and
    # avoids the one cleanup primitive (`Network.clearBrowserCache`)
    # that wipes the browser asset cache and erased iter-6's perf gain
    # in iter-8 (CI step jumped to 643s). Cache-bust for the one spec
    # that genuinely needs a fresh stylesheet response across examples
    # (admin_color_palette_config_area) is moved into that file's own
    # after-hook, so the rest of the suite keeps its warm asset cache.
    #
