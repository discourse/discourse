...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    ...
    # `max_*` that bound visible widget counts/sizes. SiteSetting
    # changes outside this regex (e.g., email tuning, search internals)
    # don't affect any cached browser response so the tracker stays
    # clean and the soft-reset perf win is preserved.
    cache_affecting_setting_regex =
      /\A(default_|enable_|disable_|allow_|tagging|navigation_menu|top_menu|max_|site_name|site_description|logo|favicon|powered_by|welcome_banner)/
    Module
      .new do
