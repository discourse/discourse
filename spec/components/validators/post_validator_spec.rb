...
describe Validators::PostValidator do
  ...
  context 'force_edit_last_validator' do
    ...
    it 'should always allow original poster to post' do
      [user, user, user, other_user, user, user, user]
    ...
    it 'should not allow posting more than 2 consecutive replies' do
      Post.create!(
        user: user, topic: topic, raw: 'post number 2', post_number: 2
      )
      Post.create!(
        user: user, topic: topic, raw: 'post number 3', post_number: 3
      )
      Post.create!(
        user: other_user, topic: topic, raw: 'post number 1', post_number: 1
      )

      post =
        Post.new(user: user, topic: topic, raw: 'post number 4', post_number: 4)
      validator.force_edit_last_validator(post)
      expect(post.errors.count).to eq(1)
    end
