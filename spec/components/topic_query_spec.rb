...
describe TopicQuery do
  ...
  context 'custom filters' do
    it 'allows custom filters to be applied' do
      ...
      TopicQuery.add_custom_filter(:only_topic_id) do |results, topic_query|
        results =
          results.where('topics.id = ?', topic_query.options[:only_topic_id])
      end

      expect(
        TopicQuery.new(nil, only_topic_id: topic1.id).list_latest.topics.map(
          &:id
        )
      ).to eq([topic1.id])
  ...
  context 'prioritize_pinned_topics' do
    it 'does the pagination correctly' do
      ...
      topics = []
      (num_topics - 1).downto(0).each { |i| topics[i] = Fabricate(:topic) }
  ...
  context 'bookmarks' do
    it 'filters and returns bookmarks correctly' do
      ...
      expect(query.topics.first.user_data.post_action_data).to eq(
            PostActionType.types[:bookmark] => [1, 2]
          )
    end
  end
  ...
  context 'deleted filter' do
    it 'filters deleted topics correctly' do
      ...
      expect(
        TopicQuery.new(admin, status: 'deleted').list_latest.topics.size
      ).to eq(1)
      expect(
        TopicQuery.new(moderator, status: 'deleted').list_latest.topics.size
      ).to eq(1)
      expect(
        TopicQuery.new(user, status: 'deleted').list_latest.topics.size
      ).to eq(0)
      expect(
        TopicQuery.new(nil, status: 'deleted').list_latest.topics.size
      ).to eq(0)
    end
  end
  ...
  context 'category filter' do
    ...
    it 'returns topics in the category when we filter to it' do
      ...
      # Filter by slug
      expect(
        TopicQuery.new(moderator, category: category.slug).list_latest.topics
          .size
      ).to eq(1)
      expect(
        TopicQuery.new(moderator, category: "#{category.id}-category")
          .list_latest
          .topics
          .size
      ).to eq(1)
      ...
      expect(
        TopicQuery.new(moderator, category: 'made up slug').list_latest.topics
          .size
      ).to eq(2)
    end
    ...
    context 'subcategories' do
      let!(:subcategory) do
        Fabricate(:category, parent_category_id: category.id)
      end
      ...
      it 'works with subcategories' do
        ...
        expect(
          TopicQuery.new(
            moderator,
            category: category.id, no_subcategories: true
          )
            .list_latest
            .topics
            .size
  ...
  context 'tag filter' do
    ...
    context 'no category filter' do
      ...
      it 'returns topics with the tag when filtered to it' do
        expect(
          TopicQuery.new(moderator, tags: tag.name).list_latest.topics
        ).to contain_exactly(tagged_topic1, tagged_topic3)

        expect(
          TopicQuery.new(moderator, tags: [tag.id]).list_latest.topics
        ).to contain_exactly(tagged_topic1, tagged_topic3)

        expect(
          TopicQuery.new(moderator, tags: [tag.name, other_tag.name])
            .list_latest
            .topics
        ).to contain_exactly(tagged_topic1, tagged_topic2, tagged_topic3)

        expect(
          TopicQuery.new(moderator, tags: [tag.id, other_tag.id]).list_latest
            .topics
        ).to contain_exactly(tagged_topic1, tagged_topic2, tagged_topic3)

        expect(
          TopicQuery.new(moderator, tags: %w[hElLo]).list_latest.topics
      ...
      it 'can return topics with all specified tags' do
        expect(
          TopicQuery.new(
            moderator,
            tags: [tag.name, other_tag.name], match_all_tags: true
          )
            .list_latest
            .topics
            .map(&:id)
      ...
      it 'returns an empty relation when an invalid tag is passed' do
        expect(
          TopicQuery.new(
            moderator,
            tags: [tag.name, 'notatag'], match_all_tags: true
          )
            .list_latest
            .topics
        ).to be_empty
      ...
      it 'can return topics with no tags' do
        expect(
          TopicQuery.new(moderator, no_tags: true).list_latest.topics.map(&:id)
        ).to eq([no_tags_topic.id])
      end
    end
    ...
    context 'and categories too' do
      ...
      it 'returns topics in the given category with the given tag' do
        ...
        tagged_topic3 =
          Fabricate(:topic, category: category1, tags: [tag, other_tag])
        _no_tags_topic = Fabricate(:topic, category: category1)

        expect(
          TopicQuery.new(moderator, category: category1.id, tags: [tag.name])
            .list_latest
            .topics
            .map(&:id)
            .sort
        ...
        expect(
          TopicQuery.new(
            moderator,
            category: category2.id, tags: [other_tag.name]
          )
            .list_latest
            .topics
            .size
  ...
  context 'a bunch of topics' do
    ...
    context 'list_latest' do
      it 'returns the topics in the correct order' do
        ...
        expect(
          TopicQuery.new(moderator).list_latest.topics.include?(invisible_topic)
        ).to eq(true)

        # includes the invisible topic if you're an admin" do
        expect(
          TopicQuery.new(admin).list_latest.topics.include?(invisible_topic)
        ).to eq(true)
      end
      ...
      context 'sort_order' do
        def ids_in_order(order, descending = true)
          TopicQuery.new(
            admin,
            order: order, ascending: descending ? 'false' : 'true'
          )
            .list_latest
            .topics
            .map(&:id)
  ...
  context 'categorized' do
    ...
    describe 'category default sort order' do
      it "can use category's default sort order" do
        ...
        topic_ids =
          TopicQuery.new(user, category: category.id).list_latest.topics.map(
            &:id
          )
        expect(topic_ids - [topic_category.id]).to eq(
              [topic_in_cat1.id, topic_in_cat2.id]
            )
      end
      ...
      it 'ignores invalid order value' do
        ...
        expect(topic_ids - [topic_category.id]).to eq(
              [topic_in_cat2.id, topic_in_cat1.id]
            )
      end
      ...
      it 'can be overridden' do
        category.update_attributes!(sort_order: 'created', sort_ascending: true)
        topic_ids =
          TopicQuery.new(user, category: category.id, order: 'activity')
            .list_latest
            .topics
            .map(&:id)
        expect(topic_ids - [topic_category.id]).to eq(
  ...
  context 'unread / read topics' do
    ...
    context 'with whispers' do
      it 'correctly shows up in unread for staff' do
        first =
          create_post(
            raw: 'this is the first post', title: 'super amazing title'
          )
    ...
    context 'with read data' do
      ...
      context 'list_unread' do
        it 'lists topics correctly' do
          ...
          expect(topic_query.list_read.topics).to match_array(
                [fully_read, partially_read]
              )
        end
      end
  ...
  context 'list_new' do
    ...
    context 'preload api' do
      ...
      it 'preloads data correctly' do
        ...
        topic =
          Fabricate.build(:topic, user: creator, bumped_at: 10.minutes.ago)
    ...
    context 'with a new topic' do
      let!(:new_topic) do
        Fabricate(:topic, user: creator, bumped_at: 10.minutes.ago)
      end
  ...
  context 'list_related_for do' do
    ...
    def create_pm(user, opts = nil)
      ...
      create_post(opts.merge(user: user, archetype: Archetype.private_message))
        .topic
    end
    ...
    it 'returns the correct suggestions' do
      pm_to_group =
        create_pm(sender, target_group_names: [group_with_user.name])
      ...
      expect(
        TopicQuery.new(user).list_related_for(pm_to_group).topics.map(&:id)
      )
        .to(eq([related_by_group_pm.id]))

      expect(TopicQuery.new(user).list_related_for(pm_to_user).topics.map(&:id))
        .to(eq([related_by_user_pm.id]))
  ...
  context 'suggested_for' do
    ...
    context 'when anonymous' do
      ...
      it 'should return the new topic' do
        expect(TopicQuery.new.list_suggested_for(topic).topics).to eq(
              [new_topic]
            )
      end
    end
    ...
    context 'when logged in' do
      ...
      let(:suggested_topics) do
        ...
        clear_cache!
        topic_query.list_suggested_for(tt).topics.map(&:id)
      end
      ...
      context 'with private messages' do
        ...
        describe 'as user part of group' do
          ...
          it 'should return the group topics' do
            expect(suggested_topics).to eq(
                  [private_group_topic.id, private_message.id]
                )
          end
        end
        ...
        context 'by tag filter' do
          ...
          it 'should return only tagged topics' do
            ...
            expect(
              TopicQuery.new(user, tags: [tag.name]).list_private_messages_tag(
                user
              )
                .topics
  ...
  describe '#list_group_topics' do
    ...
    let!(:topic2) do
      Fabricate(:topic, user: user, category: Fabricate(:category))
    end
  ...
  describe '#list_private_messages_group' do
    ...
    before { group.add(creator) }

    it 'should return the right list for a group user' do
      topics =
        TopicQuery.new(nil, group_name: group.name).list_private_messages_group(
          creator
        )
          .topics
  ...
  context 'shared drafts' do
    ...
    context 'destination_category_id' do
      it "doesn't allow regular users to query destination_category_id" do
        list =
          TopicQuery.new(user, destination_category_id: category.id).list_latest
        expect(list.topics).not_to include(topic)
      end

      it 'allows staff users to query destination_category_id' do
        list =
          TopicQuery.new(admin, destination_category_id: category.id)
            .list_latest
    ...
    context 'unread' do
      ...
      it 'does not remove topics from unread' do
        expect(TopicQuery.new(admin).list_latest.topics).not_to include(
                  partially_read
                ) # Check we set up the topic/category correctly
        expect(TopicQuery.new(admin).list_unread.topics).to include(
              partially_read
            )
      end
    end
