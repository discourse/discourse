...
describe "S3Inventory" do
  ...
  describe "verifying uploads" do
    ...
    it "marks missing uploads as not verified and found uploads as verified. uploads not checked will be verified nil" do
      expect(Upload.where(verification_status: Upload.verification_statuses[:not_checked]).count).to eq(12)
      output = capture_stdout do
        inventory.backfill_etags_and_list_missing
      end
      ...
      expect(Upload.where(verification_status: Upload.verification_statuses[:not_verified]).count).to eq(2)
      expect(Upload.where(verification_status: Upload.verification_statuses[:not_checked]).count).to eq(7)
    end
