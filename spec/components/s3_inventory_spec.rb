...
describe 'S3Inventory' do
  ...
  it 'should display missing uploads correctly' do
    ...
    CSV.foreach(csv_filename, headers: false) do |row|
      Fabricate(
        :upload,
        etag: row[S3Inventory::CSV_ETAG_INDEX], created_at: 2.days.ago
      )
    end
    ...
    inventory.expects(:files).returns(
      [{ key: 'Key', filename: "#{csv_filename}.gz" }]
    )
      .times(2)
  ...
  it 'should backfill etags to uploads table correctly' do
    files = [
      %w[
        //bucket.amazonaws.com/original/0184537a4f419224404d013414e913a4f56018f2.jpg
        defcaac0b4aca535c284e95f30d608d0
      ],
      %w[
        //bucket.amazonaws.com/original/0789fbf5490babc68326b9cec90eeb0d6590db05.png
    ...
    output =
      capture_stdout do
        expect { inventory.backfill_etags_and_list_missing }.to change do
