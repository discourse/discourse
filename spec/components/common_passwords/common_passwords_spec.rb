...
describe CommonPasswords do
  ...
  describe '#common_password?' do
    ...
    it "returns false if password isn't in the common passwords list" do
      described_class.stubs(:password_list).returns(
        stub_everything(include?: false)
      )
  ...
  describe '#password_list' do
    it "loads the passwords file if redis doesn't have it" do
      ...
      described_class.expects(:load_passwords).returns(%w[password])
  ...
  context 'missing password file' do
    it 'tolerates it' do
      described_class.stubs(:redis).returns(
        stub_everything(sismember: false, exists: false, scard: 0)
      )
      File.stubs(:readlines).with(described_class::PASSWORD_FILE).raises(
        Errno::ENOENT
      )
