...
describe SiteSettings::YamlLoader do
  class Receiver
    ...
    def load_yaml(file_arg)
      SiteSettings::YamlLoader.new(file_arg)
  ...
  let(:enum_client) do
    "#{Rails.root}/spec/fixtures/site_settings/enum_client.yml"
  end
  let(:deprecated_env) do
    "#{Rails.root}/spec/fixtures/site_settings/deprecated_env.yml"
  end
  let(:deprecated_hidden) do
    "#{Rails.root}/spec/fixtures/site_settings/deprecated_hidden.yml"
  end
  let(:locale_default) do
    "#{Rails.root}/spec/fixtures/site_settings/locale_default.yml"
  end
  let(:nil_default) do
    "#{Rails.root}/spec/fixtures/site_settings/nil_default.yml"
  end
  ...
  it 'can load enum client settings' do
    receiver.expects(:setting).with do |category, name, default, opts|
      category == ('basics') && name == ('default_locale') &&
        default == ('en') &&
        opts[:enum] == ('LocaleSiteSetting') &&
        opts[:client] == true
    end
  ...
  it 'raises deprecation when load settings based on environment' do
    expect { receiver.load_yaml(deprecated_env) }.to raise_error(
          Discourse::Deprecation
        )
  end

  it 'raises deprecation when hidden property is based on environment' do
    expect { receiver.load_yaml(deprecated_hidden) }.to raise_error(
          Discourse::Deprecation
        )
  end
