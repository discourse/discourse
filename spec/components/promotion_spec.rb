...
describe Promotion do
  ...
  context "a newuser with trust level set to basic by an admin" do

    let(:user) { Fabricate(:user, trust_level_set_by_admin: true, trust_level: TrustLevel.levels[:basic]) }
    let(:promotion) { Promotion.new(user) }
    ...
    context "that does the differential of the requisite things" do
      before do
        user.topics_entered = SiteSetting.regular_requires_topics_entered - SiteSetting.basic_requires_topics_entered
        user.posts_read_count = SiteSetting.regular_requires_read_posts - SiteSetting.basic_requires_read_posts
        user.time_read = (SiteSetting.regular_requires_time_spent_mins - SiteSetting.basic_requires_time_spent_mins) * 60
        user.days_visited = SiteSetting.regular_requires_days_visited * 60
        user.likes_received = SiteSetting.regular_requires_likes_received
        user.likes_given = SiteSetting.regular_requires_likes_given
        user.topic_reply_count = SiteSetting.regular_requires_topic_reply_count
