...
describe Middleware::AnonymousCache::Helper do
  ...
  context 'cachable?' do
    ...
    it 'is false for non GET' do
      expect(
        new_helper('ANON_CACHE_DURATION' => 10, 'REQUEST_METHOD' => 'POST')
          .cacheable?
      ).to eq(false)
    end
    ...
    it 'is false if it has an auth cookie' do
      expect(
        new_helper('HTTP_COOKIE' => "jack=1; _t=#{'1' * 32}; jill=2").cacheable?
      ).to eq(false)
    end
  end
  ...
  context 'per theme cache' do
    it 'handles theme keys' do
      ...
      with_bad_theme_key =
        new_helper('HTTP_COOKIE' => 'theme_ids=abc').cache_key
      with_no_theme_key = new_helper.cache_key

      expect(with_bad_theme_key).to eq(with_no_theme_key)

      with_good_theme_key =
        new_helper('HTTP_COOKIE' => "theme_ids=#{theme.id}").cache_key
  ...
  context 'cached' do
    ...
    it 'handles brotli switching' do
      ...
      expect(helper.cached).to eq(
            [
              200,
              { 'X-Discourse-Cached' => 'true', 'HELLO' => 'WORLD' },
              ['hello my world']
            ]
          )
      ...
      helper =
        new_helper(
          'ANON_CACHE_DURATION' => 10, 'HTTP_ACCEPT_ENCODING' => 'gz, br'
        )
  ...
  context 'crawler blocking' do
    ...
    def get(path, options)
      ...
      @env =
        env(
          { 'REQUEST_URI' => path, 'PATH_INFO' => path, 'REQUEST_PATH' => path }
    ...
    it 'applies whitelisted_crawler_user_agents correctly' do
      ...
      get '/',
          headers: {
            'HTTP_USER_AGENT' =>
              'Anotherbot/2.1 (+http://www.notgoogle.com/bot.html)'
          }
    ...
    it 'applies blacklisted_crawler_user_agents correctly' do
      ...
      get '/',
          headers: {
            'HTTP_USER_AGENT' =>
              'Twitterbot/2.1 (+http://www.notgoogle.com/bot.html)'
          }
