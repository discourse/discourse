  ...
  context 'revise wiki' do
    ...
    it 'allows the user to change it to a wiki' do
      pc =
        PostCreator.new(
          newuser,
          topic_id: topic.id, raw: 'this is a post that will become a wiki'
        )
      post = pc.create
  ...
  context 'revise' do
    ...
    describe 'ninja editing' do
      ...
      it 'does create a new version if a large diff happens' do
        ...
        expect(post.revisions.first.modifications['raw'][0]).to eq(
              'hello world'
            )
        expect(post.revisions.first.modifications['cooked'][0]).to eq(
              '<p>hello world</p>'
            )
      ...
      it "doesn't create a new version" do
        ...
        subject.revise!(
          ...
          { raw: 'updated body' },
          revised_at:
            post.updated_at + SiteSetting.editing_grace_period + 1.seconds
        )
        # "roll back"
        subject.revise!(
          post.user,
          { raw: 'Hello world' },
          revised_at:
            post.updated_at + SiteSetting.editing_grace_period + 2.seconds
        )
    ...
    describe 'with a new body' do
      ...
      let(:changed_by) { Fabricate(:coding_horror) }
      let!(:result) do
        subject.revise!(changed_by, raw: 'lets update the body. Здравствуйте')
      end
      ...
      context 'passing skip_revision as true' do
        before do
          ...
          subject.revise!(
            changed_by,
            { raw: 'yet another updated body' },
            revised_at: post.updated_at + 10.hours, skip_revision: true
          )
          post.reload
        end
    ...
    describe 'topic excerpt' do
      it 'topic excerpt is updated only if first post is revised' do
        ...
        expect do
          ...
          topic.reload
        end.to change { topic.excerpt }
        second_post =
          Fabricate(:post, post_args.merge(post_number: 2, topic_id: topic.id))
        expect do
          PostRevisor.new(second_post).revise!(
            second_post.user,
            raw: 'Edit the 2nd post'
          )
          topic.reload
    ...
    context 'staff_edit_locks_post' do
      ...
      context 'enabled' do
        ...
        it "doesn't lock a staff member's post" do
          ...
          result =
            revisor.revise!(moderator, raw: 'lets totally update the body')
          expect(result).to eq(true)
          staff_post.reload
    ...
    context 'alerts' do
      ...
      it 'generates a notification for a mention' do
        expect do
          ...
          Notification.where(notification_type: Notification.types[:mentioned])
            .count
        end
      end
      ...
      context 'tagging disabled' do
        ...
        it "doesn't add the tags" do
          result =
            subject.revise!(
              Fabricate(:user),
              raw: 'lets totally update the body', tags: %w[totally update]
            )
          expect(result).to eq(true)
          post.reload
      ...
      context 'tagging enabled' do
        ...
        context 'can create tags' do
          ...
          it 'can remove all tags' do
            ...
            result =
              subject.revise!(
                Fabricate(:user),
                raw: 'lets totally update the body', tags: []
              )
            expect(result).to eq(true)
            post.reload
            expect(post.topic.tags.size).to eq(0)
          ...
          it 'can remove all tags using tags_empty_array' do
            ...
            result =
              subject.revise!(
                Fabricate(:user),
                raw: 'lets totally update the body', tags_empty_array: 'true'
              )
            expect(result).to eq(true)
            post.reload
            expect(post.topic.tags.size).to eq(0)
          ...
          it "can't add staff-only tags" do
            ...
            result =
              subject.revise!(
                Fabricate(:user),
                raw: 'lets totally update the body', tags: %w[important stuff]
              )
          ...
          context 'with staff-only tags' do
            ...
            it "staff-only tags can't be removed" do
              result =
                subject.revise!(
                  Fabricate(:user),
                  raw: 'lets totally update the body', tags: %w[stuff]
                )
              expect(result).to eq(false)
              expect(post.topic.errors.present?).to eq(true)
              post.reload
              expect(post.topic.tags.map(&:name).sort).to eq(
                    %w[important stuff super]
                  )
            end
            ...
            it 'staff-only tags can be removed by staff' do
              ...
              expect(result).to eq(true)
              post.reload
              expect(post.topic.tags.map(&:name)).to eq(%w[stuff])
            end
        ...
        context 'cannot create tags' do
          ...
          it 'only uses existing tags' do
            ...
            expect(@result).to eq(true)
            post.reload
            expect(post.topic.tags.map(&:name)).to eq(%w[totally])
          end
        end
      end
    end
