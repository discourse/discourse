...
describe CategoryHashtag do
  describe '#query_from_hashtag_slug' do
    ...
    let(:child_category) do
      Fabricate(:category, parent_category: parent_category)
    end

    it 'should return the right result for a parent category slug' do
      expect(Category.query_from_hashtag_slug(parent_category.slug)).to eq(
            parent_category
          )
    end
    ...
    it 'should return the right result for a parent and child category slug' do
      expect(
        Category.query_from_hashtag_slug(
          "#{parent_category.slug}#{CategoryHashtag::SEPARATOR}#{child_category
            .slug}"
        )
    ...
    it 'should return nil for incorrect parent and child category slug' do
      expect(
        Category.query_from_hashtag_slug(
          "random-slug#{CategoryHashtag::SEPARATOR}random-slug"
        )
      ).to eq(nil)
    end
    ...
    it 'should be case sensitive' do
      ...
      expect(
        Category.query_from_hashtag_slug(
          "apple#{CategoryHashtag::SEPARATOR}orange"
        )
      ).to eq(nil)
    end
  end
