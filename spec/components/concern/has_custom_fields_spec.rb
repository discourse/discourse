...
describe HasCustomFields do
  context 'custom_fields' do
    before do
      DB.exec(
        'create temporary table custom_fields_test_items(id SERIAL primary key)'
      )
      DB.exec(
        'create temporary table custom_fields_test_item_custom_fields(id SERIAL primary key, custom_fields_test_item_id int, name varchar(256) not null, value text)'
      )
    ...
    it 'handles arrays properly' do
      CustomFieldsTestItem.register_custom_field_type 'array', %i[integer]
      test_item = CustomFieldsTestItem.new
      test_item.custom_fields = { 'array' => %w[1] }
      test_item.save
      ...
      db_item.custom_fields.update('a' => %w[c])
      db_item.save
    ...
    it 'supportes type coersion' do
      ...
      test_item.custom_fields = {
        'bool' => true, 'int' => 1, 'json' => { 'foo' => 'bar' }
      }
      test_item.save
      test_item.reload
      ...
      before_ids =
        CustomFieldsTestItemCustomField.where(
          custom_fields_test_item_id: test_item.id
        )
          .pluck(:id)
    ...
    it "simple modifications don't interfere" do
      ...
      expect(test_item.custom_fields).to eq(
            'jack' => 'black', 'bob' => 'marley'
          )
      expect(test_item2.custom_fields).to eq(
            'sixto' => 'rodriguez', 'de' => 'la playa'
          )
    end
    ...
    it 'will not fail to load custom fields if json is corrupt' do
      ...
      CustomFieldsTestItemCustomField.create!(
        custom_fields_test_item_id: item.id, name: field_type, value: '{test'
      )
    ...
    it 'supports bulk retrieval with a list of ids' do
      ...
      item1.custom_fields = { 'a' => %w[b c d], 'not_whitelisted' => 'secret' }
      item1.save
      ...
      fields =
        CustomFieldsTestItem.custom_fields_for_ids(
          [item1.id, item2.id],
          %w[a e]
        )
