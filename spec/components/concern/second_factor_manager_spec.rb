...
RSpec.describe SecondFactorManager do
  ...
  context 'backup codes' do
    describe '#create_backup_codes' do
      it 'should create the right record' do
        second_factor = user.create_backup_codes(enabled: true)

        expect(user.user_second_factors.backup_codes.method).to eq(UserSecondFactor.methods[:backup_codes])
        expect(user.user_second_factors.backup_codes.data).to be_present
        expect(user.user_second_factors.backup_codes.enabled).to eq(true)
      end

      describe 'when user has a second factor' do
        it 'should return nil' do
          expect(user_backup.create_backup_codes).to eq(nil)
        end
      end
    end
    ...
    describe '#authenticate_backup_code' do
      it 'should be able to authenticate a backup code' do
        freeze_time do
          expect(user1.user_second_factors.backup_codes.last_used).to eq(nil)

          code = "ffgsdgf"
          # binding.pry
          # expect(user1.authenticate_backup_code(code)).to eq(true)
          expect(user1.user_second_factors.backup_codes.last_used).to eq(DateTime.now)
          expect(user1.authenticate_backup_code(code)).to eq(false)
        end
      end

      describe 'when backup code is blank' do
        it 'should be false' do
          expect(user1.authenticate_backup_code(nil)).to eq(false)
          expect(user1.user_second_factors.backup_codes.last_used).to eq(nil)
        end
      end

      describe 'when backup code is invalid' do
        it 'should be false' do
          expect(user1.authenticate_backup_code('111111')).to eq(false)
          expect(user1.user_second_factors.backup_codes.last_used).to eq(nil)
        end
      end
    end
