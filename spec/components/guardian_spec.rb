...
describe Guardian do
  ...
  describe 'can_post_link?' do
    ...
    describe 'whitelisted host' do
      before { SiteSetting.whitelisted_link_domains = host }
      ...
      it 'allows a new user to post the link to the host' do
        ...
        expect(Guardian.new(user).can_post_link?(host: host)).to eq(true)
        expect(
          Guardian.new(user).can_post_link?(host: 'another-host.com')
        ).to eq(false)
      end
    end
  end
  ...
  describe '#post_can_act?' do
    ...
    it "returns false when you've already done it" do
      expect(
        Guardian.new(user).post_can_act?(
          post,
          :like,
          opts: { taken_actions: { PostActionType.types[:like] => 1 } }
        )
      ).to be_falsey
    end
      ...
      PostActionType.notify_flag_types.each do |type, _id|
        expect(
          Guardian.new(user).post_can_act?(
            post,
            :off_topic,
            opts: { taken_actions: { PostActionType.types[type] => 1 } }
          )
        ).to be_falsey
      end
    end
    ...
    describe 'trust levels' do
      ...
      it 'returns false for a new user flagging with notify_user' do
        user.trust_level = TrustLevel[0]
        expect(
          Guardian.new(user).post_can_act?(post, :notify_user)
        ).to be_falsey # because new users can't send private messages
      end
    end
  end
  ...
  describe 'can_send_private_message' do
    ...
    it 'returns false when you are untrusted' do
      user.trust_level = TrustLevel[0]
      expect(
        Guardian.new(user).can_send_private_message?(another_user)
      ).to be_falsey
    end
    ...
    context 'enable_personal_messages is false' do
      ...
      it 'returns false if user is not staff member' do
        expect(
          Guardian.new(trust_level_4).can_send_private_message?(another_user)
        ).to be_falsey
      end

      it 'returns true for staff member' do
        expect(
          Guardian.new(moderator).can_send_private_message?(another_user)
        ).to be_truthy
        expect(
          Guardian.new(admin).can_send_private_message?(another_user)
        ).to be_truthy
      end
    end
    ...
    context 'target user is suspended' do
      it 'returns true for staff' do
        ...
        expect(
          Guardian.new(moderator).can_send_private_message?(suspended_user)
        ).to be_truthy
      end
    ...
    context 'author is silenced' do
      ...
      it 'returns true if target is staff' do
        ...
        expect(
          Guardian.new(user).can_send_private_message?(moderator)
        ).to be_truthy
      end
    ...
    it "respects the group's messageable_level" do
      ...
      Group::ALIAS_LEVELS.each do |level, _|
        ...
        expect(Guardian.new(user).can_send_private_message?(group)).to eq(
              output
            )
      end
      ...
      context 'for a staff user' do
        it 'should return true' do
          [admin, moderator].each do |staff_user|
            expect(
              Guardian.new(staff_user).can_send_private_message?(another_user)
            ).to eq(true)
          end
        end
      end
  ...
  describe 'can_reply_as_new_topic' do
    ...
    it 'returns true for a private message' do
      expect(
        Guardian.new(user).can_reply_as_new_topic?(private_message)
      ).to be_truthy
    end
  end
  ...
  describe 'can_see_post_actors?' do
    ...
    it 'displays visibility correctly' do
      ...
      expect(
        guardian.can_see_post_actors?(nil, PostActionType.types[:like])
      ).to be_falsey
      expect(
        guardian.can_see_post_actors?(topic, PostActionType.types[:like])
      ).to be_truthy
      expect(
        guardian.can_see_post_actors?(topic, PostActionType.types[:bookmark])
      ).to be_falsey
      expect(
        guardian.can_see_post_actors?(topic, PostActionType.types[:off_topic])
      ).to be_falsey
      expect(
        guardian.can_see_post_actors?(topic, PostActionType.types[:spam])
      ).to be_falsey
      expect(
        guardian.can_see_post_actors?(topic, PostActionType.types[:notify_user])
      ).to be_falsey

      expect(
        Guardian.new(moderator).can_see_post_actors?(
          topic,
  ...
  describe 'can_invite_to_forum?' do
    ...
    context 'with groups' do
      ...
      it 'returns false when user is not allowed to edit a group' do
        ...
        expect(
          Guardian.new(Fabricate(:admin)).can_invite_to_forum?(groups)
        ).to eq(true)
      end
  ...
  describe 'can_invite_to?' do
    describe 'regular topics' do
      ...
      let(:group_owner) do
        group_private_topic.user.tap { |u| group.add_owner(u) }
      end
      ...
      it 'returns true for a group owner' do
        expect(
          Guardian.new(group_owner).can_invite_to?(group_private_topic)
        ).to be_truthy
      end
      ...
      describe 'for a private category for automatic groups' do
        ...
        let(:group_owner) do
          Fabricate(:user).tap { |user| group.add_owner(user) }
        end
        let(:topic) { Fabricate(:topic, category: category) }
  ...
  describe 'can_invite_via_email?' do
    it 'returns true for all (tl2 and above) users when sso is disabled, local logins are enabled, user approval is not required' do
      expect(
        Guardian.new(trust_level_2).can_invite_via_email?(topic)
      ).to be_truthy
  ...
  describe 'can_see?' do
    ...
    describe 'a Category' do
      ...
      it 'correctly handles secure categories' do
        ...
        secure_category =
          build(:category, read_restricted: true, email_in: 'foo@bar.com')
        expect(Guardian.new(normal_user).can_see?(secure_category)).to be_falsey
        expect(Guardian.new(staged_user).can_see?(secure_category)).to be_falsey
        expect(Guardian.new(admin_user).can_see?(secure_category)).to be_truthy
        ...
        secure_category =
          build(
            :category,
            read_restricted: true, email_in_allow_strangers: true
          )
        expect(Guardian.new(normal_user).can_see?(secure_category)).to be_falsey
        expect(Guardian.new(staged_user).can_see?(secure_category)).to be_falsey
        expect(Guardian.new(admin_user).can_see?(secure_category)).to be_truthy
    ...
    describe 'a Topic' do
      ...
      it 'restricts private topics' do
        ...
        expect(
          Guardian.new(private_topic.user).can_see?(private_topic)
        ).to be_truthy
    ...
    describe 'a Post' do
      ...
      it 'respects whispers' do
        ...
        regular_whisper =
          Fabricate.build(
            :post,
            post_type: Post.types[:whisper], user: regular_user
          )
    ...
    describe 'a PostRevision' do
      ...
      context 'edit_history_visible_to_public is true' do
        ...
        it 'is true when logged in' do
          expect(
            Guardian.new(Fabricate(:user)).can_see?(post_revision)
          ).to be_truthy
        end
      end
      ...
      context 'edit_history_visible_to_public is false' do
        ...
        it 'is true for staff' do
          expect(
            Guardian.new(Fabricate(:admin)).can_see?(post_revision)
          ).to be_truthy
          expect(
            Guardian.new(Fabricate(:moderator)).can_see?(post_revision)
          ).to be_truthy
        end

        it 'is true for trust level 4' do
          expect(
            Guardian.new(trust_level_4).can_see?(post_revision)
          ).to be_truthy
        end

        it 'is false for trust level lower than 4' do
          expect(
            Guardian.new(trust_level_3).can_see?(post_revision)
          ).to be_falsey
        end
      end
    end
  ...
  describe 'can_create?' do
    ...
    describe 'a Topic' do
      it 'does not allow moderators to create topics in readonly categories' do
        ...
        expect(
          Guardian.new(moderator).can_create?(Topic, category)
        ).to be_falsey
      end
      ...
      it 'is true for new users by default' do
        expect(
          Guardian.new(user).can_create?(Topic, Fabricate(:category))
        ).to be_truthy
      end
      ...
      it 'is false if user has not met minimum trust level' do
        ...
        expect(
          Guardian.new(build(:user, trust_level: 0)).can_create?(
            Topic,
            Fabricate(:category)
          )
        ).to be_falsey
      ...
      it 'is true if user has met or exceeded the minimum trust level' do
        SiteSetting.min_trust_to_create_topic = 1
        expect(
          Guardian.new(build(:user, trust_level: 1)).can_create?(
            Topic,
            Fabricate(:category)
          )
        ).to be_truthy
        expect(
          Guardian.new(build(:user, trust_level: 2)).can_create?(
            Topic,
            Fabricate(:category)
          )
        ).to be_truthy
        expect(
          Guardian.new(build(:admin, trust_level: 0)).can_create?(
            Topic,
            Fabricate(:category)
          )
        ).to be_truthy
        expect(
          Guardian.new(build(:moderator, trust_level: 0)).can_create?(
            Topic,
            Fabricate(:category)
          )
        ).to be_truthy
    ...
    describe 'a Post' do
      ...
      context 'closed topic' do
        ...
        it 'allows new posts from trust_level_4s' do
          expect(
            Guardian.new(trust_level_4).can_create?(Post, topic)
          ).to be_truthy
        end
      end
      ...
      context 'archived topic' do
        ...
        context 'regular users' do
          it "doesn't allow new posts from regular users" do
            expect(
              Guardian.new(coding_horror).can_create?(Post, topic)
            ).to be_falsey
          end
      ...
      context 'trashed topic' do
        before { topic.deleted_at = Time.now }
      ...
      context 'system message' do
        ...
        it 'allows the user to reply to system messages' do
          expect(Guardian.new(user).can_create_post?(private_message)).to eq(
                true
              )
      ...
      context 'private message' do
        let(:private_message) do
          Fabricate(
            :topic,
            archetype: Archetype.private_message, category_id: nil
          )
        end
    ...
    context 'as a moderator' do
      ...
      describe 'when post has been deleted' do
        it 'should return the right value' do
          ...
          expect(
            Guardian.new(moderator).can_recover_topic?(topic.reload)
          ).to be_truthy
        end
      end
    ...
    context 'as a moderator' do
      ...
      describe 'when post has been deleted' do
        it 'should return the right value' do
          ...
          expect(
            Guardian.new(moderator).can_recover_post?(post.reload)
          ).to be_truthy
        end
  ...
  describe 'can_edit?' do
    ...
    describe 'a Post' do
      ...
      it 'returns true if the post is hidden due to flagging and it been enough time' do
        post.hidden = true
        post.hidden_at =
          (SiteSetting.cooldown_minutes_after_hiding_posts + 1).minutes.ago
        expect(Guardian.new(post.user).can_edit?(post)).to be_truthy
      end
      ...
      context 'post is older than post_edit_time_limit' do
        let(:old_post) do
          build(
            :post,
            topic: topic, user: topic.user, created_at: 6.minutes.ago
          )
        end
      ...
      context 'first post of a static page doc' do
        ...
        let!(:tos_first_post) do
          build(:post, topic: tos_topic, user: tos_topic.user)
        end
        before { SiteSetting.tos_topic_id = tos_topic.id }

        it 'restricts static doc posts' do
          expect(
            Guardian.new(build(:user)).can_edit?(tos_first_post)
          ).to be_falsey
    ...
    describe 'a Topic' do
      ...
      context 'archived' do
        ...
        it 'returns true at trust level 4' do
          expect(
            Guardian.new(trust_level_4).can_edit?(archived_topic)
          ).to be_truthy
        end

        it 'is false at TL4, if `trusted_users_can_edit_others` is false' do
          SiteSetting.trusted_users_can_edit_others = false
          expect(Guardian.new(trust_level_4).can_edit?(archived_topic)).to eq(
                false
              )
        end
        ...
        it 'returns false at trust level 3' do
          expect(
            Guardian.new(trust_level_3).can_edit?(archived_topic)
          ).to be_falsey
        end
  ...
  context 'can_delete?' do
    ...
    context 'a Category' do
      ...
      it "can't be deleted if it is the Uncategorized Category" do
        ...
        expect(
          Guardian.new(admin).can_delete?(uncategorized_category)
        ).to be_falsey
      end
    ...
    context 'a PostAction' do
      ...
      it 'returns false when not the user who created it' do
        expect(
          Guardian.new(coding_horror).can_delete?(post_action)
        ).to be_falsey
      end
  ...
  context 'can_grant_admin?' do
    ...
    it 'should not allow an admin to grant admin access to a non real user' do
      begin
        ...
        expect(
          Guardian.new(admin).can_grant_admin?(Discourse.system_user)
        ).to be(false)
      ensure
  ...
  context 'can_grant_moderation?' do
    ...
    it 'should not allow an admin to grant moderation to a non real user' do
      begin
        ...
        expect(
          Guardian.new(admin).can_grant_moderation?(Discourse.system_user)
        ).to be(false)
      ensure
  ...
  context 'can_revoke_moderation?' do
    ...
    it 'wont allow a moderator to revoke their own moderator' do
      expect(
        Guardian.new(moderator).can_revoke_moderation?(moderator)
      ).to be_falsey
    end
    ...
    it 'should not allow an admin to revoke moderation from a non real user' do
      expect(
        Guardian.new(admin).can_revoke_moderation?(Discourse.system_user)
      ).to be(false)
    end
  end
  ...
  describe 'can_delete_user?' do
    ...
    shared_examples 'can_delete_user examples' do
      it 'is true if user is not an admin and has never posted' do
        expect(
          Guardian.new(actor).can_delete_user?(
            Fabricate.build(:user, created_at: 100.days.ago)
          )
        ).to be_truthy
      end
  ...
  describe 'can_delete_all_posts?' do
    ...
    it 'is false for regular users' do
      expect(
        Guardian.new(user).can_delete_all_posts?(coding_horror)
      ).to be_falsey
    end
    ...
    shared_examples 'can_delete_all_posts examples' do
      it 'is true if user has no posts' do
        SiteSetting.delete_user_max_post_age = 10
        expect(
          Guardian.new(actor).can_delete_all_posts?(
            Fabricate(:user, created_at: 100.days.ago)
          )
        ).to be_truthy
      end
  ...
  describe 'can_anonymize_user?' do
    ...
    it 'is false for admin anonymizing an admin' do
      expect(
        Guardian.new(admin).can_anonymize_user?(Fabricate(:admin))
      ).to be_falsey
    end

    it 'is false for admin anonymizing a moderator' do
      expect(
        Guardian.new(admin).can_anonymize_user?(Fabricate(:moderator))
      ).to be_falsey
    end
    ...
    it 'is false for moderator anonymizing a moderator' do
      expect(
        Guardian.new(moderator).can_anonymize_user?(Fabricate(:moderator))
      ).to be_falsey
    end
  end
  ...
  describe 'can_grant_title?' do
    ...
    context 'with title argument' do
      ...
      let(:no_title_badge) do
        Fabricate(:badge, name: 'Writer', allow_title: false)
      end
      ...
      it 'returns true if title belongs to a badge that user has' do
        ...
        expect(
          Guardian.new(user).can_grant_title?(user, title_badge.name)
        ).to eq(true)
      end
      ...
      it "returns false if title belongs to a badge that user has but can't be used as a title" do
        ...
        expect(
          Guardian.new(user).can_grant_title?(user, no_title_badge.name)
        ).to eq(false)
      end

      it 'returns true if title is from a group the user belongs to' do
        group.add(user)
        expect(Guardian.new(user).can_grant_title?(user, group.title)).to eq(
              true
            )
      end
  ...
  describe 'can_edit_username?' do
    it 'is false without a logged in user' do
      expect(
        Guardian.new(nil).can_edit_username?(
          build(:user, created_at: 1.minute.ago)
        )
      ).to be_falsey
    end
    ...
    it "is false for regular users to edit another user's username" do
      expect(
        Guardian.new(build(:user)).can_edit_username?(
          build(:user, created_at: 1.minute.ago)
        )
      ).to be_falsey
    end
    ...
    context 'for a new user' do
      ...
      it 'is true for the user to change their own username' do
        expect(
          Guardian.new(target_user).can_edit_username?(target_user)
        ).to be_truthy
      end
    end
    ...
    context 'when SSO username override is active' do
      ...
      it 'is false for moderators' do
        expect(
          Guardian.new(moderator).can_edit_username?(moderator)
        ).to be_falsey
      end
  ...
  describe 'can_edit_email?' do
    context 'when allowed in settings' do
      ...
      it 'is false when not logged in' do
        expect(
          Guardian.new(nil).can_edit_email?(
            build(:user, created_at: 1.minute.ago)
          )
        ).to be_falsey
      end
      ...
      it "is false for regular users to edit another user's email" do
        expect(
          Guardian.new(build(:user)).can_edit_email?(
            build(:user, created_at: 1.minute.ago)
          )
        ).to be_falsey
      end
  ...
  describe 'can_edit_name?' do
    it 'is false without a logged in user' do
      expect(
        Guardian.new(nil).can_edit_name?(build(:user, created_at: 1.minute.ago))
      ).to be_falsey
    end

    it "is false for regular users to edit another user's name" do
      expect(
        Guardian.new(build(:user)).can_edit_name?(
          build(:user, created_at: 1.minute.ago)
        )
      ).to be_falsey
    end
    ...
    context 'for a new user' do
      ...
      it 'is true for the user to change their own name' do
        expect(
          Guardian.new(target_user).can_edit_name?(target_user)
        ).to be_truthy
      end
    ...
    context 'when name is enabled in preferences' do
      ...
      context 'when SSO is enabled' do
        ...
        context 'when SSO name override is active' do
          before { SiteSetting.sso_overrides_name = true }
        ...
        context 'when SSO name override is not active' do
          before { SiteSetting.sso_overrides_name = false }
  ...
  describe '#allow_themes?' do
    ...
    it 'allows staff to use any themes' do
      expect(
        Guardian.new(moderator).allow_themes?([theme.id, theme2.id])
      ).to eq(false)
      expect(Guardian.new(admin).allow_themes?([theme.id, theme2.id])).to eq(
            false
          )

      expect(
        Guardian.new(moderator).allow_themes?(
          [theme.id, theme2.id],
          include_preview: true
        )
      ).to eq(true)
      expect(
        Guardian.new(admin).allow_themes?(
          [theme.id, theme2.id],
          include_preview: true
        )
      ).to eq(true)
  ...
  describe 'can_wiki?' do
    ...
    context 'post is older than post_edit_time_limit' do
      let(:old_post) do
        build(:post, user: trust_level_2, created_at: 6.minutes.ago)
      end
      before do
        SiteSetting.min_trust_to_allow_self_wiki = 2
    ...
    context 'tagging is enabled' do
      ...
      context 'min_trust_to_create_tag is 3' do
        ...
        describe 'can_tag_topics' do
          it 'returns false if trust level is too low' do
            expect(
              Guardian.new(Fabricate(:user, trust_level: 0)).can_tag_topics?
            ).to be_falsey
          end

          it 'returns true if trust level is high enough' do
            expect(
              Guardian.new(Fabricate(:user, trust_level: 1)).can_tag_topics?
            ).to be_truthy
          end
  ...
  describe(:can_see_group) do
    it 'Correctly handles owner visibile groups' do
      group =
        Group.new(
          name: 'group', visibility_level: Group.visibility_levels[:owners]
        )
    ...
    it 'Correctly handles staff visibile groups' do
      group =
        Group.new(
          name: 'group', visibility_level: Group.visibility_levels[:staff]
        )
    ...
    it 'Correctly handles member visibile groups' do
      group =
        Group.new(
          name: 'group', visibility_level: Group.visibility_levels[:members]
        )
    ...
    it 'Correctly handles public groups' do
      group =
        Group.new(
          name: 'group', visibility_level: Group.visibility_levels[:public]
        )
  ...
  context 'suspension reasons' do
    ...
    context 'with hide suspension reason enabled' do
      ...
      it 'staff can see suspensions' do
        expect(Guardian.new(moderator).can_see_suspension_reason?(user)).to eq(
              true
            )
      end
    end
  ...
  describe '#can_remove_allowed_users?' do
    context 'staff users' do
      it 'should be true' do
        expect(Guardian.new(moderator).can_remove_allowed_users?(topic)).to eq(
              true
            )
      end
    end
    ...
    context 'normal user' do
      ...
      describe 'target_user is the user' do
        describe 'when user is in a pm with another user' do
          it 'should return true' do
            expect(
              Guardian.new(user).can_remove_allowed_users?(topic, user)
            ).to eq(true)
          end
        end
        ...
        describe 'when user is the creator of the topic' do
          it 'should return false' do
            expect(
              Guardian.new(topic.user).can_remove_allowed_users?(
                topic,
                topic.user
              )
            ).to eq(false)
