...
describe PrettyText do
  ...
  describe 'Quoting' do
    describe 'with avatar' do
      ...
      it 'do off topic quoting with emoji unescape' do
        ...
        expected = <<~HTML
          <aside class="quote no-group" data-post="2" data-topic="#{topic
          .id}">
          <div class="title">
          <div class="quote-controls"></div>
          <a href="http://test.localhost/t/this-is-a-test-topic/#{topic
          .id}/2">This is a test topic <img src="/images/emoji/twitter/slight_smile.png?v=#{Emoji::EMOJI_VERSION}" title="slight_smile" alt="slight_smile" class="emoji"></a>
          </div>
          <blockquote>
          <p>ddd</p>
          </blockquote>
        ...
        expect(
          cook(
            "[quote=\"EvilTrout, post:2, topic:#{topic.id}\"]\nddd\n[/quote]",
            topic_id: 1
          )
        ).to eq(n(expected))
      end
      ...
      it 'do off topic quoting of posts from secure categories' do
        ...
        topic =
          Fabricate(
            :topic,
            title: 'this is topic with secret category', category: category
          )

        expected = <<~HTML
          <aside class="quote no-group" data-post="3" data-topic="#{topic
          .id}">
          <div class="title">
          <div class="quote-controls"></div>
          <a href="http://test.localhost/t/#{topic
          .id}/3">#{I18n.t(
          'on_another_topic'
        )}</a>
          </div>
      ...
      it 'indifferent about missing quotations' do
        md = <<~MD
          [quote=#{user
          .username}, post:123, topic:456, full:true]
        ...
        html = <<~HTML
          ...
          <div class="title">
          <div class="quote-controls"></div>
          <img alt width="20" height="20" src="//test.localhost/uploads/default/avatars/42d/57c/46ce7ee487/40.png" class="avatar"> #{user
          .username}:</div>
          <blockquote>
          <p>ddd</p>
          </blockquote>
    ...
    describe 'with primary user group' do
      ...
      it 'adds primary group class to referenced users quote' do
        ...
        expected = <<~HTML
          <aside class="quote group-#{group
          .name}" data-post="2" data-topic="#{topic
          .id}">
          <div class="title">
          <div class="quote-controls"></div>
          <img alt width="20" height="20" src="//test.localhost/uploads/default/avatars/42d/57c/46ce7ee487/40.png" class="avatar"><a href="http://test.localhost/t/this-is-a-test-topic/#{topic
          .id}/2">This is a test topic</a>
          </div>
          <blockquote>
          <p>ddd</p>
          </blockquote>
        ...
        expect(
          cook(
            "[quote=\"#{user.username}, post:2, topic:#{topic
              .id}\"]\nddd\n[/quote]",
            topic_id: 1
          )
        ).to eq(n(expected))
  ...
  describe 'Mentions' do
    ...
    it 'handles user and group mentions correctly' do
      ...
      %w[Group group2].each do |name|
        Fabricate(
          :group,
          name: name, mentionable_level: Group::ALIAS_LEVELS[:everyone]
        )
      end
    ...
    context 'subfolder' do
      ...
      it 'handles user and group mentions correctly' do
        ...
        Fabricate(
          :group,
          name: 'groupA', mentionable_level: Group::ALIAS_LEVELS[:everyone]
        )

        input = 'hi there @user1 and @groupA'
        expected =
          '<p>hi there <a class="mention" href="/forum/u/user1">@user1</a> and <a class="mention-group" href="/forum/groups/groupa">@groupA</a></p>'
    ...
    it 'does not create mention for a non mentionable group' do
      ...
      expect(PrettyText.cook("test @#{group.name} test")).to eq(
            "<p>test <span class=\"mention\">@#{group.name}</span> test</p>"
          )
    end
    ...
    it 'does not mention staged users' do
      ...
      expect(PrettyText.cook("something @#{user.username} something")).to eq(
            "<p>something <span class=\"mention\">@#{user
              .username}</span> something</p>"
          )
    end
    ...
    it 'can handle mentions inside a hyperlink' do
      expect(
        PrettyText.cook('<a> @inner</a> ')
      ).to match_html '<p><a> @inner</a></p>'
    end

    it 'can handle mentions inside a hyperlink' do
      expect(
        PrettyText.cook('[link @inner](http://site.com)')
      ).to match_html '<p><a href="http://site.com" rel="nofollow noopener">link @inner</a></p>'
    end
  ...
  describe 'code fences' do
    ...
    it 'can include code class correctly' do
      ...
      expect(PrettyText.cook("```text\ncpp\n```")).to match_html(
            "<pre><code class='lang-nohighlight'>cpp\n</code></pre>"
          )
    end
    ...
    it 'does censor code fences' do
      begin
        %w[apple banana].each do |w|
          Fabricate(
            :watched_word,
            word: w, action: WatchedWord.actions[:censor]
          )
        end
  ...
  describe 'rel nofollow' do
    ...
    it 'should inject nofollow in all user provided links' do
      expect(PrettyText.cook('<a href="http://cnn.com">cnn</a>')).to match(
            /nofollow noopener/
          )
    end
    ...
    it 'should not inject nofollow in all local links' do
      expect(
        PrettyText.cook("<a href='#{Discourse.base_url}/test.html'>cnn</a>") !~
          /nofollow/
      ).to eq(true)
    end
    ...
    it 'should not inject nofollow for foo.com' do
      expect(
        PrettyText.cook("<a href='http://foo.com/test.html'>cnn</a>") !~
          /nofollow/
      ).to eq(true)
    end
    ...
    it 'should inject nofollow for afoo.com' do
      expect(
        PrettyText.cook("<a href='http://afoo.com/test.html'>cnn</a>")
      ).to match(/nofollow/)
    end

    it 'should not inject nofollow for bar.foo.com' do
      expect(
        PrettyText.cook("<a href='http://bar.foo.com/test.html'>cnn</a>") !~
          /nofollow/
      ).to eq(true)
    end
  ...
  describe 'Excerpt' do
    it 'sanitizes attempts to inject invalid attributes' do
      spinner =
        "<a href=\"http://thedailywtf.com/\" data-bbcode=\"' class='fa fa-spin\">WTF</a>"
      expect(PrettyText.excerpt(spinner, 20)).to match_html spinner

      spinner =
        "<a href=\"http://thedailywtf.com/\" title=\"' class=&quot;fa fa-spin&quot;&gt;&lt;img src='http://thedailywtf.com/Resources/Images/Primary/logo.gif\"></a>"
      expect(PrettyText.excerpt(spinner, 20)).to match_html spinner
    end
    ...
    context 'images' do
      it 'should dump images' do
        expect(
          PrettyText.excerpt("<img src='http://cnn.com/a.gif'>", 100)
        ).to eq('[image]')
      end
      ...
      context 'alt tags' do
        ...
        describe 'when alt tag is empty' do
          it 'should not keep alt tags' do
            expect(
              PrettyText.excerpt("<img src='http://cnn.com/a.gif' alt>", 100)
            ).to eq("[#{I18n.t('excerpt_image')}]")
          end
        end
      end
      ...
      context 'title tags' do
        ...
        describe 'when title tag is empty' do
          it 'should not keep title tags' do
            expect(
              PrettyText.excerpt("<img src='http://cnn.com/a.gif' title>", 100)
            ).to eq("[#{I18n.t('excerpt_image')}]")
          end
        end
      end
      ...
      it 'should keep spoilers' do
        expect(
          PrettyText.excerpt(
            "<div class='spoiler'><img src='http://cnn.com/a.gif'></div>",
            100
          )
        ).to match_html "<span class='spoiler'>[image]</span>"
        expect(
          PrettyText.excerpt("<span class='spoiler'>spoiler</div>", 100)
        ).to match_html "<span class='spoiler'>spoiler</span>"
      end

      it 'should keep details if too long' do
        expect(
          PrettyText.excerpt(
            '<details><summary>expand</summary><p>hello</p></details>',
            6
          )
      ...
      it 'should strip images, but keep emojis when option is set' do
        emoji_image =
          "<img src='/images/emoji/twitter/heart.png?v=#{Emoji::EMOJI_VERSION}' title=':heart:' class='emoji' alt='heart'>"
        html = "<img src='http://cnn.com/a.gif'> Hello world #{emoji_image}"

        expect(PrettyText.excerpt(html, 100, strip_images: true)).to eq(
              'Hello world heart'
            )
        expect(
          PrettyText.excerpt(
            html,
            100,
            strip_images: true, keep_emoji_images: true
          )
    ...
    it 'should truncate stuff properly' do
      ...
      expect(PrettyText.excerpt('<p>hello</p><p>world</p>', 6)).to eq(
            'hello w&hellip;'
          )
    end
    ...
    it "doesn't extract empty quotes as links" do
      expect(
        PrettyText.extract_links(
          "<aside class='quote'>not a linked quote</aside>\n"
        )
          .to_a
      ).to be_empty
    end
    ...
    it 'should be able to extract links' do
      expect(extract_urls("<a href='http://cnn.com'>http://bla.com</a>")).to eq(
            %w[http://cnn.com]
          )
    end
    ...
    it 'should extract links to topics' do
      expect(
        extract_urls('<aside class=\"quote\" data-topic=\"321\">aside</aside>')
      ).to eq(%w[/t/topic/321])
    end
    ...
    it 'should lazyYT videos' do
      ...
      ).to eq(%w[https://www.youtube.com/watch?v=yXEuEUQIP3Q])
    end

    it 'should extract links to posts' do
      expect(
        extract_urls(
          '<aside class=\"quote\" data-topic=\"1234\" data-post=\"4567\">aside</aside>'
        )
      ).to eq(%w[/t/topic/1234/4567])
    end
    ...
    it 'handles custom bbcode excerpt' do
      ...
      expect(post.excerpt).to eq(
            'hello <a href=\"https://site.com\" rel=\"nofollow noopener\">site</a>'
          )
    end

    it 'handles span excerpt at the beginning of a post' do
      expect(
        PrettyText.excerpt("<span class='excerpt'>hi</span> test", 100)
      ).to eq('hi')
    ...
    it 'ignores max excerpt length if a span excerpt is specified' do
      ...
      text =
        two_hundred + "<span class='excerpt'>#{two_hundred}</span>" +
          two_hundred
    ...
    it 'should have an option to preserve emoji images' do
      emoji_image =
        "<img src='/images/emoji/twitter/heart.png?v=#{Emoji::EMOJI_VERSION}' title=':heart:' class='emoji' alt='heart'>"
      expect(
        PrettyText.excerpt(emoji_image, 100, keep_emoji_images: true)
      ).to match_html(emoji_image)
    end

    it 'should have an option to remap emoji to code points' do
      emoji_image =
        "I <img src='/images/emoji/twitter/heart.png?v=#{Emoji::EMOJI_VERSION}' title=':heart:' class='emoji' alt=':heart:'> you <img src='/images/emoji/twitter/heart.png?v=#{Emoji::EMOJI_VERSION}' title=':unknown:' class='emoji' alt=':unknown:'> "
      expect(
        PrettyText.excerpt(emoji_image, 100, remap_emoji: true)
      ).to match_html('I ❤  you :unknown:')
    end
    ...
    context 'option to preserve onebox source' do
      it 'should return the right excerpt' do
        ...
        expect(PrettyText.excerpt(onebox, 100, keep_onebox_source: true)).to eq(
              expected
            )
  ...
  describe 'strip links' do
    ...
    it 'does nothing to a string without links' do
      expect(PrettyText.strip_links("I'm the <b>batman</b>")).to eq(
            "I'm the <b>batman</b>"
          )
    end
  ...
  describe 'emoji' do
    ...
    it 'replaces skin toned emoji' do
      expect(PrettyText.cook('hello 👱🏿‍♀️')).to eq(
            "<p>hello <img src=\"/images/emoji/twitter/blonde_woman/6.png?v=#{Emoji::EMOJI_VERSION}\" title=\":blonde_woman:t6:\" class=\"emoji\" alt=\":blonde_woman:t6:\"></p>"
          )
      expect(PrettyText.cook('hello 👩‍🎤')).to eq(
            "<p>hello <img src=\"/images/emoji/twitter/woman_singer.png?v=#{Emoji::EMOJI_VERSION}\" title=\":woman_singer:\" class=\"emoji\" alt=\":woman_singer:\"></p>"
          )
      expect(PrettyText.cook('hello 👩🏾‍🎓')).to eq(
            "<p>hello <img src=\"/images/emoji/twitter/woman_student/5.png?v=#{Emoji::EMOJI_VERSION}\" title=\":woman_student:t5:\" class=\"emoji\" alt=\":woman_student:t5:\"></p>"
          )
      expect(PrettyText.cook('hello 🤷‍♀️')).to eq(
            "<p>hello <img src=\"/images/emoji/twitter/woman_shrugging.png?v=#{Emoji::EMOJI_VERSION}\" title=\":woman_shrugging:\" class=\"emoji\" alt=\":woman_shrugging:\"></p>"
          )
    end
  ...
  describe 'custom emoji' do
    it 'replaces the custom emoji' do
      ...
      expect(PrettyText.cook('hello :trout:')).to match(
            /<img src[^>]+trout[^>]+>/
          )
    end
  end
  ...
  it 'supports href schemes' do
    ...
    expected =
      '<p><a href="steam://store/452530" rel="nofollow noopener">Steam URL Scheme</a></p>'
  ...
  it 'allows only tel URL scheme to start with a plus character' do
    ...
    expected =
      '<p><a href="tel://+452530579785" rel="nofollow noopener">Tel URL Scheme</a></p>'
    expect(cooked).to eq(n expected)
  ...
  it 'produces hashtag links' do
    ...
    [
      ...
      "<a class=\"hashtag\" href=\"#{category2
        .url_with_id}\">#<span>known</span></a>",
      '<a class=\"hashtag\" href=\"http://test.localhost/tags/known\">#<span>known</span></a>',
      "<a class=\"hashtag\" href=\"#{category
        .url_with_id}\">#<span>testing</span></a>"
    ]
    ...
    cooked =
      PrettyText.cook("<a href='http://example.com'>`a` #known::tag here</a>")
    ...
    expect(PrettyText.cook(' http://somewhere.com/#known')).not_to include(
              'hashtag'
            )
    expect(PrettyText.cook(' http://somewhere.com/?#known')).not_to include(
              'hashtag'
            )
    expect(PrettyText.cook(' http://somewhere.com/?abc#known')).not_to include(
              'hashtag'
            )
  end
...
HTML
  ...
  it 'handles emoji boundaries correctly' do
    ...
    expected =
      "<p>a,<img src=\"/images/emoji/twitter/man/2.png?v=#{Emoji::EMOJI_VERSION}\" title=\":man:t2:\" class=\"emoji\" alt=\":man:t2:\">,b</p>"
    expect(cooked).to match(expected.strip)
  end

  it 'can handle emoji by translation' do
    expected =
      "<p><img src=\"/images/emoji/twitter/wink.png?v=#{Emoji::EMOJI_VERSION}\" title=\":wink:\" class=\"emoji\" alt=\":wink:\"></p>"
  ...
  it 'handles onebox correctly' do
    expect(
      PrettyText.cook("http://a.com\nhttp://b.com").split('onebox').length
    ).to eq(3)
    expect(
      PrettyText.cook("http://a.com\n\nhttp://b.com").split('onebox').length
    ).to eq(3)
  ...
  it 'can handle bbcode' do
    expect(PrettyText.cook('a[b]b[/b]c')).to eq(
          '<p>a<span class="bbcode-b">b</span>c</p>'
        )
    expect(PrettyText.cook('a[i]b[/i]c')).to eq(
          '<p>a<span class="bbcode-i">b</span>c</p>'
        )
  end
  ...
  it 'can onebox local topics' do
    ...
    quote =
      create_post(
        ...
        raw: "This is a sample reply with a quote\n\n#{url}"
      )
    quote.reload
  ...
  it 'provides safety for img bbcode' do
    cooked =
      PrettyText.cook '[img]http://aaa.com<script>alert(1);</script>[/img]'
    html =
      '<p><img src="http://aaa.com&lt;script&gt;alert(1);&lt;/script&gt;" alt></p>'
    expect(cooked).to eq(html)
  end
  ...
  it 'supports email bbcode' do
    ...
    html =
      '<p><a href="mailto:sam@sam.com" data-bbcode="true">sam@sam.com</a></p>'
    expect(cooked).to eq(html)
  end
  ...
  it 'supports url bbcode' do
    ...
    html =
      '<p><a href="http://sam.com" data-bbcode="true" rel="nofollow noopener">http://sam.com</a></p>'
    expect(cooked).to eq(html)
  end
  ...
  it 'supports nesting tags in url' do
    ...
    html =
      '<p><a href="http://sam.com" data-bbcode="true" rel="nofollow noopener"><span class="bbcode-b">I am sam</span></a></p>'
    expect(cooked).to eq(html)
  end
  ...
  it 'supports query params in bbcode url' do
    ...
    html =
      '<p><a href="https://www.amazon.com/Camcorder-Hausbell-302S-Control-Infrared/dp/B01KLOA1PI/?tag=discourse" data-bbcode="true" rel="nofollow noopener">BBcode link</a></p>'
    expect(cooked).to eq(html)
  end

  it 'supports inline code bbcode' do
    cooked =
      PrettyText.cook 'Testing [code]codified **stuff** and `more` stuff[/code]'
  ...
  it 'supports block code bbcode' do
    cooked =
      PrettyText.cook "[code]\ncodified\n\n\n  **stuff** and `more` stuff\n[/code]"
    html =
      "<pre><code class=\"lang-auto\">codified\n\n\n  **stuff** and `more` stuff</code></pre>"
    expect(cooked).to eq(html)
  end
  ...
  it 'support special handling for space in urls' do
    ...
    html =
      '<p><a href="http://testing.com?a%20b" class="onebox" target="_blank" rel="nofollow noopener">http://testing.com?a%20b</a></p>'
    expect(cooked).to eq(html)
  end
  ...
  it 'supports onebox for decoded urls' do
    ...
    html =
      '<p><a href="http://testing.com?a%50b" class="onebox" target="_blank" rel="nofollow noopener">http://testing.com?aPb</a></p>'
    expect(cooked).to eq(html)
  end
