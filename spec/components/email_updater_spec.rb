...
describe EmailUpdater do
  ...
  context 'as a regular user' do
    ...
    before do
      Jobs.expects(:enqueue).once.with(
        :critical_user_email,
        has_entries(type: :confirm_new_email, to_address: new_email)
      )
      updater.change_to(new_email)
    ...
    it 'starts the new confirmation process' do
      ...
      expect(@change_req.change_state).to eq(
    ...
    context 'confirming a valid token' do
      it "updates the user's email" do
        Jobs.expects(:enqueue).once.with(
          :critical_user_email,
          has_entries(type: :notify_old_email, to_address: old_email)
        )
  ...
  context 'as a staff user' do
    ...
    before do
      Jobs.expects(:enqueue).once.with(
        :critical_user_email,
        has_entries(type: :confirm_old_email, to_address: old_email)
      )
      updater.change_to(new_email)
      @change_req = user.email_change_requests.first
    end
  ...
  context 'hide_email_address_taken is enabled' do
    ...
    it 'sends an email to the owner of the account with the new email' do
      Jobs.expects(:enqueue).once.with(
        :critical_user_email,
        has_entries(type: :account_exists, user_id: existing.id)
      )
      updater.change_to(existing.email)
    end
  end
