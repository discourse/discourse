...
describe DiscourseTagging do
    ...
    context 'for input fields' do
      it "doesn't return selected tags if there's a search term" do
        tags =
          DiscourseTagging.filter_allowed_tags(
            Tag.all,
            Guardian.new(user),
            selected_tags: [tag2.name], for_input: true, term: 'fun'
          )
            .map(&:name)
      ...
      it "doesn't return selected tags if there's no search term" do
        tags =
          DiscourseTagging.filter_allowed_tags(
            Tag.all,
            Guardian.new(user),
            selected_tags: [tag2.name], for_input: true
          )
            .map(&:name)
        expect(tags).to contain_exactly(tag1.name, tag3.name)
      end
      ...
      context 'with tags visible only to staff' do
        ...
        it 'should return all tags to staff' do
          tags =
            DiscourseTagging.filter_allowed_tags(Tag.all, Guardian.new(admin))
              .to_a
        ...
        it 'should not return hidden tag to non-staff' do
          tags =
            DiscourseTagging.filter_allowed_tags(Tag.all, Guardian.new(user))
              .to_a
  ...
  describe 'tag_topic_by_names' do
    context 'staff-only tags' do
      ...
      before { create_staff_tags(%w[alpha]) }
      ...
      it "regular users can't add staff-only tags" do
        ...
        expect(topic.errors[:base]&.first).to eq(
              I18n.t('tags.staff_tag_disallowed', tag: 'alpha')
            )
      end
    ...
    context 'hidden tags' do
      ...
      it 'user cannot add hidden tag by knowing its name' do
        expect(
          PostRevisor.new(post).revise!(
            topic.user,
            raw: post.raw + ' edit', tags: [hidden_tag.name]
          )
        ).to be_falsey
      ...
      it 'admin can add hidden tag' do
        expect(
          PostRevisor.new(post).revise!(
            admin,
            raw: post.raw, tags: [hidden_tag.name]
          )
        ).to be_truthy
  ...
  describe '#tags_for_saving' do
    ...
    it "returns empty array if can't tag topics" do
      ...
      expect(described_class.tags_for_saving(%w[newtag], guardian)).to eq([])
    end
    ...
    context 'can tag topics but not create tags' do
      ...
      it 'returns empty array if all tags are new' do
        expect(
          described_class.tags_for_saving(%w[newtag newtagplz], guardian)
        ).to eq([])
      end
    ...
    context 'can tag topics and create tags' do
      ...
      it 'returns given tag names if can create new tags and tag topics' do
        expect(
          described_class.tags_for_saving(%w[newtag1 newtag2], guardian).try(
            :sort
          )
        ).to eq(%w[newtag1 newtag2])
      end
      ...
      it 'only sanitizes new tags' do
        ...
        expect(
          described_class.tags_for_saving(
            %w[math=fun fun*2@gmail.com],
            guardian
          )
            .try(:sort)
