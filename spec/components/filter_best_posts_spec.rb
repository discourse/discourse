...
describe FilterBestPosts do
  ...
  let!(:p1) do
    Fabricate(:post, topic: topic, user: first_poster, percent_rank: 1)
  end
  let!(:p2) do
    Fabricate(:post, topic: topic, user: coding_horror, percent_rank: 0.5)
  end
  let!(:p3) do
    Fabricate(:post, topic: topic, user: first_poster, percent_rank: 0)
  end
  ...
  it 'can find the best responses' do
    filtered_posts =
      TopicView.new(topic.id, coding_horror, best: 2).filtered_posts
  ...
  describe 'processing options' do
    before do
      @filtered_posts = TopicView.new(topic.id, nil, best: 99).filtered_posts
    end
