...
describe SiteSettingExtension do
  ...
  describe 'refresh!' do
    ...
    it 'clears the cache for site setting uploads' do
      ...
      settings.provider.save(
        :upload_type,
        upload2.id,
        SiteSetting.types[:upload]
      )

      expect { settings.refresh! }.to change do
        settings.send(:uploads)[:upload_type]
      end
        .from(upload)
        .to(nil)
  ...
  describe 'int setting' do
    ...
    it 'should have a key in all_settings' do
      expect(
        settings.all_settings.detect { |s| s[:setting] == :test_setting }
      ).to be_present
    end
    ...
    it 'should have the correct desc' do
      I18n.backend.store_translations(
        ...
        site_settings: {
          test_setting:
            "test description <a href='%{base_path}/admin'>/admin</a>"
        }
      )
    ...
    context 'when overidden' do
      ...
      it 'should publish changes to clients' do
        ...
        message =
          MessageBus.track_publish('/client_settings') do
  ...
  describe 'enum setting' do
    ...
    it 'should not hose all_settings' do
      expect(
        settings.all_settings.detect { |s| s[:setting] == :test_enum }
      ).to be_present
    end

    it 'should report error when being set other values' do
      expect { settings.test_enum = 'not_in_enum' }.to raise_error(
    ...
    context 'when overridden' do
      ...
      it 'rejects invalid values' do
        ...
        expect { settings.test_enum = 'gg' }.to raise_error(
              Discourse::InvalidParameters
            )
      end
    end
  ...
  describe 'a setting with a category' do
    ...
    it 'should return the category in all_settings' do
      expect(
        settings.all_settings.find { |s| s[:setting] == :test_setting }[
          :category
        ]
      ).to eq(:tests)
    end
  ...
  describe 'set for an invalid setting name' do
    it 'raises an error' do
      settings.setting(:test_setting, 77)
      settings.refresh!
      expect { settings.set('provider', 'haxxed') }.to raise_error(
            Discourse::InvalidParameters
          )
    end
  end
  ...
  describe 'hidden' do
    ...
    it 'is not present in all_settings by default' do
      expect(
        settings.all_settings.find { |s| s[:setting] == :superman_identity }
      ).to be_blank
    end

    it 'is present in all_settings when we ask for hidden' do
      expect(
        settings.all_settings(true).find do |s|
          s[:setting] == :superman_identity
        end
      ).to be_present
    end
  end
    ...
    context 'with global setting' do
      ...
      it 'should add the key to the hidden_settings collection' do
        ...
        ['', nil].each_with_index do |setting, index|
          ...
          settings.setting(
            :"trout_api_key_#{index}",
            'evil',
            shadowed_by_global: true
          )
          settings.refresh!
          expect(
            settings.hidden_settings.include?(:"trout_api_key_#{index}")
          ).to eq(false)
        end
      end
  ...
  describe '.default_locale=' do
    ...
    it 'expires the cache' do
      settings.default_locale = 'zh_CN'
      expect(
        Rails.cache.exist?(SiteSettingExtension.client_settings_cache_key)
      ).to be_falsey
    end
  ...
  describe 'get_hostname' do
    it 'properly extracts the hostname' do
      expect(settings.send(:get_hostname, 'discourse.org')).to eq(
            'discourse.org'
          )
      expect(settings.send(:get_hostname, ' discourse.org ')).to eq(
            'discourse.org'
      ...
      expect(settings.send(:get_hostname, 'https://discourse.org')).to eq(
            'discourse.org'
          )
    end
  end
