...
describe SiteSettingExtension do
  ...
  describe "filter domain name" do
    before do
      settings.setting(:allow_listed_spam_host_domains, "www.example.com")
      settings.refresh!
    end

    it "filters domain" do
      settings.set("allow_listed_spam_host_domains", "http://www.discourse.org/")
      expect(settings.allow_listed_spam_host_domains).to eq("www.discourse.org")
    end

    it "returns invalid domain as is, without throwing exception" do
      settings.set("allow_listed_spam_host_domains", "test!url")
      expect(settings.allow_listed_spam_host_domains).to eq("test!url")
    end
  end
