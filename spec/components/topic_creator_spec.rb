  ...
  describe '#create' do
    context 'topic success cases' do
      ...
      it 'should be possible for an admin to create a topic' do
        expect(
          TopicCreator.create(admin, Guardian.new(admin), valid_attrs)
        ).to be_valid
      end

      it 'should be possible for a moderator to create a topic' do
        expect(
          TopicCreator.create(moderator, Guardian.new(moderator), valid_attrs)
        ).to be_valid
      end
        ...
        it 'should be possible for a regular user to create a topic with blank auto_close_time' do
          expect(
            TopicCreator.create(
              user,
              Guardian.new(user),
              valid_attrs.merge(auto_close_time: '')
            )
          ).to be_valid
        end
        ...
        it 'ignores auto_close_time without raising an error' do
          topic =
            TopicCreator.create(
              user,
              Guardian.new(user),
              valid_attrs.merge(auto_close_time: '24')
            )
        ...
        it 'category name is case insensitive' do
          ...
          topic =
            TopicCreator.create(
              user,
              Guardian.new(user),
              valid_attrs.merge(category: "neil's blog")
            )
          expect(topic).to be_valid
    ...
    context 'tags' do
        ...
        it 'user can add tags to topic' do
          topic =
            TopicCreator.create(
              user,
              Guardian.new(user),
              valid_attrs.merge(tags: [tag1.name])
            )
          expect(topic).to be_valid
        ...
        it "regular users can't add staff-only tags" do
          expect do
            TopicCreator.create(
              user,
              Guardian.new(user),
              valid_attrs.merge(tags: %w[alpha])
            )
      ...
      context 'minimum_required_tags is present' do
        let!(:category) do
          Fabricate(:category, name: 'beta', minimum_required_tags: 2)
        end
        ...
        it 'fails for regular user if minimum_required_tags is not satisfied' do
          expect do
            TopicCreator.create(
              user,
              Guardian.new(user),
              valid_attrs.merge(category: 'beta')
            )
          end.to raise_error(ActiveRecord::Rollback)
        end
        ...
        it 'lets admin create a topic regardless of minimum_required_tags' do
          topic =
            TopicCreator.create(
              admin,
              Guardian.new(admin),
              valid_attrs.merge(tags: [tag1.name], category: 'beta')
            )
          expect(topic).to be_valid
          expect(topic.tags.length).to eq(1)
        end
        ...
        it 'works for regular user if minimum_required_tags is satisfied' do
          topic =
            TopicCreator.create(
              user,
              Guardian.new(user),
              valid_attrs.merge(tags: [tag1.name, tag2.name], category: 'beta')
            )
          expect(topic).to be_valid
