...
describe ThemeSettingsManager do
  ...
  context 'Enum' do
    it 'only accepts values from its choices' do
      ...
      expect { enum_setting.value = 'trust level 2' }.to raise_error(
      ...
      expect { enum_setting.value = 15 }.to raise_error(
            Discourse::InvalidParameters
          )
    end
  end
  ...
  context 'Integer' do
    ...
    it 'can have min or max value' do
      ...
      expect { int_setting.value = 0 }.to raise_error(
            Discourse::InvalidParameters
          )
      expect { int_setting.value = 61 }.to raise_error(
            Discourse::InvalidParameters
          )
  ...
  context 'Float' do
    ...
    it 'can have min or max value' do
      float_setting = find_by_name(:float_setting)
      expect { float_setting.value = 1.4 }.to raise_error(
            Discourse::InvalidParameters
          )
      expect { float_setting.value = 10.01 }.to raise_error(
            Discourse::InvalidParameters
          )
      expect { float_setting.value = 'text' }.to raise_error(
            Discourse::InvalidParameters
          )
  ...
  context 'String' do
    it 'can have min or max length' do
      ...
      expect { string_setting.value = 'a' }.to raise_error(
            Discourse::InvalidParameters
          )
      ...
      expect { string_setting.value = ('a' * 21) }.to raise_error(
            Discourse::InvalidParameters
          )
    end
