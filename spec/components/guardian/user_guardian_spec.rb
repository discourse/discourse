...
describe UserGuardian do
  ...
  describe '#can_pick_avatar?' do
    ...
    context 'anon user' do
      ...
      it 'should return the right value' do
        expect(guardian.can_pick_avatar?(user_avatar, users_upload)).to eq(
              false
            )
      end
    end
    ...
    context 'current user' do
      it 'can not set uploads not owned by current user' do
        ...
        expect(guardian.can_pick_avatar?(user_avatar, already_uploaded)).to eq(
              true
            )

        UserUpload.create!(
          upload_id: not_my_upload.id, user_id: not_my_upload.user_id
        )

        expect(guardian.can_pick_avatar?(user_avatar, not_my_upload)).to eq(
              false
            )
      ...
      it 'can handle uploads that are associated but not directly owned' do
        UserUpload.create!(
          upload_id: not_my_upload.id, user_id: user_avatar.user_id
        )
    ...
    context 'moderator' do
      ...
      it 'is secure' do
        expect(guardian.can_pick_avatar?(user_avatar, moderator_upload)).to eq(
              true
            )
        expect(guardian.can_pick_avatar?(user_avatar, users_upload)).to eq(true)
        expect(guardian.can_pick_avatar?(user_avatar, already_uploaded)).to eq(
  ...
  describe '#can_see_profile?' do
    ...
    context 'hidden profile' do
      ...
      it 'is true for the user themselves' do
        expect(Guardian.new(hidden_user).can_see_profile?(hidden_user)).to eq(
              true
            )
      end
  ...
  describe '#allowed_user_field_ids' do
    ...
    it 'returns all fields for staff' do
      ...
      expect(guardian.allowed_user_field_ids(user)).to contain_exactly(
            *fields.map(&:id)
          )
    end
    ...
    it 'returns only public fields for others' do
      guardian = Guardian.new(user)
      expect(guardian.allowed_user_field_ids(user2)).to contain_exactly(
            *fields[2..5].map(&:id)
          )
    end
