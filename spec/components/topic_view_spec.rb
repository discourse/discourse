...
describe TopicView do
  ...
  it "raises a not found error if the topic doesn't exist" do
    expect { TopicView.new(1231232, evil_trout) }.to raise_error(
          Discourse::NotFound
        )
  end
  ...
  it 'handles deleted topics' do
    ...
    topic.trash!(admin)
    expect { TopicView.new(topic.id, Fabricate(:user)) }.to raise_error(
  ...
  context 'setup_filtered_posts' do
    describe 'filters posts with ignored users' do
      ...
      let!(:ignored_user) do
        Fabricate(:ignored_user, user: evil_trout, ignored_user: user)
      end
      ...
      describe 'when SiteSetting.ignore_user_enabled is true' do
        ...
        describe 'when an anonymous (non signed-in) user is viewing a Topic' do
          ...
          it 'filters out ignored user posts only' do
            ...
            expect(tv.filtered_post_ids).to eq(
  ...
  context 'chunk_size' do
    it 'returns `chunk_size` by default' do
      expect(TopicView.new(topic.id, evil_trout).chunk_size).to eq(
  ...
  context 'with a few sample posts' do
    ...
    let!(:p2) do
      Fabricate(:post, topic: topic, user: evil_trout, percent_rank: 0.5)
    end
    let!(:p3) do
    ...
    it 'it can find the best responses' do
      ...
      best =
        TopicView.new(
          topic.id,
          nil,
          best: 99, min_trust_level: evil_trout.trust_level + 1
        )
    ...
    it "raises NotLoggedIn if the user isn't logged in and is trying to view a private message" do
      ...
      expect { TopicView.new(topic.id, nil) }.to raise_error(
    ...
    context 'log_check_personal_message is enabled' do
      ...
      it "does not log personal message view if user can't see the message" do
        expect do
          TopicView.new(private_message.id, Fabricate(:user))
    ...
    it 'provides an absolute url' do
      expect(topic_view.absolute_url).to eq(
            "http://test.localhost/t/#{topic.slug}/#{topic.id}"
          )
    end
    ...
    describe '#get_canonical_path' do
      ...
      it 'generates canonical path correctly' do
        ...
        expect(TopicView.new(1234, user, page: 5).canonical_path).to eql(
              '/1234?page=5'
            )
      end
    ...
    context '.post_counts_by_user' do
      it 'returns the two posters with their appropriate counts' do
        Fabricate(
          :post,
          topic: topic, user: evil_trout, post_type: Post.types[:whisper]
        )

        expect(topic_view.post_counts_by_user.to_a).to match_array(
              [[first_poster.id, 2], [evil_trout.id, 2]]
            )

        expect(
          TopicView.new(topic.id, first_poster).post_counts_by_user.to_a
        ).to match_array([[first_poster.id, 2], [evil_trout.id, 1]])
      end
    ...
    context '.all_post_actions' do
      ...
      it 'returns the like' do
        ...
        expect(
          topic_view.all_post_actions[p1.id][PostActionType.types[:like]]
        ).to be_present
      end
    end
    ...
    context '.all_active_flags' do
      ...
      it 'returns the active flags' do
        ...
        expect(
          topic_view.all_active_flags[p1.id][PostActionType.types[:off_topic]]
            .count
        ).to eq(2)
      end
    ...
    context '#recent_posts' do
      before do
        24.times do |t|
          Fabricate(
            # our let()s have already created 3
            :post,
            topic: topic, user: first_poster, created_at: t.seconds.from_now
          )
        end
      end
      ...
      it 'returns at most 25 recent posts ordered newest first' do
        ...
        expect(recent_posts.first.created_at).to be >
              recent_posts.last.created_at
      end
    end
  end
  ...
  context '#posts' do
    ...
    let!(:p6) do
      Fabricate(
        :post,
        topic: topic, user: Fabricate(:user), deleted_at: Time.now
      )
    end
    let!(:p4) do
      Fabricate(:post, topic: topic, user: evil_trout, deleted_at: Time.now)
    end
    ...
    describe 'contains_gaps?' do
      it 'works' do
        ...
        expect(
          TopicView.new(topic.id, evil_trout, username_filters: %w[eviltrout])
            .contains_gaps?
        ).to eq(true)
        ...
        expect(
          TopicView.new(topic.id, evil_trout, best: 5).contains_gaps?
        ).to eq(true)
      end
    end
    ...
    it '#restricts to correct topic' do
      ...
      expect(
        TopicView.new(t2.id, evil_trout, post_ids: [p1.id, p2.id]).posts.count
      ).to eq(0)
    end
    ...
    describe '#filter_posts_by_post_number' do
      def create_topic_view(post_number)
        TopicView.new(
          topic.id,
          evil_trout,
          filter_post_number: post_number, asc: asc
        )
      end
    ...
    describe 'filter_posts_near' do
      def topic_view_near(post, show_deleted = false)
        TopicView.new(
          topic.id,
          evil_trout,
          post_number: post.post_number, show_deleted: show_deleted
        )
      end
      ...
      describe 'when post_number is too large' do
        it 'snaps to the lower boundary' do
          near_view = TopicView.new(topic.id, evil_trout, post_number: 99999999)
  ...
  describe '#filtered_post_stream' do
    ...
    it 'should return the right columns' do
      expect(topic_view.filtered_post_stream).to eq(
            [[post.id, 0], [post2.id, 0], [post3.id, 0]]
          )
    end
