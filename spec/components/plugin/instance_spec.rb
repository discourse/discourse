...
describe Plugin::Instance do
  ...
  context 'find_all' do
    it 'can find plugins correctly' do
      ...
      expect(plugin.path).to eq(
            "#{Rails.root}/spec/fixtures/plugins/my_plugin/plugin.rb"
          )
    end
  ...
  it 'patches the enabled? function for auth_providers if not defined' do
    ...
    plugin = Plugin::Instance.new
    authenticator = Auth::Authenticator.new
    plugin.auth_provider(
      enabled_setting: 'ubuntu_login_enabled', authenticator: authenticator
    )
    plugin.notify_before_auth
  ...
  context 'activate!' do
    before { SiteSetting.stubs(:ubuntu_login_enabled).returns(false) }
    ...
    it 'can activate plugins correctly' do
      ...
      File.open("#{plugin.auto_generated_path}/junk", 'w') do |f|
        f.write('junk')
      end
      plugin.activate!
      ...
      plugin.assets.each { |a, opts| expect(File.exists?(a)).to eq(true) }
  ...
  context 'serialized_current_user_fields' do
    ...
    it 'correctly serializes custom user fields' do
      ...
      payload =
        JSON.parse(
          CurrentUserSerializer.new(user, scope: Guardian.new(user)).to_json
        )
      expect(payload['current_user']['custom_fields']['has_car']).to eq('true')

      payload =
        JSON.parse(UserSerializer.new(user, scope: Guardian.new(user)).to_json)
  ...
  context 'register_color_scheme' do
    it 'can add a color scheme for the first time' do
      ...
      end.to change { ColorScheme.count }.by(1)
    ...
    it "doesn't add the same color scheme twice" do
      ...
      expect do
        ...
        plugin.notify_after_initialize
      end.to_not change { ColorScheme.count }
    end
  end
  ...
  describe '.register_seedfu_fixtures' do
    it "should add the new path to SeedFu's fixtures path" do
      plugin = Plugin::Instance.new nil, '/tmp/test.rb'
      plugin.register_seedfu_fixtures(%w[some_path])
  ...
  context 'locales' do
    ...
    it 'enables the registered locales only on activate' do
      plugin.register_locale(
        'foo_BAR',
        name: 'Foo', nativeName: 'Foo Bar', plural: plural
      )
      plugin.register_locale(
        'es_MX',
        name: 'Spanish (Mexico)',
    ...
    it 'correctly registers a new locale' do
      ...
      expect(locale[:message_format]).to eq(
            [
              'foo_BAR',
              "#{plugin_path}/lib/javascripts/locale/message_format/foo_BAR.js"
            ]
          )
      expect(locale[:moment_js]).to eq(
            [
              'foo_BAR',
              "#{plugin_path}/lib/javascripts/locale/moment_js/foo_BAR.js"
            ]
          )
      expect(locale[:moment_js_timezones]).to eq(
            [
              'foo_BAR',
              "#{plugin_path}/lib/javascripts/locale/moment_js_timezones/foo_BAR.js"
            ]
          )
      expect(locale[:plural]).to eq(plural.with_indifferent_access)

      expect(Rails.configuration.assets.precompile).to include(
            'locales/foo_BAR.js'
          )
      ...
      expect(JsLocaleHelper.find_moment_locale(%w[foo_BAR])).to eq (
               locale[:moment_js]
             )
      expect(
        JsLocaleHelper.find_moment_locale(%w[foo_BAR], timezone_names: true)
      ).to eq (locale[:moment_js_timezones])
    end
    ...
    it 'correctly registers a new locale using a fallback locale' do
      ...
      expect(locale[:message_format]).to eq(
            ['es', "#{Rails.root}/lib/javascripts/locale/es.js"]
          )
      expect(locale[:moment_js]).to eq(
            [
              'es',
              "#{Rails.root}/vendor/assets/javascripts/moment-locale/es.js"
            ]
          )
      expect(locale[:moment_js_timezones]).to eq(
            [
              'es',
              "#{Rails
                .root}/vendor/assets/javascripts/moment-timezone-names-locale/es.js"
            ]
          )
      ...
      expect(JsLocaleHelper.find_moment_locale(%w[es_MX])).to eq (
               locale[:moment_js]
             )
    end
    ...
    it 'correctly registers a new locale when some files exist in core' do
      locale =
        register_locale(
          'tlh',
          name: 'Klingon', nativeName: 'tlhIngan Hol', plural: plural
        )
      ...
      expect(locale[:message_format]).to eq(
            [
              'tlh',
              "#{plugin_path}/lib/javascripts/locale/message_format/tlh.js"
            ]
          )
      expect(locale[:moment_js]).to eq(
            [
              'tlh',
              "#{Rails.root}/vendor/assets/javascripts/moment-locale/tlh.js"
            ]
          )
      expect(locale[:plural]).to eq(plural.with_indifferent_access)
      ...
      expect(JsLocaleHelper.find_moment_locale(%w[tlh])).to eq (
               locale[:moment_js]
             )
    end
    ...
    it 'does not register a new locale when the fallback locale does not exist' do
      register_locale(
        'bar',
        name: 'Bar', nativeName: 'Bar', fallbackLocale: 'foo'
      )
      expect(DiscoursePluginRegistry.locales.count).to eq(0)
    end
    ...
    %w[
      ...
      it "does not register a new locale when #{path} is missing" do
        ...
        File.stubs('exist?').with(
          regexp_matches(/#{Regexp.quote(plugin_path)}.*/)
        )
          .returns(true)
