...
describe Plugin::Instance do
  ...
  context "register asset" do
    it "populates the DiscoursePluginRegistry" do
      ...
      expect(DiscoursePluginRegistry.mobile_stylesheets[plugin.asset_name]).to be_nil
      expect(DiscoursePluginRegistry.stylesheets[plugin.asset_name].count).to eq(2)
    end
  ...
  context "activate!" do
    ...
    it "finds all the custom assets" do
      ...
      expect(DiscoursePluginRegistry.desktop_stylesheets[plugin.asset_name].count).to eq(2)
      expect(DiscoursePluginRegistry.sass_variables.count).to eq(2)
      expect(DiscoursePluginRegistry.stylesheets[plugin.asset_name].count).to eq(2)
      expect(DiscoursePluginRegistry.mobile_stylesheets[plugin.asset_name].count).to eq(1)
    end
  end
