...
describe PlainTextToMarkdown do
  ...
  context 'format=flowed' do
    ...
    it "removes the trailing space if DelSp is set to 'yes'" do
      text =
        "Lorem ipsum dolor sit amet, consectetur \nadipiscing elit. \nQuasi vero, inquit"
      markdown =
        "Lorem ipsum dolor sit amet, consecteturadipiscing elit\\.Quasi vero, inquit"

      expect(
        to_markdown(text, format_flowed: true, delete_flowed_space: true)
      ).to eq(markdown)
    end
  end
  ...
  context 'links' do
    it 'removes duplicate links' do
      expect(
        to_markdown(
          'foo https://www.example.com/foo.html <https://www.example.com/foo.html> bar'
        )
