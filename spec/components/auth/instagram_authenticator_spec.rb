...
describe Auth::InstagramAuthenticator do
  it 'takes over account if email is supplied' do
    ...
    auth_token = {
      info: { email: user.email }, uid: '123', provider: 'instagram'
    }
    ...
    info =
      UserAssociatedAccount.find_by(
        provider_name: 'instagram', user_id: user.id
      )
  ...
  it 'can connect to a different existing user account' do
    ...
    hash = { info: { email: user1.email }, uid: '100', provider: 'instagram' }
    ...
    expect(
      UserAssociatedAccount.exists?(
        provider_name: 'instagram', user_id: user1.id
      )
    ).to eq(false)
    expect(
      UserAssociatedAccount.exists?(
        provider_name: 'instagram', user_id: user2.id
      )
    ).to eq(true)
  end
  ...
  context 'revoke' do
    ...
    it 'revokes correctly' do
      UserAssociatedAccount.create!(
        provider_name: 'instagram', user_id: user.id, provider_uid: 100
      )
