...
describe Auth::ManagedAuthenticator do
  ...
  let(:hash) do
    {
      ...
      extra: { raw_info: { randominfo: 'some info' } }
    }
  end

  let(:create_hash) { { provider: 'myauth', uid: '1234' } }
  ...
  describe 'after_authenticate' do
    ...
    describe 'connecting to another user account' do
      ...
      before do
        UserAssociatedAccount.create!(
          user: user1, provider_name: 'myauth', provider_uid: '1234'
        )
      end
      ...
      it 'works normally' do
        ...
        expect(
          UserAssociatedAccount.find_by(
            provider_name: 'myauth', provider_uid: '1234'
          )
            .user_id
        ).to eq(user.id)
      end
    ...
    describe 'avatar on update' do
      ...
      let!(:associated) do
        UserAssociatedAccount.create!(
          user: user, provider_name: 'myauth', provider_uid: '1234'
        )
      end

      it 'schedules the job upon update correctly' do
        # No image supplied, do not schedule
        expect { result = authenticator.after_authenticate(hash) }.to change do
        ...
        end.to change { Jobs::DownloadAvatarFromUrl.jobs.count }.by(1)
        ...
        end.to change { Jobs::DownloadAvatarFromUrl.jobs.count }.by(0)
      end
    end
    ...
    describe 'profile on update' do
      ...
      it "updates the user's location and bio, unless already set" do
        { description: :bio_raw, location: :location }
          .each do |auth_hash_key, profile_key|
    ...
    it 'returns correct information' do
      association =
        UserAssociatedAccount.create!(
          ...
          provider_uid: '1234',
          info: {
            nickname: 'somenickname', email: 'test@domain.tld', name: 'bestname'
          }
        )
