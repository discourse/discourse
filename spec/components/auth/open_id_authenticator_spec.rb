...
describe Auth::OpenIdAuthenticator do
  it 'can lookup pre-existing user if trusted' do
    ...
    result =
      auth.after_authenticate(
        info: { email: user.email }, extra: { response: response }
      )
  ...
  it 'raises an exception when email is missing' do
    ...
    response = OpenStruct.new(identity_url: 'abc')
    expect do
      auth.after_authenticate(info: {}, extra: { response: response })
  ...
  it 'can connect to a different existing user account' do
    ...
    UserOpenId.create!(
      url: 'id/123', user_id: user1.id, email: 'bob@example.com', active: true
    )

    hash = {
      info: { email: user1.email },
      extra: { response: OpenStruct.new(identity_url: 'id/123') }
    }
  ...
  context 'revoke' do
    ...
    it 'revokes correctly' do
      UserOpenId.create!(
        url: 'id/123', user_id: user.id, email: 'bob@example.com', active: true
      )
