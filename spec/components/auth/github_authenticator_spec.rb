...
describe Auth::GithubAuthenticator do
  ...
  context 'after_authenticate' do
    let(:data) do
      {
        ...
        info: { email: user.email, nickname: user.username, name: user.name },
        uid: '100'
      }
    end
    ...
    it 'should use primary email for new user creation over other available emails' do
      hash = {
        ...
        info: { email: 'john@example.com', nickname: 'john', name: 'John Bob' },
        uid: '100'
      }
    ...
    it 'should not error out if user already has a different old github account attached' do
      ...
      GithubUserInfo.create!(
        user_id: user.id, screen_name: 'bob', github_user_id: 100
      )
      ...
      expect(result.user.id).to eq(user.id)
      expect(
        GithubUserInfo.where(user_id: user.id).pluck(:github_user_id)
      ).to eq([1001])
    end
    ...
    it 'will skip blacklisted domains for non existing users' do
      hash = {
        extra: {
          all_emails: [
            {
              email: 'not_allowed@blacklist.com', primary: true, verified: true
            },
            { email: 'allowed@whitelist.com', primary: false, verified: true }
          ]
        },
        info: {
      ...
      hash = {
        extra: {
          all_emails: [
            { email: 'person@example.com', primary: true, verified: true },
            {
              email: 'not_allowed@blacklist.com', primary: false, verified: true
            },
            { email: 'allowed@whitelist.com', primary: false, verified: true }
          ]
        },
    ...
    it 'can connect to a different existing user account' do
      ...
      GithubUserInfo.create!(
        user_id: user1.id, github_user_id: 100, screen_name: 'boris'
      )
  ...
  context 'revoke' do
    ...
    it 'revokes correctly' do
      GithubUserInfo.create!(
        user_id: user.id, github_user_id: 100, screen_name: 'boris'
      )
  ...
  describe 'avatar retrieval' do
    ...
    context 'when user has a custom avatar' do
      let(:user_avatar) do
        Fabricate(:user_avatar, custom_upload: Fabricate(:upload))
      end
      let(:user_with_custom_avatar) do
        Fabricate(:user, user_avatar: user_avatar)
      end

      it 'does not enqueue a download_avatar_from_url job' do
        expect do
          authenticator.after_authenticate(
            auth_token_for(user_with_custom_avatar)
          )
        end.to_not change(job_klass.jobs, :size)
      end
    end
    ...
    context 'when user does not have a custom avatar' do
      it 'enqueues a download_avatar_from_url job' do
        ...
        end.to change(job_klass.jobs, :size).by(1)

        job_args = job_klass.jobs.last['args'].first

        expect(job_args['url']).to eq(
              "https://avatars3.githubusercontent.com/u/#{user.username}"
            )
