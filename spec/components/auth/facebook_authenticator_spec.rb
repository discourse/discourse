  ...
  let(:hash) do
    {
      provider: 'facebook',
      extra: { raw_info: {} },
      info: { email: 'bob@bob.com', first_name: 'Bob', last_name: 'Smith' },
      uid: '100'
    }
  end
  ...
  context 'after_authenticate' do
    it 'can authenticate and create a user record for already existing users' do
      ...
      result =
        authenticator.after_authenticate(
          hash.deep_merge(info: { email: user.email })
        )
    ...
    it 'can connect to a different existing user account' do
      ...
      UserAssociatedAccount.create!(
        provider_name: 'facebook', user_id: user1.id, provider_uid: 100
      )
      ...
      expect(
        UserAssociatedAccount.exists?(
          provider_name: 'facebook', user_id: user1.id
        )
      ).to eq(false)
      expect(
        UserAssociatedAccount.exists?(
          provider_name: 'facebook', user_id: user2.id
        )
      ).to eq(true)
    end
  ...
  context 'description_for_user' do
    ...
    it 'returns correct information' do
      UserAssociatedAccount.create!(
        ...
        user_id: user.id,
        provider_uid: 100,
        info: { email: 'someuser@somedomain.tld' }
      )
      expect(authenticator.description_for_user(user)).to eq(
