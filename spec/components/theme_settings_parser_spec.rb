...
describe ThemeSettingsParser do
  after(:all) { ThemeField.destroy_all }
  ...
  class Loader
    ...
    def load_settings
      yaml =
        File.read(
          "#{Rails.root}/spec/fixtures/theme_settings/valid_settings.yaml"
        )
      field =
  ...
  it 'guesses types correctly' do
    ...
    expect(loader.find_by_name(:integer_setting_03)[:type]).to eq(
          types[:integer]
        )
  ...
  context 'description locale' do
    it 'favors I18n.locale' do
      ...
      expect(loader.find_by_name(:enum_setting_02)[:opts][:description]).to eq(
            'Arabic text'
          )
    end
    ...
    it "uses SiteSetting.default_locale if I18n.locale isn't supported" do
      ...
      expect(
        loader.find_by_name(:integer_setting_02)[:opts][:description]
      ).to eq('Spanish text')
    end
    ...
    it "doesn't set locale if no supported locale is provided" do
      expect(
        loader.find_by_name(:integer_setting_03)[:opts][:description]
      ).to be_nil
    end
  end
