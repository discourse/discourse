...
describe Oneboxer do
  ...
  context 'local oneboxes' do
    ...
    def preview(url, user = nil, category = nil, topic = nil)
      Oneboxer.preview(
        ...
        user_id: user&.id, category_id: category&.id, topic_id: topic&.id
      )
        .to_s
    end
    ...
    it 'links to a topic/post' do
      ...
      public_reply =
        Fabricate(:post, topic: public_topic, post_number: 2, user: replier)
      public_hidden =
        Fabricate(:post, topic: public_topic, post_number: 3, hidden: true)
      ...
      secured_reply =
        Fabricate(:post, user: staff, topic: secured_topic, post_number: 2)

      expect(
        preview(public_topic.relative_url, user, public_category)
      ).to include(public_topic.title)
      ...
      expect(onebox).to include(
            PrettyText.avatar_img(replier.avatar_template, 'tiny')
          )
      ...
      expect(onebox).to include(
            PrettyText.avatar_img(staff.avatar_template, 'tiny')
          )
      ...
      expect(
        preview(secured_topic.relative_url, user, public_category)
      ).to match_html(link(secured_topic.relative_url))
      expect(preview(secured_post.url, user, public_category)).to match_html(
      ...
      expect(
        preview(public_topic.relative_url, user, secured_category)
      ).to match_html(link(public_topic.relative_url))
      ...
      expect(
        preview(public_topic.relative_url, staff, secured_category)
      ).to include(public_topic.title)
      expect(preview(public_post.url, staff, secured_category)).to include(
            public_topic.title
          )
      expect(preview(public_reply.url, staff, secured_category)).to include(
            public_reply.excerpt
          )
      expect(preview(public_hidden.url, staff, secured_category)).to match_html(
            link(public_hidden.url)
          )
      expect(
        preview(secured_topic.relative_url, staff, secured_category)
      ).to include(secured_topic.title)
      expect(preview(secured_post.url, staff, secured_category)).to include(
            secured_topic.title
          )
      expect(preview(secured_reply.url, staff, secured_category)).to include(
            secured_reply.excerpt
          )
      expect(
        preview(secured_reply.url, staff, secured_category, secured_topic)
      ).not_to include(secured_topic.title)
    end

    it 'links to an user profile' do
      user = Fabricate(:user)

      expect(preview('/u/does-not-exist')).to match_html(
            link('/u/does-not-exist')
          )
    ...
    it 'links to an upload' do
      path =
        '/uploads/default/original/3X/e/8/e8fcfa624e4fb6623eea57f54941a58ba797f14d'
  ...
  context '.onebox_raw' do
    it 'should escape the onebox URL before processing' do
      ...
      expect(cpp.html).to eq(
            "<p><a href=\"#{Discourse
              .base_url}/new?%27class=black\">http://test.localhost/new?%27class=black</a></p>"
          )
    end
  end
  ...
  it 'does not crawl blacklisted URLs' do
    ...
    url =
      'https://github.com/discourse/discourse/commit/21b562852885f883be43032e03c709241e8e6d4f'
    stub_request(:head, 'https://discourse.org/').to_return(
      status: 302, body: '', headers: { location: url }
    )

    expect(Oneboxer.external_onebox(url)[:onebox]).to be_empty
    expect(
      Oneboxer.external_onebox('https://discourse.org/')[:onebox]
    ).to be_empty
  end
