...
describe Wizard::StepUpdater do
  ...
  it 'updates the forum title step' do
    updater =
      wizard.create_updater(
        ...
        short_site_description: 'best community'
      )
    updater.update
  ...
  it 'updates the introduction step' do
    ...
    updater =
      wizard.create_updater(
        'introduction',
        welcome: 'Welcome to my new awesome forum!'
      )
    updater.update
  ...
  it "won't allow updates to the default value, when required" do
    updater =
      wizard.create_updater(
        'forum_title',
        title: SiteSetting.title, site_description: 'neat place'
      )
    updater.update
  ...
  context 'contact step' do
    it 'updates the fields correctly' do
      ...
      expect(SiteSetting.contact_url).to eq(
            'http://example.com/custom-contact-url'
          )
      expect(SiteSetting.site_contact_username).to eq(user.username)

      # Should update the TOS topic
      raw =
        Post.where(topic_id: SiteSetting.tos_topic_id, post_number: 1).pluck(
          :raw
        )
          .first
      ...
      updater =
        wizard.create_updater('contact', contact_email: 'alice@example.com')
      updater.update
      raw =
        Post.where(topic_id: SiteSetting.tos_topic_id, post_number: 1).pluck(
          :raw
    ...
    it "doesn't update when there are errors" do
      updater =
        wizard.create_updater(
          'contact',
          contact_email: 'not-an-email', site_contact_username: 'not-a-username'
        )
      updater.update
  ...
  context 'corporate step' do
    it 'updates the fields properly' do
      p =
        Fabricate(
          :post,
          raw: 'company_name - governing_law - city_for_disputes template'
        )
      SiteSetting.tos_topic_id = p.topic_id
      ...
      raw =
        Post.where(topic_id: SiteSetting.tos_topic_id, post_number: 1).pluck(
          ...
          .first
      expect(raw).to eq(
            'ACME, Inc. - New Jersey law - Fairfield, New Jersey template'
          )
      ...
      raw =
        Post.where(topic_id: SiteSetting.tos_topic_id, post_number: 1).pluck(
          ...
          .first
      expect(raw).to eq(
            'Pied Piper Inc - California law - San Francisco, California template'
          )
      ...
      raw =
        Post.where(topic_id: SiteSetting.tos_topic_id, post_number: 1).pluck(
          ...
          .first
      expect(raw).to eq(
            'company_name - governing_law - city_for_disputes template'
          )
  ...
  context 'colors step' do
    context 'with an existing color scheme' do
      let!(:color_scheme) do
        Fabricate(:color_scheme, name: 'existing', via_wizard: true)
      end
    ...
    context 'without an existing theme' do
      ...
      context 'dark theme' do
        it 'creates the theme' do
          updater =
            wizard.create_updater(
              'colors',
              theme_previews: 'Dark', allow_dark_light_selection: true
            )
      ...
      context 'light theme' do
        it 'creates the theme' do
          updater =
            wizard.create_updater('colors', allow_dark_light_selection: true)
  ...
  context 'logos step' do
    it 'updates the fields correctly' do
      ...
      updater =
        wizard.create_updater(
          'logos',
          logo: upload.url, logo_small: upload2.url
        )
  ...
  context 'icons step' do
    it 'updates the fields correctly' do
      ...
      updater =
        wizard.create_updater(
          'icons',
          favicon: upload.url, apple_touch_icon: upload2.url
        )
  ...
  context 'homepage step' do
    it 'updates the fields correctly' do
      ...
      expect(SiteSetting.desktop_category_page_style).to eq(
  ...
  context 'invites step' do
    let(:invites) do
      return [
        { email: 'regular@example.com', role: 'regular' },
