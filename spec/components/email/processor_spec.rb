...
describe Email::Processor do
  ...
  context 'when reply via email is too short' do
    ...
    let!(:post_reply_key) do
      Fabricate(
        :post_reply_key,
        user: user, post: post, reply_key: '4f97315cc828096c9cb34c6f1a0d6fe8'
      )
    end
    ...
    it 'rejects reply and sends an email with custom error message' do
      ...
      expect(rejection_raw.gsub(/\r/, '')).to eq(
            I18n.t(
              ...
              count: count, destination: destination, former_title: former_title
            )
              .gsub(/\r/, '')
          )
    end
  ...
  describe 'rate limits' do
    ...
    it "doesn't enqueue a background job when retry is disabled" do
      ...
      expect { Email::Processor.process!(mail, false) }.to raise_error(
            limit_exceeded
          )
    end
  end
  ...
  context 'known error' do
    ...
    it 'only sends one rejection email per day' do
      ...
      expect { Email::Processor.process!(mail) }.to change { EmailLog.count }
            .by(1)

      expect { Email::Processor.process!(mail2) }.to change { EmailLog.count }
            .by(0)
      ...
      expect { Email::Processor.process!(mail3) }.to change { EmailLog.count }
            .by(1)
    end
  end
  ...
  context 'unrecognized error' do
    ...
    it 'sends a rejection email on an unrecognized error' do
      begin
        ...
        Email::Processor.any_instance.stubs(:can_send_rejection_email?).returns(
          true
        )
        ...
        expect(EmailLog.last.email_type).to eq(
    ...
    it 'sends more than one rejection email per day' do
      Email::Receiver.any_instance.stubs(:process_internal).raises('boom')
      key =
        "rejection_email:#{[from]}:email_reject_unrecognized_error:#{Date
          .today}"
      $redis.expire(key, 0)
  ...
  context 'from reply to email address' do
    ...
    it 'ignores the email' do
      Email::Receiver.any_instance.stubs(:process_internal).raises(
        Email::Receiver::FromReplyByAddressError.new
      )
  ...
  context 'mailinglist mirror' do
    ...
    it 'does not send rejection email' do
      ...
      expect { Email::Processor.process!(email) }.to_not change do
        EmailLog.count
      end
    end
  end
