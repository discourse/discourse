...
describe Email::Sender do
  ...
  it "doesn't deliver when the to address uses the .invalid tld" do
    ...
    message.expects(:deliver_now).never
    expect { Email::Sender.new(message, :hello).send }.to change do
  ...
  context 'host_for' do
    ...
    it 'returns localhost for a weird host' do
      expect(Email::Sender.host_for('this is not a real host')).to eq(
            'localhost'
          )
    end

    it 'parses hosts from urls' do
      expect(Email::Sender.host_for('http://meta.discourse.org')).to eq(
            'meta.discourse.org'
          )
    end

    it 'downcases hosts' do
      expect(Email::Sender.host_for('http://ForumSite.com')).to eq(
            'forumsite.com'
          )
    end
  end
  ...
  context 'with a valid message' do
    ...
    context 'email threading' do
      ...
      it "sets the 'In-Reply-To' header to the newest replied post" do
        ...
        expect(message.header['Message-Id'].to_s).to eq(
              "<topic/#{topic.id}/#{post_4.id}@test.localhost>"
            )
        expect(message.header['In-Reply-To'].to_s).to eq(
      ...
      it 'uses the incoming_email message_id when available' do
        topic_incoming_email =
          IncomingEmail.create(
            topic: topic, post: post_1, message_id: 'foo@bar'
          )
        post_2_incoming_email =
          IncomingEmail.create(
            topic: topic, post: post_2, message_id: 'bar@foo'
          )
        post_4_incoming_email =
          IncomingEmail.create(
            topic: topic, post: post_4, message_id: 'wat@wat'
          )
        ...
        expect(message.header['Message-Id'].to_s).to eq(
              "<#{post_4_incoming_email.message_id}>"
            )
    ...
    context 'merges custom mandrill header' do
      ...
      it 'should set the right header' do
        email_sender.send
        expect(message.header['X-MC-Metadata'].to_s).to match(
              message.message_id
            )
      end
    end
    ...
    context 'email parts' do
      it 'should contain the right message' do
        ...
        expect(message.text_part.content_type).to eq(
  ...
  context 'with a deleted post' do
    it 'should skip sending the email' do
      ...
      expect(log.reason_type).to eq(
            SkippedEmailLog.reason_types[:sender_post_deleted]
          )
    end
  end
