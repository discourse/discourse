...
describe Email::Sender do
  ...
  context 'with a valid message' do
    ...
    context "email threading" do
      let(:random_message_id_chunk) { "5f1330cfd941f323d7f99b9e" }
      fab!(:topic) { Fabricate(:topic) }
      ...
      before do
        message.header['X-Discourse-Topic-Id'] = topic.id
        Email::MessageIdGenerator.stubs(:random_chunk).returns(random_message_id_chunk)
      end
      ...
      it "doesn't set the 'In-Reply-To' and 'References' headers on the first post" do
        ...
        expect(message.header['Message-Id'].to_s).to eq("<topic/#{topic.id}.#{random_message_id_chunk}@test.localhost>")
      ...
      it "sets the 'In-Reply-To' header to the topic by default" do
        ...
        expect(message.header['Message-Id'].to_s).to eq("<topic/#{topic.id}/#{post_2.id}.#{random_message_id_chunk}@test.localhost>")
        expect(message.header['In-Reply-To'].to_s).to eq("<topic/#{topic.id}.#{random_message_id_chunk}@test.localhost>")
      end
      ...
      it "sets the 'In-Reply-To' header to the newest replied post" do
        ...
        expect(message.header['Message-Id'].to_s).to eq("<topic/#{topic.id}/#{post_4.id}.#{random_message_id_chunk}@test.localhost>")
        expect(message.header['In-Reply-To'].to_s).to eq("<topic/#{topic.id}/#{post_3.id}.#{random_message_id_chunk}@test.localhost>")
      end
      ...
      it "sets the 'References' header to the topic and all replied posts" do
        ...
        references = [
          "<topic/#{topic.id}.#{random_message_id_chunk}@test.localhost>",
          "<topic/#{topic.id}/#{post_3.id}.#{random_message_id_chunk}@test.localhost>",
          "<topic/#{topic.id}/#{post_2.id}.#{random_message_id_chunk}@test.localhost>",
        ]
