...
describe Email::MessageBuilder do
  ...
  let(:body) do
    'oh my glob Jake, Tree Trunks just made the tastiest apple pie ever!'
  end
  let(:builder) do
    Email::MessageBuilder.new(to_address, subject: subject, body: body)
  end
    ...
    context 'with allow_reply_by_email' do
      ...
      context 'With the SiteSetting enabled' do
        before do
          ...
          SiteSetting.stubs(:reply_by_email_address).returns(
            'r+%{reply_key}@reply.myforum.com'
          )
        end
        ...
        it 'returns a Reply-To header with the reply key' do
          ...
          expect(reply_by_email_builder.header_args[allow_reply_header]).to eq(
                true
              )
        end

        it 'cleans up the site title' do
          SiteSetting.stubs(:title).returns(
            '>>>Obnoxious Title: Deal, \"With\" It<<<'
          )
  ...
  context 'header args' do
    let(:message_with_header_args) do
      Email::MessageBuilder.new(
        to_address,
        body: 'hello world', topic_id: 1234, post_id: 4567
      )
    end

    it 'passes through a post_id' do
      expect(message_with_header_args.header_args['X-Discourse-Post-Id']).to eq(
            '4567'
          )
    end
    ...
    it 'passes through a topic_id' do
      expect(
        message_with_header_args.header_args['X-Discourse-Topic-Id']
      ).to eq('1234')
    end
  end
  ...
  context 'unsubscribe link' do
    context 'with add_unsubscribe_link false' do
      ...
      it "doesn't have the user preferences url in the body" do
        expect(builder.body).not_to match(
                  builder.template_args[:user_preferences_url]
                )
      end
    end
    ...
    context 'with add_unsubscribe_link true' do
      ...
      it 'has an List-Unsubscribe header' do
        expect(
          message_with_unsubscribe.header_args['List-Unsubscribe']
        ).to be_present
      end
      ...
      it 'does not add unsubscribe via email link without site setting set' do
        expect(message_with_unsubscribe.body).to_not match(
                  /mailto:reply@#{Discourse
                    .current_hostname}\?subject=unsubscribe/
                )
      end
    end
  end
  ...
  context 'template_args' do
    ...
    it 'has the user_preferences_url' do
      expect(template_args[:user_preferences_url]).to eq(
            "#{Discourse.base_url}/my/preferences"
          )
    end
  end
  ...
  context 'email prefix in subject' do
    context 'when use_site_subject is true' do
      let(:message_with_email_prefix) do
        Email::MessageBuilder.new(
          to_address,
          body: 'hello world', use_site_subject: true
        )
      end

      it 'when email_prefix is set it should be present in subject' do
        SiteSetting.email_prefix = 'some email prefix'
        expect(message_with_email_prefix.subject).to match(
              SiteSetting.email_prefix
            )
      end
    end
  end
  ...
  context 'subject_template' do
    ...
    it 'has the body rendered from a template' do
      I18n.expects(:t).with(
        ...
        templated_builder.template_args
      )
  ...
  context 'from field' do
    ...
    it 'title setting will be added if present' do
      ...
      expect(build_args[:from]).to eq(
            "\"Dog Talk\" <#{SiteSetting.notification_email}>"
          )
    end

    let(:finn_email) { 'finn@adventuretime.ooo' }
    let(:custom_from) do
      Email::MessageBuilder.new(to_address, from: finn_email).build_args
    end
    ...
    let(:aliased_from) do
      Email::MessageBuilder.new(to_address, from_alias: 'Finn the Dog')
    end
    ...
    let(:custom_aliased_from) do
      Email::MessageBuilder.new(
        to_address,
        from_alias: 'Finn the Dog', from: finn_email
      )
    end

    it 'allows us to alias a custom from address' do
      expect(custom_aliased_from.build_args[:from]).to eq(
            "\"Finn the Dog\" <#{finn_email}>"
          )
    end
    ...
    it "email_site_title will be added if it's set" do
      ...
      expect(build_args[:from]).to eq(
            "\"The Forum\" <#{SiteSetting.notification_email}>"
          )
    end
    ...
    it 'cleans up aliases in the from_alias arg' do
      builder =
        Email::MessageBuilder.new(
          to_address,
          from_alias: 'Finn: the Dog, <3', from: finn_email
        )
      expect(builder.build_args[:from]).to eq(
            "\"Finn the Dog 3\" <#{finn_email}>"
          )
    end

    it 'cleans up the email_site_title' do
      SiteSetting.stubs(:email_site_title).returns(
        '::>>>Best \"Forum\", EU: Award Winning<<<'
      )
      expect(build_args[:from]).to eq(
            "\"Best Forum EU Award Winning\" <#{SiteSetting
              .notification_email}>"
          )
    end
  end
