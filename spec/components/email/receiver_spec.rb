...
describe Email::Receiver do
  ...
  it "raises an EmptyEmailError when 'mail_string' is blank" do
    expect { Email::Receiver.new(nil) }.to raise_error(
          Email::Receiver::EmptyEmailError
        )
    expect { Email::Receiver.new('') }.to raise_error(
          Email::Receiver::EmptyEmailError
        )
  end

  it 'raises a ScreenedEmailError when email address is screened' do
    ScreenedEmail.expects(:should_block?).with('screened@mail.com').returns(
      true
    )
    expect { process(:screened_email) }.to raise_error(
  ...
  it 'raises EmailNotAllowed when email address is not on whitelist' do
    ...
    expect { process(:blacklist_whitelist_email) }.to raise_error(
  ...
  it 'raises an UserNotFoundError when staged users are disabled' do
    ...
    expect { process(:user_not_found) }.to raise_error(
  ...
  it 'raises a SilencedUserError when the sender has been silenced' do
    ...
    expect { process(:silenced_sender) }.to raise_error(
          Email::Receiver::SilencedUserError
        )
  end
  ...
  it "doesn't raise an InactiveUserError when the sender is staged" do
    ...
    post_reply_key =
      Fabricate(
        :post_reply_key,
        user: user, post: post, reply_key: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
      )
  ...
  it "raises a BadDestinationAddress when destinations aren't matching any of the incoming emails" do
    expect { process(:bad_destinations) }.to raise_error(
  ...
  context 'bounces' do
    it 'raises a BouncerEmailError' do
      expect { process(:bounced_email) }.to raise_error(
      ...
      it 'when bounce without verp' do
        ...
        expect(post.raw).to eq(
              I18n.t(
                ...
                email: email_address, raw: 'Your email bounced'
              )
                .strip
            )
        expect(IncomingEmail.last.is_bounce).to eq(true)
  ...
  it 'logs a blank error' do
    Email::Receiver.any_instance.stubs(:process_internal).raises(
      RuntimeError,
      ''
    )
    begin
  ...
  it 'matches the correct user' do
    user = Fabricate(:user)
    email_log =
      Fabricate(:email_log, to_address: user.email, user: user, bounce_key: nil)
  ...
  it 'strips null bytes from the subject' do
    expect { process(:null_byte_in_subject) }.to raise_error(
          Email::Receiver::BadDestinationAddress
        )
  end
  ...
  context 'bounces to VERP' do
    ...
    let!(:email_log) do
      Fabricate(
        :email_log,
        to_address: user.email, user: user, bounce_key: bounce_key
      )
    end
    let!(:email_log_2) do
      Fabricate(
        :email_log,
        to_address: user.email, user: user, bounce_key: bounce_key_2
      )
    end

    it 'deals with soft bounces' do
      expect { process(:soft_bounce_via_verp) }.to raise_error(
            Email::Receiver::BouncedEmailError
          )
    ...
    it 'deals with hard bounces' do
      ...
      expect { process(:hard_bounce_via_verp_2) }.to raise_error(
            Email::Receiver::BouncedEmailError
          )

      email_log_2.reload
      expect(email_log_2.user.user_stat.bounce_score).to eq(
            SiteSetting.hard_bounce_score * 2
          )
    ...
    it "automatically deactive users once they reach the 'bounce_score_threshold_deactivate' threshold" do
      ...
      user.user_stat.bounce_score =
        SiteSetting.bounce_score_threshold_deactivate - 1
      user.user_stat.save!
  ...
  context 'reply' do
    ...
    it "uses MD5 of 'mail_string' there is no message_id" do
      ...
      expect { Email::Receiver.new(mail_string).process! }.to change do
        IncomingEmail.count
      end
      expect(IncomingEmail.last.message_id).to eq(
            Digest::MD5.hexdigest(mail_string)
          )
    end
    ...
    it 'accepts reply from secondary email address' do
      ...
      expect { process(:reply_user_not_matching) }.to change do
        topic.posts.count
      end
    ...
    it 'raises a TopicNotFoundError when the topic was deleted' do
      ...
      expect { process(:reply_user_matching) }.to raise_error(
    ...
    it 'raises an InvalidPost when there was an error while creating the post' do
      expect { process(:too_small) }.to raise_error(
    ...
    it 'raises an InvalidPost when there are too may mentions' do
      ...
      expect { process(:too_many_mentions) }.to raise_error(
    ...
    it "raises an InvalidPostAction when they aren't allowed to like a post" do
      ...
      expect { process(:like) }.to raise_error(
    ...
    it 'handles different encodings correctly' do
      ...
      expect { process(:reply_with_weird_encoding) }.to change do
        topic.posts.count
      end
      expect(topic.posts.last.raw).to eq(
      ...
      expect { process(:reply_with_8bit_encoding) }.to change do
        topic.posts.count
      end
      expect(topic.posts.last.raw).to eq(
    ...
    it "raises a NoSenderDetectedError when the From header doesn't contain an email address" do
      expect { process(:invalid_from_2) }.to raise_error(
            Email::Receiver::NoSenderDetectedError
          )
    end
    ...
    it "doesn't raise an AutoGeneratedEmailError when the mail is auto generated but is whitelisted" do
      ...
      expect { process(:auto_generated_whitelisted) }.to change do
        topic.posts.count
      end
    end
    ...
    it "doesn't raise an AutoGeneratedEmailError when block_auto_generated_emails is disabled" do
      ...
      expect { process(:auto_generated_unblocked) }.to change do
        topic.posts.count
      end
    end
    ...
    it 'allows staged users to reply to a restricted category' do
      ...
      expect { process(:staged_reply_restricted) }.to change do
        topic.posts.count
      end
    end
    ...
    describe 'Unsubscribing via email' do
      ...
      describe 'unsubscribe_subject.eml' do
        ...
        it 'does nothing unless unsubscribe_via_email is turned on' do
          ...
          expect { process('unsubscribe_subject') }.to raise_error do
            Email::Receiver::BadDestinationAddress
          end
      ...
      describe 'unsubscribe_body.eml' do
        it 'sends an email asking the user to confirm the unsubscription' do
          expect { process('unsubscribe_body') }.to change do
            ActionMailer::Base.deliveries.count
          end
            .by(1)
          expect(last_email.to.length).to eq 1
          ...
          expect(last_email.from).to include "noreply@#{Discourse
                        .current_hostname}"
          expect(last_email.to).to include 'discourse@bar.com'
          expect(last_email.subject).to eq I18n.t(
                   :"unsubscribe_mailer.subject_template"
                 )
                   .gsub('%{site_title}', SiteSetting.title)
        end

        it 'does nothing unless unsubscribe_via_email is turned on' do
          SiteSetting.unsubscribe_via_email = false
          before_deliveries = ActionMailer::Base.deliveries.count
          expect { process('unsubscribe_body') }.to raise_error do
            Email::Receiver::InvalidPost
          end
          expect(before_deliveries).to eq ActionMailer::Base.deliveries.count
        end
      ...
      it 'raises an UnsubscribeNotAllowed and does not send an unsubscribe email' do
        before_deliveries = ActionMailer::Base.deliveries.count
        expect { process(:unsubscribe_new_user) }.to raise_error do
    ...
    it 'supports attachments' do
      ...
      expect(post.raw).to match(
            %r{\APlease find some text file attached\.\s+<a class='attachment' href='\/uploads\/default\/original\/.+?txt'>text\.txt<\/a> \(20 Bytes\)\z}
          )
      ...
      post = topic.posts.last
      expect(post.raw).to match(
            %r{\APicture below\.\s+<img.+?src="\/uploads\/default\/original\/.+?jpeg" class="">\s+Picture above\.\z}
          )
      expect(post.uploads.size).to eq 1
    end
    ...
    it 'supports eml attachments' do
      ...
      post = topic.posts.last
      expect(post.raw).to match(
            %r{\APlease find the eml file attached\.\s+<a class='attachment' href='\/uploads\/default\/original\/.+?eml'>sample\.eml<\/a> \(193 Bytes\)\z}
          )
      expect(post.uploads.size).to eq 1
    end
    ...
    context 'when attachment is rejected' do
      it 'sends out the warning email' do
        expect { process(:attached_txt_file) }.to change { EmailLog.count }.by(
              1
            )
      ...
      it 'creates the post with attachment missing message' do
        missing_attachment_regex =
          Regexp.escape(
            I18n.t('emails.incoming.missing_attachment', filename: 'text.txt')
          )
        expect { process(:attached_txt_file) }.to change { topic.posts.count }
        post = topic.posts.last
    ...
    it 'supports emails with just an attachment' do
      ...
      post = topic.posts.last
      expect(post.raw).to match(
            %r{\A\s+<a class='attachment' href='\/uploads\/default\/original\/.+?pdf'>discourse\.pdf<\/a> \(64 KB\)\z}
          )
      expect(post.uploads.size).to eq 1
    end
    ...
    it "ensures posts aren't dated in the future" do
      ...
      expect(topic.posts.last.created_at).to be_within(1.minute).of(
            DateTime.now
          )
    end
    ...
    it 'accepts emails with wrong reply key if the system knows about the forwarded email' do
      ...
      Fabricate(
        ...
        to_addresses:
          'dave@bar.com;reply+4f97315cc828096c9cb34c6f1a0d6fe8@bar.com',
      ...
      expect { process(:reply_user_not_matching_but_known) }.to change do
        topic.posts.count
      end
    end
  ...
  context 'new message to a group' do
    let!(:group) do
      Fabricate(:group, incoming_email: 'team@bar.com|meat@bar.com')
    end
    ...
    it "invites everyone in the chain but emails configured as 'incoming' (via reply, group or category)" do
      ...
      emails =
        topic.allowed_users.joins(:user_emails).pluck(:"user_emails.email")
    ...
    it 'cap the number of staged users created per email' do
      ...
      expect { process(:cc) }.to change(Topic, :count)
      expect(Topic.last.ordered_posts[-1].post_type).to eq(
            Post.types[:moderator_action]
          )
    end
    ...
    describe "when 'find_related_post_with_key' is disabled" do
      ...
      it "associates email replies using both 'In-Reply-To' and 'References' headers" do
        ...
        ordered_posts[1..-1].each do |post|
          ...
          expect(
            %w[two three].include?(post.custom_fields['action_code_who'])
          ).to eq(true)
        end
    ...
    it "supports any kind of attachments when 'allow_all_attachments_for_group_messages' is enabled" do
      ...
      expect(Post.last.raw).to match(
            %r{<a\sclass='attachment'[^>]*>discourse\.rb<\/a>}
          )
    ...
    context 'with forwarded emails enabled' do
      ...
      it 'handles forwarded emails' do
        ...
        expect(last_post.raw).to match(
              /can you have a look at this email below/
            )
    ...
    context 'when message sent to a group has no key and find_related_post_with_key is enabled' do
      ...
      it 'creates a reply when the sender and referenced message id are known' do
        expect { process(:email_reply_2) }.to change { topic.posts.count }.by(
              1
            ).and change { Topic.count }.by(0)
      end

      it 'creates a new topic when the sender is not known' do
        IncomingEmail.where(message_id: '34@foo.bar.mail').update(
          cc_addresses: 'three@foo.com'
        )
        expect { process(:email_reply_2) }.to change { topic.posts.count }.by(
              0
  ...
  context 'new topic in a category' do
    ...
    it "raises a StrangersNotAllowedError when 'email_in_allow_strangers' is disabled" do
      expect { process(:new_user) }.to raise_error(
    ...
    it 'works' do
      user =
        Fabricate(
          :user,
          email: 'existing@bar.com', trust_level: SiteSetting.email_in_min_trust
        )
    ...
    it 'creates hidden topic for X-Spam-Flag' do
      ...
      expect(post.hidden_reason_id).to eq(
    ...
    it "adds the 'elided' part of the original message when always_show_trimmed_content is enabled" do
      ...
      expect { process(:forwarded_email_to_category) }.to change { Topic.count }
            .by(1) # Topic created
  ...
  context 'new topic in a category that allows strangers' do
    ...
    it 'fails on email from a low-TL user' do
      SiteSetting.email_in_min_trust = 4
      Fabricate(:user, email: 'tl3@bar.com', trust_level: TrustLevel[3])
      expect { process(:tl3_user) }.to raise_error(
            Email::Receiver::InsufficientTrustLevelError
          )
    end
  end
  ...
  context '#reply_by_email_address_regex' do
    ...
    it "uses 'alternative_reply_by_email_addresses' site setting" do
      SiteSetting.alternative_reply_by_email_addresses =
        'alt.foo+%{reply_key}@bar.com'
      expect(Email::Receiver.reply_by_email_address_regex).to eq(
            /alt\.foo\+?(\h{32})?@bar\.com/
          )
    end
  ...
  context 'check_address' do
    ...
    it 'returns nil when the key is invalid' do
      ...
      expect(
        Email::Receiver.check_address(
          'foo+4f97315cc828096c9cb34c6f1a0d6fe8@bar.com'
        )
      ).to be_nil
    end
  ...
  context 'staged users' do
    ...
    context 'when email address is screened' do
      ...
      include_examples 'no staged users',
                       :screened_email,
                       Email::Receiver::ScreenedEmailError
    end
    ...
    context 'when From email address is not on whitelist' do
      before { SiteSetting.email_domains_whitelist = 'example.com|bar.com' }

      include_examples 'no staged users',
                       :blacklist_whitelist_email,
                       Email::Receiver::EmailNotAllowed
    end

    context 'when From email address is on blacklist' do
      before { SiteSetting.email_domains_blacklist = 'email.com|mail.com' }
    ...
    context 'when email is sent to category' do
      context 'when email is sent by a new user and category does not allow strangers' do
        let!(:category) do
          Fabricate(
            :category,
            email_in: 'category@foo.com', email_in_allow_strangers: false
          )
        end
      ...
      context 'when email has no date' do
        let!(:category) do
          Fabricate(
            :category,
            email_in: 'category@foo.com', email_in_allow_strangers: true
          )
        end
    ...
    it 'does not remove the incoming email record when staged users are deleted' do
      expect { process(:bad_destinations) }.to change do
        IncomingEmail.count
      end
        .and
  ...
  context 'mailing list mirror' do
    ...
    it 'ignores unsubscribe email' do
      ...
      expect { process('mailinglist_unsubscribe') }.to_not change do
        ActionMailer::Base.deliveries.count
      end
    end
  end
