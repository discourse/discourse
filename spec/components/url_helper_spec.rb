...
describe UrlHelper do
  ...
  describe '#is_local' do
    it 'is true when the file has been uploaded' do
      ...
      expect(
        UrlHelper.is_local('http://discuss.site.com/path/to/file.png')
      ).to eq(true)
    end
  ...
  describe '#absolute' do
    it 'returns an absolute URL for CDN' do
      begin
        ...
        expect(UrlHelper.absolute('/test.jpg')).to eq(
              'https://cdn.awesome.com/test.jpg'
            )

        Rails.configuration.action_controller.asset_host =
          'https://cdn.awesome.com'
        expect(UrlHelper.absolute('/test.jpg')).to eq(
              'https://cdn.awesome.com/test.jpg'
            )

        Rails.configuration.action_controller.asset_host =
          'http://cdn.awesome.com'
        expect(UrlHelper.absolute('/test.jpg')).to eq(
              'http://cdn.awesome.com/test.jpg'
            )
      ensure
    ...
    it 'does not change non-relative url' do
      expect(UrlHelper.absolute('http://www.discourse.org')).to eq(
            'http://www.discourse.org'
          )
    end

    it 'changes a relative url to an absolute one using base url by default' do
      expect(UrlHelper.absolute('/path/to/file')).to eq(
  ...
  describe '#schemaless' do
    it 'removes http schemas only' do
      expect(UrlHelper.schemaless('http://www.discourse.org')).to eq(
            '//www.discourse.org'
          )
      expect(UrlHelper.schemaless('https://secure.discourse.org')).to eq(
            'https://secure.discourse.org'
          )
      expect(UrlHelper.schemaless('ftp://ftp.discourse.org')).to eq(
            'ftp://ftp.discourse.org'
          )
    end
  end
