...
describe FinalDestination do
  let(:opts) do
    {
      ignore_redirects: %w[https://ignore-me.com],
      force_get_hosts: %w[https://force.get.com],
      preserve_fragment_url_hosts: %w[https://eviltrout.com],
      lookup_ip:
        lambda do |# avoid IP lookups in test
        host|
          case host
          when 'eviltrout.com'
            '52.84.143.152'
          when 'codinghorror.com'
            '91.146.108.148'
          when 'discourse.org'
            '104.25.152.10'
          when 'some_thing.example.com'
            '104.25.152.10'
          when 'private-host.com'
            '192.168.10.1'
          when 'internal-ipv6.com'
            '2001:abc:de:01:3:3d0:6a65:c2bf'
          when 'ignore-me.com'
            '53.84.143.152'
          when 'force.get.com'
            '22.102.29.40'
  ...
  def redirect_response(from, dest)
    stub_request(:head, from).to_return(
      status: 302, headers: { 'Location' => dest }
    )
  end
  ...
  describe '.resolve' do
    ...
    context 'underscores in URLs' do
      before do
        stub_request(:head, 'https://some_thing.example.com').to_return(
          doc_response
        )
      end
    ...
    context 'GET can be forced' do
      ...
      it 'will do a GET when forced' do
        ...
        expect(WebMock).to have_requested(
              :get,
              'https://force.get.com/posts?page=4'
            )
      ...
      it 'will do a HEAD if not forced' do
        ...
        expect(WebMock).to_not have_requested(
                  :get,
                  'https://eviltrout.com/posts?page=2'
                )
        expect(WebMock).to have_requested(
              :head,
              'https://eviltrout.com/posts?page=2'
    ...
    context 'HEAD not supported' do
      before do
        stub_request(:get, 'https://eviltrout.com').to_return(
          status: 301,
          headers: {
            'Location' => 'https://discourse.org', 'Set-Cookie' => 'evil=trout'
          }
        )
      ...
      it 'correctly extracts cookies during GET' do
        ...
        stub_request(:head, 'https://eviltrout.com').with(
          headers: { 'Cookie' => 'bar=1; baz=2; foo=219ffwef9w0f' }
        )
    ...
    it 'should use the correct format for cookies when there is only one cookie' do
      stub_request(:head, 'https://eviltrout.com').to_return(
        status: 302,
        headers: {
          'Location' => 'https://eviltrout.com',
          'Set-Cookie' =>
      ...
      stub_request(:head, 'https://eviltrout.com').with(
        headers: { 'Cookie' => 'foo=219ffwef9w0f' }
      )
  ...
  describe '.get' do
    it 'can correctly stream with a redirect' do
      ...
      result =
        FinalDestination.new('http://wikipedia.com', opts).get do |resp, c|
          chunk = c
          throw :done
        end
  ...
  describe '.validate_url_format' do
    ...
    it "doesn't support IP urls" do
      ...
      expect(
        fd('https://[2001:abc:de:01:0:3f0:6a65:c2bf]').validate_uri_format
      ).to eq(false)
    end
  ...
  describe '.is_dest_valid' do
    ...
    it 'returns false for short ip' do
      ...
      expect(
        FinalDestination.new('https://0/logo.png', lookup_ip: lookup)
          .is_dest_valid?
      ).to eq(false)
      expect(
        FinalDestination.new('https://1/logo.png', lookup_ip: lookup)
          .is_dest_valid?
      ).to eq(false)
    end
    ...
    it 'returns false for IPV6 via site settings' do
      ...
      expect(
        fd('https://[2001:abc:de:01:0:3f0:6a65:c2bf]').is_dest_valid?
      ).to eq(false)
      expect(
        fd('https://[2002:abc:de:01:0:3f0:6a65:c2bf]').is_dest_valid?
      ).to eq(false)
      expect(fd('https://internal-ipv6.com').is_dest_valid?).to eq(false)
      expect(
        fd('https://[2003:abc:de:01:0:3f0:6a65:c2bf]').is_dest_valid?
      ).to eq(true)
    end
    ...
    it 'returns true for the base uri' do
      ...
      expect(fd('https://final-test.example.com/onebox').is_dest_valid?).to eq(
            true
          )
    end
    ...
    it 'returns true for the CDN url' do
      ...
      expect(fd('https://cdn.example.com/some/asset').is_dest_valid?).to eq(
            true
          )
    end
  ...
  describe 'https cache' do
    it 'will cache https lookups' do
      ...
      stub_request(:head, 'http://wikipedia.com/image.png').to_return(
        status: 302,
        body: '',
        headers: { location: 'https://wikipedia.com/image.png' }
      )
  ...
  describe '#escape_url' do
    it 'correctly escapes url' do
      ...
      expect(
        fd('https://eviltrout.com?s=180&#038;d=mm&#038;r=g').escape_url.to_s
      ).to eq('https://eviltrout.com?s=180&d=mm&r=g')

      expect(fd("http://example.com/?a=\11\15").escape_url.to_s).to eq(
            'http://example.com/?a=%09%0D'
          )

      expect(
        fd('https://ru.wikipedia.org/wiki/%D0%A1%D0%B2%D0%BE%D0%B1%D0%BE')
          .escape_url
          .to_s
      ).to eq('https://ru.wikipedia.org/wiki/%D0%A1%D0%B2%D0%BE%D0%B1%D0%BE')

      expect(fd('https://ru.wikipedia.org/wiki/Свобо').escape_url.to_s).to eq(
