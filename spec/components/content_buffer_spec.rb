...
describe ContentBuffer do
  it 'handles deletion across lines properly' do
    ...
    c.apply_transform!(
      start: { row: 0, col: 0 }, finish: { col: 1, row: 1 }, operation: :delete
    )
  ...
  it 'handles deletion inside lines properly' do
    ...
    c.apply_transform!(
      start: { row: 0, col: 1 }, finish: { col: 4, row: 0 }, operation: :delete
    )
  ...
  it 'handles inserts inside lines properly' do
    ...
    c.apply_transform!(
      start: { row: 0, col: 5 }, operation: :insert, text: ' world'
    )
  ...
  it 'handles multiline inserts' do
    c = ContentBuffer.new('hello!')
    c.apply_transform!(
      start: { row: 0, col: 5 }, operation: :insert, text: "\nworld"
    )
