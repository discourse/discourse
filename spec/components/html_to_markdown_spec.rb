...
describe HtmlToMarkdown do
  ...
  it 'skips hidden tags' do
    expect(
      html_to_markdown(
        '<p>Hello <span style="display: none">cruel </span>World!</p>'
      )
    ).to eq('Hello World!')
  end
  ...
  it 'converts <a>' do
    expect(
      html_to_markdown('<a href="https://www.discourse.org">Discourse</a>')
    ).to eq('[Discourse](https://www.discourse.org)')
  end

  it 'removes empty & invalid <a>' do
    expect(html_to_markdown('<a>Discourse</a>')).to eq('Discourse')
    expect(html_to_markdown('<a href="">Discourse</a>')).to eq('Discourse')
    expect(html_to_markdown('<a href="foo.bar">Discourse</a>')).to eq(
          'Discourse'
        )
  end

  HTML_WITH_IMG ||=
    '<img src="https://www.discourse.org/logo.svg" alt="Discourse Logo">'
  HTML_WITH_CID_IMG ||=
    '<img src="cid:ii_1525434659ddb4cb" alt="Discourse Logo">'
  ...
  it "keeps <img> with 'keep_img_tags'" do
    expect(html_to_markdown(HTML_WITH_IMG, keep_img_tags: true)).to eq(
          HTML_WITH_IMG
        )
  end
  ...
  it 'removes empty & invalid <img>' do
    ...
    expect(html_to_markdown('<img src="">')).to eq('')
    expect(html_to_markdown('<img src="foo.bar">')).to eq('')
  end

  it "keeps <img> with src='cid:' whith 'keep_cid_imgs'" do
    expect(html_to_markdown(HTML_WITH_CID_IMG, keep_cid_imgs: true)).to eq(
          '![Discourse Logo](cid:ii_1525434659ddb4cb)'
        )
    expect(
      html_to_markdown(
        HTML_WITH_CID_IMG,
        keep_img_tags: true, keep_cid_imgs: true
      )
    ).to eq('<img src=\"cid:ii_1525434659ddb4cb\" alt=\"Discourse Logo\">')
  end

  it 'skips hidden <img>' do
    expect(
      html_to_markdown('<img src="https://www.discourse.org/logo.svg" width=0>')
    ).to eq('')
    expect(
      html_to_markdown(
        '<img src="https://www.discourse.org/logo.svg" height="0">'
      )
    ).to eq('')
    expect(
      html_to_markdown(
        '<img src="https://www.discourse.org/logo.svg" style="width: 0">'
      )
    ).to eq('')
    expect(
      html_to_markdown(
        '<img src="https://www.discourse.org/logo.svg" style="height:0px">'
      )
    ).to eq('')
  end
  ...
  (1..6).each do |n|
    it "converts <h#{n}>" do
      expect(html_to_markdown("<h#{n}>Header #{n}</h#{n}>")).to eq(
            '#' * n + " Header #{n}"
          )
    end
  end
  ...
  it 'converts <br>' do
    expect(html_to_markdown('Before<br>Inside<br>After')).to eq(
          "Before\nInside\nAfter"
        )
  end
  ...
  it 'supports <del>' do
    expect(html_to_markdown('This is a <del>deletion</del>')).to eq(
          'This is a <del>deletion</del>'
        )
  end
  ...
  it 'supports <sup>' do
    expect(html_to_markdown('<sup>Super Script!</sup>')).to eq(
          '<sup>Super Script!</sup>'
        )
  end

  it 'supports <small>' do
    expect(html_to_markdown('<small>Small</small>')).to eq(
          '<small>Small</small>'
        )
  end

  it 'supports <kbd>' do
    expect(html_to_markdown('<kbd>CTRL</kbd>+<kbd>C</kbd>')).to eq(
  ...
  it 'supports <abbr>' do
    expect(
      html_to_markdown(
        '<abbr title="Civilized Discourse Construction Kit, Inc.">CDCK</abbr>'
      )
    ).to eq(
          '<abbr title="Civilized Discourse Construction Kit, Inc.">CDCK</abbr>'
        )
  ...
  it 'supports <s>' do
    expect(html_to_markdown('<s>Strike Through</s>')).to eq(
          '<s>Strike Through</s>'
        )
  end

  it 'supports <strike>' do
    expect(html_to_markdown('<strike>Strike Through</strike>')).to eq(
  ...
  it 'supports <ul>' do
    expect(
      html_to_markdown('<ul><li>🍏</li><li>🍐</li><li>🍌</li></ul>')
    ).to eq("- 🍏\n- 🍐\n- 🍌")
    expect(
      html_to_markdown("<ul>\n<li>🍏</li>\n<li>🍐</li>\n<li>🍌</li>\n</ul>")
    ).to eq("- 🍏\n- 🍐\n- 🍌")
  ...
  it 'supports <ol>' do
    expect(
      html_to_markdown('<ol><li>🍆</li><li>🍅</li><li>🍄</li></ol>')
    ).to eq("1. 🍆\n1. 🍅\n1. 🍄")
  end
  ...
  it 'supports <pre>' do
    ...
    expect(
      html_to_markdown(
        "<pre><code class=\"lang-javascript\">var foo = 'bar';</code></pre>"
      )
    ).to eq("```javascript\nvar foo = 'bar';\n```")
  end
  ...
  it 'supports <pre> inside <blockquote>' do
    expect(
      html_to_markdown(
        "<blockquote><pre><code>var foo = 'bar';</code></pre></blockquote>"
      )
    ).to eq("> ```\n> var foo = 'bar';\n> ```")
  end
  ...
  it 'supports html document' do
    expect(
      html_to_markdown('<html><body>Hello<div>World</div></body></html>')
    ).to eq("Hello\nWorld")
  end
  ...
  it 'handles <div>' do
    expect(html_to_markdown('<div>1st div</div><div>2nd div</div>')).to eq(
          "1st div\n\n2nd div"
        )
  end
  ...
  it 'handles <p> and <div> within <span>' do
    html =
      '<div>1st paragraph<span><div>2nd paragraph</div><p>3rd paragraph</p></span></div>'
  ...
  context 'with an oddly placed <br>' do
    it 'handles <strong>' do
      ...
      expect(html_to_markdown('<strong>Bold<br>text</strong>')).to eq(
            "**Bold\ntext**"
          )
    end
    ...
    it 'handles <em>' do
      ...
      expect(html_to_markdown('<em>Italic<br>text</em>')).to eq(
            "*Italic\ntext*"
          )
    end
  end
  ...
  context 'with an empty tag' do
    it 'handles <strong>' do
      ...
      expect(html_to_markdown('Some<strong>    </strong>text')).to eq(
            'Some text'
          )
    end
  ...
  context 'with spaces around text' do
    it 'handles <strong>' do
      ...
      expect(html_to_markdown('<strong>Bold </strong>')).to eq('**Bold**')
      expect(html_to_markdown('<strong>Bold     </strong>')).to eq('**Bold**')
      expect(html_to_markdown('Some<strong> bold</strong> text')).to eq(
            'Some **bold** text'
          )
      expect(html_to_markdown('Some<strong>     bold</strong> text')).to eq(
            'Some **bold** text'
    ...
    it 'handles <em>' do
      ...
      expect(html_to_markdown('<em>Italic </em>')).to eq('*Italic*')
      expect(html_to_markdown('<em>Italic     </em>')).to eq('*Italic*')
      expect(html_to_markdown('Some<em> italic</em> text')).to eq(
            'Some *italic* text'
          )
      expect(html_to_markdown('Some<em>     italic</em> text')).to eq(
            'Some *italic* text'
