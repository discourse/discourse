...
describe ThemeStore::GitImporter do
  context '#import' do
    ...
    before do
      ...
      @temp_folder =
        "#{Pathname.new(Dir.tmpdir).realpath}/discourse_theme_#{hex}"
      @ssh_folder =
        "#{Pathname.new(Dir.tmpdir).realpath}/discourse_theme_ssh_#{hex}"
    end
    ...
    it 'should import from ssh url' do
      ...
      importer =
        ThemeStore::GitImporter.new(ssh_url, private_key: 'private_key')
      importer.import!
    end
