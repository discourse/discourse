...
describe ThemeStore::TgzExporter do
  let(:theme) do
    Fabricate(:theme, name: 'Header Icons').tap do |theme|
      theme.set_field(
        target: :common, name: :body_tag, value: '<b>testtheme1</b>'
      )
      theme.set_field(
        target: :settings, name: :yaml, value: 'somesetting: test'
      )
      theme.set_field(
        target: :mobile,
        name: :scss,
      ...
      upload =
        UploadCreator.new(image, 'logo.png').create_for(
      ...
      cs1 =
        Fabricate(
          ...
          color_scheme_colors: [
            Fabricate(
              :color_scheme_color,
              name: 'header_primary', hex: 'F0F0F0'
            ),
            Fabricate(
              :color_scheme_color,
  ...
  it 'exports the theme correctly' do
    package
    Dir.chdir("#{dir}") { `tar -xzf discourse-header-icons.tar.gz` }
