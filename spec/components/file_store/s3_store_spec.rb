...
describe FileStore::S3Store do
  ...
  shared_context 's3 helpers' do
    ...
    before { SiteSetting.s3_region = 'us-west-1' }
  end
  ...
  context 'uploading to s3' do
    ...
    before do
      s3_object.stubs(:put).returns(
        Aws::S3::Types::PutObjectOutput.new(etag: "\"#{etag}\"")
      )
    end
    ...
    describe '#store_upload' do
      it 'returns an absolute schemaless url' do
        ...
        s3_bucket.expects(:object).with("original/1X/#{upload.sha1}.png")
          .returns(s3_object)

        expect(store.store_upload(uploaded_file, upload)).to eq(
              "//s3-upload-bucket.s3.dualstack.us-west-1.amazonaws.com/original/1X/#{upload
                .sha1}.png"
            )
      ...
      describe 'when s3_upload_bucket includes folders path' do
        ...
        it 'returns an absolute schemaless url' do
          ...
          s3_bucket.expects(:object).with(
            "discourse-uploads/original/1X/#{upload.sha1}.png"
          )
            .returns(s3_object)

          expect(store.store_upload(uploaded_file, upload)).to eq(
                "//s3-upload-bucket.s3.dualstack.us-west-1.amazonaws.com/discourse-uploads/original/1X/#{upload
                  .sha1}.png"
              )
          expect(upload.etag).to eq(etag)
        end
    ...
    describe '#store_optimized_image' do
      it 'returns an absolute schemaless url' do
        ...
        s3_helper.expects(:s3_bucket).returns(s3_bucket)
        path =
          "optimized/1X/#{optimized_image.upload
            .sha1}_#{OptimizedImage::VERSION}_100x200.png"

        s3_bucket.expects(:object).with(path).returns(s3_object)

        expect(
          store.store_optimized_image(optimized_image_file, optimized_image)
        ).to eq(
      ...
      describe 'when s3_upload_bucket includes folders path' do
        ...
        it 'returns an absolute schemaless url' do
          store.expects(:get_depth_for).with(optimized_image.upload.id).returns(
            0
          )
          s3_helper.expects(:s3_bucket).returns(s3_bucket)
          path =
            "discourse-uploads/optimized/1X/#{optimized_image.upload
              .sha1}_#{OptimizedImage::VERSION}_100x200.png"
  ...
  context 'copying files in S3' do
    ...
    describe '#copy_file' do
      it 'copies the from in S3 with the right paths' do
        ...
        destination =
          Discourse.store.get_path_for_upload(upload).sub('.png', '.jpg')
  ...
  context 'removal from s3' do
    ...
    describe '#remove_upload' do
      it 'removes the file from s3 with the right paths' do
        ...
        s3_bucket.expects(:object).with(
          "tombstone/original/1X/#{upload.sha1}.png"
        )
          .returns(s3_object)
        s3_object.expects(:copy_from).with(
          copy_source: "s3-upload-bucket/original/1X/#{upload.sha1}.png"
        )
        s3_bucket.expects(:object).with("original/1X/#{upload.sha1}.png")
          .returns(s3_object)
      ...
      it 'removes the optimized image from s3 with the right paths' do
        ...
        path =
          "optimized/1X/#{upload.sha1}_#{optimized.version}_#{optimized
            .width}x#{optimized.height}.png"

        store.expects(:get_depth_for).with(upload.id).returns(0)
        s3_helper.expects(:s3_bucket).returns(s3_bucket).at_least_once
        optimized.update_attributes!(
          url: "//s3-upload-bucket.s3.dualstack.us-west-1.amazonaws.com/#{path}"
        )
        s3_object = stub

        s3_bucket.expects(:object).with("tombstone/#{path}").returns(s3_object)
        s3_object.expects(:copy_from).with(
          copy_source: "s3-upload-bucket/#{path}"
        )
        s3_bucket.expects(:object).with(path).returns(s3_object)
        s3_object.expects(:delete)
      ...
      describe 'when s3_upload_bucket includes folders path' do
        ...
        it 'removes the file from s3 with the right paths' do
          ...
          s3_bucket.expects(:object).with(
            "discourse-uploads/tombstone/original/1X/#{upload.sha1}.png"
          )
            .returns(s3_object)
          s3_object.expects(:copy_from).with(
            copy_source:
              "s3-upload-bucket/discourse-uploads/original/1X/#{upload
                .sha1}.png"
          )
          s3_bucket.expects(:object).with(
            "discourse-uploads/original/1X/#{upload.sha1}.png"
    ...
    describe '#remove_optimized_image' do
      ...
      before do
        optimized_image.update!(
          url:
            "//s3-upload-bucket.s3.dualstack.us-west-1.amazonaws.com#{image_path}"
        )
      end
      ...
      it 'removes the file from s3 with the right paths' do
        ...
        s3_bucket.expects(:object).with("tombstone/#{image_path}").returns(
          s3_object
        )
        s3_object.expects(:copy_from).with(
          copy_source: "s3-upload-bucket/#{image_path}"
        )
      ...
      describe 'when s3_upload_bucket includes folders path' do
        ...
        it 'removes the file from s3 with the right paths' do
          ...
          s3_bucket.expects(:object).with(
            "discourse-uploads/tombstone/#{image_path}"
          )
            .returns(s3_object)
          ...
          s3_bucket.expects(:object).with("discourse-uploads/#{image_path}")
            .returns(s3_object)
  ...
  describe '.has_been_uploaded?' do
    it 'identifies S3 uploads' do
      expect(
        store.has_been_uploaded?(
          '//s3-upload-bucket.s3.dualstack.us-east-1.amazonaws.com/1337.png'
        )
      ).to eq(true)
    end
    ...
    it 'does not match other s3 urls' do
      expect(
        store.has_been_uploaded?('//s3-upload-bucket.s3.amazonaws.com/1337.png')
      ).to eq(false)
      expect(
        store.has_been_uploaded?(
          '//s3-upload-bucket.s3-us-east-1.amazonaws.com/1337.png'
        )
      ).to eq(false)
      expect(
        store.has_been_uploaded?('//s3.amazonaws.com/s3-upload-bucket/1337.png')
      ).to eq(false)
      expect(
        store.has_been_uploaded?('//s4_upload_bucket.s3.amazonaws.com/1337.png')
      ).to eq(false)
    end
  end
  ...
  describe '.cdn_url' do
    it 'supports subfolder' do
      ...
      url =
        '//s3-upload-bucket.s3.dualstack.us-east-1.amazonaws.com/livechat/original/gif.png'
