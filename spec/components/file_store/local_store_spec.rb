...
describe FileStore::LocalStore do
  ...
  describe '#store_upload' do
    it 'returns a relative url' do
      ...
      expect(store.store_upload(uploaded_file, upload)).to match(
            %r{\/uploads\/default\/original\/.+#{upload.sha1}\.png}
          )
    end
  end
  ...
  describe '#store_optimized_image' do
    it 'returns a relative url' do
      ...
      expect(store.store_optimized_image({}, optimized_image)).to match(
            %r{\/uploads\/default\/optimized\/.+#{optimized_image.upload
              .sha1}_#{OptimizedImage::VERSION}_100x200\.png}
          )
    end
  end
  ...
  describe '#remove_upload' do
    ...
    it 'moves the file to the tombstone' do
      begin
        upload =
          UploadCreator.new(file_from_fixtures('smallest.png'), 'smallest.png')
        ...
        expect(File.mtime(tombstone_path)).to_not eq(mtime) # Delay a little for mtime to be updated
      ensure
  ...
  describe '#has_been_uploaded?' do
    it 'identifies relatives urls' do
      expect(
        store.has_been_uploaded?('/uploads/default/42/0123456789ABCDEF.jpg')
      ).to eq(true)
    end
    ...
    it 'identifies local urls' do
      ...
      expect(
        store.has_been_uploaded?(
          'http://discuss.site.com/uploads/default/42/0123456789ABCDEF.jpg'
        )
      ).to eq(true)
      expect(
        store.has_been_uploaded?(
          '//discuss.site.com/uploads/default/42/0123456789ABCDEF.jpg'
        )
      ).to eq(true)
    end
    ...
    it 'identifies local urls when using a CDN' do
      ...
      expect(
        store.has_been_uploaded?(
          'http://my.cdn.com/uploads/default/42/0123456789ABCDEF.jpg'
        )
      ).to eq(true)
      expect(
        store.has_been_uploaded?(
          '//my.cdn.com/uploads/default/42/0123456789ABCDEF.jpg'
        )
      ).to eq(true)
    end
    ...
    it 'does not match dummy urls' do
      expect(
        store.has_been_uploaded?(
          'http://domain.com/uploads/default/42/0123456789ABCDEF.jpg'
        )
      ).to eq(false)
      expect(
        store.has_been_uploaded?(
          '//domain.com/uploads/default/42/0123456789ABCDEF.jpg'
        )
      ).to eq(false)
    end
  end
  ...
  describe '#absolute_base_url' do
    it 'is present' do
      expect(store.absolute_base_url).to eq(
            'http://test.localhost/uploads/default'
          )
    end
    ...
    it 'supports subfolder' do
      stub_for_subfolder
      expect(store.absolute_base_url).to eq(
            'http://test.localhost/forum/uploads/default'
          )
    end
  end
