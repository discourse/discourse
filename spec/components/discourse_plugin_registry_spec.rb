...
describe DiscoursePluginRegistry do
  ...
  context '.register_html_builder' do
    it 'can register and build html' do
      DiscoursePluginRegistry.register_html_builder(:my_html) do
        '<b>my html</b>'
      end
      expect(DiscoursePluginRegistry.build_html(:my_html)).to eq(
            '<b>my html</b>'
          )
  ...
  context '.register_css' do
    before { registry_instance.register_css('hello.css') }
    ...
    it "won't add the same file twice" do
      expect { registry_instance.register_css('hello.css') }.not_to change(
