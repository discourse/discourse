...
describe JsLocaleHelper do
  ...
  describe '#output_locale' do
    it "doesn't change the cached translations hash" do
      ...
      expect(
        JsLocaleHelper.translations_for('fr')['fr'].keys
      ).to contain_exactly('js', 'admin_js', 'wizard_js')
    end
  end
  ...
  context 'message format' do
    ...
    it 'handles double plurals' do
      ...
      expect(localize(NUM_RESULTS: 1, NUM_APPLES: 2)).to eq(
            '1 result and 2 apples'
          )
      expect(localize(NUM_RESULTS: 2, NUM_APPLES: 1)).to eq(
            '2 results and 1 apple'
          )
    end

    it 'handles select' do
      setup_message_format(
        '{GENDER, select, male {He} female {She} other {They}} read a book'
      )
    ...
    it 'can strip out message formats' do
      ...
      expect(JsLocaleHelper.strip_out_message_formats!(hash)).to eq(
            'c.d.f_MF' => 'bob'
          )
    ...
    it 'handles message format special keys' do
      JsLocaleHelper.set_translations(
        ...
        'en' => {
          'js' => {
            'hello' => 'world',
            'test_MF' =>
              '{HELLO} {COUNT, plural, one {1 duck} other {# ducks}}',
            'error_MF' => '{{BLA}',
      ...
      expect(
        ctx.eval('I18n.messageFormat("test_MF", { HELLO: "hi", COUNT: 3 })')
      ).to eq('hi 3 ducks')
      expect(
        ctx.eval('I18n.messageFormat("error_MF", { HELLO: "hi", COUNT: 3 })')
      ).to match(/Invalid Format/)
      ...
      expect(
        ctx.eval('I18n.messageFormat("foo_MF", { HELLO: "hi", COUNT: 4 })')
      ).to eq('hi 4 ducks')
    end

    it 'load pluralizations rules before precompile' do
      message =
        JsLocaleHelper.compile_message_format(
          message_format_filename('ru'),
          'ru',
          'format'
        )
  ...
  it 'performs fallbacks to english if a translation is not available' do
    ...
    expect(ctx.eval('I18n.translations.ru.js').keys).to contain_exactly(
          'only_site',
          'english_and_site'
        )
