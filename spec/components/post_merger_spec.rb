...
describe PostMerger do
  ...
  describe '.merge' do
    it 'should merge posts into the latest post correctly' do
      reply1 =
        create_post(
          topic: topic, raw: 'The first reply', post_number: 2, user: user
        )
      reply2 =
        create_post(
          topic: topic,
          raw: "The second reply\nSecond line",
          post_number: 3,
          user: user
        )
      reply3 =
        create_post(
          topic: topic, raw: 'The third reply', post_number: 4, user: user
        )
      ...
      expect(reply3.edit_reason).to eq(
            I18n.t(
    ...
    it 'should not allow the first post in a topic to be merged' do
      ...
      reply1 =
        create_post(topic: topic, post_number: post.post_number, user: user)
      reply2 =
        create_post(topic: topic, post_number: post.post_number, user: user)

      expect do
        PostMerger.new(admin, [reply2, post, reply1]).merge
    ...
    it 'should only allow staff user to merge posts' do
      ...
      expect { PostMerger.new(user, [reply2, reply1]).merge }.to raise_error(
    ...
    it 'should not allow posts from different topics to be merged' do
      ...
      expect do
        PostMerger.new(user, [another_post, post]).merge
      end.to raise_error(
            PostMerger::CannotMergeError,
            I18n.t('merge_posts.errors.different_topics')
          )
    end
    ...
    it 'should not allow posts from different users to be merged' do
      ...
      expect do
        PostMerger.new(user, [another_post, post]).merge
            ...
            PostMerger::CannotMergeError,
            I18n.t('merge_posts.errors.different_users')
          )
    end
  end
end
