...
describe Stylesheet::Manager do
  ...
  it 'can correctly compile theme css' do
    ...
    theme.set_field(
      target: :common, name: 'scss', value: '.common{.scss{color: red;}}'
    )
    theme.set_field(
      target: :desktop, name: 'scss', value: '.desktop{.scss{color: red;}}'
    )
    theme.set_field(
      target: :mobile, name: 'scss', value: '.mobile{.scss{color: red;}}'
    )
    theme.set_field(
      target: :common,
      name: 'embedded_scss',
      value: '.embedded{.scss{color: red;}}'
    )
    ...
    child_theme.set_field(
      target: :common, name: 'scss', value: '.child_common{.scss{color: red;}}'
    )
    child_theme.set_field(
      target: :desktop,
      name: 'scss',
      value: '.child_desktop{.scss{color: red;}}'
    )
    child_theme.set_field(
      target: :mobile, name: 'scss', value: '.child_mobile{.scss{color: red;}}'
    )
    child_theme.set_field(
      target: :common,
      name: 'embedded_scss',
    ...
    old_link =
      Stylesheet::Manager.stylesheet_link_tag(:desktop_theme, 'all', theme.id)
    ...
    child_theme.set_field(
      target: :desktop, name: :scss, value: '.nothing{color: green;}'
    )
    child_theme.save!
  ...
  describe 'digest' do
    after { DiscoursePluginRegistry.stylesheets.delete 'fake_file' }
    ...
    it "can correctly account for settings in theme's components" do
      ...
      child.set_field(
        target: :common,
        name: :scss,
        value: 'body {background-color: $childcolor}'
      )
      child.save!
  ...
  describe 'color_scheme_digest' do
    it 'changes with category background image' do
      ...
      category1 =
        Fabricate(
          :category,
          uploaded_background_id: 123, updated_at: 1.week.ago
        )
      category2 =
        Fabricate(
          :category,
          uploaded_background_id: 456, updated_at: 2.days.ago
        )
      ...
      category2.update_attributes(
        uploaded_background_id: 789, updated_at: 1.day.ago
      )

      digest2 = manager.color_scheme_digest
      expect(digest2).to_not eq(digest1)

      category1.update_attributes(
        uploaded_background_id: nil, updated_at: 5.minutes.ago
      )
  ...
  describe '.precompile_css', if: ENV['RUN_LONG_TESTS'] == '1' do
    ...
    it 'correctly generates precompiled CSS' do
      ...
      core_targets = %i[desktop mobile desktop_rtl mobile_rtl admin]
      theme_targets = %i[desktop_theme mobile_theme]

      Theme.update_all(user_selectable: false)
      user_theme =
        Fabricate(:theme, user_selectable: true, color_scheme: scheme1)
      default_theme =
        Fabricate(:theme, user_selectable: true, color_scheme: scheme2)
      ...
      core_targets.each do |tar|
        expect(
          results.count do |target|
            target =~ /^#{tar}_(#{scheme1.id}|#{scheme2.id})$/
          end
        ).to eq(2)
      end
      ...
      theme_targets.each do |tar|
        expect(
          results.count do |target|
            target =~ /^#{tar}_(#{user_theme.id}|#{default_theme.id})$/
          end
        ).to eq(2)
      end
      ...
      core_targets.each do |tar|
        expect(
          results.count do |target|
            target =~ /^(#{tar}_(#{scheme1.id}|#{scheme2.id})|#{tar})$/
          end
        ).to eq(3)
      end
