...
describe Stylesheet::Importer do
  ...
  it 'applies CDN to background category images' do
    expect(compile_css('category_backgrounds')).to_not include(
              'background-image'
            )
    ...
    expect(compile_css('category_backgrounds')).to include(
          "body.category-#{category.full_slug}{background-image:url(#{background
            .url})}"
        )
    ...
    expect(compile_css('category_backgrounds')).to include(
          "body.category-#{category
            .full_slug}{background-image:url(//awesome.cdn#{background.url})}"
        )
  end
  ...
  it 'applies S3 CDN to background category images' do
    ...
    expect(compile_css('category_backgrounds')).to include(
          "body.category-#{category
            .full_slug}{background-image:url(https://s3.cdn/original"
        )
  end
