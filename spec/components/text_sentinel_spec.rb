...
describe TextSentinel do
  ...
  context 'body_sentinel' do
    [
      ...
      it 'handles a valid body in a private message' do
        expect(
          TextSentinel.body_sentinel(valid_body, private_message: true)
        ).to be_valid
      end

      it 'handles a valid body in a public post' do
        expect(
          TextSentinel.body_sentinel(valid_body, private_message: false)
        ).to be_valid
      end
    end
  end
  ...
  context 'validity' do
    ...
    it 'does allow a long alphanumeric string joined with slashes' do
      expect(
        TextSentinel.new(
          'gdfgdfgdfg/fgdfgdfgdg/dfgdfgdfgd/dfgdfgdfgf',
          max_word_length: 30
        )
      ).to be_valid
    end
  ...
  context 'body_sentinel' do
    ...
    it 'uses a sensible min entropy value when min pm body length is less than min entropy' do
      ...
      SiteSetting.body_min_entropy = 7
      expect(
        TextSentinel.body_sentinel('Lol', private_message: true)
      ).to be_valid
    end
  end
end
