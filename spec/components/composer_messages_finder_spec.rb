...
describe ComposerMessagesFinder do
  context 'delegates work' do
    ...
    let(:finder) do
      ComposerMessagesFinder.new(user, composer_action: 'createTopic')
    end
  ...
  context '.check_education_message' do
    ...
    context 'private message' do
      ...
      context 'starting a new private message' do
        let(:finder) do
          ComposerMessagesFinder.new(
            user,
            composer_action: 'createTopic', topic_id: topic.id
          )
        end
      ...
      context 'replying to a private message' do
        let(:finder) do
          ComposerMessagesFinder.new(
            user,
            composer_action: 'reply', topic_id: topic.id
          )
        end
    ...
    context 'creating reply' do
      let(:finder) do
        ComposerMessagesFinder.new(user, composer_action: 'reply')
      end
  ...
  context '.check_sequential_replies' do
    ...
    before do
      ...
      Fabricate(
        :post,
        topic: topic, user: user, post_type: Post.types[:small_action]
      )
    ...
    context 'reply' do
      ...
      context 'success' do
        ...
        it 'creates a notified_about_dominating_topic log' do
          expect(
            UserHistory.exists_for_user?(user, :notified_about_dominating_topic)
          ).to eq(true)
        end
      end
    end
  ...
  context '.check_get_a_room' do
    ...
    let!(:other_user_reply) do
      Fabricate(
        :post,
        topic: topic, user: third_user, reply_to_user_id: op.user_id
      )
    end
    ...
    context 'reply' do
      let(:finder) do
        ComposerMessagesFinder.new(
          user,
          composer_action: 'reply', topic_id: topic.id, post_id: op.id
        )
      end
      ...
      context 'success' do
        ...
        it 'works as expected' do
          ...
          expect(
            UserHistory.exists_for_user?(user, :notified_about_get_a_room)
          ).to eq(true)
        end
      end
    end
  ...
  context '.check_reviving_old_topic' do
    ...
    it 'does not give a message without a topic id' do
      expect(
        described_class.new(user, composer_action: 'createTopic')
          .check_reviving_old_topic
      ).to be_blank
      expect(
        described_class.new(user, composer_action: 'reply')
          .check_reviving_old_topic
      ).to be_blank
    end
  ...
  context 'when editing a post' do
    ...
    let!(:post) do
      PostCreator.create!(
        user,
        topic_id: topic.id, post_number: 1, raw: 'omg my first post'
      )
    end
