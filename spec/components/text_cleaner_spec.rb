...
describe TextCleaner do
  context 'exclamation marks' do
    ...
    it 'deduplicates ! when enabled' do
      expect(
        TextCleaner.clean(
          duplicated_string,
          deduplicate_exclamation_marks: true
        )
      ).to eq(deduplicated_string)
    end
  end
  ...
  context 'question marks' do
    ...
    it 'deduplicates ? when enabled' do
      expect(
        TextCleaner.clean(duplicated_string, deduplicate_question_marks: true)
      ).to eq(deduplicated_string)
    end
  end
  ...
  context 'all upper case text' do
    ...
    it 'replaces all upper case text with regular case letters when enabled' do
      expect(TextCleaner.clean(all_caps, replace_all_upper_case: true)).to eq(
            regular_case
          )
    end
    ...
    it 'ignores almost all upper case text when enabled' do
      expect(
        TextCleaner.clean(almost_all_caps, replace_all_upper_case: true)
      ).to eq(almost_all_caps)
    end
  end
  ...
  context 'first letter' do
    ...
    it 'capitalizes first letter when enabled' do
      expect(
        TextCleaner.clean(lowercased, capitalize_first_letter: true)
      ).to eq(capitalized)
      expect(
        TextCleaner.clean(capitalized, capitalize_first_letter: true)
      ).to eq(capitalized)
      expect(TextCleaner.clean(iletter, capitalize_first_letter: true)).to eq(
            iletter
          )
    end
  end
  ...
  context 'periods at the end' do
    ...
    it 'ignores unnecessary periods at the end by default' do
      ...
      expect(TextCleaner.clean(with_several_periods)).to eq(
            with_several_periods
          )
    end
    ...
    it 'removes unnecessary periods at the end when enabled' do
      expect(
        TextCleaner.clean(
          with_one_period,
          remove_all_periods_from_the_end: true
        )
  ...
  context 'extraneous space' do
    ...
    it 'ignores extraneous space before the end punctuation by default' do
      expect(TextCleaner.clean(with_space_exclamation)).to eq(
            with_space_exclamation
          )
    ...
    it 'removes extraneous space before the end punctuation when enabled' do
      expect(
        TextCleaner.clean(with_space_exclamation, remove_extraneous_space: true)
      ).to eq(without_space_exclamation)
      expect(
        TextCleaner.clean(with_space_question, remove_extraneous_space: true)
      ).to eq(without_space_question)
    end
  ...
  context 'interior spaces' do
    ...
    it 'fixes interior spaces when enabled' do
      expect(
        TextCleaner.clean(spacey_string, fixes_interior_spaces: true)
      ).to eq(unspacey_string)
    end
  end
  ...
  context 'leading and trailing whitespaces' do
    ...
    it 'strips leading and trailing whitespaces when enabled' do
      expect(TextCleaner.clean(spacey_string, strip_whitespaces: true)).to eq(
            unspacey_string
          )
    end
  end
  ...
  context 'title' do
    ...
    context 'title_prettify site setting is enabled' do
      ...
      it 'replaces all upper case unicode text with regular unicode case letters' do
        expect(
          TextCleaner.clean_title('INVESTIGAÇÃO POLÍTICA NA CÂMARA')
        ).to eq('Investigação política na câmara')
      end

      it "doesn't downcase text if only one word is upcase in a non-ascii alphabet" do
        expect(TextCleaner.clean_title('«Эта неделя в EVE»')).to eq(
              '«Эта неделя в EVE»'
            )
      end

      it 'capitalizes first unicode letter' do
        expect(TextCleaner.clean_title('épico encontro')).to eq(
              'Épico encontro'
            )
      end
    end
  ...
  describe '#normalize_whitespaces' do
    ...
    it 'does not muck with zero width white space' do
      ...
      expect(TextCleaner.normalize_whitespaces("hello\u200Bworld").strip).to eq(
            "hello\u200Bworld"
          )
      expect(TextCleaner.normalize_whitespaces("hello\uFEFFworld").strip).to eq(
            "hello\uFEFFworld"
