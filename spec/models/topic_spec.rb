...
describe Topic do
      ...
      describe 'censored words' do
        ...
        describe 'titles with censored words not on boundaries' do
          it 'should be valid' do
            Fabricate(
              :watched_word,
              word: 'apple', action: WatchedWord.actions[:censor]
            )
            topic.title =
              "Pineapples are great fruit! Applebee's is a great restaurant"
            expect(topic).to be_valid
          end
        end
  ...
  context 'html in title' do
    def build_topic_with_title(title)
      build(:topic, title: title).tap(&:valid?)
    end

    let(:topic_bold) do
      build_topic_with_title('Topic with <b>bold</b> text in its title')
    end
    let(:topic_image) do
      build_topic_with_title(
    ...
    it 'keeps combined emojis' do
      expect(topic_modifier_emoji.fancy_title).to eq(
            'I :man_farmer: candy alot'
          )
    end

    it 'escapes bold contents' do
      expect(topic_bold.fancy_title).to eq(
            'Topic with &lt;b&gt;bold&lt;/b&gt; text in its title'
          )
    end
    ...
    it 'always escapes title' do
      topic_script.title =
        topic_script.title + 'x' * Topic.max_fancy_title_length
      expect(topic_script.fancy_title).to eq(
            ERB::Util.html_escape(topic_script.title)
          )
  ...
  context 'fancy title' do
    let(:topic) do
      Fabricate.build(:topic, title: "\"this topic\" -- has ``fancy stuff''")
    end
    ...
    context 'title_fancy_entities enabled' do
      ...
      it 'converts the title to have fancy entities and updates' do
        ...
        topic.reload
        expect(topic.fancy_title).to eq(
              'This is my test hello world&hellip; yay'
            )
      ...
      it 'works with long title that results in lots of entities' do
        long_title =
          "NEW STOCK PICK: PRCT - LAST PICK UP 233%, NNCO#{'.' * 150} ofoum"
    ...
    context 'allow_uncategorized_topics is false' do
      ...
      it 'allows PMs' do
        topic =
          Fabricate.build(
            :topic,
            category: nil, archetype: Archetype.private_message
          )
        expect(topic).to be_valid
      end
      ...
      it 'passes for topics with a category' do
        expect(
          Fabricate.build(:topic, category: Fabricate(:category))
        ).to be_valid
      end
    end
  ...
  context 'similar_to' do
    ...
    context 'with a category definition' do
      ...
      it 'excludes the category definition topic from similar_to' do
        expect(
          Topic.similar_to('category definition for', 'no body')
        ).to be_blank
      end
    end
    ...
    context 'with a similar topic' do
      ...
      context 'secure categories' do
        ...
        it 'should return the cat since the user can see it' do
          Guardian.any_instance.expects(:secure_category_ids).returns(
            [category.id]
          )
          expect(
            Topic.similar_to(
  ...
  describe '#invite' do
    ...
    describe 'when user is already allowed' do
      it 'should raise the right error' do
        ...
        expect { topic.invite(user, another_user.username) }.to raise_error(
              Topic::UserExists
            )
      end
    end
    ...
    describe 'private message' do
      ...
      describe 'by username' do
        it 'should be able to invite a user' do
          ...
          expect(topic.remove_allowed_user(user, another_user.username)).to eq(
                true
              )
        ...
        context 'when PMs are enabled for TL3 or higher only' do
          before { SiteSetting.min_trust_to_send_messages = 3 }
      ...
      describe 'by email' do
        ...
        describe 'when topic belongs to a private category' do
          ...
          describe 'as a group owner' do
            it 'should be able to invite a user' do
              expect do
                expect(topic.invite(inviter, invitee.email, [group.id])).to eq(
                      true
                    )
          ...
          describe 'as a normal user' do
            it 'should not be able to invite a user' do
              expect do
                expect(
                  topic.invite(Fabricate(:user), invitee.email, [group.id])
                ).to eq(false)
              end.to_not change { Notification.count }
            end
          end
  ...
  context 'private message' do
    ...
    it 'should integrate correctly' do
      ...
      expect(TopicQuery.new(evil_trout).list_latest.topics).not_to include(
                topic
              )
    end
      ...
      context 'existing user' do
        ...
        context 'by group name' do
          ...
          it 'creates a notification for each user in the group' do
            ...
            expect { topic.invite_group(topic.user, group) }.to change do
              Notification.count
            end
              .by(1)
    ...
    context 'user actions' do
      ...
      it 'should set up actions correctly' do
        ...
        expect(actions.map(&:action_type)).not_to include(UserAction::NEW_TOPIC)
        expect(actions.map(&:action_type)).to include(
              UserAction::NEW_PRIVATE_MESSAGE
            )
        expect(coding_horror.user_actions.map(&:action_type)).to include(
  ...
  context 'bumping topics' do
    before { @topic = Fabricate(:topic, bumped_at: 1.year.ago) }
    ...
    context 'editing posts' do
      ...
      it "doesn't bump the topic on an edit to the last post that doesn't result in a new version" do
        expect do
          ...
          @topic.reload
        end.not_to change(@topic, :bumped_at)
      end
  ...
  context 'update_status' do
    ...
    context 'archived' do
      context 'disable' do
        before do
          @archived_topic =
            Fabricate(:topic, archived: true, bumped_at: 1.hour.ago)
        ...
        it 'should archive correctly' do
          ...
          expect(@archived_topic.bumped_at.to_f).to be_within(0.1).of(
                @original_bumped_at
              )
  ...
  describe 'with category' do
    ...
    it 'should not increase the topic_count with no category' do
      expect do
        ...
        @category.reload
      end.not_to change(@category, :topic_count)
    end
    ...
    it "should increase the category's topic_count" do
      expect do
        ...
        @category.reload
      end.to change(@category, :topic_count).by(1)
    end
  end
  ...
  describe '#change_category_to_id' do
    ...
    describe 'without a previous category' do
      ...
      it 'changes the category even when the topic title is invalid' do
        ...
        expect { topic.change_category_to_id(category.id) }.to change do
          topic.category_id
        end
          .to(category.id)
      end
    ...
    describe 'with a previous category' do
      ...
      describe 'to a different category' do
        let(:new_category) do
          Fabricate(:category, user: user, name: '2nd category')
        end
        ...
        describe 'user that is watching the new category' do
          it 'should generate the notification for the topic' do
            ...
            expect { topic.change_category_to_id(new_category.id) }.to change do
              Notification.count
            end
              .by(2)
        ...
        describe 'when new category is set to auto close by default' do
          before { new_category.update!(auto_close_hours: 5) }
          ...
          it 'should set a topic timer' do
            ...
            expect(topic_timer.execute_at).to be_within(1.second).of(
          ...
          describe 'when topic is already closed' do
            ...
            it 'should not set a topic timer' do
              expect do
                topic.change_category_to_id(new_category.id)
              end.to change { TopicTimer.with_deleted.count }.by(0)
          ...
          describe 'when topic has an existing topic timer' do
            ...
            it "should not inherit category's auto close hours" do
              ...
              expect(topic.public_topic_timer.execute_at).to be_within(1.second)
                    .of(topic_timer.execute_at)
            end
          end
        end
      end
  ...
  describe '#set_or_create_timer' do
    ...
    it 'can take a number of hours as a string and can handle based on last post' do
      ...
      topic.set_or_create_timer(
        ...
        '18',
        by_user: admin, based_on_last_post: true
      )
      expect(topic.topic_timers.first.execute_at).to eq(18.hours.from_now)
    end
    ...
    it 'can take a timestamp for a future time' do
      ...
      expect(topic.topic_timers.first.execute_at).to eq(
            Time.zone.local(2013, 11, 22, 5, 0)
          )
    end
    ...
    it 'sets a validation error when given a timestamp in the past' do
      ...
      expect(topic.topic_timers.first.execute_at).to eq(
            Time.zone.local(2013, 11, 19, 5, 0)
          )
    ...
    it 'sets topic status update user to topic creator if it is a staff user' do
      staff_topic =
        Fabricate.build(:topic, user: Fabricate.build(:admin, id: 999))
    ...
    it 'sets topic status update user to topic creator if it is a TL4 user' do
      tl4_topic =
        Fabricate.build(:topic, user: Fabricate.build(:trust_level_4, id: 998))
    ...
    it 'updates topic status update execute_at if it was already set to close' do
      ...
      expect(closing_topic.reload.public_topic_timer.execute_at).to eq(
            2.days.from_now
          )
    end
    ...
    it "does not update topic's topic status created_at it was already set to close" do
      ...
      end.to_not change { closing_topic.topic_timers.first.created_at }
    end
    ...
    describe 'private status type' do
      ...
      let(:reminder) do
        Fabricate(
          :topic_timer,
          user: admin, topic: topic, status_type: TopicTimer.types[:reminder]
        )
      end
  ...
  describe 'for_digest' do
    ...
    context 'no edit grace period' do
      ...
      it 'returns regular topics' do
        ...
        expect(Topic.for_digest(user, 1.year.ago, top_order: true)).to eq(
              [topic]
            )
      end
      ...
      it 'returns topics from TL0 users if given include_tl0' do
        ...
        expect(
          Topic.for_digest(user, 1.year.ago, top_order: true, include_tl0: true)
        ).to eq([topic])
      end
      ...
      it 'returns topics with both muted and not muted tags' do
        ...
        TagUser.change(
          user.id,
          muted_tag.id,
          TagUser.notification_levels[:muted]
        )
      ...
      it 'sorts by topic notification levels' do
        ...
        for_digest =
          Topic.for_digest(user, 1.year.ago, top_order: true).pluck(:id)
  ...
  describe 'trash!' do
    context "its category's topic count" do
      ...
      it 'subtracts 1 if topic is being deleted' do
        ...
        expect { topic.trash!(moderator) }.to change do
      ...
      it "doesn't subtract 1 if topic is already deleted" do
        ...
        expect { topic.trash!(moderator) }.to_not change do
          category.reload.topic_count
        end
      end
    end
  ...
  describe 'recover!' do
    context "its category's topic count" do
      ...
      it 'adds 1 if topic is deleted' do
        topic = Fabricate(:topic, category: category, deleted_at: 1.day.ago)
        expect { topic.recover! }.to change { category.reload.topic_count }.by(
              1
            )
      end
  ...
  context 'new user limits' do
    before do
      ...
      SiteSetting.stubs(:client_settings_json).returns(
    ...
    it 'limits new users to max_topics_in_first_day and max_posts_in_first_day' do
      ...
      expect { create_post(user: user) }.to raise_error(
  ...
  it 'allows users to normalize counts' do
    ...
    post2 =
      Fabricate(
        :post,
        topic: topic, post_type: Post.types[:whisper], post_number: 2
      )
  ...
  context 'featured link' do
    ...
    it 'can properly save the featured link' do
      ...
      expect(topic.featured_link).to eq(
    ...
    context 'when category restricts present' do
      ...
      it 'can save the featured link if it belongs to that category' do
        ...
        expect(link_topic.featured_link).to eq(
              'https://github.com/discourse/discourse'
            )
      end
  ...
  describe '#time_to_first_response' do
    it 'should have no results if no topics in range' do
      expect(
        Topic.time_to_first_response_per_day(5.days.ago, Time.zone.now).count
      ).to eq(0)
    end
    ...
    it "should have results if there's a topic with replies" do
      ...
      Fabricate(
        :post,
        topic: topic, user: topic.user, post_number: 1, created_at: 3.hours.ago
      )
    ...
    it 'should only count regular posts as the first response' do
      ...
      Fabricate(
        :post,
        topic: topic, user: topic.user, post_number: 1, created_at: 5.hours.ago
      )
      Fabricate(
        :post,
        topic: topic,
  ...
  describe '#with_no_response' do
    it 'returns nothing with no topics' do
      expect(
        Topic.with_no_response_per_day(5.days.ago, Time.zone.now).count
      ).to eq(0)
    end
    ...
    it 'returns 1 with one topic that has a reply by the first poster' do
      ...
      Fabricate(
        :post,
        topic: topic, user: topic.user, post_number: 2, created_at: 2.hours.ago
      )
      expect(
        Topic.with_no_response_per_day(5.days.ago, Time.zone.now).count
      ).to eq(1)
  ...
  describe '#remove_allowed_user' do
    ...
    describe 'removing oneself' do
      it 'should remove onself' do
        ...
        expect(topic.remove_allowed_user(another_user, another_user)).to eq(
              true
            )
  ...
  describe '#reset_bumped_at' do
    it "ignores hidden, deleted, moderator and small action posts when resetting the topic's bump date" do
      ...
      post2 =
        Fabricate(:post, topic: topic, post_number: 2, created_at: 9.hours.ago)
      Fabricate(
        :post,
        topic: topic,
        post_number: 3,
      ...
      Fabricate(
        :post,
        topic: topic, post_number: 4, created_at: 7.hours.ago, hidden: true
      )
      Fabricate(
        :post,
        topic: topic,
      ...
      expect { topic.reset_bumped_at }.to change { topic.bumped_at }.to(
            post2.reload.created_at
          )
