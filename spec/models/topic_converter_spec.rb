...
describe TopicConverter do
  context 'convert_to_public_topic' do
    ...
    let(:other_user) do
      private_message.topic_allowed_users.find { |u| u.user != author }.user
    end
    ...
    context 'success' do
      ...
      describe 'when uncategorized category is not allowed' do
        ...
        it 'should convert private message into the right category' do
          ...
          first_category =
            Category.where.not(id: SiteSetting.uncategorized_category_id).where(
      ...
      it 'updates user stats' do
        ...
        topic_user =
          TopicUser.create!(
            user_id: author.id, topic_id: private_message.id, posted: true
          )
      ...
      context 'with a reply' do
        ...
        it 'updates UserActions' do
          TopicConverter.new(private_message, admin).convert_to_public_topic
          expect(
            author.user_actions.where(
              action_type: UserAction::NEW_PRIVATE_MESSAGE
            )
              .count
          ).to eq(0)
          expect(
            author.user_actions.where(action_type: UserAction::NEW_TOPIC).count
          ).to eq(1)
          # TODO:
  ...
  context 'convert_to_private_message' do
    ...
    context 'success' do
      ...
      it 'updates user stats' do
        ...
        topic_user =
          TopicUser.create!(
            user_id: author.id, topic_id: topic.id, posted: true
          )
        ...
        expect(
          topic.reload.topic_allowed_users.where(user_id: author.id).count
        ).to eq(1)
    ...
    context 'topic has replies' do
      ...
      it 'adds users who replied to topic in Private Message' do
        ...
        expect(
          topic.reload.topic_allowed_users.where(user_id: @replied_user.id)
            .count
        ).to eq(1)
