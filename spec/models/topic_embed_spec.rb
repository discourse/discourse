...
describe TopicEmbed do
  ...
  context '.import' do
    ...
    let(:contents) do
      "hello world new post <a href='/hello'>hello</a> <img src='/images/wat.jpg'>"
    end
    ...
    context 'creation of a post' do
      ...
      it 'works as expected with a new URL' do
        ...
        expect(post.cooked).to have_tag(
              'a',
              with: { href: 'http://eviltrout.com/hello' }
            )
        expect(post.cooked).to have_tag(
              'img',
              with: { src: 'http://eviltrout.com/images/wat.jpg' }
            )
  ...
  context '.topic_id_for_embed' do
    it 'returns correct topic id irrespective of url protocol' do
      topic_embed =
        Fabricate(:topic_embed, embed_url: 'http://example.com/post/248')

      expect(
        TopicEmbed.topic_id_for_embed('http://exAMPle.com/post/248')
      ).to eq(topic_embed.topic_id)
      expect(
        TopicEmbed.topic_id_for_embed('https://example.com/post/248/')
      ).to eq(topic_embed.topic_id)

      expect(
        TopicEmbed.topic_id_for_embed('http://example.com/post/248/2')
      ).to eq(nil)
      expect(
        TopicEmbed.topic_id_for_embed('http://examples.com/post/248')
      ).to eq(nil)
      expect(TopicEmbed.topic_id_for_embed('http://example.com/post/24')).to eq(
            nil
          )
      expect(TopicEmbed.topic_id_for_embed('http://example.com/post')).to eq(
            nil
          )
    end
    ...
    it 'finds the topic id when the embed_url contains a query string' do
      topic_embed =
        Fabricate(
          :topic_embed,
          embed_url: 'http://example.com/post/248?key=foo'
        )
      expect(
        TopicEmbed.topic_id_for_embed('http://example.com/post/248?key=foo')
      ).to eq(topic_embed.topic_id)
    end
  end
  ...
  describe '.find_remote' do
    context '.title_scrub' do
      ...
      it "doesn't scrub the title by default" do
        ...
        expect(response.title).to eq(
              'Through the Looking Glass - Classic Books'
            )
      end
    ...
    context 'non-ascii URL' do
      ...
      let(:contents) do
        '<title>سلام</title><body>این یک پاراگراف آزمون است.</body>'
      end
      let!(:embeddable_host) { Fabricate(:embeddable_host) }
      let!(:file) { StringIO.new }
      ...
      before do
        file.stubs(:read).returns contents
        TopicEmbed.stubs(:open).with(
          'http://eviltrout.com/test/%D9%85%D8%A7%D9%87%DB%8C',
    ...
    context 'emails' do
      ...
      it 'handles mailto links' do
        ...
        expect(response.body).to have_tag(
              'a',
              with: { href: 'mailto:foo%40example.com' }
            )
        expect(response.body).to have_tag(
              'a',
              with: { href: 'mailto:bar@example.com' }
            )
      end
    end
  end
