  ...
  context 'topic_links' do
    ...
    context 'create' do
      before do
        TopicLinkClick.create(
          topic_link: @topic_link, ip_address: '192.168.1.1'
        )
      end
    ...
    context 'create_from' do
      it 'works correctly' do
        ...
        end.not_to change(TopicLinkClick, :count)

        # can handle double # in a url
        # NOTE: this is not compliant but exists in the wild
        click =
          TopicLinkClick.create_from(
            url: 'http://discourse.org#a#b', post_id: @post.id, ip: '127.0.0.1'
          )
      ...
      context 'with a valid url and post_id' do
        before do
          @url =
            TopicLinkClick.create_from(
              url: @topic_link.url, post_id: @post.id, ip: '127.0.0.1'
            )
      ...
      context 'relative urls' do
        ...
        it 'returns the url' do
          url =
            TopicLinkClick.create_from(
              url: '/relative-url', post_id: @post.id, ip: '127.0.0.1'
            )
        ...
        context 'cdn links' do
          ...
          it 'correctly handles cdn links' do
            ...
            expect(click.topic_link_id).to eq(
                  TopicLink.order('id desc').first.id
                )
          end
        end
        ...
        context 's3 cdns' do
          it 'works with s3 urls' do
            SiteSetting.s3_cdn_url =
              'https://discourse-s3-cdn.global.ssl.fastly.net'
            ...
            url =
              TopicLinkClick.create_from(
                url:
                  'https://discourse-s3-cdn.global.ssl.fastly.net/my-test-link',
                topic_id: @topic.id,
                ip: '127.0.0.3'
              )
      ...
      context 'with a HTTPS version of the same URL' do
        before do
          @url =
            TopicLinkClick.create_from(
              url: 'https://twitter.com', topic_id: @topic.id, ip: '127.0.0.3'
            )
          @click = TopicLinkClick.last
        end
      ...
      context 'with a google analytics tracking code' do
        ...
        it 'creates a click' do
          ...
          expect(@click.topic_link).to eq(@topic_link)
          expect(@url).to eq(
                'http://twitter.com?_ga=1.16846778.221554446.1071987018'
              )
        end
      end
      ...
      context 'with a query param and google analytics' do
        ...
        it 'creates a click' do
          url =
            TopicLinkClick.create_from(
              url:
                'http://twitter.com?ref=forum&_ga=1.16846778.221554446.1071987018',
              topic_id: @topic.id,
              post_id: @post.id,
              ip: '127.0.0.3'
            )
      ...
      context 'with a google analytics tracking code and a hash' do
        before do
          @url =
            TopicLinkClick.create_from(
              url:
                'http://discourse.org?_ga=1.16846778.221554446.1071987018#faq',
              topic_id: @topic.id,
              ip: '127.0.0.3'
            )
          @click = TopicLinkClick.last
