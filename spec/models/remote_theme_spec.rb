...
describe RemoteTheme do
  context '#import_remote' do
    ...
    it 'can correctly import a remote theme' do
      ...
      expect(remote.remote_version).to eq(
            `cd #{initial_repo} && git rev-parse HEAD`.strip
          )
      expect(remote.local_version).to eq(
            `cd #{initial_repo} && git rev-parse HEAD`.strip
          )
      ...
      mapped =
        Hash[
          *@theme.theme_fields.map { |f| ["#{f.target_id}-#{f.name}", f.value] }
            .flatten
        ]
      ...
      File.write(
        "#{initial_repo}/about.json",
        about_json(love_color: 'BABABA', color_scheme_name: 'Amazing 2')
      )
      `cd #{initial_repo} && git commit -am "update"`
  ...
  context '#github_diff_link' do
    ...
    it 'returns URL for comparing between local_version and remote_version' do
      expect(github_repo.github_diff_link).to eq(
            "https://github.com/org/testtheme/compare/#{github_repo
              .local_version}...#{github_repo.remote_version}"
          )
    end

    it 'is blank when theme is up-to-date' do
      github_repo.update!(
        local_version: github_repo.remote_version, commits_behind: 0
      )
  ...
  context '.joined_remotes' do
    it 'finds records that are associated with themes' do
      ...
      expect(RemoteTheme.joined_remotes).to contain_exactly(
            github_repo,
            gitlab_repo
          )
    end
