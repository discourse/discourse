...
RSpec.describe Upload do
  ...
  describe "#dominant_color" do
    ...
    let(:mismatched_image) do
      upload = Fabricate(:upload)
      file = file_from_fixtures("tiff_as.bin")
      upload.update!(url: Discourse.store.store_upload(file, upload), extension: "png")
      upload
    end
    let(:not_an_image) do
      upload = Fabricate(:upload)
    ...
    it "stores an empty string when the file content does not match the extension" do
      expect(mismatched_image.dominant_color).to eq(nil)
      expect(mismatched_image.dominant_color(calculate_if_missing: true)).to eq("")
      expect(mismatched_image.dominant_color).to eq("")
    end

    it "stores an empty string when vips produces a pixel with an unexpected number of channels" do
      expect(white_image.dominant_color).to eq(nil)
