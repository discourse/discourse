...
describe Upload do
  ...
  it 'supports <style> element in SVG' do
    ...
    upload =
      UploadCreator.new(image_svg, image_svg_filename).create_for(user_id)
  ...
  it 'can reconstruct dimensions on demand' do
    ...
    upload.update_columns(
      width: nil, height: nil, thumbnail_width: nil, thumbnail_height: nil
    )
  ...
  it 'extracts file extension' do
    created_upload =
      UploadCreator.new(image, image_filename).create_for(user_id)
  ...
  context '.get_from_url' do
    ...
    it 'works when using a cdn' do
      begin
        ...
        expect(
          Upload.get_from_url(URI.join('http://my.cdn.com', upload.url).to_s)
        ).to eq(upload)
      ensure
    ...
    it "doesn't blow up with an invalid URI" do
      expect do
        Upload.get_from_url('http://ip:port/index.html')
      end.not_to raise_error
      expect do
        Upload.get_from_url('mailto:admin%40example.com')
      end.not_to raise_error
    ...
    describe 's3 store' do
      ...
      describe 'when using a cdn' do
        ...
        it 'should return the right upload' do
          upload
          expect(Upload.get_from_url(URI.join(s3_cdn_url, path).to_s)).to eq(
                upload
              )
        end
        ...
        describe 'when upload bucket contains subfolder' do
          let(:url) do
            "#{SiteSetting.Upload.absolute_base_url}/path/path2#{path}"
          end
      ...
      it 'should return the right upload when using one CDN for both s3 and assets' do
        begin
          ...
          expect(Upload.get_from_url(URI.join(cdn_url, path).to_s)).to eq(
                upload
              )
        ensure
          Rails.configuration.action_controller.asset_host = original_asset_host
  ...
  describe '.generate_digest' do
    it 'should return the right digest' do
      expect(Upload.generate_digest(image.path)).to eq(
            'bc975735dfc6409c1c2aa5ebf2239949bcbdbd65'
          )
    end
  end
  ...
  describe '.short_url' do
    it 'should generate a correct short url' do
      upload =
        Upload.new(
          sha1: 'bda2c513e1da04f7b4e99230851ea2aafeb8cc4e', extension: 'png'
        )
  ...
  describe '.sha1_from_short_url' do
    it 'should be able to look up sha1' do
      ...
      expect(
        Upload.sha1_from_short_url('upload://r3AYqESanERjladb4vBB7VsMBm6.png')
      ).to eq(sha1)
      expect(
        Upload.sha1_from_short_url('upload://r3AYqESanERjladb4vBB7VsMBm6')
      ).to eq(sha1)
      expect(Upload.sha1_from_short_url('r3AYqESanERjladb4vBB7VsMBm6')).to eq(
            sha1
          )
    end
    ...
    it 'should be able to look up sha1 even with leading zeros' do
      ...
      expect(
        Upload.sha1_from_short_url('upload://1Eg9p8rrCURq4T3a6iJUk0ri6.png')
      ).to eq(sha1)
    end
  end
  ...
  describe '.migrate_to_new_scheme' do
    it 'should not migrate system uploads' do
      ...
      expect { Upload.migrate_to_new_scheme }.to_not change do
        Upload.pluck(:url)
      end
    end
  end
