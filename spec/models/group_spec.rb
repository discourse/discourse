...
describe Group do
  ...
  describe '#primary_group=' do
    it "updates all members' #primary_group" do
      ...
      expect { group.update(primary_group: true) }.to change do
        user.reload.primary_group
      end
        .from(nil)
        .to(group)
      expect { group.update(primary_group: false) }.to change do
        user.reload.primary_group
      end
        .from(group)
        .to(nil)
  ...
  describe '#title=' do
    it "updates the member's title only if it was blank or exact match" do
      ...
      expect { group.update(title: 'Awesome') }.to change { user.reload.title }
            .from(nil)
            .to('Awesome')
      expect { group.update(title: 'Super') }.to change { user.reload.title }
            .from('Awesome')
            .to('Super')

      user.update(title: 'Differently Awesome')
      expect { group.update(title: 'Awesome') }.to_not change do
        user.reload.title
      end
    end
    ...
    it "doesn't update non-member's title" do
      ...
      expect { group.update(title: 'Super') }.to_not change do
        user.reload.title
      end
    end
  end
  ...
  describe '.refresh_automatic_group!' do
    ...
    it 'ensures that the moderators group is messageable by all' do
      ...
      expect(group.reload.messageable_level).to eq(
            Group::ALIAS_LEVELS[:everyone]
          )
    end
    ...
    it 'does not use the localized name if name has already been taken' do
      begin
        ...
        Fabricate(
          :user_single_email,
          username: I18n.t('groups.default_names.moderators').upcase
        )
    ...
    it 'always uses the default locale' do
      ...
      expect(group.name).to eq(
            I18n.t('groups.default_names.staff', locale: 'de')
          )
    end
  end
  ...
  it 'Correctly updates all automatic groups upon request' do
    ...
    DB.exec(
      "UPDATE groups SET user_count = 0 WHERE id = #{Group::AUTO_GROUPS[
        :trust_level_2
      ]}"
    )
  ...
  it 'correctly handles trust level changes' do
    ...
    expect(user.groups.map(&:name).sort).to eq %w[
             trust_level_0
             trust_level_1
             trust_level_2
           ]
    ...
    user.reload
    expect(user.groups.map(&:name).sort).to eq %w[trust_level_0]
  end
  ...
  describe 'trust level management' do
    ...
    describe 'when a user has qualified for trust level 1' do
      let(:user) do
        Fabricate(:user, trust_level: 1, created_at: Time.zone.now - 10.years)
      end
      ...
      before do
        user.user_stat.update!(
          topics_entered: 999, posts_read_count: 999, time_read: 999
        )
      end
  ...
  describe '#remove' do
    ...
    context 'when stripping title' do
      it "only strips user's title if exact match" do
        group.update!(title: 'Awesome')
        expect { group.remove(user) }.to change { user.reload.title }.from(
              'Awesome'
            )
              .to(nil)
    ...
    it "unsets the user's primary group" do
      user.update(primary_group: group)
      expect { group.remove(user) }.to change { user.reload.primary_group }
            .from(group)
            .to(nil)
    end
  end
  ...
  describe '#add' do
    it 'grants the title only if the new member does not have title' do
      group.update(title: 'Awesome')
      expect { group.add(user) }.to change { user.reload.title }.from(nil).to(
            'Awesome'
          )
    ...
    it "always sets user's primary group" do
      ...
      expect { group.add(user) }.to change { user.reload.primary_group }.from(
            nil
          )
            .to(group)
      ...
      expect do
        new_group.add(user)
        user.reload
      end.to change { user.primary_group }.from(group).to(
            new_group
    ...
    context 'when adding a user into a public group' do
      ...
      it "should publish the group's categories to the client" do
        ...
        message =
          MessageBus.track_publish('/categories') { group.add(user) }.first
  ...
  describe '.search_groups' do
    let(:group) do
      Fabricate(
        :group,
        name: 'tEsT_more_things', full_name: 'Abc something awesome'
      )
    end
  ...
  describe '#bulk_add' do
    it 'should be able to add multiple users' do
      ...
      expect(group.group_users.map(&:user_id)).to contain_exactly(
            user.id,
            admin.id
          )
    end
    ...
    it 'updates group user count' do
      expect do
        group.bulk_add([user.id, admin.id])
        group.reload
      end.to change { group.user_count }.by(2)
    end
  end
  ...
  describe '#automatic_group_membership' do
    describe 'for a automatic_membership_retroactive group' do
      ...
      it 'should be triggered on create and update' do
        expect { group }.to change { Jobs::AutomaticGroupMembership.jobs.size }
              .by(1)
        ...
        expect { group.update!(name: 'asdiaksjdias') }.to change do
