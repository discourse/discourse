...
describe UserAuthToken do
  it 'can remove old expired tokens' do
    ...
    token =
      UserAuthToken.generate!(
        user_id: user.id, user_agent: 'some user agent 2', client_ip: '1.1.2.3'
      )
  ...
  it 'can properly rotate tokens' do
    ...
    rotated =
      user_token.rotate!(user_agent: 'a new user agent', client_ip: '1.1.2.4')
  ...
  it 'keeps prev token valid for 1 minute after it is confirmed' do
    ...
    token =
      UserAuthToken.generate!(
        user_id: user.id, user_agent: 'some user agent', client_ip: '1.1.2.3'
      )
  ...
  it 'can correctly log auth tokens' do
    ...
    UserAuthToken.lookup(
      ...
      seen: true, user_agent: 'something diff', client_ip: '1.2.3.3'
    )

    UserAuthToken.lookup(
      token.unhashed_auth_token,
      seen: true, user_agent: 'something diff2', client_ip: '1.2.3.3'
    )
    ...
    UserAuthToken.lookup(
      fake_token,
      seen: true, user_agent: 'bob', client_ip: '127.0.0.1', path: '/path'
    )
  ...
  context 'suspicious login' do
    ...
    it 'is not checked when generated by impersonate' do
      UserAuthToken.generate!(
        user_id: admin.id, staff: admin.staff?, impersonate: true
      )
