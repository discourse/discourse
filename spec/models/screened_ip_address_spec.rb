...
describe ScreenedIpAddress do
  ...
  describe 'new record' do
    ...
    it 'sets an error when ip_address is invalid' do
      expect(
        described_class.create(valid_params.merge(ip_address: '99.99.99'))
          .errors[
          :ip_address
        ]
      ).to be_present
    ...
    it 'can set action_type using the action_name virtual attribute' do
      expect(
        described_class.new(valid_params.merge(action_name: :do_nothing))
          .action_type
      ).to eq(described_class.actions[:do_nothing])
      expect(
        described_class.new(valid_params.merge(action_name: :block)).action_type
      ).to eq(described_class.actions[:block])
      expect(
        described_class.new(valid_params.merge(action_name: 'do_nothing'))
          .action_type
      ).to eq(described_class.actions[:do_nothing])
      expect(
        described_class.new(valid_params.merge(action_name: 'block'))
          .action_type
      ).to eq(described_class.actions[:block])
    end
    ...
    it 'returns a useful error if ip address matches an existing record' do
      ScreenedIpAddress.create(
        ip_address: '2600:387:b:f::7a/128', action_name: :block
      )
      r =
        ScreenedIpAddress.new(
          ip_address: '2600:387:b:f::7a', action_name: :block
        )
  ...
  describe 'ip_address_with_mask' do
    ...
    it 'returns ip_address without mask if there is no mask' do
      expect(
        described_class.new(ip_address: '123.123.23.22').ip_address_with_mask
      ).to eq('123.123.23.22')
    end

    it 'returns ip_address with mask' do
      expect(
        described_class.new(ip_address: '123.12.0.0/16').ip_address_with_mask
      ).to eq('123.12.0.0/16')
    end
  end
  ...
  describe '#watch' do
    context 'ip_address is not being watched' do
      ...
      it "a record with subnet mask exists, but doesn't match" do
        existing =
          Fabricate(:screened_ip_address, ip_address: '99.232.23.124/24')
        expect { described_class.watch('99.232.55.124') }.to change do
      ...
      it "a record with exact matching exists, but doesn't match" do
        ...
        expect { described_class.watch('99.232.23.123') }.to change do
          described_class.count
        end
      end
    end
    ...
    context 'ip_address is already being watched' do
      shared_examples 'exact match of ip address' do
        it 'should not create a new record' do
          expect { described_class.watch(ip_address_arg) }.to_not change do
            described_class.count
          end
        end
  ...
  describe '#should_block?' do
    ...
    context 'IPv6' do
      it 'returns false when when record matches and action is :do_nothing' do
        ...
        expect(described_class.should_block?('2001:db8::ff00:42:8329')).to eq(
              false
            )
      end
  ...
  describe '#is_whitelisted?' do
    ...
    context 'IPv6' do
      it 'returns true when when record matches and action is :do_nothing' do
        ...
        expect(described_class.is_whitelisted?('2001:db8::ff00:42:8329')).to eq(
              true
            )
      end
  ...
  describe '#block_admin_login?' do
    context 'no allow_admin records exist' do
      it 'returns false when use_admin_ip_whitelist is false' do
        expect(
          described_class.block_admin_login?(
      ...
      context 'use_admin_ip_whitelist is true' do
        ...
        it 'returns false when user is nil' do
          expect(described_class.block_admin_login?(nil, '123.12.12.12')).to eq(
                false
              )
        end
        ...
        it 'returns false for admin user and ip_address arg is nil' do
          expect(
            described_class.block_admin_login?(Fabricate.build(:admin), nil)
          ).to eq(false)
        end
      end
    end
    ...
    context 'allow_admin record exists' do
      ...
      context 'use_admin_ip_whitelist is true' do
        ...
        it 'returns false when user is nil' do
          expect(
            described_class.block_admin_login?(nil, @permitted_ip_address)
          ).to eq(false)
        end
