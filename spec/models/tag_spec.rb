...
describe Tag do
  ...
  describe '#top_tags' do
    ...
    context 'with categories' do
      ...
      it 'works correctly' do
        expect(Tag.top_tags(category: @category1).sort).to eq(
              [@tags[0].name].sort
            )
        expect(
          Tag.top_tags(guardian: Guardian.new(Fabricate(:admin))).sort
        ).to eq([@tags[0].name, @tags[1].name, @tags[2].name].sort)
        expect(
          Tag.top_tags(
    ...
    context 'with category-specific tags' do
      ...
      it 'for category without tags, lists allowed tags' do
        expect(Tag.top_tags(category: @category2).sort).to eq(
              [@tags[1].name, @tags[2].name].sort
            )
      end

      it 'for no category arg, lists all tags' do
        expect(Tag.top_tags.sort).to eq(
    ...
    context 'with hidden tags' do
      ...
      it 'returns all tags to staff' do
        expect(
          Tag.top_tags(guardian: Guardian.new(Fabricate(:admin)))
        ).to include(hidden_tag.name)
      end
      ...
      it "doesn't return hidden tags to non-staff" do
        expect(
          Tag.top_tags(guardian: Guardian.new(Fabricate(:user)))
        ).to_not include(hidden_tag.name)
      end
    end
  end
  ...
  describe '#pm_tags' do
    ...
    before do
      2.times do |i|
        Fabricate(:tag, topics: [personal_message], name: "tag-#{i}")
      end
    end
    ...
    it 'returns all pm tags if user is a staff and pm tagging is enabled' do
      ...
      tags =
        Tag.pm_tags(guardian: Guardian.new(admin), allowed_user: regular_user)
