...
describe TopicViewItem do
  ...
  it 'raises nothing for dupes' do
    ...
    TopicViewItem.create!(
      topic_id: 1, ip_address: '1.1.1.1', viewed_at: 1.day.ago
    )
    add(1, '1.1.1.1')
  ...
  it 'does not log IP address for logged-in users' do
    ...
    expect(
      TopicViewItem.find_by(topic_id: topic.id, user_id: user.id).ip_address
    ).to eq(nil)
    add(topic.id, '1.2.3.4', nil)
    expect(
      TopicViewItem.find_by(topic_id: topic.id, user_id: nil).ip_address
    ).to eq('1.2.3.4')
  end
end
