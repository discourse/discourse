  ...
  describe '#top_tags' do
    ...
    describe 'when there are tags restricted to a category' do
      ...
      let!(:tag) do
        Fabricate(
          :tag,
          topics: [topic], categories: [category], name: 'category-tag'
        )
      end
      let!(:other_tag) do
        Fabricate(:tag, topics: [topic], name: 'use-anywhere')
      end
      let(:topic_list) do
        TopicList.new(
          'latest',
      ...
      it 'with no category, should return all tags' do
        expect(
          TopicList.new('latest', other_topic.user, [other_topic]).top_tags.sort
        ).to eq([tag.name, other_tag.name].sort)
      end
      ...
      it 'with another category with no tags, should return no tags' do
        ...
        list =
          TopicList.new(
            ...
            topic3.user,
            [topic3],
            category: other_category.id, category_id: other_category.id
          )
