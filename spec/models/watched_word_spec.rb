...
describe WatchedWord do
  it "can't have duplicate words" do
    Fabricate(
      :watched_word,
      word: 'darn', action: described_class.actions[:block]
    )
    w =
      Fabricate.build(
        :watched_word,
    ...
    w =
      Fabricate.build(
        :watched_word,
        word: 'darn', action: described_class.actions[:flag]
      )
    expect(w.save).to eq(false)
  ...
  describe '#create_or_update_word' do
      ...
      expect do
        w =
          described_class.create_or_update_word(
            word: 'nickelback', action_key: :block
          )
    ...
    it 'can update an existing record with different action' do
      existing =
        Fabricate(:watched_word, action: described_class.actions[:flag])
      expect do
        w =
          described_class.create_or_update_word(
            word: existing.word, action_key: :block
          )
        expect(w.reload.action).to eq(described_class.actions[:block])
    ...
    it "doesn't error for existing record with same action" do
      ...
      expect do
        w =
          described_class.create_or_update_word(
            word: existing.word, action_key: :flag
          )
        expect(w.id).to eq(existing.id)
    ...
    it 'allows action param instead of action_key' do
      expect do
        w =
          described_class.create_or_update_word(
            word: 'nickelback', action: described_class.actions[:block]
          )
        expect(w.reload.action).to eq(described_class.actions[:block])
      end.to change { described_class.count }.by(1)
    end
    ...
    it 'normalizes input' do
      ...
      expect do
        w =
          described_class.create_or_update_word(
            word: "  #{existing.word.upcase}  ", action_key: :block
          )
        expect(w.reload.action).to eq(described_class.actions[:block])
        expect(w.id).to eq(existing.id)
      end.to_not change { described_class.count }
