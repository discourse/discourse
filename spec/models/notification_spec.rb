...
describe Notification do
  ...
  describe 'post' do
    ...
    describe 'replies' do
      ...
      let(:post) do
        process_alerts(
          Fabricate(:post, post_args.merge(raw: 'Hello @CodingHorror'))
        )
      end
    ...
    describe 'muting' do
      it 'does not notify users of new posts' do
        ...
        expect do
          Fabricate(
            :post,
            user: user2, topic: post.topic, raw: 'hello @' + user.username
          )
    ...
    context 'a regular notification' do
      it 'increases unread_notifications' do
        expect do
          ...
          user.reload
        end.to change(user, :unread_notifications)
      end
      ...
      it 'increases total_unread_notifications' do
        expect do
          Fabricate(:notification, user: user)
          user.reload
        end.to change(user, :total_unread_notifications)
      end
      ...
      it "doesn't increase unread_private_messages" do
        expect do
          Fabricate(:notification, user: user)
          user.reload
        end.not_to change(user, :unread_private_messages)
      end
    end
    ...
    context 'a private message' do
      it "doesn't increase unread_notifications" do
        expect do
          ...
          user.reload
        end.not_to change(user, :unread_notifications)
      end
      ...
      it 'increases unread_private_messages' do
        expect do
          Fabricate(:private_message_notification, user: user)
          user.reload
        end.to change(user, :unread_private_messages)
      end
    end
  end
  ...
  describe 'message bus' do
    it 'updates the notification count on create' do
      Notification.any_instance.expects(:refresh_notification_count).returns(
        nil
      )
  ...
  describe 'private message' do
    before do
      ...
      @target =
        @post.topic.topic_allowed_users.reject do |a|
  ...
  describe '.post' do
    ...
    let!(:notification) do
      Fabricate(
        :notification,
        user: post.user, topic: post.topic, post_number: post.post_number
      )
    end
  ...
  describe 'mark_posts_read' do
    it 'marks multiple posts as read if needed' do
      ...
      expect do
        Notification.mark_posts_read(user, 2, [1, 2, 3, 4])
      end.to change { Notification.where(read: true).count }.by(3)
    end
  end
  ...
  describe '.filter_by_display_username_and_type' do
    ...
    it 'should return the right notifications' do
      expect(
        Notification.filter_by_display_username_and_type(
          user.username_lower,
      ...
      expect do
        PostAlerter.post_created(
          Fabricate(:basic_reply, user: user, topic: post.topic)
        )
...
describe Notification do
  describe '#recent_report' do
    ...
    it 'orders stuff correctly' do
      ...
      expect(notifications.map(&:id)).to eq([a.id, d.id, c.id])
    end
    ...
    describe 'for a user that does not want to be notify on liked' do
      ...
      it 'should not return any form of liked notifications' do
        ...
        expect(Notification.recent_report(user)).to contain_exactly(
              notification
            )
      end
    end
