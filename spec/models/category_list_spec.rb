...
describe CategoryList do
  ...
  context 'security' do
    ...
    it "doesn't show topics that you can't view" do
      ...
      expect(
        CategoryList.new(Guardian.new(admin), include_topics: true).categories
      ...
      expect(
        CategoryList.new(Guardian.new(user), include_topics: true).categories
          .find { |x| x.name == public_cat.name }
          .displayable_topics
          .count
      ).to eq(1)
      ...
      expect(
        CategoryList.new(Guardian.new(nil), include_topics: true).categories
          .find { |x| x.name == public_cat.name }
          .displayable_topics
          .count
      ).to eq(1)
  ...
  context 'with a category' do
    ...
    context 'with a topic in a category' do
      ...
      let(:category) do
        category_list.categories.find { |c| c.id == topic_category.id }
      end
    ...
    context 'with pinned topics in a category' do
      ...
      let!(:topic2) do
        Fabricate(:topic, category: topic_category, bumped_at: 5.minutes.ago)
      end
      let!(:topic3) do
        Fabricate(:topic, category: topic_category, bumped_at: 2.minutes.ago)
      end
      let!(:pinned) do
        Fabricate(
          :topic,
      ...
      it 'returns pinned topic first' do
        expect(category.displayable_topics.map(&:id)).to eq(
              [pinned.id, topic3.id]
            )
      end
    ...
    context 'notification level' do
      it "returns 'regular' as default notification level" do
        category =
          category_list.categories.find { |c| c.id == topic_category.id }
        expect(category.notification_level).to eq(
  ...
  describe 'category order' do
    let(:category_ids) do
      CategoryList.new(Guardian.new(admin)).categories.map(&:id) -
    ...
    context 'fixed_category_positions is enabled' do
      ...
      it 'returns categories in specified order' do
        cat1, cat2 =
          Fabricate(:category, position: 1), Fabricate(:category, position: 0)
        expect(category_ids).to eq([cat2.id, cat1.id])
      end

      it 'handles duplicate position values' do
        cat1, cat2, cat3, cat4 =
          Fabricate(:category, position: 0),
          Fabricate(:category, position: 0),
          Fabricate(:category, position: nil),
          Fabricate(:category, position: 0)
        first_three = category_ids[]
