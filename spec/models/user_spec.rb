...
describe User do
    ...
    describe '#username' do
      ...
      describe 'when username already exists' do
        it 'should not be valid' do
          ...
          expect(new_user.errors.full_messages.first).to include(
                I18n.t(:"user.username.unique")
              )
        end
      end
  ...
  describe '#count_by_signup_date' do
    ...
    it 'collect closed interval signups' do
      expect(User.count_by_signup_date(2.days.ago, Time.now)).to include(
            signups_by_day
          )
  ...
  context '.enqueue_welcome_message' do
    ...
    it 'enqueues the system message' do
      Jobs.expects(:enqueue).with(
        :send_system_message,
        user_id: user.id, message_type: 'welcome_user'
      )
  ...
  describe '.approve' do
    ...
    it "enqueues a 'signup after approval' email if must_approve_users is true" do
      ...
      Jobs.expects(:enqueue).with(
        :critical_user_email,
        has_entries(type: :signup_after_approval)
      )
      user.approve(admin)
    end
  ...
  describe 'bookmark' do
    ...
    describe 'when removing a bookmark' do
      ...
      it 'reduces the bookmark count of the post' do
        ...
        end.to change(active, :count).by(-1)
      end
    end
  end
  ...
  describe 'delete posts in batches' do
    ...
    it 'correctly deletes posts and topics' do
      ...
      @posts.each do |p|
        expect(Topic.find_by(id: p.topic_id)).to be_nil if p.is_first_post?
      end
    end
  ...
  describe 'new' do
    ...
    context 'after_save' do
      ...
      it 'has correct settings' do
        ...
        expect(subject.user_option.email_level).to eq(
              UserOption.email_level_types[:only_when_away]
            )
      end
    end
  ...
  describe 'ip address validation' do
    it 'validates ip_address for new users' do
      ...
      AllowedIpAddressValidator.any_instance.expects(:validate_each).with(
        u,
        :ip_address,
        u.ip_address
      )
  ...
  describe 'associated_accounts' do
    it 'should correctly find social associations' do
      ...
      UserAssociatedAccount.create(
        ...
        provider_uid: '1',
        info: { email: 'sam@sam.com' }
      )
      GithubUserInfo.create(
        user_id: user.id, screen_name: 'sam', github_user_id: 1
      )
  ...
  describe '.username_available?' do
    ...
    it 'returns true when reserved username is explicity allowed' do
      ...
      expect(
        User.username_available?('tESt', nil, allow_reserved_username: true)
      ).to eq(true)
    end
    ...
    it 'returns true when username is associated to a staged user of the same email' do
      ...
      expect(
        User.username_available?(staged.username, staged.primary_email.email)
      ).to eq(true)

      user = Fabricate(:user, email: 'bar@foo.com')
      expect(
        User.username_available?(user.username, user.primary_email.email)
      ).to eq(false)
    end
  ...
  describe 'email_validator' do
    ...
    it 'should reject some emails based on the email_domains_blacklist site setting' do
      ...
      expect(
        Fabricate.build(:user, email: 'notgood@mailinator.com')
      ).not_to be_valid
    ...
    it 'should reject some emails based on the email_domains_blacklist site setting' do
      ...
      expect(
        Fabricate.build(:user, email: 'notgood@trashmail.net')
      ).not_to be_valid
      expect(
        Fabricate.build(:user, email: 'mailinator.com@gmail.com')
      ).to be_valid
    end
    ...
    it 'should reject emails based on the email_domains_blacklist site setting matching subdomain' do
      ...
      expect(
        Fabricate.build(:user, email: 'notgood@sub.domain.com')
      ).not_to be_valid
    end
    ...
    it 'blacklist should not reject developer emails' do
      Rails.configuration.stubs(:developer_emails).returns(
        'developer@discourse.org'
      )
      SiteSetting.email_domains_blacklist = 'discourse.org'
      expect(
        Fabricate.build(:user, email: 'developer@discourse.org')
      ).to be_valid
    end
    ...
    it 'should not interpret a period as a wildcard' do
      SiteSetting.email_domains_blacklist = 'trashmail.net'
      expect(
        Fabricate.build(:user, email: 'good@trashmailinet.com')
      ).to be_valid
    end
    ...
    it 'whitelist should reject some emails based on the email_domains_whitelist site setting' do
      ...
      expect(user.errors.messages[:primary_email]).to include(
            I18n.t('user.email.not_allowed')
          )
      expect(
        Fabricate.build(:user, email: 'sbauch@vaynermedia.com')
      ).to be_valid
    end
    ...
    it 'should reject some emails based on the email_domains_whitelist site setting when whitelisting multiple domains' do
      ...
      expect(
        Fabricate.build(:user, email: 'mailinator.com@vaynermedia.com')
      ).to be_valid
    end
  ...
  describe 'previous_visit_at' do
    ...
    it 'should act correctly' do
      ...
      user.reload
      expect(user.previous_visit_at).to be_within_one_second_of(
            first_visit_date
          )
  ...
  describe 'update_last_seen!' do
    ...
    it 'should update the last seen value' do
      ...
      user.update_last_seen!(first_visit_date)
      expect(user.reload.last_seen_at).to be_within_one_second_of(
            first_visit_date
          )
    end
    ...
    it "should update the first seen value if it doesn't exist" do
      user.update_last_seen!(first_visit_date)
      expect(user.reload.first_seen_at).to be_within_one_second_of(
            first_visit_date
          )
    end
  ...
  describe 'email_confirmed?' do
    ...
    context 'when user has no email tokens for some reason' do
      it 'should return false' do
        user.email_tokens.each(&:destroy)
        user.reload
        expect(user.email_confirmed?).to eq(true)
      end
    end
  ...
  describe '#readable_name' do
    context 'when name is missing' do
      it 'returns just the username' do
        expect(
          Fabricate(:user, username: 'foo', name: nil).readable_name
        ).to eq('foo')
      end
    end
    context 'when name and username are identical' do
      it 'returns just the username' do
        expect(
          Fabricate(:user, username: 'foo', name: 'foo').readable_name
        ).to eq('foo')
      end
    end
    context 'when name and username are not identical' do
      it 'returns the name and username' do
        expect(
          Fabricate(:user, username: 'foo', name: 'Bar Baz').readable_name
        ).to eq('Bar Baz (foo)')
      end
    end
  end
  ...
  describe '#new_user_posting_on_first_day?' do
    def test_user?(opts = {})
      Fabricate.build(:user, { created_at: Time.zone.now }.merge(opts))
  ...
  describe '#find_email' do
    ...
    context 'when email is exists in the email logs' do
      before do
        user.stubs(:last_sent_email_address).returns('bob@lastemail.com')
      end
  ...
  describe '.small_avatar_url' do
    ...
    it 'returns a 45-pixel-wide avatar' do
      ...
      expect(user.small_avatar_url).to eq(
            '//test.localhost/letter_avatar_proxy/v3/letter/s/5f9b8f/45.png'
          )
    end
  end
  ...
  describe 'update_posts_read!' do
    context 'with a UserVisit record' do
      ...
      it 'with existing UserVisit record, increments the posts_read value' do
        ...
        end.to_not change { UserVisit.count }
      end
      ...
      it 'with no existing UserVisit record, creates a new UserVisit record and increments the posts_read count' do
        ...
        end.to change { UserVisit.count }.by(1)
      end
    end
  end
  ...
  describe 'automatic avatar creation' do
    it 'sets a system avatar for new users' do
      ...
      expect(u.avatar_template).to eq(
            "/letter_avatar/bob/{size}/#{LetterAvatar.version}.png"
          )
    end
  end
  ...
  describe '#purge_unactivated' do
    ...
    let!(:unactivated_old) do
      Fabricate(:user, active: false, created_at: 1.month.ago)
    end
    let!(:unactivated_old_with_system_pm) do
      Fabricate(:user, active: false, created_at: 2.months.ago)
    end
    let!(:unactivated_old_with_human_pm) do
      Fabricate(:user, active: false, created_at: 2.months.ago)
    end
    ...
    it 'should only remove old, unactivated users' do
      ...
      expect(User.real.all).to match_array(
            [user, unactivated, unactivated_old_with_human_pm]
          )
    end
  ...
  describe 'automatic group membership' do
    ...
    it "doesn't automatically add staged users" do
      staged_user =
        Fabricate(:user, active: true, staged: true, email: 'wat@wat.com')
    ...
    it 'is automatically added to a group when the email matches the SSO record' do
      ...
      user.create_single_sign_on_record(
        external_id: 123, external_email: 'sso@bar.com', last_payload: ''
      )
  ...
  context 'when user preferences are overriden' do
    ...
    it 'has overriden preferences' do
      ...
      expect(options.email_messages_level).to eq(
            UserOption.email_level_types[:never]
          )
      ...
      expect(
        CategoryUser.lookup(user, :watching_first_post).pluck(:category_id)
      ).to eq([4])
    end
  ...
  describe '#publish_notifications_state' do
    it 'should publish the right message' do
      ...
      message =
        MessageBus.track_publish("/notification/#{user.id}") do
  ...
  describe '#unread_notifications' do
    ...
    it 'limits to MAX_UNREAD_NOTIFICATIONS' do
      ...
      4.times do
        Notification.create!(
          user_id: user.id, notification_type: 1, read: false, data: '{}'
        )
      end
  ...
  describe '#unstage' do
    ...
    it 'triggers an event' do
      unstaged_user = nil
      event =
        DiscourseEvent.track_events { unstaged_user = User.unstage(params) }
          .first
  ...
  def filter_by(method)
    ...
    expect(User.public_send(method, [username, username2])).to contain_exactly(
          user,
          user2
        )

    expect(User.public_send(method, %w[UNiQuE sOME])).to contain_exactly(
          user,
          user2
        )
  end
  ...
  describe '#match_title_to_primary_group_changes' do
    ...
    it "updates user's title only when it is blank or matches the previous primary group" do
      expect { user.update(primary_group: primary_group_a) }.to change do
        user.reload.title
      end
        .from(nil)
        .to('A')
      expect { user.update(primary_group: primary_group_b) }.to change do
        user.reload.title
      end
        .from('A')
        .to('B')
      ...
      expect { user.update(primary_group: primary_group_a) }.to_not change do
        user.reload.title
      end
    end
  end
  ...
  describe '#next_best_title' do
    ...
    it "picks the next best title in the order: user's primary group, primary group, groups, and badges" do
      ...
      BadgeGranter.revoke(
        UserBadge.find_by(user_id: user.id, badge_id: badge.id)
      )
      expect(user.next_best_title).to eq(nil)
    end
  end
  ...
  context '#destroy!' do
    it 'clears up associated data on destroy!' do
      ...
      UserAction.create!(
        user_id: -1, action_type: UserAction::LIKE, target_user_id: user.id
      )
      UserAction.create!(
        user_id: -1, action_type: UserAction::LIKE, acting_user_id: user.id
      )
