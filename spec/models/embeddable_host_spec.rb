...
describe EmbeddableHost do
  ...
  describe 'it works with ports' do
    ...
    it 'works as expected' do
      expect(
        EmbeddableHost.url_allowed?('http://localhost:8000/eviltrout')
      ).to eq(true)
    end
  end
  ...
  describe 'url_allowed?' do
    ...
    it 'works as expected' do
      ...
      expect(
        EmbeddableHost.url_allowed?('https://eviltrout.com/انگلیسی')
      ).to eq(true)
      expect(EmbeddableHost.url_allowed?('https://not-eviltrout.com')).to eq(
            false
          )
    end
  ...
  describe 'path_whitelist' do
    it 'matches the path' do
      ...
      expect(
        EmbeddableHost.url_allowed?(
          'http://eviltrout.com/fp/2016/08/25/test-page'
        )
      ).to eq(true)
    end
    ...
    it 'respects query parameters' do
      ...
      expect(
        EmbeddableHost.url_allowed?('http://eviltrout.com/fp?test=1')
      ).to eq(false)
    ...
    it 'allows multiple records with different paths' do
      ...
      expect(
        EmbeddableHost.url_allowed?('http://eviltrout.com/rick/smith')
      ).to eq(true)
      expect(
        EmbeddableHost.url_allowed?('http://eviltrout.com/morty/sanchez')
      ).to eq(true)
    end
    ...
    it 'works with non-english paths' do
      ...
      expect(
        EmbeddableHost.url_allowed?('http://eviltrout.com/انگلیسی/foo')
      ).to eq(true)
      expect(
        EmbeddableHost.url_allowed?('http://eviltrout.com/definição/foo')
      ).to eq(true)
      expect(EmbeddableHost.url_allowed?('http://eviltrout.com/bar/foo')).to eq(
            false
          )
    end
    ...
    it 'works with URL encoded paths' do
      ...
      expect(
        EmbeddableHost.url_allowed?(
          'http://eviltrout.com/defini%C3%A7%C3%A3o/foo'
        )
      ).to eq(true)
      expect(
        EmbeddableHost.url_allowed?('http://eviltrout.com/inglés/foo')
      ).to eq(true)
    end
  end
end
