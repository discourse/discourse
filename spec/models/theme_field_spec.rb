...
describe ThemeField do
  ...
  describe 'scope: find_by_theme_ids' do
    it 'returns result in the specified order' do
      ...
      (0..1).each do |num|
        ThemeField.create!(
          theme: theme, target_id: num, name: 'header', value: '<a>html</a>'
        )
        ThemeField.create!(
          theme: theme2, target_id: num, name: 'header', value: '<a>html</a>'
        )
        ThemeField.create!(
          theme: theme3, target_id: num, name: 'header', value: '<a>html</a>'
        )
      end

      expect(
        ThemeField.find_by_theme_ids([theme3.id, theme.id, theme2.id]).pluck(
          :theme_id
        )
      ).to eq([theme3.id, theme3.id, theme.id, theme.id, theme2.id, theme2.id])
    end
  end
  ...
  it 'does not insert a script tag when there are no inline script' do
    theme_field =
      ThemeField.create!(
        theme_id: 1, target_id: 0, name: 'body_tag', value: '<div>new div</div>'
      )
  ...
  it 'only extracts inline javascript to an external file' do
    ...
    theme_field =
      ThemeField.create!(theme_id: 1, target_id: 0, name: 'header', value: html)

    expect(theme_field.value_baked).to include(
          "<script src=\"#{theme_field.javascript_cache.url}\"></script>"
        )
...
HTML
    ...
    expect(field.value_baked).to include(
          "<script src=\"#{field.javascript_cache.url}\"></script>"
        )
    expect(field.javascript_cache.content).to include(
...
HTML
    ...
    theme_field =
      ThemeField.create!(
        theme_id: 1, target_id: 0, name: 'head_tag', value: html
      )
    javascript_cache = theme_field.javascript_cache

    expect(theme_field.value_baked).to include(
          "<script src=\"#{javascript_cache.url}\"></script>"
        )
  ...
  it 'correctly generates errors for transpiled css' do
    css = 'body {'
    field =
      ThemeField.create!(theme_id: 1, target_id: 0, name: 'scss', value: css)
    field.reload
    expect(field.error).not_to eq(nil)
  ...
  it "ensures we don't use invalid SCSS variable names" do
    expect { create_upload_theme_field!('42') }.to raise_error(
  ...
  def get_fixture(type)
    File.read(
      "#{Rails.root}/spec/fixtures/theme_settings/#{type}_settings.yaml"
    )
  end
  ...
  it 'generates errors when default value is not within allowed range' do
    field = create_yaml_field(get_fixture('invalid'))
    expect(field.error).to include(
          I18n.t("#{key}.default_out_range", name: 'default_out_of_range')
        )
    expect(field.error).to include(
          I18n.t(
            "#{key}.default_out_range",
  ...
  describe 'locale fields' do
    ...
    describe 'scopes' do
      it 'filter_locale_fields returns results in the correct order' do
        expect(
          ThemeField.find_by_theme_ids([theme3.id, theme.id, theme2.id])
            .filter_locale_fields(%w[en fr])
        ).to eq([en3, en1, fr1, en2, fr2])
      end

      it 'find_first_locale_fields returns only the first locale for each theme' do
        expect(
          ThemeField.find_first_locale_fields(
    ...
    describe '#raw_translation_data' do
      it 'errors if the top level key is incorrect' do
        fr1.update(
          value: { wrongkey: { somestring1: 'bonjourworld' } }
            .deep_stringify_keys
            .to_yaml
        )
        expect { fr1.raw_translation_data }.to raise_error(
              ThemeTranslationParser::InvalidYaml
            )
      end

      it 'errors if there are multiple top level keys' do
        fr1.update(
          value: { fr: { somestring1: 'bonjourworld' }, otherkey: 'hello' }
            .deep_stringify_keys
            .to_yaml
        )
        expect { fr1.raw_translation_data }.to raise_error(
    ...
    describe '#translation_data' do
      ...
      it 'raises errors for the current locale' do
        ...
        expect { fr1.translation_data }.to raise_error(
              ThemeTranslationParser::InvalidYaml
            )
      end
      ...
      it 'merges any overrides' do
        ...
        expect(fr1.translation_data).to eq(
              fr: { somestring1: 'bonjourworld', group: { key2: 'frval2' } },
              en: {
                somestring1: 'helloworld', group: { key1: 'overriddentest1' }
              }
            )
      end
    end
    ...
    describe 'javascript cache' do
      it 'is generated correctly' do
        ...
        expect(fr1.value_baked).to include(
              "<script src='#{fr1.javascript_cache.url}'></script>"
            )
