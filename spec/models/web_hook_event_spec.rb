...
describe WebHookEvent do
  ...
  let(:failed_event) do
    WebHookEvent.new(status: 400, web_hook: Fabricate(:web_hook))
  end
  ...
  describe '.purge_old' do
    ...
    it 'should be able to purge old web hook events' do
      ...
      WebHookEvent.create!(
        status: 200, web_hook: web_hook, created_at: 2.days.ago
      )

      expect { described_class.purge_old }.to change { WebHookEvent.count }.by(
            -1
          )
  ...
  describe '#update_web_hook_delivery_status' do
    it 'update last delivery status for associated WebHook record' do
      ...
      expect(event.web_hook.last_delivery_status).to eq(
    ...
    it 'sets last delivery status to failed' do
      ...
      expect(failed_event.web_hook.last_delivery_status).to eq(
