...
RSpec.describe UserExport do
  ...
  describe '.remove_old_exports' do
    it 'should remove the right records' do
      export =
        UserExport.create!(
          file_name: 'test', user: user, created_at: 3.days.ago
        )

      export2 =
        UserExport.create!(
          file_name: 'test2', user: user, created_at: 1.day.ago
        )

      expect { UserExport.remove_old_exports }.to change { UserExport.count }
            .by(-1)
