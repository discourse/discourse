...
describe TopTopic do
  ...
  describe '#compute_top_score_for' do
    ...
    let!(:t1_post_1) do
      Fabricate(:post, topic: topic_1, like_count: 28, post_number: 1)
    end

    let!(:topic_2) { Fabricate(:topic, posts_count: 10, like_count: 20) }
    let!(:t2_post_1) do
      Fabricate(:post, topic: topic_2, like_count: 10, post_number: 1)
    end
    ...
    it 'should compute top score' do
      ...
      expect(
        top_topics.where(topic_id: topic_1.id).pluck(:yearly_score).first
      ).to eq(27)
      expect(
        top_topics.where(topic_id: topic_2.id).pluck(:yearly_score).first
      ).to eq(18.301029995664)
      expect(
        top_topics.where(topic_id: topic_3.id).pluck(:yearly_score).first
      ).to eq(10.602059991328)
      ...
      SiteSetting.top_topics_formula_least_likes_per_post_multiplier =
        'not good'
