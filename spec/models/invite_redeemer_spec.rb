...
describe InviteRedeemer do
  describe '#create_user_from_invite' do
    it 'should be created correctly' do
      user =
        InviteRedeemer.create_user_from_invite(
          Fabricate(:invite, email: 'walter.white@email.com'),
          'walter',
          'Walter White'
        )
    ...
    it 'should unstage user' do
      ...
      user =
        InviteRedeemer.create_user_from_invite(
          Fabricate(:invite, email: 'staged@account.com'),
          'walter',
          'Walter White'
        )
  ...
  describe '#redeem' do
    ...
    it 'can set custom fields' do
      ...
      user_fields = {
        required_field.id.to_s => 'value1', optional_field.id.to_s => 'value2'
      }
      user =
        InviteRedeemer.new(invite, username, name, password, user_fields).redeem

      expect(user).to be_present
      expect(user.custom_fields["user_field_#{required_field.id}"]).to eq(
            'value1'
          )
      expect(user.custom_fields["user_field_#{optional_field.id}"]).to eq(
            'value2'
          )
    end
