...
describe CategoryUser do
  ...
  it 'should correctly auto_track' do
    ...
    expect(TopicUser.get(topic, tracking_user).notification_level).to eq(
          tracking
        )
  ...
  context 'integration' do
    ...
    it 'should operate correctly' do
      ...
      expect(
        Notification.where(
          user_id: user.id, topic_id: early_watched_post.topic_id
        )
          .count
      ).to eq 1
      expect(
    ...
    it 'unwatches categories that have been changed' do
      ...
      expect(TopicUser.get(post.topic, user).notification_level).to eq TopicUser
               .notification_levels[
               :tracking
             ]
    end
