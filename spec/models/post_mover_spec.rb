  ...
  describe "move_posts" do
    context "with topics" do
      before do
        ...
        PostActionCreator.like(another_user, p4)
      ...
      describe "when replies of a post have been moved" do
        it "updates post replies correctly" do
          p5 =
            Fabricate(:post, topic: topic, reply_to_post_number: p2.post_number, user: another_user)
      ...
      context "when moved to a new topic" do
        ...
        let(:user3) { Fabricate(:user, refresh_auto_groups: true) }
        ...
        it "moves post timings" do
          ...
          expect(
            PostTiming.where(topic_id: topic.id, user_id: some_user.id).pluck(:post_number, :msecs),
      ...
      context "when moved to an existing topic" do
        ...
        let(:content_whisper) { Fabricate(:post, topic:, user:, post_type: Post.types[:whisper]) }
        let(:action_whisper) do
          Fabricate(
            :post,
            topic:,
            user:,
        ...
        it "creates a small action with the correct post URL on subfolder installs" do
          set_subfolder "/forum"
        ...
        it "leaves post numbers unchanged when they were lower then the topic's highest post number" do
          ...
          create_topic_user(admin1, topic, last_read_post_number: 3, last_emailed_post_number: 3)
          create_topic_user(
            admin1,
            destination_topic,
            last_read_post_number: 2,
          ...
          create_topic_user(admin2, topic, last_read_post_number: 3, last_emailed_post_number: 3)
          create_topic_user(
            admin2,
            destination_topic,
      ...
      it "corrects reply_counts within original topic" do
        expect do topic.move_posts(user, [p4.id], title: "new testing topic name 1") end.to change {
          PostReply.count
        }.by(-1)
    ...
    context "with freeze_original option" do
      ...
      describe "moved_post notifications" do
        ...
        it "generates a notification pointing to the destination topic when moving a non-first post" do
          PostMover.new(
            original_topic,
            Discourse.system_user,
            [first_post.id],
        ...
        it "generates a notification pointing to the destination topic when moving the first post" do
          PostMover.new(
            original_topic,
            Discourse.system_user,
            [op.id],
