...
describe Report do
  ...
  let(:c1) do
    Fabricate(:category, parent_category: c0, skip_category_definition: true)
  end # id: 2
  ...
  %i[
    ...
    describe "#{request_type} request reports" do
      let(:report) do
        Report.find(
          "#{request_type}_reqs",
          start_date: 10.days.ago.to_time, end_date: Time.now
        )
      end
      ...
      context "with #{request_type}" do
        ...
        context 'returns a report with data' do
          ...
          it "returns today's data" do
            expect(
              report.data.select { |value| value[:x] == Date.today }
            ).to be_present
          end
    ...
    context 'private message from system user' do
      before do
        Fabricate(
          ...
          created_at: 1.hour.ago, user: Discourse.system_user
        )
      end
  ...
  describe 'new contributors report' do
    ...
    context 'with contributors' do
      before do
        jeff = Fabricate(:user)
        jeff.user_stat =
          UserStat.new(new_since: 1.hour.ago, first_post_created_at: 1.day.ago)

        regis = Fabricate(:user)
        regis.user_stat =
          UserStat.new(new_since: 1.hour.ago, first_post_created_at: 2.days.ago)
  ...
  describe 'users by types level report' do
    ...
    context 'with users at different trust levels' do
      ...
      it 'returns a report with data' do
        ...
        label =
          Proc.new { |key| I18n.t("reports.users_by_type.xaxis_labels.#{key}") }
        expect(
          report.data.find { |d| d[:x] == label.call('admin') }[:y]
        ).to eq 3
        expect(
          report.data.find { |d| d[:x] == label.call('moderator') }[:y]
        ).to eq 2
        expect(
          report.data.find { |d| d[:x] == label.call('silenced') }[:y]
        ).to eq 1
        expect(
          report.data.find { |d| d[:x] == label.call('suspended') }[:y]
        ).to eq 1
      end
    end
  end
  ...
  describe 'trending search report' do
    ...
    context 'with different searches' do
      before do
        SearchLog.log(
          term: 'ruby', search_type: :header, ip_address: '127.0.0.1'
        )
        ...
        SearchLog.log(
          term: 'ruby', search_type: :header, ip_address: '127.0.0.2'
        )
        SearchLog.log(
          term: 'php', search_type: :header, ip_address: '127.0.0.1'
        )
      end
  ...
  describe 'flags' do
    ...
    context 'with data' do
      ...
      before(:each) do
        ...
        PostAction.act(user, post3, PostActionType.types[:off_topic])
  ...
  describe 'topics_with_no_response' do
    ...
    context 'with data' do
      ...
      context 'with category filtering' do
        let(:report) do
          Report.find('topics_with_no_response', category_id: c1.id)
        end
        ...
        context 'on subcategories' do
          let(:report) do
            Report.find('topics_with_no_response', category_id: c0.id)
          end
  ...
  describe 'report_suspicious_logins' do
    ...
    context 'with data' do
      it 'works' do
        ...
        UserAuthToken.log(
          action: 'suspicious', user_id: joffrey.id, created_at: 2.hours.ago
        )
        UserAuthToken.log(
          action: 'suspicious', user_id: joffrey.id, created_at: 3.hours.ago
        )
        UserAuthToken.log(
          action: 'suspicious', user_id: robin.id, created_at: 1.hour.ago
        )
  ...
  describe 'report_staff_logins' do
    ...
    context 'with data' do
      it 'works' do
        ...
        DiscourseIpInfo.open_db(
          File.join(Rails.root, 'spec', 'fixtures', 'mmdb')
        )
        Resolv::DNS.any_instance.stubs(:getname).with(ip.join('.')).returns(
          "ip-#{ip.join('-')}.example.com"
        )
        ...
        UserAuthToken.log(
          action: 'generate', user_id: joffrey.id, client_ip: '1.2.3.4'
        )
        UserAuthToken.log(
          action: 'generate',
          user_id: joffrey.id,
        ...
        expect(report.data[1][:location]).to eq(
  ...
  describe 'consolidated_page_views' do
    ...
    context 'with data' do
      it 'works' do
        ...
        page_view_crawler_report =
          reports.data.find { |r| r[:req] == 'page_view_crawler' }
        page_view_logged_in_report =
          reports.data.find { |r| r[:req] == 'page_view_logged_in' }
        page_view_anon_report =
          reports.data.find { |r| r[:req] == 'page_view_anon' }
