...
describe TranslationOverride do
          ...
          it 'should be valid' do
            translation_override =
              TranslationOverride.upsert!(
                I18n.locale,
                'some_key',
                "#{described_class::CUSTOM_INTERPOLATION_KEYS_WHITELIST[
  ...
  it 'upserts values' do
    ...
    ovr =
      TranslationOverride.where(locale: 'en', translation_key: 'some.key').first
  ...
  it 'stores js for a message format key' do
    ...
    ovr =
      TranslationOverride.where(locale: 'ru', translation_key: 'some.key_MF')
        .first
    expect(ovr).to be_present
  ...
  context 'site cache' do
    def cached_value(guardian, types_name, name_key, attribute)
      ...
      JSON.parse(json)[types_name].find { |x| x['name_key'] == name_key }[
        attribute
      ]
    end
    ...
    shared_examples 'resets site text' do
      it 'resets the site cache when translations of post_action_types are changed' do
        ...
        expect(
          cached_value(user_guardian, types_name, name_key, attribute)
        ).to eq(original_value)
        expect(
          cached_value(anon_guardian, types_name, name_key, attribute)
        ).to eq(original_value)
