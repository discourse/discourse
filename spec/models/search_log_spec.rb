...
RSpec.describe SearchLog, type: :model do
  ...
  describe '.log' do
    context 'invalid arguments' do
      ...
      it 'no IP returns error' do
        status, _ =
          SearchLog.log(
            term: 'bounty hunter', search_type: :header, ip_address: nil
          )
        expect(status).to eq(:error)
      end
    end
    ...
    context 'when anonymous' do
      it 'logs and updates the search' do
        ...
        action, log_id =
          SearchLog.log(
            term: 'jabba', search_type: :header, ip_address: '192.168.0.33'
          )
      ...
      it 'creates a new search with a different prefix' do
        ...
        action, _ =
          SearchLog.log(
            term: 'darth', search_type: :header, ip_address: '127.0.0.1'
          )
        expect(action).to eq(:created)

        action, _ =
          SearchLog.log(
            term: 'anakin', search_type: :header, ip_address: '127.0.0.1'
          )
        expect(action).to eq(:created)
      end
      ...
      it 'creates a new search with a different ip' do
        ...
        action, _ =
          SearchLog.log(
            term: 'darth', search_type: :header, ip_address: '127.0.0.2'
          )
        expect(action).to eq(:created)
      end
    end
    ...
    context 'when logged in' do
      ...
      it 'logs again if time has passed' do
        ...
        $redis.del(
          SearchLog.redis_key(ip_address: '192.168.0.1', user_id: user.id)
        )
  ...
  context 'term_details' do
    before do
      ...
      SearchLog.log(
        term: 'ruby', search_type: :full_page, ip_address: '127.0.0.2'
      )
    end
    ...
    it 'correctly returns term details' do
      ...
      SearchLog.where(term: 'ruby', ip_address: '127.0.0.2').update_all(
        search_result_id: 24
      )
      term_click_through_details =
        SearchLog.term_details('ruby', :all, :click_through_only)
  ...
  context 'trending' do
    ...
    before do
      ...
      SearchLog.log(
        term: 'swift', search_type: :header, ip_address: '127.0.0.1'
      )
    ...
    it 'correctly returns trending data' do
      ...
      SearchLog.where(term: 'ruby', ip_address: '127.0.0.1').update_all(
        search_result_id: 12
      )
      SearchLog.where(term: 'ruby', user_id: user.id).update_all(
        search_result_id: 12
      )
      SearchLog.where(term: 'ruby', ip_address: '127.0.0.2').update_all(
        search_result_id: 24
