...
describe TopicLink do
  ...
  let(:topic) { Fabricate(:topic, title: 'unique topic name') }
  ...
  it "can't link to the same topic" do
    ftl =
      TopicLink.new(
        url: "/t/#{topic.id}", topic_id: topic.id, link_topic_id: topic.id
      )
  ...
  describe 'external links' do
    ...
    before { TopicLink.extract_from(post2) }
  ...
  describe 'internal links' do
    it 'extracts onebox' do
      ...
      other_topic.posts.create(
        user: user, raw: 'some content for the first post'
      )
      other_post =
        other_topic.posts.create(
          user: user, raw: 'some content for the second post'
        )

      url =
        "http://#{test_uri.host}/t/#{other_topic.slug}/#{other_topic
          .id}/#{other_post.post_number}"
      invalid_url =
        "http://#{test_uri.host}/t/#{other_topic
          .slug}/9999999999999999999999999999999"

      topic.posts.create(user: user, raw: 'initial post')
      post =
        topic.posts.create(
          user: user, raw: "Link to another topic:\n\n#{url}\n\n#{invalid_url}"
        )
      post.reload
    ...
    context 'topic link' do
      ...
      let(:post) { other_topic.posts.create(user: user, raw: 'some content') }
      ...
      it 'works' do
        ...
        topic.posts.create(user: user, raw: 'initial post')
        linked_post =
          topic.posts.create(user: user, raw: "Link to another topic: #{url}")
        ...
        2.times do
          ...
          expect(reflection.url).to eq(
                "http://#{test_uri.host}/t/unique-topic-name/#{topic
                  .id}/#{linked_post.post_number}"
              )
        ...
        linked_post.revise(
          post.user,
          raw: 'no more linkies https://eviltrout.com'
        )
        expect(
          other_topic.reload.topic_links.where(link_post_id: linked_post.id)
        ).to be_blank
      end
    end
    ...
    context 'link to a user on discourse' do
      let(:post) do
        topic.posts.create(
          user: user, raw: "<a href='/u/#{user.username_lower}'>user</a>"
        )
      end
    ...
    context 'link to a discourse resource like a FAQ' do
      let(:post) do
        topic.posts.create(user: user, raw: "<a href='/faq'>faq link here</a>")
      end
      before { TopicLink.extract_from(post) }
    ...
    context 'mention links' do
      let(:post) do
        topic.posts.create(user: user, raw: "Hey #{user.username_lower}")
      end
    ...
    context 'mail link' do
      let(:post) do
        topic.posts.create(user: user, raw: '[email]bar@example.com[/email]')
      end
      ...
      it 'sets quote correctly' do
        ...
        quoting_post =
          Fabricate(
            :post,
            raw:
              "[quote=\"#{user.username}, post: #{linked_post
                .post_number}, topic: #{topic.id}\"]\nquote\n[/quote]"
          )
    ...
    context 'link to a local attachments' do
      ...
      it 'extracts the link' do
        ...
        expect(link.url).to eq(
              '/uploads/default/208/87bb3d8428eb4783.rb?foo=bar'
            )
  ...
  describe 'internal link from pm' do
    it 'works' do
      pm =
        Fabricate(
          :topic,
          user: user, category_id: nil, archetype: 'private_message'
        )
      ...
      linked_post =
        pm.posts.create(user: user, raw: "Link to another topic: #{url}")
  ...
  describe 'internal link from unlisted topic' do
    it 'works' do
      ...
      linked_post =
        unlisted_topic.posts.create(
          user: user, raw: "Link to another topic: #{url}"
        )
  ...
  describe 'query methods' do
    ...
    context 'with data' do
      ...
      it 'has the correct results' do
        ...
        TopicLinkClick.create!(
          topic_link: topic_link_first, ip_address: '192.168.1.1'
        )
        TopicLinkClick.create!(
          topic_link: topic_link_first, ip_address: '192.168.1.2'
        )
        topic_link_second = post.topic.topic_links.second
        TopicLinkClick.create!(
          topic_link: topic_link_second, ip_address: '192.168.1.1'
        )
      ...
      it 'secures internal links correctly' do
        ...
        TopicLink.extract_from(post)
        TopicLinkClick.create!(
          topic_link: post.topic.topic_links.first, ip_address: '192.168.1.1'
        )

        expect(TopicLink.topic_map(Guardian.new, post.topic_id).count).to eq(1)
        expect(
          TopicLink.counts_for(Guardian.new, post.topic, [post]).length
        ).to eq(1)
        ...
        expect(
          TopicLink.topic_map(Guardian.new(admin), post.topic_id).count
        ).to eq(1)
        ...
        expect(
          TopicLink.counts_for(Guardian.new(admin), post.topic, [post]).length
        ).to eq(1)
      end
      ...
      it 'does not include links from whisper' do
        ...
        post =
          Fabricate(
            :post,
            raw: "whisper post... #{url}", post_type: Post.types[:whisper]
          )
        TopicLink.extract_from(post)
    ...
    describe '.duplicate_lookup' do
      ...
      let(:post_with_internal_link) do
        Fabricate(
          :post,
          user: user, raw: "Check out this topic #{post.topic.url}/122131"
        )
      end
      ...
      it 'should return the right response' do
        ...
        lookup =
          result["test.localhost/t/#{post.topic.slug}/#{post.topic.id}/122131"]

        expect(lookup[:domain]).to eq('test.localhost')
        expect(lookup[:username]).to eq('junkrat')
        expect(lookup[:posted_at].to_s).to eq(
              post_with_internal_link.created_at.to_s
            )
