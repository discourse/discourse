...
describe Theme do
  ...
  it 'can support child themes' do
    ...
    expect(Theme.lookup_field(child.id, :desktop, 'header')).to eq(
          "World\nDesktop"
        )
    expect(Theme.lookup_field(child.id, 'mobile', :header)).to eq(
          "World\nMobile"
        )
  ...
  it 'can automatically disable for mismatching version' do
    ...
    theme.create_remote_theme!(
      remote_url: '', minimum_discourse_version: '99.99.99'
    )
  ...
  it "doesn't allow multi-level theme components" do
    ...
    expect { child.add_child_theme!(grandchild) }.to raise_error(
          Discourse::InvalidParameters,
          I18n.t('themes.errors.no_multilevels_components')
        )

    expect { grandparent.add_child_theme!(theme) }.to raise_error(
          Discourse::InvalidParameters,
          I18n.t('themes.errors.no_multilevels_components')
        )
  end
  ...
  it "doesn't allow a child to be set as the default theme" do
    expect { child.set_default! }.to raise_error(
          Discourse::InvalidParameters,
  ...
  it 'should correct bad html in body_tag_baked and head_tag_baked' do
    ...
    expect(Theme.lookup_field(theme.id, :desktop, 'head_tag')).to eq(
          '<b>I am bold</b>'
        )
  end
...
HTML
    ...
    field =
      theme.theme_fields.find_by(
        target_id: Theme.targets[:common], name: 'header'
      )
...
HTML
  ...
  it 'should create body_tag_baked on demand if needed' do
    ...
    expect(Theme.lookup_field(theme.id, :desktop, :body_tag)).to match(
          %r{<b>test<\/b>}
        )
  end
  ...
  it 'can find fields for multiple themes' do
    ...
    theme2.set_field(
      target: :common, name: :body_tag, value: '<b>theme2test</b>'
    )
    theme.save!
    theme2.save!

    field = Theme.lookup_field([theme.id, theme2.id], :desktop, :body_tag)
    expect(field).to match(%r{<b>testtheme1<\/b>})
    expect(field).to match(%r{<b>theme2test<\/b>})
  end
  ...
  describe '.transform_ids' do
    ...
    it 'adds the child themes of the parent' do
      ...
      expect(Theme.transform_ids([theme.id, orphan1.id, orphan2.id])).to eq(
            [theme.id, orphan1.id, *sorted, orphan2.id]
          )
    end
    ...
    it "doesn't insert children when extend is false" do
      ...
      expect(
        Theme.transform_ids(
          [theme.id, fake_id3, fake_id, fake_id2, fake_id2],
          extend: false
        )
...
HTML
  ...
  context 'theme upload vars' do
    ...
    it 'can handle uploads based of ThemeField' do
      ...
      theme.set_field(
        target: :common,
        name: :scss,
        value: 'body {background-image: url($logo)}'
      )
      theme.save!

      # make sure we do not nuke it
      freeze_time (SiteSetting.clean_orphan_uploads_grace_period_hours + 1)
                    .hours
                    .from_now
      ...
      scss, _map =
        Stylesheet::Compiler.compile(
          '@import "common/foundation/variables"; @import "theme_variables"; @import "desktop_theme"; ',
          'theme.scss',
          theme_id: theme.id
        )
  ...
  context 'theme settings' do
    it 'allows values to be used in scss' do
      ...
      scss, _map =
        Stylesheet::Compiler.compile(
          '@import "theme_variables"; @import "desktop_theme"; ',
          'theme.scss',
          theme_id: theme.id
        )
      ...
      theme.set_field(
        target: :common,
        name: :scss,
        value: 'body {font-size: quote($font-size)}'
      )
      theme.save!
    ...
    it 'allows values to be used in JS' do
      ...
      (function () {
        if ('Discourse' in window && typeof Discourse._registerPluginCode === 'function') {
          var settings = Discourse.__container__.lookup("service:theme-settings").getObjectForTheme(#{theme
        .id});
  ...
  it 'correctly caches user_themes template' do
    ...
    expect(user_themes).to eq(%w[bob])
    ...
    json = Site.json_for(guardian)
    user_themes = JSON.parse(json)['user_themes'].map { |t| t['name'] }
    expect(user_themes).to eq(%w[sam])
  ...
  it 'clears color scheme cache correctly' do
    ...
    theme =
      Fabricate(
        :theme,
        user_selectable: true, user: Fabricate(:admin), color_scheme_id: cs.id
      )
  ...
  it 'handles settings cache correctly' do
    ...
    theme.set_field(
      target: :settings, name: 'yaml', value: 'boolean_setting: true'
    )
    theme.save!
    ...
    child.set_field(
      target: :settings, name: 'yaml', value: 'integer_setting: 54'
    )
  ...
  describe 'theme translations' do
    ...
    it 'can list internal theme_translation_manager objects' do
      ...
      expect(translations.map(&:key)).to contain_exactly(
            'theme_metadata.description'
          )
      expect(translations.map(&:value)).to contain_exactly(
    ...
    it 'can create a hash of overridden values' do
      ...
      expect(theme.translation_override_hash).to eq(
            'en' => {
              'group_of_translations' => { 'translation1' => 'overriddentest1' }
            },
            'fr' => {
              'group_of_translations' => { 'translation1' => 'overriddentest2' }
            }
          )
    end
    ...
    it 'fall back when listing baked field' do
      ...
      expect(
        Theme.list_baked_fields([theme.id, theme2.id], :translations, 'fr').map(
          &:id
        )
