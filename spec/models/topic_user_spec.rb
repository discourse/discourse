...
describe TopicUser do
  ...
  describe '#unwatch_categories!' do
    it 'correctly unwatches categories' do
      ...
      TopicUser.unwatch_categories!(
        user,
        [Fabricate(:category).id, Fabricate(:category).id]
      )
      expect(TopicUser.get(another_topic, user).notification_level).to eq(
            watching
          )
      ...
      expect(TopicUser.get(tracked_topic, user).notification_level).to eq(
            tracking
          )
    end
  end
  ...
  describe 'notifications' do
    ...
    it 'should reset regular topics to tracking topics if auto track is changed' do
      ...
      expect(topic_user.notification_level).to eq(
            TopicUser.notification_levels[:tracking]
          )
    end
    ...
    it 'should watch topics a user created' do
      expect(topic_creator_user.notification_level).to eq(
            TopicUser.notification_levels[:watching]
          )
  ...
  describe 'read tracking' do
    ...
    context 'auto tracking' do
      ...
      it 'should automatically track topics you reply to' do
        post_creator.create
        expect(topic_new_user.notification_level).to eq(
              TopicUser.notification_levels[:tracking]
            )
      ...
      it 'should update tracking state when you reply' do
        ...
        tu =
          TopicUser.find_by(
            user_id: topic_new_user.user_id, topic_id: topic_new_user.topic_id
          )
  ...
  describe 'change a flag' do
    ...
    describe 'after creating a row' do
      ...
      it 'has a key in the lookup for this forum topic' do
        expect(TopicUser.lookup_for(user, [topic]).has_key?(topic.id)).to eq(
              true
            )
      end
    end
  ...
  it 'is able to self heal' do
    ...
    DB.exec(
      'UPDATE topic_users set highest_seen_post_number=1, last_read_post_number=0
                       WHERE topic_id = :topic_id AND user_id = :user_id',
      topic_id: p1.topic_id, user_id: p1.user_id
    )
