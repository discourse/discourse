...
RSpec.describe TopicTrackingState do
  ...
  shared_examples "does not publish message for private topics" do |method|
    it "does not publish any message for a private topic" do
      messages =
  ...
  shared_examples "publishes message to right groups and users" do |message_bus_channel, method|
    ...
    it "publishes message to everyone for a topic in a category that is not read restricted" do
      message =
        MessageBus
    ...
    it "publishes message only to admin group and groups that have permission to read a category when topic is in category that is restricted to certain groups" do
      message =
        MessageBus
          .track_publish(message_bus_channel) do
    ...
    it "publishes message only to admin group when topic is in category that is read restricted but no groups have been granted access" do
      message =
        MessageBus
          .track_publish(message_bus_channel) do
  ...
  describe "#publish_unread" do
    ...
    describe "for a private message" do
      ...
      it "does not publish any message" do
        messages =
  ...
  describe "#publish_muted" do
    ...
    it "does not publish any message when notification level is not muted" do
      messages = MessageBus.track_publish("/latest") { TopicTrackingState.publish_muted(topic) }
    ...
    it "does not publish any message when the user was not seen in the last 7 days" do
      TopicUser.find_by(topic: topic, user: post.user).update(notification_level: 0)
