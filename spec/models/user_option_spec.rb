...
describe UserOption do
  describe '#ensure_consistency!' do
    it 'recreates missing user option records' do
      ...
      expect(user.user_option.email_level).to eq(
            SiteSetting.default_email_level
          )
      expect(user.user_option.email_messages_level).to eq(
        ...
        context 'and there are enough topics' do
          ...
          describe 'a new user' do
            ...
            it 'should have a reason for the first visit' do
              freeze_time do
                ...
                Jobs.expects(:enqueue_in).with(
                  ...
                  user_id: user.id, redirected_at: Time.zone.now
                )
            ...
            it 'should not have a reason for next visits' do
              user.user_option.expects(:last_redirected_to_top_at).returns(
                10.minutes.ago
              )
          ...
          describe 'an older user' do
            ...
            it "should have a reason when the user hasn't been seen in a month" do
              ...
              expect(user.user_option.redirected_to_top).to eq(
                    reason:
                      I18n.t('redirected_to_top_reasons.not_seen_in_a_month'),
                    period: :monthly
                  )
            end
          end
