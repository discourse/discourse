...
describe ColorScheme do
  ...
  it 'correctly invalidates theme css when changed' do
    ...
    theme.set_field(
      name: :scss, target: :desktop, value: '.bob {color: $primary;}'
    )
    theme.save!

    href =
      Stylesheet::Manager.stylesheet_data(:desktop_theme, theme.id)[0][
        :new_href
      ]
  ...
  describe 'new' do
    it 'can take colors' do
      ...
      expect { expect(c.save).to eq true }.to change { ColorSchemeColor.count }
            .by(valid_colors.size)
    end
  end

  describe 'create_from_base' do
    let(:base_colors) do
      { first_one: 'AAAAAA', second_one: '333333', third_one: 'BEEBEE' }
    end
    let!(:base) do
      Fabricate(
        :color_scheme,
        ...
        color_scheme_colors: [
          Fabricate(
            :color_scheme_color,
            name: 'first_one', hex: base_colors[:first_one]
          ),
          Fabricate(
            :color_scheme_color,
            name: 'second_one', hex: base_colors[:second_one]
          ),
          Fabricate(
            :color_scheme_color,
            name: 'third_one', hex: base_colors[:third_one]
          )
        ]
      )
    end
    ...
    it 'creates a new color scheme' do
      c =
        described_class.create_from_base(
          name: 'Yellow', colors: { first_one: 'FFFF00', third_one: 'F00D33' }
        )
    ...
    context 'hex_for_name without anything enabled' do
      ...
      it 'returns the base color for an attribute of a specified scheme' do
        ...
        ColorSchemeRevisor.revise(
          scheme,
          colors: [
            { name: 'header_background', hex: '9dc927', default_hex: '949493' }
          ]
        )
        scheme.reload
        expect(ColorScheme.hex_for_name('header_background', scheme.id)).to eq(
              '9dc927'
            )
      end

      it 'returns the base color for an attribute' do
        expect(ColorScheme.hex_for_name('second_one')).to eq base_colors[
                 :second_one
               ]
      end
    end
