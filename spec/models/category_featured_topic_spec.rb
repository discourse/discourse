...
describe CategoryFeaturedTopic do
  ...
  context 'feature_topics_for' do
    ...
    let!(:category_post) do
      PostCreator.create(
        user,
        raw: 'I put this post in the category',
    ...
    it 'works in batched mode' do
      ...
      expect(
        CategoryFeaturedTopic.where(topic_id: category_post.topic_id).count
      ).to eq(1)
      expect(CategoryFeaturedTopic.where(topic_id: post2.topic_id).count).to eq(
            0
          )
    ...
    it 'should feature topics for a secure category' do
      ...
      _uncategorized_post =
        PostCreator.create(
          user,
          raw: 'this is my new post 123 post', title: 'hello world'
        )
    ...
    it 'should not include invisible topics' do
      invisible_post =
        PostCreator.create(
          user,
          raw: "Don't look at this post because it's awful.",
    ...
    it 'should feature stuff in the correct order' do
      ...
      _t5 =
        Fabricate(:topic, category_id: category.id, bumped_at: 12.minutes.ago)
      t4 =
        Fabricate(:topic, category_id: category.id, bumped_at: 10.minutes.ago)
      ...
      expect(
        CategoryFeaturedTopic.where(category_id: category.id).order('rank asc')
          .pluck(:topic_id)
