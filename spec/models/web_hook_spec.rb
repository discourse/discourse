...
describe WebHook do
  ...
  context 'web hooks' do
    let!(:post_hook) do
      Fabricate(:web_hook, payload_url: ' https://example.com ')
    end
    ...
    describe '#active_web_hooks' do
      it 'returns unique hooks' do
        post_hook.web_hook_event_types <<
          WebHookEventType.find_by(name: 'topic')
  ...
  describe 'enqueues hooks' do
    ...
    describe 'when there are no active hooks' do
      it 'should not generate payload and enqueue anything for topic events' do
        ...
        post =
          PostCreator.create(
            user,
            raw: 'post', title: 'topic', skip_validations: true
          )
    ...
    it 'should enqueue the right hooks for post events' do
      ...
      expect { PostDestroyer.new(user, post).destroy }.to change do
      ...
      expect { PostDestroyer.new(user, post).recover }.to change do
        Jobs::EmitWebHookEvent.jobs.count
      end
        .by(3)
    ...
    it 'should enqueue the right hooks for flag events' do
      ...
      post_action =
        PostAction.act(Fabricate(:user), post, PostActionType.types[:spam])
