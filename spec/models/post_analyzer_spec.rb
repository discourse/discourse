...
describe PostAnalyzer do
  ...
  context 'links' do
    ...
    describe 'raw_links' do
      ...
      it 'finds a link within markdown' do
        ...
        expect(post_analyzer.raw_links).to eq(
              %w[http://www.imdb.com/name/nm2225369]
            )
      end
    ...
    describe 'linked_hosts' do
      ...
      it 'returns the host and a count for links' do
        post_analyzer = PostAnalyzer.new(raw_two_links_html, default_topic_id)
        expect(post_analyzer.linked_hosts).to eq(
              'disneyland.disney.go.com' => 1, 'reddit.com' => 1
            )
      end
      ...
      it 'it counts properly with more than one link on the same host' do
        post_analyzer = PostAnalyzer.new(raw_three_links, default_topic_id)
        expect(post_analyzer.linked_hosts).to eq(
              'discourse.org' => 1, 'www.imdb.com' => 1
            )
      end
    end
  end
  ...
  describe 'image_count' do
    ...
    let(:raw_post_with_favicon) do
      '<img src="/assets/favicons/wikipedia.png" class="favicon">'
    end
    let(:raw_post_with_thumbnail) do
      '<img src="/assets/emoji/smiley.png" class="thumbnail">'
    end
    ...
    it 'finds images from HTML' do
      post_analyzer =
        PostAnalyzer.new(raw_post_two_images_html, default_topic_id)
    ...
    it "doesn't count thumbnails as images" do
      post_analyzer =
        PostAnalyzer.new(raw_post_with_thumbnail, default_topic_id)
      PrettyText.stubs(:cook).returns(raw_post_with_thumbnail)
      expect(post_analyzer.image_count).to eq(0)
    end

    it "doesn't count whitelisted images" do
      Post.stubs(:white_listed_image_classes).returns(%w[classy])
      PrettyText.stubs(:cook).returns(raw_post_with_two_classy_images)
      post_analyzer =
        PostAnalyzer.new(raw_post_with_two_classy_images, default_topic_id)
      expect(post_analyzer.image_count).to eq(0)
    end
  end

  describe 'link_count' do
    let(:raw_post_one_link_md) do
    ...
    it 'finds links from HTML' do
      post_analyzer =
        PostAnalyzer.new(raw_post_two_links_html, default_topic_id)
  ...
  describe 'raw_mentions' do
    ...
    it 'ignores pre' do
      ...
      post_analyzer =
        PostAnalyzer.new('. <pre>@Jake</pre> @Finn', default_topic_id)
      expect(post_analyzer.raw_mentions).to eq(%w[finn])
    end

    it 'catches content between pre tags' do
      post_analyzer =
        PostAnalyzer.new(
          '. <pre>hello</pre> @Finn <pre></pre>',
          default_topic_id
        )
      expect(post_analyzer.raw_mentions).to eq(%w[finn])
    end
    ...
    it 'ignores code' do
      ...
      expect(post_analyzer.raw_mentions).to eq(%w[jake])
    end

    it 'ignores code in markdown-formatted code blocks' do
      post_analyzer =
        PostAnalyzer.new("    @Jake @Finn\n@Ryan", default_topic_id)
      expect(post_analyzer.raw_mentions).to eq(%w[ryan])
    end
    ...
    it 'handles underscore in username' do
      post_analyzer =
        PostAnalyzer.new('@Jake @Finn @Jake_Old', default_topic_id)
    ...
    it 'handles hyphen in groupname' do
      ...
      expect(post_analyzer.raw_mentions).to eq(%w[org-board])
    end

    it 'ignores emails' do
      post_analyzer =
        PostAnalyzer.new('1@test.com 1@best.com @best @not', default_topic_id)
