...
describe ImportExport::Importer do
  ...
  let(:import_data) do
    import_file =
      Rack::Test::UploadedFile.new(
        file_from_fixtures('import-export.json', 'json')
      )
    data =
  ...
  context '.perform' do
    it 'topics and users' do
      ...
      expect { import(data) }.to change { Category.count }.by(0).and change do
        Group.count
      end
        .by(0).and change { Topic.count }.by(2).and change do
        User.count
      end
        .by(2)
    end
    ...
    it 'categories and groups' do
      ...
      expect { import(data) }.to change { Category.count }.by(6).and change do
        Group.count
      end
        .by(2).and change { Topic.count }.by(6).and change do
        User.count
      end
        .by(0)
    end
    ...
    it 'all' do
      expect { import(import_data) }.to change { Category.count }.by(
            6
          ).and change { Group.count }.by(2).and change { Topic.count }.by(
                      8
