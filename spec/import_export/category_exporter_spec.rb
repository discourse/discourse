...
describe ImportExport::CategoryExporter do
  ...
  context '.perform' do
    it 'export the category when it is found' do
      data =
        ImportExport::CategoryExporter.new([category.id]).perform.export_data
    ...
    it 'export multiple categories' do
      ...
      category_group =
        Fabricate(:category_group, category: category, group: group)
      data =
        ImportExport::CategoryExporter.new([category.id, category2.id]).perform
          .export_data
    ...
    it 'export the category with topics and users' do
      ...
      expect(data[:users].map { |u| u[:id] }).to match_array(
