...
RSpec.describe NotificationQuery do
  ...
  describe "#bulk_counts" do
    it "returns all expected keys" do
      result = notification_query.bulk_counts
      expect(result).to have_key(:unread_count)
      expect(result).to have_key(:unread_high_priority_count)
      expect(result).to have_key(:unread_low_priority_count)
      expect(result).to have_key(:new_personal_messages_count)
      expect(result).to have_key(:grouped_unread_counts)
    end
    ...
    it "computes correct counts" do
      ...
      # PM type forces high_priority=true via before_create
      Fabricate(
        :notification,
        user:,
        read: false,
      ...
      bulk = notification_query.bulk_counts

      expect(bulk[:unread_high_priority_count]).to eq(2)
      expect(bulk[:unread_low_priority_count]).to eq(1)
      expect(bulk[:unread_count]).to eq(3)
      expect(bulk[:new_personal_messages_count]).to eq(1)
      expect(bulk[:grouped_unread_counts]).to be_present
    end

    it "respects seen_notification_id" do
      old_notification = Fabricate(:notification, user:, read: false, high_priority: false)
      user.update!(seen_notification_id: old_notification.id)
      Fabricate(:notification, user:, read: false, high_priority: false)

      bulk = notification_query.bulk_counts
      expect(bulk[:unread_count]).to eq(1)
      expect(bulk[:unread_low_priority_count]).to eq(1)
    end

    it "counts high priority regardless of seen_notification_id" do
      hp = Fabricate(:notification, user:, read: false, high_priority: true)
      user.update!(seen_notification_id: hp.id)

      bulk = notification_query.bulk_counts
      expect(bulk[:unread_high_priority_count]).to eq(1)
      expect(bulk[:unread_count]).to eq(0)
    end

    it "excludes notifications for deleted topics" do
      topic = Fabricate(:topic)
      ...
      bulk = notification_query.bulk_counts
      expect(bulk[:unread_count]).to eq(0)
      expect(bulk[:unread_high_priority_count]).to eq(0)
      expect(bulk[:grouped_unread_counts]).to be_empty
    end
    ...
    it "excludes notifications for inaccessible PMs" do
      pm = Fabricate(:private_message_topic, user: user2)
      Fabricate(:notification, user:, topic: pm, read: false)
    ...
    it "excludes badge notifications when badges are disabled" do
      SiteSetting.enable_badges = false
      badge = Fabricate(:badge, enabled: true)
      Fabricate(
        :notification,
        user:,
        read: false,
      ...
      bulk = notification_query.bulk_counts
      expect(bulk[:unread_count]).to eq(0)
      expect(bulk[:grouped_unread_counts]).not_to have_key(Notification.types[:granted_badge])
    end

    it "does not let disabled badges below seen_notification_id drive counts negative" do
      SiteSetting.enable_badges = false
      badge = Fabricate(:badge, enabled: true)
      badge_notification =
        Fabricate(
      ...
      user.update!(seen_notification_id: badge_notification.id)

      bulk = notification_query.bulk_counts
      expect(bulk[:unread_count]).to eq(0)
      expect(bulk[:unread_low_priority_count]).to eq(0)
    end

    it "caps counts at MAX_UNREAD_NOTIFICATIONS" do
      stub_const(NotificationQuery, :MAX_UNREAD_NOTIFICATIONS, 3) do
        5.times { Fabricate(:notification, user:, read: false) }

        bulk = notification_query.bulk_counts
        expect(bulk[:unread_count]).to eq(3)
        expect(bulk[:unread_low_priority_count]).to eq(3)
      end
    end
  end
  ...
  describe "#unread_count_for_type" do
    ...
    it "excludes notifications for deleted topics" do
      ...
      expect(notification_query.unread_count_for_type(Notification.types[:mentioned])).to eq(0)
    end
