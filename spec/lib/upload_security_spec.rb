...
RSpec.describe UploadSecurity do
  ...
  context "when secure uploads is enabled" do
    ...
    context "when creating the upload" do
      ...
      shared_context "with login required" do
        before { SiteSetting.login_required = true }

        it "returns true when login is required" do
      ...
      include_context "with login required"
      ...
      context "when the access control post has_secure_uploads?" do
        ...
        shared_context "with a deleted access control post" do
          before { post_in_secure_context.trash! }

          it "still determines that the post has secure uploads" do
            expect(security.should_be_secure?).to eq(true)
          end
        end

        include_context "with a deleted access control post"
      end
      ...
      context "for attachments whose access control post has secure uploads" do
        before do
    ...
    context "when checking an existing upload" do
      ...
      describe "when the upload is first used for a custom emoji" do
        ...
        shared_examples "with identical upload reference creation dates" do
        ...
        include_context "with identical upload reference creation dates"
      end
