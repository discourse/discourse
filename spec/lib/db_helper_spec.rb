...
RSpec.describe DbHelper do
  describe '.remap' do
    it 'should remap columns properly' do
      ...
      expect(post2_attributes.except('image_url')).to eq(
            post2.attributes.except('image_url')
          )
      ...
      expect(badge_attributes.except('query')).to eq(
            badge.attributes.except('query')
          )
    end
