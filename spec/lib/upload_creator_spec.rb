  ...
  describe '#create_for' do
    ...
    describe 'when image is not authorized' do
      describe 'when image is for site setting' do
        ...
        it 'should create the right upload' do
          upload =
            UploadCreator.new(file, filename, for_site_setting: true)
    ...
    describe 'when image has the wrong extension' do
      ...
      it 'should store the upload with the right extension' do
        expect do
          UploadCreator.new(
            file,
            filename,
            force_optimize: true, type: UploadCreator::TYPES_TO_CROP.first
          )
            .create_for(user.id)
        end.to change { Upload.count }.by(1)
      ...
      describe 'for webp format' do
        before { SiteSetting.authorized_extensions = '.webp|.bin' }
    ...
    describe 'pngquant' do
      ...
      it 'should apply pngquant to optimized images' do
        upload =
          UploadCreator.new(file, filename, pasted: true, force_optimize: true)
            .create_for(user.id)
        ...
        thumbnail_size =
          upload.get_optimized_image(upload.width, upload.height, {}).filesize
      ...
      it 'should not store file as jpeg if it does not meet absolute byte saving requirements' do
        ...
        expect do
          UploadCreator.new(
            small_file,
            small_filename,
            pasted: true, force_optimize: true
          )
            .create_for(user.id)
        end.to change { Upload.count }.by(1)
