  ...
  describe "log_request" do
    ...
    describe "browser_pageview event" do
      ...
      context "when SiteSetting.trigger_browser_pageview_events is true" do
        ...
        it "does not truncate session IDs generated by SecureRandom.base64(32)" do
          # The client generates session IDs using SecureRandom.base64(32) which produces
          # 44-character strings. MAX_SESSION_ID_LENGTH must accommodate this.
          client_session_id = SecureRandom.base64(32)
          expect(client_session_id.length).to eq(44)

          data =
            Middleware::RequestTracker.get_data(
              env(
                "HTTP_DISCOURSE_TRACK_VIEW" => "1",
                "HTTP_DISCOURSE_TRACK_VIEW_SESSION_ID" => client_session_id,
              ),
              ["200", { "Content-Type" => "text/html" }],
              0.2,
            )
          ...
          expect(events.length).to eq(1)
          event = events[0][:params].first
          expect(event[:session_id]).to eq(client_session_id)
        end
