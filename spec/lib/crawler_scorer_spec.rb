...
RSpec.describe CrawlerScorer do
  ...
  describe ".flag_search_logs!" do
    let(:crawler_user_agent) { "Mozilla/5.0 (X11; Linux x86_64) HeadlessChrome/120.0.0.0" }

    def flag!(window_start: 1.hour.ago)
      described_class.flag_search_logs!(window_start: window_start, window_end: Time.now)
    end

    it "flags an anonymous search sharing an address and user agent with crawler traffic" do
      make_event(user_agent: crawler_user_agent)
      score!
      log =
        Fabricate(
          :search_log,
    ...
    it "leaves a search whose address matches but whose user agent does not" do
      make_event(user_agent: crawler_user_agent)
      score!
      log =
        Fabricate(
    ...
    it "leaves a search whose user agent matches but whose address does not" do
      make_event(user_agent: crawler_user_agent)
      score!
      log =
        Fabricate(
    ...
    it "leaves a search that matches only traffic scored below the threshold" do
      make_event(user_agent: "Mozilla/5.0")
      score!
      log =
        Fabricate(
          :search_log,
    ...
    it "leaves a search whose matching crawler traffic falls outside the correlation window" do
      make_event(user_agent: crawler_user_agent, created_at: 5.hours.ago)
      described_class.score!(window_start: 6.hours.ago, window_end: Time.now)
      log =
        Fabricate(
          :search_log,
          ip_address: "1.2.3.4",
      ...
      flag!(window_start: 6.hours.ago)

      expect(log.reload.likely_crawler).to eq(false)
    end

    it "flags a search whose user agent is longer than the event column allows" do
      long_agent = "Mozilla/5.0 " + ("x" * 2000)
      make_event(user_agent: long_agent[0, BrowserPageviewEvent::MAX_USER_AGENT_LENGTH])
      BrowserPageviewEvent.update_all(score: CrawlerScorer::BOT_SCORE_THRESHOLD + 1)
      log =
        Fabricate(
          :search_log,
          ip_address: "1.2.3.4",
          user_agent: long_agent[0, SearchLog::MAXIMUM_USER_AGENT_LENGTH],
          created_at: 30.minutes.ago,
        )
    ...
    it "leaves a logged-in search alone even when crawler traffic shares its user agent" do
      user = Fabricate(:user)
      make_event(user_agent: crawler_user_agent)
      score!
      log =
