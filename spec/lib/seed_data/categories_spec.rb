...
describe SeedData::Categories do
  ...
  describe '#create' do
    ...
    it 'creates a missing category' do
      expect { create_category }.to change { Category.count }.by(
            1
      ...
      expect(category.category_groups.first).to have_attributes(
            permissions(:staff, :full)
          )
      expect(Topic.exists?(category.topic_id))
      expect(description_post(category).raw).to eq(
    ...
    it 'does not override permissions of existing category when not forced' do
      ...
      expect(category.category_groups.first).to have_attributes(
            permissions(:trust_level_2, :full)
          )

      expect { create_category('lounge_category_id') }.to change do
        CategoryGroup.count
      end
        .by(0)
  ...
  describe '#update' do
    ...
    it 'updates an existing category' do
      category = Category.last
      description_post(category).revise(
