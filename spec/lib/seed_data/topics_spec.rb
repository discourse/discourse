...
describe SeedData::Topics do
  ...
  describe '#create' do
    it 'creates a missing topic' do
      expect { create_topic }.to change { Topic.count }.by(1).and change do
        Post.count
      end
        .by(1)
    ...
    it 'creates a missing topic and a reply when `static_first_reply` is true' do
      ...
      expect { create_topic('privacy_topic_id') }.to change { Topic.count }.by(
            1
      ...
      expect(post.raw).to eq(
            I18n.t('static_topic_first_reply', page_name: topic.title).rstrip
          )
    end
    ...
    it 'updates an existing first reply when `static_first_reply` is true' do
      ...
      expect(post.raw).to eq(
            I18n.t(
              ...
              page_name: I18n.t('privacy_topic.title')
            )
              .rstrip
          )
    end
  ...
  describe '#reseed_options' do
    it 'returns only existing topics as options' do
      ...
      Post.last.revise(
        Fabricate(:admin),
        title: 'Changed Topic Title', raw: 'Hello world'
      )

      expected_options = [
        {
          id: 'guidelines_topic_id',
          name: I18n.t('guidelines_topic.title'),
          selected: true
        },
