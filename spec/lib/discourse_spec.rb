...
RSpec.describe Discourse do
  ...
  describe "readonly mode" do
    ...
    describe ".archive_mode?" do
      ...
      it "is true when the archive key is set" do
      ...
      it "does not expire" do
        Discourse.enable_readonly_mode(Discourse::ARCHIVE_MODE_KEY)
        expect(Discourse.redis.ttl(Discourse::ARCHIVE_MODE_KEY)).to eq(-1)
      end

      it "makes the site read-only" do
        Discourse.enable_readonly_mode(Discourse::ARCHIVE_MODE_KEY)
        expect(Discourse.readonly_mode?).to eq(true)
      end
    end

    describe ".archive_mode_active?" do
      after do
      ...
      it "is false when archive mode is not set" do
        expect(Discourse.archive_mode_active?).to eq(false)
      end

      it "is true when only archive mode is set" do
        Discourse.enable_readonly_mode(Discourse::ARCHIVE_MODE_KEY)
        expect(Discourse.archive_mode_active?).to eq(true)
      end

      it "is false when another readonly key is also set" do
        Discourse.enable_readonly_mode(Discourse::ARCHIVE_MODE_KEY)
        Discourse.enable_readonly_mode(user_readonly_mode_key)
        expect(Discourse.archive_mode_active?).to eq(false)
      end

      it "is false when staff writes only is also set" do
        Discourse.enable_readonly_mode(Discourse::ARCHIVE_MODE_KEY)
        Discourse.enable_readonly_mode(Discourse::STAFF_WRITES_ONLY_MODE_KEY)
        expect(Discourse.archive_mode_active?).to eq(false)
      end

      it "is false when postgres was recently readonly" do
        Discourse.enable_readonly_mode(Discourse::ARCHIVE_MODE_KEY)
        Discourse.received_postgres_readonly!
        expect(Discourse.archive_mode_active?).to eq(false)
      end

      it "is false when pg_force_readonly_mode is set via global setting" do
        Discourse.enable_readonly_mode(Discourse::ARCHIVE_MODE_KEY)
        global_setting :pg_force_readonly_mode, true
        expect(Discourse.archive_mode_active?).to eq(false)
      end
