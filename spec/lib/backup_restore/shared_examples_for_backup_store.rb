  ...
  context 'with backup files' do
    ...
    describe '#file' do
      ...
      it "includes the file's source location if it is requested" do
        ...
        expect(file.source).to match(
              source_regex('default', backup1.filename, multisite: false)
            )
      end
      ...
      it 'works with multisite', type: :multisite do
        test_multisite_connection('second') do
          ...
          expect(file.source).to match(
                source_regex('second', backup4.filename, multisite: true)
              )
        end
      end
    end
    ...
    describe '#download_file' do
      ...
      it 'raises an exception when the download fails' do
        filename = backup1.filename
        destination_path =
          Dir.mktmpdir { |path| File.join(path, File.basename(filename)) }
    ...
    describe '#stats' do
      it 'returns the correct stats' do
        ...
        expect(stats[:last_backup_taken_at]).to eq(
              Time.parse('2018-09-13T15:10:00Z')
            )
      end
    end
  end
  ...
  context 'with backups' do
    ...
    describe '#upload_file' do
      def upload_file
        ...
        backup =
          BackupFile.new(
            filename: 'foo.tar.gz', size: 33, last_modified: Time.zone.now
          )
      ...
      it 'raises an exception when a file with same filename exists' do
        Tempfile.create(backup1.filename) do |file|
          ...
          end.to raise_exception(BackupRestore::BackupStore::BackupFileExists)
        end
      end
    end
    ...
    describe '#generate_upload_url' do
      it 'generates upload URL' do
        ...
        expect(url).to match(
              upload_url_regex('default', filename, multisite: false)
            )
      end

      it 'raises an exeption when a file with same filename exists' do
        expect do
          store.generate_upload_url(backup1.filename)
        end.to raise_exception(BackupRestore::BackupStore::BackupFileExists)
      end
      ...
      it 'works with multisite', type: :multisite do
        test_multisite_connection('second') do
          ...
          expect(url).to match(
                upload_url_regex('second', filename, multisite: true)
              )
        end
      end
    end
