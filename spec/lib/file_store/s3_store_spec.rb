...
RSpec.describe FileStore::S3Store do
  ...
  describe ".signed_url_for_path" do
    it "returns signed URL with inline content disposition for a given path" do
      ...
      expect(store.signed_url_for_path("special/optimized/file.png")).to eq(
        "https://s3.example.com/special/optimized/file.png?signed=true",
      )
    end
    ...
    it "returns signed URL with attachment content disposition for non-inline-safe path" do
      ...
      expect(store.signed_url_for_path("special/optimized/file.html")).to eq(
        "https://s3.example.com/special/optimized/file.html?signed=true",
      )
    end
    ...
    it "returns signed URL with attachment content disposition when force_download is true" do
      ...
      s3_object
        ...
        .with(:get, opts)
        .returns("https://s3.example.com/special/optimized/file.png?signed=true")
      expect(store.signed_url_for_path("special/optimized/file.png", force_download: true)).to eq(
        "https://s3.example.com/special/optimized/file.png?signed=true",
      )
    end
