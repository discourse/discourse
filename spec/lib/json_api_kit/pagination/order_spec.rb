...
RSpec.describe JsonApiKit::Pagination::Order do
  ...
  describe "#compatible_with?" do
    let(:cursor) do
      JsonApiKit::Pagination::Cursor.new(
        [order.digest, 0, pinned_topic.created_at, pinned_topic.id],
      )
    end
    ...
    context "when no segment of this order has that id" do
      let(:cursor) do
        JsonApiKit::Pagination::Cursor.new([order.digest, 7, pinned_topic.created_at, 1])
      end
    ...
    context "when the cursor holds another number of values" do
      let(:cursor) { JsonApiKit::Pagination::Cursor.new([order.digest, 0, pinned_topic.id]) }
  ...
  describe "#position" do
    ...
    let(:cursor) { JsonApiKit::Pagination::Cursor.new([order.digest, 1, unpinned_topic.id]) }
