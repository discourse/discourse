...
RSpec.describe JsonApiKit::Pagination::Predicate do
  subject(:predicate) { described_class.new(keyset, values) }
  ...
  let(:values) { keyset.values_for(record) }
  ...
  describe "#apply" do
    ...
    context "when the values are those of the last row" do
    ...
    context "when the nullable key holds a value" do
