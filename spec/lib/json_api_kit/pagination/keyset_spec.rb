...
RSpec.describe JsonApiKit::Pagination::Keyset do
  ...
  describe "#values_for" do
    subject(:values) { keyset.values_for(record) }

    let(:record) { Topic.new(id: 12, created_at: Time.utc(2026, 8, 3, 12)) }

    it "returns one value for each key, in the form a cursor carries" do
      expect(values).to eql(["2026-08-03T12:00:00.000000Z", 12])
    end
  end
  ...
  describe "#joins" do
    ...
    it "returns each join once" do
      expect(joins).to eq(JsonApiKit::Joins.for(:user))
    end
  end
  ...
  describe "#compatible_with?" do
    let(:values) { ["2026-08-03T12:00:00.000000Z", 12] }

    it "accepts one value for each key" do
      expect(keyset).to be_compatible_with(values)
    end

    context "when there are fewer values than the keyset compares" do
      let(:values) { [12] }

      it "refuses them" do
        expect(keyset).not_to be_compatible_with(values)
      end
    end
  end
