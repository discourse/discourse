...
RSpec.describe JsonApiKit::Request::Contract::Collection, type: :model do
  ...
  let(:params) { {} }
  let(:cursor) { JsonApiKit::Pagination::Cursor.new([0, 12]).to_s }
  let(:related) do
  ...
  describe "Pagination" do
    ...
    let(:params) { { page: {} } }
    let(:cursor_not_matching_segment) { JsonApiKit::Pagination::Cursor.new([7, 12]).to_s }
    let(:cursor_not_matching_length) { JsonApiKit::Pagination::Cursor.new([0]).to_s }
  ...
  describe "Anchor window" do
    ...
    context "when both `page[before_size]` and `page[after_size]` are provided" do
      let(:params) { { anchor: { id: 12 }, page: { before_size: 25 } } }
