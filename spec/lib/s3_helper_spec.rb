...
describe GlobalAwsCredentials do
  ...
  context "when S3 role assumption is configured without session name" do
    it "defaults session name to hostname" do
      GlobalSetting.stubs(:s3_access_key_id).returns("test-key")
      GlobalSetting.stubs(:s3_secret_access_key).returns("test-secret")
      GlobalSetting.stubs(:s3_role_arn).returns("arn:aws:iam::123456789012:role/test-role")
...
describe SiteAwsCredentials do
  context "when s3_use_iam_profile is enabled (deprecated)" do
    it "skips explicit credentials even when keys are present" do
      SiteSetting.stubs(:s3_use_iam_profile).returns(true)
      SiteSetting.stubs(:s3_access_key_id).returns("test-key")
      SiteSetting.stubs(:s3_secret_access_key).returns("test-secret")
      SiteSetting.stubs(:s3_role_arn).returns(nil)
      SiteSetting.stubs(:s3_role_session_name).returns(nil)
      SiteSetting.stubs(:s3_region).returns("us-west-2")
      SiteSetting.stubs(:s3_endpoint).returns(nil)
      SiteSetting.stubs(:s3_http_continue_timeout).returns(0)
      SiteSetting.stubs(:enable_s3_uploads?).returns(true)

      upload_settings = stub(use_dualstack_endpoint: false)
      SiteSetting.stubs(:Upload).returns(upload_settings)
    ...
    it "uses explicit credentials when s3_use_iam_profile is false" do
      SiteSetting.stubs(:s3_use_iam_profile).returns(false)
      SiteSetting.stubs(:s3_access_key_id).returns("test-key")
      SiteSetting.stubs(:s3_secret_access_key).returns("test-secret")
      SiteSetting.stubs(:s3_role_arn).returns(nil)
      SiteSetting.stubs(:s3_role_session_name).returns(nil)
