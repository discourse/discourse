...
RSpec.describe DiscourseVips do
  describe ".crop" do
    ...
    it "crops an image in place" do
      crop(output_path: input_path)

      expect(FastImage.size(input_path)).to eq([100, 50])
    end

    it "rejects unsupported output formats" do
      expect { crop(output_format: "svg") }.to raise_error(ArgumentError, "unsupported format")
    end

    it "reports invalid dimensions as an operation error" do
      expect { crop(width: 0) }.to raise_error(
        DiscourseVips::Error,
        "invalid crop dimensions",
      ) { |error| expect(error.class).to eq(DiscourseVips::Error) }
    end
    ...
    it "preserves the destination and removes temporary files after a decode error" do
      ...
      expect { crop }.to raise_error(DiscourseVips::InvalidImage)
