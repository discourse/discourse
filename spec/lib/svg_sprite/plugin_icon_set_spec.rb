...
RSpec.describe SvgSprite do
  ...
  def plugin_sprite
    SvgSprite.symbols_for(
      "multiweight-sprite.svg",
      file_from_fixtures("multiweight-sprite.svg").read,
      strict: false,
    )
  end

  # Placeholders and the ignore setting resolve only against settings the
  # registering plugin owns. `owns:` claims ownership of a real setting for the
  # fake plugin - both sides, because SiteSetting.plugins also drives visibility,
  # which resolves the same name back to a Plugin::Instance.
  def register_plugin_set!(map:, plugin_name: "fake-icons", owns: [], **registration)
    if owns.present?
      allow(SiteSetting).to receive(:plugins).and_return(
        SiteSetting.plugins.merge(owns.to_h { |setting| [setting.to_sym, plugin_name] }),
      )
      allow(Discourse).to receive(:plugins_by_name).and_return(
        Discourse.plugins_by_name.merge(
          plugin_name => instance_double(Plugin::Instance, configurable?: true),
        ),
      )
    end
  ...
  it "warns when multiple plugins declare an icon set and uses the first" do
    allow(Rails.logger).to receive(:warn)
    registrations = [
      { map: { "bell" => "ph-regular-bell" }, plugin_name: "first-icons", plugin_dir: "/tmp" },
      { map: { "bell" => "ph-bold-bell" }, plugin_name: "second-icons", plugin_dir: "/tmp" },
    ]

    expect(SvgSprite.build_plugin_icon_set(registrations)["plugin"]).to eq("first-icons")
    expect(Rails.logger).to have_received(:warn).with(/Multiple plugins declare an icon set/)
  end
  ...
  it "aliases the plugin set onto canonical ids and drops its raw glyph ids" do
    ...
    expect(bundle[%r{<symbol id="bell".*?</symbol>}m]).to include("M128 24a8") # the set glyph
  ...
  it "resolves {placeholder} tokens from same-named site settings" do
    ...
    register_plugin_set!(map: { "bell" => "ph-{title}-bell" }, owns: %w[title])

    expect(SvgSprite.bundle(theme.id)[%r{<symbol id="bell".*?</symbol>}m]).to include("M128 16a16")
  end

  it "does not treat SiteSetting class methods as settings" do
    class_method_geometry = "M99CLASSMETHOD"
    source =
      plugin_sprite.merge(
        "ph-SiteSetting-bell" =>
          %(<symbol id="ph-SiteSetting-bell"><path d="#{class_method_geometry}"/></symbol>),
      )
    allow(DiscoursePluginRegistry).to receive(:icon_sets).and_return(
      [
        {
          map: {
            "bell" => "ph-{name}-bell",
            "SiteSetting" => "ph-regular-bell",
          },
          ignore_setting: "name",
          plugin_name: "fake-icons",
          plugin_dir: "/tmp",
        },
      ],
    )
    allow(SvgSprite).to receive(:plugin_svgs_by_plugin).and_return("fake-icons" => source)
    SiteSetting.svg_icon_subset = "SiteSetting"

    bundle = SvgSprite.bundle(theme.id)
    expect(bundle[%r{<symbol id="bell".*?</symbol>}m]).not_to include(class_method_geometry)
    expect(bundle[%r{<symbol id="SiteSetting".*?</symbol>}m]).to include("M128 24a8")
  end

  it "serves a source glyph explicitly registered by the declaring plugin" do
    allow(DiscoursePluginRegistry).to receive(:svg_icons).and_return(Set["ph-regular-bell"])
    register_plugin_set!(map: { "bell" => "ph-regular-bell" })

    bundle = SvgSprite.bundle(theme.id)
    expect(bundle[%r{<symbol id="bell".*?</symbol>}m]).to include("M128 24a8")
    expect(bundle[%r{<symbol id="ph-regular-bell".*?</symbol>}m]).to include("M128 24a8")
    expect(SvgSprite.svgs_for(theme.id)).to have_key("ph-regular-bell")
  end
  ...
  it "reads the map from a JSON file inside the plugin directory only" do
    Dir.mktmpdir do |dir|
      ...
      FileUtils.mkdir("#{dir}/maps")
      directory = { map: "maps", plugin_dir: dir, plugin_name: "fake-icons" }
      expect(SvgSprite.build_plugin_icon_set([directory])).to be_nil
    end
  end
  ...
  it "will not read a setting the registering plugin does not own" do
    # Unscoped, "ph-{title}-bell" would interpolate the site title.
    SiteSetting.title = "bold"
    register_plugin_set!(map: { "bell" => "ph-{title}-bell" }) # title is core's, not ours

    allow(Rails.logger).to receive(:warn)
    bundle = SvgSprite.bundle(theme.id)

    expect(bundle[%r{<symbol id="bell".*?</symbol>}m]).not_to include("M128 16a16") # not bold
    expect(Rails.logger).to have_received(:warn).with(/"title" is not a setting of this plugin/)
  end
end
