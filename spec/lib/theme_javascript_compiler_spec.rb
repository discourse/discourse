...
describe ThemeJavascriptCompiler do
  ...
  describe ThemeJavascriptCompiler::RawTemplatePrecompiler do
    ...
    let(:mini_racer) do
      ...
      ctx.eval(
        File.open("#{Rails.root}/vendor/assets/javascripts/handlebars.js").read
      )
      ctx.eval(helpers)
      ctx
    end
    ...
    it 'adds the theme id to the helpers' do
      ...
      expect(render("{{theme-setting 'setting_key'}}")).to eq(
            'setting(22:setting_key)'
          )
    ...
    it 'works with the old settings syntax' do
      expect(render('{{themeSettings.setting_key}}')).to eq(
            'setting(22:setting_key)'
          )

      # Works when used inside other statements
      expect(render('{{dummy-helper themeSettings.setting_key}}')).to eq(
            'dummy(setting(22:setting_key))'
          )
    end
  ...
  describe ThemeJavascriptCompiler::EmberTemplatePrecompiler do
    ...
    it 'adds the theme id to the helpers' do
      expect(statement("{{theme-prefix 'translation_key'}}")).to eq(
            [[1, [27, 'theme-prefix', [22, 'translation_key'], nil], false]]
          )
      expect(statement("{{theme-i18n 'translation_key'}}")).to eq(
            [[1, [27, 'theme-i18n', [22, 'translation_key'], nil], false]]
          )
      expect(statement("{{theme-setting 'setting_key'}}")).to eq(
            [[1, [27, 'theme-setting', [22, 'setting_key'], nil], false]]
          )

      # Works when used inside other statements
      expect(
        statement("{{dummy-helper (theme-prefix 'translation_key')}}")
      ).to eq(
            [
              [
                1,
                [
                  27,
                  'dummy-helper',
                  [[27, 'theme-prefix', [22, 'translation_key'], nil]],
                  nil
                ],
                false
              ]
    ...
    it 'works with the old settings syntax' do
      expect(statement('{{themeSettings.setting_key}}')).to eq(
            [
              [
                1,
                [
      ...
      # Works when used inside other statements
      expect(statement('{{dummy-helper themeSettings.setting_key}}')).to eq(
            [
              [
                1,
                [
