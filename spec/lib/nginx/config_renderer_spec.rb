...
require "tmpdir"
require_relative "../../nginx/support/config_renderer"
...
RSpec.describe Nginx::Support::ConfigRenderer do
  describe "#render" do
    ...
    it "uses an explicit public path" do
      Dir.mktmpdir do |tmpdir|
        public_path = File.join(tmpdir, "frontend-dist")
        sample_path = File.join(tmpdir, "nginx.sample.conf")
        File.write(sample_path, "root /var/www/discourse/public;\n")
        ...
        expect(File.read(File.join(tmpdir, "discourse.conf"))).to include("root #{public_path};")
        expect(File).to exist(public_path)
      end
    end
  end
