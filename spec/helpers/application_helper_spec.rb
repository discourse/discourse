...
describe ApplicationHelper do
  describe 'preload_script' do
    it 'provides brotli links to brotli cdn' do
      ...
      expect(link).to eq(
            "<link rel='preload' href='https://awesome.com/brotli_asset/application.js' as='script'/>\n<script src='https://awesome.com/brotli_asset/application.js'></script>"
          )
    end
    ...
    context 'with s3 CDN' do
      ...
      after { ActionController::Base.config.relative_url_root = nil }
      ...
      it 'deals correctly with subfolder' do
        ...
        expect(helper.preload_script('application')).to include(
              'https://s3cdn.com/assets/application.js'
            )
      end
      ...
      it 'returns magic brotli mangling for brotli requests' do
        ...
        expect(link).to eq(
              "<link rel='preload' href='https://s3cdn.com/assets/application.br.js' as='script'/>\n<script src='https://s3cdn.com/assets/application.br.js'></script>"
            )
      end
      ...
      it 'gives s3 cdn if asset host is not set' do
        ...
        expect(link).to eq(
              "<link rel='preload' href='https://s3cdn.com/assets/application.js' as='script'/>\n<script src='https://s3cdn.com/assets/application.js'></script>"
            )
      end
  ...
  describe 'escape_unicode' do
    ...
    it 'survives junk text' do
      expect(helper.escape_unicode("hello \xc3\x28 world")).to match(
            /hello.*world/
          )
    end
  end
    ...
    context 'enable_mobile_theme is true' do
      ...
      context 'mobile_view session is cleared' do
        ...
        it 'is false if user agent is not mobile' do
          session[:mobile_view] = '1'
          controller.request.stubs(:user_agent).returns(
  ...
  describe '#html_classes' do
    ...
    describe 'text size' do
      context 'with a user option' do
        before do
          ...
          helper.request.env[
            Auth::DefaultCurrentUserProvider::CURRENT_USER_KEY
          ] =
            user
        end
    ...
    it "includes 'anon' for anonymous users and excludes when logged in" do
      ...
      helper.request.env[Auth::DefaultCurrentUserProvider::CURRENT_USER_KEY] =
        user
  ...
  describe 'gsub_emoji_to_unicode' do
    it 'converts all emoji to unicode' do
      expect(
        helper.gsub_emoji_to_unicode(
          'Boat Talk: my :sailboat: boat: why is it so slow? :snail:'
        )
    ...
    context 'opengraph image' do
      it 'returns the correct image' do
        SiteSetting.opengraph_image =
          Fabricate(:upload, url: '/images/og-image.png')

        SiteSetting.twitter_summary_large_image =
          Fabricate(:upload, url: '/images/twitter.png')

        SiteSetting.large_icon =
          Fabricate(:upload, url: '/images/large_icon.png')
        ...
        expect(helper.crawlable_meta_data(image: 'some-image.png')).to include(
              'some-image.png'
            )
