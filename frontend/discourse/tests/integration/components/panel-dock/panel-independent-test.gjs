...
module("Integration | Component | DDockPanel independent", function (hooks) {
  ...
  test("stored width is restored, clamped, and updated after a committed resize", async function (assert) {
    ...
    assert.deepEqual(
      ...
      { side: "start", width: 240, height: 320 },
...
module("Integration | Component | DDockPanel verification", function (hooks) {
  setupRenderingTest(hooks);
  ...
  test("bottom pointer resizing preserves a width resized on another side", async function (assert) {
    ...
    await click(".d-dock-panel__dock-button.--end");

    let resizer = find(".d-dock-panel__resizer");
    ...
    await click(".d-dock-panel__dock-button.--bottom");

    resizer = find(".d-dock-panel__resizer");
    installPointerCaptureSpy(resizer);
    await triggerEvent(resizer, "pointerdown", {
      button: 0,
      clientX: 100,
  ...
  test("partially valid and wrongly typed stored layouts are ignored", async function (assert) {
    ...
    for (const [index, value] of invalidLayouts.entries()) {
      ...
      await render(
        <template>
          <DDockPanel @isOpen={{true}} @storageKey={{this.storageKey}} />
        </template>
      );
  ...
  test("a stored size is clamped to a viewport-derived maximum in ARIA", async function (assert) {
    ...
    await render(
      <template>
        <DDockPanel @isOpen={{true}} @storageKey="narrow-viewport" />
      </template>
    );
