...
module("Integration | ui-kit | DDateRangePicker", function (hooks) {
  ...
  test("hand-picking a range: click start, click end, Apply commits", async function (assert) {
    ...
    assert
      ...
      .hasClass("--end", "second click sets the pending end with --end class");
  ...
  test("clicking a date earlier than the pending start re-anchors the start", async function (assert) {
    ...
    assert
      .dom(".d-date-range-picker__apply")
      .isDisabled("Apply is disabled because the end is unset after re-anchor");
  });
  ...
  test("Cancel discards pending selection and calls onCancel", async function (assert) {
    ...
    assert.strictEqual(state.applied.length, 0, "no commit happens on Cancel");
  });
...
module(
  "Integration | ui-kit | DDateRangePicker | preset math",
  function (hooks) {
    setupRenderingTest(hooks);
...
module("Unit | ui-kit | DDateRangePicker | formatRange", function () {
