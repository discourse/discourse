...
acceptance(`Cooked quoted content`, function (needs) {
  needs.pretender((server, helper) => {
    server.get("/posts/by_number/280/3", () =>
      helper.response(200, topicFixtures["/t/280/1.json"].post_stream.posts[2])
    );
    server.get("/posts/by_number/280/5", () =>
      helper.response(200, topicFixtures["/t/280/1.json"].post_stream.posts[4])
    );
  });
