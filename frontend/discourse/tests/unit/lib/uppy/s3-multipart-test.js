...
const UPPY_SIGNING_ATTEMPTS = 5;
const LONG_RETRY_AFTER_SECONDS = 120;
...
module("Unit | Lib | uppy/s3-multipart", function (hooks) {
  ...
  test("a rate limited signing stops uppy's own retries from hitting the server", async function (assert) {
    const errorHandler = sinon.spy();
    const { signPart } = awsS3Options(this, errorHandler);

    let requests = 0;
    pretender.post(BATCH_PRESIGN_URL, () => {
      requests++;
      return middlewareRateLimit(LONG_RETRY_AFTER_SECONDS);
    });

    const file = buildFile();
    for (let attempt = 0; attempt < UPPY_SIGNING_ATTEMPTS; attempt++) {
      await signPart(file, { partNumber: PART_NUMBER }).catch(() => {});
    }

    assert.strictEqual(requests, 1, "the limiter is not hit again");
    assert.true(errorHandler.calledOnce, "the error is reported once");
  });
