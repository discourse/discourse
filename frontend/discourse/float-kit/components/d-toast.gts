...
interface DToastSignature {
  Args: {
    ...
    /** All mounted toast instances, including ones currently dismissing. */
    toasts: DToastInstance[];
    onTravelStatusChange?: (status: string) => void;
  };
}

/**
 * Renders a toast as a sheet so its entry, dismissal, and stacked position share
 * the sheet animation lifecycle.
 */
export default class DToast extends Component<DToastSignature> {
  ...
  measureHeight = modifier((element: HTMLElement) => {
    if (!this.isFront) {
      return;
    }
  ...
  @action
  handleProgressComplete(event: AnimationEvent) {
    if (
  ...
  @action
  handleTravelStatusChange(status: string) {
  ...
  @action
  handlePresentedChange(presented: boolean) {
