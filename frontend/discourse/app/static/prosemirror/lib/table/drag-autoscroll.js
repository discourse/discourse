...
function scrollTarget(anchor, axis, point) {
  ...
  for (let element = anchor; element; element = element.parentElement) {
    ...
    // Let the browser clamp the write at either boundary. This works for both
    // positive and negative RTL scrollLeft conventions; the RAF stops when the
    // applied write produces no change.
    if (delta) {
