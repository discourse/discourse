...
export default class KeyboardRouter {
  ...
  handle(event: KeyboardEvent, context: KeyboardContext): void {
    ...
    // not sit inside one — a row's own switch beside the handle that is the
    // cursor's item. A key pressed there belongs to that control. Routing it
    // anyway resolves the cursor to whichever item happens to hold the tab
    // stop and drags focus onto a different element entirely.
    if (
      config.focusStrategy === "roving-tabindex" &&
