...
// This modal displays a deep category tree (categories → subcategories →
// sub-subcategories). The backend returns a flat list in hierarchical order,
// so we only need to split the list into top-level groupings and page the
...
export default class SidebarEditNavigationMenuCategoriesModal extends Component {
  ...
  onFilterInput(filter) {
    ...
    this.fetchedCategories = [];
    this.fetchedCategoriesGroupings = [];
    this.debouncedSendRequest();
  }
