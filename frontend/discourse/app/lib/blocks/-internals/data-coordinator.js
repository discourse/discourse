...
export function blockDataKey(blockName, descriptor) {
  return `block-data:${blockName}:${stableStringify(descriptor)}`;
}

async function resolveData(key, descriptor, dataMeta, owner, signal) {
...
export function getBlockData(options) {
...
export function resetBlockData(scope) {
  if (scope === undefined) {
    cache.clear();
  } else {
    cache.delete(scope);
