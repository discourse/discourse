...
function createListWrapper(tagName, startNumber, isTight) {
...
function isListTight(listElement) {
  // A list is "tight" if its li elements do NOT have <p> as direct children
  // Tight: <li>text</li>
  // Loose: <li><p>text</p></li>
  const listItems = listElement.querySelectorAll(":scope > li");
  for (const li of listItems) {
    // Check if any direct child is a <p> element
...
function setTightAttributeOnAllLists(container) {
...
function splitRunByOriginalListType(run) {
  ...
  for (const li of run) {
    ...
    if (
      ...
      // For OL, check if numbering is continuous
      (listTag === "OL" &&
...
function wrapOrphanListItemsByOriginalType(container) {
  ...
  for (const run of runs) {
    // Check if any items have original list type annotations
    const hasAnnotations = run.some((li) => li.dataset.originalListTag);

    if (hasAnnotations) {
      // Split by original list type and wrap each sub-run separately
      ...
      // No annotations, fall back to wrapping as UL
...
function annotateOrphanListItemsWithOriginalInfo(container, range) {
  ...
  // Get the common ancestor to search for original list items
  const commonAncestor =
    range.commonAncestorContainer.nodeType === Node.ELEMENT_NODE
      ? range.commonAncestorContainer
      : range.commonAncestorContainer.parentElement;

  // Find all list items in the original DOM that are within the selection
  const originalListItems = commonAncestor.querySelectorAll("li");

  // Try to match orphan LIs to their original counterparts by text content
  for (const orphanLi of orphanLis) {
    const orphanText = orphanLi.textContent.trim();

    for (const originalLi of originalListItems) {
      if (originalLi.textContent.trim() === orphanText) {
        const parentList = originalLi.parentElement;
        if (parentList?.tagName === "OL" || parentList?.tagName === "UL") {
          const itemIndex = Array.from(
            parentList.querySelectorAll(":scope > li")
          ).indexOf(originalLi);
          const baseStart = parseInt(
            parentList.getAttribute("start") || "1",
            10
          );

          orphanLi.dataset.originalListTag = parentList.tagName;
          orphanLi.dataset.originalStartNumber = String(baseStart + itemIndex);
...
function preserveListStructureInClonedContent(container, range) {
  ...
  // First, annotate any orphan list items with their original list info
  // This must happen before wrapping so we can track which list each item came from
  ...
  if (originalListItem) {
    ...
    if (isAlreadyWrappedInCorrectList) {
      ...
      // Use the new function that respects original list types
...
export function selectedText() {
  ...
  for (let r = 0; r < selection.rangeCount; r++) {
    ...
    if (codeBlockTest) {
      ...
      setTightAttributeOnAllLists(fragmentContainer);
