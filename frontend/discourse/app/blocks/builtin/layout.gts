...
export default class Layout extends Component<LayoutSignature> {
   ...
   *
   * @param containerArgs - The child entry's container args, carrying its
   *   grid placement under the `grid` namespace.
   * @returns The cell's inline style, or `null` outside grid mode.
   */
  cellStyle = (
   ...
   * `@container d-block-layout` rule override the actual
   * `grid-template-columns` at narrow widths; an inline `style` declaration
   * would otherwise always win over the query rule.
   *
   ...
   * instead of popping.
   */
  get containerStyle(): TrustedHTML {
   ...
   */
  get renderedChildren(): ChildBlockResult[] {
   ...
   *    column at narrow widths (the
   *    `@container d-block-layout (max-width: 40rem)` query in the
   *    stylesheet), every cell gets
   ...
   * agnostic. CSS Grid flips `column: 1` to the right edge in
   * `dir=rtl`, and RTL reading is right-to-left, so the sorted DOM order
   * still matches visual reading order in both directions without a
   * special branch.
   */
  get sortedChildren(): ChildBlockResult[] {
