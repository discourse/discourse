...
/**
 * The current category augmented with the server-provided attributes the
 * banner reads that aren't declared on the base model type.
 */
...
interface CategoryBannerSignature {
  Args: {
    showLogo?: boolean;
    showIcon?: boolean;
...
export default class CategoryBanner extends Component<CategoryBannerSignature> {
  @service declare router: RouterService;

  @tracked category: BannerCategory | null = null;
   ...
   *
   * @returns The slug-with-ID segment, or `undefined` off a category route.
   */
  get categorySlugPathWithID() {
    return this.router?.currentRoute?.params?.category_slug_path_with_id as
      | string
      | undefined;
  }
   ...
   *
   * @returns Whether the banner should render.
   */
  get shouldRender() {
    return (
   ...
   * transitions.
   *
   * @returns Whether the current route is visible to the banner.
   */
  get isVisible() {
   ...
   *
   * @returns Whether the heading icon should render.
   */
   ...
   *
   * @returns The pre-rendered category-name badge HTML.
   */
   ...
   *
   * @returns Whether the category has subcategories.
   */
