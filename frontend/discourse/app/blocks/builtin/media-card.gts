...
import type { RichTextRun } from "discourse/lib/blocks/-internals/minimal-rich-text-renderer";
...
/**
 * An inline rich-text arg value: a plain string or a doc-JSON fragment,
 * matching the `RichTextRenderer` `@value` contract.
 */
type RichInlineValue = string | { content?: RichTextRun[] } | null;

interface MediaCardSignature {
  Args: {
    avatar?: BlockImageArg;
    name?: string;
    role?: RichInlineValue;
    badgeIcon?: string;
    badgeLabel?: RichInlineValue;
    title?: RichInlineValue;
    ctaLabel?: RichInlineValue;
    ctaHref?: string;
...
export default class MediaCard extends Component<MediaCardSignature> {
   ...
   *
   * @returns The inline backdrop style, or `null` when nothing is set.
   */
  get backdropStyle() {
   ...
   *
   * @returns Whether the badge should collapse on the reader page.
   */
 ...
 *
 * @param url - The upload URL to wrap.
 * @returns The `url("...")` CSS value.
 */
