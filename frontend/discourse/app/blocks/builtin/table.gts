...
/** A child placed at a specific cell, with its column / row spans. */
interface PlacedCell {
  r: number;
  c: number;
  rowspan: number;
  colspan: number;
  child: ChildBlockResult;
}

/** A single rendered table cell. */
interface RenderedCell {
  key: string;
  child: ChildBlockResult | null;
  colspan: number;
  rowspan: number;
  isHeader: boolean;
  scope: "col" | "row" | null;
...
export default class Table extends Component<TableSignature> {
   ...
   *
   * @returns The rendered rows, each with its visible cells.
   */
  get tableModel(): RenderedRow[] {
    ...
    const cellMap = new Map<string, PlacedCell>();
    const autoChildren: ChildBlockResult[] = [];

    const occupy = (r: number, c: number, rowspan: number, colspan: number) => {
