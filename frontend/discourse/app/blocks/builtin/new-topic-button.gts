...
import type Composer from "discourse/services/composer";
...
interface NewTopicButtonSignature {
  Args: {
    label?: RichInlineValue;
    categoryId?: number;
    tags?: string[];
    prefillTitle?: string;
    variant?: string;
...
export default class NewTopicButton extends Component<NewTopicButtonSignature> {
  @service declare composer: Composer;

  /**
   * Composes the DButton class list, mixing the block BEM root with the
   * variant-derived core button class.
   *
   * @returns The composed button class list.
   */
  get btnClass() {
   ...
   *
   * @returns The resolved category, or `null`.
   */
